...
mod tests {
	...
	/// collapsing PowerShell references like `Write-Host $env:SKCCODE` to
	/// `:SKCCODE`. The session-level fallback below defines `env=$env` so the
	/// expansion is the literal `$env:SKCCODE`, preserving the PowerShell
