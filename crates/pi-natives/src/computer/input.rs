...
//! in a granted `skc` session, not from a non-TCC-trusted test binary.
...
mod mac {
	...
	//! granted `skc` session; construction is gated on Accessibility.
...
mod live_tests {
	...
	/// `<repo-root>/.skc/ultragoal/artifacts/g005`.
	fn acceptance_artifacts_dir() -> std::path::PathBuf {
		if let Ok(dir) = std::env::var("COMPUTER_USE_ACCEPTANCE_DIR") {
			return std::path::PathBuf::from(dir);
		}
		std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../.skc/ultragoal/artifacts/g005")
	}
	...
	fn all_nine_acceptance_drill() {
		...
		act(InputAction::Type { text: "COMPUTER_USE_E2E sayknow ".to_string() }); // 4 type
