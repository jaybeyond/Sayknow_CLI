...
const ENABLE_ENV: &str = "SKC_NATIVE_CRASH_DIAGNOSTICS";
const DIR_ENV: &str = "SKC_CRASH_DIAGNOSTICS_DIR";
...
fn write_panic_report(info: &std::panic::PanicHookInfo<'_>) {
	let dir = std::env::var_os(DIR_ENV)
		.map_or_else(|| std::env::temp_dir().join("skc-crash-diagnostics"), PathBuf::from);
