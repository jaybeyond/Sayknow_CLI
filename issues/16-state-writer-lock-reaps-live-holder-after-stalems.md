...
- **Scope:** `packages/coding-agent/src/config/file-lock.ts:95-101,145-147` (stale-by-elapsed-time reaping), consumed by `packages/coding-agent/src/skc-runtime/state-writer.ts:439-475`
...
resolved `.skc` target and exposes `withWorkflowStateLock` as a CAS primitive.
