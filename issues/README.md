...
These issues were found by **operating real `skc --mode rpc` through the `skc_rpc`
...
Repro harness: `/tmp/skcdf/harness.py` (Python `skc_rpc` → real bun CLI) plus a
...
- **06** fixed — `skc_rpc` `SessionState.context_usage` (`ContextUsage` model + parse).
- **07** fixed — `skc_rpc` typed `negotiate_unattended`/`handoff`/`login`/`get_login_providers` + models.
...
- **08** fixed — added an env-gated (`SKC_RPC_REAL_BINARY=1`) real-binary integration lane that drives actual `skc --mode rpc` and checks the typed client against the live protocol (`context_usage`, correlated errors, negotiate floor, unknown-scope rejection); skips by default.
- **12** already fixed on dev (`$pickenv("SKC_RPC_EMIT_TITLE","PI_RPC_EMIT_TITLE")`).
...
Plan + consensus artifacts: `.skc/plans/ralplan/2026-06-13-1236-71f5/` (`pending-approval.md`).
...
| [06](06-skcrpc-sessionstate-missing-contextusage.md) | Medium | `python/skc-rpc/src/skc_rpc/protocol.py` | `SessionState` drops `contextUsage`; the typed client gives hosts no access to context pressure. |
| [07](07-skcrpc-missing-unattended-handoff-login-methods.md) | High | `python/skc-rpc/src/skc_rpc/client.py` | No typed methods for `negotiate_unattended`, `handoff`, `login`, `get_login_providers`; the unattended control plane is unreachable from the public client API. |
| [08](08-skcrpc-tested-only-against-fake-server.md) | Medium | `python/skc-rpc/tests/test_client.py` | The client is only tested against a hand-written fake server; real-skc drift (06/07 etc.) goes uncaught. |
| [09](09-rpc-no-persistent-detached-session.md) | High | `packages/coding-agent/src/modes/rpc/rpc-mode.ts` + `python/skc-rpc/src/skc_rpc/client.py` | No persistent/detached session: skc exits on stdin EOF and `skc_rpc` runs a foreground child that dies with the client. No daemon/reattach. |
| [10](10-rpc-no-session-registry.md) | High | `python/skc-rpc/src/skc_rpc/client.py` (new module) | No session registry to enumerate/discover/reattach running RPC sessions. |
...
| [12](12-rpc-emit-title-env-var-mismatch.md) | Low | `packages/coding-agent/src/modes/rpc/rpc-mode.ts` | Code reads `PI_RPC_EMIT_TITLE`; docs document `SKC_RPC_EMIT_TITLE`. The documented variable has no effect. |
