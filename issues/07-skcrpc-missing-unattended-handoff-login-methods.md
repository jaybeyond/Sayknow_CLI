# 07 — `skc_rpc` exposes no typed method for the unattended control plane (and `handoff`/`login`)

- **Severity:** High
- **Scope:** `python/skc-rpc/src/skc_rpc/client.py`
- **Surface:** `skc_rpc` typed client API
...
any of them. A search of `python/skc-rpc/src/skc_rpc` for `negotiate`,
