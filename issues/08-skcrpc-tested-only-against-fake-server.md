# 08 — `skc_rpc` is only tested against a hand-written fake server, never real skc

- **Severity:** Medium (test/coverage gap that hides real bugs)
- **Scope:** `python/skc-rpc/tests/test_client.py` (`FAKE_SERVER`), and the test suite generally
- **Surface:** `skc_rpc` integration fidelity
...
`skc --mode rpc` binary is never exercised by the client tests. The fake server
defines its own `current_state()`, `model_info()`, event framing, etc., so it
only reflects what the test author believed the protocol to be — not what skc
actually emits.
...
Reference harness used for these findings: `/tmp/skcdf/harness.py`.
