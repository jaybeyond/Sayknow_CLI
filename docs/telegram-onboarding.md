...
@BotFather, then re-run `skc notify setup` (or
`skc notify setup --token <botToken> --chat-id <chatId>`) to re-check the
capability. Also confirm there is no Telegram `409 Conflict` poller conflict.
Per-session delivery uses private-chat topics; SKC cannot enable Threaded Mode
