...
local SKC sessions continue to own loopback SDK endpoints, and Slack provides a
...
`skc notify setup slack` is non-interactive. It requires these flags:
...
`skc notify status` masks all token values. It is status output, not a credential
recovery mechanism.
