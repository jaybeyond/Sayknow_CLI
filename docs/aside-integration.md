...
Use `--project` for repo-local evaluation records. Omit it only when the operator intentionally wants the stored definition in the user-level SKC MCP config; normal standalone SKC sessions do not consume either scope as runtime tools today.
...
1. Register the MCP server definition with `skc mcp add ... --project`.
...
4. Do not expect the registration to appear as model tools in a normal standalone SKC session today.
...
| MCP server does not appear in `skc mcp list` | Re-run `skc mcp list --json`; confirm whether the registration was user-scoped or project-scoped. |
| Aside tools do not appear in a normal SKC session | Expected today. `skc mcp` stores redacted definitions for recordkeeping/inspection; normal standalone `skc`, `skc --tmux`, and print-mode sessions do not load those registrations as runtime tools. |
...
| Stored definition points at browser-action tools | Treat the server as browser automation, not search-only. Keep it as recordkeeping only for default SKC workflows unless a separate approved design covers that broader sidecar for runtime use. |

## Decision

Docs-only is the smallest safe outcome for issue #1097. Existing SKC MCP registration can store a user-provided Aside MCP server definition for redacted inspection, and Aside already documents `aside mcp`; no SKC adapter glue is required. The future-safe boundary is to keep Aside external and opt-in, document read/search/context-only use, and require a separate design before SKC claims runtime support for browser actions, login, payment, internal-tool, or private browser-session workflows.
