...
Telegram Remote is a tiny, safe operator surface for Sayknow-CLI (`skc`) session
...
session owner stays SKC/tmux/harness-side; Telegram is only the control button.
...
| Preset-bounded session creation (workdir allowlist + session command) | `SKC_COORDINATOR_MCP_WORKDIR_ROOTS`, `SKC_COORDINATOR_MCP_SESSION_COMMAND` |
| Mutation gating (fail-closed: startup opt-in **and** per-call `allow_mutation`) | `SKC_COORDINATOR_MCP_MUTATIONS` |
| Namespacing so one bot cannot enumerate another's state | `SKC_COORDINATOR_MCP_PROFILE`, `SKC_COORDINATOR_MCP_REPO` |
| Bounded artifact reads (byte-capped, symlink-safe) | `SKC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP` |
...
```
 ...
 Telegram gateway  ── default-deny auth (allowlist) ──▶ reject everyone else
   (one process)   ── preset resolve ────────────────▶ fixed workdir + command + task template
        │          ── Coordinator MCP calls ─────────▶ skc mcp-serve coordinator
        │
...
- **Cross-session enumeration.** `skc_coordinator_list_sessions` answers
  `/sessions` directly. Bridge mode serves exactly one live session per process,
  so it cannot list.
- **Preset-bounded creation.** `skc_coordinator_start_session` is constrained to
  `SKC_COORDINATOR_MCP_WORKDIR_ROOTS` and launches `SKC_COORDINATOR_MCP_SESSION_COMMAND`.
...
The web "steering wheel" remote ([`docs/sayknow-remote.md`](sayknow-remote.md), issue
...
The Coordinator MCP and the SKC/tmux/harness runtime behind it are the sole
...
| --- | --- | --- |
| `workdir` | one entry from `SKC_COORDINATOR_MCP_WORKDIR_ROOTS` | fixed; chat cannot pass a path |
| `sessionCommand` | `SKC_COORDINATOR_MCP_SESSION_COMMAND` | fixed; e.g. `skc --worktree`; chat cannot pass a command |
...
`{ cwd, prompt }` is passed to `skc_coordinator_start_session` with
...
| --- | --- | --- | --- |
| `/start-session <presetId> [task]` | Create a bounded session from an approved preset | `skc_coordinator_start_session` (preset `cwd` + templated `prompt`) | `sessions` |
| `/sessions` | List live/recent sessions with concise status | `skc_coordinator_list_sessions` (+ `skc_coordinator_read_status` for derived status) | none (read) |
| `/observe <sessionId>` | Show one session's bounded public-safe status slice | `skc_coordinator_read_status` / `skc_coordinator_read_coordination_status` | none (read) |
| `/stop <sessionId>` | Request graceful stop/retire for a session | `skc_coordinator_report_status` with `status: "cancelled"` (records terminal turn state) | `reports` |
| `/help` | Show the command set | none | none |

Notes:

- `/observe` deliberately uses **bounded status**, not `skc_coordinator_read_tail`.
...
- **Telegram is the button, SKC owns the session.** The gateway never bypasses
...
| --- | --- | --- |
| Where does it live (in-repo app / example / companion package)? | Companion **gateway** spec here; first reference implementation as an example integration / small service, not a core `skc` mode | In-repo first-class mode → later, behind a decision |
| First session backend (tmux SKC / harness / both)? | **Coordinator MCP** (covers managed + registered visible-tmux sessions) | Harness control-plane adapter behind the same four commands → later |
...
4. **PR 4 — gateway create path.** `/start-session <presetId> [task]` →
   `skc_coordinator_start_session` with preset-bound `cwd`/`prompt` and the
...
- Web "steering wheel" remote (sibling roadmap, harness/bridge-backed): [`docs/sayknow-remote.md`](sayknow-remote.md)
...
**Push notifications reuse the `skc_coordinator_watch_events` event surface.** Follow/Mute state is gateway-owned and bounded; delivery preserves the transmitted-data allowlist and does not introduce a Telegram-side session journal or shadow notification protocol.

## RPC mode (second backend)

The package also supports a second backend, `SKC_TELEGRAM_REMOTE_BACKEND=rpc`,
for a single persistent session. In this mode the gateway dials an existing
owner-only UNIX socket exposed by `skc launch --output rpc`; it never spawns,
...
- `skc-telegram-remote-coordinator.service` runs the default Coordinator MCP backend as one bot service.
- `skc-rpc-session.service` runs the persistent `skc launch --output rpc --listen <socket>` session.
- `skc-telegram-remote-rpc.service` runs the RPC Telegram bot, orders after and wants the session unit, and performs a finite socket wait because `After=` is not readiness.
...
—
*[repo owner's gaebal-sayknow (clawdbot) 🦞]*
