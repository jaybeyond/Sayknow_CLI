...
This guide shows how to wire CodeGraph into SKC through the **custom-tool extension
path** — no core changes, no built-in provider. SKC intentionally keeps third-party
...
> CodeGraph integrates with other agents over MCP, but this guide wires it as a SKC
> custom tool around CodeGraph's local **CLI** — it does not add an MCP server or a
> built-in provider. For how SKC treats MCP servers in standalone sessions, see
...
SKC discovers custom tools from a `tools/` directory in its config dirs:

- **Project-scoped**: `<project>/.skc/tools/`
- **User-scoped (all projects)**: `~/.skc/agent/tools/`
...
the tool needs no imports from SKC internals.

Save the following as `.skc/tools/codegraph.ts` (project) or
`~/.skc/agent/tools/codegraph.ts` (user):

```typescript
/**
 * CodeGraph custom tool for sayknow-cli (SKC).
 *
 ...
 */
import type { CustomToolFactory } from "@sayknow-cli/coding-agent"; // optional: editor types only
...
The `import type` line is optional — it only provides editor types when SKC is
resolvable from your tool file. It is erased at runtime, so the tool loads fine
without it.

## 3. Use it

Start SKC in the project. The `codegraph` tool is now available to the model. Ask
...
- **Read-only with respect to your code.** The tool only runs CodeGraph's
  ...
  CodeGraph's CLI (e.g. `codegraph sync`) outside SKC.
...
- **Fallback.** If the graph reports a symbol is missing, a file is flagged as
  ...
  index outside SKC or fall back to `read`/`search`.
- For background on how SKC treats external tools and MCP servers in standalone
