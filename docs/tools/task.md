...
- Key collaborators:
  ...
  - `packages/coding-agent/src/task/discovery.ts` — resolve bundled SKC agents.
...
6. It resolves the requested agent with `getAgent(...)`, rejects unknown or disabled agents, and enforces parent spawn policy plus `SKC_BLOCKED_AGENT` self-recursion prevention.
...
- Isolation merge strategy
  ...
  - Branch mode — commit each task onto `skc/task/<id>` branch, cherry-pick into parent, preserve failed branches for manual resolution.
- Agent source
  - Project/user agents discovered from configured agent directories.
  - Bundled SKC role agents — the retained canonical four-agent surface from `packages/coding-agent/src/task/agents.ts`.
- Bundled agent types
...
- Subprocesses / native bindings
  ...
  - ProjFS native bindings via `@sayknow-cli/natives` on Windows.
