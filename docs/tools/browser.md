...
| `kill` | `boolean` | No | When a tab release drops a spawned-app browser handle to refcount 0, also terminate its process tree. For Chrome profile mode this kills only a profile browser process launched by SKC; externally-owned profile CDP and `app.cdp_url` are disconnect-only. Has no effect on headless shutdown. |
...
- A matching already-running profile is reused only when its localhost CDP endpoint responds. A matching profile running normally without CDP is refused with remediation text; SKC does not kill or relaunch it.
...
- Cleanup disconnects externally-owned CDP endpoints. `kill: true` terminates only the Chrome profile process that SKC launched for this mode.
...
- Subprocesses / native bindings
  ...
  - `killExistingByPath()` / `gracefulKillTreeOnce()` use `@sayknow-cli/natives` process inspection/termination.
