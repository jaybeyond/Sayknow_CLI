# Building Applications on the Sayknow-CLI SDK

A beginner-friendly guide to using Sayknow-CLI as the **agent runtime for your own
...
Sayknow-CLI packages the runtime as a reusable component:
...
| --- | --- | --- |
| What it is | Import `@sayknow-cli/coding-agent` as a library | Connect to a running session's loopback WS endpoint |
...
This is the part that turns Sayknow-CLI from "a coding agent" into a general
...
```ts
import { SessionManager, Settings } from "@sayknow-cli/coding-agent";
...
```
<repo>/.skc/state/sdk/<sessionId>.json   →  { url, port, token, ... }
```
...
```bash
skc daemon session global --op session.create \
...
resume/fork/handoff between your embedded app and `skc`.
