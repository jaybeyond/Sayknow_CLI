...
For the external control and notification wire protocol, see [the Sayknow-CLI SDK](./sdk.md).

The SDK is the in-process integration surface for `@sayknow-cli/coding-agent`.
...
```bash
bun add @sayknow-cli/coding-agent
```

For process-isolated TypeScript integrations, install `@sayknow-cli/bridge-client` and import `SdkClient` from that standalone transport-only package. `@sayknow-cli/coding-agent/sdk` remains a compatibility re-export with the same `SdkClient` class identity and associated types. Both surfaces use only the v3 SDK transport; no historical BridgeClient backend protocol, handshake/commands/SSE endpoint, or direct host-control path is restored.

## Entry points

`@sayknow-cli/coding-agent/sdk` is the canonical entry point for embedders. The package root exports the same SDK APIs for convenience.
...
```ts
import { createAgentSession } from "@sayknow-cli/coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.skc/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@sayknow-cli/coding-agent";
...
```ts
import { SessionManager } from "@sayknow-cli/coding-agent";
