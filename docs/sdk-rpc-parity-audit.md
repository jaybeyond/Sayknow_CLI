...
`transport-gap — closed by Phase 1` means Phase 1's `skc sdk serve` and typed
`skc_sdk` Python package provide the replacement transport/client surface.
...
1. **stdio JSONL and Unix-socket transports.** Phase 1 (`skc sdk serve` plus the
   typed `skc_sdk` Python package) closes this transport/client gap, while not
...
2. **Reattach/registry — NOT implemented.** Define cross-process registry and
   ...
   `.skc/state/sdk/<sessionId>.json` provide endpoint location and token for a
