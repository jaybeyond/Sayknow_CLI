...
This document covers the runtime loader shipped by `@sayknow-cli/natives`: how `native/index.js` decides which `.node` file to require, how compiled-binary embedded payloads are extracted, and what startup failures report.
...
- **Core directories**:
  ...
  - `userDataDir` fallback:
    - Windows: `%LOCALAPPDATA%/skc` or `%USERPROFILE%/AppData/Local/skc`.
    - Non-Windows: `~/.local/bin`.
- **Natives cache root** (`getNativesDir()`):
  - if `$XDG_DATA_HOME/skc` exists, `$XDG_DATA_HOME/skc/natives`;
  - otherwise `~/.skc/natives`.
- **Compiled-binary mode** (`detectCompiledBinary`): true if any of:
  - embedded-addon manifest is non-null,
  - `SKC_COMPILED` env var is set,
  - `import.meta.url` contains Bun embedded markers (`$bunfs`, `~BUN`, `%7EBUN`).
- **Variant override**: `SKC_NATIVE_VARIANT` (`modern`/`baseline` only; invalid values ignored).
...
1. `SKC_NATIVE_VARIANT=modern|baseline` wins when valid.
...
- reinstall hint (`bun install @sayknow-cli/natives`),
