...
`@sayknow-cli/natives` is now a two-layer package around a loader:
...
There is no current `packages/natives/src` TypeScript wrapper layer. Consumers import functions/classes/enums directly from `@sayknow-cli/natives`; the type contract is the generated `native/index.d.ts` plus enum exports appended by `scripts/gen-enums.ts`.
...
`SKC_NATIVE_VARIANT` can force `modern` or `baseline`; invalid values are ignored.
...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/skc` (fallback `%USERPROFILE%/AppData/Local/skc`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/skc/natives` when `$XDG_DATA_HOME/skc` exists; otherwise it uses `~/.skc/natives`.
...
1. Consumer imports from `@sayknow-cli/natives`.
