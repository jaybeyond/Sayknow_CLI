...
> Selectors, prices, and "axis leaders" are catalog- and time-sensitive (observed 2026-06 on the current bundled catalog). Re-verify any selector with `skc -p --no-session --no-tools --model <selector> "Reply OK"`.
