...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/sayknow-cli` → `@sayknow-cli/coding-agent`
  - `@mariozechner/pi-agent-core` → `@sayknow-cli/agent-core`
  - `@mariozechner/pi-tui` → `@sayknow-cli/tui`
  - `@mariozechner/pi-ai` → `@sayknow-cli/ai`
...
- In `packages/coding-agent`, use `logger` from `@sayknow-cli/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@sayknow-cli/natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@sayknow-cli/natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field       | `pkg.skc` preferred; fallback to `pkg.pi` remains |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@sayknow-cli/natives`
