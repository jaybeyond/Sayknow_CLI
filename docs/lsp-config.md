...
- User-wide preferences → `~/.skc/agent/lsp.json`
- Project-specific overrides → `<project>/.skc/lsp.json`

> **Note:** The presence of any LSP config file disables auto-detection. When at least one file is found, SKC skips the binary-scan phase and loads all servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
