"""Discovery of running skc RPC sessions (issue 10).

Each live `skc --mode rpc` server writes a record under
...
Directory resolution mirrors the TS side: ``SKC_CODING_AGENT_DIR`` if set, else
``~/.skc/agent``, plus ``/rpc-sessions``. Custom XDG layouts can pass
...
def _resolve_sessions_dir(sessions_dir: str | Path | None) -> Path:
    ...
    agent_dir = os.environ.get("SKC_CODING_AGENT_DIR")
    base = Path(agent_dir) if agent_dir else Path.home() / ".skc" / "agent"
...
def list_sessions(sessions_dir: str | Path | None = None) -> tuple[SessionHandle, ...]:
    """List live skc RPC sessions, reaping records whose process is gone (issue 10)."""
