...
from skc_rpc import SessionHandle, list_sessions
...
class RegistryTests(unittest.TestCase):
    ...
    def test_env_resolution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            previous = os.environ.get("SKC_CODING_AGENT_DIR")
            os.environ["SKC_CODING_AGENT_DIR"] = tmp
            try:
                ...
                if previous is None:
                    os.environ.pop("SKC_CODING_AGENT_DIR", None)
                else:
                    os.environ["SKC_CODING_AGENT_DIR"] = previous
