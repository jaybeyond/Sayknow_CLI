...
Typed Python bindings for the `skc --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `skc --mode rpc` flags such as thinking level,
...
```python
from skc_rpc import MessageUpdateEvent, RpcClient
...
from skc_rpc import RpcClient, host_tool
...
```python
from skc_rpc import RpcClient, host_uri
...
```python
from skc_rpc import assistant_text, assistant_text_with_thinking
