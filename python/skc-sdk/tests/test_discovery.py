...
from skc_sdk.discovery import Endpoint, EndpointSelectionError, classify_endpoint, read_session_endpoint, select_live_endpoint


def record(tmp_path: Path, session_id: str, value: dict[str, object]) -> Path:
    path = tmp_path / ".skc" / "state" / "sdk" / f"{session_id}.json"
...
def test_selection_fails_closed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("skc_sdk.discovery.os.kill", fake_kill)
...
def test_refuses_symlinked_discovery_directory(tmp_path: Path) -> None:
    ...
    target.mkdir()
    directory = tmp_path / ".skc" / "state" / "sdk"
