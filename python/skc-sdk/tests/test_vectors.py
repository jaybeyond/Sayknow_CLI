...
from skc_sdk.discovery import read_session_endpoint
from skc_sdk.frames import GenericFrame, Reply, parse_frame
...
def test_every_vector_has_v1_schema_and_executable_shape() -> None:
    ...
    for vector in vectors:
        ...
        if vector["kind"] == "frame":
            ...
            if "lines" in vector:
                lines = vector["lines"]
                assert lines["authSuccess"] == "skc-sdk-transport/1 token=discovery-token-required\n"
...
def test_real_session_enabled_guard() -> None:
    if os.environ.get("SKC_REAL_SESSION_TESTS") != "1":
        return
    assert shutil.which("bun"), "SKC_REAL_SESSION_TESTS=1 requires bun on PATH"
    has_native = NATIVE.exists() and (any(NATIVE.glob("*.node")) or (NATIVE / "embedded-addon.js").exists())
    assert has_native, "SKC_REAL_SESSION_TESTS=1 requires a downloaded or embedded native addon"
