...
from skc_sdk import SdkClient
from skc_sdk.frames import ActionNeeded, ControlResponse, QueryResponse, parse_frame
from skc_sdk.discovery import Endpoint, EndpointSelectionError
...
def test_explicit_session_selection_fails_closed(
    ...
    monkeypatch.setattr("skc_sdk.client.list_session_endpoints", lambda _repo: [endpoint])
    monkeypatch.setattr("skc_sdk.discovery.os.kill", lambda _pid, _signal: (_ for _ in ()).throw(ProcessLookupError()))
    with pytest.raises(EndpointSelectionError, match=code):
        SdkClient._endpoint(Path("repo"), endpoint.session_id)


ROOT = Path(os.environ.get("SKC_REPO_ROOT", Path(__file__).resolve().parents[3])).resolve()
...
REAL_SESSION_ENABLED = os.environ.get("SKC_REAL_SESSION_TESTS") == "1" and BUN is not None and NATIVE.exists()
...
@pytest.mark.asyncio
@pytest.mark.skipif(not REAL_SESSION_ENABLED, reason="requires SKC_REAL_SESSION_TESTS=1, bun, and native addon")
async def test_real_session_all_transports_cleanup() -> None:
    process, metadata = await _start_fixture()
    socket_dir = Path(tempfile.mkdtemp(prefix="skc-sdk-", dir="/tmp"))
