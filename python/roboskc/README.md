...
Self-hosted GitHub triage bot. Drives [`skc --mode rpc`](https://github.com/jaybeyond/sayknow-cli)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBSKC_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same skc session
...
- **roboskc** — FastAPI + sqlite event queue + `WorkerPool` running `skc` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from roboskc, executes REST + `git push`.
...
→ `worker.run_task` spawns `skc --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBSKC_MODEL` (CSV).

The agent uses skc's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.skc/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host skc doesn't route through the gateway). roboskc lives inside the sayknow-cli
monorepo at `python/roboskc/`; both the docker build context and the
`/work/pi` bind mount default to the parent monorepo (`../..`). Override
`PI_ROOT` only if you want a different sayknow-cli checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROBSKC_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROBSKC_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run pi:image                  # build sayknow-cli/pi:dev (one-time / on pi change)
bun run roboskc:build && bun run roboskc:up
...
comment out `ROBSKC_GH_PROXY_URL` / `ROBSKC_GH_PROXY_HMAC_KEY` and set
`GITHUB_TOKEN`. The two modes are mutually exclusive (`config.py`
rejects a `.env` setting both).

Build invalidation is bounded: editing roboskc Python touches only the
runtime layer; editing pi source rebuilds `sayknow-cli/pi:dev`, which
roboskc's `Dockerfile.roboskc` extends via `FROM ${PI_BASE}`.

### Public URL

roboskc does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m roboskc serve`. Other commands run
inside the running container:

```bash
docker compose exec roboskc roboskc triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec roboskc roboskc replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec roboskc roboskc status                   # dump issues table
docker compose exec roboskc roboskc cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run roboskc:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`roboskc:dev`, `roboskc:build`, `roboskc:up`, `roboskc:down`,
`roboskc:logs`, `roboskc:restart`, `roboskc:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBSKC_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `skc --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `skc` on
...
- gh-proxy has no host port. The `roboskc_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBSKC_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBSKC_GIT_AUTHOR_NAME` +
  `ROBSKC_GIT_AUTHOR_EMAIL`.
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBSKC_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBSKC_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/roboskc.log.jsonl`.
...
| Container exits with `PI_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/roboskc/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid sayknow-cli checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROBSKC_BOT_LOGIN` ≠ PAT's account. |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROBSKC_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run pi:image` then `bun run roboskc:build`. |
| `No API key found for <provider>` | `~/.skc/agent/models.container.yml` mount missing or provider id mismatch with `ROBSKC_MODEL`. |
...
src/
  ...
  worker.py          synchronous skc RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one ROBSKC_INTEGRATION=1 smoke test
