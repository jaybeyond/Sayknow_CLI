#!/usr/bin/env bash
# roboskc container entrypoint. No per-boot pip installs — everything is baked
# into the image; we only sanity-check the runtime mount and create state dirs.
#
# Used by both the orchestrator (CMD: `python -m roboskc serve`) and the
# sibling gh-proxy (compose command: `python -m roboskc.proxy serve`). The
# proxy role does NOT need a $PI_ROOT pi checkout — it never runs skc.
set -euo pipefail

# Shared git metadata under /data/workspaces/_pool is intentionally group
# writable by the `skc` group so interrupted work can resume on a different
...
# as $@ here (after tini --), so $1=python, $2=-m, $3=roboskc.proxy is the
# canonical shape; we also accept a single concatenated arg for safety.
is_proxy_role=0
if [ "${1:-}" = "python" ] && [ "${2:-}" = "-m" ] && [[ "${3:-}" == roboskc.proxy* ]]; then
    is_proxy_role=1
elif [[ "${1:-}" == *"roboskc.proxy"* ]]; then
    is_proxy_role=1
fi

/usr/sbin/groupadd -f -g 2000 skc
max_slots="${ROBSKC_MAX_CONCURRENCY:-8}"
for i in $(seq 1 "$max_slots"); do
    user="skc-$i"
    slot_group="skc-$i"
    slot_id=$((2000 + i))
    /usr/sbin/groupadd -f -g "$slot_id" "$slot_group"
    id -u "$user" >/dev/null 2>&1 || /usr/sbin/useradd -u "$slot_id" -g "$slot_group" -G skc -M -N -s /usr/sbin/nologin "$user"
    /usr/sbin/usermod -g "$slot_group" -a -G skc "$user"
done
...
if [ ! -d "$PI_ROOT/packages/coding-agent" ]; then
    echo "roboskc: PI_ROOT=$PI_ROOT does not look like a pi checkout (no packages/coding-agent/)" >&2
    exit 1
fi
...
chown -R root:skc /data/cache /data/workspaces/_pool
...
rm -rf /srv/agent-home/.agent /srv/agent-home/.skc/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.skc/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.skc/agent ]; then
    cp -a /srv/agent-home-stage/.skc/agent/. /srv/agent-home/.skc/agent/
fi
...
touch /data/roboskc.sqlite
chown root:root /data/roboskc.sqlite
chmod 0600 /data/roboskc.sqlite
for db_file in /data/roboskc.sqlite-wal /data/roboskc.sqlite-shm; do
    if [ -e "$db_file" ]; then
