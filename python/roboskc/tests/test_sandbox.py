...
from roboskc.git_ops import GitCommandError
from roboskc.sandbox import (
    SandboxManager,
    Workspace,
    _chown_workspace,
...
def test_rename_workspace_branch_refreshes_shared_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "roboskc.sandbox._share_git_metadata_with_slots",
...
def test_rename_workspace_branch_runs_git_as_slot_when_permissions_active(
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.subprocess.run", fake_run)
    monkeypatch.setattr("roboskc.sandbox._share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)
...
def test_delete_bad_refs_removes_worktree_holding_the_ref(tmp_path: Path) -> None:
    ...
    from roboskc.git_ops import _delete_bad_refs
...
def test_ensure_workspace_creates_worktree(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    ws = mgr.ensure_workspace(
        ...
        default_branch="main",
        author_name="roboskc-bot",
        author_email="roboskc-bot@example.invalid",
    )
...
def test_chown_workspace_noops_when_not_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 1000)
    monkeypatch.setattr(
        "roboskc.sandbox.subprocess.run",
...
def test_chown_workspace_noops_off_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "roboskc.sandbox.subprocess.run",
        lambda cmd, *, check: calls.append((cmd, check)),
...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared skc group.
    assert calls == [
...
def test_reap_slot_noops_when_permissions_inactive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_reap_slot_kills_slot_uid_on_linux_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox._slot_pids", lambda _uid: (111, 222))
    monkeypatch.setattr("roboskc.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))

    tmpdir = _prepare_slot_tmpdir(_workspace(tmp_path), 2001)

    assert tmpdir == tmp_path / ".skc-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".skc-xdg" / "data", tmp_path / ".skc-xdg" / "state", tmp_path / ".skc-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "skc").is_dir()
    assert (tmp_path / ".skc-xdg" / "cache" / "bun-install").is_dir()
...
def test_chown_workspace_normalizes_to_root_when_slots_disabled(
    ...
    monkeypatch.setattr("roboskc.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.os.getegid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.subprocess.run", fake_run)
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    monkeypatch.setattr("roboskc.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboskc.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
    monkeypatch.setattr("roboskc.sandbox.subprocess.run", lambda cmd, **_kwargs: calls.append(cmd))

    ws = _workspace(tmp_path)
    bun_cache = ws.root / ".skc-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".skc-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".skc-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".skc-xdg" / "data", ws.root / ".skc-xdg" / "state", ws.root / ".skc-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "skc").is_dir()
...
def test_ensure_workspace_refreshes_permissions_for_retry_slot_and_session(
    ...
    monkeypatch.setattr("roboskc.sandbox._chown_workspace", record_chown)
    monkeypatch.setattr("roboskc.sandbox._share_git_metadata_with_slots", record_share)
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".skc-tmp",
            ws_root / ".skc-xdg" / "data",
            ws_root / ".skc-xdg" / "data" / "skc",
            ws_root / ".skc-xdg" / "state",
            ws_root / ".skc-xdg" / "state" / "skc",
            ws_root / ".skc-xdg" / "cache",
            ws_root / ".skc-xdg" / "cache" / "skc",
            ws_root / ".skc-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".skc-tmp",
        ws.root / ".skc-xdg" / "data",
        ws.root / ".skc-xdg" / "data" / "skc",
        ws.root / ".skc-xdg" / "state",
        ws.root / ".skc-xdg" / "state" / "skc",
        ws.root / ".skc-xdg" / "cache",
        ws.root / ".skc-xdg" / "cache" / "skc",
        ws.root / ".skc-xdg" / "cache" / "bun-install",
    }
...
def test_redact_credentials_strips_userinfo() -> None:
    from roboskc.sandbox import redact_credentials
...
def test_git_command_error_redacts_url_in_args_and_stderr(tmp_path: Path) -> None:
    ...
    from roboskc.sandbox import _run
...
def test_push_force_with_lease_succeeds_after_local_amend(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    from roboskc.git_ops import push as git_push
...
def test_run_git_injects_safe_directory_and_subprocess_identity(
    ...
    from roboskc.git_ops import _run_git
    ...
    monkeypatch.setattr("roboskc.git_ops.subprocess.run", fake_run)
...
def test_run_git_kills_hung_child(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    timeout."""
    from roboskc.git_ops import GitCommandError, _run_git
...
def test_ensure_workspace_populates_from_natives_cache(tmp_path: Path, upstream_repo: Path) -> None:
    from roboskc.natives_cache import NativesCache, compute_key, target_triple
...
def test_ensure_workspace_cache_miss_is_silent_noop(tmp_path: Path, upstream_repo: Path) -> None:
    from roboskc.natives_cache import NativesCache
