...
from roboskc.config import Settings, reset_settings_cache
from roboskc.dashboard import tail_jsonl
from roboskc.db import Database, close_database, get_database, issue_key
from roboskc.github_client import GitHubClient
from roboskc.manual_triage import InvalidIssueRef, ManualTriageTimeout, await_terminal_state, parse_issue_ref
from roboskc.sandbox import LocalGitTransport
from roboskc.server import create_app
...
def test_index_does_not_expose_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ROBSKC_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "roboskc-bot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "roboskc.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "roboskc.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "roboskc.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "roboskc.queue", "msg": "tool_end", "ok": False},
    ]
...
def test_parse_issue_ref_accepts_github_issue_urls() -> None:
    cases = (
        "https://github.com/jaybeyond/Sayknow_CLI/issues/1348",
        "http://github.com/jaybeyond/Sayknow_CLI/issues/1348",
        "github.com/jaybeyond/Sayknow_CLI/issues/1348",
        "https://www.github.com/jaybeyond/Sayknow_CLI/issues/1348",
        "https://github.com/jaybeyond/Sayknow_CLI/issues/1348/",
        "https://github.com/jaybeyond/Sayknow_CLI/issues/1348?foo=bar",
        "https://github.com/jaybeyond/Sayknow_CLI/issues/1348#issuecomment-99",
        "  https://github.com/jaybeyond/Sayknow_CLI/issues/1348  ",
    )
    for case in cases:
        assert parse_issue_ref(case) == ("jaybeyond/Sayknow_CLI", 1348), case
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("ROBSKC_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_fetches_and_enqueues(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            headers={"X-Roboskc-Replay-Token": token},
        )
    close_database()
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "ROBSKC_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("ROBSKC_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("ROBSKC_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("ROBSKC_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("ROBSKC_RATE_LIMIT_UNLIMITED", "jaybeyond")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def test_webhook_unlimited_allowlist_bypasses_limit(rate_limited_settings: Settings) -> None:
    app = create_app(rate_limited_settings)
    with TestClient(app) as client:
        # NONE association would normally cap at 2, but `jaybeyond` is whitelisted.
        for i in range(5):
            resp = _post_issue_opened(
                client,
                delivery=f"u-{i}",
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("ROBSKC_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_browse_returns_401_with_replay_enabled_without_valid_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        wrong = client.get(
            "/api/github/issues",
            headers={"X-Roboskc-Replay-Token": f"{token}-wrong"},
        )
    close_database()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env) -> None:
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="jaybeyond",
            number=77,
            body="@roboskc-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
    ...
    assert row is not None
    assert row.state == "queued"
    directive = row.payload.get("_roboskc_directive")
    assert directive == {"body": "please refactor X", "author": "jaybeyond", "pragmas": []}
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in ROBSKC_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("ROBSKC_MAINTAINER_LOGINS", "jaybeyond")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="jaybeyond",
                number=300 + i,
                body=("@roboskc-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from roboskc import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    from roboskc import tasks
    from roboskc.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
    ...
    async def _list_comments(self, repo_full: str, number: int):
        assert repo_full == "octo/widget"
        assert number == 900
        return [CommentInfo(id=77, author="jaybeyond", body="prior PR context", created_at="2026-05-14T00:00:00Z")]
    ...
    payload = {
        ...
        "comment": {"user": {"login": "jaybeyond"}, "body": "please fix", "id": 10, "created_at": "2026-05-15T00:00:00Z"},
        "repository": {"full_name": "octo/widget"},
    }
    ...
    assert [(m.kind, m.author, m.body) for m in call["thread"]] == [
        ...
        ("comment", "jaybeyond", "prior PR context"),
    ]
...
async def test_handle_pr_conversation_repairs_missing_pr_mapping_from_branch(
    ...
    async def _list_comments(self, repo_full: str, number: int):
        assert repo_full == "octo/widget"
        assert number == 900
        return [CommentInfo(id=78, author="jaybeyond", body="prior PR context", created_at="2026-05-14T00:00:00Z")]
    ...
    payload = {
        "action": "created",
        "issue": {"number": 900, "pull_request": {"url": "https://api.github.com/repos/octo/widget/pulls/900"}},
        "comment": {"user": {"login": "jaybeyond"}, "body": "please fix", "id": 11, "created_at": "2026-05-15T00:00:00Z"},
        "repository": {"full_name": "octo/widget"},
    }
    await tasks.handle_pr_conversation(
        settings=settings,
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    from roboskc import tasks
    from roboskc.github_client import GitHubClient, IssueInfo, RepoInfo
    ...
    payload = {
        ...
        "comment": {"user": {"login": "jaybeyond"}, "body": "do it", "id": 1, "created_at": "2026-05-14T20:00:00Z"},
        "repository": {"full_name": "octo/widget"},
        "_roboskc_directive": {"body": "please refactor X", "author": "jaybeyond"},
    }
    ...
    assert call["directive"].author == "jaybeyond"
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        "action": "created",
        "issue": {"number": 88, "user": {"login": "alice"}, "title": "boom"},
        "comment": {"user": {"login": "jaybeyond"}, "body": "redo", "id": 2, "created_at": "2026-05-14T21:00:00Z"},
        "repository": {"full_name": "octo/widget"},
        "_roboskc_directive": {"body": "redo the fix", "author": "jaybeyond"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from roboskc import tasks
    from roboskc.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "issue": {"number": 88, "user": {"login": "alice"}, "title": "boom"},
        "comment": {
            "user": {"login": "jaybeyond"},
            "body": "@roboskc do X",
            "id": 10,
            "created_at": "2026-05-03T20:00:00Z",
        },
        "repository": {"full_name": "octo/widget"},
        "_roboskc_directive": {"body": "do X", "author": "jaybeyond"},
    }
...
async def test_triage_issue_skips_when_a_closing_pr_already_exists(
    ...
    from roboskc import tasks
    from roboskc.github_client import IssueInfo, RepoInfo
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from roboskc import tasks
    from roboskc.github_client import GitHubError, IssueInfo, RepoInfo
