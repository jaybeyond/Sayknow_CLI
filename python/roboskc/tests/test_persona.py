...
from roboskc.worker import DirectiveInfo, ThreadMessage
...
class _Comment:
    id: int = 1
    author: str = "jaybeyond"
    body: str = "@roboskc please fix"
...
def test_directive_prompt_embeds_thread_and_directive_body() -> None:
    ...
    out = persona.directive(
        ...
        directive=DirectiveInfo(body="apply fix Y", author="jaybeyond", thread=thread),
        pr_status="PR #1080 is open",
    )
    assert "Directive on octo/widget#1080" in out
    assert "@jaybeyond" in out
...
def test_followup_comment_prompt_embeds_thread_context() -> None:
    thread = (
        ThreadMessage(kind="pr_body", author="roboskc", body="PR body", created_at=""),
        ThreadMessage(kind="comment", author="jaybeyond", body="prior request", created_at="2026-05-01T10:00:00Z"),
    )
...
def test_kickoff_directive_prompt_embeds_thread_and_classify_instruction() -> None:
    ...
    out = persona.kickoff_directive(
        ...
        issue=_Issue(),
        workspace=_Workspace(),
        directive=DirectiveInfo(body="reproduce + fix", author="jaybeyond", thread=thread),
    )
