...
These tests swap `roboskc.worker.RpcClient` for a recording fake so we can
observe the `extra_args` and `set_todos` decisions the driver takes based on
whether the workspace's skc session directory already holds a JSONL transcript.
"""
...
from roboskc import worker
from roboskc.config import Settings
from roboskc.git_ops import DirtyState
...
def _patch_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("roboskc.worker.RpcClient", _FakeRpcClient)
    monkeypatch.setattr("roboskc.worker._AGENT_HOME_STAGE", tmp_path / "missing-agent-home-stage")
    monkeypatch.setattr("roboskc.worker.host_tools.build", lambda _b: ())
    monkeypatch.setattr(
        "roboskc.worker.persona.system_append",
        lambda *, repo, issue, workspace: "SYS",
    )
    monkeypatch.setattr(
        "roboskc.worker.persona.seed_phases",
...
async def test_run_rpc_omits_continue_when_session_empty(
    ...
    assert client_kwargs["env"]["ROBSKC_REPLAY_TOKEN"] == ""
    assert client_kwargs["env"]["ROBSKC_GH_PROXY_HMAC_KEY"] == ""
...
def test_build_extra_env_stages_agent_home(tmp_path: Path, settings: Settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    omp_agent_dir = stage_home / ".skc" / "agent"
    ...
    assert (agent_home / ".skc" / "agent" / "models.yml").is_file()
    ...
    assert (agent_home / ".skc" / "agent").stat().st_mode & 0o777 == 0o755
    assert (agent_home / ".skc" / "agent" / "models.yml").stat().st_mode & 0o777 == 0o644
...
async def test_run_rpc_uses_workspace_xdg_dirs_without_slot(tmp_path: Path, settings: Settings) -> None:
    ...
    xdg_root = inputs.workspace.root / ".skc-xdg"
    for key in ("XDG_DATA_HOME", "XDG_STATE_HOME", "XDG_CACHE_HOME"):
        path = Path(env[key])
        assert path.is_relative_to(xdg_root)
        assert (path / "skc").is_dir()
    tmpdir = inputs.workspace.root / ".skc-tmp"
...
async def test_run_rpc_uses_workspace_xdg_dirs_for_slot_without_chown(
    ...
    monkeypatch.setattr("roboskc.sandbox.os.chown", lambda path, uid, gid: chown_calls.append((Path(path), uid, gid)))
...
async def test_run_rpc_passes_slot_uid_user_slot_group_and_omp_extra_group(tmp_path: Path, settings: Settings) -> None:
    ...
    assert client_kwargs["extra_groups"] == ["skc"]
...
async def test_run_rpc_arms_hard_timeout_timer(
    ...
    monkeypatch.setattr("roboskc.worker.threading.Timer", FakeTimer)
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    monkeypatch.setattr("roboskc.worker.threading.Timer", FiringTimer)
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False)
    loop = asyncio.new_event_loop()
    try:
    ...
    # `_closed_error` unset (skc_rpc bug), so the worker would hang otherwise.
    assert len(fake.mark_closed_calls) == 1
    from skc_rpc import RpcProcessExitError
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    an upstream skc_rpc bug where `stop()` does not set `_closed_error`, leaving
    `_wait_for_agent_end` blocked until timeout."""
    captured: list = []
    monkeypatch.setattr("roboskc.worker.register_cancel_hook", lambda hook: captured.append(hook))
    monkeypatch.setattr("roboskc.worker.unregister_cancel_hook", lambda: None)
...
async def test_run_rpc_sends_reminder_when_pr_class_quits_early(tmp_path: Path, settings: Settings) -> None:
    ...
    try:
        ...
        loop.close()
    fake = _FakeRpcClient.instances[0]
    # kickoff + 2 reminders (default ROBSKC_TASK_COMPLETION_MAX_REMINDERS=2)
