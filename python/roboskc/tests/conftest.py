...
from roboskc.dashboard import reset_index_cache, static_dir
from roboskc.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBSKC_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>roboskc</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="roboskc-config" type="application/json">__ROBSKC_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBSKC_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBSKC_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBSKC_BOT_LOGIN": "roboskc-bot",
        "ROBSKC_GIT_AUTHOR_NAME": "roboskc-bot",
        "ROBSKC_GIT_AUTHOR_EMAIL": "roboskc-bot@example.invalid",
        "ROBSKC_REPO_ALLOWLIST": "octo/widget",
        "ROBSKC_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBSKC_THINKING": "high",
        "ROBSKC_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBSKC_SQLITE_PATH": str(tmp_path / "roboskc.sqlite"),
        "ROBSKC_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBSKC_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBSKC_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBSKC_NATIVES_CACHE_ENABLED": "false",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBSKC_PROVIDER", raising=False)
    monkeypatch.setenv("ROBSKC_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBSKC_GH_PROXY_URL", None)
    baseline.pop("ROBSKC_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBSKC_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBSKC_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBSKC_PROVIDER", raising=False)
    monkeypatch.setenv("ROBSKC_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
