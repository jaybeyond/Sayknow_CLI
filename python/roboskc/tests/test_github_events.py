...
from roboskc.github_events import (
    extract_mention,
    is_maintainer,
    rate_limit_cap,
    route,
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @roboskc-bot please look", "roboskc-bot") == "hey please look"
    assert extract_mention("@roboskc-bot do X", "roboskc-bot") == "do X"


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "roboskc-bot") is None
    assert extract_mention(None, "roboskc-bot") is None
    assert extract_mention("", "roboskc-bot") is None
...
def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @roboskc-bot-helper must NOT match @roboskc-bot.
    assert extract_mention("@roboskc-bot-helper hi", "roboskc-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@roboskc-bot one, then @roboskc-bot two", "roboskc-bot") == "one, then two"
...
def test_route_directive_set_on_issue_comment_when_owner_mentions_bot() -> None:
    ...
    assert decision.directive_author == "jaybeyond"
...
def test_route_directive_set_when_login_in_maintainers_list() -> None:
    decision = route(
        ...
        allowlist=ALLOWLIST,
        bot_login=BOT,
        maintainers=frozenset({"jaybeyond"}),
    )
    assert decision.directive is True
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "jaybeyond"},
                "author_association": "OWNER",
                "body": "@roboskc-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "jaybeyond"},
                "author_association": "OWNER",
                "body": "@roboskc-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "jaybeyond"},
                "author_association": "OWNER",
                "body": "@roboskc-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
