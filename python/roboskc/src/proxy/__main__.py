"""`python -m roboskc.proxy serve` — run the gh-proxy FastAPI app."""
...
from roboskc.config import Settings, load_proxy_settings
...
def _settings_or_die() -> Settings:
    ...
    `ROBSKC_GH_PROXY_HMAC_KEY` — the orchestrator's webhook secret,
...
def serve() -> None:
    ...
    if cfg.gh_proxy_hmac_key is None:
        click.echo("gh-proxy: ROBSKC_GH_PROXY_HMAC_KEY is required in proxy mode", err=True)
        sys.exit(2)
