...
from roboskc import github_events
from roboskc.autoclose import AutocloseScheduler
from roboskc.config import Settings, get_settings
from roboskc.dashboard import render_index, static_dir, tail_jsonl
from roboskc.db import (
...
from roboskc.github_backend import GitHubBackend
from roboskc.github_client import GitHubError, IssueSummary
from roboskc.manual_triage import (
    InvalidIssueRef,
...
from roboskc.proxy_client import GitHubProxyClient, ProxyGitTransport
from roboskc.queue import WorkerPool
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "roboskc orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "roboskc orchestrator requires ROBSKC_GH_PROXY_URL and "
            "ROBSKC_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="roboskc", version="0.1.0", lifespan=lifespan)
    ...
    async def replay(
        request: Request,
        x_roboskc_token: str | None = Header(None, alias="X-Roboskc-Replay-Token"),
        delivery_id: str = "",
    ) -> JSONResponse:
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        ...
        if x_roboskc_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set ROBSKC_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        """Browse issues across `ROBSKC_REPO_ALLOWLIST` for the trigger picker.
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_roboskc_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in ROBSKC_REPO_ALLOWLIST")
            try:
    ...
    async def api_cancel(
        ...
        """Stop a running event. The skc subprocess is killed; the row lands in
    ...
    async def events(
        ...
        _require_dashboard_token(cfg, x_roboskc_token)
    ...
    async def api_logs(
        ...
        entries = tail_jsonl(cfg.log_dir / "roboskc.log.jsonl", limit=capped)
