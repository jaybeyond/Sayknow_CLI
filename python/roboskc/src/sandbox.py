...
PAT injection via `--config-env` (see `roboskc.git_ops`); the `ProxyGitTransport`
in `roboskc.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.skc-session/`, `context/`, `artifacts/`, `.skc-tmp/`, `.skc-xdg`):
   single-owner. Owned by the active slot UID/GID (`skc-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:skc` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:skc` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from roboskc.natives_cache import CacheHit, NativesCache
from roboskc.natives_cache import compute_key as natives_compute_key
...
class GitTransport(Protocol):
    ...
    - `roboskc.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.skc-tmp`` (symlink-protection: a user
...
def _slot_subprocess_kwargs(slot_uid: int | None) -> dict[str, Any]:
    ...
    if not _slot_permissions_active(slot_uid):
        return {}
    assert slot_uid is not None
    return {"user": slot_uid, "group": slot_uid, "extra_groups": [_SHARED_SKC_GID], "umask": 0o002}
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``skc``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".skc-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "skc").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.skc-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.skc-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".skc-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `skc-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `skc` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        session_dir = ws_root / ".skc-session"
        ...
        # keeps its `root:skc` ownership (the slot reads through group `skc`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        ...
        `.node` files are LEFT at `root:skc` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `skc`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:skc cache and the slot reads them via
        group `skc`. Everything else (the directory itself, copied
