"""Host tools exposed to the agent through `skc_rpc.host_tool`.
...
from skc_rpc import HostTool, HostToolContext, RpcCommandError, host_tool

from roboskc import persona
from roboskc.config import Settings
from roboskc.db import Database, issue_key
from roboskc.git_ops import GitCommandError, HeadDriftError
...
class AbortController:
    ...
    teardown of the skc subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching skc. Carries the abort-task signal
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent skc teardown.
