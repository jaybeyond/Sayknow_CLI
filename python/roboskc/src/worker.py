...
from roboskc import host_tools, persona, pragmas
from roboskc.cancellation import register_cancel_hook, unregister_cancel_hook
from roboskc.config import Settings
from roboskc.db import Database, issue_key
from roboskc.git_ops import DirtyState, inspect_dirty_state
from roboskc.github_backend import GitHubBackend
from roboskc.github_client import CommentInfo, IssueInfo, RepoInfo
from roboskc.host_tools import AbortController, ToolBindings, _git_identity_env
from roboskc.natives_cache import NativesCache
from roboskc.natives_cache import compute_key as natives_compute_key
from roboskc.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    # `bash` tool could otherwise `printenv` them out of roboskc's env.
    "GITHUB_TOKEN",
    "GITHUB_WEBHOOK_SECRET",
    "ROBSKC_REPLAY_TOKEN",
    "ROBSKC_GH_PROXY_HMAC_KEY",
)
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".skc/agent")):
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the skc subprocess.

    `skc_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an skc JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    with RpcClient(
        executable=settings.skc_command,
        ...
        extra_groups=["skc"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the skc subprocess
        ...
        # it's idempotent.
        #
        # NOTE: skc_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("skc task exceeded hard timeout")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
