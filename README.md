...
> **Not on npm yet** — install from source. `bun install -g sayknow-cli` will 404.
> Sayknow-CLI is a Bun monorepo, so the global `skc` command is linked from your
> local checkout. **Run every command below from inside the cloned folder.**
>
> **Prerequisites:** [Bun](https://bun.sh) **and** a [Rust toolchain](https://rustup.rs)
> (`cargo`) — the native addon (`@sayknow-cli/natives`) is compiled from source on
> your machine during install.

```sh
# 1. Install Bun and Rust if you don't have them, then restart your terminal
curl -fsSL https://bun.sh/install | bash         # Bun  (macOS / Linux)
curl -fsSL https://sh.rustup.rs | sh             # Rust (cargo)

# 2. Clone and enter the folder
git clone https://github.com/jaybeyond/Sayknow_CLI.git
cd Sayknow_CLI                                    # ← run everything below from here

# 3. Install deps, build the native addon (~1 min Rust compile), link `skc`
bun install
bun run build:native                              # compiles @sayknow-cli/natives
bun run install:dev                               # links skc + default skills

# 4. Verify (open a NEW terminal first so PATH refreshes)
skc --version
skc --smoke-test
```

`build:native` compiles the Rust addon `@sayknow-cli/natives` (needs `cargo`);
`install:dev` then links `skc` into `~/.local/bin` and installs default skills.
After that, `skc` runs this checkout's source — `git pull` to update.

**`skc: command not found`?** The link lives in `~/.local/bin`, which must be on
your `PATH`. Add it (and reload your shell):
...
Don't want to link globally? Just run it from the repo with `bun run dev`.
