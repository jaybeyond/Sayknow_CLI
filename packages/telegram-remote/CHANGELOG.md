...
- Rich messaging (optional, default on via `SKC_TELEGRAM_REMOTE_ENABLE_RICH`): HTML formatting and
  ...
  - New transport reply contract (`OutgoingReply` over `IncomingUpdate`), optional
    ...
    (`SKC_TELEGRAM_REMOTE_ENABLE_RICH`, `_RICH_CALLBACK_TTL_MS`, `_RICH_CALLBACK_MAX_TOKENS`,
  ...
  - Push notifications are deferred: rich UI does not proactively notify; future push must reuse the
    existing `skc_coordinator_watch_events` event surface, not a Telegram-side poller.

- Initial v0 Telegram Remote gateway (`@sayknow-cli/telegram-remote`) for issue #681: a tiny,
  ...
  - MCP stdio coordinator client, Bot API long-poll transport, env config loader, and a
    runnable service entry point (`skc-telegram-remote`).
