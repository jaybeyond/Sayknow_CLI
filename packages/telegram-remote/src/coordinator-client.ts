/**
 * MCP stdio client for the Coordinator MCP. Spawns `skc mcp-serve coordinator`
...
export class McpStdioCoordinatorClient implements CoordinatorClient {
	...
	async getCoordinationStatus(): Promise<CoordinationStatus> {
		try {
			const payload = await this.callTool("skc_coordinator_read_coordination_status", {});
			if (payload.ok === false) {
	...
	async startSession(input: { cwd: string; prompt?: string }): Promise<StartSessionResult> {
		try {
			const payload = await this.callTool("skc_coordinator_start_session", {
				cwd: input.cwd,
	...
	async reportStatus(input: {
		...
		try {
			const payload = await this.callTool("skc_coordinator_report_status", {
	...
	async watchEvents(input: WatchEventsInput): Promise<WatchEventsResult> {
		...
		try {
			const payload = await this.callTool("skc_coordinator_watch_events", args);
			if (payload.ok === false) {
				return {
					ok: false,
	...
	private async start(): Promise<void> {
		...
		await this.request("initialize", {
			...
			capabilities: {},
			clientInfo: { name: "skc-telegram-remote", version: "0.0.1" },
		});
	}
