...
describe("parseCommand", () => {
	test("parses the command vocabulary", () => {
		...
		expect(parseCommand("/attach /tmp/skc.sock")).toEqual({ kind: "attach", socketPath: "/tmp/skc.sock" });
	...
	test("strips @botname mention and is case-insensitive", () => {
		expect(parseCommand("/Sessions@MySkcBot")).toEqual({ kind: "sessions", query: null });
		expect(parseCommand("/Presets@MySkcBot")).toEqual({ kind: "presets" });
		expect(parseCommand("/STOP sess-1 CONFIRM")).toEqual({ kind: "stop", sessionId: "sess-1", confirm: true });
	});
	...
	test("/start parses to onboarding, ignoring any payload and @mention", () => {
		...
		expect(parseCommand("/start@MySkcBot")).toEqual({ kind: "start" });
