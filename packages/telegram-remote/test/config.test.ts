...
function baseEnv(extra: Record<string, string | undefined> = {}): Record<string, string | undefined> {
	return {
		SKC_TELEGRAM_REMOTE_BOT_TOKEN: "123:abc",
		SKC_TELEGRAM_REMOTE_ALLOWED_USER_IDS: "100, 200",
		SKC_TELEGRAM_REMOTE_PRESETS: presetsJson,
		...extra,
	};
}
...
describe("loadConfigFromEnv", () => {
	test("loads a minimal, valid config", () => {
		...
		expect(config.coordinator.command).toBe("skc");
	...
	test("parses RPC backend knobs", () => {
		const config = loadConfigFromEnv(
			baseEnv({
				SKC_TELEGRAM_REMOTE_BACKEND: "rpc",
				SKC_TELEGRAM_REMOTE_RPC_SOCKET: "/tmp/skc-rpc.sock",
				SKC_TELEGRAM_REMOTE_STATE_DIR: "/tmp/telegram-remote-rpc",
				SKC_TELEGRAM_REMOTE_LIVENESS_MS: "120000",
				SKC_TELEGRAM_REMOTE_ALLOW_ATTACH_SOCKET_ARG: "true",
			}),
		);
		expect(config.backend).toBe("rpc");
		expect(config.rpc).toEqual({
			socketPath: "/tmp/skc-rpc.sock",
	...
	test("RPC backend requires socket and state dir with safe defaults", () => {
		expect(() => loadConfigFromEnv(baseEnv({ SKC_TELEGRAM_REMOTE_BACKEND: "rpc" }))).toThrow(/RPC_SOCKET/);
		expect(() =>
			loadConfigFromEnv(
				baseEnv({ SKC_TELEGRAM_REMOTE_BACKEND: "rpc", SKC_TELEGRAM_REMOTE_RPC_SOCKET: "/tmp/skc.sock" }),
			),
		).toThrow(/STATE_DIR/);
		const config = loadConfigFromEnv(
			baseEnv({
				SKC_TELEGRAM_REMOTE_BACKEND: "rpc",
				SKC_TELEGRAM_REMOTE_RPC_SOCKET: "/tmp/skc.sock",
				SKC_TELEGRAM_REMOTE_STATE_DIR: "/tmp/gtr",
			}),
		);
	...
	test("requires a bot token", () => {
		const env = baseEnv();
		delete env.SKC_TELEGRAM_REMOTE_BOT_TOKEN;
		expect(() => loadConfigFromEnv(env)).toThrow(/missing_env:SKC_TELEGRAM_REMOTE_BOT_TOKEN/);
	});

	test("default-denies when no allowlist is configured", () => {
		const env = baseEnv();
		delete env.SKC_TELEGRAM_REMOTE_ALLOWED_USER_IDS;
	...
	test("forces the smallest mutation set and never enables questions", () => {
		...
		expect(readOnly.coordinator.env.SKC_COORDINATOR_MCP_MUTATIONS).toBe("sessions");
		const withStop = loadConfigFromEnv(baseEnv({ SKC_TELEGRAM_REMOTE_ENABLE_STOP: "true" }));
		expect(withStop.coordinator.env.SKC_COORDINATOR_MCP_MUTATIONS).toBe("sessions,reports");
		expect(withStop.coordinator.env.SKC_COORDINATOR_MCP_MUTATIONS).not.toContain("questions");
	});

	test("derives workdir roots and session command from presets", () => {
		const config = loadConfigFromEnv(baseEnv());
		expect(config.coordinator.env.SKC_COORDINATOR_MCP_WORKDIR_ROOTS).toBe("/home/bot/src/project");
		expect(config.coordinator.env.SKC_COORDINATOR_MCP_SESSION_COMMAND).toBe("skc --worktree");
	});

	test("preset name is optional", () => {
		const config = loadConfigFromEnv(
			baseEnv({
				SKC_TELEGRAM_REMOTE_PRESETS: JSON.stringify([
					{ id: "plain", workdir: "/home/bot/src/project", sessionCommand: "skc --worktree" },
				]),
			}),
		);
	...
	test("rejects ambiguous session commands without an explicit override", () => {
		const env = baseEnv({
			SKC_TELEGRAM_REMOTE_PRESETS: JSON.stringify([
				{ id: "a", workdir: "/home/bot/a", sessionCommand: "skc --worktree" },
				{ id: "b", workdir: "/home/bot/b", sessionCommand: "skc --tmux" },
			]),
		});
	...
	test("rejects a preset workdir outside explicit roots", () => {
		const env = baseEnv({ SKC_COORDINATOR_MCP_WORKDIR_ROOTS: "/home/bot/other" });
		expect(() => loadConfigFromEnv(env)).toThrow(/workdir_outside_roots/);
	});

	test("rejects malformed preset JSON", () => {
		expect(() => loadConfigFromEnv(baseEnv({ SKC_TELEGRAM_REMOTE_PRESETS: "{not json" }))).toThrow(
	...
	test("rich UI knobs have safe defaults and never enable questions", () => {
		...
		expect(config.coordinator.env.SKC_COORDINATOR_MCP_MUTATIONS).not.toContain("questions");
	});
	...
	test("rich UI can be disabled and tuned via env", () => {
		const config = loadConfigFromEnv(
			baseEnv({
				SKC_TELEGRAM_REMOTE_ENABLE_RICH: "false",
				SKC_TELEGRAM_REMOTE_RICH_CALLBACK_TTL_MS: "120000",
				SKC_TELEGRAM_REMOTE_RICH_CALLBACK_MAX_TOKENS: "50",
				SKC_TELEGRAM_REMOTE_ENABLE_EDIT_MESSAGE_TEXT: "true",
				SKC_TELEGRAM_REMOTE_REGISTER_COMMANDS: "false",
			}),
		);
	...
	test("push subscription knobs can be tuned via env", () => {
		const config = loadConfigFromEnv(
			baseEnv({
				SKC_TELEGRAM_REMOTE_STATE_DIR: "/tmp/telegram-remote-state",
				SKC_TELEGRAM_REMOTE_FOLLOW_TTL_MS: "120000",
				SKC_TELEGRAM_REMOTE_ENABLE_PUSH: "true",
				SKC_TELEGRAM_REMOTE_SUBSCRIPTIONS_MAX: "25",
			}),
		);
	...
	test("rejects invalid state directories", () => {
		for (const stateDir of ["relative/state", "/tmp/../state", "/tmp/state\0bad"]) {
			expect(() => loadConfigFromEnv(baseEnv({ SKC_TELEGRAM_REMOTE_STATE_DIR: stateDir }))).toThrow(
