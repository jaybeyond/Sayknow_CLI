...
function serviceConfig(overrides: Partial<ServiceConfig["policy"]> = {}): ServiceConfig {
	return {
		...
		backend: "coordinator",
		coordinator: { command: "skc", args: ["mcp-serve", "coordinator"], env: {} },
	};
}
...
test("RPC mode does not instantiate or browse the coordinator", async () => {
	...
	const rpcBackend = new FakeRpcBackend("/tmp/skc.sock");
	...
	await runService(
		{
			...
			backend: "rpc",
			stateDir,
			rpc: { socketPath: "/tmp/skc.sock", stateDir, livenessMs: 60_000, allowAttachSocketArg: false },
		},
