...
const KEYS = ["SKC_CONFIG_DIR", "PI_CONFIG_DIR"] as const;
...
describe("config directory name containment", () => {
	...
	it("honors an ordinary configured name", () => {
		setOnly("SKC_CONFIG_DIR", ".skc-alt");
		expect(getConfigDirName()).toBe(".skc-alt");
		expect(userAgentDirUnderHome()).toBe(path.join(os.homedir(), ".skc-alt", "agent"));
	});
	...
	it("keeps an absolute-looking name beneath home, as documented", () => {
		setOnly("SKC_CONFIG_DIR", "/etc/skc");
