...
const CFG: CliConfig = { bin: "skc", version: "1.0.0", commands: new Map() };

// A command with a constrained positional action and a constrained flag, mirroring
// the real `plugin` command that surfaced the crash (`skc plugin help`).
...
// Mirrors `skc harness`: a single required positional "verb" and no required
...
async function runCapturing(
	...
	try {
		...
		await run({ bin: "skc", version: "1.0.0", argv, commands });
...
describe("cli parse — CliParseError for invalid input", () => {
	it("throws CliParseError for a positional action outside its options", async () => {
		const cmd = new Demo(["help"], CFG); // e.g. `skc plugin help`
	...
	it("throws CliParseError when a required positional argument is missing (the `skc harness` no-verb case)", async () => {
...
describe("cli run — usage instead of uncaught crash", () => {
	...
	it("renders usage + exits 2 (no throw) when a required argument is missing (mirrors `skc harness`)", async () => {
		sideEffect.ran = false;
