...
function tempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-trust-iso-"));
	tempDirs.push(dir);
	return dir;
}

function projectDir(dotenv?: string): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-shell-prefix-trust-"));
	tempDirs.push(dir);
...
async function resolvePrefixIn(cwd: string, overrides: Record<string, string> = {}): Promise<string | null> {
	...
	// the agent `.env`, the SKC config `.env`, `~/.env` and the login shell rc
	...
	env.HOME = tempDir();
	env.SKC_CODING_AGENT_DIR = tempDir();
