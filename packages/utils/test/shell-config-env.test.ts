...
/**
 * `docs/environment-variables.md` documents `SKC_BASH_NO_CI` / `SKC_BASH_NO_LOGIN`,
 ...
 * do NOT enable), SKC-first precedence (an explicit falsey SKC beats a truthy
...
describe("spawn env CI suppression (SKC_BASH_NO_CI)", () => {
	it("injects CI=true when no knob is set", () => {
		expect(getShellConfig().env.CI).toBe("true");
	});

	it("honors the documented SKC_BASH_NO_CI", () => {
		expect(configWith({ SKC_BASH_NO_CI: "1" }).env.CI).toBeUndefined();
	});
	...
	it.each(["0", "false", "off", "no"])("treats SKC_BASH_NO_CI=%s as not set", value => {
		expect(configWith({ SKC_BASH_NO_CI: value }).env.CI).toBe("true");
	});

	it("resolves SKC-first: an explicit SKC_BASH_NO_CI=0 beats a truthy legacy alias", () => {
		expect(configWith({ SKC_BASH_NO_CI: "0", PI_BASH_NO_CI: "1" }).env.CI).toBe("true");
	});

	it("skips a blank SKC_BASH_NO_CI and falls through to the legacy alias", () => {
		expect(configWith({ SKC_BASH_NO_CI: "   ", PI_BASH_NO_CI: "1" }).env.CI).toBeUndefined();
	});
});

describe("login shell args (SKC_BASH_NO_LOGIN)", () => {
	it("uses a login shell when no knob is set", () => {
		expect(getShellConfig().args).toEqual(["-l", "-c"]);
	});

	it("honors the documented SKC_BASH_NO_LOGIN", () => {
		expect(configWith({ SKC_BASH_NO_LOGIN: "1" }).args).toEqual(["-c"]);
	});
	...
	it.each(["0", "false", "off", "no"])("treats SKC_BASH_NO_LOGIN=%s as not set", value => {
		expect(configWith({ SKC_BASH_NO_LOGIN: value }).args).toEqual(["-l", "-c"]);
	});

	it("resolves SKC-first: an explicit SKC_BASH_NO_LOGIN=0 beats a truthy legacy alias", () => {
		expect(configWith({ SKC_BASH_NO_LOGIN: "0", PI_BASH_NO_LOGIN: "1" }).args).toEqual(["-l", "-c"]);
	});

	it("accepts case-insensitive truthy spellings", () => {
		expect(configWith({ SKC_BASH_NO_LOGIN: "true" }).args).toEqual(["-c"]);
		expect(configWith({ SKC_BASH_NO_LOGIN: "YES" }).args).toEqual(["-c"]);
		expect(configWith({ SKC_BASH_NO_LOGIN: "on" }).args).toEqual(["-c"]);
	});
});
