/**
 * Regression for the Windows `bun install -g` update path: when a `skc`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@sayknow-cli/natives/native/pi_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.skc/natives/<version>/`) so the
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@sayknow-cli\\pi-natives\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\sayknow-cli\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@sayknow-cli/natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.skc\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\skc";
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.skc/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
