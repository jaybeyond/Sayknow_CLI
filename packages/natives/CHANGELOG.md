...
- Refreshed native package metadata and version sentinel for the SKC 0.2.2 release.

## [0.2.1] - 2026-05-30

### Changed

- Refreshed native package metadata and version sentinel for the SKC 0.2.1 release.

## [0.2.0] - 2026-05-28

### Changed

- Refreshed native package metadata and version sentinel for the SKC 0.2.0 release.
...
- Added `applyBashFixups(command)` — a synchronous brush-parser-driven rewrite that strips trailing `| head|tail …`, redundant `2>&1`, and the `|&` shorthand from top-level pipelines, returning `{ command, stripped }`. Replaces the hand-rolled top-level mask scanner in `sayknow-cli`; tokenization, quoting, heredocs, command substitution, and nested compound commands are now handled by the real shell AST instead of regex/character-walking. Lives in `pi_shell::fixup` on the Rust side.

### Fixed

- Fixed `<sym> is not a function` crashes on Windows after `bun install -g @sayknow-cli/coding-agent` updates while a `skc` process was running. Bun cannot overwrite a locked `node_modules/@sayknow-cli/natives/native/pi_natives.win32-x64.node` and silently keeps the old binary alongside the new ESM wrapper, so the next launch loads mismatched code. The loader now mirrors the addon into `~/.skc/natives/<version>/` on Windows npm installs and prefers that copy at load time — each version gets its own filesystem path, so future updates land in `node_modules` unchallenged. The new version sentinel detects any remaining drift up front.

### Fixed

- Fixed `$env:NAME` PowerShell references being collapsed to `:NAME` when brush forwarded a command to a PowerShell (or any) subprocess. `pi-shell` now defines `env=$env` as a non-exported global on every brush session so the bash parameter expansion of `$env` yields the literal `$env`, leaving `$env:NAME` intact. User-driven assignments (`env=prod`) push their own command-scope binding and shadow the fallback, preserving the bash POSIX contract. ([#1079](https://github.com/jaybeyond/Sayknow_CLI/issues/1079))
...
- Added Dart support to `astGrep` and `astEdit` through the native tree-sitter Dart grammar ([#748](https://github.com/jaybeyond/Sayknow_CLI/pull/748) by [@0fflineuser](https://github.com/0fflineuser))
...
- Removed `getIndentation`, `getDefaultTabWidth`, and `setDefaultTabWidth` (moved to `@sayknow-cli/utils`)
...
- Updated native module loader to check `XDG_DATA_HOME` environment variable for native addon location before falling back to `~/.skc/natives`
...
- Removed inline pi-utils helpers and dependency on `@sayknow-cli/utils` from native module loader
...
- Added automatic extraction of embedded native addon to `~/.skc/natives/<version>` on first run for compiled binaries
...
- Embedded native addon payload for compiled binaries and extract to `~/.skc/natives/<version>` on first run
...
- Added `SKC_DEV` environment variable support for loading and debugging development native builds
