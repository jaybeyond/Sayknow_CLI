...
- Refreshed stats package metadata for the SKC 0.4.0 release.

## [0.2.2] - 2026-05-31

### Changed

- Refreshed stats package metadata for the SKC 0.2.2 release.

## [0.2.1] - 2026-05-30

### Changed

- Refreshed stats package metadata for the SKC 0.2.1 release.

## [0.2.0] - 2026-05-28

### Changed

- Refreshed stats package metadata for the SKC 0.2.0 release.
...
- Fixed `skc stats` crashing on first session sync in published `skc-{linux,darwin,windows}-*` binaries with `BuildMessage: ModuleNotFound resolving "./packages/stats/src/sync-worker.ts"`; the release build script now lists the stats sync, browser tab, and JS eval workers as explicit `--compile` entrypoints so Bun emits them into bunfs, matching the dev build script and the AGENTS.md worker spawn contract. ([#1150](https://github.com/jaybeyond/Sayknow_CLI/issues/1150))
...
- Fixed `skc stats` in compiled binaries by using the serial sync path instead of spawning a raw file-asset worker that cannot import bundled parser code.
...
- Fixed `skc stats` failing to build the client on globally-installed installs by promoting `tailwindcss` from `devDependencies` to `dependencies` (the client build runs at runtime)
...
- Include subtask session files in usage stats ([#250](https://github.com/jaybeyond/Sayknow_CLI/issues/250))