// MUST be first: pins terminal-capability env before @sayknow-cli/tui evaluates.
import "./render-goldens-env";
import { describe, expect, it } from "bun:test";
import { type Component, Editor, type MouseEvent, Text, TUI } from "@sayknow-cli/tui";
import type { Terminal, TerminalAppearance } from "@sayknow-cli/tui/terminal";
...
describe("keyboard input priority scheduler red-team", () => {
	...
	it("re-anchors the macOS IME cursor after transcript repaint when the prompt draft is unchanged", async () => {
		const previousIme = Bun.env.SKC_TUI_IME_CURSOR;
		Bun.env.SKC_TUI_IME_CURSOR = "1";
		...
		try {
			...
			tui.stop();
			if (previousIme === undefined) delete Bun.env.SKC_TUI_IME_CURSOR;
			else Bun.env.SKC_TUI_IME_CURSOR = previousIme;
		}
	}, 30000);
