 ...
 *
 * `@sayknow-cli/tui/terminal-capabilities` computes `TERMINAL`/`TERMINAL_ID`
 * at module-import time from env vars (TERM_PROGRAM etc.), so the baseline
 * MUST be applied before any `@sayknow-cli/tui` import is evaluated. Import
