...
import { type Component, CURSOR_MARKER, TUI } from "@sayknow-cli/tui";
...
describe("TUI overlays", () => {
	...
	let previousSkcTmuxLaunched: string | undefined;
	...
	beforeEach(() => {
		...
		previousSkcTmuxLaunched = Bun.env.SKC_TMUX_LAUNCHED;
		previousTerm = Bun.env.TERM;
		previousImeCursor = Bun.env.SKC_TUI_IME_CURSOR;
		delete Bun.env.TMUX;
		delete Bun.env.STY;
		...
		delete Bun.env.SKC_TUI_IME_CURSOR;
	...
	afterEach(() => {
		...
		if (previousSkcTmuxLaunched === undefined) {
			delete Bun.env.SKC_TMUX_LAUNCHED;
		} else {
			Bun.env.SKC_TMUX_LAUNCHED = previousSkcTmuxLaunched;
		}
		...
		if (previousImeCursor === undefined) {
			...
			Bun.env.SKC_TUI_IME_CURSOR = previousImeCursor;
		}
	});
