...
import { $which, logger, Snowflake } from "@sayknow-cli/utils";
import { $ } from "bun";
...
$r = Mci "open new type waveaudio alias skc_rec"
if ($r -ne 0) { exit 1 }

Mci "set skc_rec channels 1 samplespersec 16000 bitspersample 16"

$r = Mci "record skc_rec"
if ($r -ne 0) {
    Mci "close skc_rec"
    exit 1
}
...
async function startPowerShellRecording(outputPath: string): Promise<RecordingHandle> {
	...
	const scriptPath = path.join(os.tmpdir(), `skc-stt-record-${Snowflake.next()}.ps1`);
