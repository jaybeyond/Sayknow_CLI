...
import { type AuthStorage, getBundledModels } from "@sayknow-cli/ai";
import { decodeJwt } from "@sayknow-cli/ai/utils/oauth/openai-codex";
import { $env, readSseJson } from "@sayknow-cli/utils";
...
function getConfiguredModel(): string | undefined {
	const configuredModel = ($env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL ?? $env.PI_CODEX_WEB_SEARCH_MODEL)?.trim();
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (!auth) {
		throw new Error(
			"No Codex OAuth credentials found. Login with 'skc /login openai-codex' to enable Codex web search.",
		);
	}
