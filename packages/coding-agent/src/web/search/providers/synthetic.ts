...
import { type AuthStorage, getEnvApiKey } from "@sayknow-cli/ai";
...
export async function searchSynthetic(params: SearchParams): Promise<SearchResponse> {
	...
	if (!apiKey) {
		throw new Error("Synthetic credentials not found. Set SYNTHETIC_API_KEY or login with 'skc /login synthetic'.");
	}
