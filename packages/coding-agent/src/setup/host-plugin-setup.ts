/**
 * Host plugin setup for `skc setup claude` and `skc setup codex`.
 *
...
export function buildHostPluginSetup(host: HostPluginKind, flags: HostPluginSetupFlags = {}): HostPluginSetupResult {
	...
	const pluginDir = path.join(marketplaceRoot, "sayknow-cli");
	...
	const env: Record<string, string> = {
		SKC_COORDINATOR_MCP_WORKDIR_ROOTS: projectRoot,
	...
	if (host === "claude") {
		...
		return {
			...
			installGuidance: [
				...
				"Install the plugin: /plugin install sayknow-cli",
				"Then call skc_delegate_plan / skc_delegate_execute / skc_delegate_team from Claude Code.",
			],
			coordinatorConfigPreview: { command: "skc", args: ["mcp-serve", "coordinator"], env },
			mutationPolicy:
				"Fail-closed: delegation is read-only until you set SKC_COORDINATOR_MCP_MUTATIONS=sessions and pass allow_mutation:true per call.",
			notes: [],
			...(flags.check
	...
	return {
		...
		installGuidance: [
			...
			"Install the plugin: codex plugin add sayknow-cli@sayknow-cli-local",
			"Then call skc_delegate_plan / skc_delegate_execute / skc_delegate_team from Codex.",
		],
		coordinatorConfigPreview: { command: "skc", args: ["mcp-serve", "coordinator"], env },
		mutationPolicy:
			"Fail-closed: delegation is read-only until you set SKC_COORDINATOR_MCP_MUTATIONS=sessions and pass allow_mutation:true per call.",
		notes: [
			"Verified on Codex CLI 0.139.0: marketplace add + plugin add install the plugin (enabled) and `codex mcp list` registers skc-coordinator with the fail-closed env.",
			"The bundled .codex.mcp.json workdir root is host-neutral; `skc setup codex` renders a concrete root, and operators should re-run the local marketplace smoke on their target Codex version.",
		],
		...(flags.check
			? {
					check: verifyBundleFiles([
						...
						path.join(pluginDir, "skills", "skc-delegation", "SKILL.md"),
					]),
				}
			: {}),
	};
...
export function formatHostPluginSetup(result: HostPluginSetupResult): string {
	...
	lines.push(
		`  SKC_COORDINATOR_MCP_WORKDIR_ROOTS=${result.coordinatorConfigPreview.env.SKC_COORDINATOR_MCP_WORKDIR_ROOTS}`,
	);
