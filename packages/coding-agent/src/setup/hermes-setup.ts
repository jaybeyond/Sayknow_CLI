...
const MANAGED_BY = "skc";
const SETUP_SCHEMA_VERSION = "1";
const DEFAULT_SERVER_KEY = "skc_coordinator";
const DEFAULT_SKC_COMMAND = "skc";
...
function resolveHermesSessionCommand(skcCommand: string, flags: HermesSetupFlags): string {
	...
	if (!worktree.enabled) return skcCommand;
	return worktree.name ? `${skcCommand} --worktree ${worktree.name}` : `${skcCommand} --worktree`;
}
...
export function buildHermesSetupSpec(flags: HermesSetupFlags): CoordinatorSetupSpec {
	...
	const skcCommand = optionalTrim(flags.skcCommand) ?? DEFAULT_SKC_COMMAND;
	const sessionCommand = resolveHermesSessionCommand(skcCommand, flags);
	return {
		schemaVersion: 1,
		coordinator: "hermes",
...
export function renderHermesServerBlock(spec: CoordinatorSetupSpec): Record<string, unknown> {
	const env: Record<string, string> = {
		SKC_COORDINATOR_MCP_WORKDIR_ROOTS: spec.roots.join(path.delimiter),
		SKC_COORDINATOR_MCP_SETUP_MANAGED_BY: MANAGED_BY,
		SKC_COORDINATOR_MCP_SETUP_SCHEMA_VERSION: SETUP_SCHEMA_VERSION,
		SKC_COORDINATOR_MCP_SETUP_SIGNATURE: computeHermesSetupSignature(spec),
	};
	if (spec.namespace.profile) env.SKC_COORDINATOR_MCP_PROFILE = spec.namespace.profile;
	if (spec.namespace.repo) env.SKC_COORDINATOR_MCP_REPO = spec.namespace.repo;
	if (spec.stateRoot) env.SKC_COORDINATOR_MCP_STATE_ROOT = spec.stateRoot;
	if (spec.mutationPolicy.classes.length > 0)
		env.SKC_COORDINATOR_MCP_MUTATIONS = spec.mutationPolicy.classes.join(",");
	if (spec.artifactByteCap !== undefined) env.SKC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP = String(spec.artifactByteCap);
	if (spec.sessionCommand) env.SKC_COORDINATOR_MCP_SESSION_COMMAND = spec.sessionCommand;
	return {
		command: spec.skcCommand,
		args: spec.args,
		env,
...
function renderOperatorTemplate(spec: CoordinatorSetupSpec): string {
	return operatorInstructionsTemplate
		...
		.replaceAll("{{TOOL_PREFIX}}", "skc_coordinator")
...
function serverBlockIsManaged(block: unknown): boolean {
	...
	return (
		isRecord(env) &&
		env.SKC_COORDINATOR_MCP_SETUP_MANAGED_BY === MANAGED_BY &&
		env.SKC_COORDINATOR_MCP_SETUP_SCHEMA_VERSION === SETUP_SCHEMA_VERSION &&
		typeof env.SKC_COORDINATOR_MCP_SETUP_SIGNATURE === "string"
	);
}
...
function mergeHermesConfig(
	...
	if (existingBlock !== undefined && !serverBlockIsManaged(existingBlock) && !force) {
		throw new HermesSetupError(`Hermes MCP server '${spec.serverKey}' already exists and is not managed by SKC.`, 3);
	}
	return {
		...existing,
		mcp_servers: {
...
function operatorPathForTarget(spec: CoordinatorSetupSpec): string | null {
	...
	return path.join(spec.installTarget.path, "skills", "autonomous-ai-agents", "sayknow-cli", "SKILL.md");
}
...
async function installConfig(spec: CoordinatorSetupSpec, force: boolean): Promise<string[]> {
	...
	if (operatorPath) {
		if ((await Bun.file(operatorPath).exists()) && !force) {
			...
			if (
				!current.includes("SKC Hermes operator instructions") ||
				!current.includes(`Server key: ${spec.serverKey}`)
			) {
				throw new HermesSetupError(
					`Operator instruction target already exists and is not managed by SKC: ${operatorPath}`,
					3,
				);
			}
		}
	...
	return {
		...
		warnings:
			spec.sessionCommandSource === "explicit"
				? [
						"Using explicit SKC_COORDINATOR_MCP_SESSION_COMMAND exactly as supplied; provider/model/worktree validation is not performed.",
					]
				: spec.worktree.enabled
					? [
							`SKC_COORDINATOR_MCP_SESSION_COMMAND defaults to '${spec.sessionCommand}' so SKC owns worktree creation and resume identity.`,
						]
					: [
							"SKC_COORDINATOR_MCP_SESSION_COMMAND defaults to the configured skc command with worktree isolation disabled by user request.",
						],
		smoke,
	};
}
