 ...
 *
 * This is the testable core behind `skc setup credentials` (CLI, primary entry)
...
import type { AuthCredential, OAuthCredential } from "@sayknow-cli/ai";
import { isEnoent } from "@sayknow-cli/utils";
import { redactSecret } from "./provider-onboarding";

/** skc provider ids that external credentials map onto. */
...
/** A credential that can be safely imported into skc's store. */
