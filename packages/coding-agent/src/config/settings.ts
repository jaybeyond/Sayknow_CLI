 ...
 *   settings.set("theme.dark", "red-octopus");              // sync write, saves in background
 *
...
import { type LanguagePref, setLanguage } from "../i18n";
...
export class Settings implements NotificationSettingsReader {
	...
	async #loadYaml(filePath: string): Promise<RawSettings> {
		...
		if (filePath === this.#configPath) {
			try {
				...
				if (!(error instanceof Error) || error.message !== "skc_notify_daemon_invalid_configuration") throw error;
	...
	#migrateRawSettings(raw: RawSettings): RawSettings {
		...
		if (typeof raw.theme === "string") {
			...
			if (oldTheme === "dark" && migratedTheme === "blue-octopus") {
				raw.theme = { dark: migratedTheme };
			} else if (oldTheme === "light" && migratedTheme === "blue-octopus") {
	...
	#recomputeNotificationValidationFromRaw(): void {
		...
		try {
			...
			if (error instanceof Error && error.message === "skc_notify_daemon_invalid_configuration") {
				this.#hasMalformedConfigRoot = false;
				this.#hasInvalidNotificationConfiguration = true;
				return;
			}
...
const SETTING_HOOKS: Partial<Record<SettingPath, SettingHook<any>>> = {
	...
	language: value => {
		if (typeof value === "string") {
			setLanguage(value as LanguagePref);
		}
	},
