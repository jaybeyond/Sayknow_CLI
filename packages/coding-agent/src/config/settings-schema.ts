 ...
 *   settings.set("theme.dark", "red-octopus")  // sync, saves in background
 */
...
export type StatusLineSegmentId =
	| "sayknow"
	| "pi" // legacy custom alias; public presets use sayknow
	| "model"
	| "mode"
	| "path"
	| "git"
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `skc auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`SKC_AUTH_BROKER_URL` / `SKC_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"skc.deepInterview.ambiguityThreshold": {
		type: "number",
		default: 0.05,
	...
	"completion.notifyCommand": {
		...
		ui: {
			...
			description:
				"Optional user-level shell command to run when an agent turn completes; receives SKC_NOTIFICATION_* environment variables. On Windows, this can call PowerShell [Console]::Beep when terminal BEL is silent.",
		},
	},
	...
	"hindsight.retainContext": { type: "string", default: "skc" },
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description: "Cache rendered issue/PR view output in ~/.skc/cache/github-cache.db so repeated reads are free",
		},
	},
