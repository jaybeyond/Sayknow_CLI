...
export function detectSessionImportFormat(
	text: string,
	requestedProvider?: SessionImportProviderId,
	...
	if (!detected) {
		throw new SessionImportError(
			"unsupported_format",
			"detect",
			"Could not detect a supported Codex or Claude transcript format. Pass --provider codex|claude to declare the expected provider, or export the session again.",
		);
	}
