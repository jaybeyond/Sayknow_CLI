/**
 * `skc auth-broker` command handlers.
 *
 * Sub-verbs:
...
import { $which, APP_NAME, getAgentDbPath, getConfigRootDir, isEnoent, logger, VERSION } from "@sayknow-cli/utils";
import { $ } from "bun";
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// skip the rotating ~/.skc/logs/ file the TUI default would have used.
...
async function runLogin(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!providerArg) {
		throw new Error("Usage: skc auth-broker login <provider> [--via=user@host]");
	}
...
async function runLocalLogin(provider: OAuthProvider): Promise<void> {
	...
	const piAiCli = Bun.fileURLToPath(import.meta.resolve("@sayknow-cli/ai/cli"));
...
async function runLogout(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	const providerArg = flags.provider;
	if (!providerArg) {
		throw new Error("Usage: skc auth-broker logout <provider>");
	}
...
/**
 * Maps the `type` field of a CLIProxyAPI credential JSON to the skc provider id.
...
async function loadImportPlan(
	...
	for (const file of files) {
		...
		if (!provider) {
			skipped.push({
				file,
				reason: `cannot determine skc provider from type=${json.type ?? "?"} (pass --provider to override)`,
			});
			continue;
		}
...
async function runImport(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!target) {
		throw new Error("Usage: skc auth-broker import <file|dir> [--provider=<id>] [--include-disabled] [--dry-run]");
	}
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"SKC_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
	if (flags.fromLocal !== true) {
		throw new Error(
			"`skc auth-broker migrate` requires an explicit source. Pass `--from-local` to migrate from the local SQLite store and env vars.",
		);
	}
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set SKC_AUTH_BROKER_URL to enable).";
