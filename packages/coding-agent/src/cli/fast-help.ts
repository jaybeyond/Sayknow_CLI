import { APP_NAME, CONFIG_DIR_NAME } from "@sayknow-cli/utils/dirs";
...
export function getExtraHelpText(): string {
  ...
  ${APP_NAME} setup                - Install SKC defaults or optional dependencies
  ${APP_NAME} session              - List, inspect, create, remove, or attach sessions
  ${APP_NAME} state                - Inspect or manage persisted SKC state
  ...
  ${APP_NAME} update               - Update SKC installation artifacts
...
Environment Variables:
  ...
  # Configuration
  SKC_CODING_AGENT_DIR       - Session storage directory (default: ~/${CONFIG_DIR_NAME}/agent)
  SKC_PACKAGE_DIR            - Override package directory (for Nix/Guix store paths)
  SKC_SMOL_MODEL              - Override smol/fast model (see --smol)
