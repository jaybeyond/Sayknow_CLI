/**
 * CLI handler for `skc worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.skc/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "@sayknow-cli/utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.skc/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `skc worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.skc/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
