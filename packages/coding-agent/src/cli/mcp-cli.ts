 ...
 *
 * This surface only writes explicit user-provided server definitions to SKC's
 * own MCP config. It never imports or inherits live configs from other agents.
 */
import { getMCPConfigPath, getProjectDir } from "@sayknow-cli/utils";
...
const STORAGE_ONLY_RUNTIME_DISCLOSURE: RuntimeDisclosure = {
	runtimeStatus: "storage-only",
	runtimeLoadedByStandalone: false,
	runtimeNote: "Stored MCP registrations are not loaded by normal standalone skc sessions today.",
};
...
function buildServerConfig(args: MCPCommandArgs): MCPServerConfig {
	...
	if (type === "stdio") {
		...
		if (!command) {
			throw new MCPArgsError("`skc mcp add` requires --command <cmd> or a positional command for stdio servers.");
		}
	...
	if (!url) {
		throw new MCPArgsError(`\`skc mcp add --type ${type}\` requires --url <url> or a positional URL.`);
	}
...
async function runAdd(args: MCPCommandArgs, scoped: ScopedPath): Promise<void> {
	if (!args.name) throw new MCPArgsError("`skc mcp add` requires a server name.");
	...
	if (result.status === "skipped") {
		process.stdout.write(
			`MCP server "${args.name}" already exists in ${scoped.scope} config. Pass --force to overwrite. ` +
				"Status: storage-only; normal standalone skc sessions do not load stored MCP registrations today.\n",
		);
		return;
	}
	process.stdout.write(
...
async function runRemove(args: MCPCommandArgs, scoped: ScopedPath): Promise<void> {
	if (!args.name) throw new MCPArgsError("`skc mcp remove` requires a server name.");
