 ...
 *
 * Handles `skc plugin <command>` subcommands for plugin lifecycle management.
 */
	...
	try {
		if (pluginId) {
			...
			if (flags.scope) {
				console.error(
					chalk.yellow(
						`Warning: --scope is ignored when upgrading all plugins. Use 'skc plugin upgrade <id> --scope ${flags.scope}' to target a specific plugin and scope.`,
					),
				);
			}
...
async function handleInstall(
	...
	if (packages.length === 0) {
		...
		console.error(chalk.dim(`  ${APP_NAME} plugin install @sayknow-cli/exa`));
...
${chalk.bold("Examples:")}
  ${APP_NAME} plugin install @sayknow-cli/exa[search]
