 ...
 *
 * Handles `skc setup [component]` to install the normal defaults or optional feature dependencies.
 */
...
import {
	...
	mergeSkcManagedCodexHooksConfig,
...
export function parseSetupArgs(args: string[]): SetupCommandArgs | undefined {
	...
	for (let i = 1; i < args.length; i++) {
		...
		} else if (arg === "--skc-command") {
			flags.skcCommand = args[++i];
		} else if (arg === "--target") {
			flags.target = args[++i];
...
// package dependencies beyond a working interpreter. `skc setup python --check`
...
async function handleHooksSetup(flags: { json?: boolean; check?: boolean }): Promise<void> {
	...
	const existingContent = await Bun.file(hooksPath)
		.text()
		.catch(() => null);
	const status = readSkcManagedCodexHooksStatus(existingContent, hooksPath);
	...
	if (flags.check) {
		...
		if (!status.installed) {
			process.stderr.write(`${chalk.red(`${theme.status.error} SKC native Codex hooks are not fully installed`)}\n`);
			process.stderr.write(`${chalk.dim(`Target: ${hooksPath}`)}\n`);
			process.stderr.write(`${chalk.dim(`Missing events: ${status.missingEvents.join(", ")}`)}\n`);
			process.exit(1);
		}
		process.stdout.write(`${chalk.green(`${theme.status.success} SKC native Codex hooks are installed`)}\n`);
		process.stdout.write(`${chalk.dim(`Target: ${hooksPath}`)}\n`);
		return;
	}

	const merged = mergeSkcManagedCodexHooksConfig(existingContent);
	await Bun.write(hooksPath, merged.content);
	const installed = readSkcManagedCodexHooksStatus(merged.content, hooksPath);
	...
	process.stdout.write(`${chalk.green(`${theme.status.success} SKC native Codex hooks installed`)}\n`);
	process.stdout.write(`${chalk.dim(`Target: ${hooksPath}`)}\n`);
...
async function handleDefaultsSetup(flags: { json?: boolean; check?: boolean; force?: boolean }): Promise<void> {
	...
	if (flags.check) {
		if (hasCheckFailure) {
			console.error(chalk.red(`${theme.status.error} Default SKC workflow skills are not fully installed`));
...
export function printSetupHelp(): void {
	console.log(`${chalk.bold(`${APP_NAME} setup`)} - Install SKC defaults or optional feature dependencies

${chalk.bold("Usage:")}
  ${APP_NAME} setup [component] [options]

${chalk.bold("Components:")}
  defaults  Install bundled SKC default workflow skills (default)
  hermes   Optional: render/install a Hermes MCP bridge setup package
  hooks     Optional: install SKC native Codex UserPromptSubmit/Stop skill-state hooks
...
${chalk.bold("Hermes example:")}
  ...
  ${APP_NAME} setup hermes --root /path/to/repo --profile my-bot --repo sayknow-cli --profile-dir /path/to/hermes/profile --install
  ${APP_NAME} setup hermes --root /path/to/repo --worktree-name hermes-sayknow-cli
  ${APP_NAME} setup hermes --root /path/to/repo --session-command "skc --worktree hermes-custom --model <provider/model>"
...
${chalk.bold("Options:")}
  ...
  --session-command Explicit SKC session command; disables generated worktree flags
  --no-worktree     Disable default SKC --worktree isolation for Hermes sessions
  --worktree-name   Named SKC --worktree branch for Hermes sessions
...
${chalk.bold("Examples:")}
  ...
  ${APP_NAME} setup defaults         Install bundled SKC default workflow skills explicitly
  ${APP_NAME} setup defaults --check Check bundled SKC default workflow skills are installed
