...
# SKC prelude helpers (loaded once into the runner namespace)
if "__skc_prelude_loaded__" not in globals():
    __skc_prelude_loaded__ = True
    from pathlib import Path
    import os, json

    # __skc_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _skc_display = __skc_display  # type: ignore[name-defined]
    ...
    def _emit_status(op: str, **data):
        ...
        _skc_display({"application/x-skc-status": {"op": op, **data}}, raw=True)
