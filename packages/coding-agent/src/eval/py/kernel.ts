...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "skc-python-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	return [
		"import os, sys",
		`__skc_cwd = ${JSON.stringify(cwd)}`,
		"os.chdir(__skc_cwd)",
		`__skc_env = ${JSON.stringify(envPayload)}`,
		"for __skc_key, __skc_val in __skc_env.items():\n    os.environ[__skc_key] = __skc_val",
		"if __skc_cwd not in sys.path:\n    sys.path.insert(0, __skc_cwd)",
	].join("\n");
}
