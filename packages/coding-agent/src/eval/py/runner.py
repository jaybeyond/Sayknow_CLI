"""SKC Python runner — subprocess wrapper used by the coding-agent host.
...
def transform_cell(source: str) -> str:
    ...
    Rules
    -----
    * ``%name args``              -> ``__skc_magic("name", "args")``
    * ``var = %name args``        -> ``var = __skc_magic("name", "args")``
    * ``!cmd``                    -> ``__skc_shell("cmd")``
    * ``var = !cmd``              -> ``var = __skc_shell("cmd")``
    * ``%%name args\\n<body>``    -> ``__skc_magic_cell("name", "args", "<body>")``
    ...
    while i < len(lines):
        ...
        if stripped.startswith("%%"):
            ...
            body = "\n".join(body_lines)
            out.append(
                f"{indent}__skc_magic_cell({_quote_arg(name)}, {_quote_arg(args)}, {_quote_arg(body)})"
            )
            return "\n".join(out)
        ...
        if stripped.startswith("%") and not stripped.startswith("%%"):
            ...
            name, args = head
            out.append(f"{indent}__skc_magic({_quote_arg(name)}, {_quote_arg(args)})")
            i += consumed
            continue
        ...
        if stripped.startswith("!"):
            ...
            out.append(f"{indent}__skc_shell({_quote_arg(cmd)})")
            i += consumed
            continue
        ...
        if m:
            ...
            if rhs.startswith("!"):
                cmd = rhs[1:].strip()
                out.append(f"{m.group('indent')}{m.group('lhs').rstrip()} = __skc_shell({_quote_arg(cmd)})")
                i += 1
                continue
            if rhs.startswith("%") and not rhs.startswith("%%"):
                ...
                name, args = head
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __skc_magic({_quote_arg(name)}, {_quote_arg(args)})"
                )
                i += 1
                continue
...
def _emit_status(op: str, **data: Any) -> None:
    bundle = {"application/x-skc-status": {"op": op, **data}}
    rid = _STATE.current_id
    if rid is None:
        return
...
def __skc_magic(name: str, args: str) -> Any:
...
def __skc_magic_cell(name: str, args: str, body: str) -> Any:
...
def __skc_shell(cmd: str) -> _ShellResult:
    proc = subprocess.run(
        cmd,
        shell=True,
        stdout=subprocess.PIPE,
...
def __skc_display(value: Any, *, raw: bool = False, kind: str = "display") -> None:
    if raw:
...
def _install_builtins(ns: dict) -> None:
    ns["display"] = __skc_display
    ns["__skc_display"] = __skc_display
    ns["__skc_magic"] = __skc_magic
    ns["__skc_magic_cell"] = __skc_magic_cell
    ns["__skc_shell"] = __skc_shell
...
def _exec_source(source: str, ns: dict) -> None:
    ...
    its value through ``__skc_display`` so dataframes/figures render rich.
    ...
    if isinstance(last, ast.Expr):
        ...
        if value is not None:
            __skc_display(value, kind="result")
        return
...
def _start_parent_watchdog() -> None:
    ...
    thread = threading.Thread(target=watch, name="skc-parent-watchdog", daemon=True)
    thread.start()
