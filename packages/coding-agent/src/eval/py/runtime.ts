...
import { $env, $which, getPythonEnvDir } from "@sayknow-cli/utils";

export const RLM_MANAGED_PYTHON_PACKAGES: readonly string[] = ["numpy", "pandas", "matplotlib", "polars"];

export interface PythonRuntimeOptions {
	/** Create/use <cwd>/.skc/python-env when no BYO venv/conda env is present. */
...
function resolveWorkspaceManagedPythonCandidate(cwd: string): { venvPath: string; pythonPath: string; binDir: string } {
	return resolvePythonCandidateInVenv(path.join(cwd, ".skc", "python-env"));
}
...
async function ensureWorkspaceManagedVenv(
	...
	const markerPath = path.join(managed.venvPath, ".skc-seeded.json");
	let seeded = false;
	try {
