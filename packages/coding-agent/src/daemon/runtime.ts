...
export interface SkcRuntimeSpawnInfo {
	execPath: string;
	mode: "source" | "compiled";
	/** Prefix prepended before the skc subcommand args; `[Bun.main]` in source mode, otherwise `[]`. */
	argsPrefix: string[];
...
/**
 * Resolve how to spawn a detached skc subcommand for the current runtime.
 *
 ...
 */
export function resolveSkcRuntimeSpawnInfo(execPath: string = process.execPath): SkcRuntimeSpawnInfo {
	const base = path.basename(execPath).toLowerCase();
	const fromSource = base === "bun" || base === "node" || base.startsWith("bun") || base.startsWith("node");
	const sourceEntry = fromSource ? path.resolve(import.meta.dir, "../../bin/skc.js") : undefined;
	if (fromSource) {
		return {
			execPath,
			mode: "source",
