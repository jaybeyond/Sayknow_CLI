/**
 * Shared operator contract for the `skc daemon` command surface.
 *
...
/** Exit codes for `skc daemon`. Backward compatible: success 0, any failure 1. */
...
export function ownershipMismatchRecovery(): DaemonRecovery {
	return {
		...
		steps: [
			"Confirm the intended bot token and chat with `skc notify` (or edit notifications.telegram in your config).",
			"If the running daemon is stale or unwanted, stop it with `skc daemon stop --force`, then rerun `skc daemon restart`.",
