/**
 * Protocol handler for skc:// URLs.
 *
 * Serves statically embedded documentation files bundled at build time.
 *
 * URL forms:
 * - skc:// - Lists all available documentation files
 ...
 */
export class SkcProtocolHandler implements ProtocolHandler {
	readonly scheme = "skc";
	...
	async #listDocs(url: InternalUrl): Promise<InternalResource> {
		...
		const listing = EMBEDDED_DOC_FILENAMES.map(f => `- [${f}](skc://${f})`).join("\n");
	...
	async #readDoc(filename: string, url: InternalUrl): Promise<InternalResource> {
		...
		if (path.isAbsolute(filename)) {
			throw new Error("Absolute paths are not allowed in skc:// URLs");
		}
		...
		if (content === undefined) {
			...
			const suffix =
				suggestions.length > 0
					...
					: "\nUse skc:// to list available files.";
