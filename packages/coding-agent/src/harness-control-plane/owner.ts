 ...
 *  - own the {@link HarnessRpc} subprocess (injected; real `SayknowCliRpc` in prod, fake in tests),
 *  - serve owner-routed primitives over the {@link ControlServer} endpoint,
 *  - be the SOLE writer of the severity event stream,
 *  - heartbeat the lease.
 *
 * Stateless `skc harness` CLI calls reach the owner via {@link resolveOwner} + the endpoint.
 */
