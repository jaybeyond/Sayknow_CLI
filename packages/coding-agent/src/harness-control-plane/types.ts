/**
 * Core types for the sayknow-cli-native coding-harness operations control plane (v1).
 *
 * See the approved consensus plan at
 * `.skc/plans/ralplan/2026-06-02-0853-3e33/stage-02-revision.md` and the spec at
 * `.skc/specs/deep-interview-harness-control-plane.md`.
 *
 * v1 implements the sayknow-cli adapter only. omx/codex/remote/auth are deferred seams.
 */

import type { AgentWireObservedSignal } from "../modes/shared/agent-wire/event-contract";

/** Harnesses the control plane can operate. v1 implements `sayknow-cli` only. */
export type Harness = "sayknow-cli" | "codex" | "omx";
...
/** The CLI verbs / primitives exposed by `skc harness <verb>`. */
