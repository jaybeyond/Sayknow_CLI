...
export interface CreateSdkSessionTransportOptions {
	repo: string;
	sessionId: string;
	/** Starts the normal SKC session before discovery. Test callers may provide a deterministic SDK host. */
...
export function spawnNormalHarnessSession(
	...
	if (process.env.SKC_SDK_DISABLE === "1") {
	...
	if (!entry)
		throw new HarnessSdkTransportError(
			"endpoint_unavailable",
			"Cannot determine the SKC CLI entrypoint for harness session startup.",
		);
	const child = Bun.spawn([process.execPath, path.resolve(entry)], {
		cwd: repo,
		env: { ...process.env, SKC_LIFECYCLE_REQUEST_ID: `harness-${sessionId}`, SKC_SESSION_ID: sessionId },
