/**
 * sayknow-cli RPC adapter + single-flight acceptance.
 *
 * The sayknow-cli harness is driven via `skc --mode rpc` (see docs/rpc.md). Acceptance
 ...
 * {@link SayknowCliRpc} provides the real `skc --mode rpc` subprocess implementation
...
/** Abstract handle to a live sayknow-cli RPC session. */
...
/**
 * Real adapter: spawns `skc --mode rpc --session-dir <dir>` and speaks the JSONL
 * protocol from docs/rpc.md. Verified end-to-end in the M10 suite.
 */
export class SayknowCliRpc implements HarnessRpc {
	...
	constructor(opts: { sessionDir: string; command?: string[]; cwd?: string; env?: NodeJS.ProcessEnv }) {
		const base = opts.command ?? ["skc", "--mode", "rpc"];
