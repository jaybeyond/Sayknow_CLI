 ...
 *
 * Layout (under the harness state root, default `<cwd>/.skc/_session-{sessionid}/state/harness`):
 ...
 *   sessions/<encoded-id>/skc-session/       underlying sayknow-cli --session-dir
 *
...
import { harnessStateRoot } from "../skc-runtime/session-layout";
...
function harnessRootRegistryDir(env: NodeJS.ProcessEnv = process.env): string {
	const override = env.SKC_HARNESS_ROOT_REGISTRY_DIR?.trim();
	if (override) return path.resolve(override);
	return path.join(os.tmpdir(), `skch${process.getuid?.() ?? "u"}`, "harness-roots");
}
...
function socketBase(env: NodeJS.ProcessEnv, allowOverride: boolean): { base: string; fromOverride: boolean } {
	const override = env.SKC_HARNESS_SOCKET_DIR?.trim();
	if (allowOverride && override) return { base: path.resolve(override), fromOverride: true };
	return { base: path.join(os.tmpdir(), `skch${process.getuid?.() ?? "u"}`), fromOverride: false };
}
...
export function resolveHarnessRoot(opts?: {
	...
	const fromEnv = env.SKC_HARNESS_STATE_ROOT;
	if (fromEnv?.trim()) return path.resolve(fromEnv.trim());
	const skcSessionId = opts?.skcSessionId ?? env.SKC_SESSION_ID?.trim();
	if (!skcSessionId) {
		throw new StorageError("SKC session id is required for default harness state root", "missing_skc_session_id");
	}
	return harnessStateRoot(opts?.cwd ?? process.cwd(), skcSessionId);
}
...
export function sessionPaths(root: string, sessionId: string): SessionPaths {
	...
	return {
		...
		skcSessionDir: path.join(dir, "skc-session"),
	};
}
