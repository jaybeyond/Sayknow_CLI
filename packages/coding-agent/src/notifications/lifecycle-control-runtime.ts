...
export function daemonSpawnCreate(env: NodeJS.ProcessEnv = process.env) {
	return async (
		...
		const tmux = resolveSkcTmuxCommand(env);
		...
		if (created.exitCode !== 0) {
			throw new Error(created.stderr.toString().trim() || "skc_lifecycle_spawn_failed");
		}
		const target = buildSkcTmuxExactOptionTarget(name);
		const metaCommands = buildSkcTmuxProfileCommands(target, env, {
		...
		const status = statusSkcTmuxSession(name, env);
		return {
			sessionId: ids.intendedSessionId,
			tmuxSession: name,
...
export function daemonCloseSession(env: NodeJS.ProcessEnv = process.env) {
	return async (target: { sessionId: string; tmuxSession?: string; sessionStateFile?: string }) => {
		...
		forceCloseSkcTmuxSession(name, env, target.sessionId, target.sessionStateFile);
		return { processGone: findSkcTmuxSessionByName(name, env) === undefined };
	};
}
...
export function daemonResumeSession(env: NodeJS.ProcessEnv = process.env, opts: { sessionsRoot?: string } = {}) {
	return async (target: {
		...
		const live = listSkcTmuxSessions(env).filter(
		...
		const command = `${cwdPrefix}exec env SKC_TMUX_LAUNCHED=1 SKC_NOTIFICATIONS=1 skc --resume ${shellQuote(resumeId)}`;
		const r = Bun.spawnSync([tmux, "new-session", "-d", "-s", name, "sh", "-c", command], {
			stdout: "pipe",
			stderr: "pipe",
			env,
		});
		if (r.exitCode !== 0) throw new Error(r.stderr.toString().trim() || "skc_lifecycle_resume_failed");
		const tgt = buildSkcTmuxExactOptionTarget(name);
		for (const cmd of buildSkcTmuxProfileCommands(tgt, env, { sessionId: resumeId, project: resolvedResumeCwd })) {
			Bun.spawnSync([tmux, ...cmd.args], { stdout: "pipe", stderr: "pipe", env });
		}
		return {
			sessionId: resumeId,
