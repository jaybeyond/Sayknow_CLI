...
function recordDirectorySyncCompatibilityDiagnostic(): void {
	try {
		process.stderr.write("skc lifecycle ledger directory sync unsupported\n");
	} catch {
...
function recordLifecycleControlDiagnostic(): void {
	try {
		process.stderr.write("skc lifecycle control request failed\n");
	} catch {
...
export const createNativeControlServer: LifecycleControlServerFactory = ({ token, ownerId, agentDir }) => {
	...
	const { NotificationControlServer } = require("@sayknow-cli/natives") as typeof import("@sayknow-cli/natives");
