...
export function buildSanitizedEffectiveSkillConfigContext(input: EffectiveSkillConfigInput | undefined): string {
	if (!input || input.unavailableReason) {
		...
		return `Sanitized effective skill config unavailable (${reason}); bundled SKC workflow activation remains available for deep-interview, ralplan, ultragoal, team.`;
	}
	...
	return [
		"Sanitized effective skill config for filesystem/custom skill discovery; bundled SKC workflow activation remains available for exactly deep-interview, ralplan, ultragoal, team.",
...
function parseExplicitSkillInvocations(text: string): {
	...
	const explicitPattern = /\$((?:skc:)?[a-z][a-z0-9-]*)/gi;
	const seenSkills = new Set<string>();
	let match = explicitPattern.exec(text);
	while (match !== null) {
		sawExplicitLikeInvocation = true;
		const token = match[1] ?? "";
		const normalized = token.startsWith("skc:") ? token.slice(4) : token;
		if (isSkcWorkflowSkill(normalized) && !seenSkills.has(normalized)) {
			seenSkills.add(normalized);
			matches.push({
				keyword: match[0],
				skill: normalized,
				priority: SKC_SKILL_KEYWORD_DEFINITIONS.find(definition => definition.skill === normalized)?.priority ?? 0,
			});
		}
...
function isWorkflowActiveEntry(entry: SkillActiveEntry): entry is SkillActiveEntry & { skill: SkcWorkflowSkill } {
	return isSkcWorkflowSkill(entry.skill);
}
...
/**
 * Idempotently seed `.skc/state` for a workflow skill that was invoked directly
 ...
 * (for example after a `skc state handoff` promotion that carries
 ...
 */
function isHandoffRequiredSkill(skill: SkcWorkflowSkill): boolean {
 ...
 * the artifact under `.skc/specs/`, so a crystallized state carries a
...
async function detectUncrystallizedDeepInterviewStop(
	...
	return `the deep-interview run reached a terminal phase ("${phase || "unknown"}") without crystallizing a usable spec/handoff. Run \`skc deep-interview --write --stage final\` (optionally \`--handoff ralplan\`) to persist the distilled interview spec, hand off through the deep-interview policy, or explicitly cancel/clear the interview before stopping`;
}
...
function buildDeepInterviewPlaintextAskLeakMessage(statePath: string): string {
	return `SKC deep-interview emitted a Deep Interview question/options block as plain text (${statePath}). It must not wait for a prose answer. Continue immediately by calling the ask tool with the Restate gate question and options: Yes, crystallize; Adjust wording; Missing scope; plus free text/custom input.`;
}
	...
	for (const entry of activeEntries) {
		...
		if (await shouldRescueDeepInterviewPlaintextAskLeak(entry.skill, modeState, input.cwd, input.sessionFile)) {
			...
			return {
				...
				stopReason: "skc_skill_deep_interview_plaintext_ask_leak",
		...
		if (modeStateReleasesStop(modeState, handoffRequired)) {
			...
			if (staleRelease) {
				...
				return {
					...
					stopReason: `skc_skill_${entry.skill.replace(/-/g, "_")}_stale_mode_state`,
			...
			if (uncrystallized) {
				...
				return {
					...
					stopReason: "skc_skill_deep_interview_uncrystallized",
		...
		return {
			...
			stopReason: `skc_skill_${entry.skill.replace(/-/g, "_")}_${phase.replace(/[^a-z0-9]+/gi, "_").toLowerCase()}`,
			systemMessage,
		};
	}
...
export function buildSkillActivationAdditionalContext(
	...
	return [
		`SKC native UserPromptSubmit detected workflow keyword "${state.keyword}" -> ${state.skill}.`,
		state.initialized_mode && state.initialized_state_path
			? `skill: ${state.initialized_mode} activated and initial state initialized at ${state.initialized_state_path}; use \`skc state write/read/clear --input '<json>' --json\` for runtime state updates.`
			: null,
		state.skill === "ultragoal"
			? "Ultragoal is active. If the user prompt is a steering request, use `skc ultragoal steer` to add or steer subgoals."
			: null,
		buildSanitizedEffectiveSkillConfigContext(effectiveSkillConfig),
		"Follow AGENTS.md routing and preserve SKC workflow transition and planning-safety rules.",
	]
