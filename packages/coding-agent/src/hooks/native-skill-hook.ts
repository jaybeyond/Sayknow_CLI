...
export interface SkcNativeHookDispatchResult {
	hookEventName: SkcNativeHookEventName | null;
...
export function clearSkcNativeSkillHookCachesForTesting(): void {
	effectiveSkillConfigCache.clear();
	effectiveSkillConfigResolutionCount = 0;
}

export function getSkcNativeSkillHookCacheStatsForTesting(): { effectiveSkillConfigResolutions: number } {
...
function resolveConfigPaths(cwd: string, override?: string[]): string[] {
	...
	const configDirName = process.env.SKC_CONFIG_DIR ?? process.env.PI_CONFIG_DIR ?? ".skc";
	const userAgentDir = process.env.SKC_CODING_AGENT_DIR ?? path.join(os.homedir(), configDirName, "agent");
...
export async function resolveSkcNativeSkillConfigForTesting(input: {
	cwd: string;
	configPaths?: string[];
	sessionId?: string;
	threadId?: string;
...
function readHookEventName(payload: HookPayload): SkcNativeHookEventName | null {
...
function readSessionFile(payload: HookPayload): string | undefined {
	return (
		...
		process.env.SKC_SESSION_FILE?.trim() ||
		undefined
	);
}

export async function dispatchSkcNativeSkillHook(
	payload: HookPayload,
...
export async function runSkcNativeSkillHookInProcess(payload: HookPayload): Promise<string> {
	const result = await dispatchSkcNativeSkillHook(payload);
...
export async function runSkcNativeSkillHookCli(): Promise<void> {
	...
	if (parseError) {
		...
		process.stdout.write(
			`${JSON.stringify({
				decision: "block",
				reason: "SKC native hook received malformed JSON input.",
				hookSpecificOutput: {
					hookEventName: "Unknown",
					additionalContext: `stdin JSON parsing failed inside skc codex-native-hook: ${parseError.message}`,
				},
			})}\n`,
		);
		return;
	...
	try {
		...
		if (readHookEventName(payload) === "Stop") {
			...
			process.stdout.write(
				`${JSON.stringify({
					decision: "block",
					reason: "SKC native Stop hook failed before normal continuation handling.",
					stopReason: "skc_native_stop_dispatch_failure",
					systemMessage: `SKC native Stop hook failed before normal continuation handling. Failure: ${detail}`,
				})}\n`,
			);
		} else {
			process.exitCode = 1;
