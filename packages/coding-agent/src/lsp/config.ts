...
import { $which, isRecord, logger, pathIsWithin } from "@sayknow-cli/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.skc/, .gemini/)
	...
	// User config directories (~/.skc/agent/, ~/.gemini/)
 ...
 * 2. Project config dirs: .skc/lsp.*, .gemini/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.skc/agent/lsp.*, ~/.gemini/lsp.* (+ hidden variants)
