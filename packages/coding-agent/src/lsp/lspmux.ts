...
import { $flag, $which, logger } from "@sayknow-cli/utils";
 ...
 *
 * Set SKC_DISABLE_LSPMUX=1 or PI_DISABLE_LSPMUX=1 to disable.
 */
export async function detectLspmux(cwd = process.cwd()): Promise<LspmuxState> {
	const now = Date.now();
	if ($flag("SKC_DISABLE_LSPMUX") || $flag("PI_DISABLE_LSPMUX")) {
