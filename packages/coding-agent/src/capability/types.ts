 ...
 * paths like `.skc`, `.gemini`, or `.vscode`, they simply ask for `load("mcps")`
...
export interface Provider<T> {
	/** Unique provider ID (e.g., "Anthropic model", "skc", "mcp-json", "agents-md") */
	id: string;

	/** Human-readable name for UI display (e.g., "Anthropic Code", "OpenAI code provider") */
	displayName: string;

	/** Short description for settings UI (e.g., "Load config from .skc/") */
	description: string;

	/**
	 * Priority (higher = checked first, wins on conflicts).
	 * Suggested ranges:
	 *   100+ : Primary providers (skc, pi)
