 ...
 *   - ~/.skc/agent/agents/*.md (user-level, primary)
 *   - .skc/agents/*.md (project-level, primary)
 *   - installed SKC plugin roots
 *
...
import { rootContainsSkcManifest } from "../extensibility/skc-plugins/paths";
 ...
 *
 * Precedence (highest wins): .skc project, .skc user, SKC plugins, then bundled
 *
...
export async function discoverAgents(cwd: string, home: string = os.homedir()): Promise<DiscoveryResult> {
	...
	// Get user directories (priority order: .skc, ...)
	...
	// Load agents from SKC marketplace plugins.
	const { roots: pluginRoots } = isProviderEnabled("claude-plugins")
		? await listClaudePluginRoots(home, resolvedCwd)
		: { roots: [] };
	const nonSkcPluginRoots = [];
	for (const plugin of pluginRoots) {
		if (await rootContainsSkcManifest(plugin.path)) continue;
		nonSkcPluginRoots.push(plugin);
	}
	const sortedPluginRoots = nonSkcPluginRoots.sort((a, b) => {
