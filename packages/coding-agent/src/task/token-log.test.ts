...
import { SKC_SESSION_ACTIVITY_FILE, sessionRoot } from "../skc-runtime/session-layout";
import {
	computeCacheHitRate,
	persistTaskTokenLog,
	readTaskTokenLogs,
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
	const dir = await mkdtemp(join(tmpdir(), "skc-token-log-"));
	try {
		return await fn(dir);
	} finally {
	...
	it("prefers the current session manager over a stale latest-active session", async () => {
		await withTempDir(async cwd => {
			...
			for (const [sessionId, updatedAt] of [
				...
				const markerPath = join(sessionRoot(cwd, sessionId), SKC_SESSION_ACTIVITY_FILE);
