 ...
 *   - Bundled agents (shipped with skc-coding-agent)
 *   - ~/.skc/agent/agents/*.md (user-level)
 *   - .skc/agents/*.md (project-level)
 *
 * Supports:
...
} from "../skc-runtime/repository-binding";
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	private constructor(
		...
		this.#blockedAgent = $pickenv("SKC_BLOCKED_AGENT", "PI_BLOCKED_AGENT");
	...
	async #executeSync(
		...
		const tempArtifactsDir = artifactsDir ? null : path.join(os.tmpdir(), `skc-task-${Snowflake.next()}`);
				...
				try {
					...
					if (mergeMode === "branch" && resultWithForkContext.exitCode === 0) {
						try {
							...
							const branchName = `skc/task/${task.id}`;
