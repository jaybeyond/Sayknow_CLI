...
import { getWorktreeDir, hashPath, logger, Snowflake } from "@sayknow-cli/utils";
...
async function writeSyntheticTree(repoDir: string, baseTreeish: string, patches: readonly string[]): Promise<string> {
	const tempIndex = path.join(os.tmpdir(), `skc-task-index-${Snowflake.next()}`);
	try {
...
export async function commitToBranch(
	...
	const branchName = `skc/task/${taskId}`;
	...
	if (rootPatch.trim()) {
		...
		const tmpDir = path.join(os.tmpdir(), `skc-branch-${Snowflake.next()}`);
		try {
...
export async function mergeTaskBranches(
	...
	const didStash = await git.stash.push(repoRoot, "skc-task-merge");
