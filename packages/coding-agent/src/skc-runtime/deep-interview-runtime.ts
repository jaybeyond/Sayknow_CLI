...
/**
 * Native implementation of `skc deep-interview`.
 *
...
async function readSettingsAmbiguityThreshold(
	...
	const candidate = (parsed as { skc?: { deepInterview?: { ambiguityThreshold?: unknown } } })?.skc?.deepInterview
		?.ambiguityThreshold;
...
async function resolveSpecWriteArgs(args: readonly string[], cwd: string): Promise<ResolvedDeepInterviewSpecWriteArgs> {
	...
	for (const arg of args) {
		...
		if (arg.startsWith("-") && !allowedFlags.has(arg)) {
			throw new DeepInterviewCommandError(2, `unknown flag for skc deep-interview --write: ${arg}`);
		}
	}
...
export async function persistDeepInterviewSpec(
	...
	const specPath = path.join(cwd, ".skc", "specs", `deep-interview-${resolved.slug}.md`);
	const content = resolved.spec.endsWith("\n") ? resolved.spec : `${resolved.spec}\n`;
	await writeArtifact(specPath, content, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "skc-runtime", skill: "deep-interview" },
	});

	const sha256 = createHash("sha256").update(content).digest("hex");
	const createdAt = new Date().toISOString();
	await appendJsonl(
		path.join(cwd, ".skc", "specs", "deep-interview-index.jsonl"),
		{ slug: resolved.slug, stage: resolved.stage, path: specPath, created_at: createdAt, sha256 },
		{ cwd, audit: { category: "ledger", verb: "append", owner: "skc-runtime", skill: "deep-interview" } },
	);
	...
	await writeWorkflowEnvelopeAtomic(statePath, payload, {
		...
		receipt: {
			...
			skill: "deep-interview",
			owner: "skc-runtime",
			command: "skc deep-interview persist-spec-state",
...
async function seedDeepInterviewState(cwd: string, resolved: ResolvedDeepInterviewArgs): Promise<string> {
	...
	await writeWorkflowEnvelopeAtomic(statePath, payload, {
		...
		receipt: {
			...
			skill: "deep-interview",
			owner: "skc-runtime",
			command: "skc deep-interview seed",
			sessionId: resolved.sessionId,
			nowIso: now,
		},
...
export async function runNativeDeepInterviewCommand(
	...
	try {
		...
		if (!resolved.idea) {
			throw new DeepInterviewCommandError(
				2,
				'skc deep-interview requires an idea, e.g. `skc deep-interview "<idea>"`.',
			);
		}
