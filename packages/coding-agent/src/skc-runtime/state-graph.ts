...
export type StateGraphSkill = CanonicalSkcWorkflowSkill | "all";
...
function skillsFor(skill: StateGraphSkill): CanonicalSkcWorkflowSkill[] {
	return skill === "all" ? [...CANONICAL_SKC_WORKFLOW_SKILLS] : [skill];
}
...
function dotId(skill: CanonicalSkcWorkflowSkill, state: string): string {
	return `"${skill}:${state}"`;
}

function renderDot(skill: StateGraphSkill): string {
	const lines = ["digraph skc_state {", "  rankdir=LR;"];
	for (const item of skillsFor(skill)) {
		const manifest = getSkillManifest(item);
