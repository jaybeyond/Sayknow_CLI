 ...
 * set-window-option profile values skc emits, which is what unblocks the
 * native Windows skc --tmux path. This module detects that capability so skc
 ...
 *   - SKC_PSMUX_FORCE_DETECT=1 re-probes each call.
 *   - SKC_PSMUX_DETECTION=off skips probing entirely.
 */

export const SKC_PSMUX_COMMAND_ENV = "SKC_PSMUX_COMMAND";
export const SKC_PSMUX_DETECTION_ENV = "SKC_PSMUX_DETECTION";
export const SKC_PSMUX_FORCE_DETECT_ENV = "SKC_PSMUX_FORCE_DETECT";
...
/**
 * SKC_PSMUX_FORCE_DETECT opt-in re-probe switch. Any non-empty value other
...
export function detectPsmux(
	...
	const explicit = env[SKC_PSMUX_COMMAND_ENV]?.trim();
	if (explicit) {
		// The override is authoritative on its own — we trust the user's
		// SKC_PSMUX_COMMAND value when they name a psmux-class binary, even
	...
	if (envDisabled(env[SKC_PSMUX_DETECTION_ENV])) return false;
	const force = options.force === true || envForcesProbe(env[SKC_PSMUX_FORCE_DETECT_ENV]);
...
export interface ResolveSkcTmuxBinaryOptions {
...
/**
 * Resolve the tmux command SKC should invoke. Honors the existing
 * SKC_TMUX_COMMAND / SKC_TEAM_TMUX_COMMAND overrides; on Windows when no
 * override is set, psmux (installed as psmux, pmux, or tmux) is picked
 * automatically so the default skc --tmux flow lands on a real multiplexer.
 */
export function resolveSkcTmuxBinary(options: ResolveSkcTmuxBinaryOptions = {}): ResolvedTmuxBinary {
	const env = options.env ?? process.env;
...
export function probePsmux(
	...
	const output = probeVersionOutput(resolved, runner);
	const isPsmux = outputMentionsPsmux(output) || env[SKC_PSMUX_COMMAND_ENV]?.trim() === resolved;
