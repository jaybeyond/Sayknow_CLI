 ...
 *
 * 2. **Artifact write**: `skc ralplan --write --stage <type> --stage_n <N> --artifact
 *    <path-or-string> [--run-id <id>] [--session-id <id>] [--json]` persists Planner / Architect
 *    / Critic / revision / ADR / final markdown under `.skc/plans/ralplan/<run-id>/`, maintains
...
function ralplanStatePath(cwd: string, sessionId: string | undefined): string {
	const stateDir = sessionId
		? path.join(cwd, ".skc", "state", "sessions", encodeSessionSegment(sessionId))
...
async function persistActiveRunId(
	...
	await writeWorkflowEnvelopeAtomic(statePath, existing, {
		...
		audit: { category: "state", verb: "write", owner: "skc-runtime", skill: "ralplan" },
	});
}
...
async function persistArtifact(
	...
	const runDir = path.join(cwd, ".skc", "plans", "ralplan", resolved.runId);

	const fileName = `stage-${pad2(resolved.stageN)}-${resolved.stage}.md`;
	const filePath = path.join(runDir, fileName);
	await writeArtifact(filePath, content, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "skc-runtime", skill: "ralplan" },
	});
	...
	await appendJsonlIdempotent(path.join(runDir, "index.jsonl"), indexEntry, {
		cwd,
		audit: { category: "ledger", verb: "append", owner: "skc-runtime", skill: "ralplan" },
		key: ralplanIndexKey,
	});
...
async function findExistingStageArtifact(
	...
	const indexPath = path.join(cwd, ".skc", "plans", "ralplan", runId, "index.jsonl");
	let text: string;
	try {
...
function buildDeduplicatedResult(
	...
	if (resolved.stage === "final") {
		payload.pending_approval_path = path.join(cwd, ".skc", "plans", "ralplan", resolved.runId, "pending-approval.md");
	}
	const stdout = resolved.json
		? `${JSON.stringify(payload, null, 2)}\n`
...
async function seedRalplanState(
	...
	const stateDir = resolved.sessionId
		? path.join(cwd, ".skc", "state", "sessions", encodeSessionSegment(resolved.sessionId))
		: path.join(cwd, ".skc", "state");
