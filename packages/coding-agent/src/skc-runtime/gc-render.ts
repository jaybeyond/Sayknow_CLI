...
export function buildGcReportText(report: GcReport): string {
	...
	if (report.operation === "repair_session_index") {
		lines.push("skc gc — session-index repair (other stores are report-only)");
	} else {
	...
	if (report.session_index) {
		...
		if (index.status === "corrupt")
			lines.push("  Run `skc gc --repair-session-index` to quarantine the corrupt suffix.");
		if (index.status === "unsupported")
			lines.push("  Upgrade SKC before attempting a repair; no index data was changed.");
