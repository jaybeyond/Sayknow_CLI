...
function parseValidationBatchInput(
	...
	if (skcGoalMode !== "aggregate") throw new Error("validation batches require aggregate ultragoal mode");
...
function normalizePlan(raw: unknown): UltragoalPlan {
	...
	const skcGoalMode = record.skcGoalMode === "per-story" ? "per-story" : "aggregate";
	const skcObjective = nonEmptyString(record.skcObjective) ?? DEFAULT_ULTRAGOAL_OBJECTIVE;
	...
	const aliases = Array.isArray(record.skcObjectiveAliases)
		? record.skcObjectiveAliases.filter(
...
export async function createUltragoalPlan(input: {
	...
	const validationBatches =
		validationBatchInput === undefined
			? []
			: parseValidationBatchInput(validationBatchInput, goalIds, input.skcGoalMode ?? "aggregate");
...
export function getUltragoalRunCompletionState(
	...
	return {
		...
		needsFinalAggregateReceipt: plan.skcGoalMode === "aggregate" && incompleteGoals.length === 0,
	};
}
...
export function categorizeComputerChangePath(value: string): UltragoalChangeCategory {
	...
	if (
		...
		normalized === "packages/coding-agent/src/defaults/skc/skills/ultragoal/SKILL.md" ||
...
async function readOptionalExecutorQa(cwd: string, value: string | undefined): Promise<JsonObject> {
	if (!value) {
		return {
			...
			e2eCommands: ["skc ultragoal review"],
			redTeamCommands: ["skc ultragoal review"],
			artifactRefs: [],
			contractCoverage: [],
			surfaceEvidence: [],
			adversarialCases: [],
...
function renderUltragoalHelp(args: readonly string[]): string | null {
	...
	if (subject === "checkpoint") {
		return [
			...
			"",
			"USAGE",
			"  $ skc ultragoal checkpoint --goal-id <id> --status <status> --evidence <text> [FLAGS]",
			"",
			"FLAGS",
			"      --goal-id=<value>            Durable .skc/ultragoal goal id, e.g. G001",
	...
	if (subject === "review") {
		return [
			...
			"",
			"USAGE",
			"  $ skc ultragoal review [--pr <n> | --branch <ref>] [--spec <path>] [--executor-qa-json <json-or-path>] [FLAGS]",
			"",
			"FLAGS",
...
function renderCheckpointContinuation(
	...
	if (status === "complete") {
		if (result.allComplete) {
			...
			lines.push(`SKC objective: ${result.plan.skcObjective}`);
			lines.push(
				result.startedNext
					? "The next ultragoal goal is active; continue the current aggregate SKC goal and checkpoint this story when verified."
					: "Run `skc ultragoal complete-goals` to activate the next ultragoal story.",
			);
		}
	} else if (status === "failed") {
		lines.push("Resume failed goals with `skc ultragoal complete-goals --retry-failed` after the blocker is fixed.");
 ...
 * ultragoal mode-state + active-state/HUD so `skc state ultragoal read`, the
 * skill-tool chain guard, and the HUD chip mirror the plan/ledger. Session scope
 * follows `skc state` (`SKC_SESSION_ID`). This is a derived repair: it never changes
