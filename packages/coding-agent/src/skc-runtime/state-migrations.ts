...
function canonicalSkillOrThrow(skill: string): CanonicalSkcWorkflowSkill {
	const canonical = canonicalWorkflowSkill(skill);
	if (!canonical) throw new Error(`Unsupported SKC workflow skill: ${skill}`);
	return canonical;
}
...
function legacyPhaseForSkill(skill: CanonicalSkcWorkflowSkill, phase: string): string {
	if (phase === "planning") return initialPhaseForSkill(skill);
	return phase;
}

function normalizePhase(skill: CanonicalSkcWorkflowSkill, value: unknown): string {
...
function receiptWithRequiredFields(raw: unknown, skill: CanonicalSkcWorkflowSkill): Record<string, unknown> {
	const receipt =
	...
	receipt.skill = skill;
	if (receipt.owner !== "skc-state-cli" && receipt.owner !== "skc-runtime" && receipt.owner !== "skc-hook") {
		receipt.owner = "skc-state-cli";
	}
...
function migrateV1ToV2(state: Record<string, unknown>, skill: CanonicalSkcWorkflowSkill): Record<string, unknown> {
 ...
 * the only path allowed to write normalized upgrades back to `.skc/state/**`.
 */
...
export async function migrateAndPersistLegacyState(
	...
	const persistedPath = await writeWorkflowEnvelopeAtomic(args.statePath, state, {
		...
		receipt: {
			...
			owner: "skc-state-cli",
			command: `skc state ${canonicalSkill} migrate`,
