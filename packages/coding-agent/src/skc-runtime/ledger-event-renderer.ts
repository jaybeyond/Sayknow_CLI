 ...
 * events (those skills persist through `bash`-backed `skc` CLI calls whose tool
 * `details` carry no structured payload). The durable source of truth is the
 * append-only ledgers:
 *   - ultragoal: `.skc/ultragoal/ledger.jsonl`
 *   - ralplan:   `.skc/plans/ralplan/<run-id>/index.jsonl`
 *
