 ...
 *   structural, not a convention: `resolveSkcTarget` confines every sanctioned
 *   write to `<cwd>/.skc/**`, and adoption runs with `cwd` at the target.
 ...
 *   activated `skcObjective` all stay behind as provenance. The successor plan is
 ...
 * worktrees keep separate `.skc` state; and there is deliberately no `revoke`
...
export const ULTRAGOAL_SUCCESSION_OPERATION_SCHEMA = "skc.ultragoal_succession_operation.v1" as const;
export const ULTRAGOAL_SUCCESSION_OFFER_SCHEMA = "skc.ultragoal_succession_offer.v1" as const;
export const ULTRAGOAL_SUCCESSION_FENCE_SCHEMA = "skc.ultragoal_succession_fence.v1" as const;
export const ULTRAGOAL_SUCCESSION_ADOPTION_SCHEMA = "skc.ultragoal_succession_adoption.v1" as const;
export const ULTRAGOAL_SUCCESSION_CLAIM_SCHEMA = "skc.ultragoal_succession_claim.v1" as const;
...
export function ultragoalSuccessionClaimPath(cwd: string, operationId: string): string {
	return path.join(skcRoot(cwd), "ultragoal", SUCCESSION_DIR_NAME, "claims", `${operationId}.json`);
}
...
export async function assertUltragoalGoalNotFenced(
	...
	throw new UltragoalSuccessionError(
		...
		`Goal ${goalId} was handed off to a successor run in ${fence.targetRepository.worktreeRoot} by succession ${fence.operationId}; ` +
			...
			"Run `skc ultragoal succession status` for the recorded operation.",
	);
}
...
export async function assertUltragoalAdoptionPublished(
	...
	throw new UltragoalSuccessionError(
		...
		`This run was adopted from ${adoption.source.repository.worktreeRoot} by succession ${adoption.operationId}, but its ` +
			...
			"`skc ultragoal succession adopt` with the same offer to reconcile the recorded operation.",
	);
}
...
async function offerUnderSourceOwnership(
	...
	const auditContext = {
		cwd: input.cwd,
		audit: { category: "artifact" as const, verb: "write", owner: "skc-runtime" as const, sessionId },
	};
	try {
...
export async function adoptUltragoalSuccession(
	...
	// `.skc`, so two admissible worktrees could each take a local claim for one
	...
	if (path.resolve(targetActual.worktreeRoot) !== path.resolve(targetDeclared.worktreeRoot)) {
		throw new UltragoalSuccessionError(
			...
			`This worktree (${targetActual.worktreeRoot}) shares a repository with the named target ` +
				`(${targetDeclared.worktreeRoot}) but is not that worktree. Linked worktrees keep separate .skc state, so ` +
...
async function publishAdoptionUnderTargetOwnership(
	...
	const auditContext = {
		cwd: input.cwd,
		audit: { category: "state" as const, verb: "write", owner: "skc-runtime" as const, sessionId },
	};
	...
	await appendJsonlIdempotent(
		...
		{
			cwd: input.cwd,
			audit: { category: "ledger", verb: "append", owner: "skc-runtime", sessionId },
			key: entry =>
...
async function renderSuccessionStatus(cwd: string, json: boolean): Promise<UltragoalCommandResult> {
	const sessionId = (await resolveSkcSessionForRead(cwd, { envSessionId: process.env.SKC_SESSION_ID })).skcSessionId;
...
export async function runUltragoalSuccessionCommand(
	...
	try {
		...
		if (subcommand === "adopt") {
			const mode = flagValue(args, "--skc-goal-mode");
			const result = await adoptUltragoalSuccession({
				cwd,
				offerPath: flagValue(args, "--offer") ?? "",
				skcGoalMode: mode === "per-story" || mode === "aggregate" ? mode : undefined,
			});
			return {
				status: 0,
		...
		return {
			status: 1,
			stderr: `Unknown skc ultragoal succession subcommand: ${subcommand ?? "(missing)"}; supported: offer, adopt, status\n`,
		};
	} catch (error) {
