...
export function withTeamLaunchTransport(
	options: SkcTeamStartOptions,
	mailboxDeliveryTransport: SkcTeamStartOptions["mailboxDeliveryTransport"],
): SkcTeamStartOptions {
...
export function buildWorkerCommand(
	...
	const envLines = [
		...
		...(config.skc_session_id ? [envAssignment("SKC_SESSION_ID", config.skc_session_id)] : []),
...
export interface SkcTeamLaunchRuntime {
	maxWorkers: number;
	resolveWorkerCliPlan(workerCount: number, env: NodeJS.ProcessEnv): SkcTeamWorkerCli[];
	...
	resolveDefaultWorktreeMode(mode?: SkcTeamWorktreeMode): SkcTeamWorktreeMode;
	resolveTmuxBinary(input: { env: NodeJS.ProcessEnv; platform: NodeJS.Platform }): SkcTmuxBinary;
	readTmuxLeaderContext(tmuxCommand: string, env: NodeJS.ProcessEnv): SkcTmuxLeaderContext;
	buildWorkers(workerCount: number, agentType: string, stateRoot: string): SkcTeamWorker[];
	buildInitialTasks(task: string, workers: SkcTeamWorker[]): SkcTeamTask[];
	...
	rollbackCreatedWorktrees(workers: SkcTeamWorker[]): Promise<void>;
	...
	writeTask(dir: string, task: SkcTeamTask): Promise<void>;
	...
	writeWorkerLifecycleForConfig(
		...
		config: SkcTeamConfig,
		state: "starting",
		updates: (worker: SkcTeamWorker) => Partial<SkcTeamWorkerLifecycle>,
	): Promise<unknown>;
...
export async function startSkcTeamLaunch(
	runtime: SkcTeamLaunchRuntime,
	options: SkcTeamStartOptions,
): Promise<SkcTeamSnapshot> {
	const cwd = options.cwd ?? process.cwd();
	const env = options.env ?? process.env;
	const skcSessionId = resolveSessionIdFromSources({ envSessionId: env.SKC_SESSION_ID })?.skcSessionId;
	...
	const config: SkcTeamConfig = {
		...
		...(skcSessionId ? { skc_session_id: skcSessionId } : {}),
		...
		leader: {
			session_id: env.SKC_SESSION_ID ?? env.CODEX_SESSION_ID ?? "",
