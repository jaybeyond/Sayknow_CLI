...
export type SkcTeamWorkerMemoryGuardState = "idle" | "advisory" | "retrying" | "checkpointed" | "replaced" | "blocked";

export type SkcTeamWorkerMemoryGuardCheckpointKind =
	| "clean"
	| "eligible"
	| "protected_only"
	| "conflicted"
...
export type SkcTeamWorkerMemoryGuardPidProbe =
	| { kind: "live"; start_time: string }
	| { kind: "absent" }
	| { kind: "unverifiable"; reason: string };

export interface SkcTeamWorkerMemoryGuardCheckpoint {
	kind: SkcTeamWorkerMemoryGuardCheckpointKind;
	files: string[];
	head?: string | null;
	commit?: string | null;
	recorded_at: string;
...
export interface SkcTeamWorkerMemoryGuardReplacement {
	old_pane_id?: string;
	new_pane_id?: string;
	recorded_at: string;
}

export interface SkcTeamWorkerMemoryGuardLedger {
	schema_version: 1;
	worker_id: string;
	platform: string;
	state: SkcTeamWorkerMemoryGuardState;
	...
	last_reason?: string;
	last_pid_probe?: SkcTeamWorkerMemoryGuardPidProbe;
	last_checkpoint?: SkcTeamWorkerMemoryGuardCheckpoint;
	last_replacement?: SkcTeamWorkerMemoryGuardReplacement;
	updated_at: string;
}

export interface SkcTeamWorkerMemoryGuardSelectionCandidate {
	worker_id: string;
	platform: string;
	excess_bytes: number;
	retry_count: number;
...
export interface SkcTeamWorkerMemoryGuardSelection {
	worker_id: string;
	excess_bytes: number;
	retry_count: number;
	current_task_id?: string;
...
function isWorkerMemoryGuardState(value: unknown): value is SkcTeamWorkerMemoryGuardState {
	return ["idle", "advisory", "retrying", "checkpointed", "replaced", "blocked"].includes(String(value));
}

function isCheckpointKind(value: unknown): value is SkcTeamWorkerMemoryGuardCheckpointKind {
...
export function normalizeSkcTeamWorkerMemoryGuardPidProbe(
	value: unknown,
): SkcTeamWorkerMemoryGuardPidProbe | undefined {
...
export function isCanonicalSkcTeamWorkerMemoryGuardCheckpoint(
	value: unknown,
): value is SkcTeamWorkerMemoryGuardCheckpoint {
	return (
		isRecord(value) &&
...
export function isCanonicalSkcTeamWorkerMemoryGuardReplacement(
	value: unknown,
): value is SkcTeamWorkerMemoryGuardReplacement {
	return (
		isRecord(value) &&
...
export function isCanonicalSkcTeamWorkerMemoryGuardPidProbe(value: unknown): value is SkcTeamWorkerMemoryGuardPidProbe {
	try {
		return normalizeSkcTeamWorkerMemoryGuardPidProbe(value) !== undefined;
	} catch {
		return false;
	}
}

export function isCanonicalSkcTeamWorkerMemoryGuardLedger(value: unknown): value is SkcTeamWorkerMemoryGuardLedger {
	return (
		isRecord(value) &&
		...
		(value.last_pid_probe === undefined || isCanonicalSkcTeamWorkerMemoryGuardPidProbe(value.last_pid_probe)) &&
		(value.last_checkpoint === undefined || isCanonicalSkcTeamWorkerMemoryGuardCheckpoint(value.last_checkpoint)) &&
		(value.last_replacement === undefined ||
			isCanonicalSkcTeamWorkerMemoryGuardReplacement(value.last_replacement)) &&
		isTimestamp(value.updated_at)
	);
}

export function createInitialSkcTeamWorkerMemoryGuardLedger(input: {
	workerId: string;
	platform: string;
	now: string;
	retryLimit?: number;
}): SkcTeamWorkerMemoryGuardLedger {
...
export function selectSkcTeamWorkerMemoryGuardCandidate(
	candidates: readonly SkcTeamWorkerMemoryGuardSelectionCandidate[],
): SkcTeamWorkerMemoryGuardSelection | undefined {
