/**
 * Provenance and lifecycle primitives for tmux-resident SKC owners.
...
export function tmuxOwnerIsolationBootstrapArgv(): string[] {
	...
	return [process.execPath, path.resolve(import.meta.dir, "../../bin/skc.js"), TMUX_OWNER_ISOLATION_CLI_FLAG];
}
...
export function planTmuxOwnerIsolationSync(request: PlanRequest, probe: OwnerIsolationProbeSync): PlanResponse {
	...
	try {
		...
		const expectedScope = `skc-owner-${token}.scope`;
...
export async function bootstrapTmuxOwnerIsolation(
	...
	try {
		...
		if (
			...
			request.expected_scope !== `skc-owner-${request.attempt.token}.scope`
		)
			return {
				schema_version: 1,
				ok: false,
