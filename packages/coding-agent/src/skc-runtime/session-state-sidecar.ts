...
export const SKC_TMUX_OWNER_GENERATION_ENV = "SKC_TMUX_OWNER_GENERATION";
export const SKC_TMUX_OWNER_STATE_DIR_ENV = "SKC_TMUX_OWNER_STATE_DIR";
export const SKC_TMUX_OWNER_SERVER_KEY_ENV = "SKC_TMUX_OWNER_SERVER_KEY";
...
export const SKC_COORDINATOR_SESSION_LAUNCH_ID_ENV = "SKC_COORDINATOR_SESSION_LAUNCH_ID";
export const SKC_COORDINATOR_SESSION_READINESS_FILE_ENV = "SKC_COORDINATOR_SESSION_READINESS_FILE";
...
export async function persistCoordinatorRuntimeInputReady(): Promise<RuntimeInputReadyMarker | null> {
	...
	const sessionId = process.env[SKC_COORDINATOR_SESSION_ID_ENV]?.trim();
	const launchId = process.env[SKC_COORDINATOR_SESSION_LAUNCH_ID_ENV]?.trim();
	const readinessFile = process.env[SKC_COORDINATOR_SESSION_READINESS_FILE_ENV]?.trim();
...
function errorMessageForPostmortem(reason: postmortem.Reason): string {
	return publicSafeErrorMessage(`SKC process cleanup ran for ${reason}`);
}
				...
				async () =>
					await withStateFileLock(stateFile, async () => {
						...
						const payload = {
							...
							...(state === "errored"
								? { error: { code: "agent_error", message: "SKC agent reported an error", recoverable: true } }
								: {}),
						};
...
export function ownerTerminalContextFromEnvironment(): OwnerTerminalContext | "invalid" | null {
	const generation = process.env[SKC_TMUX_OWNER_GENERATION_ENV];
	const stateDir = process.env[SKC_TMUX_OWNER_STATE_DIR_ENV];
	const socketKey = process.env[SKC_TMUX_OWNER_SERVER_KEY_ENV];
	const supplied = [generation, stateDir, socketKey].some(value => value !== undefined);
	const managedLaunch = process.platform === "linux" && process.env.SKC_TMUX_LAUNCHED === "1";
...
async function persistInvalidOwnerTerminalMetadata(
	...
	await writeStateFileSync(stateFile, {
		...
		error: {
			...
			message: "SKC managed tmux owner metadata was unavailable or invalid",
			recoverable: true,
		},
		recovery: {
			action: "recover_or_resume_session",
	...
	try {
			...
			...(expected
				...
				: {
						error: {
							...
							message: "SKC owner terminal verdict requires session recovery",
							recoverable: true,
						},
						recovery: {
							action: "recover_or_resume_session",
		...
		await writeStateFileSync(stateFile, {
			...
			error: {
				...
				message: "SKC owner terminal verdict was unavailable",
				recoverable: true,
			},
			recovery: {
				action: "recover_or_resume_session",
