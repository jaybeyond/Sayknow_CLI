/**
 * Canonical zod schemas for SKC workflow state (Workstream A, v4).
 *
...
const CANONICAL_SKC_WORKFLOW_SKILLS = ["deep-interview", "ralplan", "ultragoal", "team"] as const;
const skillEnum = z.enum([...CANONICAL_SKC_WORKFLOW_SKILLS]);
const ownerEnum = z.enum(["skc-state-cli", "skc-runtime", "skc-hook"]);
 ...
 */
export type ReadSkcJsonResult<T> = { ok: true; value: T; raw: unknown } | { ok: false; error: string; raw: unknown };
...
/**
 * Parse + schema-validate a `.skc` JSON file at the read boundary.
 ...
 */
export async function readSkcJson<T>(filePath: string, schema: z.ZodType<T>): Promise<ReadSkcJsonResult<T> | null> {
	let text: string;
	try {
