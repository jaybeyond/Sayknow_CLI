...
export type SkcTeamMailboxDeliveryTransportKind = "sdk" | "pane";
...
export interface SkcTeamMailboxDeliveryInput {
	team_name: string;
	state_dir: string;
...
export type SkcTeamMailboxDeliveryResult =
	| {
			transport: "sdk";
			state: SkcTeamNotificationDeliveryState;
			reason?: string;
	  }
	| { transport: "pane"; state: SkcTeamPaneAttemptResult; reason?: string };
export interface SkcTeamMailboxDeliveryTransport {
	deliverMailboxMessage(input: SkcTeamMailboxDeliveryInput): Promise<SkcTeamMailboxDeliveryResult | null>;
}
...
export type SkcTeamTaskMetadataInput = Partial<
	Pick<SkcTeamTask, "owner" | "lane" | "required_role" | "allowed_roles" | "depends_on" | "blocked_by">
>;
export interface SkcTeamTaskWorker {
	id: string;
	role: string;
	agent_type: string;
}
...
export function isCanonicalPersistedSkcTeamTaskClaim(value: unknown): value is SkcTeamTaskClaim {
...
function isCanonicalCompletionEvidence(value: unknown): value is SkcTeamTaskCompletionEvidence {
...
export function isCanonicalPersistedSkcTeamTask(value: unknown, fileId?: string): value is SkcTeamTask {
	...
	if (value.claim !== undefined) {
		if (!isCanonicalPersistedSkcTeamTaskClaim(value.claim)) return false;
...
const writerOptions = (filePath: string, category: "state" | "prune", verb: string) => {
	...
	const sessionId =
		resolved
			...
			.find((value): value is string => Boolean(value)) ?? process.env.SKC_SESSION_ID?.trim();
	return sessionId
		? {
				cwd,
				audit: { category, verb, owner: "skc-runtime" as const, sessionId },
			}
		: { cwd };
};
 ...
 */
export async function withSkcTeamMutationFence<T>(dir: string, fn: () => Promise<T>): Promise<T> {
...
export function taskMetadataFromInput(input: Record<string, unknown>, includeOwner = false): SkcTeamTaskMetadataInput {
	const result: SkcTeamTaskMetadataInput = {};
...
export function normalizeSkcTeamTask(raw: SkcTeamTask): SkcTeamTask {
	return {
		...raw,
		status: raw.status === ("complete" as SkcTeamTaskStatus) ? "completed" : raw.status,
...
function verified(item: SkcTeamTaskCompletionEvidenceItem) {
	return (
		(item.kind === "command" && item.status === "passed") || (item.kind !== "command" && item.status === "verified")
	);
}
export function normalizeSkcTeamTaskCompletionEvidence(
	id: string,
	owner: string,
	input: unknown,
	recordedAt = now(),
	...
	const items = input.items.map(value => {
		...
		const status = required(id, "items.status", value.status) as SkcTeamTaskCompletionEvidenceStatus;
		if (
...
export function getSkcTeamTaskCompletionEvidenceFailure(task: SkcTeamTask): string | null {
...
export const isSkcTeamTaskCompletionVerified = (task: SkcTeamTask) =>
	getSkcTeamTaskCompletionEvidenceFailure(task) === null;
function taskRecord(value: unknown): value is SkcTeamTask {
	return (
		isRecord(value) &&
...
export async function readSkcTeamTasksFromDir(dir: string): Promise<SkcTeamTask[]> {
	try {
...
async function writeSkcTeamTaskToDir(dir: string, task: SkcTeamTask): Promise<void> {
	await writeJson(taskPath(dir, task.id), normalizeSkcTeamTask(task));
}

function eligibility(task: SkcTeamTask, worker: SkcTeamTaskWorker, tasks: SkcTeamTask[]): string | null {
	...
	for (const dependency of task.depends_on ?? [])
		if (!tasks.find(candidate => candidate.id === dependency && isSkcTeamTaskCompletionVerified(candidate)))
			return `task_dependency_incomplete:${task.id}:${dependency}`;
	return null;
}
function claimRecord(value: unknown): SkcTeamTaskClaim | undefined {
	if (
		!isRecord(value) ||
 ...
 * `withSkcTeamTaskMutation`, but cannot create an active capability. Each method
 * closes over a private brand and is revoked when that callback settles.
 */
export interface SkcTeamTaskMutationCapability {
	create(subject: string, description: string, options: SkcTeamTaskMetadataInput): Promise<SkcTeamTask>;
	update(
		id: string,
		updates: Partial<
			Pick<
	...
	claim(worker: SkcTeamTaskWorker, id?: string): Promise<SkcTeamApiClaimResult>;
	transition(
		id: string,
		status: SkcTeamTaskStatus,
		token?: string,
	...
	release(id: string, token: string, workerId: string): Promise<SkcTeamTask>;
	writeRecovered(task: SkcTeamTask): Promise<void>;
}

export async function withSkcTeamTaskMutation<T>(
	store: SkcTeamTaskStore,
	fn: (capability: SkcTeamTaskMutationCapability) => Promise<T>,
): Promise<T> {
	return withSkcTeamMutationFence(store.dir, () => store.withMutationCapability(teamTaskMutationCapability, fn));
}

export class SkcTeamTaskStore {
	constructor(
		readonly dir: string,
	...
	async withMutationCapability<T>(
		...
		const capability = Object.freeze<SkcTeamTaskMutationCapability>({
			...
			writeRecovered: task => track(() => writeSkcTeamTaskToDir(this.dir, task)),
		});
	...
	async list() {
		return readSkcTeamTasksFromDir(this.dir);
	}
	...
	async create(subject: string, description: string, options: SkcTeamTaskMetadataInput) {
		return withSkcTeamMutationFence(this.dir, () => this.#createUnlocked(subject, description, options));
	}
	async #createUnlocked(subject: string, description: string, options: SkcTeamTaskMetadataInput) {
		...
		await writeSkcTeamTaskToDir(this.dir, task);
	...
	async update(
		...
		return withSkcTeamMutationFence(this.dir, () => this.#updateUnlocked(id, updates));
	}
	async #updateUnlocked(
		...
		const updated = normalizeSkcTeamTask({
			...task,
			...updates,
		...
		await writeSkcTeamTaskToDir(this.dir, updated);
		await this.appendEvent({
			type: "task_updated",
			task_id: id,
	...
	async claim(worker: SkcTeamTaskWorker, id?: string): Promise<SkcTeamApiClaimResult> {
		return withSkcTeamMutationFence(this.dir, () => this.#claimUnlocked(worker, id));
	}
	async #claimUnlocked(worker: SkcTeamTaskWorker, id?: string): Promise<SkcTeamApiClaimResult> {
		...
		if (currentReason || current.status !== "pending") {
			await deleteIfOwned(claimPath(this.dir, task.id), {
				...
				predicate: current => (current as SkcTeamTaskClaim).token === claim.token,
			});
			return {
				ok: false,
	...
	async transition(id: string, status: SkcTeamTaskStatus, token?: string, workerId?: string, evidenceInput?: unknown) {
		return withSkcTeamMutationFence(this.dir, () =>
	...
	async #transitionUnlocked(
		...
		const evidence =
			status === "completed"
				? normalizeSkcTeamTaskCompletionEvidence(id, task.claim.owner, evidenceInput, transitionedAt)
				: undefined;
	...
	async release(id: string, token: string, workerId: string) {
		return withSkcTeamMutationFence(this.dir, () => this.#releaseUnlocked(id, token, workerId));
	}
