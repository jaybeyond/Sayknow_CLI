/**
 * TypeScript is the authoritative source of truth for SKC workflow manifests.
...
const COMMON_TYPED_ARGS: TypedArgSpec[] = [
	...
	{ name: "mode", type: "enum", enumValues: [...CANONICAL_SKC_WORKFLOW_SKILLS], appliesToVerbs: [...STATE_VERBS] },
	...
	{ name: "turn-id", type: "string", appliesToVerbs: ["write", "clear", "handoff"] },
	{
		name: "to",
		type: "enum",
		enumValues: [...CANONICAL_SKC_WORKFLOW_SKILLS],
		required: true,
	...
	{ name: "skill", type: "enum", enumValues: [...CANONICAL_SKC_WORKFLOW_SKILLS], appliesToVerbs: ["doctor"] },
...
export const WORKFLOW_MANIFEST: Record<CanonicalSkcWorkflowSkill, SkillManifest> = {
...
export function getSkillManifest(skill: CanonicalSkcWorkflowSkill): SkillManifest {
	return WORKFLOW_MANIFEST[skill];
}

export function isKnownWorkflowState(skill: CanonicalSkcWorkflowSkill, state: string): boolean {
	return WORKFLOW_MANIFEST[skill].states.some(entry => entry.id === state);
}

export function isValidTransition(skill: CanonicalSkcWorkflowSkill, from: string, to: string): boolean {
	if (from === to) return true;
	return WORKFLOW_MANIFEST[skill].transitions.some(transition => transition.from === from && transition.to === to);
}

export function listVerbs(skill: CanonicalSkcWorkflowSkill): string[] {
	return WORKFLOW_MANIFEST[skill].verbs.map(verb => verb.name);
}

export function typedArgsFor(skill: CanonicalSkcWorkflowSkill, verb: string): TypedArgSpec[] {
