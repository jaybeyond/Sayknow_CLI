/**
 * `skc gc` runtime — a global, liveness-only, dry-run-by-default garbage
 * collector for stale SKC session/PID records.
 *
 * Design (see .skc/plans/ralplan/2026-06-13-1347-954f/pending-approval.md):
...
function resolveGcAgentDir(env: NodeJS.ProcessEnv): string {
	return env.SKC_CODING_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim() || getAgentDir();
}
...
export async function runSkcGcCommand(
	argv: string[],
	...
	try {
		...
		return { stdout: "", stderr: `skc gc: ${message}\n`, status: 2 };
	}
...
export function gcHelpText(): string {
	return [
		"skc gc - garbage-collect stale SKC session/PID records",
		"",
		"USAGE",
		"  $ skc gc [--prune|--force] [--repair-session-index] [--json]",
