...
/**
 * Sole sanctioned project `.skc/**` writer module (gate G1).
 *
 * All native `.skc/**` filesystem mutations must route through these primitives.
 * The primitives validate project `.skc/**` ownership, create parent directories,
...
export interface StateWriterAuditContext {
	...
	verb: string;
	owner: WorkflowStateMutationOwner;
	skill?: CanonicalSkcWorkflowSkill | string;
	mutationId?: string;
	fromPhase?: string;
	toPhase?: string;
	forced?: boolean;
...
function resolveSkcTarget(targetPath: string, cwd = process.cwd()): string {
	if (!targetPath.trim()) throw new Error("targetPath is required");
	const projectRoot = path.resolve(cwd);
	const skcRoot = path.join(projectRoot, ".skc");
	const resolved = path.resolve(projectRoot, targetPath);
	const relative = path.relative(skcRoot, resolved);
	if (relative === "" || relative.startsWith("..") || path.isAbsolute(relative)) {
		throw new Error(`target path must be within project .skc/**: ${targetPath}`);
	}
	return resolved;
}
...
async function recordInvalidWorkflowTransition(args: {
	...
	try {
		await appendAuditEntry(cwd, options?.audit?.sessionId ?? "", {
			...
			owner: options?.audit?.owner ?? "skc-runtime",
 ...
 * writers of the same `.skc/**` target. Uses the cross-process directory lock
...
export async function writeArtifact(
	...
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "artifact", verb: "write", owner: "skc-runtime" },
	});
}
...
export async function writeReport(targetPath: string, content: string, options?: StateWriterOptions): Promise<string> {
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "report", verb: "write", owner: "skc-runtime" },
	});
}
...
export async function writeLogJsonl(targetPath: string, entry: unknown, options?: StateWriterOptions): Promise<string> {
	return appendJsonl(targetPath, entry, {
		...options,
		audit: options?.audit ?? { category: "log", verb: "append", owner: "skc-runtime" },
	});
}
...
export async function softDelete(
	...
	return updateJsonAtomic<Record<string, unknown>>(
		...
		{
			...options,
			audit: options?.audit ?? { category: "prune", verb: "soft-delete", owner: "skc-runtime" },
		},
	);
}
...
export async function hardPrune(
	...
	for (const target of targets) {
		const filePath = resolveSkcTarget(target.path, cwd);
		let stat: Stats;
		try {
...
export async function forceOverwrite(
	...
	const auditOptions = {
		...options,
		audit: options?.audit ?? { category: "force", verb: "force-overwrite", owner: "skc-state-cli", forced: true },
	};
	if (options?.raw === true) {
		const filePath = resolveSkcTarget(targetPath, cwdForOptions(options));
