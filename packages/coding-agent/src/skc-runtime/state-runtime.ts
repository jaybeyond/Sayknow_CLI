...
// Subpath import keeps this module native-free for the skc-state-gates shards:
// the package barrel pulls procmgr/ptree → @sayknow-cli/natives.
 ...
 * `.skc/_session-{id}/state/`. This is the sanctioned CLI mediator for
 * mutation-guarded SKC state — agents call it instead of editing those files directly.
 */
...
function assertKnownMode(mode: string): asserts mode is CanonicalSkcWorkflowSkill {
	if (!isKnownMode(mode)) {
...
interface ResolvedSelectors {
	mode: CanonicalSkcWorkflowSkill | undefined;
	skcSessionId: string;
...
async function resolveSelectors(args: readonly string[], cwd: string, action: StateAction): Promise<ResolvedSelectors> {
	...
	const session = WRITE_SESSION_ACTIONS.has(action)
		? resolveSkcSessionForWrite(cwd, sessionSources)
		: await resolveSkcSessionForRead(cwd, sessionSources);
	...
	return {
		mode: mode as CanonicalSkcWorkflowSkill | undefined,
		skcSessionId: session.skcSessionId,
		threadId,
		turnId,
		payload,
	};
...
async function inferModeFromActiveState(
	cwd: string,
	sessionId: string,
): Promise<CanonicalSkcWorkflowSkill | undefined> {
...
async function readActivePhaseForSkill(
	cwd: string,
	sessionId: string,
	mode: CanonicalSkcWorkflowSkill,
 ...
 * {@link StateCommandResult.stderr} channel, so `skc state` automation still
...
async function collectDoctorSummary(
	cwd: string,
	skill: CanonicalSkcWorkflowSkill | undefined,
	sessionId: string,
): Promise<DoctorSummary> {
	const root = sessionStateDir(cwd, sessionId);
	const skills = skill ? [skill] : [...CANONICAL_SKC_WORKFLOW_SKILLS];
		...
		if (raw.error) {
			problems.push(
				doctorProblem(
					...
					`skc state ${currentSkill} migrate`,
					currentSkill,
				),
			);
			...
			if (activeFlag(entry.value) && (!state.exists || !activeFlag(state.value))) {
				problems.push(
					doctorProblem(
						...
						canonical ? `skc state ${canonical} clear` : "skc state prune --hard",
						canonical ?? undefined,
					),
				);
			}
...
async function handleDoctor(
	...
	const session = await resolveSkcSessionForRead(cwd, {
		flagValue: flagValue(args, "--session-id"),
		payloadSessionId: payload?.session_id,
		envSessionId: process.env.SKC_SESSION_ID,
	});
	const summary = await collectDoctorSummary(
		cwd,
		rawSkill as CanonicalSkcWorkflowSkill | undefined,
		session.skcSessionId,
	);
	return {
...
async function writeJsonAtomic(
	...
	const writeResult = await writeGuardedWorkflowEnvelopeAtomic(filePath, value, {
		...
		audit: {
			...
			category: "state",
			verb,
			owner: options?.owner ?? "skc-state-cli",
			skill: options?.skill,
			mutationId: options?.mutationId,
...
function parseLimitFlag(args: readonly string[], defaultLimit = 50): number {
	...
	if (!Number.isInteger(parsed) || parsed <= 0 || parsed > 500) {
		throw new StateCommandError(2, "skc state --limit requires an integer from 1 to 500");
	}
	return parsed;
}
...
function parseSinceFlag(args: readonly string[]): string | undefined {
	...
	if (Number.isNaN(Date.parse(raw)))
		throw new StateCommandError(2, "skc state --since requires an ISO timestamp or duration like 30m, 6h, 7d");
 ...
 * payload. Unlike `skc state write`, this is a derived repair: callers reconcile from
 ...
 * detection. Receipts carry `owner: "skc-runtime"` and `verb: "reconcile"` so the
...
export async function readWorkflowStateJson(
	...
	return (await readJsonFile(modeStateFile(cwd, skill, session.skcSessionId), onWarning)) ?? {};
}
...
async function handleRead(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	// path also surfaces them on the command result so `skc state read`
	...
	if (mode) {
		const filePath = modeStateFile(cwd, mode, selectors.skcSessionId);
		const existing = await readWorkflowStateJson(cwd, mode, selectors.skcSessionId, warning =>
	...
	const filePath = activeStateFile(cwd, selectors.skcSessionId);
...
async function handleStatus(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"skc state status requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.skcSessionId);
	const warnings: string[] = [];
	const existing = await readWorkflowStateJson(cwd, mode, selectors.skcSessionId, warning => warnings.push(warning));
...
async function handleClear(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode)
		throw new StateCommandError(
			2,
			"skc state clear requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
 ...
 *     `skc state <skill> write` are declaring "I am ready to be handed off";
...
async function handleHandoffUnlocked(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!caller) {
		throw new StateCommandError(
			2,
			"skc state handoff requires --mode <caller>, positional <caller>, input.skill, or an active workflow in the current session active state",
		);
	}
	const calleeRaw = flagValue(args, "--to")?.trim();
	if (!calleeRaw) {
		throw new StateCommandError(2, "skc state handoff requires --to <callee>");
	}
	assertSafePathComponent(calleeRaw, "to");
	const callee = calleeRaw;
	const calleeIsWorkflow = isKnownMode(callee);
	if (callee === caller) {
		throw new StateCommandError(2, `skc state handoff: --to must differ from caller (both are "${caller}")`);
	}
	...
	if (callerRead.kind === "absent") {
		throw new StateCommandError(
			2,
			`skc state ${caller} handoff: caller is not active (no mode-state file at ${callerPath})`,
		);
	}
	...
	const callerReceipt = buildWorkflowStateReceipt({
		...
		skill: caller,
		owner: "skc-state-cli",
		command: `skc state ${caller} handoff --to ${callee}`,
		sessionId,
		nowIso: handoffAt,
		mutationId,
	});
	...
	if (!calleePath) {
		throw new StateCommandError(2, `skc state handoff failed to resolve workflow callee path for ${callee}`);
	}
	...
	if (process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER === mutationId) {
...
function parseNonNegativeIntegerFlag(args: readonly string[], flag: string): number | undefined {
	...
	if (!Number.isInteger(parsed) || parsed < 0) {
		throw new StateCommandError(2, `skc state ${flag} requires a non-negative integer value`);
	}
	return parsed;
}
...
interface GcSummary {
	skill: CanonicalSkcWorkflowSkill | "all";
	dry_run: boolean;
	eligible: string[];
	pruned: string[];
...
async function collectRetentionCandidates(
	cwd: string,
	sessionId: string,
	skills: readonly CanonicalSkcWorkflowSkill[],
...
async function buildGcSummary(
	...
	const skills = rawSkill === "all" ? CANONICAL_SKC_WORKFLOW_SKILLS : [rawSkill as CanonicalSkcWorkflowSkill];
	const session = await resolveSkcSessionForRead(cwd, {
		flagValue: flagValue(args, "--session-id"),
		envSessionId: process.env.SKC_SESSION_ID,
	});
	const eligible = selectRetentionEligible(await collectRetentionCandidates(cwd, session.skcSessionId, skills));
	const counts: Record<string, number> = {};
	...
	if (!dryRun && targets.length > 0) {
		...
		pruned = await hardPrune(targets, context => eligiblePaths.has(path.resolve(context.path)), {
			...
			audit: {
				cwd,
				sessionId: session.skcSessionId,
				skill: rawSkill,
				category: "prune",
				verb: "gc",
				owner: "skc-state-cli",
	...
	return {
		skill: rawSkill as CanonicalSkcWorkflowSkill | "all",
		dry_run: dryRun,
		eligible: eligible.map(candidate => candidate.relativePath),
		pruned: pruned.map(filePath => path.relative(sessionStateDir(cwd, session.skcSessionId), filePath)),
		counts,
	};
}
...
async function handleGraph(
	...
	if (hasFlag(args, "--history")) {
		const session = await resolveSkcSessionForRead(_cwd, {
			flagValue: flagValue(args, "--session-id"),
			envSessionId: process.env.SKC_SESSION_ID,
		});
		const history = await readAuditWindow(_cwd, args, session.skcSessionId);
		return {
			status: 0,
	...
	return {
		status: 0,
		stdout: renderStateGraph(rawSkill as CanonicalSkcWorkflowSkill | "all", format as StateGraphFormat),
	};
}
...
async function handlePrune(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"skc state prune requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.skcSessionId);
	...
	const audit: StateWriterAuditContext = {
		cwd,
		sessionId: selectors.skcSessionId,
		skill: mode,
		category: "prune",
	...
	try {
		...
		if (await matchesSelector(stat, async () => JSON.parse(await fs.readFile(filePath, "utf-8")))) {
			const archivedPath = await softDelete(
				filePath,
				{ skill: mode, reason: "skc state prune", status: status ?? null, older_than_days: olderThanDays ?? null },
				{ cwd, audit },
			);
...
async function handleMigrate(args: readonly string[], cwd: string): Promise<StateCommandResult> {
	...
	if (!mode) {
		throw new StateCommandError(
			2,
			"skc state migrate requires --mode <skill>, positional <skill>, input.skill, or an active workflow in the current session active state",
		);
	}
	const filePath = modeStateFile(cwd, mode, selectors.skcSessionId);
	const forced = hasFlag(args, "--force");
	const mismatchWarning = await warnAndAuditOutOfBandIfNeeded(cwd, selectors.skcSessionId, filePath, mode, {
		forced,
	});
