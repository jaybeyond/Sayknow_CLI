...
function activeAskDiagnostic(input: {
	...
	return {
		...
		message: `${input.reason} Use \`skc ultragoal record-review-blockers\` instead of asking the user.`,
	};
}
...
function findReceiptGoal(
	...
	if (
		currentObjective === plan.skcObjective ||
...
export async function readUltragoalVerificationState(input: {
	...
	if (!plan) {
		if (isKnownUltragoalObjective(currentObjective) || (await hasDurableUltragoalState(input.cwd))) {
			return {
				state: "unreadable_fail_closed",
				message: "Active Ultragoal objective is missing durable .skc/ultragoal/goals.json state.",
			};
		}
	...
	if (runState.incompleteGoals.length > 0) {
		return {
			state: "active_missing_final_receipt",
			message: `Ultragoal still has incomplete required goals: ${runState.incompleteGoals.map(goal => goal.id).join(", ")}. Run \`skc ultragoal complete-goals\` to continue.`,
...
export async function isUltragoalAskBlocked(
	...
	if (sessionId === null) {
		return inactiveAskDiagnostic({
			reason: "No active SKC session resolved; ultragoal is inactive.",
			source: "absent",
	...
	try {
		...
		if (isEnoent(error)) {
			return inactiveAskDiagnostic({
				reason: "No durable .skc/ultragoal state exists.",
				source: "absent",
				goalsPath: paths.goalsPath,
				ledgerPath: paths.ledgerPath,
			});
		...
		return activeAskDiagnostic({
			reason: `Durable .skc/ultragoal state is present but unreadable: ${error instanceof Error ? error.message : String(error)}`,
			source: "durable_state_unreadable",
			goalsPath: paths.goalsPath,
			ledgerPath: paths.ledgerPath,
		});
	...
	if (!plan) {
		...
		return activeAskDiagnostic({
			reason: "Durable .skc/ultragoal state exists but goals.json is missing or empty.",
			source: "durable_state_unreadable",
			goalsPath: paths.goalsPath,
			ledgerPath: paths.ledgerPath,
		});
...
export async function isUltragoalPauseBlocked(cwd: string): Promise<UltragoalPauseBlockDiagnostic> {
	...
	if (terminalCriticCeilingReached(ledger) && !terminalCriticGateOverridden(ledger)) {
		return {
			blocked: true,
			reason:
				"The Ultragoal run hit the terminal-critic ceiling; requires human/leader `skc ultragoal record-critic-gate-override` before further terminal attempts.",
		};
	}
...
export async function assertUltragoalPauseAllowed(cwd: string): Promise<void> {
	...
	throw new Error(
		[
			...
			'If the blocker is genuinely human-only, record `skc ultragoal classify-blocker --classification human_blocked --evidence "<human-only dependency>"`, then record a clean bound `skc ultragoal record-critic-verdict --terminus pause --classification-event-id <eventId> --verdict OKAY --evidence "<critic evidence>"` before pausing.',
		].join("\n"),
	);
}
/**
