...
import { resolveSkcSessionForWrite, SessionResolutionError, writeSessionActivityMarker } from "./session-resolution";
...
/**
 * Staged JSON transitions for deep-interview state (`skc deep-interview stage|check|apply|discard`).
 *
 ...
 *   so no `--draft-id` is needed; the session resolves from `SKC_SESSION_ID` (or
 ...
 * (`--write`, `skc state handoff/clear`), never through a staged patch.
 */
...
async function readCurrentState(cwd: string, sessionId: string): Promise<CurrentState> {
	...
	if (read.kind === "corrupt") {
		throw new DeepInterviewStageError(
			...
			'repair or clear it with `skc state clear --force --mode deep-interview`, then re-seed with `skc deep-interview "<idea>"`',
		);
	}
	if (read.kind === "absent")
...
function computeMergedEnvelope(
	...
	try {
		...
		throw new DeepInterviewStageError(
			...
			"fix the payload and re-stage (`skc deep-interview discard` then `stage`)",
		);
	}
 ...
 * payload `session_id` (stage only), or `SKC_SESSION_ID`. Mutating verbs never
		...
		async () => {
			...
			if (existingDraft.kind === "valid") {
				throw new DeepInterviewStageError(
					...
					"apply it (`skc deep-interview apply`) or discard it (`skc deep-interview discard`) before staging again",
				);
			}
			...
			if (!current.exists && transition !== "initialize-context") {
				throw new DeepInterviewStageError(
					...
					'seed the interview first with `skc deep-interview "<idea>"` or stage --for initialize-context',
				);
			}
...
function requireDraftRead(read: DraftReadResult, sessionId: string): DeepInterviewStageDraft {
	if (read.kind === "absent") {
		throw new DeepInterviewStageError(
			...
			"stage one first: `skc deep-interview stage --for <transition> --input '<json>'`",
		);
	}
	if (read.kind === "corrupt") {
		throw new DeepInterviewStageError(
			...
			"discard it (`skc deep-interview discard`) and re-stage",
		);
	}
	return read.draft;
}
		...
		async () => {
			...
			if (draftIsStale(draft, current)) {
				...
				throw new DeepInterviewStageError(
					...
					"re-stage against current state: `skc deep-interview stage --for <transition> --input '<json>'`",
				);
			}
			const nowIso = new Date().toISOString();
			...
			try {
				const written = await writeGuardedWorkflowEnvelopeAtomic(statePath, merged, {
					...
					receipt: {
						...
						skill: "deep-interview",
						owner: "skc-runtime",
						command: `skc deep-interview apply (${draft.transition})`,
						sessionId,
						nowIso,
		...
		async () => {
			...
			if (pendingDraft.kind === "valid") {
				throw new DeepInterviewStageError(
					...
					"apply it (`skc deep-interview apply`) or discard it (`skc deep-interview discard`) first",
				);
			}
			const current = await readCurrentState(cwd, sessionId);
			const nowIso = new Date().toISOString();
			...
			const written = await writeGuardedWorkflowEnvelopeAtomic(statePath, merged, {
				...
				receipt: {
					...
					skill: "deep-interview",
					owner: "skc-runtime",
					command: `skc deep-interview write${reset ? " --reset" : ""}`,
					sessionId,
					nowIso,
...
async function syncStageHud(cwd: string, sessionId: string, envelope: Record<string, unknown>): Promise<void> {
	try {
		...
		await syncSkillActiveState({
			...
			phase,
			sessionId,
			source: "skc-deep-interview-native",
...
/** Thin lifecycle passthroughs: same runtime plumbing, skc deep-interview surface. */
			...
			error instanceof DeepInterviewStageError
				...
				: error instanceof SessionResolutionError
					? new DeepInterviewStageError(
							"DI_STAGE_SESSION_REQUIRED",
							error.message,
							"pass --session-id, set SKC_SESSION_ID, or include session_id in the staged payload, then retry",
						)
					: undefined;
		if (staged) {
			const body = json
