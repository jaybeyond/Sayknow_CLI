 ...
 * goal-arg execution, and resumable .skc/rlm/<session> artifacts.
 */
...
export function extractRlmFlags(argv: string[]): ExtractedRlmFlags {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (arg === "--data") {
			...
			const next = argv[i + 1];
			if (!next || next.startsWith("-")) throw new Error("skc rlm --resume requires an RLM session id.");
...
export async function runRlmCommand(argv: string[]): Promise<void> {
	...
	// `skc rlm "question"` still loads the TUI instead of being coerced into
	// print mode.
