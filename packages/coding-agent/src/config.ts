...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@sayknow-cli/utils";
...
export function getPackageDir(): string {
	...
	const envDir = process.env.SKC_PACKAGE_DIR ?? process.env.PI_PACKAGE_DIR;
	if (envDir) {
 ...
 * User-level: ~/.skc/agent, ~/.gemini
 * Project-level: .skc, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g. ".skc"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.skc/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.skc/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.skc/agent/commands", source: ".skc", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.skc, .Anthropic model) - the nearest one found.
