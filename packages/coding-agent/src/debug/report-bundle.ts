...
import type { WorkProfile } from "@sayknow-cli/natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@sayknow-cli/utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `skc-report-${timestamp}.tar.gz`);
