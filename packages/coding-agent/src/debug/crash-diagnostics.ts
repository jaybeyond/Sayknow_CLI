...
const CRASH_DIAGNOSTICS_ENV = "SKC_CRASH_DIAGNOSTICS";
const CRASH_DIAGNOSTICS_DIR_ENV = "SKC_CRASH_DIAGNOSTICS_DIR";
...
export function getCrashDiagnosticsDirectory(env: NodeJS.ProcessEnv = process.env): string {
	return env[CRASH_DIAGNOSTICS_DIR_ENV] ?? path.join(os.tmpdir(), "skc-crash-diagnostics");
}
