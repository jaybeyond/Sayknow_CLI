...
export class RootHelpCommand extends Command {
	...
	static flags = {
		...
		smol: Flags.string({ description: "Smol/fast model for lightweight tasks (or SKC_SMOL_MODEL env)" }),
		slow: Flags.string({ description: "Slow/reasoning model for thorough analysis (or SKC_SLOW_MODEL env)" }),
		plan: Flags.string({ description: "Plan model for architectural planning (or SKC_PLAN_MODEL env)" }),
...
/** Normalize the sole `skc resume` alias into the value-less launch intent. */
...
export async function runCli(argv: string[]): Promise<void> {
	...
	// (MACOS_MALLOC_ENV_VARS / SKC_MALLOC_ENV_REEXEC) loads only when a re-exec is due.
	if (
		process.platform === "darwin" &&
		process.env.SKC_MALLOC_ENV_REEXEC === undefined &&
