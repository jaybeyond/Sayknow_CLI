 ...
 */
const petOverlayEmitterOwners = new WeakMap<TUI, SayknowPetWidget>();
...
const IDLE_LOOP: Array<[SayknowPixelFrameName, number]> = [
	["base", 1100],
	["gazeL", 350],
	["base", 500],
	["gazeR", 350],
...
/**
 * The sayknow pet: a 16x16 real-pixel sprite living in a reserved area beside
...
export class SayknowPetWidget {
	...
	#mode: PetMode = "off";
	#pixel: SayknowPixelFrames | undefined;
	#frame: SayknowPixelFrameName = "base";
	...
	#applyMode(mode: PetMode, mountComposer: boolean): void {
		...
		const protocol = this.#forcedProtocol ?? SayknowPetWidget.pixelProtocol();
		if (!protocol) return;
	...
	#buildPixel(protocol: "sixel" | "kitty"): void {
		...
		this.#pixel = buildSayknowPixelFrames({
			protocol,
			skin,
	...
	#pickFrame(now: number): SayknowPixelFrameName {
