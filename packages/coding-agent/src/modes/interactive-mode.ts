...
import { type Agent, type AgentMessage, type AgentToolResult, ThinkingLevel } from "@sayknow-cli/agent-core";
...
import type { Component, EditorTheme, SlashCommand } from "@sayknow-cli/tui";
import {
	Container,
	clearRenderCache,
...
import { consumePendingGoalModeRequest } from "../skc-runtime/goal-mode-request";
...
export class InteractiveMode implements InteractiveModeContext {
	...
	petWidget: SayknowPetWidget | undefined;
	...
	#rebuildSkillSlashCommands(fileCommandNames: ReadonlySet<string> = new Set()): SlashCommand[] {
		...
		const defaultSkcNames = new Set<string>(DEFAULT_SKC_DEFINITION_NAMES);
		return resolvedCommands.map(command => ({
			name: command.name,
			description: command.description,
			// Pin the bundled SKC workflow skills above generic commands in autocomplete.
			...(defaultSkcNames.has(command.skill.name) ? { priority: 100 } : {}),
		}));
	}
	...
	#createPetWidget(editor: CustomEditor): SayknowPetWidget {
		return new SayknowPetWidget({
			ui: this.ui,
			editor,
