	...
	switch (event.type) {
		...
		case "model_fallback_switched":
			return [
				toSessionNotification(sessionId, {
					...
					_meta: {
						skcModelFallbackSwitched: true,
						skcModelFallbackEventId: event.eventId,
						skcModelFallbackFrom: event.from,
						skcModelFallbackTo: event.to,
						skcModelFallbackReason: event.reason,
						skcModelFallbackRole: event.role,
						skcModelFallbackScope: event.scope,
						skcModelFallbackActiveIndex: event.activeIndex,
						skcModelFallbackChainLength: event.chainLength,
						skcModelFallbackAttemptsUsed: event.attemptsUsed,
					},
				}),
			];
		...
		case "auto_retry_start":
			return [
				toSessionNotification(sessionId, {
					...
					_meta: {
						skcPhase: "retrying",
						skcRetryState: "waiting",
						skcRetryAttempt: event.attempt,
						skcRetryMaxAttempts: event.maxAttempts,
						skcRetryDelayMs: event.delayMs,
						skcRetryErrorMessage: event.errorMessage,
						skcRetryUnbounded: event.unbounded ?? false,
						running: true,
						skcRunning: true,
					},
				}),
		...
		case "auto_retry_end":
			return [
				toSessionNotification(sessionId, {
					sessionUpdate: "session_info_update",
					_meta: {
						skcPhase: event.success ? "responding" : "retry_failed",
						skcRetryState: event.success ? "succeeded" : "failed",
						skcRetryAttempt: event.attempt,
						...(event.finalError ? { skcRetryFinalError: event.finalError } : {}),
						running: true,
						skcRunning: true,
					},
				}),
		...
		case "ttsr_triggered":
			return [
				toSessionNotification(sessionId, {
					sessionUpdate: "session_info_update",
					_meta: {
						skcTtsrTriggered: true,
						skcTtsrRuleCount: event.rules.length,
					},
				}),
			];
		case "irc_message":
		...
		case "thinking_level_changed":
			return [
				toSessionNotification(sessionId, {
					sessionUpdate: "session_info_update",
					_meta: { skcThinkingLevel: event.thinkingLevel ?? "off" },
				}),
			];
		case "goal_updated":
			return [
				toSessionNotification(sessionId, {
					sessionUpdate: "session_info_update",
					_meta: {
						skcGoalActive: event.goal !== null,
						...(event.goal
							? {
									skcGoalId: event.goal.id,
									skcGoalStatus: event.goal.status,
									skcGoalObjective: event.goal.objective,
								}
							: {}),
						...(event.state ? { skcGoalModeState: event.state } : {}),
					},
				}),
			];
		default:
