...
export class AcpAgent implements Agent {
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.skc.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up Sayknow-CLI in terminal",
				description: "Launch the skc TUI to add provider keys and select models.",
				args: [ACP_TERMINAL_AUTH_FLAG],
			});
		}
		return {
			protocolVersion: PROTOCOL_VERSION,
			agentInfo: { name: "sayknow-cli", title: "Sayknow-CLI", version: packageJson.version },
			authMethods,
			agentCapabilities: {
				loadSession: true,
	...
	async extMethod(method: string, params: JsonObject): Promise<JsonObject> {
		...
		// payload. Recognized `_skc/*` methods keep their `{ok:false}` result contract.
		if (
			method !== "session/set_model" &&
			method !== "_skc/sdk/global" &&
			method !== "_skc/sdk/control" &&
			method !== "_skc/sdk/query"
		)
			...
			await this.#publishSessionUpdate(
				id,
				{
					...
					update: {
						...
						_meta: { skcPhase: "working", running: true, skcRunning: true },
					},
				},
				adapter,
			);
					...
					await this.#publishSessionUpdate(
						id,
						{
							...
							update: {
								...
								...(resolution.final.truncated ? { _meta: { skcFinalTextTruncated: true } } : {}),
							},
						},
						adapter,
					);
		...
		await this.#publishSessionUpdate(
			id,
			{
				...
				update: {
					...
					...(title ? { title } : {}),
					updatedAt,
					_meta: { skcPhase: "idle", running: false, skcRunning: false },
				},
			},
			adapter,
		);
		...
		if (modelPreset !== undefined) {
			...
			if (activePreset && activeProfile?.available === false) {
				record.authFailure =
					...
					"Authenticate the required provider in Sayknow-CLI or select an available preset before prompting.";
				...
				if (!imageLimitationReported) {
					...
					await this.#publishSessionUpdate(
						id,
						{
							...
							update: {
								sessionUpdate: "session_info_update",
								_meta: { skcTranscriptImageReplay: content.images },
							},
						},
						adapter,
					);
