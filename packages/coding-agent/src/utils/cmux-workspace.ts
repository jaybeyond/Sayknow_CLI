...
const CMUX_NO_RENAME_ENV = "SKC_NO_CMUX_RENAME";
const CONTROL_CHARS = /[\u0000-\u001f\u007f-\u009f]/g;
const CMUX_WORKSPACE_TITLE_PREFIX = "SKC: ";
...
export interface CmuxWorkspaceOwnership {
	/** cmux marks a workspace `has_custom_title` once an explicit title is set (by the user or by SKC). */
...
/** Only rename when SKC owns the name:
 ...
 * This makes SKC name a fresh workspace once and then leave it alone, so it
...
/**
 * Best-effort sync of the containing cmux workspace title to the current SKC
 * session name. Ownership-guarded: SKC reads the current workspace title and
 ...
 * CMUX_WORKSPACE_ID) set. Opt out with SKC_NO_CMUX_RENAME.
 */
