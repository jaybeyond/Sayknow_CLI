import { $pickenv, $pickflag } from "@sayknow-cli/utils";
...
const SIXEL_PLACEHOLDER_PREFIX = "__SKC_SIXEL_SEQUENCE_";
...
export function isSixelPassthroughEnabled(): boolean {
	const forcedProtocol = $pickenv("SKC_FORCE_IMAGE_PROTOCOL", "PI_FORCE_IMAGE_PROTOCOL")?.trim().toLowerCase();
	return forcedProtocol === "sixel" && $pickflag("SKC_ALLOW_SIXEL_PASSTHROUGH", "PI_ALLOW_SIXEL_PASSTHROUGH");
}
...
export function sanitizeWithOptionalSixelPassthrough(text: string, sanitize: (text: string) => string): string {
	...
	return sanitized.replace(/__SKC_SIXEL_SEQUENCE_(\d+)__/gu, (_, indexText: string) => {
