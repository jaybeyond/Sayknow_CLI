...
- `tasks`: tasks to execute in parallel
 ...
 - `.specialty` (optional): the kind of work — `backendArchitecture`, `frontendDesign`, `implementation`, `testing`, or `review`. When the user assigned a model to that specialty under `/model`, this child runs on it, deterministically: no classifier, no routing switch. It only leaves that model on a transport error (429, 5xx, auth, quota), falling back to the agent's role model. Declare it whenever the work clearly is one of these — a frontend build is `frontendDesign` even for `executor`. Omit to keep the role model or let auto-detection decide.
