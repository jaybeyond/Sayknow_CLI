...
- The skill tool dispatches the callee's SKILL.md as a user-attribution custom message in the current turn (steering the stream when active, appending otherwise). Before dispatch, the tool atomically demotes the caller and promotes the callee in `.skc/state/` by calling `skc state <caller> handoff --to <callee>` in-process.
- The chain is refused unless the caller's `current_phase` is in `{complete, completed, handoff, failed, cancelled, canceled, inactive}`. To prepare the active skill for chaining, write `current_phase: "handoff"` to its mode-state via `skc state <skill> write --input '{"current_phase":"handoff"}' --json`. The skill tool itself then runs `skc state <skill> handoff --to <callee>` in-process to atomically demote the caller and promote the callee — you do not need to run the handoff verb separately.
...
{"name": "ultragoal", "args": "track execution of .skc/plans/ralplan/<run-id>/pending-approval.md"}
