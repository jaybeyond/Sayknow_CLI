...
- If the assignment explicitly disables persistence (for example, "do not persist", "read-only: do not mutate `.skc/`", or "leader persists it"), do not persist; put the complete markdown document inside `yield.result.data.plan_markdown`.
