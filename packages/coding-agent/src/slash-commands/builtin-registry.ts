...
import { SayknowPetWidget } from "../modes/components/sayknow-pet-widget";
...
function formatModelAssignmentSummary(runtime: SlashCommandRuntime): string {
	...
	for (const targetId of SKC_MODEL_ASSIGNMENT_TARGET_IDS) {
...
const BUILTIN_SLASH_COMMAND_REGISTRY: ReadonlyArray<SlashCommandSpec> = [
	{
		...
		handle: async (command, runtime) => {
			...
			const stateRoot = path.join(runtime.cwd, ".skc", "state");
			switch (action) {
				case "status":
				...
				case "setup":
					return usage(
						"Run `skc notify setup` in a terminal to pair a Telegram bot token with a private chat (interactive; requires a TTY).",
						runtime,
					);
				default:
	...
	{
		name: "pet",
		description: "Sayknow pet living beside the composer",
		subcommands: [
		...
		handleTui: (command, runtime) => {
			...
			if (arg !== "off" && !SayknowPetWidget.pixelProtocol()) {
				...
				ctx.setPetMode(arg);
				const name = arg === "off" ? "Sayknow pet hidden" : `${PET_SKINS[arg].label} is here`;
				ctx.showStatus(name);
			} else {
...
export const BUILTIN_SLASH_COMMAND_DEFS: ReadonlyArray<BuiltinSlashCommand> = ACTIVE_BUILTIN_SLASH_COMMAND_REGISTRY.map(
	command => ({
		name: command.name,
		// Translated by command name (English description is the fallback source).
		get description() {
			return td(`cmd:${command.name}:d`, command.description);
		},
