 ...
 *
 * MCP servers are intentionally NOT inherited live from OpenCode config: SKC
 * owns MCP runtime execution. Use `skc mcp import opencode` to copy definitions
 * into SKC's own mcp.json instead.
 *
 * Priority: 55 (tool-specific provider)
 */
import * as path from "node:path";
import { parseFrontmatter, tryParseJson } from "@sayknow-cli/utils";
