/**
 * SKC Marketplace Plugin Provider
 *
 * Loads plugin roots from SKC plugin registries and explicit plugin-dir roots.
 */
...
import { rootContainsSkcManifest } from "../extensibility/skc-plugins";
import {
...
const DISPLAY_NAME = "SKC Marketplace";
const PRIORITY = 70;
...
async function listNonSkcPluginRoots(
	home: string,
	cwd: string,
	...
	for (const root of roots) {
		if (await rootContainsSkcManifest(root.path)) {
			filteredWarnings.push(`[claude-plugins] Skipping sayknow-cli plugin root (binding-only): ${root.path}`);
			continue;
		}
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	...
	const { roots, warnings: rootWarnings } = await listNonSkcPluginRoots(ctx.home, ctx.cwd);
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	for (const root of roots) {
		...
		//   nested: { "mcpServers": { name: cfg, ... } }   (SKC/Anthropic Code project shape)
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Load skills from SKC marketplace plugins",
	priority: PRIORITY,
	load: loadSkills,
});
...
registerProvider<SlashCommand>(slashCommandCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load slash commands from SKC marketplace plugins",
	priority: PRIORITY,
...
registerProvider<Hook>(hookCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load hooks from SKC marketplace plugins",
	priority: PRIORITY,
	load: loadHooks,
});
...
registerProvider<CustomTool>(toolCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load custom tools from SKC marketplace plugins",
	priority: PRIORITY,
	load: loadTools,
});
