/**
 * Builtin Provider (.skc)
 *
 * Primary provider for SKC native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@sayknow-cli/utils";
...
const DISPLAY_NAME = "SKC";
const DESCRIPTION = "Native SKC configuration from ~/.skc and .skc/";
const PRIORITY = 100;
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .skc/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.skc/agent/skills/
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// User scope:    ~/.skc/agent/RULES.md
	// Project scope: nearest .skc/RULES.md walking up from cwd to repoRoot
	for (const userAgentDir of getUserAgentDirs()) {
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .skc/ directories",
	priority: PRIORITY,
