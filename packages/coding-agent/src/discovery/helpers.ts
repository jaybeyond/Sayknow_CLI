...
import { FileType, glob } from "@sayknow-cli/natives";
import {
	CONFIG_DIR_NAME,
	getConfigDirName,
	getPluginsDir,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ skc?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.skc ?? pkg?.pi;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "skc"/"pi" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.skc/`.
 *    The first match returns `<dir>/.skc/plugins/installed_plugins.json`.
 * 2. If no `.skc/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.skc/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .skc/ directory (nearest wins).
	// Stop before os.homedir() — ~/.skc/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.skc/plugins/installed_plugins.json`
 * when no project anchor (.skc/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .skc/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
...
/**
 * List installed SKC plugin roots from the SKC plugin registry and, when present,
...
export async function listClaudePluginRoots(
	...
	// ── SKC installed plugins registry ───────────────────────────────────────
	...
	const skcRegistryPath = path.join(getPluginsDir(home), "installed_plugins.json");
	const skcContent = await readFile(skcRegistryPath);
	if (skcContent) {
		const skcRegistry = parseClaudePluginsRegistry(skcContent);
		if (skcRegistry) {
			for (const [pluginId, entries] of Object.entries(skcRegistry.plugins)) {
			...
			warnings.push(`Failed to parse SKC plugin registry: ${skcRegistryPath}`);
		}
	}

	// ── Project-scoped SKC registry ────────────────────────────────────────
	// Loaded from the nearest .skc/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before SKC/Anthropic model entries).
