...
import { ImageProtocol, TERMINAL, Text } from "@sayknow-cli/tui";
import { getProjectDir, isEnoent, logger, prompt } from "@sayknow-cli/utils";
...
export class BashTool implements AgentTool<BashToolSchema, BashToolDetails> {
	...
	async #prepareBashExecution(
		...
		const resolvedEnv = {
			...
			...(allowedPrefixes && allowedPrefixes.length > 0 ? { [SKC_RESTRICTED_ROLE_AGENT_BASH_ENV]: "1" } : {}),
		};
