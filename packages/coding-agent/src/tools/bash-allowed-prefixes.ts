...
const CANONICAL_STATE_TARGETS = new Set<string>(CANONICAL_SKC_WORKFLOW_SKILLS);
...
function malformedStateShape(): StateActionClassification {
	return { reason: "restricted role-agent bash only allows documented `skc state` action shapes" };
}
...
function classifyStateAction(words: readonly string[]): StateActionClassification {
	...
	if (modeFlags.length > 1 || inputFlags.length > 1) {
		return { reason: "restricted role-agent bash rejects repeated or conflicting `skc state` target selectors" };
	}
	if (classification.inputs.some(input => input.kind === "file")) {
		return { reason: "restricted role-agent bash does not allow file-backed `skc state --input` values" };
	}
	...
	if (distinctTargets.size > 1 || (runtimeTarget && suppliedTargets.some(target => target !== runtimeTarget))) {
		return {
			reason:
				"restricted role-agent bash rejects conflicting `skc state` selectors that disagree with runtime precedence",
		};
	}
	if (!runtimeTarget || !CANONICAL_STATE_TARGETS.has(runtimeTarget)) {
		return { reason: "restricted role-agent bash requires a canonical workflow skill for `skc state` commands" };
	}
...
function validateMatchedSkcCommand(words: readonly string[]): BashAllowedPrefixesCheck {
	if (words[0] !== "skc") return { allowed: true };

	if (words[1] === "ralplan") {
		if (!words.includes("--write")) {
			return { allowed: false, reason: "restricted role-agent bash only allows `skc ralplan --write ...`" };
		}
		return { allowed: true };
	}
	...
	if (words[1] === "state") {
		...
		if (!classification.parsed) {
			return {
				allowed: false,
				reason:
					classification.reason ?? "restricted role-agent bash only allows documented `skc state` action shapes",
			};
		}
		if (!ALLOWED_STATE_ACTIONS.has(classification.parsed.action)) {
			return {
				allowed: false,
				reason: `restricted role-agent bash does not allow \`skc state ${classification.parsed.action}\``,
			};
		}
		return { allowed: true };
	}
...
export function checkBashAllowedPrefixes(
	...
	return validateMatchedSkcCommand(parsed.words);
}
