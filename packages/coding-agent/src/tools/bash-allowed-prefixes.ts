...
function validateMatchedSkcCommand(words: readonly string[]): BashAllowedPrefixesCheck {
	...
	if (words[1] === "state") {
		...
		if (!action) {
			return {
				allowed: false,
				reason: "restricted role-agent bash only allows documented `skc state` action shapes",
			};
		}
		if (!ALLOWED_STATE_ACTIONS.has(action)) {
			return { allowed: false, reason: `restricted role-agent bash does not allow \`skc state ${action}\`` };
		}
		return { allowed: true };
	}
