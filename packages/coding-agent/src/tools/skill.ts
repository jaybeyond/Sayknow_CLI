...
import { isCanonicalSkcWorkflowSkill } from "../skill-state/active-state";
...
export class SkillTool implements AgentTool<typeof skillSchema, SkillToolDetails> {
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			// so there is no `skc state <skill>` command to run for them.
			if (activeSkill && isCanonicalSkcWorkflowSkill(activeSkill)) {
				const phase = (this.#session.getActiveSkillPhase?.() ?? "running").trim().toLowerCase();
				if (!TERMINAL_PHASES.has(phase)) {
					throw new ToolError(
						`skill tool: refusing to chain from "${activeSkill}" (phase=${phase}) into "${requestedName}". Finalize the current skill (skc state ${activeSkill} write --input '{"current_phase":"handoff"}' --json) or run skc state ${activeSkill} handoff --to ${requestedName} --json directly before chaining.`,
					);
				}
