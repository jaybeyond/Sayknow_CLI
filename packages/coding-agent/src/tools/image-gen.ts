...
import { getAntigravityUserAgent, getEnvApiKey, type Model } from "@sayknow-cli/ai";
import {
	CODEX_BASE_URL,
	getCodexAccountId,
...
async function saveImageToTemp(image: InlineImageData): Promise<string> {
	...
	const filename = `skc-image-${Snowflake.next()}.${ext}`;
			...
			if (provider === "openrouter") {
				...
				const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
					...
					headers: {
						"Content-Type": "application/json",
						Authorization: `Bearer ${apiKey.apiKey}`,
						"HTTP-Referer": "https://github.com/jaybeyond/Sayknow_CLI/",
						"X-OpenRouter-Title": "Sayknow-CLI",
