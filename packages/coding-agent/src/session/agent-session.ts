...
import { MacOSPowerAssertion } from "@sayknow-cli/natives";
import {
	extractRetryHint,
	isEnoent,
...
} from "../extensibility/skc-plugins";
import { resolveCurrentPhaseForParent } from "../extensibility/skc-plugins/injection";
import { readActiveSubskillsForParent, toActiveSubskillEntry } from "../extensibility/skc-plugins/state";
...
import { assertDeepInterviewIntentManifest } from "../skc-runtime/deep-interview-state";
...
} from "../skc-runtime/memory-guard-owner-claims";
import {
...
function buildSessionMetadata(
	...
	if (provider === "anthropic") {
		...
		if (typeof accountUuid === "string" && accountUuid.length > 0) {
			...
			userId.device_id = crypto.createHash("sha256").update(`skc-device-id-v1:${accountUuid}`).digest("hex");
		}
	}
...
export class AgentSession {
	...
	#skcSubskillToolNames = new Set<string>();
	#skcSubskillToolSignature: string | undefined;
	...
	#acquirePowerAssertion(): void {
		...
		try {
			this.#powerAssertion = MacOSPowerAssertion.start({
				reason: "Sayknow-CLI agent session",
				idle,
				system,
				user,
				display,
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#workerIntegrationScheduler = new WorkerIntegrationRequestScheduler(
			config.workerIntegrationRequest ??
				(async signal => {
					await requestSkcWorkerIntegrationAttempt(this.sessionManager.getCwd(), process.env, { signal }).catch(
						error => {
	...
	getDeepInterviewAskStage(): "topology" | "post-topology" | undefined {
		...
		try {
			assertNonEmptySkcSessionId(currentSessionId, "AgentSession.getDeepInterviewAskStage");
	...
	#handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "tool_execution_end") {
			...
			if (event.toolName === "bash" && !event.isError) {
				await this.#activatePendingSkcGoalModeRequest();
			}
		}
	...
	async #hasActiveSkcSubskillTools(parent: string, sessionId: string | undefined): Promise<boolean> {
	...
	#computeSkcSubskillToolSignature(tools: CustomTool[]): string {
		return tools
	 ...
	 */
	async refreshSkcSubskillTools(): Promise<void> {
		...
		if (!parent) {
			if (this.#skcSubskillToolNames.size === 0) return;
			const previousSkcSubskillToolNames = new Set(this.#skcSubskillToolNames);
			const previousActiveToolNames = this.getActiveToolNames();
			for (const name of previousSkcSubskillToolNames) {
				this.#toolRegistry.delete(name);
			}
			this.#skcSubskillToolNames.clear();
			this.#invalidateDiscoveryCaches();
			await this.#applyActiveToolsByName(
				previousActiveToolNames.filter(name => !previousSkcSubskillToolNames.has(name)),
			);
			return;
		}
		...
		if (this.#skcSubskillToolNames.size === 0 && !(await this.#hasActiveSkcSubskillTools(parent, sessionId))) return;

		const phase = await resolveCurrentPhaseForParent({ cwd, sessionId, parent });
		const reservedToolNames = Array.from(this.#toolRegistry.keys()).filter(
			name => !this.#skcSubskillToolNames.has(name),
		);
		...
		if (uniqueToolNames.size !== nextToolNames.length) {
			throw new Error("SKC sub-skill tool names must be unique");
		}

		const previousSkcSubskillToolNames = new Set(this.#skcSubskillToolNames);
		const nextSignature = this.#computeSkcSubskillToolSignature(customTools);
		if (this.#skcSubskillToolSignature === nextSignature) {
			return;
		}
		...
		this.#skcSubskillToolNames.clear();
		this.#skcSubskillToolSignature = undefined;
		...
		for (const customTool of customTools) {
			...
			this.#toolRegistry.set(finalTool.name, finalTool);
			this.#skcSubskillToolNames.add(finalTool.name);
		}
		this.#skcSubskillToolSignature = nextSignature;

		this.#invalidateDiscoveryCaches();
		const activeNonSkcSubskillToolNames = previousActiveToolNames.filter(
			name => !previousSkcSubskillToolNames.has(name),
		);
		const preservedSkcSubskillToolNames = previousActiveToolNames.filter(
			name => previousSkcSubskillToolNames.has(name) && this.#skcSubskillToolNames.has(name),
		);
		const autoActivatedSkcSubskillToolNames = customTools
			.filter(tool => !tool.hidden && !previousSkcSubskillToolNames.has(tool.name))
			.map(tool => tool.name);
		await this.#applyActiveToolsByName(
			Array.from(
				new Set([
					...activeNonSkcSubskillToolNames,
					...preservedSkcSubskillToolNames,
					...autoActivatedSkcSubskillToolNames,
				]),
			),
		);
	}
	...
	#constructWorkflowGateEmitter(sessionId = this.sessionManager.getSessionId()): WorkflowGateEmitter {
		assertNonEmptySkcSessionId(sessionId, "AgentSession workflow-gate session");
	...
	async #attachAskToolIfWorkflowActive(): Promise<void> {
		...
		if (activeSkill && isCanonicalSkcWorkflowSkill(activeSkill.trim())) {
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		// A newer human prompt makes any in-flight /rename regeneration stale.
		...
		await this.#withSessionAdmission("prompt", async admission => {
			...
			await this.refreshSkcSubskillTools();
	...
	async #syncSkillPromptActiveState(
		...
		if (active && isCanonicalSkcWorkflowSkill(skill)) this.#attachAskTool();
		const sessionId = this.sessionManager.getSessionId();
		// Canonical SKC workflow skills (deep-interview, ralplan, ultragoal, team)
		// own their `.skc/state/skill-active-state.json` row through the
		// `skc state handoff` and `skc state clear` runtime verbs. The prompt
		...
		// `/skill:<name>` invocation has no row yet, so seed `.skc/state`
	...
	/** Skills loaded by SDK (always includes bundled SKC workflow defaults unless explicitly overridden by SDK callers) */
	...
	async #checkActiveDeepInterviewCompletion(
		...
		if (stopReason !== "skc_skill_deep_interview_interviewing") {
			if (!stopReason.startsWith("skc_skill_deep_interview_")) return "not_applicable";
			this.#handledDeepInterviewAssistantIds.add(identity);
		...
		try {
			...
			const reminder = [
				"<system-reminder>",
				`You stopped while the SKC deep-interview workflow is still active (stop gate: ${stopReason}).`,
	...
	#resolveRoleModelFull(
		...
		const target = SKC_MODEL_ASSIGNMENT_TARGETS[role as keyof typeof SKC_MODEL_ASSIGNMENT_TARGETS];
		const roleModelStr =
