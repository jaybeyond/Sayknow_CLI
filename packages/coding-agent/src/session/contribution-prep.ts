...
import type { AssistantMessage, ToolResultMessage, UserMessage } from "@sayknow-cli/ai";
import { $ } from "bun";
import { resolveSkcCommand } from "../task/skc-command";
...
export async function prepareContributionPrep(
	...
	const artifactDir = path.join(
		options.artifactRoot ?? path.join(context.cwd, ".skc", "contribution-prep"),
		safeTimestamp,
	);
	...
	if (options.spawnWorker) {
		...
		const command = resolveSkcCommand();
		await spawn(
