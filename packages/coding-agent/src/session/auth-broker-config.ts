/**
 * Resolve auth-broker connection configuration for the local skc client.
 *
 * Precedence (highest first):
 *   1. `SKC_AUTH_BROKER_URL` / `SKC_AUTH_BROKER_TOKEN` env vars.
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.skc/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.skc/auth-broker.token` (paired with URL from env or config).
 *
...
import { getAgentDir, getConfigRootDir, isEnoent, logger } from "@sayknow-cli/utils";
...
/** Path to the local bearer token file. Created on the broker host by `skc auth-broker token`. */
...
export async function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const envUrl = process.env.SKC_AUTH_BROKER_URL;
	const envToken = process.env.SKC_AUTH_BROKER_TOKEN;
	...
	if (!token) {
		throw new Error(
			`SKC_AUTH_BROKER_URL is set (${url}) but no bearer token is available. ` +
				`Set SKC_AUTH_BROKER_TOKEN, the \`auth.broker.token\` config entry, or place one at ${getAuthBrokerTokenFilePath()}.`,
		);
	}
	return { url, token };
}
