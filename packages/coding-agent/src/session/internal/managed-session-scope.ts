...
export const MANAGED_SESSION_BINDING_FILE = ".skc-managed-session-scope.v2.json";
...
function scopeDigest(platform: "posix" | "win32", canonicalPath: string): string {
	const bytes = createHash("sha256")
		.update("skc-managed-session-scope\0identity-v1\0", "utf8")
...
const MANAGED_INTERNAL_DIRECTORY = ".skc-managed-session-internal";
...
function isQuarantinePath(target: RetiredTarget, pathname: unknown): pathname is string {
	return (
		...
		path.basename(pathname).startsWith(".skc-delete-")
	);
}
...
function restorePreparedArtifactRoot(scope: ManagedScope, source: ManagedCandidate): void {
	...
	if (
		...
		!path.basename(quarantine.detachedPath).startsWith(".skc-migrate-") ||
