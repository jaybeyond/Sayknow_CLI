 ...
 *
 * - `actionable`: usage evidence points to a user-controllable cause and SKC
...
export interface CacheBehaviorWarning {
	...
	nextStep?: string;
	/** What SKC cannot determine — present for non-actionable attributions. */
	unknown?: string;
	costUsd: number;
}
	...
	if (
		...
		if (noCacheActivity) {
			...
			return {
				...
				unknown:
					"the provider reported no cache reads or writes, so SKC cannot tell whether prompt caching is unsupported, missing cache-control fields, or provider-side eviction",
		...
		// the large new input was not cached. From usage alone SKC cannot tell new
