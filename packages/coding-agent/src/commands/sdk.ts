...
export async function runSessionHost(
	...
	const request = readSessionLifecycleLaunchRequest(process.env.SKC_SDK_LIFECYCLE_REQUEST, now());
	const agentDir = process.env.SKC_AGENT_DIR;
	if (!agentDir) throw new Error("SKC_AGENT_DIR is required for sdk session-host-internal.");
	...
	if (
		process.env.SKC_STATE_ROOT !== undefined &&
		path.resolve(process.env.SKC_STATE_ROOT) !== path.resolve(request.stateRoot)
	)
		throw new Error("Lifecycle state root does not match the broker-issued request.");
	if (request.effectMarker && process.env.SKC_LIFECYCLE_REQUEST_ID !== request.effectMarker)
	...
	try {
		const modelProfileStartup =
			process.env.SKC_SDK_TEST_HANG_MODEL_PROFILE === cwd
				? new Promise<void>(() => {})
		...
		throwIfCutoff();
		if (process.env.SKC_SDK_TEST_FAIL_AFTER_REGISTRATION === cwd)
