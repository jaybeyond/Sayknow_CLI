/**
 * `skc auth-broker` — manage the skc credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the skc auth-broker (credential vault)";
	...
	static examples = [
		...
		"# Boot on a non-default port\n  skc auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  skc auth-broker token",
		"# Rotate the bearer token\n  skc auth-broker token --regenerate",
		"# Local login (run on the broker host)\n  skc auth-broker login anthropic",
		"# Remote login over SSH tunnel\n  skc auth-broker login anthropic --via=user@broker",
		"# Import a CLIProxyAPI auth dump\n  skc auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  skc auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  skc auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  skc auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  skc auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("skc", "auth-broker", AuthBroker);
			return;
		}
