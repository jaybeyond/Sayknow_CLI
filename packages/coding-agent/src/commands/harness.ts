/**
 * `skc harness <verb>` — AI-native stateless JSON CLI for the coding-harness
 * operations control plane (v1, sayknow-cli adapter).
 *
...
import { SayknowCliRpc } from "../harness-control-plane/rpc-adapter";
...
import { resolveSkcTmuxBinary, resolveSkcTmuxCommand, sanitizeTmuxToken } from "../skc-runtime/tmux-common";
import {
...
function ownerExitGuidance(reason: string, startupBlocker: boolean): string {
	if (startupBlocker) {
		return "owner started and reported live but exited before accepting the first prompt; run `skc harness recover --session <id>` to respawn the owner, then resubmit the prompt";
	}
	switch (reason) {
		case "owner-exited-after-prompt-acceptance":
			return "owner exited after accepting a prompt; run `skc harness recover --session <id>` to preserve in-flight work and classify the vanish before resubmitting";
		case "owner-lease-expired":
		case "owner-endpoint-unreachable":
			return "owner lease is stale or its endpoint did not route; run `skc harness recover --session <id>` to respawn or take over the owner";
		...
		default:
			return "no live owner holds this session; run `skc harness recover --session <id>` to (re)spawn an owner, then resubmit";
	}
}
...
function deterministicHarnessTmuxSessionName(sessionId: string): string {
	return `sayknow_cli_harness_${sanitizeTmuxToken(sessionId)}`;
}
...
export default class Harness extends Command {
	static description = "Operate coding harnesses (v1: sayknow-cli) as a session/evidence/recovery/PR control plane";
	static strict = false;
	...
	static examples = [
		`skc harness start --input '{"harness":"sayknow-cli","workspace":".","branch":"feat/x"}'`,
		"skc harness observe --session <id>",
		`skc harness classify --input '{"observation":{"ownerLive":false,"gitDelta":"dirty","risk":"vanished-dirty"}}'`,
		"skc harness events --session <id> --follow",
	];
		...
		writeJson({
			...
			evidence: {
				...
				guidance: preflight.ok
					...
					: "fix blockers before skc harness start; branch must match the actual checkout and issueOrPr must be numeric or a recognized PR/issue form",
			},
		});
	...
	async #runOwner(root: string, input: Record<string, unknown>, flagSession: string | undefined): Promise<void> {
		const sessionId = requireSessionId(input, flagSession);
		const sessionDir = sessionPaths(root, sessionId).skcSessionDir;
		// Optional rpc command override (tests / non-default hosts); defaults to `skc --mode rpc`.
		const override = process.env.SKC_HARNESS_RPC_COMMAND;
		const command = override ? (JSON.parse(override) as string[]) : undefined;
		// Mark the RPC owner as a SKC-spawned child (canonical provenance marker)
	...
	async #harnessOwnerIsolationProbe(tmuxCommand: string): Promise<OwnerIsolationProbe> {
		return {
			readCallerCgroup: async () =>
				process.env.SKC_HARNESS_TEST_CALLER_CGROUP ??
			...
			probeServer: async (socketKey, tmuxControlArgv): Promise<TmuxServerProof> => {
				...
				const cgroupText =
					process.env.SKC_HARNESS_TEST_SERVER_CGROUP ??
					(process.platform === "linux"
						? await fs.readFile(`/proc/${pid}/cgroup`, "utf8").catch(() => null)
						: null);
				const testStartTime = process.env.SKC_HARNESS_TEST_SERVER_START_TIME;
				const stat =
	...
	async #cleanupTmuxAttempt(
		...
		const killed = Bun.spawnSync(
			[
				...
				"-F",
				predicate,
				`kill-session -t '${nativeSessionId}' ; display-message -p __skc_harness_cleanup_ok__`,
				"display-message -p __skc_harness_cleanup_refused__",
			],
			{ stdout: "pipe", stderr: "pipe" },
		);
		if (killed.exitCode !== 0 || killed.stdout.toString().trim() !== "__skc_harness_cleanup_ok__")
			throw new Error("tmux-owner-cleanup_uncertain");
	}
	...
	async #startTmuxResidentOwner(
		...
		const tmuxCommand = resolveSkcTmuxCommand();
		...
		if (resolveSkcTmuxBinary({ env: process.env }).isPsmux)
			return {
				started: false,
				sessionName,
				socketKey: null,
		...
		const socketKey = `skc-owner-${randomBytes(24).toString("hex")}`;
		...
		const envAssignments = [
			`SKC_HARNESS_STATE_ROOT=${shellQuote(root)}`,
		...
		if (process.env.SKC_HARNESS_RPC_COMMAND)
			envAssignments.push(`SKC_HARNESS_RPC_COMMAND=${shellQuote(process.env.SKC_HARNESS_RPC_COMMAND)}`);
		if (process.env.SKC_HARNESS_TEST_NODE_MODULES)
			envAssignments.push(`SKC_HARNESS_TEST_NODE_MODULES=${shellQuote(process.env.SKC_HARNESS_TEST_NODE_MODULES)}`);
		...
		const child = Bun.spawn(cmd, {
			...
			env: {
				...
				...(process.env[RECEIPT_SPOOL_DIR_ENV]
					...
					: {}),
				...(process.env.SKC_HARNESS_TEST_NODE_MODULES
					? { SKC_HARNESS_TEST_NODE_MODULES: process.env.SKC_HARNESS_TEST_NODE_MODULES }
					: {}),
			},
			stdout: "ignore",
			stderr: "ignore",
	...
	async #start(root: string, input: Record<string, unknown>): Promise<void> {
		const harness = (typeof input.harness === "string" ? input.harness : "sayknow-cli") as HarnessKind;
		if (harness !== "sayknow-cli") {
			writeJson({
				ok: false,
				error: `harness_unsupported_in_v1:${harness}`,
				evidence: { seam: true, supported: ["sayknow-cli"] },
			});
			process.exitCode = 1;
			return;
		}
		...
		if (fatalBlockers.length > 0) {
			writeJson({
				...
				evidence: {
					...
					guidance:
						"fix blockers before start; run skc harness preflight with the same input for branch and issue/PR diagnostics",
				},
			});
			process.exitCode = 1;
			return;
		...
		const handle: SessionHandle = {
			...
			rpcHandle: { kind: "rpc-subprocess", pid: null, sessionDir: `${root}/sessions/${sessionId}/skc-session` },
