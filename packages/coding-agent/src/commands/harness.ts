...
import { resolveSkcTmuxCommand, sanitizeTmuxToken } from "../skc-runtime/tmux-common";
...
export default class Harness extends Command {
	...
	async #runOwner(root: string, input: Record<string, unknown>, flagSession: string | undefined): Promise<void> {
		...
		const rpc = new SayknowCliRpc({ sessionDir, command });
	...
	#startTmuxResidentOwner(
		...
		const envAssignments = [`SKC_HARNESS_STATE_ROOT=${shellQuote(root)}`];
