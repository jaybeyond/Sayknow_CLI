...
export default class Team extends Command {
	static description =
		"Run native SKC tmux team orchestration from inside an existing tmux/SKC --tmux session; --dry-run writes ephemeral .skc/state/team state only";
	static strict = false;
	...
	static flags = {
		...
		"dry-run": Flags.boolean({
			description:
				"Create ephemeral .skc/state/team state without starting tmux panes; do not commit generated state",
			default: false,
		}),
	};
	...
	async run(): Promise<void> {
		...
		writeText([
			...
			...(dryRun ? ["dry-run: wrote ephemeral .skc/state/team state only; do not commit generated .skc state"] : []),
		]);
	}
}
