...
import { renderCliWriteReceipt } from "../skc-runtime/cli-write-receipt";
import { renderTeamStatusMarkdown } from "../skc-runtime/state-renderer";
import {
	buildTeamHudSummary,
...
async function syncTeamHud(snapshot: SkcTeamSnapshot): Promise<void> {
	try {
		const events = await readSkcTeamEvents(snapshot.team_name);
		...
		await persistSkcTeamModeStateSummary(snapshot, process.cwd());
	} catch {
...
function snapshotWriteReceipt(snapshot: SkcTeamSnapshot): Record<string, unknown> {
	return {
		ok: true,
...
export default class Team extends Command {
	static description =
		"Run native SKC tmux team orchestration from inside an existing tmux/SKC --tmux session; --dry-run writes ephemeral .skc/_session-{sessionid}/state/team state only";
	static strict = false;
	...
	static flags = {
		...
		"dry-run": Flags.boolean({
			description:
				"Create ephemeral .skc/_session-{sessionid}/state/team state without starting tmux panes; do not commit generated state",
			default: false,
		}),
	};
	...
	static examples = [
		...
		'skc team api claim-task --input \'{"team_name":"demo","worker_id":"worker-1"}\' --json',
		'skc team 2:executor --dry-run --json "Preview state only"',
	...
	async run(): Promise<void> {
		...
		if (action === "list") {
			const teams = await listSkcTeams();
			if (json) {
				writeJson({ teams });
				return;
			}
		...
		if (action === "status") {
			...
			const snapshot = await readSkcTeamSnapshot(teamName);
			if (json) {
				writeJson(snapshot);
				return;
			}
			writeText([
				...
				"- mode: read-only status; use `skc team monitor <team>` or `skc team resume <team>` for recovery/integration",
			]);
		...
		if (action === "monitor" || action === "resume") {
			const teamName = rest.find(arg => !arg.startsWith("--"));
			if (!teamName) throw new Error("missing_team_name");
			const snapshot = await monitorSkcTeamSnapshot(teamName);
		...
		if (action === "shutdown") {
			const teamName = rest.find(arg => !arg.startsWith("--"));
			if (!teamName) throw new Error("missing_team_name");
			const snapshot = await shutdownSkcTeam(teamName);
			await syncTeamHud(snapshot);
			if (json) {
				writeReceipt(snapshotWriteReceipt(snapshot));
				return;
		...
		if (action === "api") {
			...
			if (!operation || operation === "--help" || operation === "help") {
				writeText([
					...
					"Completion example:",
					'transition-task-status --input \'{"team_name":"demo","task_id":"task-1","to":"completed","claim_token":"...","completion_evidence":{"summary":"done","items":[{"kind":"command","status":"passed","summary":"focused tests passed","command":"bun test packages/coding-agent/test/skc-runtime/team-runtime.test.ts"}]}}\' --json',
			...
			const result = await executeSkcTeamApiOperation(operation, input);
			const teamName = String(input.team_name ?? input.teamName ?? "").trim();
			if (teamName) {
				try {
					await syncTeamHud(await readSkcTeamSnapshot(teamName));
				} catch {
		...
		const snapshot = await startSkcTeam({ ...options, dryRun });
		await syncTeamHud(snapshot);
		if (json) {
			writeReceipt(snapshotWriteReceipt(snapshot));
			return;
		...
		writeText([
			...
			...(dryRun
				? [
						"dry-run: wrote ephemeral .skc/_session-{sessionid}/state/team state only; do not commit generated .skc state",
					]
				: []),
		]);
	}
