...
export default class McpServe extends Command {
	static description = "Serve SKC MCP compatibility bridges";
	static strict = false;
	...
	async run(): Promise<void> {
		...
		if (flags.check) {
			if (!flags.json) {
				const serverName = server === "sdk" ? "skc-sdk-mcp" : COORDINATOR_MCP_SERVER_NAME;
			...
			const payload =
				server === "sdk"
					? { ok: true, server: { name: "skc-sdk-mcp" }, readOnly: false, tools: [...SDK_MCP_TOOL_NAMES] }
