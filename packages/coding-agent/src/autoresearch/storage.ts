...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@sayknow-cli/utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.skc/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.SKC_AUTORESEARCH_DB_DIR;
