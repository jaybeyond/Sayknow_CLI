...
function toolSchema(name: CoordinatorToolName): {
	...
	const cwd = {
		type: "string",
		description: "Canonicalized SKC worktree or project directory inside configured roots.",
	};
	const sessionId = {
		type: "string",
		description: "SKC coordinator bridge session id.",
	};
	const pathField = {
		type: "string",
	...
	const mpreset = {
		type: "string",
		description:
			"Optional SKC model profile (`skc --mpreset <profile>`) to authoritatively activate for a fresh session; resolved through the merged built-in/custom profile registry and applied from the first turn. Unknown names are rejected with the available-profile listing.",
	};
	const common = { type: "object", properties: {} as Record<string, unknown> };
	if (name === "skc_coordinator_register_session") {
		return {
			name,
			description: "Register an existing visible tmux SKC session as a coordinator-authoritative session.",
			inputSchema: {
				type: "object",
				properties: {
	...
	if (name === "skc_coordinator_start_session") {
		return {
			name,
			description: "Start a SKC worktree/tmux oriented session through the coordinator bridge.",
			inputSchema: {
				type: "object",
				properties: {
					cwd,
	...
	if (name === "skc_coordinator_send_prompt") {
		return {
			name,
			description:
	...
	if (name === "skc_coordinator_read_turn") {
		return {
			name,
	...
	if (name === "skc_coordinator_await_turn") {
		return {
			name,
	...
	if (name === "skc_coordinator_submit_question_answer") {
		return {
			name,
	...
	if (name === "skc_coordinator_report_status") {
		return {
			name,
	...
	if (name === "skc_coordinator_read_artifact") {
		return {
			name,
	...
	if (name === "skc_coordinator_read_status") {
		return {
			name,
	...
	if (name === "skc_coordinator_read_tail") {
		return {
			name,
	...
	if (name === "skc_coordinator_list_questions") {
		return {
			name,
	...
	if (name === "skc_coordinator_list_artifacts") {
		return {
			name,
	...
	if (name === "skc_coordinator_read_coordination_status") {
		return {
			name,
	...
	if (name === "skc_coordinator_watch_events") {
		return {
			name,
		...
		return {
			...
			inputSchema: {
				...
				properties: {
					...
					task: {
						type: "string",
						description: "Delegated task or objective to run through the selected SKC workflow.",
					},
					prompt: {
						type: "string",
					...
					session_id: {
						type: "string",
						description:
							"Optional existing SKC coordinator bridge session id to reuse; omitted starts a fresh session.",
					},
					queue: {
						type: "boolean",
					...
					timeout_ms: {
						type: "number",
						description:
							"Bounded await timeout in milliseconds, capped at 30 minutes like skc_coordinator_await_turn.",
					},
					poll_interval_ms: {
						type: "number",
	...
	return {
		name,
		description: "List known scoped SKC coordinator bridge sessions.",
		inputSchema: common,
	};
}
...
function workflowForDelegateTool(name: string): DelegateWorkflow | null {
	switch (name) {
		case "skc_delegate_plan":
			return "plan";
		case "skc_delegate_execute":
			return "execute";
		case "skc_delegate_team":
			return "team";
		default:
			return null;
	}
...
function delegateToolDescription(workflow: DelegateWorkflow): string {
	switch (workflow) {
		case "plan":
			return "Delegate consensus planning to SKC: start a session and run /skill:ralplan to completion, returning durable turn status and artifact references.";
		case "execute":
			return "Delegate execution to SKC: start a session and run /skill:ultragoal to completion, returning durable turn status and artifact references.";
		case "team":
			return "Delegate parallel team execution to SKC: start a session and run /skill:team to completion, returning durable turn status and artifact references.";
	}
}
...
function workflowPrompt(
	...
	return [
		...
		task,
		"",
		"Return durable status and artifact references through SKC runtime/coordinator state. Do not expose host-facing tmux controls.",
	].join("\n");
}
...
async function readRuntimeReadinessMarker(
	...
	try {
		...
		return marker.schema_version === 1 &&
			...
			marker.source === "skc_interactive_runtime" &&
...
function unavailableSessionEvidence(turn: TurnRecord, reason: string, timestamp: string): Record<string, unknown>[] {
	...
	return [
		...turn.evidence,
		{
			type: reason,
			message:
				"The tmux session disappeared after SKC runtime acknowledged the prompt, before any terminal final_response or error was recorded. Treat this as an in-flight vanished session and inspect/restart with recovery evidence rather than resubmitting blindly.",
			tmux_keys_sent: true,
...
async function markTurnAcknowledgedFromRuntimeState(
	...
	await appendCoordinatorEvent(namespaceDir, {
		...
		summary: `Turn ${acknowledged.turn_id} was acknowledged by the SKC runtime`,
...
async function markTurnFailedForUnacknowledgedDelivery(
	...
	const timestamp = new Date().toISOString();
	const message = `Tmux key delivery succeeded, but the SKC runtime did not acknowledge the prompt or emit turn_start within ${ackTimeoutMs}ms. The turn never started; stop waiting and inspect/retry the coordinator session.`;
	const failed: TurnRecord = {
		...turn,
		status: "failed",
		delivery: {
 ...
 * command so the spawned SKC session activates the selected profile from its
 ...
 *
 * Precedence note: this matches `skc --mpreset <profile>` semantics exactly, so
 ...
 * follows the same CLI precedence as a direct `skc --mpreset ... --model ...`
...
async function sendTmuxPromptKeys(
	...
	const bufferName = `skc-coordinator-prompt-${randomUUID()}`;
...
async function startTmuxSession(
	...
	const socketKey = `skc-coordinator-${randomUUID().slice(0, 8)}`;
	...
	const sessionCommand = [
		"exec env",
		`${SKC_COORDINATOR_SESSION_STATE_FILE_ENV}=${shellQuote(runtimeStateFile)}`,
		`${SKC_COORDINATOR_SESSION_ID_ENV}=${shellQuote(sessionName)}`,
		`${SKC_TMUX_OWNER_GENERATION_ENV}=${shellQuote(ownerGeneration)}`,
		`${SKC_TMUX_OWNER_STATE_DIR_ENV}=${shellQuote(ownerStateDir)}`,
		`${SKC_TMUX_OWNER_SERVER_KEY_ENV}=${shellQuote(socketKey)}`,
		`${SKC_COORDINATOR_SESSION_LAUNCH_ID_ENV}=${shellQuote(input.launchId)}`,
		`${SKC_COORDINATOR_SESSION_READINESS_FILE_ENV}=${shellQuote(input.readinessMarkerFile)}`,
	...
	if (plan.execution.mode === "direct" && plan.execution.server_absent_before) {
		plan = await planTmuxOwnerIsolation(
			...
			{
				...probe,
				readCallerCgroup: async () => "0::/skc-coordinator-bootstrap.service",
			},
		);
		if (!plan.ok) throw new Error(`coordinator_tmux_owner_${plan.code}`);
	}
	...
	const cleanupFailedAttempt = async (): Promise<"cleaned" | "failed" | "unverifiable"> => {
		...
		try {
			const result = await runner([
				...
				`kill-session -t '${spawnedNativeSessionId}' ; display-message -p __skc_coordinator_cleanup_ok__`,
				"display-message -p __skc_coordinator_cleanup_refused__",
			]);
			if (result.exitCode !== 0) return "failed";
			return result.stdout.trim() === "__skc_coordinator_cleanup_ok__" ? "cleaned" : "unverifiable";
		} catch {
			return "unverifiable";
		}
	};
...
function summarizePaneTail(lines: string[]): Record<string, unknown> {
	...
	const assistantLine = lastMatchingLine(nonEmpty, /^(sayknow|assistant)\b/i);
...
export function createCoordinatorMcpServer(options: CoordinatorMcpServerOptions = {}) {
	...
	const promptAckTimeoutMs = boundedRuntimePromptAckTimeoutMs(env.SKC_COORDINATOR_MCP_PROMPT_ACK_TIMEOUT_MS);
	...
	const readinessTimeoutMs = boundedRuntimeReadinessTimeoutMs(env.SKC_COORDINATOR_MCP_RUNTIME_READINESS_TIMEOUT_MS);
	...
	async function startCoordinatorSession(
		...
		const sessionId = `skc-coordinator-${randomUUID().slice(0, 8)}`;
	...
	async function completePreparedTurn(session: Record<string, unknown>, pendingTurn: TurnRecord): Promise<TurnRecord> {
		...
		return await withSessionTransition(pendingTurn.session_id, async () => {
			...
			if (!ready) {
				...
				const message = `SKC runtime did not attest interactive input readiness within ${readinessTimeoutMs}ms.`;
				const failed: TurnRecord = {
					...current,
					status: "failed",
					delivery: {
	...
	async function callToolUnlocked(name: string, args: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
		try {
			if (name === "skc_coordinator_list_sessions") return { ok: true, sessions: await listSessions() };
			if (name === "skc_coordinator_register_session") {
			...
			if (name === "skc_coordinator_list_questions") return { ok: true, questions: await listQuestions(args) };
			if (name === "skc_coordinator_list_artifacts") return { ok: true, roots: config.allowedRoots };
			if (name === "skc_coordinator_read_artifact")
			...
			if (delegateWorkflow) {
				...
				if (args.await_completion === true && !shouldQueue) {
					...
					// Mirror skc_coordinator_await_turn timeout semantics: a still-active
	...
	async function callToolInternal(name: string, args: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
		try {
			if (
				name === "skc_coordinator_await_turn" ||
				name === "skc_coordinator_watch_events" ||
				(name === "skc_coordinator_start_session" && typeof args.prompt === "string" && args.prompt.length > 0) ||
 ...
 *
 * A long-running tool call (e.g. skc_coordinator_await_turn, which polls for
