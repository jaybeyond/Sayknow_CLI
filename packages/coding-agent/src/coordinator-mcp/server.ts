...
import { getAgentDir, isKnownSinkPeerClosedError } from "@sayknow-cli/utils";
import { normalizePathForComparison, VERSION } from "@sayknow-cli/utils/dirs";
...
import { readLinuxProcStartTimeSync } from "../skc-runtime/linux-proc";
import {
...
function toolSchema(name: CoordinatorToolName): {
	...
	const sessionId = { type: "string", description: "SKC coordinator bridge session id." };
	const pathField = { type: "string", description: "Artifact path inside configured safe roots." };
	const mpreset = {
		type: "string",
		description:
			"Optional SKC model profile (`skc --mpreset <profile>`). Unknown names are rejected with the available-profile listing.",
	};
	...
	if (name === "skc_coordinator_register_session") {
		return {
			name,
			description:
				"Register an existing broker-indexed SKC session; tmux identifiers are advisory process metadata only.",
			inputSchema: {
				type: "object",
				properties: {
	...
	if (name === "skc_coordinator_start_session") {
		return {
			name,
			description: "Start a broker-managed SKC session through canonical SDK lifecycle control.",
			inputSchema: {
				type: "object",
				properties: {
					cwd,
		...
		return {
			...
			inputSchema: {
				...
				properties: {
					...
					force: {
						type: "boolean",
						description: "Close a non-ephemeral session; requires the SKC_COORDINATOR_MCP_FORCE_STOP capability.",
					},
	...
	if (name === "skc_coordinator_read_status") {
		return {
			name,
			description: "Read selected broker-indexed SKC session status from SDK discovery.",
	...
	return { name, description: "List known scoped SKC coordinator bridge sessions.", inputSchema: common };
}
...
function coordinatorLifecycleTarget(sessionCommand: string | null, cwd: string): Record<string, unknown> {
	...
	if (executable !== "skc")
		throw new SdkClientError(
			"invalid_input",
			"SKC_COORDINATOR_MCP_SESSION_COMMAND must be exactly skc with an optional --worktree [name] selector.",
		);
	...
	if (
		...
		throw new SdkClientError(
			"invalid_input",
			"SKC_COORDINATOR_MCP_SESSION_COMMAND supports only skc or skc --worktree [name] under SDK lifecycle control.",
		);
	return {
		path: cwd,
...
export function createCoordinatorMcpServer(options: CoordinatorMcpServerOptions = {}) {
		...
		return await withSessionTransition(id, async () => {
			...
			await withSessionTransaction(questionPaths, id, async transaction => {
				...
				transaction.requests.operations[deletionId] = {
					...
					tool: "skc_coordinator_stop_session",
	...
	async function callTool(name: string, args: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
		try {
			if (name === "skc_coordinator_list_sessions")
			...
			if (name === "skc_coordinator_list_questions") return await listQuestions(args);
