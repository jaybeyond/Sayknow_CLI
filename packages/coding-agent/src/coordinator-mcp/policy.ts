...
export function coordinatorNamespaceIdentity(env: NodeJS.ProcessEnv = process.env): string {
	return `ns1_${createHash("sha256")
		.update(
			canonicalJson({
				profile_exact: env.SKC_COORDINATOR_MCP_PROFILE ?? null,
				profile_present: env.SKC_COORDINATOR_MCP_PROFILE !== undefined,
				repo_exact: env.SKC_COORDINATOR_MCP_REPO ?? null,
				repo_present: env.SKC_COORDINATOR_MCP_REPO !== undefined,
			}),
		)
		.digest("hex")
		.slice(0, 32)}`;
...
export function buildCoordinatorMcpConfig(env: NodeJS.ProcessEnv = process.env): CoordinatorMcpConfig {
	...
	return {
		...
		mutationClasses: parseMutationClasses(
			env.SKC_COORDINATOR_MCP_MUTATIONS ?? env.SKC_COORDINATOR_MCP_ENABLE_MUTATION_CLASSES,
		),
		artifactByteCap: parseByteCap(
			env.SKC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP ?? env.SKC_COORDINATOR_MCP_ARTIFACT_MAX_BYTES,
		),
		namespace: {
			profile: cleanScope(env.SKC_COORDINATOR_MCP_PROFILE),
			repo: cleanScope(env.SKC_COORDINATOR_MCP_REPO),
			identity: coordinatorNamespaceIdentity(env),
		},
		stateRoot: path.resolve(stateRoot),
		sessionCommand: env.SKC_COORDINATOR_MCP_SESSION_COMMAND?.trim() || null,
		sessionIdleTtlMs: parsePositiveIntMs(
			env.SKC_COORDINATOR_MCP_SESSION_IDLE_TTL_MS,
			DEFAULT_SESSION_IDLE_TTL_MS,
			MIN_SESSION_IDLE_TTL_MS,
		),
		sessionSweepIntervalMs: parsePositiveIntMs(
			env.SKC_COORDINATOR_MCP_SESSION_SWEEP_INTERVAL_MS,
			DEFAULT_SESSION_SWEEP_INTERVAL_MS,
			MIN_SESSION_SWEEP_INTERVAL_MS,
		),
		forceStopEnabled: parseBool(env.SKC_COORDINATOR_MCP_FORCE_STOP),
	};
}
