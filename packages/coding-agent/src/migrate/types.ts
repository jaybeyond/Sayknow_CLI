/**
 * Shared types for `skc migrate`.
 *
 * Imports MCP servers and skills from other coding agents (Claude Code, Codex,
 * OpenCode) into native SKC config. See the consensus plan under
 * `.skc/plans/ralplan/` for the full taxonomy and force/collision semantics.
 */
...
export interface MigrateAction {
	...
	name?: string;
	/** For skills: the effective SKC-loaded name (== slug). */
