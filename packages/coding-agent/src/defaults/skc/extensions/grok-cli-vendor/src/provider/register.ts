/**
 * SKC Grok Build provider — SuperGrok OAuth + cli-chat-proxy models.
 */
...
import { loginXai, refreshXaiToken, XAI_OAUTH_SCOPE } from '@sayknow-cli/ai/utils/oauth/xai';
import type { ExtensionAPI, ProviderConfig } from '@sayknow-cli/coding-agent';
...
export default function registerGrokCli(api: ExtensionAPI) {
  ...
  api.on('session_start', (_event, ctx) => {
    ...
    if (isGrokBuildBaseUrlOverrideIgnored()) {
      ctx.ui.notify(
        '[Grok Build] Ignoring unsafe Grok base URL override for OAuth credential safety. Set SKC_GROK_CLI_ALLOW_UNSAFE_BASE_URL=1 only for trusted local testing.',
        'warning',
      );
    }
  });
