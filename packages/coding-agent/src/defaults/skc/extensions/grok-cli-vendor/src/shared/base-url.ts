...
export function getBaseUrl(): string {
  const configured = process.env.SKC_GROK_CLI_BASE_URL || process.env.GROK_CLI_BASE_URL;
  if (!configured) return DEFAULT_GROK_BUILD_BASE_URL;

  const normalized = normalizeBaseUrl(configured);
  if (isAllowedCredentialHost(normalized)) return normalized;

  if (process.env.SKC_GROK_CLI_ALLOW_UNSAFE_BASE_URL === '1') return normalized;
...
export function isGrokBuildBaseUrlOverrideIgnored(): boolean {
  ...
  const normalized = normalizeBaseUrl(configured);
  return (
    !isAllowedCredentialHost(normalized) && process.env.SKC_GROK_CLI_ALLOW_UNSAFE_BASE_URL !== '1'
  );
}
