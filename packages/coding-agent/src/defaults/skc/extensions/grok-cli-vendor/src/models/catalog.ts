...
export function supportsReasoningEffort(modelId: string): boolean {
  ...
  // Effort-capable id not listed in SKC_GROK_CLI_MODELS env list — still honor prefix (avoids spurious 400s).
  return true;
}

// ─── SKC_GROK_CLI_MODELS env override ─────────────────────────────────────

/**
 * Resolve the active model list. If `SKC_GROK_CLI_MODELS` is set,
 * it filters/reorders the fallback list; unknown IDs get sensible defaults.
 */
export function resolveModels(): GrokCliModelConfig[] {
  const env = (process.env.SKC_GROK_CLI_MODELS || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
