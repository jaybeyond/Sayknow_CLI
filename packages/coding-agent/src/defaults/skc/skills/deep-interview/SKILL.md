...
handoff: .skc/_session-{sessionid}/specs/deep-interview-{slug}.md
level: 3
...
3.7. **Artifact path discipline**:
   - Final specs MUST resolve to `.skc/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly.
   - Write final specs and all ephemeral interview artifacts through the active SKC workflow/state CLI when available.
   - Direct `.skc/` file edits are forbidden unless an explicit force override is active; do not use `write`, `edit`, or `ast_edit` against `.skc/_session-{sessionid}/specs`, `.skc/_session-{sessionid}/plans`, `.skc/_session-{sessionid}/state`, or other `.skc/` paths during normal workflow operation.
...
**IMPORTANT:** On explicit execution selection, **MUST** use the chosen bundled SKC workflow skill entrypoint (`/skill:ralplan`, `/skill:ultragoal`, or `/skill:team`) inside the agent session. `skc ralplan` is a native CLI that accepts the documented skill flags and seeds local `.skc/_session-{sessionid}/state` receipts; agent sessions should still drive the consensus loop through `/skill:ralplan`. Implementation handoff defaults to `/skill:ultragoal`; `/skill:team` is reserved for when tmux-based interactive worker parallelization is genuinely required, and `skc team` is a native tmux runtime command used only when the Team workflow explicitly requires the CLI runtime. Do NOT implement directly. The deep-interview agent is a requirements agent, not an execution agent. If oversized initial context was summarized, pass the spec and prompt-safe summary forward, not the raw oversized source material. Without explicit execution selection, stop with the spec marked `pending approval`.
...
That command persists `.skc/_session-{sessionid}/specs/deep-interview-{slug}.md`, seeds ralplan in deliberate mode, and performs the safe deep-interview → ralplan state handoff. Skipping spec persistence leaves the Phase 5 chain blocked by design.
...
- [ ] Spec persisted to `.skc/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly via the SKC CLI (no direct `.skc/` edits without force override), covering every active topology component plus goal/constraints/acceptance criteria/clarity/ontology/transcript
...
See the Phase 5b "Approval-Gated Refinement Path" diagram for the full flow. In short: interview → spec at `.skc/_session-{sessionid}/specs/deep-interview-{slug}.md` → user selects "Refine with ralplan consensus" → `/skill:ralplan` (Planner/Architect/Critic consensus, plan written to `.skc/_session-{sessionid}/plans/`) → stop at `pending approval`. Execution is always a separate approval-gated step; deep-interview and ralplan never auto-invoke ultragoal or team just because a spec or plan exists.
