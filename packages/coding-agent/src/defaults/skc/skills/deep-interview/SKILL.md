...
handoff: .skc/specs/deep-interview-{slug}.md
level: 3

source: "forked from upstream deep-interview skill and rebranded for SKC"
---
...
- Auto-mode architects are read-only: no code edits, no `.skc/` mutation, no workflow chaining, no formatters, and no execution delegation.
...
If this raw bundled skill is loaded by SKC's native skill loader through `/skill:deep-interview`, do not treat that path as permission to skip rendered SKC setup. The user-facing invocation is `/skill:deep-interview`; do not recommend or advertise CLI bridge commands as the deep-interview entrypoint. Regardless of invocation path, Phase 0 below remains blocking and must resolve `skc.deepInterview.ambiguityThreshold` from settings before any announcement, state write, question, or ambiguity score.
...
4. **Carry threshold source forward mechanically**:
   ...
   - Include `threshold_source` in the first `skc state write` payload and preserve it on later state updates; do not edit `.skc/state` files directly unless an explicit force override is active.
...
3.7. **Artifact path discipline**:
   - Final specs MUST resolve to `.skc/specs/deep-interview-{slug}.md` exactly.
   - Write final specs and all ephemeral interview artifacts through the active SKC workflow/state CLI when available.
   - Direct `.skc/` file edits are forbidden unless an explicit force override is active; do not use `write`, `edit`, or `ast_edit` against `.skc/specs`, `.skc/plans`, `.skc/state`, or other `.skc/` paths during normal workflow operation.
...
1. **Generate the specification** using opus model with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
   ...
   - Apply the self-proofread once to newly generated spec prose before persistence, including generated natural-language table cells such as coverage notes, while preserving transcript answers, quoted/source text, code identifiers, file paths, commands, JSON/settings keys, table structure/fixed labels, and `.skc/specs/deep-interview-{slug}.md` unchanged.
2. **Write the final spec through the workflow CLI**: persist the artifact at `.skc/specs/deep-interview-{slug}.md`
   - Always use this exact final spec path. Do not write temporary working files to the repo root or other ad hoc paths; repos may allowlist `.skc/` for planning artifacts while protecting product branches.
   - Use the native deep-interview write command with `--write --stage final --slug {slug} --spec <markdown-or-path> [--json]` for artifact and state persistence; direct `.skc/` file edits are forbidden unless an explicit force override is active.
...
**IMPORTANT:** On explicit execution selection, **MUST** use the chosen bundled SKC workflow skill entrypoint (`/skill:ralplan`, `/skill:ultragoal`, or `/skill:team`) inside the agent session. `skc ralplan` is a native CLI that accepts the documented skill flags and seeds local `.skc/state` receipts; agent sessions should still drive the consensus loop through `/skill:ralplan`. Implementation handoff defaults to `/skill:ultragoal`; `/skill:team` is reserved for when tmux-based interactive worker parallelization is genuinely required, and `skc team` is a native tmux runtime command used only when the Team workflow explicitly requires the CLI runtime. Do NOT implement directly. The deep-interview agent is a requirements agent, not an execution agent. If oversized initial context was summarized, pass the spec and prompt-safe summary forward, not the raw oversized source material. Without explicit execution selection, stop with the spec marked `pending approval`.
...
That command persists `.skc/specs/deep-interview-{slug}.md`, seeds ralplan in deliberate mode, and performs the safe deep-interview → ralplan state handoff. Skipping spec persistence leaves the Phase 5 chain blocked by design.
...
- Preserve the SKC `ask` tool path for native interaction; do not introduce parallel structured-question transport into this skill
...
- Use `skc state write` / `skc state read` for interview state persistence; the initial and subsequent deep-interview state payloads must include `threshold_source` alongside `threshold`; do not edit `.skc/state` directly without force override.
- Use the SKC workflow CLI to save the final spec at `.skc/specs/deep-interview-{slug}.md` exactly; do not use `write`, `edit`, or `ast_edit` directly on `.skc/` paths without force override.
...
- [ ] Spec persisted to `.skc/specs/deep-interview-{slug}.md` exactly via the SKC CLI (no direct `.skc/` edits without force override), covering every active topology component plus goal/constraints/acceptance criteria/clarity/ontology/transcript
...
Optional settings in `.skc/settings.json`:
...
If interrupted, run `/skill:deep-interview` again. The skill resumes from SKC workflow state via `skc state read`; do not read or edit `.skc/state` files directly unless an explicit force override is active.
...
See the Phase 5b "Approval-Gated Refinement Path" diagram for the full flow. In short: interview → spec at `.skc/specs/deep-interview-{slug}.md` → user selects "Refine with ralplan consensus" → `/skill:ralplan` (Planner/Architect/Critic consensus, plan written to `.skc/plans/`) → stop at `pending approval`. Execution is always a separate approval-gated step; deep-interview and ralplan never auto-invoke ultragoal or team just because a spec or plan exists.
