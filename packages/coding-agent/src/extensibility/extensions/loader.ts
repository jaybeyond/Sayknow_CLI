...
import type { ImageContent, Model, TextContent, Tool, UsageReport } from "@sayknow-cli/ai";
import type { KeyId } from "@sayknow-cli/tui";
import { hasFsCode, isEacces, isEnoent, logger } from "@sayknow-cli/utils";
...
class ConcreteExtensionAPI implements ExtensionAPI, IExtensionRuntime {
	...
	constructor(
		public readonly pi: typeof import("@sayknow-cli/coding-agent"),
	...
	registerTool<
		TParams extends import("@sayknow-cli/ai").TSchema = import("@sayknow-cli/ai").TSchema,
		TDetails = unknown,
...
async function loadExtension(
	...
	try {
		...
		const api = new ConcreteExtensionAPI(
			await import("@sayknow-cli/coding-agent"),
			extension,
			runtime,
			cwd,
			eventBus,
...
export async function loadExtensionFromFactory(
	...
	const api = new ConcreteExtensionAPI(await import("@sayknow-cli/coding-agent"), extension, runtime, cwd, eventBus);
	await factory(api);
	return extension;
}
...
async function readExtensionManifest(packageJsonPath: string): Promise<ExtensionManifest | null> {
	try {
		const pkg = (await Bun.file(packageJsonPath).json()) as { skc?: ExtensionManifest; pi?: ExtensionManifest };
...
export async function discoverAndLoadExtensions(
	...
	// 1. Discover extension modules via capability API (native .skc/.pi only)
