...
import { getAgentDir, getProjectDir, isEnoent } from "@sayknow-cli/utils";
...
export async function installPlugin(packageName: string): Promise<InstalledPlugin> {
	...
	if (!(await pkgJson.exists())) {
		await pkgJson.write(JSON.stringify({ name: "skc-plugins", private: true, dependencies: {} }, null, 2));
	}
	...
	return {
		...
		manifest: pkg.skc || pkg.pi || { version: pkg.version },
