...
// fork, or the canonical @sayknow-cli scope itself) are remapped to this scope and
// resolved against the bundled copy that ships inside the skc binary. This
...
const CANONICAL_PI_SCOPE = "@sayknow-cli";

// Scopes that have historically been used to publish (or alias) the same set
// of internal pi-* packages. `@sayknow-cli` is intentionally included so that
...
const PI_SCOPE_ALIASES = ["sayknow-cli", "mariozechner", "earendil-works"] as const;

// Internal package basenames historically used by Pi plugins plus the current
// Sayknow package basenames bundled inside the skc binary.
...
const LEGACY_PI_FILE_PREFIX = "skc-legacy-pi-file:";
const LEGACY_PI_FILE_NAMESPACE = "skc-legacy-pi-file";
...
export async function loadLegacyPiModule(resolvedPath: string): Promise<unknown> {
	const root = path.join(os.tmpdir(), "skc-legacy-pi-file", `entry-${Bun.hash(resolvedPath).toString(36)}`);
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): { path: string } | undefined {
	...
	// Primary: resolve the canonical @sayknow-cli/* specifier from the host binary
...
export function installLegacyPiSpecifierShim(): void {
	...
	Bun.plugin({
		...
		setup(build) {
			...
			build.onResolve({ filter: /^skc-legacy-pi-file:/, namespace: "file" }, args => ({
