...
import { SkcPluginLoadError, type SkcPluginRegistry, type SkcPluginRegistryEntry, type SkcPluginScope } from "./types";
...
export function registryRootForScope(scope: SkcPluginScope, cwd: string): string {
	return scope === "user" ? skcPluginUserRoot() : skcPluginProjectRoot(cwd);
}

export function registryPathForScope(scope: SkcPluginScope, cwd: string): string {
	return path.join(registryRootForScope(scope, cwd), REGISTRY_FILENAME);
}

function emptyRegistry(scope: SkcPluginScope): SkcPluginRegistry {
 ...
 */
export function sortRegistryEntries(entries: SkcPluginRegistryEntry[]): SkcPluginRegistryEntry[] {
	const scopeRank = (scope: SkcPluginScope): number => (scope === "user" ? 0 : 1);
...
export async function readRegistry(scope: SkcPluginScope, cwd: string): Promise<SkcPluginRegistry> {
	...
	try {
		...
		throw new SkcPluginLoadError("invalid_manifest", `Corrupt SKC plugin registry at ${registryPath}`, {
			cause: error instanceof Error ? error : undefined,
		});
	}
	if (typeof parsed !== "object" || parsed === null || (parsed as SkcPluginRegistry).version !== 1) {
		throw new SkcPluginLoadError("invalid_manifest", `Unsupported SKC plugin registry shape at ${registryPath}`);
	}
	const registry = parsed as SkcPluginRegistry;
		...
		try {
			...
			if (Date.now() > deadline) {
				throw new SkcPluginLoadError(
					"install_conflict",
					`Timed out acquiring SKC plugin registry lock at ${lockPath}; remove it manually if no install is running`,
				);
			}
...
export async function withRegistryLock<T>(scope: SkcPluginScope, cwd: string, fn: () => Promise<T>): Promise<T> {
 ...
 */
export async function writeRegistryUnlocked(registry: SkcPluginRegistry, cwd: string): Promise<void> {
	const registryPath = registryPathForScope(registry.scope, cwd);
	await fs.mkdir(path.dirname(registryPath), { recursive: true });
	const sorted: SkcPluginRegistry = { ...registry, plugins: sortRegistryEntries(registry.plugins) };
 ...
 */
export async function writeRegistry(registry: SkcPluginRegistry, cwd: string): Promise<void> {
...
export async function updateRegistry(
	...
	cwd: string,
	mutator: (entries: SkcPluginRegistryEntry[]) => SkcPluginRegistryEntry[],
): Promise<SkcPluginRegistry> {
	return await withRegistryLock(scope, cwd, async () => {
		...
		const next: SkcPluginRegistry = { version: 1, scope, plugins: sortRegistryEntries(nextEntries) };
 ...
 */
export async function loadEffectiveSkcPluginRegistry(cwd: string): Promise<SkcPluginRegistryEntry[]> {
...
export function registryEntryFingerprint(entry: SkcPluginRegistryEntry): string {
