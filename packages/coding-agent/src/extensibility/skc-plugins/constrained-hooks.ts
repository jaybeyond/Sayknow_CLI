...
import { SkcPluginLoadError, type SkcPluginRegistryEntry } from "./types";
...
function collectDeclaredHooks(entries: readonly SkcPluginRegistryEntry[]): DeclaredHook[] {
...
async function loadOneHook(
	...
	try {
		...
		const code = error instanceof SkcPluginLoadError ? error.code : "security_policy";
		return {
			hook: null,
			quarantine: {
				plugin: declared.plugin,
...
export async function loadConstrainedPluginHooks(input: { cwd: string }): Promise<ConstrainedHookLoadResult> {
	const effective = await loadEffectiveSkcPluginRegistry(input.cwd);
