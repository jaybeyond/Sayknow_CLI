...
export async function loadActiveSubskillTools(input: {
	...
	for (const error of result.errors) {
		logger.warn("Skipping SKC plugin sub-skill tool", { path: error.path, error: error.error });
	}
	...
	for (const loadedTool of result.tools) {
		...
		if (reservedToolNames.has(name)) {
			logger.warn("Skipping SKC plugin sub-skill tool name because it conflicts with a reserved tool", { name });
			continue;
		}
		if (seenNames.has(name)) {
			logger.warn("Skipping duplicate SKC plugin sub-skill tool name", { name, path: loadedTool.path });
			continue;
		}
		seenNames.add(name);
