...
import type { NormalizedAppendixSurface, SkcPluginRegistryEntry, SkcSubskillParentAgent } from "./types";
...
function sanitizeBody(text: string): string {
	...
	// <developer>/<skc-subskill> tag that escapes the lower-authority block.
	const stripped = text.replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/g, "");
	return stripped.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

async function readAppendixBody(entry: SkcPluginRegistryEntry, surface: NormalizedAppendixSurface): Promise<string> {
...
export interface RenderedPluginAppendices {
	...
	byAgent: Map<SkcSubskillParentAgent, string>;
...
export async function renderPluginAppendices(
	...
	const byAgent = new Map<SkcSubskillParentAgent, string[]>();
	...
	for (const entry of entries) {
		...
		for (const sa of entry.surfaces.systemAppendices) {
			...
			if (!body) continue;
			const block = `<skc-plugin-system-appendix plugin="${escapeAttr(entry.name)}" name="${escapeAttr(sa.name)}" sha256="${sa.contentHash}" authority="appendix-lower-than-system">\n${body}\n</skc-plugin-system-appendix>`;
		...
		for (const aa of entry.surfaces.agentAppendices) {
			...
			if (!body) continue;
			const block = `<skc-plugin-agent-appendix plugin="${escapeAttr(entry.name)}" agent="${escapeAttr(aa.agent)}" name="${escapeAttr(aa.name)}" sha256="${aa.contentHash}" authority="appendix-lower-than-agent">\n${body}\n</skc-plugin-agent-appendix>`;
			if (!admit(block)) continue;
