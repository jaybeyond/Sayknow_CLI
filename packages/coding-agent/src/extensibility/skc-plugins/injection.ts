...
import { type LoadedSubskillActivation, SKC_SUBSKILL_PARENT_AGENTS } from "./types";
...
export function wrapSubskillBlock(
	...
	return `\n\n---\n\n<skc-subskill plugin="${escapeAttribute(activation.plugin)}" name="${escapeAttribute(activation.subskillName)}" parent="${escapeAttribute(activation.parent)}" phase="${escapeAttribute(activation.phase)}" arg="${escapeAttribute(activation.activationArg)}">\n${body}\n</skc-subskill>`;
}
...
export async function buildAgentSubskillInjection(input: {
	...
	if (!(SKC_SUBSKILL_PARENT_AGENTS as readonly string[]).includes(input.agentName)) return "";
...
import type { SkcPluginRegistryEntry } from "./types";
...
function wrapAdvert(kind: "skill" | "agent", parent: string, lines: string[]): string {
	return `<skc-plugin-subskill-advertisement parent="${clampField(parent)}" kind="${kind}">\n${lines.join("\n")}\n</skc-plugin-subskill-advertisement>`;
}
...
function collectAdverts(entries: readonly SkcPluginRegistryEntry[], parent: string, phase?: string): AdvertItem[] {
...
export function buildSubskillAdvertisement(
	entries: readonly SkcPluginRegistryEntry[],
	parent: string,
	phase?: string,
): string {
...
export function buildAgentSubskillAdvertisement(entries: readonly SkcPluginRegistryEntry[], agentName: string): string {
