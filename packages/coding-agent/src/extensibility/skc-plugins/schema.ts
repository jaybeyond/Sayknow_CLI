import {
	...
	type SkcPluginAgentAppendixManifestEntry,
	type SkcPluginAppendixManifestEntry,
	type SkcPluginHookManifestEntry,
	SkcPluginLoadError,
	type SkcPluginManifest,
	type SkcPluginMcpManifestEntry,
	type SkcPluginMcpTransport,
	type SkcPluginToolManifestEntry,
	type SkcSubskillParentAgent,
	type SubskillFrontmatter,
} from "./types";

/**
 * Top-level surfaces that may never appear in a SKC plugin bundle: bundles may
...
const MCP_TRANSPORTS: readonly SkcPluginMcpTransport[] = ["stdio", "http", "sse"];
...
function manifestString(value: unknown, field: string, manifestPath: string): string {
	if (typeof value !== "string" || value.trim().length === 0) {
		throw new SkcPluginLoadError(
			"invalid_manifest",
			`Invalid SKC plugin manifest at ${manifestPath}: ${field} must be a non-empty string`,
		);
	}
	return value;
}
...
function optionalArray(value: unknown, field: string, manifestPath: string): unknown[] {
	...
	if (!Array.isArray(value)) {
		throw new SkcPluginLoadError(
			"invalid_manifest",
			`Invalid SKC plugin manifest at ${manifestPath}: ${field} must be an array`,
		);
	}
	return value;
}
...
function parseTools(value: unknown, manifestPath: string): SkcPluginToolManifestEntry[] {
	...
	return raw.map((entry, index) => {
		...
		if (typeof entry === "string") {
			if (entry.trim().length === 0) {
				throw new SkcPluginLoadError(
					"invalid_manifest",
					`Invalid SKC plugin manifest at ${manifestPath}: tools[${index}] must be a non-empty path`,
				);
			}
		...
		if (!isRecord(entry)) {
			throw new SkcPluginLoadError(
				"invalid_manifest",
				`Invalid SKC plugin manifest at ${manifestPath}: tools[${index}] must be a string or object`,
			);
		}
...
function parseHooks(value: unknown, manifestPath: string): SkcPluginHookManifestEntry[] {
	const raw = optionalArray(value, "hooks", manifestPath);
	return raw.map((entry, index) => {
		if (!isRecord(entry)) {
			throw new SkcPluginLoadError(
				"invalid_manifest",
				`Invalid SKC plugin manifest at ${manifestPath}: hooks[${index}] must be an object`,
			);
		}
		...
		if (entry.phase !== undefined) {
			if (entry.phase !== "before" && entry.phase !== "after") {
				throw new SkcPluginLoadError(
					"invalid_manifest",
					`Invalid SKC plugin manifest at ${manifestPath}: hooks[${index}].phase must be "before" or "after"`,
				);
			}
			phase = entry.phase;
		}
...
function parseMcps(value: unknown, manifestPath: string): SkcPluginMcpManifestEntry[] {
	const raw = optionalArray(value, "mcps", manifestPath);
	return raw.map((entry, index) => {
		if (!isRecord(entry)) {
			throw new SkcPluginLoadError(
				"invalid_manifest",
				`Invalid SKC plugin manifest at ${manifestPath}: mcps[${index}] must be an object`,
			);
		}
		const name = manifestString(entry.name, `mcps[${index}].name`, manifestPath);
		const transport = entry.transport;
		if (typeof transport !== "string" || !MCP_TRANSPORTS.includes(transport as SkcPluginMcpTransport)) {
			throw new SkcPluginLoadError(
				"invalid_manifest",
				`Invalid SKC plugin manifest at ${manifestPath}: mcps[${index}].transport must be one of ${MCP_TRANSPORTS.join(", ")}`,
			);
		}
		const command =
		...
		if (entry.args !== undefined) {
			if (!Array.isArray(entry.args) || !entry.args.every(item => typeof item === "string")) {
				throw new SkcPluginLoadError(
					"invalid_manifest",
					`Invalid SKC plugin manifest at ${manifestPath}: mcps[${index}].args must be a string array`,
				);
			}
		...
		if (entry.headers !== undefined) {
			if (!isRecord(entry.headers) || !Object.values(entry.headers).every(v => typeof v === "string")) {
				throw new SkcPluginLoadError(
					"invalid_manifest",
					`Invalid SKC plugin manifest at ${manifestPath}: mcps[${index}].headers must be a string map`,
				);
			}
		...
		return { name, transport: transport as SkcPluginMcpTransport, command, args, cwd, url, headers, sha256 };
	});
}

function parseAppendixEntry(entry: unknown, field: string, manifestPath: string): SkcPluginAppendixManifestEntry {
	if (!isRecord(entry)) {
		throw new SkcPluginLoadError(
			"invalid_manifest",
			`Invalid SKC plugin manifest at ${manifestPath}: ${field} must be an object`,
		);
	}
	...
	if (entry.content !== undefined && typeof entry.content !== "string") {
		throw new SkcPluginLoadError(
			"invalid_manifest",
			`Invalid SKC plugin manifest at ${manifestPath}: ${field}.content must be a string`,
		);
	}
...
function parseSystemAppendix(value: unknown, manifestPath: string): SkcPluginAppendixManifestEntry[] {
...
function parseAgentAppendix(value: unknown, manifestPath: string): SkcPluginAgentAppendixManifestEntry[] {
	...
	return raw.map((entry, index) => {
		...
		if (typeof agent !== "string" || !SKC_SUBSKILL_PARENT_AGENTS.includes(agent as SkcSubskillParentAgent)) {
			throw new SkcPluginLoadError(
				"invalid_parent",
				`Invalid SKC plugin manifest at ${manifestPath}: agent-appendix[${index}].agent must be one of ${SKC_SUBSKILL_PARENT_AGENTS.join(", ")}`,
			);
		}
		return { ...base, agent: agent as SkcSubskillParentAgent };
	});
}
...
export function parseManifest(raw: unknown, manifestPath: string): SkcPluginManifest {
	...
	for (const key of UNSUPPORTED_ALIAS_KEYS) {
		if (Object.hasOwn(raw, key)) {
			throw new SkcPluginLoadError(
				"unsupported_surface",
				`Unsupported SKC plugin surface in ${manifestPath}: ${key} (use the canonical "mcps" key)`,
			);
		}
	}
	...
	for (const key of Object.keys(raw)) {
		if (!KNOWN_MANIFEST_KEYS.has(key)) {
			throw new SkcPluginLoadError(
				"unsupported_surface",
				`Unsupported SKC plugin surface in ${manifestPath}: ${key}`,
			);
		}
	}
