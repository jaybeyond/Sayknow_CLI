...
import { loadEffectiveSkcPluginRegistry, registryPathForScope } from "./registry";
import { type SessionQuarantine, type SessionValidationResult, validateSessionBundles } from "./session-validation";
import type { SkcPluginRegistryEntry, SkcPluginScope } from "./types";
...
interface ValidatedPluginRegistry {
	effective: SkcPluginRegistryEntry[];
	active: SkcPluginRegistryEntry[];
	quarantine: SessionQuarantine[];
...
const registryScopes: SkcPluginScope[] = ["user", "project"];
...
async function snapshotPluginFiles(entries: readonly SkcPluginRegistryEntry[]): Promise<FileSnapshot[]> {
...
async function verifyEntryHashesCached(entry: SkcPluginRegistryEntry): Promise<SessionQuarantine | null> {
	for (const file of entry.copiedFiles) {
		const abs = path.join(entry.pluginRoot, file.relativePath);
		const snapshot = await snapshotExistingFile(abs);
		if (!snapshot) {
