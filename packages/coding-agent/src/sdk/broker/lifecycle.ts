...
const defaultStateRoot = (cwd: string) => path.join(path.resolve(cwd), ".skc", "state");
...
export function readSessionLifecycleLaunchRequest(
	...
	if (!value) throw new Error("SKC_SDK_LIFECYCLE_REQUEST is required.");
	...
	if (
		...
		throw new Error("SKC_SDK_LIFECYCLE_REQUEST is invalid.");
...
function stateRoot(input: Input, cwd: string | undefined): string | undefined {
	...
	return cwd ? path.join(cwd, ".skc", "state") : undefined;
}
...
async function validateLiveResumeScope(
	...
	if (!suppliedRoot || !hasDefaultStateRoot(requestedCwd, suppliedRoot))
		return fail("invalid_input", "stateRoot must be the default .skc/state for cwd.");
	try {
		...
		return [
			{
				...
				attempt,
				plannedPath: path.join(directory, `.skc-delete-${suffix}-${path.basename(file)}`),
			},
		];
	});
...
function validateLifecycleCleanupFile(root: string, id: string, file: LifecycleCleanupFile): boolean {
	...
	if (
		...
		!path.basename(planned).startsWith(".skc-delete-") ||
...
function legacyMetadataCleanupPlan(cleanup: CleanupEvidence): CleanupEvidence | undefined {
	...
	if (
		...
		!path.basename(plannedPath).startsWith(".skc-delete-") ||
				...
				? [
						{
							...
							attempt: 1,
							plannedPath: path.join(directory, `.skc-delete-${randomUUID()}-${path.basename(readyPath)}`),
						},
					]
				: []),
		],
	...
	return {
		...
		lifecycleFiles: files.map(({ metadataPath, metadata }) => ({
			...
			plannedPath: path.join(
				...
				`.skc-delete-${randomUUID()}-${path.basename(metadataPath)}`,
			),
		})),
	};
}
	...
	for (let index = 0; index < activeCleanup.lifecycleFiles!.length; index++) {
		...
		if (file.detachedPath || activePath === file.plannedPath) {
			const nextFile: LifecycleCleanupFile = {
				...
				plannedPath: path.join(path.dirname(file.path), `.skc-delete-${randomUUID()}-${path.basename(file.path)}`),
			};
			const lifecycleFiles = activeCleanup.lifecycleFiles!.map((candidate, candidateIndex) =>
...
async function removeOwnedLifecycleArtifacts(root: string, id: string, expected: EffectMarker): Promise<boolean> {
	...
	const plannedEndpointPath = path.join(
		...
		`.skc-delete-endpoint-${expected.effectMarker}-${path.basename(endpointPath)}`,
	);
	const retryEndpointPath = path.join(
		path.dirname(endpointPath),
		`.skc-delete-endpoint-retry-${expected.effectMarker}-${path.basename(endpointPath)}`,
	);
	const finalEndpointPath = path.join(
		path.dirname(endpointPath),
		`.skc-delete-endpoint-final-${expected.effectMarker}-${path.basename(endpointPath)}`,
	);
...
function worktreeIntent(plan: SkcLaunchWorktreePlan | undefined): LifecycleWorktreeIntent | undefined {
...
function preparePlannedWorktree(plan: SkcLaunchWorktreePlan): SessionLifecycleWorktreeReceipt {
...
async function launchInput(
	...
	let cwd = sourceCwd;
	let worktreePlan: SkcLaunchWorktreePlan | undefined;
	if (worktree) {
		try {
			const planned = planLaunchWorktree(
				sourceCwd,
	...
	if (
		(plannedArtifactsPath &&
			(path.dirname(plannedArtifactsPath) !== path.dirname(cleanup.transcriptPath) ||
				!path.basename(plannedArtifactsPath).startsWith(".skc-delete-"))) ||
		(plannedTranscriptPath &&
			(path.dirname(plannedTranscriptPath) !== path.dirname(cleanup.transcriptPath) ||
				!path.basename(plannedTranscriptPath).startsWith(".skc-delete-"))) ||
		(cleanup.artifactsRemoved !== true &&
			...
			(path.dirname(cleanup.artifactTree.plannedPath) !== path.dirname(cleanup.transcriptPath) ||
				!path.basename(cleanup.artifactTree.plannedPath).startsWith(".skc-delete-") ||
		...
		try {
			...
			const spawned = spawn(cmd.file, cmd.args, {
				...
				env: {
					...
					SKC_AGENT_DIR: broker.settings.agentDir,
					SKC_CODING_AGENT_DIR: broker.settings.agentDir,
					SKC_SESSION_ID: launch.id,
					SKC_STATE_ROOT: launch.root,
					SKC_LIFECYCLE_REQUEST_ID: effectMarker,
			...
			validated.target.detachedArtifactsPath !== validated.target.plannedArtifactsPath
				...
				: {
						plannedArtifactsPath: path.join(
							...
							`.skc-delete-${randomUUID()}-artifacts`,
						),
					}),
			...
			validated.target.detachedTranscriptPath !== validated.target.plannedTranscriptPath
				...
				: {
						plannedTranscriptPath: path.join(
							path.dirname(validated.target.transcriptPath),
							`.skc-delete-${randomUUID()}-transcript`,
						),
					}),
		};
