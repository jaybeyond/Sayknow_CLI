...
function notificationConfigurationError(): Error {
	return new Error("skc_notify_daemon_invalid_configuration");
}
...
export function isNotificationStreamingEnabled(input: { cfg: NotificationConfig; env: NodeJS.ProcessEnv }): boolean {
	const override = input.env.SKC_NOTIFICATIONS_STREAM?.trim().toLowerCase();
...
export interface NotificationHostEligibilityInput {
	...
	/** Whether this process was spawned by one of SKC's marked child spawn sites. */
...
export function isNotificationHostEligible(input: NotificationHostEligibilityInput): boolean {
	...
	if (input.spawnedBySkc && input.sessionScope === "primary") return false;
	return true;
}
...
export function shouldRegisterNotificationsExtension(input: {
	...
	if (
		!isNotificationHostEligible({
			...
			spawnedBySkc: input.spawnedBySkc,
		})
	) {
		return false;
	}
	return (
		input.env.SKC_NOTIFICATIONS === "1" ||
		Boolean(input.env.SKC_NOTIFICATIONS_TOKEN) ||
 ...
 *  1) env.SKC_NOTIFICATIONS === "0"  -> false (hard opt-out)
 *  2) sessionDisabled === true       -> false (local /notify off)
 *  3) env.SKC_NOTIFICATIONS === "1" || env.SKC_NOTIFICATIONS_TOKEN present -> true (legacy explicit)
 *  4) spawned SKC child with `sessionScope=primary` -> false
...
export function isSessionNotificationsEnabled(input: {
	...
	/** This process was programmatically spawned by SKC (consumed at SDK startup). */
	spawnedBySkc?: boolean;
}): boolean {
	if (input.env.SKC_NOTIFICATIONS === "0") return false;
