#!/usr/bin/env bun
/**
 * Reference CLI for the Sayknow-CLI SDK Telegram client.
 *
 ...
 * under <repo>/.skc/state/sdk/ is used.
 */
...
function printHelpAndExit(): never {
	process.stdout.write(
		[
			"skc notifications — Telegram reference client",
			"",
			"  --bot-token <token>     Telegram bot token (or env SKC_TG_BOT_TOKEN)",
			"  --chat-id <id>          Target chat id (or env SKC_TG_CHAT_ID; auto-resolved if omitted)",
			"  --endpoint-file <path>  Session endpoint discovery file",
			"  --session-id <id>       Resolve <repo>/.skc/state/sdk/<id>.json",
...
async function main(): Promise<void> {
	...
	const botToken = args.botToken ?? process.env.SKC_TG_BOT_TOKEN;
	if (!botToken) {
		process.stderr.write("error: --bot-token (or SKC_TG_BOT_TOKEN) is required\n");
		process.exit(2);
	}

	const endpointFile =
		args.endpointFile ??
		(args.sessionId ? path.join(args.repo, ".skc", "state", "sdk", `${args.sessionId}.json`) : undefined) ??
		findLatestEndpoint(args.repo);
	if (!endpointFile || !fs.existsSync(endpointFile)) {
		process.stderr.write(
			`error: no endpoint file found (looked under ${args.repo}/.skc/state/sdk). Start a session with SKC_NOTIFICATIONS=1 first.\n`,
		);
		process.exit(2);
	}

	const chatId = args.chatId ?? process.env.SKC_TG_CHAT_ID ?? (await resolveChatId(botToken, apiBase));
	if (!args.force && activeDaemonOwnsToken({ botToken, chatId })) {
		process.stderr.write(
			"an active skc notifications daemon already owns this bot token; running a second poller will cause Telegram 409 conflicts. Re-run with --force to override.\n",
		);
		process.exit(1);
	}
