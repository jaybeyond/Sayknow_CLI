...
// contract in `crates/skc-sdk/src/lifecycle.rs`).
//
...
function resolveToken(): string {
	// `SKC_NOTIFICATIONS_TOKEN` remains an enablement compatibility flag, never
...
export function createNotificationsExtension(
	...
	async function startSession(ctx: ExtensionContext): Promise<SessionStartResult> {
		...
		const lifecycleRequestId = safeLifecycleRequestId(process.env.SKC_LIFECYCLE_REQUEST_ID);
		...
		try {
			...
			// @sayknow-cli/natives binary predates it (linked/deduped installs where the
			...
			if (typeof server.onNegotiatedCapabilities !== "function") {
				throw new Error(
					"@sayknow-cli/natives is out of date: missing onNegotiatedCapabilities. Rebuild the native addon (bun --cwd=packages/natives run build).",
				);
			}
