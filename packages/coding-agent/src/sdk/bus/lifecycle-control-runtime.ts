...
import type { NotificationControlServer as NativeNotificationControlServer } from "@sayknow-cli/natives";
import { logger } from "@sayknow-cli/utils";
import { readLinuxProcStartTime } from "../../skc-runtime/linux-proc";
import {
...
} from "../../skc-runtime/managed-owner-admission";
import {
...
import { tmuxRuntimeSessionPath } from "../../skc-runtime/session-layout";
import {
...
function ledgerReadError(error: unknown): Error {
	...
	return new Error(`skc_lifecycle_ledger_read_failed:${code.slice(0, 32)}`);
}
...
function fsyncLedgerParentDirectory(directory: string): void {
	...
	try {
		...
		if (process.platform === "win32" && isUnsupportedDirectorySyncError(error)) {
			recordBoundedLifecycleDiagnostic("SKC lifecycle ledger directory sync unsupported", true);
			return;
		}
		throw error;
	}
...
function tmuxSessionNameFor(sessionId: string): string {
	return `skc_lc_${sessionId}`;
}

/** Build the `skc` argv for a create target (existing path / worktree / dir).
 *
...
function lifecycleRuntimeStateFile(cwd: string, sessionId: string, tmuxSession: string): string {
	return tmuxRuntimeSessionPath(cwd, sessionId, buildSkcTmuxSessionSlug(tmuxSession));
}
...
async function preflightLifecycleTmuxOwner(input: {
	...
	if (server.state === "unsafe") throw new Error("skc_lifecycle_owner_server_unsafe");
	if (server.state === "unverifiable") throw new Error("skc_lifecycle_owner_server_unverifiable");
	if (
		server.state === "safe" &&
	...
	if (server.state === "absent") {
		try {
			...
			if (error instanceof Error && error.message === "skc_lifecycle_owner_server_unverifiable") throw error;
			throw new Error("skc_lifecycle_owner_server_unverifiable");
		}
	}
}
...
async function executeLifecycleTmuxOwnerPlan(input: {
	...
	if (!plan.ok) throw new Error(`skc_lifecycle_owner_${plan.code}`);
	const preSpawnProof =
	...
	if (created.exitCode !== 0) {
		if (!attempt.nativeSessionId || attempt.serverPid === undefined || attempt.serverStartTime === undefined)
			throw new Error("skc_lifecycle_spawn_failed_cleanup_uncertain");
		throw new Error("skc_lifecycle_spawn_failed");
	}
	if (!attempt.nativeSessionId) throw new Error("skc_lifecycle_spawn_failed_cleanup_uncertain");
...
function cleanupUncertain(): Error {
	return new Error("skc_lifecycle_cleanup_uncertain");
}
...
async function cleanupLifecycleAttempt(input: {
	...
	const guarded = Bun.spawnSync(
		[
			...
			`kill-session -t '${input.nativeSessionId}' ; display-message -p __skc_lifecycle_cleanup_ok__`,
			"display-message -p __skc_lifecycle_cleanup_refused__",
		],
		{ stdout: "pipe", stderr: "pipe", env: input.env },
	);
	if (guarded.exitCode !== 0 || guarded.stdout.toString().trim() !== "__skc_lifecycle_cleanup_ok__")
		throw cleanupUncertain();
}
...
async function assertLifecycleTmuxServerSafe(input: {
	...
	if (
		...
		throw new Error(`skc_lifecycle_owner_${proof.state === "unsafe" ? "server_unsafe" : "server_unverifiable"}`);
}
...
/** Real daemon-safe tmux launcher: canonical owner-isolation plan + SKC tags. */
export function daemonSpawnCreate(
	...
	return async (
		...
		const tmuxBinary = resolveSkcTmuxBinary({ env });
		if (tmuxBinary.isPsmux) throw new Error("skc_lifecycle_psmux_unsupported");
		...
		// Shared identity create fence, before the final baseline capture and any
		// spawn preparation. The SDK-bus producer is a separate implementation
		// from the notification one and must acquire it in its own right.
		...
		try {
			...
			const childEnv: Record<string, string> = {
				...
				SKC_MANAGED_OWNER_COMMAND_JSON: JSON.stringify(["skc", ...args]),
				...
				...(predecessor
					? {
							...
							[MANAGED_OWNER_TRANSCRIPT_PATH_ENV]: env.SKC_SESSION_FILE ?? "",
						}
					: {}),
			};
			...
			const envPairs = Object.entries(childEnv)
				...
				.join(" ");
			const command = `cd ${shellQuote(cwd)} && exec env ${envPairs} skc ${shellQuote(MANAGED_OWNER_SUPERVISOR_ARG)}`;
...
async function applyRequiredLifecycleTmuxMetadata(
	...
	if (
		...
		throw new Error("skc_lifecycle_metadata_required_missing");
	const commands = buildSkcTmuxProfileCommands(target, env, metadata)
	...
	const result = Bun.spawnSync(
		[
			...
			`${commands} ; display-message -p __skc_lifecycle_metadata_ok__`,
			"display-message -p __skc_lifecycle_metadata_refused__",
		],
		{ stdout: "pipe", stderr: "pipe", env },
	);
	if (result.exitCode !== 0 || result.stdout.toString().trim() !== "__skc_lifecycle_metadata_ok__")
		throw new Error("skc_lifecycle_metadata_write_failed");
}

/** Real force-close effect (SKC-managed only, id-matched). */
export function daemonCloseSession(
	...
	deps: {
		...
		findSession?: (name: string, env: NodeJS.ProcessEnv) => SkcTmuxSessionStatus | undefined;
	} = {},
) {
	return async (target: { sessionId: string; tmuxSession?: string; sessionStateFile?: string }) => {
		...
		await (deps.forceClose ?? forceCloseSkcTmuxSession)(name, env, target.sessionId, target.sessionStateFile);
		return { processGone: (deps.findSession ?? findSkcTmuxSessionByName)(name, env) === undefined };
	};
}

/** Real resume effect: reattach if a live SKC session matches; else resolve the
...
export function daemonResumeSession(
	...
	opts: {
		...
		sessionsRoot?: string;
		listSessions?: (env: NodeJS.ProcessEnv) => SkcTmuxSessionStatus[];
		ownerIsolationProbe?: OwnerIsolationProbe;
	} = {},
) {
	return async (target: {
		...
		const tmuxBinary = resolveSkcTmuxBinary({ env });
		if (tmuxBinary.isPsmux) throw new Error("skc_lifecycle_psmux_unsupported");
		const live = (opts.listSessions?.(env) ?? listSkcTmuxSessions(env)).filter(
		...
		if (live.length === 1) {
			...
			await assertLifecycleTmuxServerSafe({
				tmux: resolveSkcTmuxCommand(env),
				env,
				ownerIsolationProbe: opts.ownerIsolationProbe,
			});
			return {
		...
		// blindly spawning `skc --resume <prefix>` against a non-authoritative id.
		...
		if (recent.kind === "error") throw new Error(`skc_lifecycle_saved_sessions_unavailable: ${recent.message}`);
		...
		if (typeof resolvedResumeCwd !== "string" || !resumeCwdStat?.isDirectory()) {
			throw new Error(`skc_lifecycle_resume_cwd_unavailable: ${resolvedResumeCwd ?? "(missing)"}`);
		}
		const tmux = tmuxBinary.command;
		const name = tmuxSessionNameFor(resumeId);
		const sessionStateFile = lifecycleRuntimeStateFile(resolvedResumeCwd, resumeId, name);
		const stateDir = path.dirname(sessionStateFile);
		// Same shared fence: the cold restart reuses the historical id, so it can
		// collide with a restore of that exact identity.
		...
		try {
			...
			const childEnv: Record<string, string> = {
				...
				[SKC_TMUX_OWNER_STATE_DIR_ENV]: stateDir,
				[SKC_TMUX_OWNER_SERVER_KEY_ENV]: "default",
				SKC_MANAGED_OWNER_COMMAND_JSON: JSON.stringify(["skc", "--resume", resumeId]),
				[MANAGED_OWNER_RUN_ID_ENV]: runId,
				[MANAGED_OWNER_INCARNATION_ENV]: incarnation,
				...(predecessor
					? {
			...
			const envPairs = Object.entries(childEnv)
				.map(([key, value]) => `${key}=${shellQuote(value)}`)
				.join(" ");
			const command = `cd ${shellQuote(resolvedResumeCwd)} && exec env ${envPairs} skc ${shellQuote(MANAGED_OWNER_SUPERVISOR_ARG)}`;
			// Record the attempt name BEFORE anything can spawn. Without this the
			// reservation stays at `reserved`, and a successor would read a crash
			// here as "no child can exist" and reclaim an identity that already
			// has an untagged child.
...
function recordLifecycleControlDiagnostic(): void {
	recordBoundedLifecycleDiagnostic("SKC lifecycle control request failed");
}
...
export function buildOrchestratorDeps(input: {
	...
	return {
		...
		isPsmuxProvider: () => resolveSkcTmuxBinary({ env }).isPsmux,
...
export const createNativeControlServer: LifecycleControlServerFactory = ({ token, ownerId, agentDir }) => {
	...
	const { NotificationControlServer } = require("@sayknow-cli/natives") as NativeControlServerModule;
