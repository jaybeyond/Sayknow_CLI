 ...
 *
 * Ranks SKC sessions by session-history file mtime (most recent first) and
...
import { verifyOwnerOnlyPathSecurity } from "@sayknow-cli/natives";
import { getAgentDir, getSessionsDir } from "@sayknow-cli/utils";
		...
		for (const directory of directories) {
			...
			try {
				const binding = JSON.parse(
					await fs.readFile(path.join(sessionsRoot, directory.name, ".skc-managed-session-scope.v2.json"), "utf8"),
