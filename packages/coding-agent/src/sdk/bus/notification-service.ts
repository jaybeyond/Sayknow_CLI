 ...
 *
 * Transport-agnostic, secret-safe operations consumed by BOTH the `skc notify`
...
const nodeServiceFs: NotificationServiceFs = {
	...
	exactUnlink: async (file, identity) =>
		exactUnlinkNotificationFile(file, identity, `.skc-delete-notification-endpoint-${crypto.randomUUID()}.json`),
...
function defaultStateRoot(): string {
	return path.join(process.cwd(), ".skc", "state");
}
...
export async function sendNotificationTest(opts: TestOptions): Promise<NotificationTestResult> {
	...
	if (!isTelegramConfigured(cfg)) {
		return {
			...
			adapter: "telegram",
			chatId: cfg.chatId,
			detail: "Telegram is not configured (need notifications.enabled + botToken + chatId). Run `skc notify setup`.",
		};
	}
	const fetchImpl = deps.fetchImpl ?? globalThis.fetch;
	const apiBase = (deps.apiBase ?? DEFAULT_API_BASE).replace(/\/$/, "");
	const text = opts.text ?? "SKC notifications test message. If you can read this, delivery works.";
	try {
