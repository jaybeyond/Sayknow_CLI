...
import { $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@sayknow-cli/utils";
import {
...
import { type ConstrainedPluginHook, loadConstrainedPluginHooks } from "../extensibility/skc-plugins/constrained-hooks";
import { resolveCurrentPhaseForParent } from "../extensibility/skc-plugins/injection";
import {
	buildPluginMcpConfigs,
	loadAlwaysOnPluginTools,
	renderAlwaysOnSystemAppendices,
} from "../extensibility/skc-plugins/runtime-adapters";
import { loadActiveSubskillTools } from "../extensibility/skc-plugins/tools";
...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.skc/agent */
	agentDir?: string;
	...
	/** Explicit parent/phase used to load active SKC sub-skill tools for this session. */
	skcSubskillToolContext?: { parent: string; phase: string; sessionId?: string; cwd?: string };
	...
	/** Skills. Default: bundled SKC defaults, plus filesystem skills when enabled */
	skills?: Skill[];
	...
	/** Prompt templates. Default: discovered from cwd/.skc/prompts/ + agentDir/prompts/ */
 ...
 *
 * // With explicit model
 * import { getModel } from '@sayknow-cli/ai';
 * const { session } = await createAgentSession({
...
function withEmbeddedDefaultSkcSkills(skills: Skill[]): Skill[] {
	const byName = new Map(skills.map(skill => [skill.name, skill]));
	for (const defaultSkill of getEmbeddedDefaultSkcSkills()) {
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (options.skills !== undefined) {
			// The four public SKC workflow skills are a product invariant, not
			...
			// accidental `.skc` deletion or overzealous caller filtering.
			skills = withEmbeddedDefaultSkcSkills(options.skills);
			skillWarnings = [];
		...
		// MCP runtime discovery is quarantined for the SKC surface. A top-level
		...
		const skcSubskillToolContext = options.skcSubskillToolContext;
		if (skcSubskillToolContext?.parent.trim() && skcSubskillToolContext.phase.trim()) {
			const pluginTools = await loadActiveSubskillTools({
				cwd: skcSubskillToolContext.cwd ?? cwd,
				sessionId: skcSubskillToolContext.sessionId ?? logicalSessionId,
				parent: skcSubskillToolContext.parent,
				phase: skcSubskillToolContext.phase,
		...
		// Always-on SKC plugin bundle tools (validated registry surfaces). This is
		...
		try {
			...
			for (const q of pluginToolResult.quarantine) {
				logger.warn("Quarantined SKC plugin surface", { plugin: q.plugin, surface: q.surfaceId, code: q.code });
			}
		} catch (error) {
			logger.warn("Failed to load always-on SKC plugin tools", { error });
		}
		...
		if (explicitMcpConfigPath !== undefined) {
			...
			// Always-on SKC plugin-bundle MCP servers. Top-level sessions own a manager
			...
			try {
				...
				for (const q of quarantine) {
					logger.warn("Quarantined SKC plugin MCP", { plugin: q.plugin, surface: q.surfaceId, code: q.code });
				}
					...
					try {
						const sources = Object.fromEntries(
							Object.keys(configs).map(name => [
								name,
								{ provider: "skc-plugins", providerName: "SKC plugin bundle", level: "project" as const },
							]),
						);
						const result = await owned.connectServers(configs, sources as never);
						for (const [server, err] of result.errors) {
							logger.warn("SKC plugin MCP connect failed", { path: `mcp:${server}`, error: err });
						}
				...
				logger.warn("Failed to wire SKC plugin MCP servers", { error });
			}
		...
		// Custom tool and extension discovery is quarantined from the public SKC utility surface.
		...
		try {
			...
			for (const q of pluginHookResult.quarantine) {
				logger.warn("Quarantined SKC plugin hook", { plugin: q.plugin, surface: q.surfaceId, code: q.code });
			}
		} catch (error) {
			logger.warn("Failed to load constrained SKC plugin hooks", { error });
		}
		...
		// explicit SDK session opt-in (SKC_NOTIFICATIONS=1) still wins.
		...
		if (
			...
			inlineExtensions.push(async api => {
				try {
					...
					if (lifecycleStartupCapability && process.env.SKC_SDK_TEST_FACTORY_FAILURE === cwd)
						throw new Error(process.env.SKC_SDK_TEST_FACTORY_SECRET ?? "Lifecycle factory test failure.");
		...
		const rebuildSystemPrompt = async (
			...
			try {
				...
				logger.warn("Failed to render SKC plugin system appendices", { error });
			}
