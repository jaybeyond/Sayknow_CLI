...
describe("update-cli binary release assets", () => {
	it("downloads fallback binaries from the current owner release repository", () => {
		expect(buildReleaseBinaryUrlForTest("0.2.3", "linux", "x64")).toBe(
			"https://github.com/jaybeyond/sayknow-cli/releases/download/v0.2.3/skc-linux-x64",
		);
	});

	it("uses the existing Windows .exe release asset name", () => {
		expect(buildReleaseBinaryUrlForTest("0.2.3", "win32", "x64")).toBe(
			"https://github.com/jaybeyond/sayknow-cli/releases/download/v0.2.3/skc-windows-x64.exe",
		);
	});
	...
	it("reports actionable Unix manual update commands for unsupported fallback paths", () => {
		...
		expect(instructions).toContain(
			"curl -fsSL https://raw.githubusercontent.com/jaybeyond/sayknow-cli/main/scripts/install.sh | sh -s -- --binary",
		);
	});
	...
	it("reports actionable Windows manual update commands for unsupported fallback paths", () => {
		...
		expect(instructions).toContain("npm, pnpm, or another package manager");
		expect(instructions).toContain(
			"irm https://raw.githubusercontent.com/jaybeyond/sayknow-cli/main/scripts/install.ps1 | iex",
		);
	});
	...
	it("keeps manual reinstall guidance aligned with bundled installer repositories", async () => {
		...
		expect(instructions).toContain("raw.githubusercontent.com/jaybeyond/sayknow-cli/main/scripts/install.sh");
		expect(shellInstaller).toContain('REPO="jaybeyond/sayknow-cli"');
		expect(windowsInstaller).toContain('$Repo = "jaybeyond/sayknow-cli"');
		expect(formatManualUpdateInstructionsForTest("win32")).toContain(
			"raw.githubusercontent.com/jaybeyond/sayknow-cli/main/scripts/install.ps1",
		);
	});
	...
	it("includes actionable guidance when a release asset download fails", () => {
		...
		expect(message).toContain("jaybeyond/sayknow-cli/releases/download/v0.2.3/skc-linux-x64");
