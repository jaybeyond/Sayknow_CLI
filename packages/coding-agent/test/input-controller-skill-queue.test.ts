...
import { getEmbeddedDefaultSkcSkills } from "@sayknow-cli/coding-agent/defaults/skc-defaults";
import { resolveSkillSlashCommands, type Skill } from "@sayknow-cli/coding-agent/extensibility/skills";
import { EventController } from "@sayknow-cli/coding-agent/modes/controllers/event-controller";
import { InputController } from "@sayknow-cli/coding-agent/modes/controllers/input-controller";
...
import { SKILL_PROMPT_MESSAGE_TYPE, type SkillPromptDetails } from "@sayknow-cli/coding-agent/session/messages";
...
describe("InputController #invokeSkillCommand (E1-E3)", () => {
	...
	it("E3b: embedded default skill command does not require .skc on disk", async () => {
		const embedded = getEmbeddedDefaultSkcSkills().find(skill => skill.name === "deep-interview");
		...
		expect(messageArg.details.path).toBe("embedded:skc/skills/deep-interview/SKILL.md");
...
describe("skill slash command resolution", () => {
	...
	it("exposes only namespaced skill commands", () => {
		const deepInterview = getEmbeddedDefaultSkcSkills().find(skill => skill.name === "deep-interview");
...
describe("UiHelpers / InputController against the queued-display layer (E8-E9)", () => {
	...
	beforeEach(async () => {
		...
		// post-state.
		const themeInstance = await getThemeByName("red-octopus");
