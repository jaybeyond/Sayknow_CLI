 ...
 *
 * Gated behind SKC_RLM_LIVE=1 because it makes real LLM calls (creds + network)
 * and is therefore not part of the default CI suite. When enabled, it builds a
 * real RLM session via the same SDK path the `skc rlm` command uses (research
 ...
 *
 * Run: SKC_RLM_LIVE=1 [RLM_MODEL=layofflabs/gpt-5.5] bun test test/rlm-live-model-e2e.test.ts
 */
...
import { readNotebookDocument } from "@sayknow-cli/coding-agent/edit/notebook";
import { disposeKernelSessionsByOwner } from "@sayknow-cli/coding-agent/eval/py/executor";
import {
	ensureRlmSessionDir,
...
import { createRlmPreset } from "@sayknow-cli/coding-agent/rlm/index";
import { RlmNotebookWriter } from "@sayknow-cli/coding-agent/rlm/notebook";
import { assertRlmToolAllowlist } from "@sayknow-cli/coding-agent/rlm/preset";
import { createRlmPythonTool } from "@sayknow-cli/coding-agent/rlm/python-tool";
import { synthesizeRlmReport } from "@sayknow-cli/coding-agent/rlm/report";
import { type CreateAgentSessionOptions, createAgentSession } from "@sayknow-cli/coding-agent/sdk";

const LIVE = process.env.SKC_RLM_LIVE === "1";
