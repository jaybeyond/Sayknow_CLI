...
describe("runSkcGcCommand", () => {
	...
	test("rejects repair combined with prune or dry-run", async () => {
		for (const args of [
			...
			const result = await runSkcGcCommand(args, "/tmp", {}, adapters);
			expect(result.status).toBe(2);
	...
	test("--json emits a report with all five store arrays", async () => {
		const result = await runSkcGcCommand(["--json"], "/tmp", {}, adapters);
	...
	test("default text mode reports dry run", async () => {
		const result = await runSkcGcCommand([], "/tmp", {}, adapters);
	...
	test("--dry-run overrides --prune", async () => {
		...
		const result = await runSkcGcCommand(["--prune", "--dry-run", "--json"], "/tmp", {}, [a]);
		expect(pruned).toBe(false);
	...
	test("--prune actually prunes removable records", async () => {
		...
		const result = await runSkcGcCommand(["--prune", "--json"], "/tmp", {}, [a]);
