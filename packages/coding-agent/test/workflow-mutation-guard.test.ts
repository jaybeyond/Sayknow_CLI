...
} from "@sayknow-cli/coding-agent/skill-state/workflow-mutation-guard";
...
describe("workflow mutation guard", () => {
	...
	it("blocks product mutation during active ralplan and allows neutral temp scratch", async () => {
		...
		const skcBash = await getWorkflowMutationDecision({
			cwd,
			sessionId: "session-a",
			tool: tool("bash"),
	...
	it("blocks .skc raw paths in deferred ast_edit apply even with no planning skill or forceOverride", async () => {
		const cwd = await makeTempRoot();
		await expect(
			assertWorkflowMutationRawPathsAllowed({ cwd, rawPaths: [".skc/specs/x.md"] }),
		).rejects.toBeInstanceOf(ToolError);
		await expect(
			assertWorkflowMutationRawPathsAllowed({
				cwd,
