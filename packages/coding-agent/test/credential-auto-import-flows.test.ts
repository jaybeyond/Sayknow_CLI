...
import { logger, VERSION } from "@sayknow-cli/utils";
...
describe("credential auto-import state classification and compatibility", () => {
	...
	async function createTemporaryAgentDir(): Promise<string> {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-credential-auto-import-"));
...
describe("bare /login external credential import gate", () => {
		...
		const ctx = {
			...
			settings: {
				getAgentDir: () => {
					...
					return args.agentDir ?? path.join(os.tmpdir(), "skc-credential-auto-import-controller");
				},
			},
	...
	test("bare /login reads resolved state from the configured agent directory", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-credential-auto-import-controller-"));
		try {
