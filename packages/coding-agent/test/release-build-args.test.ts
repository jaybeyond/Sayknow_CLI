...
const releaseArgs = buildReleaseCompileArgs("bun-darwin-arm64", "packages/coding-agent/binaries/skc-darwin-arm64");
...
describe("release build compile args", () => {
	...
	it("marks release binaries with release build metadata", () => {
		...
		expect(valuesAfter(releaseArgs, "--define")).toContain('process.env.SKC_BUILD_CHANNEL="release"');
		expect(valuesAfter(releaseArgs, "--define")).not.toContain('process.env.SKC_BUILD_CHANNEL="dev"');
	});
	...
	it("marks dev-compiled binaries as dev builds explicitly", () => {
		...
		expect(devDefines).toContain('process.env.SKC_BUILD_CHANNEL="dev"');
	});
	...
	it("does not list models.json as an extra compile entrypoint", () => {
		...
		// import in @sayknow-cli/ai instead, so re-adding these args would regress
	...
	it("has exactly one target and outfile", () => {
		...
		expect(valuesAfter(releaseArgs, "--outfile")).toEqual(["packages/coding-agent/binaries/skc-darwin-arm64"]);
	});
