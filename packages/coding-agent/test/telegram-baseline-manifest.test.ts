...
async function tempDir(): Promise<string> {
	const directory = await fs.mkdtemp(path.join(os.tmpdir(), "skc-telegram-manifest-"));
...
async function fakeBun(directory: string): Promise<{ binDirectory: string; log: string }> {
	...
	if (process.platform === "win32") {
		...
		await Bun.write(
			source,
			'import { appendFile } from "node:fs/promises";\n\nconst args = process.argv.slice(2);\nawait appendFile(process.env.SKC_MANIFEST_RECEIPT_LOG!, args.join(" ") + "\\n");\nif (args.includes("fail-command.test.ts")) {\n\tconsole.log("1 fail");\n\tprocess.exit(1);\n}\nconsole.log("1 pass");\n',
		);
...
describe("telegram baseline manifest generator", () => {
	it("rejects missing, wrong, and unknown manifest versions", async () => {
		for (const version of [undefined, 0, 2, "1"]) {
			...
			const result = run(generator, ["--check"], { SKC_TELEGRAM_BASELINE_MANIFEST: manifestPath });
	...
	it("refuses to generate a manifest when discovery finds zero tests", async () => {
		...
		const result = run(generator, [], {
			SKC_TELEGRAM_BASELINE_MANIFEST: manifestPath,
			SKC_TELEGRAM_BASELINE_TEST_DIR: emptyTestDir,
		});
...
describe("test manifest runner", () => {
	...
	it("does not run parity rows after a behavioral command fails", async () => {
		...
		const result = run(runner, [manifestPath], {
			SKC_MANIFEST_RECEIPT_LOG: fake.log,
...
describe("skc-sdk rename scanner", () => {
	it.skipIf(process.platform === "win32")("fails closed when a tracked file cannot be read", async () => {
		...
		const scanner = path.join(packageRoot, "scripts", "verify-skc-sdk-rename.ts");
		const result = run(scanner, [], { SKC_SDK_RENAME_SCAN_ROOT: scanRoot });
	...
	it("permits notification subsystem identifiers", async () => {
		...
		await Bun.write(
			...
			"SKC_NOTIFICATIONS configures the notification daemon; skc-notif-switch- is a temporary prefix.\n",
		);
		git("add", "clean.txt");
		git("commit", "-m", "initial");
		const scanner = path.join(packageRoot, "scripts", "verify-skc-sdk-rename.ts");
	...
	it("rejects old SDK branding in tracked and untracked files", async () => {
		...
		await Bun.write(path.join(scanRoot, legacyPath), `${["use", ["skc", "notifications"].join("_")].join(" ")}\n`);
		const scanner = path.join(packageRoot, "scripts", "verify-skc-sdk-rename.ts");
		const result = run(scanner, [], { SKC_SDK_RENAME_SCAN_ROOT: scanRoot });
		...
		expect(output(result)).toContain(
			`${legacyPosixPath}:1: forbidden ${JSON.stringify(["skc", "notifications"].join("_"))}`,
		);
	});
});
