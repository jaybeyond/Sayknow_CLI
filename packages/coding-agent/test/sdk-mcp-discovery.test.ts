...
import { AuthStorage, Effort, getBundledModel, type Model } from "@sayknow-cli/ai";
...
import type { CustomTool } from "@sayknow-cli/coding-agent/extensibility/custom-tools/types";
...
import { logger, Snowflake } from "@sayknow-cli/utils";
...
const validSixSurfacePluginBundle = path.join(import.meta.dir, "fixtures", "skc-plugins", "valid-six-surface-bundle");
	...
	it("rejects exact MCP tool name collisions and cleans up the owned manager", async () => {
		...
		const collisions: Array<{
			...
			},
			{
				...
				prepare: async () => {
					await installSkcPluginBundle(validSixSurfacePluginBundle, { scope: "project", cwd: tempDir });
				},
			},
		];
		const ownedManagers: MCPManager[] = [];
