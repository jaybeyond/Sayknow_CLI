...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".skc");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		...
		fs.mkdirSync(path.join(tempHomeDir, ".skc", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".skc", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".skc", "SYSTEM.md"), "Project SYSTEM prompt");
	...
	it("loads skc's own user-global AGENTS.md before project context files", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(projectDir, { recursive: true });
		fs.writeFileSync(path.join(projectDir, "AGENTS.md"), "Project instructions");
		const userAgentsPath = path.join(tempHomeDir, ".skc", "agent", "AGENTS.md");
