...
describe("notification-service health", () => {
	...
	test("healthy daemon with fresh heartbeat and matching identity is ok", async () => {
		const { fs } = mockFs({
			...
			[path.join("/tmp/skc-none", "sdk", "session-a.json")]: JSON.stringify({
				sessionId: "session-a",
	...
	test("reports a current-root unavailable endpoint hint only for an active matching daemon", async () => {
		...
		expect(hint).toEqual({
			...
			level: "warn",
			detail:
				"No local notification endpoint for this working directory. In this SKC terminal run /notify on; if it does not report notifications enabled, start a new local SKC session. Do not re-pair Telegram.",
		});
	...
	test("ignores shared lifecycle, ready, and broker records when discovering endpoints", async () => {
		const stateRoot = "/tmp/skc-shared-sdk-state";
		const { fs } = mockFs({
	...
	test.each([
		...
		const endpointPath = path.join("/tmp/skc-none", "sdk", "session-a.json");
		const { fs } = mockFs({
...
describe("notification-service recovery", () => {
	...
	const stateRoot = "/tmp/skc-recovery-state";
	...
	test("reports a detached endpoint after native post-detach failure for retry", async () => {
		...
		const detached = path.join(epDir, ".skc-delete-notification-endpoint-retry.json");
		const { fs, store, unlinked } = mockFs(
			{
	...
	test("reports detached stale endpoints and retained successors as separate recovery paths", async () => {
		...
		const detached = path.join(epDir, ".skc-delete-notification-endpoint-raced.json");
		const successor = path.join(epDir, ".skc-exact-unlink-placeholder-raced.json");
		const { fs, unlinked } = mockFs(
			{
	...
	test("reports a retained internal exchange placeholder separately from stale objects and live successors", async () => {
		...
		const placeholder = path.join(epDir, ".skc-exact-unlink-placeholder-verified");
		const { fs, unlinked } = mockFs(
			{
	...
	test("reports an unverified retained cleanup entry separately from stale objects and verified placeholders", async () => {
		...
		const unknown = path.join(epDir, ".skc-exact-unlink-placeholder-mismatch");
		const { fs, unlinked } = mockFs(
			{
...
describe("notification-service endpoint liveness (owner-proof)", () => {
	...
	const stateRoot = "/tmp/skc-liveness-state";
	const epDir = path.join(stateRoot, "sdk");
