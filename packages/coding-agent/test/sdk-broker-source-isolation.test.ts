...
async function tempRoot(): Promise<string> {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-isolation-"));
	roots.add(root);
	return root;
}
...
it("starts a fresh detached source broker without loading hostile cwd bunfig or dotenv", async () => {
	...
	await Bun.write(path.join(hostileCwd, ".env"), "SKC_2178_DOTENV=dotenv-loaded\n");
	await Bun.write(
		preload,
		[
			`await Bun.write(${JSON.stringify(preloadSentinel)}, "preload-loaded");`,
			`if (process.env.SKC_2178_DOTENV) await Bun.write(${JSON.stringify(dotenvSentinel)}, process.env.SKC_2178_DOTENV);`,
		].join("\n"),
	);
...
it("starts the default source session host with isolated bootstrap policy and workspace cwd", async () => {
	...
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(workspace, ".skc", "state");
