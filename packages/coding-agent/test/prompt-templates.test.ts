...
import { expandPromptTemplate, type PromptTemplate } from "@sayknow-cli/coding-agent/config/prompt-templates";
import { expandSlashCommand, type FileSlashCommand } from "@sayknow-cli/coding-agent/extensibility/slash-commands";
import { parseCommandArgs, substituteArgs } from "@sayknow-cli/coding-agent/utils/command-args";
...
describe("tool prompt descriptions", () => {
	test("retains browser safety, structured-action, and shipped-reference contracts", () => {
		...
		expect(browserDescription).toContain("only a Chrome process SKC launched");
		...
		expect(browserDescription).toContain("skc://tools/browser.md");
