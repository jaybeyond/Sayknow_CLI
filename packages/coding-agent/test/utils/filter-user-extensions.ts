...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@sayknow-cli/utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.skc/`), plus
//    the legacy non-XDG `~/.skc/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.skc/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/skc/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.skc` when
...
// symlinks `~/.skc/agent/extensions/foo.ts` to a dotfiles repo would see the
