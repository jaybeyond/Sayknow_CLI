...
import { installSkcPluginBundle } from "../src/extensibility/skc-plugins";
import { buildPluginMcpConfigs } from "../src/extensibility/skc-plugins/runtime-adapters";
...
describe("always-on plugin-bundle MCP in a live session", () => {
	test("connects an installed bundle MCP server and surfaces its tools as always-on", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
	...
	test("keeps always-on plugin MCP tools active across newSession and switchSession resume", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-resume-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
	...
	test("filters an explicitly requested mandatory plugin tool from persisted selection authority", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-explicit-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
		const sessionManager = SessionManager.inMemory(cwd);
		const sessionSettings = Settings.isolated();
	...
	test("does not connect any MCP server when no plugin bundle is installed", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-empty-"));
		tempDirs.push(cwd);
	...
	test("subagent inherits the parent's always-on MCP tools and never tears down the parent manager", async () => {
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-sub-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
	...
	test.each([
		...
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-mcp-session-forged-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
