...
import { type AssistantMessageEventStream, clearCustomApis, Effort, getCustomApi } from "@sayknow-cli/ai";
import { getOAuthProviders, unregisterOAuthProviders } from "@sayknow-cli/ai/utils/oauth";
import type { OAuthCredentials } from "@sayknow-cli/ai/utils/oauth/types";
import { ModelRegistry, type ProviderConfigInput } from "@sayknow-cli/coding-agent/config/model-registry";
...
describe("ModelRegistry runtime provider registration", () => {
	...
	test("loads custom provider API keys from typed apiKeyEnv without falling back to the env var name", async () => {
		const keyEnv = `SKC_TEST_PROVIDER_KEY_${Snowflake.next()}`;
