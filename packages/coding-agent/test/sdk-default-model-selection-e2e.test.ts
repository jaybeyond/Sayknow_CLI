...
import { closeModelCache, Effort } from "@sayknow-cli/ai";
...
test("model.set executes every Q10-advertised selection and persists the public current readback", async () => {
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-default-model-"));
	...
	const endpointFile = path.join(tempDir, ".skc", "state", "sdk", `${session.sessionId}.json`);
