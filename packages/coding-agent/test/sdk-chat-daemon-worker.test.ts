...
describe("chat daemon worker", () => {
	...
	it("creates a real configured runtime, maps event threads, routes safe replies, handles lifecycle transitions, and cleans up", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-chat-worker-"));
		...
		const turnStreamPosted = provider.waitForMessage(message => message.content === "SKC turn stream\noutbound");
		expect(provider.messages).toContainEqual({
			threadId: "thread-1",
			content: "SKC identity header\ntitle: Replay identity\nrepo: replay-repo\nbranch: replay-branch",
		});
		...
		expect(provider.messages).toContainEqual({ threadId: "thread-1", content: "SKC turn stream\noutbound" });
		const finalizedTurnStreamPosted = provider.waitForMessage(
			message => message.content === "SKC turn stream\nfinalized",
		);
	...
	it("fails closed when a replacement client cannot connect", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-chat-replace-"));
		const agentDir = path.join(root, "agent");
		const stateRoot = path.join(root, ".skc", "state");
		const endpointPath = path.join(stateRoot, "sdk", "session.json");
		await fs.mkdir(path.dirname(endpointPath), { recursive: true });
	...
	it("discards queued frames emitted by a replaced attachment", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-chat-frame-"));
		const agentDir = path.join(root, "agent");
		const stateRoot = path.join(root, ".skc", "state");
		const endpointPath = path.join(stateRoot, "sdk", "session.json");
		await fs.mkdir(path.dirname(endpointPath), { recursive: true });
	...
	it("persists Slack action authority across restart, restores it for inbound replies, and clears resolved actions", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-slack-worker-"));
		const agentDir = path.join(root, "agent");
		const stateRoot = path.join(root, ".skc", "state");
		const endpointPath = path.join(stateRoot, "sdk", "session.json");
		await fs.mkdir(path.dirname(endpointPath), { recursive: true });
	...
	it("uses the production SdkClient loopback boundary while Discord remains fake", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-chat-worker-wire-"));
		const agentDir = path.join(root, "agent");
		const stateRoot = path.join(root, ".skc", "state");
		const endpointPath = path.join(stateRoot, "sdk", "session.json");
	...
	it("routes Slack safe queries through the production Session SDK host across generation and worker restart", async () => {
		root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-slack-production-host-"));
		const agentDir = path.join(root, ".skc", "agent");
		const host = await startProductionSdkHost(root);
		const endpointPath = path.join(root, ".skc", "state", "sdk", `${host.sessionId}.json`);
		const index = await new SessionIndex(agentDir).open();
		let tick: (() => void) | undefined;
		const config = {
			identity: "fingerprint-only",
		...
		try {
			await index.append({
				...
				locator: { repo: root, stateRoot: path.join(root, ".skc", "state") },
				endpointGeneration: 1,
				pid: process.pid,
				endpointMtimeMs: (await fs.stat(endpointPath)).mtimeMs,
			});
