...
describe("ACP event mapper", () => {
	...
	it("maps retry, thinking, and goal progress into ACP session metadata", () => {
		...
		expect(retry).toMatchObject({
			skcPhase: "retrying",
			skcRetryAttempt: 2,
			skcRetryMaxAttempts: 4,
			skcRetryDelayMs: 1_500,
			running: true,
		});
		...
		expect(thinking[0]!.update._meta).toEqual({ skcThinkingLevel: "high" });
		...
		expect(goal[0]!.update._meta).toMatchObject({
			skcGoalActive: true,
			skcGoalId: "goal-1",
			skcGoalStatus: "active",
			skcGoalObjective: "Finish ACP support",
		});
	...
	it("maps model fallback switches to one ACP session notice", () => {
		...
		expect(updates[0]!.update).toEqual({
			...
			_meta: {
				...
				skcModelFallbackEventId: "fallback-1",
				skcModelFallbackFrom: "anthropic/claude-sonnet",
				skcModelFallbackTo: "openai/gpt-5",
				skcModelFallbackReason: "rate_limit",
				skcModelFallbackRole: "default",
				skcModelFallbackScope: "session",
				skcModelFallbackActiveIndex: 1,
				skcModelFallbackChainLength: 2,
				skcModelFallbackAttemptsUsed: 3,
			},
		});
	});
