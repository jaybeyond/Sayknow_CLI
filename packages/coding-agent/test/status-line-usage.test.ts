...
function makeSession(
	...
	return {
		...
		sessionId: options.credentialSessionId,
		isStreaming: false,
...
describe("status line usage segment", () => {
	...
	it("keeps Grok monthly credits out of the 7d quota window", async () => {
		const now = Date.now();
		// Shape emitted by packages/ai/src/usage/grok-cli.ts: SuperGrok monthly credits filed under "7d".
		const session = makeSession(
			async () => [
				{
					provider: "grok-build",
					...
					limits: [
						{
							...
							window: { id: "7d", label: "Monthly credits", resetsAt: now + 20 * 24 * 3_600_000 },
	...
	it("reprojects cached reports after switching the active provider", async () => {
					...
					limits: [
						{
							...
							window: { id: "5h", resetsAt: now + 56 * 60_000 },
		...
		setSessionModel(session, { id: "glm-5.3", provider: "zai", contextWindow: 200_000 });
		const zaiText = stripAnsi(component.getTopBorder(120).content);

		expect(zaiText).toContain("5h 18%");
		expect(zaiText).not.toContain("100%");
		expect(zaiText).not.toContain("7d");
		component.dispose();
	});
