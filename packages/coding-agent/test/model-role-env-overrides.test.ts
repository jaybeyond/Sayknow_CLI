...
describe("resolveModelRoleOverrides", () => {
	...
	test("reads the documented SKC_*_MODEL names for all three roles", () => {
		clearEnv();
		Bun.env.SKC_SMOL_MODEL = "prov/smol";
		Bun.env.SKC_SLOW_MODEL = "prov/slow";
		Bun.env.SKC_PLAN_MODEL = "prov/plan";
		expect(resolveModelRoleOverrides(noFlags)).toEqual({
			smol: "prov/smol",
			slow: "prov/slow",
			plan: "prov/plan",
	...
	test("falls back to the legacy PI_*_MODEL name when SKC_* is unset", () => {
		clearEnv();
		Bun.env.PI_SLOW_MODEL = "prov/legacy-slow";
		expect(resolveModelRoleOverrides(noFlags)).toEqual({ slow: "prov/legacy-slow" });
	});

	test("prefers SKC_* over the legacy PI_* when both are set", () => {
		clearEnv();
		Bun.env.SKC_SMOL_MODEL = "prov/new";
	...
	test("lets the CLI flag win over both env names", () => {
		clearEnv();
		Bun.env.SKC_PLAN_MODEL = "prov/env";
	...
	test("passes an arbitrary model id through unchanged (validation is downstream)", () => {
		clearEnv();
		Bun.env.SKC_SMOL_MODEL = "not-a-real/model-id";
	...
	test("resolves fresh per call, so a later invocation does not inherit an earlier one", () => {
		clearEnv();
		Bun.env.SKC_SMOL_MODEL = "prov/first";
