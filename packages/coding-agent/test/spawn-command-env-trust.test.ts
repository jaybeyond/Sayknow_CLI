 ...
 *
 * - `SKC_SDK_SESSION_COMMAND` becomes the broker's session-host `spawn(file, args)`
 * - `SKC_HARNESS_PROCESS_START_COMMAND` becomes the harness `Bun.spawnSync([...command, pid])`
 *
...
const COMMAND_KEYS = ["SKC_SDK_SESSION_COMMAND", "SKC_HARNESS_PROCESS_START_COMMAND"] as const;
...
function projectDir(dotenv?: string): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-spawn-command-trust-"));
	tempDirs.push(dir);
...
describe("spawn command env trust boundary", () => {
	...
	it("ignores an SDK session command planted by the project .env", async () => {
		const cwd = projectDir("SKC_SDK_SESSION_COMMAND=/tmp/attacker-host --take-over\n");
	...
	it("ignores a harness process-start command planted by the project .env", async () => {
		const cwd = projectDir('SKC_HARNESS_PROCESS_START_COMMAND=["/tmp/attacker-probe"]\n');
	...
	it("still honors overrides inherited from the launching shell", async () => {
		const resolved = await resolveIn(projectDir(), {
			SKC_SDK_SESSION_COMMAND: "/opt/skc/session-host --flag",
			SKC_HARNESS_PROCESS_START_COMMAND: '["ps","-o","lstart=","-p"]',
		});
		expect(resolved.sdkSessionCommand).toEqual({ file: "/opt/skc/session-host", args: ["--flag"] });
	...
	it("keeps a malformed inherited harness override fatal rather than falling back", async () => {
		const resolved = await resolveIn(projectDir(), { SKC_HARNESS_PROCESS_START_COMMAND: "not json" });
		expect(resolved.processStartCommand).toEqual({ kind: "invalid" });
	});

	it("does not let the project .env override inherited values", async () => {
		const cwd = projectDir("SKC_SDK_SESSION_COMMAND=/tmp/attacker-host\n");
		expect((await resolveIn(cwd, { SKC_SDK_SESSION_COMMAND: "/opt/skc/session-host" })).sdkSessionCommand).toEqual({
			file: "/opt/skc/session-host",
			args: [],
		});
	});
});
