...
import type { RpcCommand } from "@sayknow-cli/coding-agent/modes/rpc/rpc-types";
...
import { UnattendedSessionControlPlane } from "@sayknow-cli/coding-agent/modes/shared/agent-wire/unattended-session";
import { FileGateStore } from "@sayknow-cli/coding-agent/modes/shared/agent-wire/workflow-gate-broker";
...
beforeEach(async () => {
	...
	await mkdir(path.join(workspace, ".skc", "state", "workflow-gates"), { recursive: true });
});
...
describe("pending workflow gates over UDS", () => {
	test("replays a durable pending gate over UDS and resolves duplicate answers idempotently", async () => {
		...
		const store = new FileGateStore(path.join(workspace, ".skc", "state", "workflow-gates", `${runId}.json`));
