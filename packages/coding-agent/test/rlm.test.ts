...
import { createEmptyNotebook, readNotebookDocument } from "@sayknow-cli/coding-agent/edit/notebook";
...
import {
	...
	RLM_RESEARCH_PROMPT,
	RLM_TOOL_ALLOWLIST,
} from "@sayknow-cli/coding-agent/rlm/preset";
...
} from "@sayknow-cli/coding-agent/tools/bash-allowed-prefixes";
...
describe("rlm preset tool boundary", () => {
	...
	test("RLM read-only bash allows simple inspections and blocks unsafe shell shapes", () => {
		...
		expect(checkBashAllowedPrefixes("ls -la .skc", RLM_READ_ONLY_BASH_PREFIXES, { profile: "read-only" })).toEqual({
			allowed: true,
		});
		expect(
