...
async function createScopedHarnessSeam(): Promise<{
	    ...
	    if [ -f "$state" ]; then
	      ...
	      if kill -0 "$pid" 2>/dev/null; then
	        if [[ "${"$"}{!#}" == *'#{session_id}'*'#{session_name}'* ]]; then
	          printf '%s\\t%s\n' '${"$"}1' 'sayknow_cli_harness_tmux-owner-test'
	        else
	          printf '%s\n' "$pid"
	        fi
	        exit 0
    ...
    exit 0 ;;
	  if-shell)
	    if [ "\${SKC_HARNESS_TEST_KILL_FAIL:-}" = "1" ]; then exit 1; fi
	    [ -f "$state" ] && kill "$(cat "$state")" 2>/dev/null || true
	...
	await writeFile(
		...
		request=""
		IFS= read -r request
		if [ "\${SKC_HARNESS_TEST_REPLACE_GENERATION:-}" = "1" ]; then
		  lifecycle="\${SKC_HARNESS_STATE_ROOT}/tmux-owner-test/owner-lifecycle"
		  mkdir -p "$lifecycle"
...
async function runHarness(
	...
	const proc = Bun.spawn(
		[
			...
			"start",
			"--input",
			JSON.stringify({ harness: "sayknow-cli", workspace, sessionId, detach: true }),
		],
		{
			cwd: workspace,
			env: {
...
describe("HarnessCommand tmux-resident owner startup", () => {
	it("uses the canonical owner-isolation plan route for a safe tmux server", async () => {
		const argv = [
			...
			"-d",
			"-s",
			"sayknow_cli_harness_tmux-owner-test",
			"-c",
			"/work",
			"owner",
		];
		...
		if (plan.ok)
			expect(plan.execution).toMatchObject({
				mode: "direct",
				argv,
				attempt_session: "sayknow_cli_harness_tmux-owner-test",
	...
	it("proves a private tmux owner through the HarnessCommand path", async () => {
		...
		expect(
			calls.some(call => call.startsWith(`-L ${socket} new-session -d -s sayknow_cli_harness_tmux-owner-test`)),
		).toBe(true);
