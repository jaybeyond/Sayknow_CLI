...
const originalStateFileEnv = process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
const originalSessionIdEnv = process.env.SKC_COORDINATOR_SESSION_ID;

let getProjectDir: typeof import("@sayknow-cli/utils").getProjectDir;
let setProjectDir: typeof import("@sayknow-cli/utils").setProjectDir;
...
let persistCoordinatorRuntimeStateFromEvent: typeof import("../src/skc-runtime/session-state-sidecar").persistCoordinatorRuntimeStateFromEvent;

beforeAll(async () => {
	const utils = await import("@sayknow-cli/utils");
	...
	({ persistCoordinatorRuntimeStateFromEvent } = await import("../src/skc-runtime/session-state-sidecar"));
...
afterEach(() => {
	...
	if (originalStateFileEnv === undefined) delete process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
	else process.env.SKC_COORDINATOR_SESSION_STATE_FILE = originalStateFileEnv;
	if (originalSessionIdEnv === undefined) delete process.env.SKC_COORDINATOR_SESSION_ID;
	else process.env.SKC_COORDINATOR_SESSION_ID = originalSessionIdEnv;
	mock.restore();
...
describe("advisory performance baselines", () => {
	...
	it("records streamed tool arg clone/stringify counts during large edit updates", async () => {
		const tempDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "skc-perf-tool-"));
	...
	it("records sidecar sync readFileSync invocations per state-mapped event", async () => {
		const tempDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "skc-perf-sidecar-"));
		const stateFile = path.join(tempDir, "runtime-state.json");
		process.env.SKC_COORDINATOR_SESSION_STATE_FILE = stateFile;
		process.env.SKC_COORDINATOR_SESSION_ID = "session-sidecar-baseline";
