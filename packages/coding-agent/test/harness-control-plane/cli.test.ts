...
function runHarness(args: string[]): HarnessResult {
	const proc = Bun.spawnSync(["bun", cliEntry, "harness", ...args], {
		cwd: workspace,
		env: { ...cliEnv.env, SKC_HARNESS_STATE_ROOT: root },
		stdout: "pipe",
		stderr: "pipe",
	});
...
function harnessEnvWithoutStateRoot(): NodeJS.ProcessEnv {
	...
	delete env.SKC_HARNESS_STATE_ROOT;
	return env;
}
...
async function appendSignal(sessionId: string, cursor: number, signal: string): Promise<void> {
	await appendEvent(root, sessionId, {
		...
		state: { sessionId, lifecycle: "observing", harness: "sayknow-cli", ownerLive: true, blockers: [] },
		evidence: { signal },
		nextAllowedActions: [],
...
async function seedOwnerDiedBeforeFirstPrompt(sessionId: string): Promise<void> {
	...
	await appendEvent(root, sessionId, {
		...
		severity: "info",
		kind: "owner_started",
		state: { sessionId, lifecycle: "started", harness: "sayknow-cli", ownerLive: true, blockers: [] },
...
describe("skc harness CLI (foundation)", () => {
	it("isolates overlapping workspace links and leaves pre-existing repository links untouched", async () => {
		...
		try {
			...
			await writeFile(path.join(packageDir, "package.json"), JSON.stringify({ name: "@sayknow-cli/ai" }), "utf8");
			const repositoryLink = path.join(fakeRepo, "node_modules", "@sayknow-cli", "ai");
			...
			const firstLink = path.join(first.env.NODE_PATH!, "@sayknow-cli", "ai");
			const secondLink = path.join(second.env.NODE_PATH!, "@sayknow-cli", "ai");
	...
	it("preflight rejects a declared branch that differs from the actual checkout", async () => {
		...
		const res = runHarness([
			...
			JSON.stringify({
				harness: "sayknow-cli",
				workspace,
				branch: "sayknow-cli-pr-265-review",
				issueOrPr: "PR-265",
			}),
		]);
		...
		expect(res.json.evidence.preflight.declaredBranch).toBe("sayknow-cli-pr-265-review");
	...
	it("normalizes recognized issueOrPr forms and rejects ambiguous mixed ids", async () => {
		...
		for (const issueOrPr of [
			...
			"PR-265",
			"pr_265",
			"jaybeyond/Sayknow_CLI#265",
			"https://github.com/jaybeyond/Sayknow_CLI/pull/265",
		]) {
			const res = runHarness([
				"preflight",
				"--input",
				JSON.stringify({ harness: "sayknow-cli", workspace, issueOrPr }),
			]);
		...
		const bad = runHarness([
			"preflight",
			"--input",
			JSON.stringify({ harness: "sayknow-cli", workspace, issueOrPr: "pr-2725-recovery" }),
		]);
	...
	it("start persists normalized branch and issueOrPr metadata", async () => {
		...
		const res = runHarness([
			"start",
			"--input",
			JSON.stringify({ harness: "sayknow-cli", workspace, branch, issueOrPr: "owner/repo#266" }),
		]);
		expect(res.code).toBe(0);
	...
	it("resolves documented relative workspace sessions across caller cwd changes", async () => {
		...
		try {
			const started = runHarness(["start", "--input", JSON.stringify({ harness: "sayknow-cli", workspace: "." })]);
	...
	it("start creates a session and reports submit owner-not-live", () => {
		const res = runHarness(["start", "--input", JSON.stringify({ harness: "sayknow-cli", workspace })]);
		expect(res.code).toBe(0);
	...
	it("rejects non-sayknow-cli harness as an unsupported v1 seam", () => {
	...
	it("re-acquires observe and recover by session id across cwd without state-root env", async () => {
		...
		try {
			...
			const started = runHarnessInCwd(
				["start", "--input", JSON.stringify({ harness: "sayknow-cli", workspace })],
				workspace,
				env,
			);
			expect(started.code).toBe(0);
	...
	it("observe exposes durable completion evidence after the owner has exited", async () => {
		...
		await appendEvent(root, sessionId, {
			...
			state: { sessionId, lifecycle: "finalizing", harness: "sayknow-cli", ownerLive: true, blockers: [] },
