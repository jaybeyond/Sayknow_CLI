/**
 * Sayknow Trinity compatibility tests.
 *
 * The sayknow receipt runtime (separate repo) consumes ReceiptEnvelope JSON
 ...
 * golden fixtures consumed by sayknow's connector tests must always validate
...
async function loadGoldenReceipt(): Promise<ReceiptEnvelope<Record<string, unknown>>> {
	return (await Bun.file(
		path.join(FIXTURES_DIR, "sayknow-trinity-completion-receipt.json"),
	).json()) as ReceiptEnvelope<Record<string, unknown>>;
}

describe("sayknow trinity wire contract", () => {
	...
	it("golden envelope carries every field the sayknow whitelist mapper requires", async () => {
		const golden = await loadGoldenReceipt();
	...
	it("the recorded RPC exchange is replayable JSONL matching the real wire contract", async () => {
		const text = await Bun.file(path.join(FIXTURES_DIR, "sayknow-trinity-rpc-exchange.jsonl")).text();
		...
		expect((rows[1].frame.data as { sessionId?: string }).sessionId).toBe("skc-rpc-session-0001");
	...
	it("the supported-harness seam is unchanged", () => {
		expect(SUPPORTED_HARNESSES).toEqual(["sayknow-cli"]);
	});
