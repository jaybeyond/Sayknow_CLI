...
const WORKSPACE_NODE_MODULES_ENV = "SKC_HARNESS_TEST_NODE_MODULES";
...
function linkWorkspacePackages(scopeDir: string, packages: LinkedWorkspacePackage[]): string[] {
	...
	for (const pkg of packages) {
		const unscopedName = pkg.name.slice("@sayknow-cli/".length);
...
function createRepoNodeModulesLinks(repoRoot: string, packages: LinkedWorkspacePackage[]): () => void {
	...
	const scopeDir = path.join(nodeModulesDir, "@sayknow-cli");
	const markerDir = path.join(nodeModulesDir, ".skc-harness-test-links");
...
export function createHarnessCliEnv(repoRoot: string, baseEnv: NodeJS.ProcessEnv = process.env): HarnessCliEnv {
	const nodePathRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skc-harness-node-path-"));
	const registryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skc-harness-root-registry-"));
	const packages = collectWorkspacePackages(repoRoot);
	linkWorkspacePackages(path.join(nodePathRoot, "@sayknow-cli"), packages);
	...
	const env: NodeJS.ProcessEnv = {
		...
		SKC_HARNESS_ROOT_REGISTRY_DIR: registryRoot,
