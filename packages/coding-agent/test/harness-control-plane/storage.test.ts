...
function state(sessionId: string): SessionState {
	...
	return {
		...
		lifecycle: "started",
		harness: "sayknow-cli",
		handle: { sessionId, harness: "sayknow-cli", workspace: "." } as SessionHandle,
		retries: {},
		blockers: [],
		createdAt: now,
		updatedAt: now,
...
function envelope(cursor: number): EventEnvelope {
	return {
		...
		severity: "info",
		kind: "test",
		state: { sessionId: "h-1", lifecycle: "started", harness: "sayknow-cli", ownerLive: false, blockers: [] },
		evidence: {},
		nextAllowedActions: [],
...
describe("harness storage", () => {
	...
	it("resolveHarnessRoot honors SKC_HARNESS_STATE_ROOT then cwd default", () => {
	...
	it("controlSocketPath is stable, short, and records metadata", async () => {
		...
		const first = controlSocketPath(root, id, { SKC_HARNESS_SOCKET_DIR: socketDir } as NodeJS.ProcessEnv);
		const second = controlSocketPath(root, id, { SKC_HARNESS_SOCKET_DIR: socketDir } as NodeJS.ProcessEnv);
	...
	it("controlSocketPath uses SKC_HARNESS_SOCKET_DIR when set", async () => {
		const socketDir = await mkdtemp(path.join(tmpdir(), "e-"));
		const socketPath = controlSocketPath(root, "h-env", { SKC_HARNESS_SOCKET_DIR: socketDir } as NodeJS.ProcessEnv);
	...
	it("controlSocketPath falls back to tmp base when override is too long", async () => {
		...
		try {
			...
			const socketPath = controlSocketPath(root, "h-fallback", {
				SKC_HARNESS_SOCKET_DIR: longDir,
			} as NodeJS.ProcessEnv);
			expect(socketPath.startsWith(longDir)).toBe(false);
			expect(socketPath.includes("skch")).toBe(true);
		...
		try {
			expect(() =>
				controlSocketPath(root, "h-too-long", {
					SKC_HARNESS_SOCKET_DIR: path.join(root, "o".repeat(80)),
				} as NodeJS.ProcessEnv),
	...
	it("controlSocketPath extends the hash on metadata collision", async () => {
		...
		const socketPath = controlSocketPath(root, id, { SKC_HARNESS_SOCKET_DIR: socketDir } as NodeJS.ProcessEnv);
