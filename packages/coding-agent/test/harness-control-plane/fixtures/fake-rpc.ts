#!/usr/bin/env bun
/**
 * Minimal `skc --mode rpc` protocol emulator — a TEST FIXTURE (never shipped).
 *
 * It speaks the real JSONL protocol from docs/rpc.md so the harness e2e exercises the
 * genuine `SayknowCliRpc` adapter (ready frame, prompt ack + agent_start, get_state) against
...
function handle(line: string): void {
	...
	if (frame.type === "get_state") {
		...
		// `skc --mode rpc` emits and the only shape observeRpcOutboundFrame maps to
		...
		w({ type: "agent_start" });
		if (process.env.SKC_FAKE_RPC_STORM === "1") {
