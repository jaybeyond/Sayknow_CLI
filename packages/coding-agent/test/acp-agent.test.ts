...
// module-global resolver AND sets `process.env.SKC_CODING_AGENT_DIR`, so the
...
const originalAgentEnv = process.env.SKC_CODING_AGENT_DIR;
...
afterEach(async () => {
	...
	if (originalAgentEnv !== undefined) {
		process.env.SKC_CODING_AGENT_DIR = originalAgentEnv;
	} else {
...
async function createHarness(options?: { createSessionGate?: Promise<void> }): Promise<AgentHarness> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "skc-acp-test-"));
...
describe("ACP agent", () => {
	beforeAll(async () => {
		const installed = await getThemeByName("red-octopus");
	...
	it("accepts only ACP underscore-prefixed extension methods", async () => {
		...
		const result = await harness.agent.extMethod("_skc/sessions/listAll", { limit: 2 });

		expect(Array.isArray(result.sessions)).toBe(true);
		expect(typeof result.total).toBe("number");
		await expect(harness.agent.extMethod("skc/sessions/listAll", { limit: 2 })).rejects.toThrow(
	...
	it("omits ACP usage updates when the context count is unknown", async () => {
		...
		expect(
			endOfTurnUpdates.some(
				update => update.update.sessionUpdate === "session_info_update" && update.update._meta?.skcPhase === "idle",
			),
		).toBe(true);
	...
	it("forwards prompt compaction once and returns the session phase to idle", async () => {
		...
		const sessionInfoUpdates = harness.updates
			...
			.map(notification => notification.update);
		const compactionUpdates = sessionInfoUpdates.filter(update => update._meta?.skcCompactionState !== undefined);
		expect(compactionUpdates).toHaveLength(2);
		expect(compactionUpdates[0]?._meta).toMatchObject({
			skcPhase: "compacting",
			skcCompactionState: "start",
			skcCompactionTrigger: "overflow",
			running: true,
		});
	...
	it("forwards idle compaction through the session lifetime subscription", async () => {
		...
		const compactionUpdates = harness.updates
			...
			.filter(update => update.update._meta?.skcCompactionState !== undefined)
			.map(notification => notification.update);
		expect(compactionUpdates).toHaveLength(2);
		expect(compactionUpdates[0]?._meta).toMatchObject({
			skcPhase: "compacting",
			skcCompactionTrigger: "idle",
