...
test("ACP SDK adapter maps native and extension methods and keeps endpoint credentials machine-only", async () => {
	...
	await adapter.handle("_skc/sdk/control", { operation: "runtime.reload", input: { components: ["tools"] } });
...
test("ACP generic routes honor provider, machine, and secret field dispositions", async () => {
	...
	const adapter = new AcpSdkAdapter({ url: "ws://unused", token: "secret", client: sdk as never });
	await adapter.start();
	await expect(adapter.handle("_skc/sdk/control", { operation: "host_tools.register" })).rejects.toMatchObject({
		code: "provider_required",
	});
	await expect(adapter.handle("_skc/sdk/control", { operation: "auth.login" })).rejects.toMatchObject({
		code: "provider_required",
	});
	await expect(adapter.handle("_skc/sdk/global", { operation: "session.get_endpoint" })).rejects.toMatchObject({
		code: "endpoint_credential_forbidden",
	});
	await expect(
		adapter.handle("_skc/sdk/control", { operation: "config.patch", input: { apiToken: "secret" } }),
	).rejects.toMatchObject({ code: "secret_field_forbidden" });
	await adapter.handle("_skc/sdk/control", { operation: "config.patch", input: { killSwitchHotkey: true } });
...
test("ACP SDK adapter exposes SDK event frames while rejecting raw lifecycle globals", async () => {
	...
	await adapter.start();
	await expect(
		adapter.handle("_skc/sdk/global", {
