...
} from "@sayknow-cli/coding-agent/eval/py/runner-artifact";
...
describe("Python runner artifact", () => {
	...
	it("ignores a prepositioned legacy artifact and coalesces concurrent initialization", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-python-artifact-test-"));
		roots.push(root);
		const legacyDirectory = path.join(root, "skc-python-runner");
		...
		const privateDirectories = (await fs.readdir(root)).filter(name => name.startsWith("skc-python-runner-"));
		expect(privateDirectories).toHaveLength(1);
	});
	it("removes the process-private artifact directory during cleanup", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-python-artifact-cleanup-"));
		roots.push(root);
	...
	it("retries after initialization failure instead of memoizing rejection", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-python-artifact-retry-"));
		roots.push(root);
