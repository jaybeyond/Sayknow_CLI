...
import { planLaunchWorktree } from "../src/skc-runtime/launch-worktree";
...
test("startup diagnostics redact identifier-prefixed assignment secrets before bounded truncation", () => {
	...
	const message = sanitizeSdkStartupMessage(
		`OPENAI_API_KEY=${secret} SKC_NOTIFICATIONS_TOKEN=${secret} SERVICE-password=${secret} ${"x".repeat(600)}０`,
	);
...
test("ledger restart quarantines terminal response and durable-effect digest corruption", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-ledger-digest-"));
	try {
...
test("ledger reopen bounds malformed persisted rows before they gain cleanup authority", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-ledger-bounds-"));
...
test("legacy metadata cleanup rejects mixed lifecycle and arbitrary receipt keys before mutation", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-legacy-metadata-allowlist-"));
	...
	try {
		...
		const cleanup: BrokerCleanupEvidence = {
			...
			plannedMetadataPath: path.join(stateRoot, "sdk", `.skc-delete-${sessionId}.lifecycle.json`),
		};
...
async function liveLifecycleSession(root: string, agentDir: string, sessionId: string, staleMarkerFirst = false) {
	...
	const child = Bun.spawn([process.execPath, "run", cliEntrypoint, "sdk", "session-host-internal"], {
		...
		env: {
			...process.env,
			HOME: root,
			SKC_AGENT_DIR: agentDir,
			SKC_CODING_AGENT_DIR: agentDir,
			SKC_SESSION_ID: sessionId,
			SKC_LIFECYCLE_REQUEST_ID: "subprocess-proof",
			SKC_SDK_LIFECYCLE_REQUEST: JSON.stringify(request),
		},
		stdout: "pipe",
		stderr: "pipe",
	});
...
test("lifecycle child ignores a stale marker until its current effect marker replaces it", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-stale-marker-"));
...
test("lifecycle host rejects a transcript replaced after strict authorization before it can be consumed", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-lifecycle-transcript-race-"));
	const agentDir = path.join(root, "agent");
				...
				openLifecycleSessionManager(
					{
						...
						cwd: root,
						stateRoot: path.join(root, ".skc", "state"),
						sessionPath,
...
test("lifecycle fork rejects a source replaced after capture without destination residue", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-lifecycle-fork-race-"));
	const agentDir = path.join(root, "agent");
	...
	try {
		...
		try {
				...
				openLifecycleSessionManager(
					{
						...
						cwd: targetCwd,
						stateRoot: path.join(targetCwd, ".skc", "state"),
						...deriveLifecycleDeadlines(Date.now(), 4_000),
						sourceCwd,
			...
			expect(initializedEntries).toContain(".skc-managed-session-scope.v2.json");
...
test("session host exact cutoff writes proven pre-session absence", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-lifecycle-exact-cutoff-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
...
test("broker bounds a hanging WebSocket upgrade by the lifecycle deadline and cleans its child", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-hanging-upgrade-"));
	const stateRoot = path.join(agentDir, ".skc", "state");
	...
	const previousCommand = process.env.SKC_SDK_SESSION_COMMAND;
	const previousUrl = process.env.SKC_HANGING_UPGRADE_URL;
...
const root=process.env.SKC_STATE_ROOT, id=process.env.SKC_SESSION_ID, agent=process.env.SKC_AGENT_DIR;
fs.mkdirSync(path.join(root,'sdk'),{recursive:true});
fs.writeFileSync(${JSON.stringify(fixturePidPath)},String(process.pid));
fs.writeFileSync(${JSON.stringify(fixtureRequestPath)},process.env.SKC_SDK_LIFECYCLE_REQUEST);
const endpoint=path.join(root,'sdk',id+'.json');
fs.writeFileSync(endpoint,JSON.stringify({sessionId:id,pid:process.pid,url:process.env.SKC_HANGING_UPGRADE_URL,token:'hang'}));
...
`,
		...
		process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${fixture}`;
		process.env.SKC_HANGING_UPGRADE_URL = `ws://127.0.0.1:${hangingUpgrade.port}`;
		await broker.start();
		...
		if (previousCommand === undefined) delete process.env.SKC_SDK_SESSION_COMMAND;
		else process.env.SKC_SDK_SESSION_COMMAND = previousCommand;
		if (previousUrl === undefined) delete process.env.SKC_HANGING_UPGRADE_URL;
		else process.env.SKC_HANGING_UPGRADE_URL = previousUrl;
...
test("broker rejects an endpoint-only lifecycle child that never authenticates session_ready", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-life-"));
	const stateRoot = path.join(agentDir, ".skc", "state");
...
fs.writeFileSync(path.join(agent,'fixture.request.json'),process.env.SKC_SDK_LIFECYCLE_REQUEST);
...
`,
	...
	const previous = process.env.SKC_SDK_SESSION_COMMAND;
	process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${fixture}`;
	const broker = new Broker({ agentDir });
	await broker.start();
	try {
...
test("broker rejects a cross-workspace cold fork source before spawning", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-cross-workspace-"));
	const agentDir = path.join(root, "agent");
	const sourceCwd = path.join(root, "source");
	const targetCwd = path.join(root, "target");
...
test("broker rejects duplicate owned source candidates before spawning", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-duplicate-owned-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const spawnedPath = path.join(root, "spawned");
	...
	const broker = new Broker({ agentDir });
	const previousCommand = process.env.SKC_SDK_SESSION_COMMAND;
	const previousRequestId = process.env.SKC_LIFECYCLE_REQUEST_ID;
	...
	try {
		...
		const createDuplicate = async (suffix: string) => {
			process.env.SKC_LIFECYCLE_REQUEST_ID = `duplicate-prepare-${suffix}`;
			process.env.SKC_SESSION_ID = "duplicate-owned-source";
			const session = SessionManager.create(root, SessionManager.managedDestination(root, agentDir));
			await session.ensureOnDisk();
		...
		delete process.env.SKC_LIFECYCLE_REQUEST_ID;
		...
		process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${command}`;
		await broker.start();
		expect(
			await broker.handleRequest(
				"session.fork",
		...
		if (previousRequestId === undefined) delete process.env.SKC_LIFECYCLE_REQUEST_ID;
		else process.env.SKC_LIFECYCLE_REQUEST_ID = previousRequestId;
...
test("broker directly resumes and forks a canonical cold saved session with scoped cleanup", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-canonical-cold-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const broker = new Broker({ agentDir });
	try {
...
test("broker replays one identity-bound lifecycle metadata cleanup plan after the first delete detach", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-delete-metadata-crash-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
...
test("broker uses incarnation-aware observations before fresh lifecycle metadata cleanup", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-delete-incarnation-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const broker = new Broker({ agentDir });
	try {
...
test("broker refuses fresh lifecycle cleanup when ready sibling has a different owner marker", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-mismatched-ready-cleanup-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const saved = SessionManager.create(root, SessionManager.managedDestination(root, agentDir));
	const broker = new Broker({ agentDir });
...
test("broker preserves ready-only lifecycle metadata without canonical marker authority during fresh delete", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-ready-only-cleanup-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const saved = SessionManager.create(root, SessionManager.managedDestination(root, agentDir));
	const broker = new Broker({ agentDir });
...
test("broker replays an unmarked base metadata cleanup receipt and rejects a replaced ready sibling after marker loss", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-legacy-metadata-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	const sessionId = "legacy-metadata-replay";
	const markerPath = path.join(stateRoot, "sdk", `${sessionId}.lifecycle.json`);
	const readyPath = path.join(stateRoot, "sdk", `${sessionId}.lifecycle.ready.json`);
	const plannedPath = path.join(stateRoot, "sdk", `.skc-delete-base-${sessionId}.lifecycle.json`);
	...
	try {
		...
		const mismatchedPlannedPath = path.join(
			stateRoot,
			"sdk",
			`.skc-delete-base-${mismatchedSessionId}.lifecycle.json`,
		);
		const replacedReady = {
			pid: process.pid + 1,
...
test("broker rejects a corrupt completed lifecycle cleanup receipt when its ready sibling remains", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-completed-lifecycle-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
						...
						lifecycleFiles: [
							{
								...
								plannedPath: path.join(
									...
									`.skc-delete-marker-${sessionId}.lifecycle.json`,
								),
...
test("broker rejects malformed lifecycle cleanup receipts without mutating metadata", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-malformed-lifecycle-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
	...
	try {
		...
		const validFile = {
			...
			identity,
			attempt: 1,
			plannedPath: path.join(stateRoot, "sdk", ".skc-delete-malformed-marker"),
		};
		const malformed = [
			{ lifecycleFiles: [null] },
			{ lifecycleFiles: [{}] },
...
test("broker rejects oversized lifecycle marker and readiness receipts before hashing or unlinking", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-oversized-lifecycle-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
		...
		lifecycleFiles: [
			{
				...
				attempt: 1,
				plannedPath: path.join(stateRoot, "sdk", ".skc-delete-oversized-marker"),
			},
			{
				path: readyPath,
				identity: await capture(readyPath),
				attempt: 1,
				plannedPath: path.join(stateRoot, "sdk", ".skc-delete-oversized-ready"),
			},
		],
	});
	try {
...
test("broker rejects duplicate lifecycle marker replay authorities without unlinking siblings", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-duplicate-lifecycle-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
			...
			response: {
				...
				error: {
					...
					cleanup: {
						...
						metadataRoot: stateRoot,
						lifecycleFiles: [
							cleanupFile(path.join(stateRoot, "sdk", ".skc-delete-one")),
							cleanupFile(path.join(stateRoot, "sdk", ".skc-delete-two")),
						],
					},
				},
			},
...
test("broker rejects a ready-only lifecycle replay entry without marker authority", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-ready-only-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
						...
						lifecycleFiles: [
							{
								...
								attempt: 1,
								plannedPath: path.join(stateRoot, "sdk", ".skc-delete-ready-only"),
							},
						],
					},
				},
...
test("broker fails closed when a lifecycle ready sibling is swapped after marker reconciliation", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-lifecycle-swap-replay-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
				...
				error: {
					...
					cleanup: {
						...
						lifecycleFiles: captures.map(({ path: file, stat, bytes }) => ({
							...
							attempt: 1,
							plannedPath: path.join(stateRoot, "sdk", `.skc-delete-swap-${path.basename(file)}`),
						})),
					},
				},
			},
...
test("broker terminalizes default command resolver failures", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-resolver-failure-"));
	const agentDir = path.join(root, "agent");
	const previousCommand = process.env.SKC_SDK_SESSION_COMMAND;
	const broker = new Broker({ agentDir });
	try {
		delete process.env.SKC_SDK_SESSION_COMMAND;
		...
		const response = await broker.handleRequest(
			"session.create",
			{ cwd: root, stateRoot: path.join(root, ".skc", "state") },
			requestId,
		);
...
test("broker propagates an owned lifecycle startup failure without semantic readiness or endpoint survivors", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-child-exit-"));
	...
	try {
		await fs.writeFile(
			fixture,
			`require('fs').writeFileSync(${JSON.stringify(sessionIdPath)}, process.env.SKC_SESSION_ID); setTimeout(() => process.exit(0), 100);`,
		);
		process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${fixture}`;
		await broker.start();
		const started = Date.now();
		...
		await expect(
			fs.stat(path.join(agentDir, ".skc", "state", "sdk", `${sessionId}.lifecycle.ready.json`)),
		).rejects.toThrow();
		await expect(fs.stat(path.join(agentDir, ".skc", "state", "sdk", `${sessionId}.json`))).rejects.toThrow();
...
test("broker replays immutable lifecycle cleanup after a crash immediately after an exact detach", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-ledger-crash-"));
	const agentDir = path.join(root, "agent");
...
const request = JSON.parse(process.env.SKC_SDK_LIFECYCLE_REQUEST!);
...
const index = await new SessionIndex(process.env.SKC_AGENT_DIR!).open();
...
`,
				...
				cleanup: {
					...
					lifecycleFiles: expect.arrayContaining([
						expect.objectContaining({
							...
							plannedPath: expect.stringContaining(".skc-delete-"),
						}),
					]),
				},
			},
		});
		const stateRoot = path.join(root, ".skc", "state", "sdk");
		...
		const normalRoot = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-ledger-normal-"));
		...
		try {
			...
			process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${path.join(normalRoot, "owned-startup-failure.ts")}`;
			...
			await expect(
				fs.stat(path.join(normalRoot, ".skc", "state", "sdk", `${normalSessionId}.lifecycle.json`)),
			).rejects.toThrow();
			expect({
...
test("session index rejects a stale unregister from an earlier matching PID-generation registration", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-session-index-unregister-"));
...
test("session index proves ordinary host unregistration using a newer matching registration sequence", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-session-index-ordinary-close-"));
	const index = await new SessionIndex(agentDir).open();
	const shared = {
...
test("broker records the resolved worktree state root and preserves pre-child preparation failures", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-lifecycle-worktree-prechild-"));
	...
	try {
		...
		const response = await broker.handleRequest(
			...
			{
				cwd: repo,
				stateRoot: path.join(repo, ".skc", "state"),
		...
		expect(terminal).toMatchObject({
			response,
			effectIntent: {
				stateRoot: path.join(worktreeRoot, ".skc", "state"),
...
test("broker fails closed when the reopened terminal ledger cannot reproduce its response", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-ledger-mismatch-"));
	const broker = new Broker({ agentDir });
...
test("broker rejects a ready foreign host for the spawned session id", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-foreign-ready-"));
	const stateRoot = path.join(agentDir, ".skc", "state");
	const fixture = path.join(agentDir, "foreign.js");
	const foreignIdPath = path.join(agentDir, "foreign-session-id");
	const previousCommand = process.env.SKC_SDK_SESSION_COMMAND;
	const previousEndpoint = process.env.SKC_FOREIGN_ENDPOINT_URL;
...
const endpoint=path.join(root,'sdk',id+'.json');
fs.writeFileSync(endpoint,JSON.stringify({sessionId:id,pid:process.ppid,url:process.env.SKC_FOREIGN_ENDPOINT_URL,token:'foreign'}));
const m=fs.statSync(endpoint).mtimeMs;
const log=path.join(agent,'sdk','sessions','index.jsonl');fs.mkdirSync(path.dirname(log),{recursive:true});const indexSeq=fs.existsSync(log)?fs.readFileSync(log,'utf8').trim().split('\\n').filter(Boolean).length+1:1;
...
`,
		...
		process.env.SKC_SDK_SESSION_COMMAND = `${process.execPath} ${fixture}`;
		process.env.SKC_FOREIGN_ENDPOINT_URL = `ws://127.0.0.1:${foreign.port}`;
		await broker.start();
		expect(
			await broker.handleRequest(
				"session.create",
		...
		if (previousEndpoint === undefined) delete process.env.SKC_FOREIGN_ENDPOINT_URL;
		else process.env.SKC_FOREIGN_ENDPOINT_URL = previousEndpoint;
		foreign.stop(true);
		await broker.stop();
		await fs.rm(agentDir, { recursive: true, force: true });
	}
...
test("broker refuses a stale registered PID when no durable effect marker proves ownership", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-stale-"));
...
test("broker refuses same-generation close authority from a prior endpoint incarnation", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-close-incarnation-"));
	const stateRoot = path.join(agentDir, "state");
...
test("broker rebinds implicit close only for a matching non-empty lifecycle request id", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-close-rebind-"));
	const stateRoot = path.join(agentDir, "state");
	const broker = new Broker({ agentDir });
...
test("broker atomically reuses the indexed live owner for distinct resume keys", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-resume-live-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
...
test("broker never signals a PID reused after its lifecycle marker was written", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-reused-"));
	const stateRoot = path.join(agentDir, "state");
...
test("broker records terminal uncertainty when SIGKILL re-verification fails after SIGTERM", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-uncertain-"));
	const stateRoot = path.join(agentDir, "state");
...
if (process.platform === "darwin") {
	test("broker records terminal uncertainty when a spawned child incarnation is unreadable", async () => {
		const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-incarnation-"));
		const previousCommand = process.env.SKC_SDK_SESSION_COMMAND;
		let incarnationReads = 0;
		let childPid: number | undefined;
		const broker = new Broker({ agentDir });
		process.env.SKC_SDK_SESSION_COMMAND = "/bin/sleep 60";
...
test("broker starts from the production broker entrypoint with no sessions", async () => {
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-zero-"));
	const broker = new Broker({ agentDir });
	try {
...
test("shipped sdk session-host-internal stays alive only after a semantic ready event and serves real requests", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-subprocess-"));
	const agentDir = path.join(root, "agent");
...
test("session-host-internal exits with a sanitized startup failure before writing lifecycle readiness", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-startup-failure-"));
	const agentDir = path.join(root, "agent");
	...
	try {
		...
		const child = Bun.spawn([process.execPath, "run", cliEntrypoint, "sdk", "session-host-internal"], {
			...
			env: {
				...
				SKC_CODING_AGENT_DIR: agentDir,
				SKC_SESSION_ID: sessionId,
				SKC_LIFECYCLE_REQUEST_ID: "startup-failure-proof",
				SKC_SDK_LIFECYCLE_REQUEST: JSON.stringify({
					operation: "session.create",
					sessionId,
					cwd: root,
					stateRoot,
...
test("production lifecycle factory failure preserves reason and redacts collected secrets", async () => {
	...
	const agentDir = path.join(root, "agent");
	const broker = new Broker({ agentDir });
	const names = ["SKC_SDK_TEST_FACTORY_FAILURE", "SKC_SDK_TEST_FACTORY_SECRET"] as const;
	const previous = names.map(name => process.env[name]);
	const bare = "factory-bare-secret";
	const overlap = `${bare}-overlap`;
	const normalized = "factory-secret０".normalize("NFKC");
	process.env.SKC_SDK_TEST_FACTORY_FAILURE = root;
	process.env.SKC_SDK_TEST_FACTORY_SECRET = `${overlap} ${normalized} ${"x".repeat(600)}`;
	try {
		await broker.start();
		const response = await broker.handleRequest(
			"session.create",
...
test("never-settling model profile startup cuts off with proven pre-registration cleanup", async () => {
	...
	const agentDir = path.join(root, "agent");
	const broker = new Broker({ agentDir });
	const previous = process.env.SKC_SDK_TEST_HANG_MODEL_PROFILE;
	process.env.SKC_SDK_TEST_HANG_MODEL_PROFILE = root;
	try {
		await broker.start();
		...
		if (previous === undefined) delete process.env.SKC_SDK_TEST_HANG_MODEL_PROFILE;
		else process.env.SKC_SDK_TEST_HANG_MODEL_PROFILE = previous;
		await broker.stop();
		await fs.rm(root, { recursive: true, force: true });
	}
}, 10_000);
test("production post-registration startup failure proves cleanup and exact replay", async () => {
	...
	const agentDir = path.join(root, "agent");
	const broker = new Broker({ agentDir });
	const previousFailure = process.env.SKC_SDK_TEST_FAIL_AFTER_REGISTRATION;
	process.env.SKC_SDK_TEST_FAIL_AFTER_REGISTRATION = root;
	try {
		await broker.start();
...
test("production broker session.create authenticates a source-workspace v3 native endpoint", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-v3-broker-"));
	const agentDir = path.join(root, "agent");
	const broker = new Broker({ agentDir });
	try {
		...
		const sdkEntries = await fs.readdir(path.join(root, ".skc", "state", "sdk"));
...
test("ACP, MCP, and daemon global requests bootstrap a broker with zero sessions", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-zero-global-"));
	...
	try {
		...
		expect(await mcp.callTool("skc_session_global", { operation: "session.list" })).toMatchObject({
			ok: true,
			result: { sessions: [] },
		});
...
test("lifecycle cleanup rejects transplanted and ambiguous receipts before mutation", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-cleanup-receipt-"));
	const stateRoot = path.join(root, ".skc", "state");
	...
	try {
		...
		const deleteCleanup: BrokerCleanupEvidence = {
			...
			sessionId,
			metadataRoot: stateRoot,
			lifecycleFiles: [file(path.join(stateRoot, "sdk", ".skc-delete-cleanup"))],
		};
		for (const [operation, input] of [
			["session.delete", { cwd: root, stateRoot, sessionId: "other-cleanup-receipt" }],
			[
				"session.delete",
				{ cwd: path.join(root, "other"), stateRoot: path.join(root, "other", ".skc", "state"), sessionId },
			],
		...
		const duplicate: BrokerCleanupEvidence = {
			...
			metadataRoot: stateRoot,
			lifecycleFiles: [
				file(path.join(stateRoot, "sdk", ".skc-delete-one")),
				file(path.join(stateRoot, "sdk", ".skc-delete-two")),
			],
		};
		const mixed: BrokerCleanupEvidence = {
			...duplicate,
			metadataPath: markerPath,
			lifecycleFiles: [file(path.join(stateRoot, "sdk", ".skc-delete-mixed"))],
		};
		const shared: BrokerCleanupEvidence = {
			phase: "lifecycle",
			sessionId,
			metadataRoot: stateRoot,
			lifecycleFiles: [{ ...file(path.join(stateRoot, "sdk", ".skc-delete-shared")), detachedPath: markerPath }],
		};
...
test("lifecycle cleanup receipt parser rejects hostile bounded inputs without touching user data", async () => {
	const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-hostile-lifecycle-receipt-"));
	const agentDir = path.join(root, "agent");
	const stateRoot = path.join(root, ".skc", "state");
		...
		lifecycleFiles: [
			{
				...
				identity: await capture(markerPath),
				attempt: 1,
				plannedPath: path.join(stateRoot, "sdk", ".skc-delete-hostile-marker"),
			},
			{
				path: readyPath,
				identity: await capture(readyPath),
				attempt: 1,
				plannedPath: path.join(stateRoot, "sdk", ".skc-delete-hostile-ready"),
			},
		],
	});
