...
function expectLoadError(fn: () => unknown, code: SkcPluginLoadErrorCode): void {
	try {
		fn();
	} catch (error) {
...
async function expectCompileError(root: string, code: SkcPluginLoadErrorCode): Promise<void> {
	try {
		await compileSkcPluginBundle(root);
	} catch (error) {
		expect(error).toBeInstanceOf(SkcPluginLoadError);
		expect((error as SkcPluginLoadError).code).toBe(code);
		return;
...
async function expectCompileErrorOneOf(root: string, codes: SkcPluginLoadErrorCode[]): Promise<void> {
	try {
		await compileSkcPluginBundle(root);
	} catch (error) {
		expect(error).toBeInstanceOf(SkcPluginLoadError);
		expect(codes).toContain((error as SkcPluginLoadError).code);
		return;
	}
	throw new Error(`Expected one of ${codes.join(", ")} compile errors`);
}

async function expectReadRegistryError(cwd: string, code: SkcPluginLoadErrorCode): Promise<void> {
	try {
...
async function makeTempPlugin(prefix: string, manifest: Record<string, unknown>): Promise<string> {
	...
	tempDirs.push(dir);
	await fs.writeFile(path.join(dir, "sayknow-plugin.json"), JSON.stringify(manifest));
	return dir;
}
...
function registryEntry(name: string, cwd: string): SkcPluginRegistryEntry {
	...
	return {
		...
		copiedFiles: [{ relativePath: "sayknow-plugin.json", sha256: "a".repeat(64), bytes: 1 }],
		surfaces: {
			subskills: [],
			tools: [],
			hooks: [],
...
describe("SKC plugin Milestone 1 red-team QA", () => {
	test("rejects manifest injection, forbidden top-level surfaces, MCP aliases, and malformed surface shapes", () => {
		const cases: Array<[string, Record<string, unknown>, SkcPluginLoadErrorCode]> = [
	...
	test("rejects traversal paths and never imports plugin code while compiling", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-traversal-"));
		tempDirs.push(dir);
		const outside = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-outside-"));
		tempDirs.push(outside);
		const sentinel = path.join(outside, "sentinel.txt");
		await fs.mkdir(path.join(dir, "tools"), { recursive: true });
		await fs.writeFile(
			path.join(dir, "tools", "sentinel.ts"),
			`import * as fs from "node:fs"; fs.writeFileSync(process.env.SKC_TEST_IMPORT_SENTINEL!, "imported");\n`,
		);
		await fs.writeFile(
			path.join(dir, "sayknow-plugin.json"),
		...
		try {
			await compileSkcPluginBundle(dir);
		} finally {
		...
		for (const [label, overrides] of traversalCases) {
			const root = await makeTempPlugin(`skc-redteam-${label.replaceAll(" ", "-")}-`, baseManifest(overrides));
			await expectCompileError(root, "missing_file");
		}
	});

	test("rejects a symlink that resolves outside the plugin root", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-symlink-"));
		tempDirs.push(dir);
		const outside = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-symlink-outside-"));
		tempDirs.push(outside);
	...
	test("rejects declared sha256 tampering", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-hash-"));
		tempDirs.push(dir);
		await fs.mkdir(path.join(dir, "tools"), { recursive: true });
	...
	test("reports corrupt registry JSON as invalid_manifest", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-corrupt-registry-"));
		tempDirs.push(cwd);
		const registryDir = path.join(cwd, ".skc", "skc-plugins");
	...
	test("parallel registry updates keep all entries sorted and leave no tmp files", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-redteam-concurrent-registry-"));
		tempDirs.push(cwd);
		...
		const leftovers = (await fs.readdir(path.join(cwd, ".skc", "skc-plugins"))).filter(name =>
			name.includes(".tmp-"),
		);
