...
const binWrapper = path.join(repoRoot, "packages", "sayknow-cli", "bin", "skc.js");
...
async function copyWrapperIntoInstallRoot(root: string): Promise<string> {
	const installedBinDir = path.join(root, "node_modules", "sayknow-cli", "bin");
	await fs.mkdir(installedBinDir, { recursive: true });
	const installedWrapper = path.join(installedBinDir, "skc.js");
...
async function writeMockCodingAgentPackage(root: string): Promise<string> {
	const packageDir = path.join(root, "node_modules", "@sayknow-cli", "coding-agent");
...
describe("sayknow-cli global bin wrapper", () => {
	it("invokes runCli from the installed coding-agent dependency instead of only side-effect importing it", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-global-bin-wrapper-"));
		cleanupRoot = root;
