...
import { CustomProviderWizardComponent } from "@sayknow-cli/coding-agent/modes/components/custom-provider-wizard";
import {
	type ProviderOnboardingAction,
	ProviderOnboardingSelectorComponent,
} from "@sayknow-cli/coding-agent/modes/components/provider-onboarding-selector";
...
import { addApiCompatibleProvider, type ProviderSetupInput } from "@sayknow-cli/coding-agent/setup/provider-onboarding";
...
async function withRegistry<T>(
	...
	tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-provider-wizard-redteam-"));
