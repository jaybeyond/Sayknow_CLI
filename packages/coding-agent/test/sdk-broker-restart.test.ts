...
describe("SDK broker restart", () => {
	it("takes over a stale lock and rotates discovery token", async () => {
		const dir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-restart-"));
	...
	it("reclaims repeated identical stale locks into distinct tombstones", async () => {
		const dir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-restart-repeat-"));
	...
	it("allows one owner during simultaneous primary lock takeover", async () => {
		const dir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-restart-race-"));
		const lock = path.join(dir, "sdk", "broker.lock");
		await fs.mkdir(lock, { recursive: true });
		await fs.writeFile(
	...
	it("takes over a legacy regular-file stale lock", async () => {
		const dir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-restart-legacy-"));
		const lock = path.join(dir, "sdk", "broker.lock");
