...
describe("plugin MCP live connection", () => {
	test("installs and connects a bundled stdio MCP server, exposing its tools", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-connect-"));
		tempDirs.push(cwd);
		...
		const sources = {
			domain_docs: { provider: "skc-plugins", providerName: "SKC plugin bundle", level: "project" as const },
		};
	...
	test("plugin stdio configs request no-env isolation and the child cannot read host secrets", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-noenv-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(mcpBundle, { scope: "project", cwd });
		...
		const secretKey = "SKC_PLUGIN_TEST_SECRET";
