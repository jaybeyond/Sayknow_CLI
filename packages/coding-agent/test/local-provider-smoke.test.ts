...
} from "@sayknow-cli/coding-agent/cli/local-provider-smoke";
import { hookFetch } from "@sayknow-cli/utils/hook-fetch";
...
describe("local provider streaming smoke", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `skc-local-provider-smoke-${crypto.randomUUID()}`);
