...
import { resolveSkcRuntimeSpawnInfo } from "../src/daemon/runtime";
import {
...
function tempAgentDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "skc-daemon-control-test-"));
}
...
describe("daemon runtime detection", () => {
	test("source runtime picks up edits; compiled warns", () => {
		const source = resolveSkcRuntimeSpawnInfo("/usr/local/bin/node");
		...
		expect(source.argsPrefix[0]).toEndWith(path.join("packages", "coding-agent", "bin", "skc.js"));

		const compiled = resolveSkcRuntimeSpawnInfo("/opt/skc/skc");
	...
	test("chat daemon spawn uses source and compiled command forms", () => {
		...
		expect(compiled.command).toBe("/opt/skc/skc");
...
describe("cli registration", () => {
	test("skc daemon is registered in the explicit command registry", () => {
