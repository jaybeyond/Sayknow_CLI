...
import { type ConfiguredModelChain, SessionManager } from "@sayknow-cli/coding-agent/session/session-manager";

const tempDirs: string[] = [];

function makeTempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-configured-model-chain-"));
	tempDirs.push(dir);
	return dir;
}
