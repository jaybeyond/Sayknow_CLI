...
import { syncSkillActiveState } from "@sayknow-cli/coding-agent/skill-state/active-state";
...
beforeEach(async () => {
	tempDir = TempDir.createSync("@skc-plugin-tool-refresh-");
...
describe("AgentSession SKC plugin sub-skill tool refresh", () => {
	test("adds and removes sub-skill tools as the active phase changes", async () => {
		...
		await session.refreshSkcSubskillTools();
