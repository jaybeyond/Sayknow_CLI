...
import {
	...
	gcPidProbe,
	runSkcGcCommand,
} from "@sayknow-cli/coding-agent/skc-runtime/gc-runtime";
import { teamWorkersGcAdapter } from "@sayknow-cli/coding-agent/skc-runtime/team-gc";
...
function ctxFor(base: string, registryDir: string, probe: GcPidProbe): GcContext {
	return {
		...
		env: {
			...process.env,
			SKC_HARNESS_ROOT_REGISTRY_DIR: registryDir,
			SKC_RECEIPT_SPOOL_DIR: path.join(base, "spool"),
		},
		cwd: base,
	};
}
...
describe("gc red-team invariants", () => {
	...
	test("dry-run JSON with a real reaped dead lease deletes nothing on disk and exits zero", async () => {
		...
		const result = await runSkcGcCommand(
			["--json"],
			base,
			{
				...process.env,
	...
	test("unknown gc flag exits with parse status 2", async () => {
		const result = await runSkcGcCommand(["--bogus"], "/tmp", process.env, []);
