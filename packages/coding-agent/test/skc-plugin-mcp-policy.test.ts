...
function expectPolicyError(fn: () => unknown): void {
	try {
		...
		expect((error as SkcPluginLoadError).code).toBe("security_policy");
		return;
	}
	throw new Error("Expected security_policy error");
}

describe("SKC MCP policy: URL", () => {
	const denied = [
		"http://example.com/",
		"https://localhost/",
		"https://127.0.0.1/",
...
describe("SKC MCP policy: IP ranges", () => {
	test("ipv4 deny set", () => {
		for (const ip of [
			"127.0.0.1",
			"10.1.2.3",
...
describe("SKC MCP policy: headers", () => {
