 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@sayknow-cli`
 ...
 *   - `@plannotator/pi-extension`         ⇒ `@sayknow-cli/agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@sayknow-cli/coding-agent/...`
 *
...
import { loadExtensions } from "@sayknow-cli/coding-agent/extensibility/extensions/loader";
import { TempDir } from "@sayknow-cli/utils";

const canonicalCodingAgent = Bun.resolveSync("@sayknow-cli/coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@sayknow-cli/coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@sayknow-cli/utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@sayknow-cli/tui", import.meta.dir);
...
const canonicalAiOauth = Bun.resolveSync("@sayknow-cli/ai/utils/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// @sayknow-cli self-import — canonical scope must still flow through the shim
	// so a duplicate copy is never dragged in from a plugin's own node_modules.
	{ id: "ohmypi-utils", aliasSpecifier: "@sayknow-cli/utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		id: "ohmysayknow-cli",
		aliasSpecifier: "@sayknow-cli/coding-agent",
	...
	// @mariozechner — defends the original remap (regression: issue #973).
	{
		...
		aliasSpecifier: "@mariozechner/sayknow-cli/extensibility/extensions",
