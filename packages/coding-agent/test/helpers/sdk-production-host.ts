...
export async function startProductionSdkHost(
	...
	return withFixtureBrokerEnvironment(async () => {
		...
		try {
			...
			// under SKC_SDK_DISABLE=1; the explicit extension hosts later at
			...
			const priorSdkDisable = process.env.SKC_SDK_DISABLE;
			...
			try {
				...
				if (priorSdkDisable === undefined) delete process.env.SKC_SDK_DISABLE;
				else process.env.SKC_SDK_DISABLE = priorSdkDisable;
			}
