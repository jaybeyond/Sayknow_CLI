...
async function managedWorkspace(
	...
	try {
		...
		process.env.SKC_LIFECYCLE_REQUEST_ID = `prepare-${name.toLowerCase()}-${sessionId}`;
		...
		await expect(
			fs.access(path.join(resolved.scope.directoryPath, ".skc-managed-session-scope.v2.json")),
		).resolves.toBeNull();
...
export async function createSharedLifecycleFixture(
	...
	const root = await fs.mkdtemp(path.join(tmpdir(), "skc-sdk-shared-lifecycle-"));
...
export async function createLifecycleFixture(): Promise<LifecycleFixture> {
	const repo = await fs.mkdtemp(path.join(tmpdir(), "skc-sdk-machine-lifecycle-"));
	const agentDir = path.join(repo, ".skc", "agent");
	const stateRoot = path.join(repo, ".skc", "state");
