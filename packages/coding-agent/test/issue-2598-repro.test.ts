...
const workspaceUtilsSpecifier = "@sayknow-cli/utils/";
 ...
 * of `skc --smoke-test` remains the Darwin compiled-binary contract.
 */
describe("issue #2598 — tab worker source and compiled smoke contract", () => {
	...
	it("rejects workspace utility subpaths that reach native bindings", async () => {
		const graph = await collectRuntimeImportGraph(await resolveWorkspaceUtilsModule("@sayknow-cli/utils/procmgr"));
	...
	it("rejects unsafe or unresolved workspace utility subpaths without falling back to package resolution", async () => {
		await expect(resolveWorkspaceUtilsModule("@sayknow-cli/utils/not-a-runtime-module")).rejects.toThrow(
			"Unresolved workspace utility subpath",
		);
		await expect(resolveWorkspaceUtilsModule("@sayknow-cli/utils/../procmgr")).rejects.toThrow(
