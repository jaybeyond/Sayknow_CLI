...
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the skc path that
...
// does not follow filesystem links, so it misclassified Bun-installed skc
// as "binary" and tried to swap skc.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies skc reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies skc at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "skc");
