...
import type { ExtensionRunner } from "@sayknow-cli/coding-agent/extensibility/extensions";
import { GoalTool } from "@sayknow-cli/coding-agent/goals/tools/goal-tool";
...
import { createTools, type Tool, type ToolSession } from "@sayknow-cli/coding-agent/tools";
...
describe("InteractiveMode goal mode integration", () => {
	...
	it("arms the active goal tool after skc ultragoal create-goals succeeds", async () => {
		...
		expect(harness.session.getGoalModeState()?.goal.objective).toContain(".skc/ultragoal/goals.json");
