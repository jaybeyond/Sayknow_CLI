...
import { GoalRuntime } from "@sayknow-cli/coding-agent/goals/runtime";
import type { Goal, GoalModeState, GoalTokenUsage } from "@sayknow-cli/coding-agent/goals/state";
import { GoalTool } from "@sayknow-cli/coding-agent/goals/tools/goal-tool";
import { createUltragoalPlan, startNextUltragoalGoal } from "@sayknow-cli/coding-agent/skc-runtime/ultragoal-runtime";
	...
	it("blocks direct unified goal completion for active ultragoal objectives without verification receipt", async () => {
		...
		try {
			...
			const harness = createRuntimeHarness({
				enabled: true,
				mode: "active",
				goal: createGoal({ objective: plan.skcObjective }),
			});
			const tool = new GoalTool(
				createToolSession({
					cwd: root,
