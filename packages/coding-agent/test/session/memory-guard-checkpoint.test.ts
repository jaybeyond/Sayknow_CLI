...
} from "@sayknow-cli/coding-agent/session/memory-guard-checkpoint-participant";
import { memoryGuardCanonicalJson } from "@sayknow-cli/coding-agent/session/memory-guard-checkpoint-participant";
...
} from "@sayknow-cli/coding-agent/skc-runtime/memory-guard-owner-claims";
import { openRecoveryFsRoot, type RecoveryFsRoot } from "@sayknow-cli/natives";

const tempRoots: string[] = [];
const authStores: AuthStorage[] = [];

async function makeTempRoot(): Promise<string> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-memory-guard-checkpoint-"));
	tempRoots.push(root);
	return root;
}
