...
} from "@sayknow-cli/coding-agent/config/model-registry";
import {
	DEFAULT_SKC_DEFINITION_NAMES,
	getDefaultSkcDefinitions,
	getEmbeddedDefaultSkcSkillFragments,
...
async function makeTempRoot(): Promise<string> {
	...
	// ~/.skc/agents as a project config directory.
	const tempRoot = await fs.mkdtemp(path.join(path.sep, "tmp", "skc-default-definitions-"));
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
	...
	const home = await fs.mkdtemp(path.join(os.tmpdir(), "skc-default-home-"));
	tempRoots.push(home);
...
describe("default SKC definitions", () => {
	...
	it("exposes deep-interview fragments only through the parent-scoped fragment accessor", () => {
		const fragments = getEmbeddedDefaultSkcSkillFragments("deep-interview");
	...
	it("wires the ai-slop-cleaner into the ultragoal completion gate before verification and red-team", () => {
		const ultragoal = getDefaultSkcDefinitions().find(
	...
	it("keeps the four role agents bundled when project .skc is absent", async () => {
	...
	it("exposes default and four SKC role agents as model assignment targets", () => {
	...
	it("makes installed project workflow skills discoverable without installing project agent stubs", async () => {
		await withTempHome(async home => {
			const repoRoot = await makeTempRoot();
			const projectSkcRoot = path.join(repoRoot, ".skc");
			await installDefaultSkcDefinitions({ targetRoot: projectSkcRoot });
	...
	it("preserves project .skc agent overrides at runtime", async () => {
		await withTempHome(async home => {
			const repoRoot = await makeTempRoot();
			const agentsDir = path.join(repoRoot, ".skc", "agents");
...
`,
	...
	it("documents role-agent delegation in system and ultragoal prompts", async () => {
		...
		expect(systemPrompt).toContain("committed repo-visible `.skc` defaults are not the source of truth");
	...
	it("documents leader-owned Ultragoal checkpoints for Team bridge workers", async () => {
		const team = await Bun.file(
			path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "skc", "skills", "team", "SKILL.md"),
		).text();
		const ultragoal = await Bun.file(
			path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "skc", "skills", "ultragoal", "SKILL.md"),
		).text();

		expect(team).toContain("current-session active SKC goal snapshot");
		expect(ultragoal).toContain("current-session SKC goal snapshot");
		for (const content of [team, ultragoal]) {
			expect(content).toContain("Workers must not run `skc ultragoal checkpoint`");
	...
	it("keeps bundled deep-interview skill on SKC-native workflow vocabulary", () => {
		const deepInterview = getDefaultSkcDefinitions().find(
	...
	it("keeps bundled ralplan stage artifacts on CLI write path", () => {
		...
		expect(content).toContain("skc ralplan --write --stage <type> --stage_n <N> --artifact");
	...
	it("installs bundled workflow skill definitions without overwriting local edits unless forced", async () => {
		...
		const initial = await installDefaultSkcDefinitions({ targetRoot });
	...
	it("does not make installed fragments reachable as skill-relative internal URL assets", async () => {
		await withTempHome(async () => {
			const repoRoot = await makeTempRoot();
			await installDefaultSkcDefinitions({ targetRoot: path.join(repoRoot, ".skc") });
...
describe("bundled skills CLI", () => {
	it("reads embedded workflow skills from outside the repository without .skc files", async () => {
		const externalRoot = await makeTempRoot();
		...
		expect(parsed.path).toBe("embedded:skc/skills/ultragoal/SKILL.md");
