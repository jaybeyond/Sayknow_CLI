...
import { Agent, type AgentMessage, type StreamFn } from "@sayknow-cli/agent-core";
...
import { type AbortOutcome, AgentSession } from "@sayknow-cli/coding-agent/session/agent-session";
...
describe("AgentSession.cancelAndSubmit", () => {
	...
	it("holds the duplicate token through prompt preflight and rolls back a preflight failure", async () => {
		...
		vi.spyOn(s, "refreshSkcSubskillTools").mockImplementationOnce(() => preflight.promise);
		s.setCancelAndSubmitAbortOutcomeProviderForTests(async () => ({ kind: "settled" }));
