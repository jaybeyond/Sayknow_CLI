...
import { getConfigRootDir, getMCPConfigPath, setAgentDir } from "@sayknow-cli/utils";
...
describe("skc mcp CLI helpers", () => {
	beforeEach(async () => {
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-cli-"));
	...
	it("adds project-scoped HTTP servers and redacts headers from text output", async () => {
		...
		expect(output).toContain("normal standalone skc sessions do not load stored MCP registrations today");
