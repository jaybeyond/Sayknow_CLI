...
import { loadActiveSubskillTools } from "../src/extensibility/skc-plugins/tools";
import { syncSkillActiveState } from "../src/skill-state/active-state";

const tempRoots: string[] = [];

async function makeTempRoot(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugin-tools-"));
	tempRoots.push(cwd);
	return cwd;
}
...
describe("SKC plugin sub-skill tools", () => {
