...
import {
	...
	writeCrashReport,
} from "@sayknow-cli/coding-agent/debug/crash-diagnostics";
import { executeBash } from "@sayknow-cli/coding-agent/exec/bash-executor";

const tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-crash-diagnostics-test-"));
	tempDirs.push(dir);
	return dir;
}
...
afterEach(async () => {
	...
	delete process.env.SKC_CRASH_DIAGNOSTICS;
	delete process.env.SKC_CRASH_DIAGNOSTICS_DIR;
});
...
describe("crash diagnostics", () => {
	...
	it("writes opt-in structured reports only for crashed classes", async () => {
		...
		const env = { SKC_CRASH_DIAGNOSTICS: "1", SKC_CRASH_DIAGNOSTICS_DIR: dir } as NodeJS.ProcessEnv;
	...
	it("creates the default diagnostics directory and reports privately under umask 022", async () => {
		...
		try {
			const env = { SKC_CRASH_DIAGNOSTICS: "1" } as NodeJS.ProcessEnv;
			const crashed = await writeCrashReport(
			...
			expect(crashed.path).not.toBeNull();
			const defaultDir = path.join(tempRoot, "skc-crash-diagnostics");
	...
	it("appends a bash crash notice and artifact when diagnostics are enabled", async () => {
		const dir = await makeTempDir();
		process.env.SKC_CRASH_DIAGNOSTICS = "1";
		process.env.SKC_CRASH_DIAGNOSTICS_DIR = dir;
