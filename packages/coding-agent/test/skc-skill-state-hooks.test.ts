...
describe("SKC native skill-state hooks", () => {
	...
	async function cwd(): Promise<string> {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-skill-hooks-"));
		return tempDir;
	}
	...
	it("detects only the public SKC workflow skill surface", () => {
	...
	it("deep-interview plaintext ask leak blocks stop with ask-tool recovery", async () => {
		...
		for (const sessionFile of [
			...
			const genericBlocked = await dispatchSkcNativeSkillHook({
				hookEventName: "Stop",
				cwd: root,
				sessionId,
				threadId: sessionId,
			...
			expect(genericBlocked.outputJson?.stopReason).not.toBe("skc_skill_deep_interview_plaintext_ask_leak");
		...
		const crystallizedAllowed = await dispatchSkcNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: crystallizedSessionId,
			threadId: crystallizedSessionId,
		...
		const cancelledAllowed = await dispatchSkcNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: cancelledSessionId,
			threadId: cancelledSessionId,
		...
		const ralplanBlocked = await dispatchSkcNativeSkillHook({
			hookEventName: "Stop",
			cwd: root,
			sessionId: ralplanSessionId,
			threadId: ralplanSessionId,
		...
		expect(ralplanBlocked.outputJson?.stopReason).not.toBe("skc_skill_deep_interview_plaintext_ask_leak");
	});
	...
	it("rich deep-interview prompt activation blocks product mutation and direct spec artifacts", async () => {
		...
		const blocked = await getDeepInterviewMutationDecision({
			...
			sessionId: "session-rich",
			tool: { name: "write" } as never,
			args: { path: ".skc/state/sessions/session-rich/deep-interview-state.json", content: "{}" },
		});
	...
	it("UserPromptSubmit injects sanitized effective skill config without raw paths or settings-file instructions", async () => {
		...
		expect(context).not.toContain("~/.skc");
		expect(context).not.toContain(".skc/settings.json");
...
`,
	...
	it("Stop forces deep-interview crystallization when an ordinary stop would terminalize without a spec", async () => {
		...
		expect(String(blocked.outputJson?.reason ?? "")).toContain("skc deep-interview --write");
	});
	...
	it("UserPromptSubmit recovers active Ultragoal objective from session transcript", async () => {
		...
		await Bun.write(
			sessionFile,
			`${JSON.stringify({ type: "session", id: "session-ultra-transcript", timestamp: new Date().toISOString(), cwd: root })}\n${JSON.stringify({ type: "mode_change", id: "1", parentId: null, timestamp: new Date().toISOString(), mode: "goal", data: { goal: { objective: plan.skcObjective, status: "active" } } })}\n`,
		);
		await dispatchSkcNativeSkillHook(
			{
				hookEventName: "UserPromptSubmit",
	...
	it("Stop blocks verified Ultragoal stories while later required goals remain", async () => {
		...
		await checkpointUltragoalGoal({
			...
			skcGoalJson: goalSnapshot(plan.skcObjective),
	...
	it("Stop blocks when stale Ultragoal mode-state would release but the plan still has pending goals", async () => {
		...
		expect(blocked.outputJson).toMatchObject({
			decision: "block",
			stopReason: "skc_skill_ultragoal_stale_mode_state",
		});
	...
	it("merges managed Codex UserPromptSubmit/Stop hooks without dropping user hooks", () => {
		const existing = JSON.stringify({
			hooks: {
				...
				Stop: [
					{ hooks: [{ type: "command", command: "skc codex-native-hook" }] },
					{ hooks: [{ type: "command", command: "echo user-stop" }] },
				],
			},
		});

		const merged = mergeSkcManagedCodexHooksConfig(existing);
		...
		expect(readSkcManagedCodexHooksStatus(merged.content, "/tmp/hooks.json")).toMatchObject({
			installed: true,
			missingEvents: [],
			managedHookCount: 2,
		});
