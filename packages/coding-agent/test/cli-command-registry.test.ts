...
describe("CLI command registry", () => {
	it("registers the `plugin` command so `skc plugin …` resolves instead of routing to launch", () => {
		...
		// `isSubcommand()` therefore returned false for "plugin", so `skc plugin
		...
		// `skc plugin install <package>`, an unreachable command.
	...
	it("registers the `stats` command so `skc stats` resolves instead of routing to launch", () => {
		// Regression: `src/commands/stats.ts` (and the `@sayknow-cli/stats`
		// dependency it drives via `src/cli/stats-cli.ts`) existed, but the
		// entry was never added to the `commands` registry in cli.ts.
		// `isSubcommand()` therefore returned false for "stats", so `skc stats`
