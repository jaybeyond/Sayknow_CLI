...
import { type AuthBrokerServerHandle, AuthStorage, SqliteAuthCredentialStore, startAuthBroker } from "@sayknow-cli/ai";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.SKC_AGENT_DIR;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.SKC_AGENT_DIR;
		else process.env.SKC_AGENT_DIR = originalAgentDir;
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to skc providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.SKC_AUTH_BROKER_URL = process.env.SKC_AUTH_BROKER_URL;
		savedEnv.SKC_AUTH_BROKER_TOKEN = process.env.SKC_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.SKC_AUTH_BROKER_URL = handle.url;
		process.env.SKC_AUTH_BROKER_TOKEN = token;
	});
	...
	afterEach(async () => {
		...
		await fs.rm(cliproxyDir, { recursive: true, force: true });
		for (const key of ["SKC_AUTH_BROKER_URL", "SKC_AUTH_BROKER_TOKEN"] as const) {
