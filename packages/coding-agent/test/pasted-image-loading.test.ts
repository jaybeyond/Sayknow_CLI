...
describe("loadPastedImageBatch", () => {
	...
	beforeEach(async () => {
		testDirectory = await fs.mkdtemp(path.join(os.tmpdir(), "skc-pasted-image-loading-"));
	});
	...
	it("keeps automatic clipboard paths inside the canonical temp root", async () => {
		const outsideDirectory = await fs.mkdtemp(path.join(os.homedir(), ".skc-pasted-image-outside-"));
