...
import { ToolExecutionComponent } from "@sayknow-cli/coding-agent/modes/components/tool-execution";
...
import { resetVimRendererStateForTest, VimTool, vimToolRenderer } from "@sayknow-cli/coding-agent/tools/vim";
import { VimBuffer } from "@sayknow-cli/coding-agent/vim/buffer";
import { VimEngine } from "@sayknow-cli/coding-agent/vim/engine";
import { parseKeySequences } from "@sayknow-cli/coding-agent/vim/parser";
...
describe("vim renderer", () => {
	it("reuses the last real vim snapshot while args stream", async () => {
		...
		const theme = await themeModule.getThemeByName("red-octopus");
