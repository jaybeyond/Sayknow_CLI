...
import type { AuthStorage } from "@sayknow-cli/ai";
...
const originalDocumentedCodexSearchModel = process.env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL;
...
describe("searchCodex model selection", () => {
	...
	afterEach(() => {
		...
		if (originalDocumentedCodexSearchModel === undefined) {
			delete process.env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL;
		} else {
			process.env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL = originalDocumentedCodexSearchModel;
		}
	});
	...
	it("uses the documented SKC_OPENAI_CODE_WEB_SEARCH_MODEL when provided", async () => {
		process.env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL = "gpt-5.4-mini";
		using _hook = mockCodexFetch("gpt-5.4-mini");
	...
	it("resolves SKC-first: SKC_OPENAI_CODE_WEB_SEARCH_MODEL wins over legacy PI_CODEX_WEB_SEARCH_MODEL", async () => {
		process.env.SKC_OPENAI_CODE_WEB_SEARCH_MODEL = "gpt-5.4-mini";
		process.env.PI_CODEX_WEB_SEARCH_MODEL = "gpt-5.4";
		using _hook = mockCodexFetch("gpt-5.4-mini");

		const result = await searchCodex(makeSearchParams("skc-first codex model"));
