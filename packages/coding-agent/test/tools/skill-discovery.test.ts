...
describe("SkillDiscoveryTool", () => {
	it("discovers project runtime skills from .skc/skills", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-project-skills-"));
	...
	it("discovers user runtime skills from ~/.skc/skills", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-user-skills-cwd-"));
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "skc-user-skills-home-"));
		const originalHome = process.env.HOME;
		process.env.HOME = home;
		try {
			await makeSkill(path.join(home, ".skc", "skills"), "user-helper", "User helper skill");
			const settings = runtimeSkillSettings();
	...
	it("does not classify home .skc skills as project skills while walking up", async () => {
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "skc-home-skill-boundary-"));
		const cwd = path.join(home, "work", "project", "nested");
		await fs.mkdir(cwd, { recursive: true });
		await makeSkill(path.join(home, ".skc", "skills"), "home-helper", "Home helper skill", "Home body.");
		const originalHome = process.env.HOME;
		process.env.HOME = home;
		try {
	...
	it("does not return bundled built-in skills or grow the core prompt catalog", async () => {
		...
		const builtInSkill: Skill = {
			...
			filePath: "embedded:skc/skills/ralplan/SKILL.md",
			baseDir: "embedded:skc/skills/ralplan",
			source: "embedded",
		};
		...
		const prompt = await buildSystemPrompt({
			...
			skills: [
				builtInSkill,
				{
					...
					filePath: path.join(cwd, ".skc", "skills", "project-helper", "SKILL.md"),
					baseDir: path.join(cwd, ".skc", "skills", "project-helper"),
	...
	it("loads selected discovered skill content through the skill invocation path", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-selected-skill-"));
		await makeSkill(path.join(cwd, ".skc", "skills"), "project-helper", "Project helper skill", "Loaded narrowly.");
		const settings = runtimeSkillSettings();
		const sent: Array<{ content: string; details?: unknown }> = [];
		const tool = new SkillTool(
			createSession(cwd, {
	...
	it("does not discover or invoke runtime skills when skills.enabled is false", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-skills-disabled-"));
		await makeSkill(path.join(cwd, ".skc", "skills"), "project-helper", "Project helper skill", "Blocked body.");
	...
	it("explains empty results caused by disabled discovery scopes, and stays silent for genuine emptiness", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-skills-notice-"));
		await makeSkill(path.join(cwd, ".skc", "skills"), "project-helper", "Project helper skill");
	...
	it("applies source enable flags and skill filters to discovery and invocation", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-skills-policy-"));
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "skc-skills-policy-home-"));
		await makeSkill(path.join(cwd, ".skc", "skills"), "project-helper", "Project helper skill", "Project body.");
		await makeSkill(path.join(home, ".skc", "skills"), "user-helper", "User helper skill", "User body.");
		const originalHome = process.env.HOME;
		process.env.HOME = home;
		try {
