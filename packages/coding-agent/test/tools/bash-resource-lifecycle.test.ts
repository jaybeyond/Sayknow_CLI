...
import { ArtifactManager } from "@sayknow-cli/coding-agent/session/artifacts";
import { DEFAULT_ARTIFACT_MAX_BYTES, OutputSink } from "@sayknow-cli/coding-agent/session/streaming-output";
import { BashTool, type ToolSession } from "@sayknow-cli/coding-agent/tools";
import type { Shell } from "@sayknow-cli/natives";
import * as piNatives from "@sayknow-cli/natives";

function makeTempDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "skc-bash-lifecycle-"));
}
...
describe("bash resource lifecycle", () => {
	...
	it("caps minimized raw output artifacts through ArtifactManager", async () => {
		...
		const bashModule = await import("@sayknow-cli/coding-agent/tools/bash");
