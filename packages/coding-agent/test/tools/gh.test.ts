...
} from "@sayknow-cli/coding-agent/tools/gh";
import * as git from "@sayknow-cli/coding-agent/utils/git";
import { getAgentDir, hashPath, setAgentDir } from "@sayknow-cli/utils";
 ...
 * user's real `~/.skc/wt`. Returns the temp home and a cleanup hook.
 */
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	setAgentDir(path.join(home, ".skc", "agent"));
	return {
		home,
		cleanup: async () => {
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".skc", "wt", segment));
}
...
describe("github tool", () => {
	...
	it("checks out multiple pull requests in a single call when pr is an array", async () => {
		...
		try {
			...
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-100.skcPrUrl"])).toBe(
				"https://github.com/owner/repo/pull/100",
			);
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-200.skcPrUrl"])).toBe(
