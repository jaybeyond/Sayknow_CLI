...
import { AskTool, askToolRenderer } from "@sayknow-cli/coding-agent/tools/ask";
import { ToolAbortError } from "@sayknow-cli/coding-agent/tools/tool-errors";
...
describe("AskTool deep-interview rendering middleware", () => {
	...
	it("recognizes topology questions even when the agent prepends an intro", async () => {
		...
		const rawQuestion = [
			...
			"",
			'**Your idea:** "Refresh the SKC UX"',
			"**Project type:** brownfield",
			"",
			"Round 0 | Topology confirmation | Ambiguity: not scored yet",
			"",
			"I'm currently reading the scope as these 2 top-level components.",
			"1. Brand and theme system: red-octopus/SKC default theme and semantic color separation.",
		...
		expect(prompt).not.toContain('**Your idea:** "Refresh the SKC UX"');
