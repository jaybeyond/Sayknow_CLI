...
describe("renderJsonTreeLines", () => {
	...
	it("wraps long path-like string values as deterministic continuations", async () => {
		...
		const target = "../../../sayknow-cli.sayknow-cli-worktrees/release-0-5-4-64d49adc/packages/coding-agent";
		...
		expect(lines[0]).toContain('target: "../../../sayknow-cli.sayknow-c');
		expect(lines[0]).not.toContain(
			'"../../../sayknow-cli.sayknow-cli-worktrees/release-0-5-4-64d49adc/packages/coding-agent"',
		);
		expect(lines.slice(1).every(line => line.includes("↳ "))).toBe(true);
		expect(lines.at(-1)).toContain('"');
		expect(lines.join("\n")).toContain("worktrees/release-0-5-4-64d");
	...
	it("marks long string values truncated when continuation lines exhaust the line budget", async () => {
		const theme = await testTheme();
		const real =
			"/Users/example/Documents/Workspace/sayknow-cli.sayknow-cli-worktrees/release-0-5-4-64d49adc/packages/coding-agent/src/tools/json-tree.ts";
	...
	it("keeps escaped tabs readable in wrapped string values", async () => {
		const theme = await testTheme();
		const value = "node_modules/@sayknow-cli/coding-agent\tpackages/coding-agent/src/tools/json-tree.ts";
