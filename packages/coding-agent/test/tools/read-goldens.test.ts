...
import { EditTool, getFileReadCache } from "@sayknow-cli/coding-agent/edit";
import { computeLineHash } from "@sayknow-cli/coding-agent/hashline/hash";
...
import { wrapToolWithMetaNotice } from "@sayknow-cli/coding-agent/tools/output-meta";
import * as scrapers from "@sayknow-cli/coding-agent/web/scrapers/types";
...
const nativeIndexUrl = import.meta.resolve("@sayknow-cli/natives");
...
function installRealNativesMock(): void {
	mock.module("@sayknow-cli/natives", () => realNatives);
}
...
const realMarkit = await import("@sayknow-cli/coding-agent/utils/markit");

mock.module("@sayknow-cli/coding-agent/utils/markit", () => ({
	...realMarkit,
...
const UPDATE = process.env.SKC_UPDATE_READ_GOLDENS === "1";
const SEED_INVARIANT = process.env.SKC_SEED_READ_GOLDENS === "1";
const SEED_SURFACE = process.env.SKC_SEED_READ_SURFACE_GOLDENS === "1";
const FORCE_TRUNCATION = process.env.SKC_FORCE_READ_TRUNCATION as "head" | "last" | "both" | undefined;
...
function writeGolden(bucket: Bucket, name: string, variant: Variant, value: Captured): void {
	...
	if (!UPDATE) throw new Error("writeGolden called without SKC_UPDATE_READ_GOLDENS=1");
...
function seedGolden(bucket: Bucket, name: string, variant: Variant, value: Captured): void {
	...
	if (!SEED_INVARIANT) throw new Error("seedGolden called without SKC_SEED_READ_GOLDENS=1");
...
function seedSurfaceGolden(name: string, variant: Variant, value: Captured): void {
	if (!SEED_SURFACE) throw new Error("seedSurfaceGolden called without SKC_SEED_READ_SURFACE_GOLDENS=1");
...
const REQUESTED_PHASE = process.env.SKC_READ_GOLDENS_PHASE;
...
async function captureSurface(entry: Entry, tool: { description: string }): Promise<Captured> {
	switch (entry.name) {
		...
		case "cli-read-help": {
			...
			if (process.exitCode !== 0) throw new Error(`skc read --help failed: ${error}`);
...
describe("read truncation golden harness", () => {
	...
	it("round-trips tail hashline anchors through ReadTool, cache, and EditTool", async () => {
		...
		const previousSkcVariant = Bun.env.SKC_EDIT_VARIANT;
		Bun.env.PI_EDIT_VARIANT = "hashline";
		Bun.env.SKC_EDIT_VARIANT = "hashline";
		...
		try {
			...
			if (previousSkcVariant === undefined) delete Bun.env.SKC_EDIT_VARIANT;
			else Bun.env.SKC_EDIT_VARIANT = previousSkcVariant;
		}
	});
