...
describe("BashTool restricted role-agent allowlist", () => {
	function createRestrictedBashTool(
		cwd = process.cwd(),
		bashAllowedPrefixes = ["skc ralplan --write", "skc state"],
	...
	it("surfaces restricted prefixes in the tool description", () => {
		...
		expect(tool.description).toContain("skc ralplan --write");
		expect(tool.description).toContain("skc state");
	});
	...
	it("allows read-only commands and still blocks env overrides or non-inspection commands", async () => {
		...
		try {
			...
			await expect(tool.execute("tool-call", { command: "ls", env: { PATH: "/tmp/fake" } })).rejects.toThrow(
				"Read-only bash only allows the SKC_RALPLAN_ARTIFACT env override for --artifact-env.",
			);
		} finally {
	...
	it("blocks ralplan invocations that are not artifact writes", async () => {
		...
		await expect(tool.execute("tool-call", { command: "skc ralplan --consensus 'task'" })).rejects.toThrow(
			"skc ralplan --write",
		);
	});
	...
	it("blocks per-command env overrides in restricted mode", async () => {
		...
		await expect(
			tool.execute("tool-call", {
				command: "skc ralplan --write --stage architect --stage_n 1 --artifact ok",
				env: { PATH: "/tmp/fake" },
			}),
		).rejects.toThrow("only allows the SKC_RALPLAN_ARTIFACT env override");
	});
	...
	it("allows the sanctioned ralplan artifact env override in restricted mode", async () => {
		...
		try {
			...
			const result = await tool.execute("tool-call", {
				command: `${bunPath} ${cliPath} ralplan --write --stage critic --stage_n 1 --artifact-env SKC_RALPLAN_ARTIFACT --run-id env-marker --session-id restricted-bash-test`,
				env: {
					SKC_RALPLAN_ARTIFACT: '# Review\n\nContains `"studio"`, `use client`, $VALUE, and C:\\tmp.\n',
				},
				timeout: 30,
			});
