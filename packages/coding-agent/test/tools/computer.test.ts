...
import { summarizeComputerDetails } from "@sayknow-cli/coding-agent/tools/computer/render";
...
describe("computer tool dispatch", () => {
	...
	it("persists screenshot fallbacks in private per-session directories with restrictive file modes", async () => {
		...
		try {
			expect(firstDir).not.toBe(path.join(os.tmpdir(), "skc-computer-screenshots"));
			expect(path.basename(firstDir)).toStartWith("skc-computer-screenshots-");
