 ...
 *
 * Each test isolates `SKC_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@sayknow-cli/coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@sayknow-cli/coding-agent/tools/github-cache";
import * as git from "@sayknow-cli/coding-agent/utils/git";
...
beforeEach(async () => {
	...
	originalEnv = process.env.SKC_GITHUB_CACHE_DB;
	process.env.SKC_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.SKC_GITHUB_CACHE_DB;
	} else {
		process.env.SKC_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.SKC_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
