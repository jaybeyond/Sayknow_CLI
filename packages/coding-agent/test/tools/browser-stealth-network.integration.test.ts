...
function chromiumAvailable(): boolean {
	...
	const cache = path.join(os.homedir(), ".skc", "puppeteer", "chrome");
	let available = false;
	try {
		available = fs.existsSync(cache) && fs.readdirSync(cache).length > 0;
	} catch {}
	if (!available && process.env.SKC_REQUIRE_CHROMIUM === "1") {
		throw new Error("SKC_REQUIRE_CHROMIUM=1 requires a resolvable Chromium executable");
	}
	return available;
}
