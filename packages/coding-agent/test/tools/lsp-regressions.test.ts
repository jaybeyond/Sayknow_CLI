...
import type { RenderResultOptions } from "@sayknow-cli/agent-core";
import { LspTool } from "@sayknow-cli/coding-agent/lsp";
import * as lspClient from "@sayknow-cli/coding-agent/lsp/client";
import * as lspConfig from "@sayknow-cli/coding-agent/lsp/config";
import { getServersForFile, loadConfig } from "@sayknow-cli/coding-agent/lsp/config";
import { renderCall, renderResult } from "@sayknow-cli/coding-agent/lsp/render";
import type {
	CodeAction,
	Diagnostic,
	LspClient,
	ServerConfig,
	SymbolInformation,
} from "@sayknow-cli/coding-agent/lsp/types";
import {
	applyCodeAction,
	collectGlobMatches,
	...
	resolveSymbolColumn,
} from "@sayknow-cli/coding-agent/lsp/utils";
import { getThemeByName } from "@sayknow-cli/coding-agent/modes/theme/theme";
import type { ToolSession } from "@sayknow-cli/coding-agent/tools";
import { clampTimeout } from "@sayknow-cli/coding-agent/tools/tool-timeouts";
import * as piUtils from "@sayknow-cli/utils";
import { sanitizeText, TempDir } from "@sayknow-cli/utils";
...
describe("lsp regressions", () => {
	...
	it("refuses workspace build diagnostics without spawning project commands", async () => {
		...
		for (const marker of projectMarkers) {
			const tempDir = TempDir.createSync("@skc-lsp-workspace-diags-");
			try {
	...
	it("limits glob collection to avoid large diagnostic stalls", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-glob-");
		try {
			await Promise.all([
	...
	it("treats existing bracket paths as literal diagnostic targets", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-bracket-path-");
		try {
	...
	it("resolves the requested symbol occurrence on a line", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-regression-");
		try {
	...
	it("throws when symbol does not exist on the target line", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-missing-symbol-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("throws when occurrence is out of bounds", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-occurrence-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("does not reuse stale file diagnostics after another URI publishes", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-stale-diags-");
		try {
	...
	it("does not auto-launch LSP binaries from project node_modules/.bin", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-project-bin-");
	...
	it("detects tlaplus files for LSP startup and language ids", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-tlaplus-");
		const cwd = path.join(tempDir.path(), "repo");
		const externalBinDir = path.join(os.homedir(), `.skc-lsp-tlaplus-${process.pid}-${Date.now()}`);
	...
	it("detects csharp-ls as the preferred C# LSP when installed", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-csharp-ls-");
		const cwd = path.join(tempDir.path(), "repo");
		const externalBinDir = path.join(os.homedir(), `.skc-lsp-csharp-${process.pid}-${Date.now()}`);
	...
	it("keeps omnisharp as the C# fallback when csharp-ls is unavailable", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-omnisharp-fallback-");
		const cwd = path.join(tempDir.path(), "repo");
		const externalBinDir = path.join(os.homedir(), `.skc-lsp-omnisharp-${process.pid}-${Date.now()}`);
		const omnisharp = path.join(externalBinDir, "omnisharp");
		try {
			await fs.promises.mkdir(cwd, { recursive: true });
			await fs.promises.mkdir(externalBinDir, { recursive: true });
	...
	it("rename_file applies LSP willRenameFiles edits and renames the file", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-rename-file-");
		try {
	...
	it("rename_file with apply:false previews edits without filesystem changes", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-rename-file-preview-");
		try {
	...
	it("rename_file enumerates every file inside a directory rename", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-rename-dir-");
		try {
	...
	it("request action sends raw LSP method with auto-built textDocument/position params", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-request-");
		try {
	...
	it("request action forwards explicit JSON payload verbatim", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-request-payload-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("capabilities action dumps server capabilities", async () => {
		const tempDir = TempDir.createSync("@skc-lsp-caps-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
