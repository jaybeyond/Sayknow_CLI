...
import { PetFramedEditor, SayknowPetWidget } from "../src/modes/components/sayknow-pet-widget";
...
function makeWidget(
	...
	const widget = new SayknowPetWidget({
		ui: stubs.ui,
		editor: stubs.editor,
...
describe("SayknowPetWidget", () => {
	...
	it("stays off when no pixel protocol is available", () => {
		vi.spyOn(SayknowPetWidget, "pixelProtocol").mockReturnValue(null);
		const stubs = makeStubs();
		const widget = new SayknowPetWidget({
			ui: stubs.ui,
			editor: stubs.editor,
