...
import type { AssistantMessage, Context, ToolResultMessage } from "@sayknow-cli/ai";
...
import { SETTINGS_SCHEMA } from "@sayknow-cli/coding-agent/config/settings-schema";
...
import { TempDir, withTimeout } from "@sayknow-cli/utils";
...
describe("AgentSession pre-admission artifact spill", () => {
	...
	it("spills oversized UTF-8 tool results before provider admission and rehydrates byte-exactly", async () => {
		tempDir = TempDir.createSync("@skc-pre-admission-spill-");
	...
	it("keeps the canonical inline tool result when artifact writing fails", async () => {
		tempDir = TempDir.createSync("@skc-pre-admission-spill-failure-");
		const model = getBundledModel("anthropic", "claude-sonnet-4-5");
		if (!model) throw new Error("Expected bundled Anthropic model");
		const agent = new Agent({
			initialState: {
