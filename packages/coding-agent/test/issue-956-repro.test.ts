...
import { getConfigRootDir, getProjectDir, setAgentDir, setProjectDir } from "@sayknow-cli/utils";
...
describe("issue #956: interactive /mcp test", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-issue-956-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-issue-956-agent-"));
