/**
 * ACP stdout-hygiene smoke: launching `skc acp` must not leak any banner,
 ...
 * unwind, but force-kill quickly if it hasn't reaped — `skc acp` blocks on
...
describe("ACP stdout hygiene", () => {
	it("emits a JSON-RPC initialize response as the first bytes on stdout", async () => {
		const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "skc-acp-stdout-"));
