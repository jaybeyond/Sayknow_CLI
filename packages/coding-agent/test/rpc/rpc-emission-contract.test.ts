...
const ORIGINAL_SKC_RPC_EMIT_TITLE = process.env.SKC_RPC_EMIT_TITLE;
const ORIGINAL_PI_RPC_EMIT_TITLE = process.env.PI_RPC_EMIT_TITLE;

afterEach(() => {
	if (ORIGINAL_SKC_RPC_EMIT_TITLE === undefined) delete process.env.SKC_RPC_EMIT_TITLE;
	else process.env.SKC_RPC_EMIT_TITLE = ORIGINAL_SKC_RPC_EMIT_TITLE;
...
describe("RPC title emission flag", () => {
	it("prefers the documented SKC flag while preserving the legacy PI alias", () => {
		delete process.env.SKC_RPC_EMIT_TITLE;
		process.env.PI_RPC_EMIT_TITLE = "1";
		expect(shouldEmitRpcTitlesForTest()).toBe(true);

		process.env.SKC_RPC_EMIT_TITLE = "false";
		expect(shouldEmitRpcTitlesForTest()).toBe(false);

		process.env.SKC_RPC_EMIT_TITLE = "yes";
		expect(shouldEmitRpcTitlesForTest()).toBe(true);
	});
});
