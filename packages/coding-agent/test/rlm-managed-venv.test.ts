 ...
 *
 * Verifies BYO (VIRTUAL_ENV / .venv) precedence and, absent a BYO env, that skc
 * auto-creates and uses a per-workspace venv under <cwd>/.skc/python-env, with a
...
import { ensurePythonRuntime } from "@sayknow-cli/coding-agent/eval/py/runtime";
...
describe.skipIf(!RUN)("RLM managed per-workspace venv", () => {
	test("auto-creates <cwd>/.skc/python-env and the kernel interpreter is that venv", async () => {
		const runtime = await ensurePythonRuntime(cwd, baseEnv(), { managedWorkspaceVenv: true, seedPackages: [] });
		const managedDir = path.join(cwd, ".skc", "python-env");
	...
	test("honors a BYO .venv and does not create the managed env", async () => {
		...
		expect(await Bun.file(path.join(cwd, ".skc", "python-env")).exists()).toBe(false);
	}, 120_000);
});
