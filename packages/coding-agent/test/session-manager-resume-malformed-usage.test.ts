...
import { SessionManager, type UsageStatistics } from "@sayknow-cli/coding-agent/session/session-manager";
...
function makeTempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-malformed-usage-"));
	tempDirs.push(dir);
	return dir;
}
