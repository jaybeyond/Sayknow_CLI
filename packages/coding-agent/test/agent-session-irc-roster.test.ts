...
import type { SimpleStreamOptions } from "@sayknow-cli/ai";
import { createMockModel, type MockModel, registerMockApi } from "@sayknow-cli/ai/providers/mock";
...
import { AgentSession, type EphemeralTurnPurpose } from "@sayknow-cli/coding-agent/session/agent-session";
...
describe("AgentSession IRC roster delivery", () => {
	...
	it("preserves the delivered roster signature across same-session reload", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-irc-roster-"));
		tempDirs.push(dir);
