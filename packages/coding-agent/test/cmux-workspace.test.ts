...
const LIST_JSON = JSON.stringify({
	workspaces: [
		...
		{ id: "DF98857C", ref: "workspace:8", title: "SKC: sayknow-cli", has_custom_title: true },
		{ id: "CCCC-9999", ref: "workspace:9", title: "~/dev/x", has_custom_title: false },
	],
});

describe("cmux workspace title sync", () => {
	it("builds an explicit workspace rename command with the SKC prefix", () => {
		expect(buildCmuxWorkspaceRenameCommand("Investigate Resolver", cmuxEnv())).toEqual({
			command: "cmux",
			args: ["workspace", "rename", "workspace-123", "--title", "SKC: Investigate Resolver"],
		});
	});
	...
	it("prefixes cmux workspace titles once", () => {
		expect(formatCmuxWorkspaceTitle("Investigate Resolver")).toBe("SKC: Investigate Resolver");
		expect(formatCmuxWorkspaceTitle("SKC: Investigate Resolver")).toBe("SKC: Investigate Resolver");
	});
	...
	describe("shouldRenameCmuxWorkspace", () => {
		...
		it("skips when ownership is unknown (read failed)", () => {
			expect(shouldRenameCmuxWorkspace(null, "SKC: Desired")).toBe(false);
		});

		it("skips when the title already matches", () => {
			expect(shouldRenameCmuxWorkspace(owned({ title: "SKC: Desired" }), "SKC: Desired")).toBe(false);
		});

		it("renames when the workspace still has the default title", () => {
			expect(shouldRenameCmuxWorkspace(owned({ hasCustomTitle: false }), "SKC: Desired")).toBe(true);
		});

		it("skips a user- or peer-owned custom title", () => {
			expect(shouldRenameCmuxWorkspace(owned({ title: "My Pinned Name" }), "SKC: Desired")).toBe(false);
			expect(shouldRenameCmuxWorkspace(owned({ title: "SKC: Session A" }), "SKC: Session B")).toBe(false);
		});
	});
	...
	it("does not spawn when SKC_NO_CMUX_RENAME is set", async () => {
		let spawned = false;
		await syncCmuxWorkspaceTitle("Investigate Resolver", {
			env: cmuxEnv("ws-optout", { SKC_NO_CMUX_RENAME: "1" }),
			isTty: true,
			which: () => "/usr/local/bin/cmux",
			readOwnership: async () => ({ hasCustomTitle: false, title: "default" }),
			spawn: () => {
	...
	it("renames a default-titled workspace inside a tty cmux workspace", async () => {
		...
		expect(calls).toEqual([
			["/usr/local/bin/cmux", "workspace", "rename", "ws-default", "--title", "SKC: Investigate Resolver"],
		]);
	...
	it("does not thrash a workspace shared by multiple sessions", async () => {
		...
		await syncCmuxWorkspaceTitle("Session B task", {
			...
			isTty: true,
			which: () => "/usr/local/bin/cmux",
			readOwnership: async () => ({ hasCustomTitle: true, title: "SKC: Session A task" }),
			spawn,
		});
		expect(calls).toEqual([
			["/usr/local/bin/cmux", "workspace", "rename", "ws-shared", "--title", "SKC: Session A task"],
		]);
	});
});
