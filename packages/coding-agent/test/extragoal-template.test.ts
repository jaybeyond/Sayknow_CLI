...
describe("Extragoal skill template", () => {
	it("documents local override installation without changing the default workflow surface", async () => {
		...
		expect(template).toContain("Extragoal is **not** a bundled workflow skill; `skc extragoal` does not exist.");
		// Install path must target the scanned user-level location, frontmatter-first.
		expect(template).toContain("mkdir -p ~/.skc/agent/skills/extragoal");
		expect(template).toContain(
			"sed -n '/^---$/,$p' docs/extragoal-skill-template.md > ~/.skc/agent/skills/extragoal/SKILL.md",
		);
		expect(template).toContain("<project>/.skc/skills/extragoal/SKILL.md");
	});
	...
	it("pins the gate contract: verdict parsing, injection stance, secret scan, and tool-restricted reviewer", async () => {
		...
		expect(template).toContain("never commit `.skc/_session-*` gate artifacts");
		expect(template).toContain("The one-shot session's `default` model authors the verdict");
		expect(template).toContain("skc -p --no-session --model openai-codex/gpt-5.5:xhigh --tools read,search,find");
		expect(template).toContain(
	...
	it("pins the optional maximalist N-of-N reviewer recipe without adding core dependencies", async () => {
		...
		expect(template).toContain("No browser automation, Playwright, or Repomix dependency is added to SKC core");
		expect(template).toContain("Maximum **2 re-sign rounds**");
