...
// Snapshot the actual `SKC_CODING_AGENT_DIR` env var and the SDK/utils resolver
// state independently. `setAgentDir(...)` (called per-test in `createAgent`)
// re-seeds the module-global resolver AND sets `process.env.SKC_CODING_AGENT_DIR`,
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "skc-acp-init-"));
