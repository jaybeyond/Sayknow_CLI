...
import type { NativeOwnerOnlySecurityResult } from "@sayknow-cli/natives";
...
describe.skipIf(process.platform !== "linux")("managed scope owner-only self-heal", () => {
	it("re-secures a drifted (group/other-readable) descendant instead of failing closed", () => {
		const home = fs.mkdtempSync(path.join(os.tmpdir(), "skc-scope-heal-home-"));
		temporaryDirectories.push(home);
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-scope-heal-cwd-"));
...
describe("managed scope write resolver", () => {
	function fixture(): { cwd: string; agentDir: string; sessionsRoot: string } {
		const home = fs.mkdtempSync(path.join(os.tmpdir(), "skc-scope-resolver-home-"));
		temporaryDirectories.push(home);
		const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-scope-resolver-cwd-"));
		temporaryDirectories.push(cwd);
		const agentDir = path.join(home, "agent");
		const sessionsRoot = path.join(agentDir, "sessions");
		fs.mkdirSync(sessionsRoot, { recursive: true, mode: 0o700 });
