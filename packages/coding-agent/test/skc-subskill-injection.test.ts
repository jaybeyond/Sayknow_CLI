...
import { type LoadedSubskillActivation, loadSkcPlugin, toActiveSubskillEntry } from "../src/extensibility/skc-plugins";
...
async function tempProject(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-subskill-injection-"));
	tempRoots.push(cwd);
	await fs.mkdir(path.join(cwd, ".skc", "skc-plugins"), { recursive: true });
	await fs.cp(
		path.join(fixturesRoot, "valid-skill-plugin"),
		path.join(cwd, ".skc", "skc-plugins", "valid-skill-plugin"),
		{
			recursive: true,
		},
	);
...
async function activationFromFixture(cwd: string): Promise<LoadedSubskillActivation> {
	const plugin = await loadSkcPlugin(path.join(cwd, ".skc", "skc-plugins", "valid-skill-plugin"));
...
describe("SKC sub-skill prompt injection", () => {
	test("buildSkillPromptMessage appends matching active sub-skill block for the current phase", async () => {
		...
		expect(built.message).toContain(
			'<skc-subskill plugin="valid-skill-plugin" name="design" parent="ralplan" phase="planner" arg="design">',
		);
		expect(built.message).toContain(
		...
		expect(built.message.indexOf("Skill: /bundled/ralplan/SKILL.md")).toBeLessThan(
			built.message.indexOf("<skc-subskill"),
		);
	...
	test("phase mismatch does not append a persisted active sub-skill block", async () => {
		...
		expect(built.message).not.toContain("<skc-subskill");
