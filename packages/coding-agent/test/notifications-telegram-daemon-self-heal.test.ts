/**
 * Regression for https://github.com/jaybeyond/Sayknow_CLI/issues/2956
 *
...
function tempAgentDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-tg-selfheal-"));
...
describe("telegram daemon self-heal (#2956)", () => {
	...
	test("isNotificationLeakArtifactName matches quarantine prefixes only", () => {
		expect(isNotificationLeakArtifactName(".skc-delete-daemon-transition-abc.json")).toBe(true);
		expect(isNotificationLeakArtifactName(".skc-exact-unlink-placeholder-xyz")).toBe(true);
		expect(isNotificationLeakArtifactName(".skc-delete-notification-endpoint-1.json")).toBe(true);
	...
	test("pruneMissingNotificationRoots drops dead roots and keeps live ones", async () => {
		...
		const liveRoot = path.join(liveCwd, ".skc", "state");
		const deadRoot = path.join(deadCwd, ".skc", "state");
	...
	test("reapStaleNotificationArtifacts removes aged leak files and keeps fresh ones", async () => {
		...
		const aged = path.join(dir, ".skc-delete-daemon-transition-old.json");
		const fresh = path.join(dir, ".skc-exact-unlink-placeholder-fresh");
		...
		expect(result.removed.some(p => p.endsWith(".skc-delete-daemon-transition-old.json"))).toBe(true);
	...
	test("a transiently unreadable root is kept (not pruned) and does not block the live root", async () => {
		...
		const liveRoot = path.join(liveCwd, ".skc", "state");
		const flakyRoot = path.join(flakyCwd, ".skc", "state");
	...
	test("healTelegramDaemonNotificationState combines prune and reap", async () => {
		...
		const dir = daemonPaths(agentDir).dir;
		const leak = path.join(dir, ".skc-delete-daemon-transition-stale.json");
		...
		expect(result.removedArtifacts.some(p => p.endsWith(".skc-delete-daemon-transition-stale.json"))).toBe(true);
