...
function sessionLine(name = "sayknow_cli_test", branch = ""): string {
...
function injectSafeAbsentToSafeOwnerProof(plannedExecutions: string[][]): string[] {
	...
	tmuxSessions.__setCreateOwnerIsolationForTests({
		probe: {
			readCallerCgroup: () => "0::/skc-owner-test.scope\n",
...
describe("skc session command", () => {
	...
	it("emits JSON failure wrappers", async () => {
		...
		expect(payload).toEqual({ ok: false, reason: "skc_tmux_session_not_found" });
	});

	it("creates and reports a detached managed session as exact JSON DTO", async () => {
		const calls: string[][] = [];
		const stateRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-command-"));
		const previousSession = process.env.SKC_TMUX_SESSION;
		const previousStateFile = process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
		process.env.SKC_TMUX_SESSION = "custom_session";
		process.env.SKC_COORDINATOR_SESSION_STATE_FILE = path.join(stateRoot, "state.json");
		...
		try {
			...
			if (previousSession === undefined) delete process.env.SKC_TMUX_SESSION;
			else process.env.SKC_TMUX_SESSION = previousSession;
			if (previousStateFile === undefined) delete process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
			else process.env.SKC_COORDINATOR_SESSION_STATE_FILE = previousStateFile;
	...
	it("refuses unsafe or unverifiable owner servers before any tmux mutation", async () => {
		const stateRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-command-preflight-"));
		const previousSession = process.env.SKC_TMUX_SESSION;
		const previousStateFile = process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
		process.env.SKC_TMUX_SESSION = "preflight_session";
		process.env.SKC_COORDINATOR_SESSION_STATE_FILE = path.join(stateRoot, "state.json");
		try {
	...
	it("surfaces an untagged-session diagnostic with a detail hint in JSON failures", async () => {
		...
		expect(payload.ok).toBe(false);
		expect(payload.reason).toBe("skc_tmux_session_untagged");
	...
	it("awaits force-close before reporting JSON success", async () => {
		...
		mock.module("../src/skc-runtime/tmux-sessions", () => ({
			...REAL_TMUX_SESSIONS,
			forceCloseSkcTmuxSession: (...args: unknown[]) => {
				received.args = args;
	...
	it("preserves an asynchronous force-close error instead of reporting success", async () => {
		mock.module("../src/skc-runtime/tmux-sessions", () => ({
			...REAL_TMUX_SESSIONS,
			forceCloseSkcTmuxSession: async () => {
