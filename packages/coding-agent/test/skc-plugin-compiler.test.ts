...
describe("SKC plugin compiler", () => {
	test("compiles a valid six-surface bundle with extension ids and digests", async () => {
		const bundle = await compileSkcPluginBundle(sixSurface);
		...
		expect(bundle.files.some(f => f.relativePath === "sayknow-plugin.json")).toBe(true);
	...
	test("never imports plugin tool/hook code during compile", async () => {
		const sentinel = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "skc-sentinel-")), "sentinel.txt");
		...
		try {
			await compileSkcPluginBundle(sixSurface);
		} finally {
	...
	test("rejects a path that escapes the plugin root", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-escape-"));
		tempDirs.push(dir);
		await fs.writeFile(
	...
	test("rejects a declared sha256 that does not match file content", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-hash-"));
		tempDirs.push(dir);
	...
	test("rejects appendix with both path and content", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-appendix-"));
		tempDirs.push(dir);
		await fs.mkdir(path.join(dir, "prompts"), { recursive: true });
