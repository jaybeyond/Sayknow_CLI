// Entrypoint-level proof: `skc mcp-serve sdk` speaks JSON-RPC over stdio and its
...
try {
	...
	await mkdir(path.join(repo, ".skc", "state", "sdk"), { recursive: true });
	await writeFile(
		path.join(repo, ".skc", "state", "sdk", "s1.json"),
		JSON.stringify({ url: `ws://127.0.0.1:${server.port}`, token: "tok" }),
	);

	// Default: package source under test (CI monorepo with natives).
	// Override with SKC_MCP_E2E_BIN for local machines lacking matching natives.
	const explicitBin = process.env.SKC_MCP_E2E_BIN?.trim();
	const spawnCmd =
	...
	proc.stdin.write(
		`${JSON.stringify({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "skc_session_query", arguments: { sessionId: "s1", query: "session.metadata" } } })}\n`,
	);
	proc.stdin.write(
		`${JSON.stringify({ jsonrpc: "2.0", id: 4, method: "tools/call", params: { name: "skc_session_global", arguments: { operation: "session.get_endpoint" } } })}\n`,
	);
	...
	if (byId[1]?.result?.serverInfo?.name !== "skc-sdk-mcp") throw new Error(`initialize failed: ${out}`);
	if (!byId[2]?.result?.tools?.some((t: { name: string }) => t.name === "skc_session_query"))
