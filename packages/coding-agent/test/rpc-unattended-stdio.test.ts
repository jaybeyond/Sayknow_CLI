 ...
 * hand-built context. This spawns the REAL `skc --mode rpc` server so it
...
beforeEach(async () => {
	...
	const agentDir = path.join(workspace, ".skc", "agent");
...
async function driveRpcServer(commands: object[]): Promise<{ frames: Frame[]; raw: string; stderr: string }> {
	const proc = Bun.spawn(["bun", cliEntry, "--mode", "rpc"], {
		cwd: workspace,
		env: { ...cliEnv.env, SKC_HARNESS_STATE_ROOT: workspace },
		stdin: "pipe",
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("skc --mode rpc unattended control plane (stdio transport)", () => {
