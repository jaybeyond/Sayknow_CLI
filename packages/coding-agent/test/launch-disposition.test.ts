...
import { resolveLaunchDisposition } from "@sayknow-cli/coding-agent/cli/launch-disposition";
...
describe("non-TTY CLI startup", () => {
	it("exits instead of hanging when stdin is ignored and no prompt is provided", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-non-tty-startup-"));
		try {
			const result = await runCliWithIgnoredStdin(["--no-session"], {
				...process.env,
				SKC_CODING_AGENT_DIR: root,
				PI_CODING_AGENT_DIR: root,
				SKC_NOTIFICATIONS: "0",
				SKC_CLEANUP_DEADLINE_MS: "250",
				NO_COLOR: "1",
			});
	...
	it("routes a positional prompt without waiting for ignored stdin", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-non-tty-prompt-"));
		const env = { ...process.env };
		delete env.SKC_SDK_DISABLE;
		delete env.SKC_SESSION_ID;
		delete env.SKC_SESSION_FILE;
		delete env.SKC_SESSION_CWD;
		delete env.SKCCODE;
