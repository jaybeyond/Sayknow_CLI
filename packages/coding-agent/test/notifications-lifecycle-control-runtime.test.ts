...
function closeFrame(over: Partial<SessionCloseFrame> = {}): SessionCloseFrame {
	return {
		...
		chatId: PAIRED,
		token: "control-token",
		target: { sessionId: "sess_1", tmuxSession: "skc-sess_1" },
		force: true,
		...over,
	};
}
...
it("rejects a whitespace-only token before lifecycle, timer, or Bot API activity", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-daemon-whitespace-token-"));
	const settings = daemonSettings(agentDir);
...
it("does not resume daemon startup after stop during deferred lifecycle startup", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-daemon-lifecycle-stop-"));
	const settings = daemonSettings(agentDir);
...
it("does not restore running state after stop during deferred initial heartbeat renewal", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-daemon-renewal-stop-"));
	const settings = daemonSettings(agentDir);
	...
	const daemonFs: TelegramDaemonFs = {
		...
		exactUnlink: async (file, identity) =>
			await exactUnlinkNotificationFile(file, identity, ".skc-test-daemon-transition.json"),
	};
	const daemon = new TelegramNotificationDaemon({
		settings,
...
it("does not clean up foreign durable state when initial heartbeat renewal rejects ownership", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-daemon-foreign-renewal-"));
	const settings = daemonSettings(agentDir);
...
describe("lifecycle control runtime", () => {
	...
	it("distinguishes a missing ledger from corrupt or unreadable durable state", async () => {
		...
		try {
			...
			await expect(store.read()).rejects.toThrow("skc_lifecycle_ledger_read_failed:invalid");
			...
			await expect(fileLedgerStore(path.join(nonDirectory, "ledger.json")).read()).rejects.toThrow(
				"skc_lifecycle_ledger_read_failed:ENOTDIR",
			);
			...
			try {
				await expect(store.read()).rejects.toThrow("skc_lifecycle_ledger_read_failed:EACCES");
			} finally {
				readSpy.mockRestore();
			}
		} finally {
	...
	it("closes and never publishes a ledger temporary file after write or fsync failure", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "skc-lifecycle-ledger-temp-"));
		const ledgerPath = path.join(root, "ledger.json");
	...
	it("persists complete ledger JSON across short writes", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "skc-lifecycle-ledger-short-write-"));
		const ledgerPath = path.join(root, "ledger.json");
	...
	it("preserves the prior ledger and removes the temporary file when a write makes no progress", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "skc-lifecycle-ledger-zero-write-"));
		const ledgerPath = path.join(root, "ledger.json");
	...
	posixTmuxIt("daemonResumeSession fails closed against saved history (notFound / ambiguous)", async () => {
		const root = managedFixtureRoot("skc-resume-");
	...
	posixTmuxIt("daemonResumeSession cold-restarts saved sessions from their recorded cwd", async () => {
		const root = managedFixtureRoot("skc-resume-cwd-");
		...
		fs.writeFileSync(
			...
			[
				"#!/usr/bin/env bash",
				'printf \'%s\\n\' "$*" >> "$TMUX_CALLS"',
				'if [ "$3" = "display-message" ]; then printf \'$42\\tskc_lc_abc123\\n\'; exit 0; fi',
			...
			},
			{
				...
				ownerIsolationProbe: {
					readCallerCgroup: async () =>
						"0::/user.slice/user-1000.slice/user@1000.service/app.slice/skc-lifecycle-test.scope\n",
					probeServer: async () =>
						++probeCalls === 1
							? { state: "absent" }
							: {
		...
		expect(calls).toContain("new-session -d -P -F #{session_id} -s skc_lc_abc123 sh -c");
		expect(calls).toContain("SKC_TMUX_LAUNCHED='1' SKC_NOTIFICATIONS='1'");
		expect(calls).toContain("SKC_COORDINATOR_SESSION_ID='abc123'");
		expect(calls).toContain("SKC_TMUX_OWNER_GENERATION=");
		expect(calls).toContain("SKC_TMUX_OWNER_STATE_DIR=");
		...
		expect(calls.indexOf("new-session -d -P -F #{session_id} -s skc_lc_abc123 sh -c")).toBeGreaterThanOrEqual(0);
	...
	posixTmuxIt("daemonResumeSession rejects a live session when its tmux server cannot be proven safe", async () => {
		...
		fs.chmodSync(tmux, 0o755);
		const liveSession = tmuxStatus("skc_lc_live123", "live123");
		...
		).rejects.toThrow("skc_lifecycle_owner_server_unverifiable");
		fs.rmSync(root, { recursive: true, force: true });
	});
	...
	posixTmuxIt("daemonResumeSession rejects a live session when its target server is unsafe", async () => {
		...
		).rejects.toThrow("skc_lifecycle_owner_server_unsafe");
	});
	it("awaits daemon force-close before determining processGone", async () => {
		...
		const result = daemonCloseSession(process.env, {
			...
			findSession: () => {
				findCalls++;
				return tmuxStatus("skc_lc_close-1", "close-1");
			},
		})({ sessionId: "close-1", tmuxSession: "skc_lc_close-1" });
		await Bun.sleep(0);
	...
	posixTmuxIt("cleans the immutable spawned session after post-spawn generation proof fails", async () => {
		...
		const stateDir = path.join(project, ".skc", `_session-${sessionId}`, "runtime", "tmux-sessions");
		...
		fs.writeFileSync(
			...
			[
				"#!/usr/bin/env bash",
				'printf "%s\\n" "$*" >> "$TMUX_CALLS"',
				'if [ "$3" = "display-message" ]; then printf \'$42\\tskc_lc_stale-123\\n\'; exit 0; fi',
				'if [[ "$*" == *"__skc_lifecycle_cleanup_ok__"* ]]; then printf "__skc_lifecycle_cleanup_ok__\\n"; exit 0; fi',
				'if [ "$3" = "if-shell" ]; then printf "__skc_lifecycle_metadata_ok__\\n"; exit 0; fi',
		...
		try {
							...
							probeServer: async () =>
								++probeCalls === 1
									...
									: {
											...
											startTime: "1",
											cgroup: { classification: "safe", scope: "/skc-lifecycle-test.scope" },
											sessionNames: [`skc_lc_${sessionId}`],
										},
						},
					},
			...
			).rejects.toThrow("skc_lifecycle_owner_generation_changed");
	...
	posixTmuxIt("fails create when required tmux owner metadata cannot be written", async () => {
		...
		fs.writeFileSync(
			...
			[
				"#!/usr/bin/env bash",
				'if [ "$3" = "display-message" ]; then printf \'$42\\tskc_lc_metadata-123\\n\'; exit 0; fi',
		...
		await expect(
			daemonSpawnCreate(
				{ ...process.env, SKC_TMUX_COMMAND: tmux },
				{
					ownerIsolationProbe: {
						readCallerCgroup: async () => "/skc-lifecycle-test.scope\n",
						probeServer: async () =>
							++probeCalls === 1
								...
								: {
										...
										startTime: "1",
										cgroup: { classification: "safe", scope: "/skc-lifecycle-test.scope" },
										sessionNames: ["skc_lc_metadata-123"],
									},
					},
				},
			)(createFrame({ target: { kind: "existing_path", path: proj } }), {
	...
	posixTmuxIt(
		...
		async () => {
			const root = managedFixtureRoot("skc-pre-mutation-refusal-");
			const project = path.join(root, "project");
			const callsFile = path.join(root, "tmux-calls.log");
			const tmux = path.join(root, "fake-tmux.sh");
			fs.mkdirSync(project, { recursive: true });
	...
	posixTmuxIt(
		...
		async () => {
			const root = managedFixtureRoot("skc-cold-resume-metadata-");
			const project = path.join(root, "project");
			const callsFile = path.join(root, "tmux-calls.log");
			const tmux = path.join(root, "fake-tmux.sh");
			fs.mkdirSync(project, { recursive: true });
	...
	posixTmuxIt("refuses psmux before create or cold-resume can mutate lifecycle state", async () => {
		const root = managedFixtureRoot("skc-lifecycle-psmux-");
		const project = path.join(root, "project");
	...
	posixTmuxIt("combines required metadata failure with cleanup preproof or guarded-mutation uncertainty", async () => {
		...
		fs.writeFileSync(
			...
			[
				"#!/usr/bin/env bash",
				'printf "%s\\n" "$*" >> "$TMUX_CALLS"',
				'if [ "$3" = "display-message" ]; then printf \'$42\\tskc_lc_cleanup-123\\n\'; exit 0; fi',
				'if [ "$1" = "new-session" ]; then printf \'$42\\n\'; fi',
				'if [ "$1" = "set-option" ]; then exit 1; fi',
				'if [[ "$*" == *"__skc_lifecycle_cleanup_ok__"* ]] && [ "$KILL_FAIL" = "1" ]; then exit 1; fi',
				"exit 0",
				"",
			].join("\n"),
		);
							...
							ownerIsolationProbe: {
								...
								probeServer: async () => {
									...
									return {
										...
										startTime: "1",
										cgroup: { classification: "safe" as const },
										sessionNames: ["skc_lc_cleanup-123"],
									};
								},
							},
						},
	...
	posixTmuxIt("does not publish generation when the tmux server changes during metadata writes", async () => {
		...
		fs.writeFileSync(
			...
			[
				"#!/usr/bin/env bash",
				'if [ "$3" = "display-message" ]; then printf \'$42\\tskc_lc_metadata-race\\n\'; exit 0; fi',
				'if [ "$1" = "new-session" ]; then printf \'$42\\n\'; fi',
				"exit 0",
				"",
			].join("\n"),
						...
						ownerIsolationProbe: {
							...
							probeServer: async () => {
								...
								return {
									...
									startTime: "1",
									cgroup: { classification: "safe" as const },
									sessionNames: ["skc_lc_metadata-race"],
								};
							},
						},
					},
	...
	it("forwards exact daemon force-close provenance without defaulting a target", async () => {
		...
		const env = { TEST_OWNER: "private", SKC_TMUX_SESSION: "not-a-default" };
