...
import type { UsageLimit, UsageReport } from "@sayknow-cli/ai";
import { renderUsageReports } from "@sayknow-cli/coding-agent/modes/controllers/command-controller";
...
describe("usage report column ordering", () => {
	beforeAll(async () => {
		const loaded = await getThemeByName("red-octopus");
