...
import { CommandController } from "@sayknow-cli/coding-agent/modes/controllers/command-controller";
...
const originalAgentDirOverride = process.env.SKC_CODING_AGENT_DIR;
...
describe("/share temporary export security", () => {
	beforeAll(async () => {
		...
		if (!theme) throw new Error("Expected red-octopus theme");
	...
	afterEach(() => {
		...
		if (originalAgentDirOverride === undefined) delete process.env.SKC_CODING_AGENT_DIR;
		else process.env.SKC_CODING_AGENT_DIR = originalAgentDirOverride;
		process.env.PATH = originalPath;
	});

	it("uses exclusive owner-private staging and removes it after export failure", async () => {
		const tempDir = path.join(os.tmpdir(), "skc-share-random");
		...
		expect(mkdtemp).toHaveBeenCalledWith(path.join(os.tmpdir(), "skc-share-"));
	...
	it("does not replace the export error when cleanup fails", async () => {
		const tempDir = path.join(os.tmpdir(), "skc-share-cleanup-failure");
	...
	it("keeps a private custom-share export until the handler settles, then removes the directory", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-share-test-"));
...
`,
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-share-cancel-test-"));
