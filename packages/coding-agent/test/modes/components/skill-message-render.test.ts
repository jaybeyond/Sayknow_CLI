...
import { SkillMessageComponent } from "@sayknow-cli/coding-agent/modes/components/skill-message";
import * as themeModule from "@sayknow-cli/coding-agent/modes/theme/theme";
import {
	type CustomMessage,
	SKILL_PROMPT_MESSAGE_TYPE,
	type SkillPromptDetails,
} from "@sayknow-cli/coding-agent/session/messages";
...
const DETAILS: SkillPromptDetails = {
	name: "deep-interview",
	path: "embedded:skc/skills/deep-interview/SKILL.md",
	lineCount: 858,
};
...
describe("SkillMessageComponent rendering", () => {
	it("collapsed view shows `[skill] <name>` with readable args and no debug metadata", () => {
		...
		expect(out).not.toContain("embedded:skc/skills/deep-interview/SKILL.md");
	...
	it("collapsed view preserves multi-line args in a bounded preview", () => {
		const args = [
			...
			"make sure gaebal-sayknow skill-invocation modal does not truncate skill args,",
		...
		expect(out).toContain("make sure gaebal-sayknow skill-invocation modal");
	...
	it("expanded view reveals path, line count, args, and full prompt body", () => {
		...
		expect(out).toContain("embedded:skc/skills/deep-interview/SKILL.md");
