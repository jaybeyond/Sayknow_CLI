...
} from "@sayknow-cli/coding-agent/modes/components/settings-selector";
import { getPreset } from "@sayknow-cli/coding-agent/modes/components/status-line/presets";
	...
	const component = new SettingsSelectorComponent(
		{
			...
			availableThemes: ["red-octopus", "blue-octopus"],
			cwd: process.cwd(),
		},
		{
...
describe("SettingsSelectorComponent status line custom editor", () => {
	...
	it("refreshes the parent preview while editing and cancelling custom rows", () => {
		...
		for (let i = 0; i < 3; i++) component.handleInput("\x1b[B"); // Segment: sayknow.
		component.handleInput("\n"); // hidden -> left.
		expect(Bun.stripANSI(component.render(120).join("\n"))).toContain("left:path,git,sayknow");
		...
		expect(restored).not.toContain("left:path,git,sayknow");
	});
	...
	it("edits segment placement and typed options before saving", () => {
		...
		expect(settings.get("statusLine.leftSegments")).toEqual([...getPreset("minimal").leftSegments, "sayknow"]);
	});
	...
	it("persists approved custom settings across settings reload", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-status-line-settings-"));
		try {
			resetSettingsForTest();
