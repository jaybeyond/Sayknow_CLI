...
import {
	...
	summarizeSkcPluginObservability,
	writeRegistry,
...
async function mkCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-obs-"));
	tempDirs.push(cwd);
	return cwd;
}
...
describe("plugin observability summary", () => {
	test("lists every surface with stable extension ids and enabled status", async () => {
		...
		const summary = await summarizeSkcPluginObservability(cwd);
...
describe("observability quarantine keying", () => {
	...
	function entry(name: string): SkcPluginRegistryEntry {
		return {
			name,
			version: "1.0.0",
			scope: "project",
