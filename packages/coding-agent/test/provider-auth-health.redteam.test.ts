...
} from "@sayknow-cli/coding-agent/config/provider-auth-health";
import {
	compareRankedProviders,
	PROVIDER_RANK_TIER,
	providerRankTier,
	type RankableProvider,
} from "@sayknow-cli/coding-agent/config/provider-ranking";
...
import { OAuthSelectorComponent } from "@sayknow-cli/coding-agent/modes/components/oauth-selector";
import { getThemeByName, setThemeInstance } from "@sayknow-cli/coding-agent/modes/theme/theme";
import { AuthStorage, type AuthStorage as AuthStorageType } from "@sayknow-cli/coding-agent/session/auth-storage";
import type { TUI } from "@sayknow-cli/tui";

const model = (provider: string, id: string): Model =>
	({ provider, id, name: id, api: "openai-responses", contextWindow: 1000, maxTokens: 1000 }) as Model;

const PROBE_API_KEY_ENV = "SKC_REDTEAM_PROVIDER_AUTH_HEALTH_PROBE_KEY";
...
describe("hasConfiguredProviderAuth contract attacks", () => {
	test("agrees with hasConfiguredAuth for a configured provider that owns a model", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-provider-auth-health-"));
	...
	test("returns true for a keyless provider with no stored credentials", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-provider-auth-health-keyless-"));
		const modelsPath = path.join(root, "models.yml");
		const authStorage = trackAuthStorage(await AuthStorage.create(":memory:"));
		await Bun.write(
			modelsPath,
