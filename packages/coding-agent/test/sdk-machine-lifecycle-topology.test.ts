...
async function mcpGlobal(
	...
	const child = Bun.spawn([process.execPath, "run", cliEntrypoint, "mcp-serve", "sdk"], {
		cwd: repo,
		env: { ...(environment ?? process.env), SKC_CODING_AGENT_DIR: agentDir, SKC_AGENT_DIR: agentDir },
		stdin: "pipe",
		stdout: "pipe",
		stderr: "pipe",
	});
	child.stdin.write(
		`${JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "skc_session_global", arguments: { operation, input, idempotencyKey } } })}\n`,
	);
...
async function acpGlobal(
	...
	const child = Bun.spawn([process.execPath, cliEntrypoint, "--mode", "acp"], {
		cwd: repo,
		env: { ...process.env, SKC_CODING_AGENT_DIR: agentDir, SKC_AGENT_DIR: agentDir, PI_NO_TITLE: "1", NO_COLOR: "1" },
		stdin: "pipe",
		stdout: "pipe",
		stderr: "pipe",
	});
	...
	await readFrame();
	child.stdin.write(
		`${JSON.stringify({ jsonrpc: "2.0", id: 2, method: "_skc/sdk/global", params: { operation, input, idempotencyKey } })}\n`,
	);
	child.stdin.flush();
