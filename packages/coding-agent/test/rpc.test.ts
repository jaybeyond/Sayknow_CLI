...
import type { AgentEvent, AgentMessage } from "@sayknow-cli/agent-core";
import { type AssistantMessage, Effort, type TextContent } from "@sayknow-cli/ai";
import {
...
import type { BashExecutionMessage } from "@sayknow-cli/coding-agent/session/messages";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `skc-rpc-test-${Snowflake.next()}`);
