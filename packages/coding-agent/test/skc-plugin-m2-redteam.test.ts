...
import { installSkcPluginBundle, readRegistry, SkcPluginLoadError } from "../src/extensibility/skc-plugins";

const tempDirs: string[] = [];
const fixtureRoot = path.join(import.meta.dir, "fixtures", "skc-plugins", "valid-six-surface-bundle");
const fixtureFiles = [
	"sayknow-plugin.json",
...
async function mkProjectCwd(): Promise<string> {
	return await mkTempDir("skc-m2-redteam-project-");
}
...
async function writeTarball(entries: TarEntry[], options: { gzip?: boolean } = {}): Promise<string> {
	const dir = await mkTempDir("skc-m2-redteam-tar-");
...
async function makeBundleCopy(name: string, extra?: (dir: string) => Promise<void>): Promise<string> {
	const dir = await mkTempDir("skc-m2-redteam-bundle-");
	await fs.cp(fixtureRoot, dir, { recursive: true });
	const manifestPath = path.join(dir, "sayknow-plugin.json");
...
describe("SKC plugin installer M2 red-team", () => {
	test("rejects a tarball traversal entry and writes nothing outside extraction root", async () => {
		...
		await expect(installSkcPluginBundle(tarball, { scope: "project", cwd })).rejects.toMatchObject({
	...
	test("rejects a tarball whose root and first-level directories lack sayknow-plugin.json", async () => {
		const cwd = await mkProjectCwd();
	...
	test("installs a valid gzipped tarball whose bundle root is a subdirectory", async () => {
		...
		expect(
			await exists(path.join(cwd, ".skc", "skc-plugins", "valid-six-surface-bundle", "sayknow-plugin.json")),
		).toBe(true);
	});

	test("install of a forbidden-surface bundle leaves no scope files and no registry entry", async () => {
		const cwd = await mkProjectCwd();
		const bad = await mkTempDir("skc-m2-redteam-bad-");
		await fs.writeFile(
			path.join(bad, "sayknow-plugin.json"),
			JSON.stringify({ kind: "sayknow-cli-plugin", name: "forbidden-bundle", version: "1.0.0", commands: [] }),
		);

		await expect(installSkcPluginBundle(bad, { scope: "project", cwd })).rejects.toMatchObject({
			code: "forbidden_surface",
		});
		expect(await listEntries(path.join(cwd, ".skc", "skc-plugins"))).toEqual([]);
		expect(await readRegistry("project", cwd)).toMatchObject({ plugins: [] });
	});
	...
	test("reinstall identical content is unchanged, changed content conflicts, and force updates", async () => {
		...
		expect((await installSkcPluginBundle(original, { scope: "project", cwd })).status).toBe("installed");
		expect((await installSkcPluginBundle(original, { scope: "project", cwd })).status).toBe("unchanged");
		await expect(installSkcPluginBundle(modified, { scope: "project", cwd })).rejects.toMatchObject({
			code: "install_conflict",
		});
		expect((await installSkcPluginBundle(modified, { scope: "project", cwd, force: true })).status).toBe("updated");
		const installedPrompt = await fs.readFile(
			path.join(cwd, ".skc", "skc-plugins", "m2-reinstall-bundle", "prompts", "system-appendix.md"),
			"utf8",
		);
	...
	test("concurrent installs of the same bundle leave exactly one registry entry", async () => {
		...
		const results = await Promise.allSettled([
			installSkcPluginBundle(bundle, { scope: "project", cwd }),
			installSkcPluginBundle(bundle, { scope: "project", cwd }),
		]);
		for (const result of results) {
			if (result.status === "rejected" && result.reason instanceof SkcPluginLoadError) {
		...
		expect(await exists(path.join(cwd, ".skc", "skc-plugins", "m2-concurrent-bundle", "sayknow-plugin.json"))).toBe(
			true,
		);
	});
});
