...
import { harnessLeasesGcAdapter } from "@sayknow-cli/coding-agent/harness-control-plane/gc-adapter";
import { runSkcGcCommand } from "@sayknow-cli/coding-agent/skc-runtime/gc-runtime";
...
describe("skc gc end-to-end (harness lease adapter)", () => {
	test("dry-run reports a dead lease as would_remove and deletes nothing", async () => {
		...
		const env = { ...process.env, SKC_CODING_AGENT_DIR: base, SKC_HARNESS_ROOT_REGISTRY_DIR: registryDir };

		const result = await runSkcGcCommand(["--json"], base, env, [harnessLeasesGcAdapter]);
	...
	test("--prune removes the dead lease and exits 0", async () => {
		...
		const result = await runSkcGcCommand(["--prune", "--json"], base, env, [harnessLeasesGcAdapter]);
		const report = JSON.parse(result.stdout);
	...
	test("text dry-run output names the harness store and dry-run mode", async () => {
		...
		const env = {
			...process.env,
			SKC_CODING_AGENT_DIR: base,
			SKC_HARNESS_ROOT_REGISTRY_DIR: path.join(base, "reg-empty"),
		};
		const result = await runSkcGcCommand([], base, env, [harnessLeasesGcAdapter]);
	...
	test("explicit repair quarantines corruption and is idempotent", async () => {
		...
		const repairEnv = { ...process.env, SKC_CODING_AGENT_DIR: agentDir };
		const repaired = await runSkcGcCommand(["--repair-session-index", "--json"], base, repairEnv, [
		...
		const retry = await runSkcGcCommand(["--repair-session-index", "--json"], base, repairEnv, [
			harnessLeasesGcAdapter,
		]);
