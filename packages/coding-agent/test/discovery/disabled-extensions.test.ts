...
import { type ContextFile, contextFileCapability } from "@sayknow-cli/coding-agent/capability/context-file";
import { resetSettingsForTest, Settings } from "@sayknow-cli/coding-agent/config/settings";
import { initializeWithSettings, loadCapability } from "@sayknow-cli/coding-agent/discovery";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-disabled-ext-home-"));
		process.env.HOME = tempHomeDir;
		vi.spyOn(os, "homedir").mockReturnValue(tempHomeDir);
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".skc"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".skc", "AGENTS.md"), "# project instructions\n");
