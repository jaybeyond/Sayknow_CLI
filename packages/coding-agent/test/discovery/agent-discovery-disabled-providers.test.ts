 ...
 * discoverAgents() must skip SKC plugin roots when the plugin provider is disabled.
 */
...
const PLUGIN_AGENT_MD = [
	"---",
	"name: simplifier",
	"description: A code simplifier agent from a SKC plugin",
	"---",
	"Simplify code.",
].join("\n");
...
describe("discoverAgents — claude-plugins disabled provider", () => {
	...
	beforeEach(() => {
		...
		// Build a fake SKC plugin install with an agents/ subdirectory.
		...
		// Register the plugin in the SKC registry so listAnthropic modelPluginRoots picks it up.
		const skcPluginsDir = path.join(tempHome, ".skc", "plugins");
		fs.mkdirSync(skcPluginsDir, { recursive: true });
		fs.writeFileSync(
			path.join(skcPluginsDir, "installed_plugins.json"),
			JSON.stringify({
				version: 2,
				plugins: {
