...
import { loadCapability } from "@sayknow-cli/coding-agent/discovery";
...
async function canCreateFileSymlink(): Promise<boolean> {
	const probeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-symlink-probe-"));
...
async function canCreateDirectoryLink(): Promise<boolean> {
	const probeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-directory-link-probe-"));
...
async function canCreateHardLink(): Promise<boolean> {
	const probeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-hard-link-probe-"));
	const targetPath = path.join(probeDir, "target.json");
...
describe("standalone mcp.json oauth env expansion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-json-"));
...
describe("explicit MCP JSON exact-file trust", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "skc-mcp-exact-file-")));
	});
