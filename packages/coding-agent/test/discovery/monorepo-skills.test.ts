...
import { clearCache } from "@sayknow-cli/coding-agent/capability/fs";
import type { Skill } from "@sayknow-cli/coding-agent/capability/skill";
import type { LoadContext, LoadResult } from "@sayknow-cli/coding-agent/capability/types";
import { scanSkillsFromDir } from "@sayknow-cli/coding-agent/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	beforeEach(() => {
		clearCache();
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-monorepo-skills-"));
	...
	test("finds skills in ancestor .skc/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".skc", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".skc", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".skc", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".skc", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".skc", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".skc", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".skc", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".skc", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".skc", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".skc", "skills"), "root-skill", "At repo root");
