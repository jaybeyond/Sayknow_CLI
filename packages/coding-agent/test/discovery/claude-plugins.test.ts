...
import { loadCapability } from "@sayknow-cli/coding-agent/capability";
import { clearCache as clearFsCache } from "@sayknow-cli/coding-agent/capability/fs";
import {
	clearClaudePluginRootsCache,
	listClaudePluginRoots,
	parseClaudePluginsRegistry,
} from "@sayknow-cli/coding-agent/discovery/helpers";
import { discoverAgents } from "@sayknow-cli/coding-agent/task/discovery";
import "@sayknow-cli/coding-agent/discovery/claude-plugins";
import type { Skill } from "@sayknow-cli/coding-agent/capability/skill";
import type { SlashCommand } from "@sayknow-cli/coding-agent/capability/slash-command";
...
describe("listClaudePluginRoots", () => {
	...
	beforeEach(async () => {
		...
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugins-test-"));
	...
	test("parses plugin with user scope", async () => {
		const pluginsDir = path.join(tempDir, ".skc", "plugins");
...
describe("discoverAgents plugin precedence", () => {
	...
	beforeEach(async () => {
		clearClaudePluginRootsCache();
		clearFsCache();
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugins-precedence-test-"));
	});
	...
	test("prefers project-scoped plugin agent over user-scoped plugin agent", async () => {
		const pluginRegistryDir = path.join(tempDir, ".skc", "plugins");
