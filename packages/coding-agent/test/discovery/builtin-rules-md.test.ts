 ...
 * from both `~/.skc/agent/RULES.md` (user) and the nearest `.skc/RULES.md`
...
import { getCapability } from "@sayknow-cli/coding-agent/capability";
import { clearCache } from "@sayknow-cli/coding-agent/capability/fs";
import { type Rule, ruleCapability } from "@sayknow-cli/coding-agent/capability/rule";
import type { LoadContext } from "@sayknow-cli/coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@sayknow-cli/coding-agent/discovery";
...
beforeEach(() => {
	clearCache();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-rules-md-"));
...
test("user ~/.skc/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".skc", "agent", "RULES.md"),
...
test("project .skc/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".skc", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".skc", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".skc", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".skc", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .skc/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".skc", "agent", "rules", "other.md"), "# Unrelated rule\n");
