...
import {
	...
	type SessionEvent,
} from "@sayknow-cli/coding-agent/extensibility/hooks";
import { theme } from "@sayknow-cli/coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `skc-compaction-hooks-test-${Snowflake.next()}`);
