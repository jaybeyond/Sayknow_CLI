...
async function tempProjectWithFixture(fixtureName: string): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugin-activation-"));
	tempRoots.push(cwd);
	await fs.mkdir(path.join(cwd, ".skc", "skc-plugins"), { recursive: true });
	await fs.cp(path.join(fixturesRoot, fixtureName), path.join(cwd, ".skc", "skc-plugins", fixtureName), {
		recursive: true,
	});
	return cwd;
}
...
describe("SKC sub-skill activation dispatch", () => {
	...
	test("rejects duplicate activation args across plugin roots instead of choosing a winner", async () => {
		...
		await expect(
			...
			constructor: SkcPluginLoadError,
			code: "duplicate_arg",
		});
	});
