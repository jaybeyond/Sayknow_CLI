...
test("shipped MCP stdio advertises confirm and forwards confirmed destructive controls", async () => {
	const repo = await mkdtemp(path.join(tmpdir(), "skc-sdk-mcp-stdio-"));
	...
	try {
		...
		await writeFile(
			path.join(repo, ".skc", "state", "sdk", `${sessionId}.json`),
		...
		const control = toolList.tools?.find(tool => tool.name === "skc_session_control");
