...
import { exportFromFile } from "@sayknow-cli/coding-agent/export/html";
import {
	type ColdSpillRef,
	SessionManager,
	type SessionMessageEntry,
} from "@sayknow-cli/coding-agent/session/session-manager";
import { getBlobsDir, Snowflake } from "@sayknow-cli/utils";
...
describe("session HTML export fidelity", () => {
	it("exports rehydrated pre-compaction content instead of tombstone notices", async () => {
		const tempDir = path.join(os.tmpdir(), `skc-html-export-fidelity-${Snowflake.next()}`);
	...
	it("renders an explicit unavailable notice when a cold-spill blob is missing", async () => {
		const tempDir = path.join(os.tmpdir(), `skc-html-export-missing-blob-${Snowflake.next()}`);
		fs.mkdirSync(tempDir, { recursive: true });
		try {
