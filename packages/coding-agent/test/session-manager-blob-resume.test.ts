...
function makeTempDir(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-blob-resume-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("SessionManager image blob resume", () => {
	it("keeps resumed image-heavy canonical entries resident instead of loading base64 strings", async () => {
		...
		try {
			...
			for (const entry of entries) {
				...
				expect(serializedCanonical).toContain("__skcResidentBlob");
	...
	it("uses the resident missing-blob placeholder for resumed images and never leaks blob refs", async () => {
		...
		try {
			...
			expect(entriesSerialized).not.toContain("__skcResidentBlob");
			...
			expect(contextSerialized).not.toContain("__skcResidentBlob");
		} finally {
			await reopened.close();
		}
	});
