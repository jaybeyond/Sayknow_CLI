...
const KEYS = ["SKC_COMMIT_MAP_REDUCE", "PI_COMMIT_MAP_REDUCE"] as const;
...
// SKC_COMMIT_MAP_REDUCE / PI_COMMIT_MAP_REDUCE gate.
...
describe("shouldUseMapReduce env gating", () => {
	...
	it("honors the documented SKC_COMMIT_MAP_REDUCE=false", () => {
		clear();
		Bun.env.SKC_COMMIT_MAP_REDUCE = "false";
	...
	it("resolves SKC-first: SKC=true is not overridden by PI=false", () => {
		clear();
		Bun.env.SKC_COMMIT_MAP_REDUCE = "true";
		Bun.env.PI_COMMIT_MAP_REDUCE = "false";
		expect(shouldUseMapReduce("", forceEligible)).toBe(true);
	});
});
