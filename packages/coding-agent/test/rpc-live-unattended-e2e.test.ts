 ...
 * spawned `skc --mode rpc-ui` server, actually calls the ask tool which emits a
	...
	it(
		...
		async () => {
			...
			const client = new RpcClient({
				...
				args: ["--mode", "rpc-ui"],
				env: { ...cliEnv.env, ANTHROPIC_API_KEY: key as string, SKC_HARNESS_STATE_ROOT: workspace },
			});
