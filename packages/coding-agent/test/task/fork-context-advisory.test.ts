...
describe("fork context advisory", () => {
	...
	it.each([
		...
		[".skc/plans/fork-context.md", "prior-session-reference:skc-plans-path"],
		[".skc/specs/fork-context.md", "prior-session-reference:skc-specs-path"],
	...
	it("does not recommend bounded or full from heuristic triggers", () => {
		const advisory = adviseForkContextMode({
			assignment: "As discussed, see above and use .skc/plans/context.md from the previous review.",
		});
	...
	it("does not mutate the input object", () => {
		const input = {
			...
			context: "Use .skc/specs/context.md.",
