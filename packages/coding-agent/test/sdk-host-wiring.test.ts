...
afterEach(async () => {
	...
	delete process.env.SKC_SDK_DISABLE;
	delete process.env.SKC_NOTIFICATIONS;
	delete process.env.SKC_LIFECYCLE_TEST_TOKEN;
	delete process.env.SKC_LIFECYCLE_TEST_SECRET;
	delete process.env.SKC_LIFECYCLE_TEST_API_KEY;
});
...
test("lifecycle startup production secret collection redacts before normalization and truncation", () => {
	...
	const nfkc = "secret０";
	const names = ["SKC_LIFECYCLE_TEST_TOKEN", "SKC_LIFECYCLE_TEST_SECRET", "SKC_LIFECYCLE_TEST_API_KEY"] as const;
	const previous = names.map(name => process.env[name]);
	try {
		process.env.SKC_LIFECYCLE_TEST_TOKEN = bare;
		process.env.SKC_LIFECYCLE_TEST_SECRET = overlap;
		process.env.SKC_LIFECYCLE_TEST_API_KEY = nfkc;
...
test("lifecycle teardown swallows dual owner failures without surfacing an extension error and retains exact retry authority", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-lifecycle-cleanup-proof-"));
	dirs.push(cwd);
...
test("lifecycle cleanup fences same-id startup and preserves proven owner release across retry", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-lifecycle-cleanup-retry-"));
	dirs.push(cwd);
...
test("production SDK host starts exactly one instrumented server (no duplicate auto-host)", async () => {
	...
	try {
		...
		const sdkDir = path.join(cwd, ".skc", "state", "sdk");
...
test("lifecycle session shutdown disposes the exact endpoint once", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-lifecycle-once-"));
	dirs.push(cwd);
...
test("lifecycle startup settles failure when native callback registration throws before host start", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prestart-failure-"));
	dirs.push(cwd);
...
test("lifecycle startup reports an actionable error when native capability registration is missing", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-missing-capability-callback-"));
	dirs.push(cwd);
	const capability = new SdkStartupCapability();
	...
	try {
		...
		expect(fs.existsSync(path.join(cwd, ".skc", "state", "sdk", "missing-capability-callback.json"))).toBe(false);
	} finally {
...
test("lifecycle startup settles native capability incompatibility before constructing the host", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-native-incompatible-"));
	dirs.push(cwd);
	const capability = new SdkStartupCapability();
	...
	try {
		...
		expect(fs.existsSync(path.join(cwd, ".skc", "state", "sdk", "native-incompatible.json"))).toBe(false);
	} finally {
...
test("SDK broker registration records an absolute lifecycle scope", async () => {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-locator-"));
...
test("startup records identity before an early lifecycle event and publishes it only after NotificationServer starts", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-identity-startup-"));
	dirs.push(cwd);
...
test("concurrent /notify on waits for startup before activating notification answers", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-notify-startup-"));
	dirs.push(cwd);
...
test("/notify on refuses a startup result for a rotated runtime identity", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-notify-rotation-"));
	dirs.push(cwd);
...
test("/notify on fences teardown and permits a later same-ID replacement runtime", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-notify-teardown-"));
	dirs.push(cwd);
...
test("SDK host replays file attachment data as base64 while passing raw bytes to N-API", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-file-replay-"));
	dirs.push(cwd);
...
test("SDK host replays event frames over direct v3 ingress and routes queries through the v2 control-command seam", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-"));
	dirs.push(cwd);
...
test("SDK host preserves ordered prompt image blocks in the host payload", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-images-"));
	dirs.push(cwd);
...
test("SDK host correlates follow-up acknowledgements with the later agent start", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-follow-up-correlation-"));
	dirs.push(cwd);
...
test("SDK host directly delivers correlated lifecycle frames for an accepted prompt", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-success-"));
	dirs.push(cwd);
...
test("SDK host buffers synchronous pre-ack start and end until after acknowledgement", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-pre-ack-end-"));
	dirs.push(cwd);
...
test("SDK host buffers synchronous pre-ack accepted failure until after acknowledgement", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-pre-ack-failed-"));
	dirs.push(cwd);
...
test("SDK host replays an accepted prompt terminal after its requester disconnects", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-disconnect-replay-"));
	dirs.push(cwd);
...
test("SDK host serializes concurrent prompt admission and replays correlated lifecycle", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-concurrent-"));
	dirs.push(cwd);
...
test("SDK host delivers accepted prompt failures after their acknowledgement", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-terminal-"));
	dirs.push(cwd);
...
test("SDK host terminalizes a cancelled preflight and releases prompt authority", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-preflight-cancelled-"));
	dirs.push(cwd);
...
test("SDK host terminalizes a never-resolving preflight on abort and fences late acceptance", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-prompt-preflight-never-"));
	dirs.push(cwd);
...
test("SDK host abort-and-prompt cancels a never-resolving preflight before replacement submission", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-abort-prompt-never-preflight-"));
	dirs.push(cwd);
...
test("SDK host waits for asynchronous abort unwind before delivering an abort-and-prompt replacement", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-abort-prompt-"));
	dirs.push(cwd);
...
test("SDK session switches rotate endpoint authority before publishing the replacement host", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-switch-"));
	dirs.push(cwd);
...
test("SDK host binds session query and control seams and excludes uninstalled resources", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-bindings-"));
	dirs.push(cwd);
...
test("SDK host routes pure ACP permission prompts through a live reverse provider", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-permission-provider-"));
	dirs.push(cwd);
...
test("rejects malformed provider definitions without replacing a valid tools registry", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-provider-validation-"));
	dirs.push(cwd);
...
test("Q17 returns resource_gone without an assistant and reads a completed persisted turn after reopen", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-last-assistant-"));
	dirs.push(cwd);
...
test("terminal shutdown removes session snapshot spills", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-snapshots-"));
	dirs.push(cwd);
	...
	const snapshotDirectory = path.join(cwd, ".skc", "state", "sdk", "snapshots", sessionId);
...
test("diff queries return typed errors outside a Git working tree", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-no-git-"));
	dirs.push(cwd);
...
test("diff queries return a bounded error for oversized diffs", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-large-diff-"));
	dirs.push(cwd);
	for (const args of [
		["init", "-q"],
...
test("SDK host honors disable opt-out and excludes subagent sessions", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-gate-"));
	dirs.push(cwd);
	process.env.SKC_SDK_DISABLE = "1";
	start(context(cwd, "disabled"));
	await Bun.sleep(100);
	expect(fs.existsSync(path.join(cwd, ".skc", "state", "sdk", "disabled.json"))).toBe(false);
	delete process.env.SKC_SDK_DISABLE;
	start(context(cwd, "subagent", "sub"));
	await Bun.sleep(100);
	expect(fs.existsSync(path.join(cwd, ".skc", "state", "sdk", "subagent.json"))).toBe(false);
});

test("context.get reports live streaming state and typed queue depths without notifications", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-live-"));
	dirs.push(cwd);
...
test("SDK endpoint applies typed skill, plan, goal, and config controls with observable readback", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-typed-controls-"));
	dirs.push(cwd);
...
test("Q12 records the runtime-turn correlation before a workflow gate is exposed", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-q12-runtime-turn-"));
	dirs.push(cwd);
...
test("SDK host discovers, answers, and advances a durable workflow gate", async () => {
	...
	const gateStore = new FileGateStore(path.join(cwd, ".skc", "state", "workflow-gates.json"));
...
test("session teardown drains admitted direct gate resolution before detaching its controller", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-direct-resolution-drain-"));
	dirs.push(cwd);
...
test("SDK host omits direct workflow controls for a legacy workflow-gate emitter", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-legacy-workflow-gate-"));
	dirs.push(cwd);
...
test("AC2/AC8: SDK host completes successful session mutations over its live WebSocket", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-host-successful-verbs-"));
	dirs.push(cwd);
	const sessionId = `successful-verbs-${Date.now()}`;
	const emitter = new BrokerWorkflowGateEmitter(
		sessionId,
		new FileGateStore(path.join(cwd, ".skc", "state", "workflow-gates.json")),
	);
