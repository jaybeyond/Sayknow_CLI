...
test("ACP registers a permission provider only for prompt handling", () => {
	expect(providerNames({ _meta: { skc: { permissionHandling: "prompt" } } })).toContain("permission");
	expect(providerNames({ _meta: { skc: { permissionHandling: "auto" } } })).not.toContain("permission");
	expect(providerNames({ _meta: { skc: { permissionHandling: "always-allow" } } })).not.toContain("permission");
	expect(providerNames(undefined, { SKC_ACP_PERMISSION_MODE: "prompt" })).toContain("permission");
	expect(providerNames(undefined, { SKC_ACP_PERMISSION_MODE: "auto" })).not.toContain("permission");
	expect(providerNames({ _meta: { skc: { permissionHandling: "invalid" } } })).toContain("permission");
});
...
test("ACP maps non-prompt permission handling to the SDK allow policy", async () => {
	...
	await applyAcpPermissionMode(adapter, { _meta: { skc: { permissionHandling: "prompt" } } } as never);
	await applyAcpPermissionMode(adapter, { _meta: { skc: { permissionHandling: "auto" } } } as never);
	await applyAcpPermissionMode(adapter, { _meta: { skc: { permissionHandling: "always-allow" } } } as never);
...
test("ACP rejects --mcp-config instead of ignoring it", () => {
	const parsed = parseArgs(["--mcp-config", "/tmp/skc-mcp.json"]);
...
test("ACP preserves --models rejection alongside --mcp-config", () => {
	...
	const both = parseArgs(["--models", "openai-codex/gpt-5.6", "--mcp-config", "/tmp/skc-mcp.json"]);
