...
import { Agent, type AgentEvent, type AgentToolContext } from "@sayknow-cli/agent-core";
...
import * as skillState from "@sayknow-cli/coding-agent/hooks/skill-state";
import { ensureWorkflowSkillActivationState } from "@sayknow-cli/coding-agent/hooks/skill-state";
...
describe("AgentSession deep-interview continuation", () => {
	...
	it("continues when the model stops during an active interview", async () => {
		...
		expect(reminder).toContain("stop gate: skc_skill_deep_interview_");
	...
	it("atomically commits only two overlapping ordinary-stop reservations", async () => {
		...
		vi.spyOn(skillState, "buildSkillStopOutput").mockImplementation(async () => {
			...
			await gate.promise;
			return { decision: "block", stopReason: "skc_skill_deep_interview_interviewing" };
		});
