...
import { forceCloseSkcTmuxSession } from "@sayknow-cli/coding-agent/skc-runtime/tmux-sessions";
import { createCoordinatorMcpServer } from "../../src/coordinator-mcp/server";

// BLOCKER 1 (#2044): prove the reaper terminates a *genuine* SKC-managed owned session by driving
// the reaper's real path through the coordinator's stop_session tool. The reaper's injectable
// services.forceCloseSession forwards to the *real* forceCloseSkcTmuxSession with the same
...
// trivial always-success double. An unrelated SKC session (no coordinator record) must survive.
//
...
describe.skipIf(!isLinux || !tmux || !userScopeAvailable)("reaper owner-proof termination integration", () => {
	afterEach(async () => {
		for (const c of cleanups.splice(0)) {
			Bun.spawnSync([c.env.SKC_TMUX_COMMAND!, "kill-server"], { stdout: "pipe", stderr: "pipe", env: c.env });
			Bun.spawnSync(["systemctl", "--user", "stop", c.scopeName], { stdout: "pipe", stderr: "pipe" });
			await fs.rm(c.stateDir, { recursive: true, force: true });
		}
	});

	it("stop_session drives the real forceClose to terminate a genuine owned session and purge it; an unrelated SKC session survives", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-reaper-owner-"));
		const tmuxTmpDir = path.join(stateDir, "tmux");
		const socketName = `skc-reap-${crypto.randomUUID().slice(0, 8)}`;
		const scopeName = `skc-reaper-test-${crypto.randomUUID().slice(0, 8)}.scope`;
		const tmuxWrapper = path.join(stateDir, "isolated-tmux");
		const sessionName = `skc_reap_${crypto.randomUUID().slice(0, 8)}`;
		...
		// An unrelated SKC session with no coordinator record — must be left untouched.
		run(
		...
		// The reaper forwards through services.forceCloseSession to the REAL forceCloseSkcTmuxSession
		...
		const server = createCoordinatorMcpServer({
			env: {
				...env,
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: stateDir,
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions,questions,reports",
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(stateDir, ".coord"),
				SKC_COORDINATOR_MCP_PROFILE: "reaper-owner",
				SKC_COORDINATOR_MCP_REPO: "repo-owner",
			},
			services: {
				forceCloseSession: async (name, closeEnv, _sid, expectedStateFile) => {
					...
					return await forceCloseSkcTmuxSession(name, closeEnv, sessionId, expectedStateFile, {
						resolveOwner: async () => ({
							sessionId,
							stateDir,
		...
		expect(hasSession(siblingSessionName)).toBe(true); // unrelated SKC session survives
		const status = await server.callTool("skc_coordinator_read_status", { session_id: "owned-eph" });
