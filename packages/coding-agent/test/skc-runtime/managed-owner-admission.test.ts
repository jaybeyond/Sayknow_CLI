...
async function admit(stateDir: string, token?: string): Promise<{ admitted: boolean; exitCode: number; root: string }> {
	...
	const child = Bun.spawn({
		...
		env: {
			...
			SKC_COORDINATOR_SESSION_ID: "session-2681",
			SKC_TMUX_OWNER_GENERATION: "generation-2681",
			SKC_MANAGED_OWNER_RUN_ID: "run-2681",
			SKC_MANAGED_OWNER_INCARNATION: "incarnation-2681",
			...(token ? { SKC_MANAGED_OWNER_CHILD_TOKEN: token } : {}),
		},
	});
...
async function recover(
	...
	const child = Bun.spawn({
		...
		env: {
			...
			SKC_TMUX_OWNER_STATE_DIR: stateDir,
			SKC_COORDINATOR_SESSION_ID: "session-2681",
			SKC_TMUX_OWNER_GENERATION: "replacement-generation-2681",
			SKC_MANAGED_OWNER_RUN_ID: "replacement-run-2681",
			SKC_MANAGED_OWNER_INCARNATION: "replacement-incarnation-2681",
			SKC_MANAGED_OWNER_CHILD_TOKEN: "replacement-child-token",
			SKC_MANAGED_OWNER_PREDECESSOR_TOKEN: token,
			SKC_MANAGED_OWNER_PREDECESSOR_GENERATION: "generation-2681",
			SKC_MANAGED_OWNER_PREDECESSOR_RUN_ID: "run-2681",
			SKC_MANAGED_OWNER_PREDECESSOR_INCARNATION: "incarnation-2681",
			SKC_MANAGED_OWNER_TRANSCRIPT_PATH: transcriptPath,
		},
	});
	const [stdout, exitCode] = await Promise.all([new Response(child.stdout).text(), child.exited]);
	...
	await fs.writeFile(
		...
		`${JSON.stringify({
			...
			command_sha256: crypto
				.createHash("sha256")
				.update(JSON.stringify(["skc", "--resume"]))
				.digest("hex"),
			supervisor_pid: 1,
...
describe("managed owner admission", () => {
	it("admits only the exact token binding for the current session and generation", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-managed-admission-"));
		try {
	...
	it("turns a recovery admission into a durable terminal handoff without changing B0 or dirty files", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-managed-recovery-"));
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-managed-recovery-cwd-"));
		try {
			const root = lifecyclePaths(stateDir, "session-2681", "generation-2681").root;
