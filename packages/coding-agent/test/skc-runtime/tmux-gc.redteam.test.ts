...
describe("tmux GC red-team adversarial safety", () => {
	...
	it("revalidates before pruning and skips kill when a terminal-marker candidate becomes attached after classification", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({
				...
				workdir: "/tmp/skc-redteam-deleted-project",
				session_file: null,
			}),
		);
		const calls: string[][] = [];
		...
		spawnSyncSpy.mockImplementation((cmd: string[]) => {
			...
			if (optionValue(cmd, "@skc-owner-generation")) return spawnResult(0, "generation-1\n");
	...
	it("refuses a same-name replacement with a different native session identity", async () => {
		const stateFile = "/tmp/skc-redteam-native-identity-marker.json";
		await Bun.write(
			stateFile,
			JSON.stringify({
				schema_version: 1,
		...
		spawnSyncSpy.mockImplementation((cmd: string[]) => {
			...
			if (cmd.includes("list-sessions")) {
				const format = cmd[cmd.indexOf("-F") + 1] ?? "";
				if (format === "#{session_name}") return spawnResult(0, "sayknow_cli_reused_name\n");
				richListCount += 1;
				return spawnResult(
					0,
					sessionLine({
						name: "sayknow_cli_reused_name",
						branch: "deleted-branch",
						project: "/tmp/skc-redteam-deleted-project",
			...
			if (optionValue(cmd, "@skc-project")) return spawnResult(0, "/tmp/skc-redteam-deleted-project\n");
			if (optionValue(cmd, "@skc-branch")) return spawnResult(0, "deleted-branch\n");
			if (optionValue(cmd, "@skc-session-id"))
				return spawnResult(0, richListCount === 1 ? "collected-session\n" : "replacement-session\n");
			if (optionValue(cmd, "@skc-session-state-file")) return spawnResult(0, `${stateFile}\n`);
			if (optionValue(cmd, "@skc-owner-generation"))
				return spawnResult(0, richListCount === 1 ? "generation-one\n" : "generation-two\n");
			return spawnResult(0, "");
		});
		try {
			const result = await tmuxSessionsGcAdapter.collect(ctx());
			const record = result.records.find(entry => entry.id === "sayknow_cli_reused_name");
