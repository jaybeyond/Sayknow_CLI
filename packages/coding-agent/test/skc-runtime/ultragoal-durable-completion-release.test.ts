...
import { computeUltragoalPlanGeneration } from "@sayknow-cli/coding-agent/skc-runtime/ultragoal-receipt-freshness";
import {
	addUltragoalSubgoal,
...
async function createTwoGoalPlan(root: string, mode: "aggregate" | "per-story" = "aggregate"): Promise<void> {
	await createUltragoalPlan({ cwd: root, brief: "Ship the feature", skcGoalMode: mode });
...
function reviewedBatchReplacementFixture(options: { multiple?: boolean; staleReplacement?: boolean } = {}): {
	...
	const plan: UltragoalPlan = {
		...
		skcObjective: "Complete reviewed batch",
