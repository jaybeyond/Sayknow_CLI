...
// Tests in this file assume root-scoped `.skc/state` paths. The state runtime
// falls back to the `SKC_SESSION_ID` env var when no `--session-id` flag is
// provided, which would route writes into `.skc/state/sessions/<id>/` and
// break these root-scoped assertions when run inside a shell session that
// exports `SKC_SESSION_ID` (e.g. the coding-agent dev loop). Clear and
...
describe("native skc state runtime", () => {
	...
	it("prefers CLI --mode over --input payload mode", async () => {
		...
		const ralplanFile = path.join(root, ".skc", "state", "ralplan-state.json");
	...
	it("clear flips active:false and removes the entry from skill-active-state", async () => {
		const root = await tempDir();
		const activeStateDir = path.join(root, ".skc", "state");
	...
	it("infers the active workflow for write when --mode/positional/input.skill are absent", async () => {
		...
		// Bundled prompt shape: skc state write --input '<json>' (no --mode)
		const result = await runNativeStateCommand(
