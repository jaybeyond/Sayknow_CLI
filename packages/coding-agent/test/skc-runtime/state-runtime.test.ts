...
import { deepInterviewCharacterCount } from "@sayknow-cli/coding-agent/skc-runtime/deep-interview-state";
import {
	activeSnapshotPath,
	modeStatePath,
	sessionStateDir,
...
describe("native skc state runtime", () => {
	...
	it("rejects write when no session id is resolvable", async () => {
		...
		try {
			...
			else process.env.SKC_SESSION_ID = TEST_SESSION_ID;
		}
	});
	...
	it("errors read/status when no session directories exist", async () => {
		...
		try {
			...
			expect(read.stderr).toContain("no active SKC session found");
			const status = await runNativeStateCommand(["status", "--mode", "deep-interview", "--json"], root);
			expect(status.status).toBe(2);
			expect(status.stderr).toContain("no active SKC session found");
		} finally {
			if (prior !== undefined) process.env.SKC_SESSION_ID = prior;
			else process.env.SKC_SESSION_ID = TEST_SESSION_ID;
		}
