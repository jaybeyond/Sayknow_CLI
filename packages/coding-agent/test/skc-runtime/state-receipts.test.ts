...
import { auditPath, modeStatePath } from "@sayknow-cli/coding-agent/skc-runtime/session-layout";
...
async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-state-receipts-"));
...
function expectValidReceipt(state: Record<string, unknown>, skill: string): void {
	...
	expect(["skc-state-cli", "skc-runtime", "skc-hook"]).toContain(receipt?.owner as string);
...
function findAuditEntry(
	...
	return entries.find(entry => entry.category === "state" && entry.verb === verb && entry.owner === "skc-state-cli");
}

describe("G5 skc state receipts", () => {
	it("persists receipts and audit entries for write, clear, and handoff", async () => {
		await withTempCwd(async cwd => {
			...
			const handoffEntries = entries.filter(
				entry => entry.category === "state" && entry.verb === "handoff" && entry.owner === "skc-state-cli",
			);
	...
	it("rejects missing receipt sessions instead of constructing a default path", async () => {
		expect(() =>
			buildWorkflowStateReceipt({
				...
				skill: "ralplan",
				owner: "skc-state-cli",
				command: "skc state ralplan write",
				sessionId: " ",
			}),
		).toThrow("non-empty SKC session id");

		await withTempCwd(async cwd => {
			const sessionId = process.env.SKC_SESSION_ID;
			...
			try {
				...
				if (sessionId !== undefined) process.env.SKC_SESSION_ID = sessionId;
			}
		});
	});
});
