...
import { runNativeUltragoalCommand } from "../../src/skc-runtime/ultragoal-runtime";
import { getSkillManifest, type TypedArgSpec, typedArgsFor } from "../../src/skc-runtime/workflow-manifest";
...
describe("ultragoal terminal critic command contract", () => {
	it("publishes terminal critic verbs, typed arguments, and help", async () => {
		...
		expect(verdict.stdout).toContain(
			"skc ultragoal record-critic-verdict --terminus <completion|pause> --verdict <OKAY|ITERATE|REJECT> --evidence <text>",
		);
		...
		expect(override.stdout).toContain("skc ultragoal record-critic-gate-override --evidence <text> [--json]");
