...
import { type SkcTeamTask, selectCurrentClaimedTaskForWorker } from "@sayknow-cli/coding-agent/skc-runtime/team-store";
import {
...
} from "@sayknow-cli/coding-agent/skc-runtime/team-worker-memory-guard";
...
async function makeWorkerDir(): Promise<string> {
	const skcRoot = path.join(process.cwd(), ".skc");
	await fs.mkdir(skcRoot, { recursive: true });
	const root = await fs.mkdtemp(path.join(skcRoot, "tmp-team-worker-memory-guard-"));
	tempRoots.push(root);
