...
import { appendJsonlIdempotent } from "@sayknow-cli/coding-agent/skc-runtime/state-writer";

const tempRoots: string[] = [];

async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-idempotent-append-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("appendJsonlIdempotent (issue #660)", () => {
	it("appends a fresh entry and reports appended: true", async () => {
		...
		const target = ".skc/ledger/index.jsonl";
	...
	it("throws when neither key nor equals is supplied", async () => {
		const root = await tempDir();
		await expect(
			appendJsonlIdempotent(".skc/ledger/index.jsonl", { id: "a" }, { cwd: root } as never),
