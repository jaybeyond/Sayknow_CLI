...
import { readWorkflowStateJson, runNativeStateCommand } from "@sayknow-cli/coding-agent/skc-runtime/state-runtime";
...
describe("skc state read markdown", () => {
	it("defaults read output to markdown and keeps --json parseable", async () => {
		...
		await runNativeStateCommand(
			[
				...
				"deep-interview",
				"--input",
				JSON.stringify({ active: true, current_phase: "interviewing", artifact_path: ".skc/specs/draft.md" }),
			],
			root,
		);
		...
		expect(markdown.stdout).toContain(".skc/specs/draft.md");
	...
	it("rejects unknown skc state flags", async () => {
		const root = await tempDir();
		const result = await runNativeStateCommand(["read", "--mode", "deep-interview", "--bogus"], root);
		expect(result.status).toBe(2);
		expect(result.stderr).toContain("unknown skc state flag: --bogus");
	});
});
