...
function injectSafeAbsentToSafeOwnerProof(): void {
	...
	__setCreateOwnerIsolationForTests({
		probe: {
			readCallerCgroup: () => "0::/user.slice/user-1000.slice/user@1000.service/skc-owner-test.scope\n",
			probeServer: () => {
				probeCount += 1;
				return probeCount === 1
					? { state: "absent" }
...
describe("SKC tmux session management", () => {
	...
	it("lists only SKC-managed tmux sessions", () => {
		spyOn(Bun, "spawnSync").mockReturnValue(
			spawnResult(
				0,
				[
					"sayknow_cli_abc\t1\t0\t1770000000\t1\troot\t2\t12345\tfeature/demo\tfeature-demo\t/repo-a\t\t\t\t\t$1",
		...
		expect(Bun.spawnSync).toHaveBeenCalledWith(
			[
				...
				"list-sessions",
				"-F",
				"#{session_name}\t#{session_windows}\t#{session_attached}\t#{session_created}\t#{@skc-profile}\t#{session_key_table}\t#{session_panes}\t#{pane_pid}\t#{@skc-branch}\t#{@skc-branch-slug}\t#{@skc-project}\t#{@skc-session-id}\t#{@skc-session-state-file}\t#{@skc-owner-generation}\t#{@skc-version}\t#{session_id}",
			],
			expect.any(Object),
		);
	});
	...
	it("guards status and remove to SKC-managed sessions", () => {
		...
		spawnSyncSpy.mockImplementation((cmd: string[]) => {
			...
			if (cmd.includes("if-shell")) return spawnResult(0, "__skc_tmux_guarded_mutation_ok__\n");
	...
	it("refuses a same-name replacement before the final guarded remove", () => {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation(command => {
			...
			if (command.includes("list-sessions")) {
				return spawnResult(
					0,
					"sayknow_cli_work\t1\t0\t1770000000\t1\troot\t0\t\tmain\tmain\t/repo\tsession-1\t/tmp/runtime-state.json\tgeneration-one\t\t$1\n",
				);
			}
			if (command.includes("display-message")) return spawnResult(0, "$2\n");
			if (command.includes("show-options")) {
				return spawnResult(0, command.at(-1) === "@skc-owner-generation" ? "generation-one\n" : "1\n");
			}
			return spawnResult(0, "");
		});
		...
		).toThrow("skc_tmux_owner_changed:sayknow_cli_work");
	...
	it("refuses unsafe or unverifiable server proof before any remove, attach, or force-close mutation", async () => {
		for (const proofError of [
			"skc_tmux_owner_isolation_server_unsafe",
			...
			(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((command: string[]) => {
				...
				if (command.includes("list-sessions"))
					return spawnResult(
						0,
						"sayknow_cli_work\t1\t0\t1770000000\t1\troot\t0\t\t\t\tsession\t/state/marker\tgeneration\tsayknow_cli_work\n",
					);
				...
				if (command.includes("show-options")) {
					...
					return spawnResult(
						...
						option === "@skc-profile"
							? "1\n"
							: option === "@skc-session-id"
								? "session\n"
								: option === "@skc-owner-generation"
									? "generation\n"
									: option === "@skc-owner-server-key"
										? "sayknow_cli_work\n"
										: "/state/marker\n",
					);
				}
			...
			expect(() => removeSkcTmuxSession("sayknow_cli_work", { SKC_TMUX_COMMAND: "tmux" })).toThrow(proofError);
			expect(() => attachSkcTmuxSession("sayknow_cli_work", { SKC_TMUX_COMMAND: "tmux" })).toThrow(proofError);
			await expect(
				forceCloseSkcTmuxSession("sayknow_cli_work", { SKC_TMUX_COMMAND: "tmux" }, undefined, undefined, {
					resolveOwner: async () => ({
						sessionId: "session",
						stateDir: "/state",
						socketKey: "sayknow_cli_work",
	...
	it("hydrates native Windows tmux sessions from exact option reads when list-sessions omits user options", () => {
		...
		const session = statusSkcTmuxSession("win_session", {
	...
	it("builds psmux-aware targets for session-scoped commands", () => {
		...
		try {
			expect(
				buildSkcTmuxExactSessionTarget("work", {
			...
			expect(
				buildSkcTmuxExactSessionTarget("work", {
					env: { SKC_TMUX_COMMAND: "pmux", SKC_PSMUX_COMMAND: "pmux" },
				}),
			).toBe("work");
		} finally {
	...
	it("drops the tmux `=NAME` exact-session prefix on psmux for option commands", () => {
		...
		try {
			expect(
				buildSkcTmuxExactOptionTarget("work", {
					env: { SKC_TMUX_COMMAND: "tmux" },
				}),
			).toBe("=work:");
			expect(
	...
	it("rejects psmux before creating or tagging a managed session", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-sessions-test-"));
		...
		try {
			...
			expect(() =>
				createSkcTmuxSession({
					SKC_TMUX_COMMAND: "psmux",
					SKC_PSMUX_COMMAND: "psmux",
					SKC_TMUX_SESSION: "psmux_session",
					SKC_COORDINATOR_SESSION_STATE_FILE: path.join(stateDir, "runtime-state.json"),
				} as NodeJS.ProcessEnv),
			).toThrow("skc_tmux_owner_isolation_native_session_identity_unavailable");
	...
	it("refuses psmux before attach-session mutation", () => {
		...
		try {
			...
			expect(() =>
				attachSkcTmuxSession("managed", {
					SKC_TMUX_COMMAND: "psmux",
					SKC_PSMUX_COMMAND: "psmux",
				}),
			).toThrow("skc_tmux_owner_unverifiable:managed");
	...
	it("refuses an unbound native creation receipt before tagging or cleanup", () => {
		...
		expect(() =>
			createSkcTmuxSession({
				SKC_TMUX_COMMAND: "tmux",
				SKC_TMUX_SESSION: "managed",
				SKC_COORDINATOR_SESSION_STATE_FILE: path.join(os.tmpdir(), `skc-unbound-${crypto.randomUUID()}.json`),
			}),
		).toThrow("skc_tmux_owner_changed_after_create");
		expect(calls.some(cmd => cmd.includes("set-option") || cmd.includes("set-window-option"))).toBe(false);
	...
	it("refuses CAS-failure cleanup when a replacement server appears before the guarded kill", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-generation-cas-"));
		fixtureDirectories.push(stateDir);
		const calls: string[][] = [];
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation((command: string[]) => {
			...
			if (command.includes("if-shell")) {
				...
				return spawnResult(
					0,
					guardedMutationCount === 1
						? "__skc_tmux_guarded_mutation_ok__\n"
						: "__skc_tmux_guarded_mutation_refused__\n",
				);
			}
		...
		expect(() =>
			createSkcTmuxSession({
				SKC_TMUX_COMMAND: "tmux",
				SKC_TMUX_SESSION: "managed",
				SKC_COORDINATOR_SESSION_ID: "managed",
				SKC_COORDINATOR_SESSION_STATE_FILE: path.join(stateDir, "runtime-state.json"),
			}),
		).toThrow("skc_tmux_owner_generation_publish_failed_cleanup_failed");
		expect(calls).toContainEqual([
			"tmux",
			"if-shell",
			"-t",
	...
	it("refuses profile tagging when a replacement server appears at the receipt-to-tag boundary", () => {
		...
		(spyOn(Bun, "spawnSync") as unknown as SpawnSyncSpy).mockImplementation(command => {
			calls.push(command);
			if (command.includes("if-shell")) return spawnResult(0, "__skc_tmux_guarded_mutation_refused__\n");
			if (command.includes("display-message")) {
				if (command.includes("#{session_id}\t#{session_name}")) return spawnResult(0, "$1\tmanaged\n");
				return spawnResult(0, command.includes("#{session_name}") ? "managed\n" : "$1\n");
			}
		...
		expect(() =>
			createSkcTmuxSession({
				SKC_TMUX_COMMAND: "tmux",
				SKC_TMUX_SESSION: "managed",
				SKC_COORDINATOR_SESSION_STATE_FILE: path.join(os.tmpdir(), `skc-replacement-${crypto.randomUUID()}.json`),
			}),
		).toThrow("skc_tmux_profile_tag_failed_cleanup_failed");
		...
		expect(calls[1]?.[6]).toContain('"@skc-profile" "1"');
		expect(calls[1]?.[7]).toBe("display-message -p __skc_tmux_guarded_mutation_refused__");
	});
	...
	it("rejects psmux force-close before signal or cleanup", async () => {
		...
		try {
			await expect(
				forceCloseSkcTmuxSession(
					"managed",
					{ SKC_TMUX_COMMAND: "psmux", SKC_PSMUX_COMMAND: "psmux" },
					undefined,
					undefined,
					{
						signalTerm,
			...
			).rejects.toThrow("skc_tmux_owner_unverifiable:managed");
			expect(signalTerm).not.toHaveBeenCalled();
			expect(cleanupSession).not.toHaveBeenCalled();
		} finally {
			__setBinaryResolverForTests(null);
	...
	it("requires a matching SIGTERM verdict before compatibility cleanup", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-close-"));
		...
		injectSafeMutationProof();
		let signaled = false;
		await forceCloseSkcTmuxSession(
			"managed",
			{ SKC_TMUX_COMMAND: "tmux" },
			sessionId,
			path.join(stateDir, "marker"),
		...
		expect(calls).toEqual(
			expect.arrayContaining([
				...
				["tmux", "show-options", "-qv", "-t", "$0", "@skc-profile"],
				[
					"tmux",
					"if-shell",
					"-t",
	...
	it("surfaces an exact compatibility cleanup failure after a matching SIGTERM verdict", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-close-cleanup-failure-"));
		const sessionId = "session";
		const generation = "generation";
		...
		injectSafeMutationProof();
		await expect(
			forceCloseSkcTmuxSession("managed", { SKC_TMUX_COMMAND: "tmux" }, sessionId, marker, {
				resolveOwner: async () => ({
					sessionId,
					stateDir,
					socketKey: "managed",
	...
	it("does not kill a same-name replacement when the original native session ID disappears during verdict observation", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-close-replacement-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		await fs.mkdir(path.join(stateDir, sessionId, "owner-lifecycle"), {
		...
		).rejects.toThrow("skc_tmux_owner_changed:managed");
		expect(cleanupSession).not.toHaveBeenCalled();
		expect(Bun.spawnSync).toHaveBeenCalledWith(
	...
	it("rejects a missing native session ID before SIGTERM or cleanup", async () => {
		...
		await expect(
			forceCloseSkcTmuxSession("managed", { SKC_TMUX_COMMAND: "tmux" }, undefined, undefined, {
				resolveOwner: async () => ({
					sessionId: "session",
					stateDir: "/state",
					socketKey: "managed",
	...
	it("cancels an intent when the owner PID changes after the initial start-time proof", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-tmux-close-race-"));
		const sessionId = "session";
		const generation = "generation";
		const marker = path.join(stateDir, "marker");
		await fs.mkdir(path.join(stateDir, sessionId, "owner-lifecycle"), {
