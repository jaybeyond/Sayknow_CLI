...
import { isTmuxOwnerIsolationCliArgv } from "@sayknow-cli/coding-agent/skc-runtime/tmux-owner-isolation-cli";

const repoRoot = path.resolve(import.meta.dir, "..", "..", "..", "..");
const ownerIsolationCliEntry = path.join(repoRoot, "packages", "coding-agent", "src", "cli.ts");
const packagedSkcEntry = path.join(repoRoot, "packages", "coding-agent", "bin", "skc.js");
...
const ownerIsolationEntries: Array<[string, string[]]> = [
	...
	["packaged CLI", [process.execPath, packagedSkcEntry, ownerIsolationFlag]],
	[
		"direct main entry",
		[
			process.execPath,
...
describe("tmux owner isolation", () => {
	...
	it.each(ownerIsolationEntries)(
		...
		async (entry, command) => {
			const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-cli-generation-"));
	...
	it.each(ownerIsolationEntries)(
		...
		async (entry, command) => {
			const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-cli-observe-"));
			const sessionId = `session-${entry.replace(/\W+/g, "-")}`;
			const generation = `generation-${entry.replace(/\W+/g, "-")}`;
			try {
	...
	it("rejects a stale caller baseline before direct or scoped planning", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-plan-baseline-"));
		try {
	...
	it("plans and executes managed paths synchronously with a post-spawn server proof", () => {
		...
		const executed = executeTmuxOwnerIsolationPlanSync(scoped, {
			...
			probeServer: socketKey => ({
				...
				pid: 12,
				startTime: "34",
				cgroup: { classification: "safe", scope: `/skc-${socketKey}.scope` },
			}),
		});
	...
	it("requires bootstrap self-proof before spawning exact argv", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-bootstrap-"));
		...
		const bootstrap: BootstrapRequest = {
			...
			state_dir: state,
			socket_key: "socket",
			expected_scope: "skc-owner-token.scope",
		...
		const result = await bootstrapTmuxOwnerIsolation(bootstrap, {
			readSelfCgroup: async () => "0::/skc-owner-token.scope",
			...
			bootstrapTmuxOwnerIsolation(
				{
					...bootstrap,
					socket_key: "opaque socket",
					expected_scope: "skc-owner-explicit-token.scope",
					tmux_argv: ["tmux", "-L", "explicit", "new-session", "-s", "owned-explicit", "a b"],

					attempt: explicitAttempt,
				},
				{
					readSelfCgroup: async () => "0::/skc-owner-explicit-token.scope",
	...
	it("never cleans after an unsafe or unrelated post-spawn server proof", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-bootstrap-cleanup-"));
				...
				const result = await bootstrapTmuxOwnerIsolation(
					{
						...
						expected_scope: `skc-owner-${attempt.token}.scope`,
						tmux_argv: ["tmux", "-L", attempt.token, "new-session", "-s", attempt.session_name, "a b"],
						attempt,
					},
					{
						readSelfCgroup: async () => `0::/skc-owner-${attempt.token}.scope`,
						spawn: () => {
							spawnCount += 1;
							return { exitCode: 0, stdout: "$0\n" };
						},
	...
	it("rejects a stale bootstrap generation before consuming its capability or spawning", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-bootstrap-stale-"));
		const attempt = {
			token: "stale-token",
			session_name: "owned",
			socket_key: "socket",
			...
			const result = await bootstrapTmuxOwnerIsolation(
				{
					...
					state_dir: state,
					socket_key: "socket",
					expected_scope: "skc-owner-stale-token.scope",
					tmux_argv: ["tmux", "new-session", "-s", "owned"],
					attempt,
				},
				{
					readSelfCgroup: async () => "0::/skc-owner-stale-token.scope",
					spawn: () => {
						spawnCount += 1;
						return { exitCode: 0, stdout: "$0" };
					},
	...
	it("serializes concurrent generation replacements without reusable lock files", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-generation-lock-"));
		try {
	...
	it("rejects stale generations before creating a SIGTERM intent", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-"));
		await replaceOwnerGeneration(state, "session", "current");
		await expect(
			closeExactTmuxOwner(
				{
	...
	it("elects one immutable verdict from concurrent unique contenders", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-concurrent-"));
		try {
			const generation = await replaceOwnerGeneration(state, "session", "generation");
			const observation = {
				schema_version: 1 as const,
	...
	it("publishes only when the captured generation baseline remains exact", () => {
		const state = fsSync.mkdtempSync(path.join(os.tmpdir(), "skc-owner-generation-cas-"));
		try {
	...
	it("rejects noncanonical terminal observations before writing a journal or verdict", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-terminal-timestamp-"));
		try {
			await replaceOwnerGeneration(state, "session", "generation");
			const paths = lifecyclePaths(state, "session", "generation");
			await expect(
	...
	it("rejects malformed owner intents before they can authorize an expected terminal verdict", async () => {
		const state = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-intent-"));
		try {
			await expect(
