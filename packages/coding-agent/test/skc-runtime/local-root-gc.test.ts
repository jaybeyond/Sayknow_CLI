...
import type { GcContext } from "../../src/skc-runtime/gc-runtime";

/**
 * `resolveLocalRoot` gives every session its own `<tmp>/skc-local/<session-id>`
...
const MARKER = ".skc-local-legacy-migrated-v1";
const PAST_GRACE_MS = 25 * 60 * 60 * 1000;

const tempDirs: string[] = [];

function makeTmpRoot(): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-local-gc-test-"));
	tempDirs.push(dir);
	fs.mkdirSync(path.join(dir, "skc-local"), { recursive: true });
	return dir;
}

function seedRoot(tmpRoot: string, id: string, opts: { extra?: string; ageMs?: number } = {}): string {
	const dir = path.join(tmpRoot, "skc-local", id);
...
describe("local root GC", () => {
	it("reports nothing when the parent directory does not exist", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-local-gc-empty-"));
		tempDirs.push(dir);
