...
/**
 * `SKC_CONFIG_DIR` is documented as "Config root dirname under home", and
 ...
 * (`SKC_CONFIG_DIR?.trim() || path.join(os.homedir(), ".skc")`), so a user who
...
function scenario(settings: Record<string, unknown>, dirName = ".myconfig"): { home: string; repo: string } {
	const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "skc-config-root-")));
	roots.push(root);
...
async function resolveIn(home: string, repo: string, configDir: string | undefined): Promise<Record<string, unknown>> {
	const env: Record<string, string | undefined> = { ...process.env, HOME: home, SKC_CONFIG_DIR: configDir };
...
describe("config root is resolved under home", () => {
	it("reads ralplan settings from <home>/<SKC_CONFIG_DIR>", async () => {
		const { home, repo } = scenario({ "skc.ralplan.maxIterations": 9 });
	...
	it("reads ultragoal settings from <home>/<SKC_CONFIG_DIR>", async () => {
		const { home, repo } = scenario({ "skc.ultragoal.nudgeBudget": 7 });
	...
	it("keeps using the default config dir name when unset", async () => {
		const { home, repo } = scenario({ "skc.ralplan.maxIterations": 4 }, ".skc");
		...
		expect(result.source).toBe(path.join(home, ".skc", "settings.json"));
	});
});
