...
import { normalizeLegacyState } from "../../src/skc-runtime/state-migrations";
import { runNativeStateCommand } from "../../src/skc-runtime/state-runtime";

async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-state-migration-"));
...
describe("G7 skc state migration gate", () => {
	...
	it("migrates tampered state with --force and audits the forced mismatch", async () => {
		await withTempCwd(async cwd => {
			...
			expect(persisted.receipt).toMatchObject({ owner: "skc-state-cli", status: "fresh" });
