...
} from "@sayknow-cli/coding-agent/skc-runtime/state-schema";
import { WORKFLOW_STATE_VERSION } from "@sayknow-cli/coding-agent/skill-state/workflow-state-contract";

const roots: string[] = [];

async function tempFile(name: string, content: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-state-schema-"));
	roots.push(dir);
...
describe("state-schema (A1)", () => {
	...
	it("RequiredOnWriteEnvelopeSchema requires anchored fields incl. stamped receipt", () => {
		const validReceipt = {
			...
			state_path: "/x/.skc/state/ralplan-state.json",
			storage_path: "/x/.skc/state/ralplan-state.json",
...
describe("readSkcJson (A2)", () => {
	it("returns null for a missing file", async () => {
		const result = await readSkcJson(path.join(os.tmpdir(), "skc-nope-xyz.json"), WorkflowStateEnvelopeSchema);
	...
	it("returns ok for valid content and preserves raw", async () => {
		...
		const result = await readSkcJson(file, WorkflowStateEnvelopeSchema);
