...
describe("native skc deep-interview runtime", () => {
	...
	it("handles missing, valid, and corrupt deep-interview state during spec persistence", async () => {
		...
		const missingState = JSON.parse(
			await fs.readFile(path.join(missingRoot, ".skc", "state", "deep-interview-state.json"), "utf-8"),
		);
		expect(missingState.spec_slug).toBe("missing-state");

		const validRoot = await tempDir();
		const validStatePath = path.join(validRoot, ".skc", "state", "deep-interview-state.json");
	...
	it("fails closed on corrupt deep-interview state unless --force is supplied", async () => {
		...
		await expect(fs.access(path.join(root, ".skc", "specs", "deep-interview-corrupt-rejected.md"))).rejects.toThrow();
		...
		expect(forcedState.receipt).toMatchObject({ skill: "deep-interview", owner: "skc-runtime" });
		const audit = (await fs.readFile(path.join(root, ".skc", "state", "audit.jsonl"), "utf-8"))
			.trim()
			.split("\n")
	...
	it("persists a final spec under .skc/specs through the native CLI/API", async () => {
		const root = await tempDir();
		...
		expect(payload.path).toBe(path.join(root, ".skc", "specs", "deep-interview-persist-me.md"));
		...
		await expect(fs.access(path.join(root, ".skc", "plans"))).rejects.toThrow();
	});
	...
	it("uses --deliberate to persist the final spec and hand off to ralplan", async () => {
		...
		const specPath = path.join(root, ".skc", "specs", "deep-interview-deliberate-spec.md");
		...
		const ralplanState = JSON.parse(
			await fs.readFile(path.join(root, ".skc", "state", "ralplan-state.json"), "utf-8"),
		);
	...
	it("keeps deep-interview spec persistence distinct from ralplan plan writes", async () => {
		...
		expect(deepPayload.path).toContain(path.join(".skc", "specs", "deep-interview-separate.md"));
		...
		expect(ralplanPayload.path).toContain(path.join(".skc", "plans", "ralplan", "separate", "stage-01-final.md"));
	...
	it("honors skc.deepInterview.ambiguityThreshold in project .skc/settings.json", async () => {
		const root = await tempDir();
		await fs.mkdir(path.join(root, ".skc"), { recursive: true });
		await fs.writeFile(
			path.join(root, ".skc", "settings.json"),
			JSON.stringify({ skc: { deepInterview: { ambiguityThreshold: 0.08 } } }),
		);
		const result = await runNativeDeepInterviewCommand(["--standard", "--json", "idea"], root);
		expect(result.status).toBe(0);
		const payload = JSON.parse(result.stdout ?? "{}");
		expect(payload.threshold).toBeCloseTo(0.08);
		expect(payload.threshold_source).toBe(path.join(root, ".skc", "settings.json"));
	});
	...
	it("prefers modern config.yml threshold over legacy project settings.json", async () => {
		...
		resetSettingsForTest();
		await fs.writeFile(path.join(agentDir, "config.yml"), "skc:\n  deepInterview:\n    ambiguityThreshold: 0.2\n");
		await fs.mkdir(path.join(root, ".skc"), { recursive: true });
		await fs.writeFile(
			path.join(root, ".skc", "settings.json"),
			JSON.stringify({ skc: { deepInterview: { ambiguityThreshold: 0.08 } } }),
