...
import { DEFAULT_ULTRAGOAL_OBJECTIVE } from "@sayknow-cli/coding-agent/skc-runtime/goal-mode-request";
import {
...
import { assertUltragoalAskAllowed } from "@sayknow-cli/coding-agent/tools/ultragoal-ask-guard";

const TEST_SESSION_ID = "ultragoal-nudge-guard-test-session";
const ORIGINAL_SKC_SESSION_ID = process.env.SKC_SESSION_ID;
const ORIGINAL_SKC_CONFIG_DIR = process.env.SKC_CONFIG_DIR;
...
async function setProjectBudget(cwd: string, budget: number): Promise<void> {
	const skcDir = path.join(cwd, ".skc");
	await fs.mkdir(skcDir, { recursive: true });
	await fs.writeFile(path.join(skcDir, "settings.json"), JSON.stringify({ "skc.ultragoal.nudgeBudget": budget }));
}
...
afterEach(async () => {
	...
	if (ORIGINAL_SKC_CONFIG_DIR === undefined) delete process.env.SKC_CONFIG_DIR;
	else process.env.SKC_CONFIG_DIR = ORIGINAL_SKC_CONFIG_DIR;
...
describe("ultragoal nudge guard", () => {
	...
	it("AC3: per-story isolation and ledger-reset zeroing", async () => {
		const cwd = await tempDir();
		process.env.SKC_SESSION_ID = TEST_SESSION_ID;
		await createUltragoalPlan({ cwd, brief: MULTI_BRIEF, skcGoalMode: "per-story" });
	...
	it("AC6: skc ultragoal status reports the consumed nudge target and count", async () => {
		const cwd = await tempDir();
		process.env.SKC_SESSION_ID = TEST_SESSION_ID;
		await createUltragoalPlan({ cwd, brief: SINGLE_BRIEF });
