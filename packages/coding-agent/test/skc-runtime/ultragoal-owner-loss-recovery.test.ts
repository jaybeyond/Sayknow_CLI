...
describe("Ultragoal owner-loss recovery", () => {
	it("resumes only from exact durable terminal JSONL, preserving dirty product files despite live sidecars", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-loss-"));
		try {
			...
			await fs.mkdir(path.join(cwd, ".skc", "projection"), { recursive: true });
			await fs.writeFile(
				path.join(cwd, ".skc", "projection", "runtime.json"),
	...
	it("rejects reserved-artifact collisions and symlink traversal before snapshot or recovery authority", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-loss-"));
		const outside = await fs.mkdtemp(path.join(os.tmpdir(), "skc-owner-loss-outside-"));
		try {
			await setup(cwd);
			expect(
