...
import { renderStateGraph } from "@sayknow-cli/coding-agent/skc-runtime/state-graph";
import { runNativeStateCommand } from "@sayknow-cli/coding-agent/skc-runtime/state-runtime";

describe("SKC state graph rendering", () => {
	...
	it("renders one skill as dot", () => {
		...
		expect(output).toContain("digraph skc_state {");
