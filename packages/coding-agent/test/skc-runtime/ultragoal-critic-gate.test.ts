...
import { verifyUltragoalDurableCompletionState } from "@sayknow-cli/coding-agent/skc-runtime/ultragoal-guard";
import {
...
} from "@sayknow-cli/coding-agent/skc-runtime/ultragoal-receipt-freshness";
import {
	appendLedger,
...
describe("ultragoal terminal critic gate", () => {
	it("requires a clean OKAY criticReview for a single-goal final aggregate", async () => {
		...
		for (const [name, mutate] of cases) {
			...
			await fs.rm(path.join(root, ".skc"), { recursive: true, force: true });
		}
	...
	it("rejects final aggregate completion at the hard stop until a gate override is recorded", async () => {
		...
		expect(blocked.stderr).toContain(
			"terminal-critic ceiling reached; requires human/leader skc ultragoal record-critic-gate-override",
		);
