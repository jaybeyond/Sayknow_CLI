...
} from "@sayknow-cli/coding-agent/skc-runtime/goal-mode-request";
...
describe("SKC ultragoal goal mode request", () => {
	...
	it("reads skcObjective from the generated ultragoal plan", async () => {
		const root = await tempDir();
		const goalsPath = path.join(root, ".skc", "ultragoal", "goals.json");
		await fs.mkdir(path.dirname(goalsPath), { recursive: true });
		await Bun.write(goalsPath, JSON.stringify({ skcObjective: "Complete .skc/ultragoal/goals.json" }));

		const result = await readUltragoalSkcObjective(root);

		expect(result.objective).toBe("Complete .skc/ultragoal/goals.json");
	...
	it("queues a pending activation request even when the session file already has an active goal", async () => {
		...
		const result = Bun.spawnSync(["bun", cliPath, "ultragoal", "create-goals", "--brief", "Ship native goal"], {
			cwd: root,
			env: { ...process.env, [SKC_SESSION_FILE_ENV]: sessionFile, [SKC_SESSION_ID_ENV]: "session-owner" },
			stdout: "pipe",
			stderr: "pipe",
		});
		...
		expect(pending?.objective).toContain(".skc/ultragoal/goals.json");
	...
	it("surfaces corrupt pending request json", async () => {
		const root = await tempDir();
		const requestPath = path.join(root, ".skc", "state", "goal-mode-request.json");
	...
	it("surfaces corrupt ultragoal goals json", async () => {
		...
		await expect(readUltragoalSkcObjective(root)).rejects.toThrow(SyntaxError);
	});
});
