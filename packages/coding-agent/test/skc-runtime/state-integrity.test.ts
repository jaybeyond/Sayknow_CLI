...
import { writeJsonAtomic } from "../../src/skc-runtime/state-writer";

async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-state-integrity-"));
...
async function readAuditEntries(cwd: string): Promise<Array<Record<string, unknown>>> {
	const raw = await fs.readFile(path.join(cwd, ".skc/state/audit.jsonl"), "utf-8");
	return raw
		.trim()
		.split("\n")
		.filter(Boolean)
...
describe("skc state integrity", () => {
	it("blocks out-of-band edits on the next mode-state write unless forced", async () => {
		await withTempCwd(async cwd => {
			...
			const statePath = path.join(cwd, ".skc/state/ralplan-state.json");
			...
			expect(mismatch).toMatchObject({ skill: "ralplan", category: "state", owner: "skc-state-cli" });
	...
	it("does not checksum generic non-envelope JSON written by the shared writer", async () => {
		await withTempCwd(async cwd => {
			await writeJsonAtomic(
				path.join(cwd, ".skc/state/team/tasks/task-1.json"),
				{ id: "task-1", status: "open" },
				{ cwd },
			);
			const task = await readJson(path.join(cwd, ".skc/state/team/tasks/task-1.json"));
	...
	it("handoff writes and removes its per-mutation journal on success", async () => {
		await withTempCwd(async cwd => {
			...
			const entries = await fs.readdir(path.join(cwd, ".skc/state/transactions")).catch(() => [] as string[]);
	...
	it("an injected mid-handoff failure leaves a same-mutation journal while unrelated writes still proceed", async () => {
		await withTempCwd(async cwd => {
			...
			process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER = "__never__";
			const originalNow = Date.prototype.toISOString;
			Date.prototype.toISOString = () => "2026-06-03T00:00:00.000Z";
			process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER = "deep-interview:handoff:ralplan:2026-06-03T00:00:00.000Z";
			try {
				...
				delete process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER;
			}

			const journals = await fs.readdir(path.join(cwd, ".skc/state/transactions"));
			expect(journals).toHaveLength(1);
			const journal = await readJson(path.join(cwd, ".skc/state/transactions", journals[0]));
			...
			const remainingAfterRecovery = await fs.readdir(path.join(cwd, ".skc/state/transactions"));
			expect(remainingAfterRecovery).toEqual([]);

			await fs.writeFile(
				path.join(cwd, ".skc/state/transactions/orphan-unrelated.json"),
			...
			expect(await readJson(path.join(cwd, ".skc/state/ultragoal-state.json"))).toMatchObject({
				skill: "ultragoal",
			});
		});
	});
