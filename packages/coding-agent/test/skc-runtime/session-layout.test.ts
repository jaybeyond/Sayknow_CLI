...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-layout-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("session-layout (pure)", () => {
	...
	it("builds session root and category dirs under .skc/_session-<id>", () => {
		const root = sessionRoot("/proj", "abc");
		expect(root).toBe(path.join("/proj", ".skc", "_session-abc"));
	...
	it("rejects traversal in dynamic single-segment components (mode, slug)", () => {
		...
		expect(modeStatePath("/proj", "abc", "deep-interview")).toBe(
			path.join("/proj", ".skc", "_session-abc", "state", "deep-interview-state.json"),
		);
	});

	it("recovers session id from a _session-* dir name and rejects invalid names", () => {
		expect(sessionIdFromDirName(`${SKC_SESSION_PREFIX}abc`)).toBe("abc");
		expect(sessionIdFromDirName(`${SKC_SESSION_PREFIX}a%2Eb`)).toBe("a.b");
		expect(sessionIdFromDirName("state")).toBeUndefined();
		expect(sessionIdFromDirName(SKC_SESSION_PREFIX)).toBeUndefined();
	});
});
...
describe("session-resolution (boundary)", () => {
	it("resolves precedence flag > payload > env", () => {
		...
		expect(resolveSessionIdFromSources({ envSessionId: "e" })).toEqual({ skcSessionId: "e", source: "env" });
	...
	it("write resolution refuses a missing id", () => {
		expect(() => resolveSkcSessionForWrite("/proj", {})).toThrow(SessionResolutionError);
		expect(resolveSkcSessionForWrite("/proj", { envSessionId: "e" }).source).toBe("env");
	});

	it("read resolution errors when zero session dirs exist", async () => {
		const cwd = await tempDir();
		await expect(resolveSkcSessionForRead(cwd, {})).rejects.toThrow(/no active SKC session/);
	});
	...
	it("auto-detects the latest session by activity marker, not raw dir mtime", async () => {
		...
		expect(ctx.skcSessionId).toBe("new");
	...
	it("ignores session dirs without an activity marker", async () => {
		...
		const ctx = await detectLatestSession(cwd);
		expect(ctx.skcSessionId).toBe("marked");
	});
});
