...
async function tempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-di-redteam-"));
	tempRoots.push(dir);
	return dir;
}
...
describe("deep-interview redteam: idempotency and lossless shape normalization", () => {
	...
	it("strips leaked envelope-reserved keys from nested state and self-heals recursive nesting", () => {
		...
		const corrupt = {
			...
			receipt: { owner: "skc-state-cli", command: "skc state deep-interview write" },
			state_revision: 3,
			state: {
				rounds: [
		...
		expect(normalized.receipt).toEqual({ owner: "skc-state-cli", command: "skc state deep-interview write" });
...
describe("deep-interview redteam: reconcile and state write HUD parity", () => {
	...
	it("produces identical active-state HUD chips through reconcile and skc state write", async () => {
