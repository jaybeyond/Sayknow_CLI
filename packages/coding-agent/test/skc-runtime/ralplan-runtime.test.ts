...
import { readVisibleSkillActiveState } from "@sayknow-cli/coding-agent/skill-state/active-state";
...
describe("native skc ralplan runtime — consensus handoff", () => {
	it("accepts the documented flag surface without rejecting --interactive/--deliberate", async () => {
		...
		const state = JSON.parse(await fs.readFile(path.join(root, ".skc", "state", "ralplan-state.json"), "utf-8"));
	...
	it("emits receipt-only json for consensus handoff", async () => {
		...
		expect(payload.state_path).toContain(path.join(".skc", "state", "ralplan-state.json"));
	...
	it("syncs ralplan HUD chips for the active run", async () => {
		...
		const active = JSON.parse(
			await fs.readFile(path.join(root, ".skc", "state", "skill-active-state.json"), "utf-8"),
		);
		const entry = (
	...
	it("visible HUD prefers canonical final over a stale active-state snapshot", async () => {
		...
		const snapshotPath = path.join(root, ".skc", "state", "skill-active-state.json");
		...
		await fs.writeFile(
			path.join(root, ".skc", "state", "active", "ralplan.json"),
			`${JSON.stringify(
				{
					skill: "ralplan",
					active: true,
...
describe("native skc ralplan runtime — --write artifact path", () => {
	it("persists an inline artifact under .skc/plans/ralplan/<run-id>/", async () => {
		const root = await tempDir();
		const result = await runNativeRalplanCommand(
			[
				"--write",
		...
		const filePath = path.join(root, ".skc", "plans", "ralplan", "test-run-1", "stage-01-planner.md");
		const content = await fs.readFile(filePath, "utf-8");
		expect(content).toBe("# Plan body\n");
		const indexLine = (
			await fs.readFile(path.join(root, ".skc", "plans", "ralplan", "test-run-1", "index.jsonl"), "utf-8")
		).trim();
	...
	it("--artifact <file> reads contents from disk", async () => {
		...
		const content = await fs.readFile(
			path.join(root, ".skc", "plans", "ralplan", "file-run", "stage-02-architect.md"),
			"utf-8",
		);
	...
	it("restricted role-agent bash treats --artifact paths as inline text", async () => {
		...
		const previous = process.env[SKC_RESTRICTED_ROLE_AGENT_BASH_ENV];
		process.env[SKC_RESTRICTED_ROLE_AGENT_BASH_ENV] = "1";
		try {
			const result = await runNativeRalplanCommand(
				[
					"--write",
			...
			expect(result.status).toBe(0);
			const content = await fs.readFile(
				path.join(root, ".skc", "plans", "ralplan", "restricted-file-run", "stage-02-architect.md"),
				"utf-8",
			);
			expect(content).toBe(`${artifactPath}\n`);
		} finally {
			if (previous === undefined) {
				delete process.env[SKC_RESTRICTED_ROLE_AGENT_BASH_ENV];
			} else {
	...
	it("final stage emits pending-approval.md alongside the stage artifact", async () => {
		...
		const pendingApproval = await fs.readFile(
			path.join(root, ".skc", "plans", "ralplan", "final-run", "pending-approval.md"),
			"utf-8",
		);
	...
	it("appends index.jsonl entries instead of overwriting", async () => {
		...
		const indexLines = (
			await fs.readFile(path.join(root, ".skc", "plans", "ralplan", "multi", "index.jsonl"), "utf-8")
		)
			.trim()
			.split("\n");
	...
	it("keeps multiple --write calls in the same run when no --run-id is supplied", async () => {
		...
		const indexLines = (
			await fs.readFile(path.join(root, ".skc", "plans", "ralplan", firstPayload.run_id, "index.jsonl"), "utf-8")
		)
			.trim()
			.split("\n");
		expect(indexLines.length).toBe(2);
...
describe("native skc ralplan runtime — run-state phase coherence", () => {
	const readPhase = async (root: string): Promise<string> => {
		const raw = await fs.readFile(path.join(root, ".skc", "state", "ralplan-state.json"), "utf-8");
	...
	it("advances current_phase to final on the final stage write", async () => {
		...
		await runNativeRalplanCommand(["--deliberate", "--json", "task"], root);
		const runId = (
			JSON.parse(await fs.readFile(path.join(root, ".skc", "state", "ralplan-state.json"), "utf-8")) as {
				run_id: string;
			}
		).run_id;
		await runNativeRalplanCommand(
...
describe("native skc ralplan runtime — duplicate --write guard", () => {
	const runDir = (root: string, runId: string) => path.join(root, ".skc", "plans", "ralplan", runId);
...
describe("native skc ralplan runtime — persisted Planner state", () => {
	const statePath = (root: string) => path.join(root, ".skc", "state", "ralplan-state.json");
	...
	it("records fallback metadata together with a fresh planner id", async () => {
		...
		const result = await runNativeRalplanCommand(
			[
				...
				".skc/plans/ralplan/pp-fb/stage-03-revision.md",
				"--json",
			],
			root,
		);
		...
		expect(state.planner_fallback_receipt_path).toBe(".skc/plans/ralplan/pp-fb/stage-03-revision.md");
	...
	it("does not persist an artifact when planner flags are invalid (fail-fast)", async () => {
		...
		expect(result.status).toBe(2);
		const filePath = path.join(root, ".skc", "plans", "ralplan", "no-side-effect", "stage-01-planner.md");
...
describe("native skc ralplan runtime — post-clear re-activation (#644)", () => {
	...
	it("re-asserts active:true and resets phase out of terminal lock when a new run_id is written after a clear", async () => {
		...
		// Simulate `skc state ralplan clear`: active -> false, phase -> complete.
