...
import { deepInterviewDraftPath } from "@sayknow-cli/coding-agent/skc-runtime/deep-interview-stage";
...
describe("deep-interview staged transitions", () => {
	...
	it("inherits the session from SKC_SESSION_ID with no identity flags", async () => {
		const root = await tempDir();
		await seed(root);
		const staged = await run(root, [
			"stage",
	...
	it("reports a typed session error when no session id is resolvable", async () => {
		const root = await tempDir();
		const saved = process.env.SKC_SESSION_ID;
		...
		try {
			...
			process.env.SKC_SESSION_ID = saved;
		}
	});
	...
	it("refreshes the active-state/HUD projection after write and apply", async () => {
		const root = await tempDir();
		await seed(root);
		const snapshotPath = path.join(root, ".skc", `_session-${TEST_SESSION_ID}`, "state", "skill-active-state.json");
