...
import { renderCommandRefBlock } from "@sayknow-cli/coding-agent/skc-runtime/workflow-command-ref";
import { CANONICAL_SKC_WORKFLOW_SKILLS } from "@sayknow-cli/coding-agent/skill-state/canonical-skills";
...
const skillFiles = CANONICAL_SKC_WORKFLOW_SKILLS.map(skill =>
	path.join(repoRoot, "packages", "coding-agent", "src", "defaults", "skc", "skills", skill, "SKILL.md"),
);
...
describe("workflow command-reference proof spike", () => {
	it("renders deterministic command-reference blocks", () => {
		...
		expect(first.bytes).toContain("<!-- skc:cmdref:start state -->");
		expect(first.bytes).toContain("skc state ralplan write --input");
	...
	it("reports byte-equal or gap status without mutating SKILL.md files", async () => {
		...
		const result = await runBun(["scripts/generate-skc-skill-command-refs.ts", "--check", "--json"]);
		...
		expect(parsed.results).toHaveLength(CANONICAL_SKC_WORKFLOW_SKILLS.length);
		for (const item of parsed.results) {
			expect([...CANONICAL_SKC_WORKFLOW_SKILLS] as string[]).toContain(item.skill);
	...
	it("prints per-skill token budget report", async () => {
		...
		expect(parsed.skills).toHaveLength(CANONICAL_SKC_WORKFLOW_SKILLS.length);
