...
import {
	claimSkcTeamTask,
	classifySkcTeamCheckpointFiles,
	executeSkcTeamApiOperation,
	listSkcTeams,
	monitorSkcTeam,
	monitorSkcTeamSnapshot,
	parseTeamLaunchArgs,
	readSkcTeamSnapshot,
	readSkcTeamTask,
	requestSkcWorkerIntegrationAttempt,
	resolveSkcTeamWorkerCli,
	resolveSkcTeamWorkerCliPlan,
	resolveSkcWorkerCommand,
	type SkcTeamConfig,
	sendSkcTeamMessage,
	shutdownSkcTeam,
	startSkcTeam,
	transitionSkcTeamTask,
	translateSkcWorkerLaunchArgsForCli,
} from "../../src/skc-runtime/team-runtime";
...
async function createFakeTmuxBin(
	root: string,
	options: { failDisplay?: boolean; failSplit?: boolean; skcProfile?: boolean } = {},
): Promise<string> {
...
case "$1" in
  ...
  show-options)
    ...
    if [ "${options.skcProfile === false ? "0" : "1"}" = "1" ]; then echo "1"; exit 0; fi
  ...
  set-option)
    profile_file=${JSON.stringify(path.join(root, "tmux-profile-tag"))}
    for arg in "$@"; do
      if [ "$arg" = "@skc-profile" ]; then echo "1" > "$profile_file"; fi
    done
    exit 0
    ;;
  split-window)
...
async function createGitRepo(): Promise<string> {
	const repo = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-git-"));
	runGit(repo, ["init"]);
	runGit(repo, ["config", "user.email", "skc@example.test"]);
	runGit(repo, ["config", "user.name", "SKC Test"]);
...
async function readTeamConfig(stateDir: string): Promise<SkcTeamConfig> {
	return Bun.file(path.join(stateDir, "config.json")).json() as Promise<SkcTeamConfig>;
}
		...
		items: [
			{
				...
				command:
					"bun test packages/coding-agent/test/skc-runtime/team-runtime.test.ts --test-name-pattern completion evidence",
				output: "passed",
			},
		],
		files: ["packages/coding-agent/src/skc-runtime/team-runtime.ts"],
...
function inspectionCompletionEvidence(summary = "Completed by inspection") {
	return {
		...
		files: ["packages/coding-agent/test/skc-runtime/team-runtime.test.ts"],
	};
}
		...
		items: [
			{
				...
				artifact: ".skc/state/team/demo/report.md",
			},
		],
	};
}
...
describe("native skc team runtime", () => {
	it("creates SKC-scoped team state, task mailboxes, and telemetry without delegating to legacy runtimes", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-"));
		const snapshot = await startSkcTeam({
			workerCount: 1,
			agentType: "executor",
		...
		expect(snapshot.state_dir).toContain(path.join(".skc", "state", "team", "demo-team"));
		...
		expect(telemetry).toContain("Native skc team dry-run state initialized");
	...
	it("separates managed worker lifecycle from worker-reported status", async () => {
		...
		const initialStatus = (await executeSkcTeamApiOperation(
			"read-worker-status",
		...
		const startupAck = (await executeSkcTeamApiOperation(
			"worker-startup-ack",
		...
		let snapshot = await readSkcTeamSnapshot("worker-lifecycle-team", cleanupRoot, { PATH: "" });
		...
		const workingStatus = (await executeSkcTeamApiOperation(
		...
		snapshot = await readSkcTeamSnapshot("worker-lifecycle-team", cleanupRoot, { PATH: "" });
		...
		await executeSkcTeamApiOperation(
			"update-worker-status",
		...
		const forceRequest = (await executeSkcTeamApiOperation(
	...
	it("persists the active worker command so tmux workers use the same skc entrypoint", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-"));
		const snapshot = await startSkcTeam({
			workerCount: 1,
			agentType: "executor",
			...
			cwd: cleanupRoot,
			dryRun: true,
			env: { PATH: "", SKC_TEAM_WORKER_COMMAND: "bun ./packages/coding-agent/src/cli.ts" },
		});
		...
		expect(resolveSkcWorkerCommand(cleanupRoot, { SKC_TEAM_WORKER_COMMAND: "skc-dev" })).toBe("skc-dev");
	});

	it("keeps worker CLI selection limited to SKC teammate sessions", async () => {
		expect(resolveSkcTeamWorkerCli({})).toBe("skc");
		expect(resolveSkcTeamWorkerCli({ SKC_TEAM_WORKER_CLI: "auto" })).toBe("skc");
		expect(resolveSkcTeamWorkerCli({ SKC_TEAM_WORKER_CLI: "skc" })).toBe("skc");
		expect(resolveSkcTeamWorkerCliPlan(3, { SKC_TEAM_WORKER_CLI_MAP: "auto" })).toEqual(["skc", "skc", "skc"]);
		expect(resolveSkcTeamWorkerCliPlan(2, { SKC_TEAM_WORKER_CLI_MAP: "skc,auto" })).toEqual(["skc", "skc"]);
		expect(translateSkcWorkerLaunchArgsForCli("skc", ["--model", "frontier"])).toEqual(["--model", "frontier"]);
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-"));
		const snapshot = await startSkcTeam({
			workerCount: 2,
			agentType: "executor",
			task: "Launch SKC teammate sessions",
			teamName: "skc-worker-cli-team",
			cwd: cleanupRoot,
			dryRun: true,
			env: { PATH: "", SKC_TEAM_WORKER_CLI_MAP: "skc,auto" },
		});
		const config = await Bun.file(path.join(snapshot.state_dir, "config.json")).json();
		const manifest = await Bun.file(path.join(snapshot.state_dir, "manifest.v2.json")).json();
		const telemetry = await Bun.file(path.join(snapshot.state_dir, "telemetry.jsonl")).text();
		expect(config.worker_cli_plan).toEqual(["skc", "skc"]);
		expect(manifest.worker_cli_plan).toEqual(["skc", "skc"]);
		expect(telemetry).toContain('"worker_cli_plan":["skc","skc"]');

		for (const provider of ["codex", "claude", "gemini"]) {
			expect(() => resolveSkcTeamWorkerCli({ SKC_TEAM_WORKER_CLI: provider })).toThrow(
				/SKC team launches SKC teammate sessions only/,
			);
			expect(() => resolveSkcTeamWorkerCliPlan(1, { SKC_TEAM_WORKER_CLI_MAP: provider })).toThrow(
				/SKC team launches SKC teammate sessions only/,
			);
			expect(() =>
				resolveSkcTeamWorkerCliPlan(1, { SKC_TEAM_WORKER_CLI: provider, SKC_TEAM_WORKER_CLI_MAP: "skc" }),
			).toThrow(/SKC team launches SKC teammate sessions only/);
			if (!cleanupRoot) cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-"));
			await expect(
				startSkcTeam({
					workerCount: 1,
					agentType: "executor",
					...
					cwd: cleanupRoot,
					dryRun: true,
					env: { PATH: "", SKC_TEAM_WORKER_CLI: provider },
				}),
			).rejects.toThrow(/SKC team launches SKC teammate sessions only/);
		}
	});
	...
	it("creates worker worktrees by default for the tmux launch path", async () => {
		...
		const snapshot = await startSkcTeam({
			...
			cwd: cleanupRoot,
			env: { PATH: process.env.PATH ?? "", SKC_TEAM_WORKER_COMMAND: "true", SKC_TEAM_TMUX_COMMAND: fakeTmux },
		});
		...
		for (const worker of snapshot.workers) {
			...
			expect(worker.worktree_path).toContain(path.join(".skc", "state", "team", "worktree-team", "worktrees"));
	...
	it("resolves the team tmux leader from SKC_TMUX_COMMAND, not only SKC_TEAM_TMUX_COMMAND", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot);
		const snapshot = await startSkcTeam({
			workerCount: 1,
			...
			cwd: cleanupRoot,
			env: { PATH: process.env.PATH ?? "", SKC_TEAM_WORKER_COMMAND: "true", SKC_TMUX_COMMAND: fakeTmux },
		});
	...
	it("distributes explicit markdown lane sections into worker-owned initial tasks", async () => {
		...
		const task1 = await readSkcTeamTask("lane-distribution-team", "task-1", cleanupRoot, { PATH: "" });
		const task2 = await readSkcTeamTask("lane-distribution-team", "task-2", cleanupRoot, { PATH: "" });
	...
	it("rejects ambiguous inline lane splits before duplicating broad multi-worker work", async () => {
		...
		expect(await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "ambiguous-lane-team")).exists()).toBe(
			false,
		);
	});
	...
	it("rejects ambiguous lane splits before non-dry-run state, worktree, or pane mutation", async () => {
		...
		expect(
			await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "ambiguous-lane-worktree-team")).exists(),
		).toBe(false);
	...
	it("fails outside current tmux before creating team state or worktrees", async () => {
		...
		).rejects.toThrow(/skc_team_requires_tmux_leader: run `skc --tmux` first/);

		expect(await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "fail-team", "phase.json")).exists()).toBe(
			false,
		);
		expect(
			await Bun.file(
				path.join(cleanupRoot, ".skc", "state", "team", "fail-team", "worktrees", "worker-1", ".git"),
			).exists(),
		).toBe(false);
	});
	...
	it("rejects unmanaged tmux sessions before state, worktree, split, or profile mutation", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot, { skcProfile: false });
		...
		expect(
			await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "unmanaged-team", "phase.json")).exists(),
		).toBe(false);
		expect(
			await Bun.file(
				path.join(cleanupRoot, ".skc", "state", "team", "unmanaged-team", "worktrees", "worker-1", ".git"),
			).exists(),
		).toBe(false);
		const tmuxLog = await Bun.file(path.join(cleanupRoot, "tmux.log")).text();
		expect(tmuxLog).toContain("display-message -p #S:#I #{pane_id}");
		...
		expect(tmuxLog).toContain("show-options -qv -t =test-session: @skc-profile");
		expect(tmuxLog).not.toContain("show-options -qv -t =test-session @skc-profile");
		expect(tmuxLog).not.toContain("split-window");
		expect(tmuxLog).not.toContain("set-option -t test-session:0");
	});

	it("self-heals a missing @skc-profile tag when the leader pane was launched by skc --tmux", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot, { skcProfile: false });
		...
		const snapshot = await startSkcTeam({
			...
			env: {
				...
				SKC_TEAM_WORKER_COMMAND: "true",
				SKC_TEAM_TMUX_COMMAND: fakeTmux,
				SKC_TMUX_LAUNCHED: "1",
			},
		});
		...
		expect(tmuxLog).toContain("set-option -t =test-session: @skc-profile 1");
		expect(tmuxLog).not.toContain("set-option -t =test-session @skc-profile 1");
		expect(tmuxLog).toContain("split-window");
	});

	it("keeps rejecting untagged sessions when SKC_TMUX_LAUNCHED is not set even with TMUX present", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot, { skcProfile: false });
	...
	it("cleans partial worker worktrees without killing the leader session when pane startup fails", async () => {
		...
		await expect(
			Bun.file(
				path.join(cleanupRoot, ".skc", "state", "team", "split-fail-team", "worktrees", "worker-1", ".git"),
			).text(),
		).rejects.toThrow();
	});
	...
	it("removes clean created worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownSkcTeam("cleanup-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("does not kill stale or leader pane ids during shutdown", async () => {
		...
		await shutdownSkcTeam("stale-pane-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("preserves dirty worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownSkcTeam("dirty-cleanup-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("supports task claim, transition, list, status, and shutdown lifecycle operations", async () => {
		...
		await expect(
			transitionSkcTeamTask("life-team", "task-1", "completed", cleanupRoot, { PATH: "" }),
		).rejects.toThrow("claim_token_required:task-1");

		const claim = await claimSkcTeamTask("life-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await expect(
			transitionSkcTeamTask("life-team", "task-1", "completed", cleanupRoot, { PATH: "" }),
		).rejects.toThrow("claim_token_required:task-1");
		await expect(
			transitionSkcTeamTask("life-team", "task-1", "pending", cleanupRoot, { PATH: "" }, claim.claim_token),
		).rejects.toThrow("invalid_task_transition:task-1:pending_requires_release");
		expect(claim.task?.status).toBe("in_progress");
		const task = await transitionSkcTeamTask(
			"life-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		expect(
			await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "life-team", "claims", "task-1.json")).exists(),
		).toBe(false);
		await expect(
			executeSkcTeamApiOperation(
				"release-task-claim",
		...
		const reclaim = await claimSkcTeamTask("life-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
		expect(reclaim.ok).toBe(false);
		expect(reclaim.reason).toBe("task_not_pending:task-1");

		const status = await readSkcTeamSnapshot("life-team", cleanupRoot, { PATH: "" });
		expect(status.task_counts.completed).toBe(1);
		expect(await listSkcTeams(cleanupRoot, { PATH: "" })).toHaveLength(1);

		const stopped = await shutdownSkcTeam("life-team", cleanupRoot, { PATH: "" });
		...
		const shutdownRequest = (await Bun.file(
			path.join(cleanupRoot, ".skc", "state", "team", "life-team", "workers", "worker-1", "shutdown-request.json"),
	...
	it("chains claim, transition, and release using receipt-only task fields", async () => {
		...
		const claimReceipt = (await executeSkcTeamApiOperation(
			"claim-task",
		...
		const releaseReceipt = (await executeSkcTeamApiOperation(
			"release-task-claim",
			{
		...
		const secondClaimReceipt = (await executeSkcTeamApiOperation(
			"claim-task",
		...
		const transitionReceipt = (await executeSkcTeamApiOperation(
			"transition-task-status",
			{
				team_name: "receipt-team",
	...
	it("writes versioned structured traces linked to legacy events", async () => {
		...
		const claim = await claimSkcTeamTask("trace-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await transitionSkcTeamTask(
			"trace-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		const traceRead = (await executeSkcTeamApiOperation("read-traces", { team_name: "trace-team" }, cleanupRoot, {
			PATH: "",
		})) as {
	...
	it("sanitizes mailbox bodies from structured traces", async () => {
		...
		const traceRead = (await executeSkcTeamApiOperation(
			"read-traces",
	...
	it("stores structured completion evidence in task listings and honors claim tokens without implicit worker defaults", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "evidence-team");

		const workerTwoClaim = await claimSkcTeamTask("evidence-team", "worker-2", cleanupRoot, { PATH: "" }, "task-2");
		...
		const completedTask = await readSkcTeamTask("evidence-team", "task-2", cleanupRoot, { PATH: "" });
		...
		const listed = (await executeSkcTeamApiOperation("list-tasks", { team_name: "evidence-team" }, cleanupRoot, {
			PATH: "",
		...
		const workerOneClaim = await claimSkcTeamTask("evidence-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
	...
	it("rejects completed transitions without valid evidence and leaves task state unchanged", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "invalid-evidence-team");
		const claim = await claimSkcTeamTask("invalid-evidence-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		const taskBefore = await readSkcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, { PATH: "" });
		...
		const taskAfter = await readSkcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, { PATH: "" });
	...
	it("allows non-command completion evidence and requires evidence-backed shutdown completion", async () => {
		...
		const firstClaim = await claimSkcTeamTask(
		...
		const first = await transitionSkcTeamTask(
			"review-evidence-team",
			"task-1",
			"completed",
			cleanupRoot,
		...
		const secondClaim = await claimSkcTeamTask(
			"review-evidence-team",
			"worker-2",
			cleanupRoot,
			{ PATH: "" },
		...
		const stopped = await shutdownSkcTeam("review-evidence-team", cleanupRoot, { PATH: "" });
		expect(stopped.phase).toBe("complete");
	});
	...
	it("treats legacy evidence-free completed tasks as failed on shutdown", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "legacy-completed-team");
		const task = await readSkcTeamTask("legacy-completed-team", "task-1", cleanupRoot, { PATH: "" });
		await Bun.write(
			path.join(stateDir, "tasks", "task-1.json"),
			`${JSON.stringify({ ...task, status: "completed", completed_at: new Date().toISOString() }, null, 2)}\n`,
		);

		const stopped = await shutdownSkcTeam("legacy-completed-team", cleanupRoot, { PATH: "" });
	...
	it("recovers expired task claims before new claim attempts", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "expired-claim-team");
		const claim = await claimSkcTeamTask("expired-claim-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		const claimedTask = await readSkcTeamTask("expired-claim-team", "task-1", cleanupRoot, { PATH: "" });
		...
		const recoveredClaim = await claimSkcTeamTask("expired-claim-team", "worker-1", cleanupRoot, { PATH: "" });
	...
	it("recovers stale heartbeat claims during monitor and blocks stale workers from reclaiming", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "stale-heartbeat-team");
		const claim = await claimSkcTeamTask("stale-heartbeat-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await Bun.write(
		...
		const snapshot = await monitorSkcTeam("stale-heartbeat-team", cleanupRoot, {
			PATH: "",
			SKC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
		expect(snapshot.task_counts.pending).toBe(1);
		...
		const retry = await claimSkcTeamTask("stale-heartbeat-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
	...
	it("keeps read-only status separate from mutating monitor recovery", async () => {
		...
		const stateDir = path.join(cleanupRoot, ".skc", "state", "team", "status-semantics-team");
		const claim = await claimSkcTeamTask("status-semantics-team", "worker-1", cleanupRoot, { PATH: "" });
		expect(claim.ok).toBe(true);
		await Bun.write(
			path.join(stateDir, "workers", "worker-1", "heartbeat.json"),
			`${JSON.stringify(
		...
		const statusSnapshot = await readSkcTeamSnapshot("status-semantics-team", cleanupRoot, {
			PATH: "",
			SKC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
		...
		const monitorSnapshot = await monitorSkcTeamSnapshot("status-semantics-team", cleanupRoot, {
			PATH: "",
			SKC_TEAM_HEARTBEAT_STALE_MS: "1",
		});
	...
	it("recovers missing-pane claims and marks the worker lifecycle failed", async () => {
		...
		const stateDir = snapshot.state_dir;
		const claim = await claimSkcTeamTask("missing-pane-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		expect(claim.ok).toBe(true);
		...
		const recovered = await monitorSkcTeam("missing-pane-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
	...
	it("enforces typed lane claim eligibility before leases are granted", async () => {
		...
		const wrongOwner = await claimSkcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-2");
		expect(wrongOwner.ok).toBe(false);
		expect(wrongOwner.reason).toBe("task_owner_mismatch:task-2:worker-2");

		const dependentTask = (await executeSkcTeamApiOperation(
			"create-task",
			{
				team_name: "lane-team",
		...
		const blockedByDependency = await claimSkcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-3");
		...
		const wrongRole = await claimSkcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-4");
		expect(wrongRole.ok).toBe(false);
		expect(wrongRole.reason).toBe("task_role_mismatch:task-4:architect");

		const implementationClaim = await claimSkcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1");
		...
		const verificationClaim = await claimSkcTeamTask("lane-team", "worker-1", cleanupRoot, { PATH: "" }, "task-3");
	...
	it("allows only one worker to claim a task under concurrent claim attempts", async () => {
		...
		const claims = await Promise.all([
			claimSkcTeamTask("claim-race-team", "worker-1", cleanupRoot, { PATH: "" }, "task-1"),
			claimSkcTeamTask("claim-race-team", "worker-2", cleanupRoot, { PATH: "" }, "task-1"),
		]);
		...
		const status = await readSkcTeamSnapshot("claim-race-team", cleanupRoot, { PATH: "" });
		expect(status.task_counts.in_progress).toBe(1);
	});

	it("supports SKC team parity behavioral API operations", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-team-runtime-"));
		await startSkcTeam({
			workerCount: 2,
			agentType: "executor",
		...
		const created = (await executeSkcTeamApiOperation(
			"create-task",
		...
		const read = (await executeSkcTeamApiOperation(
			"read-task",
		...
		const updated = (await executeSkcTeamApiOperation(
			"update-task",
		...
		const claim = (await executeSkcTeamApiOperation(
			"claim-task",
		...
		const claimed = (await executeSkcTeamApiOperation(
			"claim-task",
			{ team_name: "api-team", task_id: created.task_id, worker: "worker-1" },
			cleanupRoot,
			{ PATH: "" },
		...
		const message = (await executeSkcTeamApiOperation(
			"send-message",
		...
		const mailbox = (await executeSkcTeamApiOperation(
			"mailbox-list",
		...
		const heartbeat = (await executeSkcTeamApiOperation(
		...
		const awaited = (await executeSkcTeamApiOperation("await-event", { team_name: "api-team" }, cleanupRoot, {
			PATH: "",
		})) as { status: string };
		...
		const monitor = (await executeSkcTeamApiOperation(
		...
		const approval = (await executeSkcTeamApiOperation(
			"read-task-approval",
			{ team_name: "api-team", task_id: created.task_id },
			cleanupRoot,
			{ PATH: "" },
	...
	it("stores mailbox messages per recipient and maintains native notification transitions", async () => {
		...
		const first = (await executeSkcTeamApiOperation(
			"send-message",
			{
		...
		const second = (await executeSkcTeamApiOperation(
			"send-message",
			{
				team_name: "notification-team",
				from_worker: "worker-1",
		...
		let notifications = (await executeSkcTeamApiOperation(
			"notification-list",
			{ team_name: "notification-team" },
			cleanupRoot,
			{ PATH: "" },
		...
		notifications = (await executeSkcTeamApiOperation(
			"notification-list",
			{ team_name: "notification-team" },
			cleanupRoot,
			{ PATH: "" },
	...
	it("rejects path-like worker ids and reports lifecycle nudges without automatic worker action", async () => {
		...
		expect(
			await Bun.file(
				path.join(cleanupRoot, ".skc", "state", "team", "guard-team", "escaped", "heartbeat.json"),
			).exists(),
		).toBe(false);

		const monitored = await monitorSkcTeam("guard-team", cleanupRoot, {
			PATH: "",
			SKC_TEAM_STARTUP_GRACE_MS: "0",
			SKC_TEAM_HEARTBEAT_STALE_MS: "0",
			SKC_TEAM_NUDGE_COOLDOWN_MS: "60000",
		});
		expect(monitored.workers[0]?.status).toBe("idle");
		const nudgeDir = path.join(cleanupRoot, ".skc", "state", "team", "guard-team", "workers", "worker-1", "nudges");
		const nudges = await fs.readdir(nudgeDir);
		expect(nudges.length).toBeGreaterThan(0);
		const events = await readEvents(path.join(cleanupRoot, ".skc", "state", "team", "guard-team"));
		expect(events).toContain("worker_lifecycle_nudge");
		expect(events).toContain("auto_action_taken");
	});

	it("monitor integrates dirty detached worker worktrees and records SKC-scoped hygiene artifacts", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot);
		const snapshot = await startSkcTeam({
			workerCount: 1,
		...
		const monitored = await monitorSkcTeam("integrate-dirty-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "integrate-dirty-team.ledger.json"),
		).json();
	...
	it("checkpoint classification excludes SKC runtime paths from worker auto-commits", async () => {
		expect(
			classifySkcTeamCheckpointFiles([
				"src/feature.ts",
				".skc/state/team/demo/worker.json",
				".skc/reports/team-commit-hygiene/demo.ledger.json",
			]),
		).toEqual({
			eligible: ["src/feature.ts"],
			protected: [".skc/state/team/demo/worker.json", ".skc/reports/team-commit-hygiene/demo.ledger.json"],
		});
		...
		await Bun.write(path.join(worker.worktree_path, ".skc", "state", "team", "runtime.json"), "{}\n");

		await monitorSkcTeam("protected-checkpoint-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});

		expect(await Bun.file(path.join(cleanupRoot, "semantic.txt")).text()).toBe("semantic\n");
		expect(await Bun.file(path.join(cleanupRoot, ".skc", "state", "team", "runtime.json")).exists()).toBe(false);
	});
	...
	it("worker turn-end integration requests notify the leader once per fingerprint", async () => {
		...
		const env = {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_NAME: "turn-end-request-team",
			SKC_TEAM_WORKER_ID: "worker-1",
			SKC_TEAM_STATE_ROOT: config.state_root,
			SKC_TEAM_WORKTREE_PATH: worker.worktree_path,
		};

		const first = await requestSkcWorkerIntegrationAttempt(worker.worktree_path, env);
		const second = await requestSkcWorkerIntegrationAttempt(worker.worktree_path, env);
		...
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "turn-end-request-team.ledger.json"),
		).json();
	...
	it("reports awaiting integration when all worker tasks completed after an integration request", async () => {
		...
		const requestEnv = {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_NAME: "awaiting-request-team",
			SKC_TEAM_WORKER_ID: "worker-1",
			SKC_TEAM_STATE_ROOT: config.state_root,
			SKC_TEAM_WORKTREE_PATH: worker.worktree_path,
		};
		const requested = await requestSkcWorkerIntegrationAttempt(worker.worktree_path, requestEnv);
		expect(requested.requested).toBe(true);

		const claim = await claimSkcTeamTask("awaiting-request-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
		});
		await transitionSkcTeamTask(
		...
		const status = await readSkcTeamSnapshot("awaiting-request-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		expect(status.task_counts.completed).toBe(1);
		expect(status.phase).toBe("awaiting_integration");
		expect(status.phase).not.toBe("running");

		const stopped = await shutdownSkcTeam("awaiting-request-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
	...
	it("monitor cherry-picks diverged worker commits and stays idempotent on repeated status checks", async () => {
		...
		const first = await monitorSkcTeam("diverged-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		const leaderAfterFirst = runGit(cleanupRoot, ["rev-parse", "HEAD"]);
		const second = await monitorSkcTeam("diverged-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "diverged-team.ledger.json"),
		).json();
	...
	it("monitor reports merge conflicts without falsely advancing last integrated head", async () => {
		...
		const monitored = await monitorSkcTeam("merge-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "merge-conflict-team.ledger.json"),
		).json();
	...
	it("keeps completed conflicting teams in awaiting integration instead of plain running", async () => {
		...
		await commitFile(workerPath, "README.md", "# worker\n", "worker readme");
		await Bun.write(path.join(cleanupRoot, "README.md"), "# leader dirty\n");
		const claim = await claimSkcTeamTask("awaiting-conflict-team", "worker-1", cleanupRoot, {
			PATH: process.env.PATH ?? "",
		});
		await transitionSkcTeamTask(
		...
		const monitored = await monitorSkcTeam("awaiting-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const stopped = await shutdownSkcTeam("awaiting-conflict-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		expect(stopped.task_counts.completed).toBe(1);
		expect(stopped.phase).toBe("awaiting_integration");
		expect(stopped.phase).not.toBe("complete");
	});
	...
	it("monitor reports cherry-pick conflicts and aborts cleanly", async () => {
		...
		const monitored = await monitorSkcTeam("pick-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		expect(await readMailbox(snapshot.state_dir, "worker-1")).toContain("Manual resolution required");
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "pick-conflict-team.ledger.json"),
		).json();
		expect(JSON.stringify(ledger)).toContain('"status":"conflict"');
		expect(JSON.stringify(ledger)).toContain("integration_cherry_pick");
	});
	...
	it("cross-rebases idle, done, and failed workers while skipping working workers", async () => {
		...
		const monitored = await monitorSkcTeam("cross-rebase-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_TEAM_TMUX_COMMAND: fakeTmux,
		});
		...
		const ledger = await Bun.file(
			path.join(cleanupRoot, ".skc", "reports", "team-commit-hygiene", "cross-rebase-team.ledger.json"),
		).json();
	...
	it("pure team reads, status, and list operations stay read-only while monitor and resume can mutate", async () => {
		...
		const listed = await listSkcTeams(cleanupRoot, { PATH: process.env.PATH ?? "" });
		const read = await readSkcTeamSnapshot("pure-read-team", cleanupRoot, { PATH: process.env.PATH ?? "" });
		...
		expect(commandSource).toContain("readSkcTeamSnapshot(teamName)");
		expect(commandSource).toContain('action === "monitor" || action === "resume"');
		expect(commandSource).toContain("monitorSkcTeamSnapshot(teamName)");
		expect(commandSource).toContain("listSkcTeams()");
		...
		const runtimeSource = await Bun.file(path.join(import.meta.dir, "../../src/skc-runtime/team-runtime.ts")).text();
