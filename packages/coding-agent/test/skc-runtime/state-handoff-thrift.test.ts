...
const INITIAL_SESSION_ID = process.env.SKC_SESSION_ID;

function restoreSessionId(sessionId: string | undefined): void {
	if (sessionId === undefined) delete process.env.SKC_SESSION_ID;
	else process.env.SKC_SESSION_ID = sessionId;
}
...
async function tempDir(prefix = "skc-handoff-thrift-"): Promise<string> {
...
const activeTempArtifact = new RegExp(`${escapedTempRoot}/(?:skill-tool|skc)-[^\\n"]+`, "g");
...
describe("CONSUMER/KEY-FIELD MATRIX for compact handoff payloads", () => {
	it("goldens and asserts every preserved consumer key field", async () => {
		...
		expect(scrub(ralplanSeed.stdout ?? "")).toMatchInlineSnapshot(`
			"{"ok":true,"skill":"ralplan","mode":"short","state_path":"/tmp/SCRUBBED","run_id":"run-b","handoff":"/skill:ralplan","repository_binding":{"schema":"skc.repository_binding.v1","worktreeRoot":"/tmp/SCRUBBED","commonDir":null,"displayPath":"/tmp/SCRUBBED"}}
			"
			`);
		...
		expect(scrub(ultragoalHandoff.stdout ?? "")).toMatchInlineSnapshot(`
			...
			skc-objective=Complete the durable ultragoal plan in .skc/ultragoal/goals.json, including later accepted/appended stories, under the original brief constraints; use .skc/ultragoal/ledger.jsonl as the audit trail.
	...
	it("documents the ralplan receipt-only guideline for role agents", async () => {
		...
		expect(skillDoc).toContain("skc ralplan --write");
