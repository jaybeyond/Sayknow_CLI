...
async function runDoctorUnchanged(
	...
	const before = await snapshotFiles(path.join(root, ".skc"));
	const result = await runNativeStateCommand(args, root);
	const after = await snapshotFiles(path.join(root, ".skc"));
...
describe("skc state doctor", () => {
	...
	it("detects orphan transaction journals and prints the hard prune fix command", async () => {
		...
		expect(text.stdout).toContain("fix=skc state prune --hard");

		const json = await runDoctorUnchanged(root, ["doctor", "--json"]);
		const parsed = JSON.parse(json.stdout ?? "{}");
		expect(parsed.ok).toBe(false);
		expect(parsed.problems).toEqual([
			expect.objectContaining({ type: "orphan_journal", path: journalPath, fixCommand: "skc state prune --hard" }),
		]);
	});
	...
	it("detects checksum mismatches and prints the migrate fix command", async () => {
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				skill: "ralplan",
				path: statePath,
				fixCommand: "skc state ralplan migrate",
			}),
		]);
	});
	...
	it("detects schema violations and prints the migrate fix command", async () => {
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				skill: "ultragoal",
				path: statePath,
				fixCommand: "skc state ultragoal migrate",
			}),
		]);
	});
	...
	it("detects stale active-state from raw snapshot and per-skill active entries", async () => {
		...
		expect(text.stdout).toContain("fix=skc state team clear");
		expect(text.stdout).toContain("fix=skc state ralplan clear");
		...
		expect(parsed.problems).toEqual([
			expect.objectContaining({
				...
				fixCommand: "skc state ralplan clear",
			}),
			expect.objectContaining({
				type: "stale_active_state",
				skill: "team",
				path: activeEntryPath,
				fixCommand: "skc state team clear",
			}),
		]);
	});
});
