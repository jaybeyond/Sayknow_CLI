...
async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-state-handoff-"));
...
describe("skc state handoff", () => {
	...
	it("writes callee mode-state before caller mode-state using the caller-write failpoint", async () => {
		await withTempCwd(async cwd => {
			...
			const priorFailpoint = process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER;
			const originalToISOString = Date.prototype.toISOString;
			Date.prototype.toISOString = () => handoffAt;
			process.env.SKC_STATE_HANDOFF_FAIL_AFTER_CALLER = mutationId;
			try {
				const result = await runNativeStateCommand(
					["handoff", "--mode", "deep-interview", "--to", "ralplan", "--json"],
					cwd,
				...
				restoreEnvironmentValue("SKC_STATE_HANDOFF_FAIL_AFTER_CALLER", priorFailpoint);
			}
