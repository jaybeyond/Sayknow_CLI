...
async function withTempCwd(fn: (cwd: string) => Promise<void>): Promise<void> {
	const dir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-state-handoff-"));
	// Most tests assume root-scoped state; clear SKC_SESSION_ID so the
	...
	// default restore SKC_SESSION_ID inside their own setup.
...
describe("skc state handoff", () => {
	it("transitions caller -> callee atomically across mode-state and active-state", async () => {
		await withTempCwd(async cwd => {
			const callerPath = path.join(cwd, ".skc/state/deep-interview-state.json");
			...
			const callee = await readJson(path.join(cwd, ".skc/state/ralplan-state.json"));
			...
			const activeState = await readJson(path.join(cwd, ".skc/state/skill-active-state.json"));
	...
	it("normalizes legacy caller and callee envelopes to v2 during handoff", async () => {
		await withTempCwd(async cwd => {
			const callerPath = path.join(cwd, ".skc/state/deep-interview-state.json");
			const calleePath = path.join(cwd, ".skc/state/ralplan-state.json");
			await writeJson(callerPath, {
				skill: "deep-interview",
				version: 1,
				active: true,
	...
	it("rejects missing --to", async () => {
		await withTempCwd(async cwd => {
			await writeJson(path.join(cwd, ".skc/state/deep-interview-state.json"), {
				skill: "deep-interview",
				version: 1,
				active: true,
				current_phase: "interviewing",
	...
	it("rejects --to equal to caller", async () => {
		await withTempCwd(async cwd => {
			await writeJson(path.join(cwd, ".skc/state/ralplan-state.json"), {
				skill: "ralplan",
				version: 1,
				active: true,
	...
	it("supports backward chain ultragoal -> ralplan", async () => {
		await withTempCwd(async cwd => {
			await writeJson(path.join(cwd, ".skc/state/ultragoal-state.json"), {
				skill: "ultragoal",
				version: 1,
				active: true,
			...
			expect(result.status).toBe(0);
			const ultragoal = await readJson(path.join(cwd, ".skc/state/ultragoal-state.json"));
			expect(ultragoal?.active).toBe(false);
			expect(ultragoal?.current_phase).toBe("handoff");
			expect(ultragoal?.handoff_to).toBe("ralplan");
			const ralplan = await readJson(path.join(cwd, ".skc/state/ralplan-state.json"));
			expect(ralplan?.active).toBe(true);
	...
	it("handoffs session-scoped state when --session-id is forwarded", async () => {
		await withTempCwd(async cwd => {
			...
			const sessionDir = path.join(cwd, ".skc/state/sessions", encodedSession);
			...
			await expect(fs.access(path.join(cwd, ".skc/state/deep-interview-state.json"))).rejects.toThrow();
	...
	it("propagates strict sync failure when active-state write fails after mode-state writes succeed", async () => {
		await withTempCwd(async cwd => {
			...
			await fs.mkdir(path.join(cwd, ".skc/state/skill-active-state.json"), { recursive: true });
			...
			// reflects intent, and a subsequent retry (or explicit `skc state
			...
			const callee = JSON.parse(
				await fs.readFile(path.join(cwd, ".skc/state/ralplan-state.json"), "utf-8"),
			) as Record<string, unknown>;
			expect(callee.active).toBe(true);
			expect(callee.handoff_from).toBe("deep-interview");
		});
	...
	it("treats corrupt active-state JSON as a strict failure", async () => {
		await withTempCwd(async cwd => {
			...
			await fs.mkdir(path.join(cwd, ".skc/state"), { recursive: true });
			await fs.writeFile(path.join(cwd, ".skc/state/skill-active-state.json"), "{ not valid json");
	...
	it("defaults session-id from SKC_SESSION_ID env var when no --session-id flag is passed", async () => {
		await withTempCwd(async cwd => {
			...
			try {
				...
				if (prior === undefined) delete process.env.SKC_SESSION_ID;
