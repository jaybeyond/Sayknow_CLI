 ...
 * boundary. It used to read `SKC_CODING_AGENT_DIR` / `SKC_CONFIG_DIR` straight
...
function scenario(dotenv: string | undefined, configs: Record<string, string>): { dir: string; home: string } {
	const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "skc-skill-hook-trust-")));
	scenarios.push(dir);
	const repo = path.join(dir, "repo");
	const home = path.join(dir, "home");
	fs.mkdirSync(repo, { recursive: true });
	fs.mkdirSync(path.join(home, ".skc", "agent"), { recursive: true });
...
async function customDirectoriesIn(dir: string, home: string): Promise<string[]> {
	const proc = Bun.spawn([process.execPath, PROBE], {
		...
		env: { ...process.env, HOME: home, SKC_CODING_AGENT_DIR: undefined, SKC_CONFIG_DIR: undefined },
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("skill hook agent-dir trust boundary", () => {
	it("ignores an agent dir the project .env points at", async () => {
		const { dir, home } = scenario(`SKC_CODING_AGENT_DIR=${path.join("/tmp", "planted")}\n`, {});
		...
		fs.writeFileSync(path.join(dir, "repo", ".env"), `SKC_CODING_AGENT_DIR=${planted}\n`);
	...
	it("still honors config in the trusted agent dir", async () => {
		...
		fs.writeFileSync(path.join(home, ".skc", "agent", "config.yml"), skillConfig("/tmp/legit-skills"));
	...
	it("ignores a config dir name the project .env points at", async () => {
		const { dir, home } = scenario("SKC_CONFIG_DIR=.evil\n", {});
