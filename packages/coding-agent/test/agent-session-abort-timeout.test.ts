...
import { AgentSession, WorkerIntegrationRequestScheduler } from "@sayknow-cli/coding-agent/session/agent-session";
...
describe("AgentSession abort timeout", () => {
	...
	it("bounds dispose, force-invalidates an abort-ignoring run, and drops its late events", async () => {
		tempDir = TempDir.createSync("@skc-dispose-timeout-");
		authStorage = await AuthStorage.create(path.join(tempDir.path(), "testauth.db"));
