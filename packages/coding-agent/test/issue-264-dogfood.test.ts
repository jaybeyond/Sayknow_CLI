...
import type { AgentTelemetryWarning, AgentTelemetryWarningCode } from "@sayknow-cli/agent-core";
...
describe("issue #264 — dogfood build and telemetry boundaries", () => {
	...
	it("stages workspace native addons next to the compiled dev binary", async () => {
		...
		expect(source.indexOf("await stageWorkspaceNativeAddons();")).toBeGreaterThan(source.indexOf('"dist/skc"'));
	});
});
