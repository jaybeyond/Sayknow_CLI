...
async function withDaemon(
	...
	const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-discord-daemon-"));
...
function inbound(threadId: string, id: string, generation = 1, customId?: string): DiscordInboundEvent {
	return {
		...
		interaction: {
			...
			token: `token-${id}`,
			customId:
				customId ?? actionCustomIds.get(threadId) ?? `skc:${generation}:ask:00000000-0000-0000-0000-000000000000`,
			value: "yes",
		},
	};
}
...
describe("DiscordNotificationDaemon fake-provider acceptance", () => {
				...
				expect(provider.messages[0]?.components).toEqual([
					{
						...
						components: [
							{
								type: 3,
								customId: expect.stringMatching(/^skc:4:ask:[0-9a-f-]{36}$/),
	...
	test("reclaims an expired create fence and commits a terminal create-thread receipt at startup", async () => {
		...
		await withDaemon(
			async (_daemon, provider, agentDir) => {
				const nonce = `skc-${createHash("sha256").update("create:session:recovery-nonce").digest("hex").slice(0, 21)}`;
				...
				await journal.enqueue({
					...
					sessionId: "session",
					endpointGeneration: 1,
					payload: { guildId: "guild", parentId: "parent", name: "SKC session", nonce },
				});
	...
	test("consumes terminal create intents represented by closed or archived mappings and recreates with a fresh effect", async () => {
		for (const state of ["closed", "archived"] as const) {
			await withDaemon(async (_daemon, provider, agentDir) => {
				const nonce = `skc-${createHash("sha256").update("create:session:recovery").digest("hex").slice(0, 21)}`;
				const thread = await provider.createThread({
					guildId: "guild",
					parentId: "parent",
					name: "SKC session",
	...
	test("does not activate a terminal create receipt when another mapping already owns the session", async () => {
		await withDaemon(async (_daemon, provider, agentDir) => {
			const nonce = `skc-${createHash("sha256").update("create:session:stale").digest("hex").slice(0, 21)}`;
	...
	test("recovers a callback-defer intent after the callback lease expires and fences a stale claimer", async () => {
		...
		await withDaemon(
			async (_daemon, provider, agentDir) => {
				...
				const retryCustomId = `skc:1:ask-retry:${retry.pendingActionNonce!}`;
				provider.deferInteraction = async () => {
				...
				const uncertainCustomId = `skc:1:ask-uncertain:${uncertain.pendingActionNonce!}`;
				provider.deferInteraction = async () => {};
	...
	test("reconstructs an uncertain post receipt from its stable nonce without a duplicate", async () => {
		await withDaemon(async (daemon, provider, agentDir) => {
			...
			expect([...provider.messageNonces.keys()][0]).toMatch(/^skc-[a-f0-9]{21}$/);
			const restarted = new DiscordNotificationDaemon({
				agentDir,
				repo: agentDir,
				guildId: "guild",
	...
	test("recovers a dead unexpired inbound lease without another gateway event and cancels it on stop", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-discord-lease-recovery-"));
	...
	test("does not scan conversation mappings below the terminal prune threshold", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-discord-prune-bound-"));
		try {
			const journal = new ChatEffectJournal({ agentDir, transport: "discord" });
			await journal.enqueue({
