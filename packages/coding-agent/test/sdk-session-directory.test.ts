...
import { getAgentDir, getSessionsDir, setAgentDir } from "@sayknow-cli/utils";
import {
...
describe("managed session directory SDK", () => {
	it("uses the configured agent layout for the default sessions root", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-layout-"));
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-layout-cwd-"));
	...
	it.skipIf(process.platform !== "linux")(
		...
		async () => {
			const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-xdg-"));
			const xdgData = path.join(root, "data");
			const agentDir = path.join(os.homedir(), ".skc", "agent");
			const cwd = path.join(root, "workspace");
			temporaryDirectories.push(root);
			await fs.mkdir(path.join(xdgData, "skc"), { recursive: true });
			...
			try {
				...
				expect(resolved).toMatchObject({
					kind: "resolved",
					scope: { sessionsRoot: path.join(xdgData, "skc", "sessions") },
				});
			} finally {
	...
	it("creates managed scope components beneath the configured agent root", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-components-"));
		temporaryDirectories.push(root);
	...
	it("uses distinct fixed-width v2 components for legacy collision vectors", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-directory-"));
		temporaryDirectories.push(root);
		const agentDir = path.join(root, "agent");
	...
	it("fails closed when an existing v2 binding identifies another workspace", async () => {
		...
		await fs.writeFile(
			path.join(resolved.scope.directoryPath, ".skc-managed-session-scope.v2.json"),
			`${JSON.stringify({
				schemaVersion: 1,
				layoutVersion: 2,
				identityVersion: 1,
