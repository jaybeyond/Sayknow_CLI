...
// relative path is used instead of the @sayknow-cli/natives package resolution).
...
function tempAgentDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "skc-rich-e2e-"));
}

/** Isolate getAgentDir() to a temp dir so daemon persistence never touches ~/.skc/agent. */
...
class CapturingBotApi implements BotApi {
	...
	async call(method: string, body: unknown, options?: { noRetry?: boolean; signal?: AbortSignal }): Promise<unknown> {
		...
		switch (method) {
			...
			case "getMe":
				return { ok: true, result: { id: 1, username: "skc_bot" } };
...
test("rich e2e: /btw deterministic lifecycle rotations", async () => {
	const seed = process.env.SKC_RICH_LIFECYCLE_SEED ?? "29691855561";
	expect(seed, "invalid lifecycle seed").toBe("29691855561");
	const iterations = Number(process.env.SKC_RICH_LIFECYCLE_ITERATIONS ?? "5");
