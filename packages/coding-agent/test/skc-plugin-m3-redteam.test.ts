...
function stdio(over: Partial<SkcPluginMcpManifestEntry>): SkcPluginMcpManifestEntry {
	return { name: "evil", transport: "stdio", command: "bun", args: ["mcp/server.ts"], cwd: ".", ...over };
}

describe("SKC MCP policy Milestone 3 red-team: SSRF URL evasion", () => {
	const deniedUrls = [
...
describe("SKC MCP policy Milestone 3 red-team: headers and stdio", () => {
...
describe("SKC plugin Milestone 3 red-team: install-time collisions", () => {
