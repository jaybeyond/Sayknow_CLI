...
import { MAX_EDIT_FILE_BYTES, readEditFileText } from "@sayknow-cli/coding-agent/edit/read-file";
import { executeReadQuery } from "@sayknow-cli/coding-agent/tools/sqlite-reader";
...
async function writeTmp(name: string, bytes: number): Promise<string> {
	const file = path.join(os.tmpdir(), `skc-g008-${Date.now()}-${name}`);
