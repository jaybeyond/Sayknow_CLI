...
describe("build metadata", () => {
	...
	it("uses neutral diagnostic wording for unknown explicit metadata", () => {
		process.env.SKC_BUILD_CHANNEL = "surprise-channel";
	...
	it("classifies local source trees as local source without using dev wording", async () => {
		delete process.env.SKC_BUILD_CHANNEL;
		const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-build-metadata-"));
		await fs.mkdir(path.join(repoRoot, ".git"));
		await Bun.write(path.join(repoRoot, "bun.lock"), "");
		await Bun.write(path.join(repoRoot, "package.json"), JSON.stringify({ name: "sayknow-cli" }));
	...
	it("classifies unmarked user installs as package installs instead of dev", () => {
		...
		expect(resolveBuildMetadata("/opt/homebrew/lib/node_modules/@sayknow-cli/coding-agent/src")).toEqual({
	...
	it("does not classify package installs inside another Bun repo as local source", async () => {
		delete process.env.SKC_BUILD_CHANNEL;
		const consumerRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-consumer-project-"));
		...
		expect(resolveBuildMetadata(path.join(consumerRoot, "node_modules/@sayknow-cli/coding-agent/src"))).toEqual({
			channel: "package-install",
			label: "package install",
		});
	});
