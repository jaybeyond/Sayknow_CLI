...
import {
	...
	resolveTextBlobSync,
} from "@sayknow-cli/coding-agent/session/blob-store";
import {
...
function makeStore(): EphemeralBlobStore {
	return new EphemeralBlobStore(path.join(tmpRoot("skc-resident-"), "resident"));
}
...
describe("SessionManager retained-entry ownership and snapshot correctness", () => {
	function newManager(): SessionManager {
		const root = tmpRoot("skc-session-resident-");
