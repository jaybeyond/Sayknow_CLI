...
import { parseManifest } from "../src/extensibility/skc-plugins/schema";
import { SkcPluginLoadError } from "../src/extensibility/skc-plugins/types";
import { buildSkillPromptMessage } from "../src/extensibility/skills";
import { serializeManifestProjection } from "../src/skc-runtime/workflow-manifest";
...
async function installMixedRootRegistry(): Promise<void> {
	...
	const pluginsDir = path.join(tempHome, ".skc", "plugins");
...
async function installProjectSkillFixture(): Promise<void> {
	await fs.mkdir(path.join(tempCwd, ".skc", "skc-plugins"), { recursive: true });
	await fs.cp(
		path.join(fixturesRoot, "valid-skill-plugin"),
		path.join(tempCwd, ".skc", "skc-plugins", "valid-skill-plugin"),
		{ recursive: true },
	);
}
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugin-home-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-plugin-nosurface-"));
...
describe("SKC plugin roots never surface through legacy claude plugin providers", () => {
	test("sayknow-plugin.json root is excluded from all legacy claude-plugin capability categories", async () => {
		...
		for (const result of [skills, commands, hooks, tools, mcps]) {
			expect(result.warnings.join("\n")).toContain("Skipping sayknow-cli plugin root");
		}
	});

	test("task agent discovery excludes agents from sayknow-plugin.json roots", async () => {
		await installMixedRootRegistry();
		...
		await fs.writeFile(
			...
			[
				"---",
				"name: malicious-skc-agent",
				"description: should not load from sayknow roots",
				"---",
				"Do not load me.",
			].join("\n"),
		);
		...
		expect(result.agents.map(agent => agent.name)).not.toContain("malicious-skc-agent");
	});
	...
	test("parseManifest rejects agents as a forbidden extension surface", () => {
		try {
			parseManifest(
				...
				"/plugin/agents/sayknow-plugin.json",
			);
		} catch (error) {
			expect(error).toBeInstanceOf(SkcPluginLoadError);
			expect((error as SkcPluginLoadError).code).toBe("forbidden_surface");
			return;
		}
	...
	test("phase graph manifest projection is byte-identical across plugin discovery, activation, state persist, and injection", async () => {
		...
		const roots = await discoverSkcPluginRoots({ cwd: tempCwd });
		expect(roots.some(root => root.endsWith(path.join(".skc", "skc-plugins", "valid-skill-plugin")))).toBe(true);
		...
		expect(built.message).toContain("<skc-subskill");
