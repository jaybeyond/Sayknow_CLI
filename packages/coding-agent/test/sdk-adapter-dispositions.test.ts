...
async function fixture(): Promise<AdapterFixture> {
	const repo = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-adapter-dispositions-"));
	const agentDir = path.join(repo, ".skc", "adapter-agent");
...
async function stopFixture(host: AdapterFixture, operation: Operation): Promise<void> {
	...
	await Bun.sleep(500);
	const runtimeAgentDir = path.join(host.repo, ".skc", "agent");
