...
const temp = () => fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-fixture-cleanup-"));
...
describe("fixture broker root cleanup", () => {
	...
	it("sanitizes and restores the production SDK opt-out without contacting operator state", async () => {
		const prior = process.env.SKC_SDK_DISABLE;
		process.env.SKC_SDK_DISABLE = "1";
		try {
			await withFixtureBrokerEnvironment(async () => {
				expect(process.env.SKC_SDK_DISABLE).toBeUndefined();
			});
			expect(process.env.SKC_SDK_DISABLE).toBe("1");
		} finally {
			if (prior === undefined) delete process.env.SKC_SDK_DISABLE;
			else process.env.SKC_SDK_DISABLE = prior;
		}
	});
	...
	it("builds an allowlisted fixture child environment without SDK opt-out state", () => {
		...
		expect(environment.SKC_SDK_DISABLE).toBeUndefined();
	});
