...
import * as themeModule from "@sayknow-cli/coding-agent/modes/theme/theme";
import * as nativesModule from "@sayknow-cli/natives";
import { MacOSAppearance } from "@sayknow-cli/natives";
...
describe("theme auto-detection", () => {
	beforeEach(async () => {
		themeModule.stopThemeWatcher();
		const darkTheme = await themeModule.getThemeByName("red-octopus");
		if (!darkTheme) {
	...
	it("prefers COLORFGBG before macOS fallback inside Zellij", async () => {
		...
		await themeModule.initTheme(false, undefined, undefined, "red-octopus", "blue-octopus");

		expect(themeModule.getCurrentThemeName()).toBe("red-octopus");
	...
	it("keeps honoring terminal-reported appearance outside fallback mode", async () => {
		...
		await themeModule.initTheme(true, undefined, undefined, "red-octopus", "blue-octopus");
	...
	it("restores previewed themes without leaving preview mode active", async () => {
		...
		await themeModule.previewTheme("blue-octopus");
		expect(themeModule.theme.getFgAnsi("accent")).not.toBe(darkAccent);

		await themeModule.restoreThemePreview("red-octopus");
		expect(themeModule.getCurrentThemeName()).toBe("red-octopus");
		expect(themeModule.theme.getFgAnsi("accent")).toBe(darkAccent);

		themeModule.setAutoThemeMapping("dark", "red-octopus");
		await Bun.sleep(0);
		expect(themeModule.getCurrentThemeName()).toBe("red-octopus");
		expect(themeModule.theme.getFgAnsi("accent")).toBe(darkAccent);
	});
	...
	it("restores the latest detected auto theme when terminal appearance changes during preview", async () => {
		...
		await themeModule.previewTheme("red-octopus");
		themeModule.onTerminalAppearanceChange("light");
		await Bun.sleep(0);
		await themeModule.restoreThemePreview("red-octopus");
	...
	it("restores the resolved auto theme after saving the inactive theme slot from preview", async () => {
		...
		await themeModule.previewTheme("blue-octopus");
		themeModule.setAutoThemeMapping("light", "blue-octopus");
		await Bun.sleep(0);
