...
class Bot implements BotApi {
	...
	async call(method: string, body: unknown, options?: { noRetry?: boolean; signal?: AbortSignal }): Promise<unknown> {
		...
		switch (method) {
			...
			case "getMe":
				return { ok: true, result: { username: "skc_bot" } };
...
test("/btw travels through NotificationServer and a real WebSocket with one strict terminal dispatch", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-btw-e2e-"));
	try {
		const settings = isolatedSettings(agentDir);
...
test("/btw generation replacement terminalizes an old pending request exactly once", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-btw-crash-e2e-"));
	try {
		const settings = isolatedSettings(agentDir);
		...
		let server = new NotificationServer(sessionId, "token", path.join(cwd, ".skc", "state"), true);
		installCallbacks(server);
		await server.start();
		const bot = new Bot();
		const daemon = new TelegramNotificationDaemon({
		...
		try {
			...
			server = new NotificationServer(sessionId, "token", path.join(cwd, ".skc", "state"), true);
			installCallbacks(server);
			await server.start();
			await daemon.scanRoots();
			await waitFor(
