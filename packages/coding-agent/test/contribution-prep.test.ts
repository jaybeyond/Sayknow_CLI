...
describe("contribution prep", () => {
	...
	it("writes a manifest with redacted file-pointer artifacts", async () => {
		const tempDir = TempDir.createSync("@skc-contribution-prep-");
		try {
	...
	it("can prepare a worker spawn without mutating source-session identity", async () => {
		const tempDir = TempDir.createSync("@skc-contribution-prep-spawn-");
		try {
	...
	it("resolves worker spawn argv through the SKC command and prompt file", async () => {
		const tempDir = TempDir.createSync("@skc-contribution-prep-real-spawn-");
		try {
