...
describe("status line preview highlight", () => {
	it("clears transient highlight when later settings omit it", () => {
		...
		component.updateSettings({
			...
			rightSegments: [],
			separator: "powerline-thin",
			previewHighlightSegment: "sayknow",
		});
...
describe("path segment truncation at varying maxLength", () => {
	...
	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-overflow-very-long-directory-name-for-testing-"));
...
describe("overflow: path shrinks before git is dropped", () => {
	...
	beforeAll(() => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-overflow-a-very-long-worktree-directory-name-here-"));
		setProjectDir(tmpDir);
	});
	...
	it("shrinks a short path when maxLength exceeds actual path length", () => {
		...
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-short-"));
	...
	it("preserves git when overflow is only 1-2 columns", () => {
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-narrow-ovf-"));
		setProjectDir(shortDir);
		try {
