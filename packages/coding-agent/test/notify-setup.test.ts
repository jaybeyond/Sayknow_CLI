...
function setupSettings(globalSettings: Record<string, unknown> = {}): Settings {
	...
	const agentDir = path.join(os.tmpdir(), `skc-notify-setup-test-${process.pid}-${isolatedSettingsCounter++}`);
...
describe("notify setup cli", () => {
	...
	describe("Telegram setup activation boundary", () => {
		...
		test("notify-cli setup leaves pre-existing root registrations untouched", async () => {
			...
			const existing = {
				version: 1,
				roots: ["/existing/.skc/state"],
				managedRoots: ["/existing/.skc/state"],
				sessions: { existing: "/existing/.skc/state" },
			};
...
test("injected setup preflight overrides ambient foreign daemon state", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-notify-setup-preflight-"));
...
test("ambient setup preflight accepts only matching canonical daemon provenance", async () => {
	const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-notify-setup-incarnation-"));
...
describe("notify daemon-internal lightweight startup", () => {
	function tempAgentDir(): string {
		return fs.mkdtempSync(path.join(os.tmpdir(), "skc-notify-daemon-agent-"));
	}
...
`,
	...
	test("lightweight daemon settings fall back to safe notification defaults", () => {
		const settings = createLightweightDaemonSettings({ agentDir: "/tmp/skc-agent", rawConfig: {} });
		const cfg = getNotificationConfig(settings);
		...
		try {
			await runDaemonInternal(["--owner-id", `12345-${token}`, "--agent-dir", tempAgentDir()], {
				...
				SettingsImpl: {
					async init() {
						settingsLoaded = true;
						return createLightweightDaemonSettings({ agentDir: "/tmp/skc-agent", rawConfig: {} });
					},
				},
				DaemonImpl: class {
					constructor() {
			...
			expect(warning).toHaveBeenCalledWith("SKC notify daemon exiting because its owner is not alive");
