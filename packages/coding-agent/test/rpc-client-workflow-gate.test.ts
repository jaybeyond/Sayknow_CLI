...
import type { RpcExtensionUIRequest, RpcWorkflowGate } from "@sayknow-cli/coding-agent/modes/rpc/rpc-types";
...
async function withFakeServer(source: string, run: (client: RpcClient) => Promise<void>): Promise<void> {
	const scriptPath = path.join(os.tmpdir(), `skc-rpc-workflow-gate-${Date.now()}-${Math.random()}.js`);
