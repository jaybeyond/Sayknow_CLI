...
function entry(name: string, over: Partial<SkcPluginRegistryEntry> = {}): SkcPluginRegistryEntry {
	return {
		name,
		version: "1.0.0",
		scope: "project",
...
describe("plugin prompt appendices", () => {
	test("renders lower-authority system + agent appendix blocks from an installed bundle", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-appx-"));
		tempDirs.push(cwd);
		...
		expect(rendered.system).toContain("<skc-plugin-system-appendix");
		expect(rendered.system).toContain('authority="appendix-lower-than-system"');
		expect(rendered.system).toContain("Domain policy");
		expect(rendered.byAgent.get("executor")).toContain("<skc-plugin-agent-appendix");
		expect(rendered.digest).toMatch(/^[0-9a-f]{64}$/);
	});

	test("digest changes when appendix content changes", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-appx2-"));
		tempDirs.push(cwd);
		await installSkcPluginBundle(sixSurface, { scope: "project", cwd });
		const before = (await renderPluginAppendices(await loadEffectiveSkcPluginRegistry(cwd))).digest;
		const modified = await fs.mkdtemp(path.join(os.tmpdir(), "skc-appx-mod-"));
		...
		await installSkcPluginBundle(modified, { scope: "project", cwd, force: true });
		const after = (await renderPluginAppendices(await loadEffectiveSkcPluginRegistry(cwd))).digest;
...
describe("Tier-1 subskill advertisement", () => {
	...
	test("renders bounded metadata-only block for the target parent", () => {
		...
		expect(block).toContain("<skc-plugin-subskill-advertisement");
		expect(block).toContain('activation_arg="design"');
		expect(block).not.toContain("<skc-subskill"); // no Tier-2 body
	});
...
describe("system prompt appendix integration", () => {
	test("buildSystemPrompt appends plugin appendices as a trailing lower-authority block", async () => {
		const block =
			'<skc-plugin-system-appendix plugin="p" name="x" authority="appendix-lower-than-system">domain policy text</skc-plugin-system-appendix>';
		const res = await buildSystemPrompt({ cwd: process.cwd(), pluginAppendices: block });
		const joined = res.systemPrompt.join("\n\n");
		expect(joined).toContain("<skc-plugin-system-appendix");
		expect(joined).toContain("domain policy text");
		// Appendix is last (lower authority than everything above it).
		expect(joined.trimEnd().endsWith("</skc-plugin-system-appendix>")).toBe(true);
	});
	...
	test("buildSystemPrompt omits the appendix block when none provided", async () => {
		...
		expect(res.systemPrompt.join("\n\n")).not.toContain("<skc-plugin-system-appendix");
	});
});
...
describe("M5 blocker fixes", () => {
	...
	test("parseManifest rejects unknown agent-appendix agent with invalid_parent", async () => {
		const { SkcPluginLoadError, parseManifest } = await import("../src/extensibility/skc-plugins");
		try {
			parseManifest(
				{
			...
			expect((error as InstanceType<typeof SkcPluginLoadError>).code).toBe("invalid_parent");
		}
	});
});
