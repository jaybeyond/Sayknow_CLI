...
const LIFECYCLE_ENV = ["SKC_LIFECYCLE_REQUEST_ID", "SKC_SESSION_ID"] as const;
...
test("normal root launch creates a current SessionManager for root token logs", async () => {
	const agentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "skc-root-token-session-"));
...
// bare `skc` launch with SKC_SESSION_ID/SKC_LIFECYCLE_REQUEST_ID. With autoResume
...
test("lifecycle /session_create bypasses autoResume; normal launch still resumes", async () => {
	const agentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "skc-lc-autoresume-"));
	setAgentDir(agentDir);
	const cwd = path.join(agentDir, "repo");
	fs.mkdirSync(cwd, { recursive: true });
	...
	process.env.SKC_LIFECYCLE_REQUEST_ID = "lc-autoresume-1";
	process.env.SKC_SESSION_ID = "s-prealloc-autoresume-1";
	const created = await createSessionManager({} as Args, cwd, settings);
