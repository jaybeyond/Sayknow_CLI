...
import type { CasReceipt } from "@sayknow-cli/coding-agent/config/atomic-yaml-patch";
...
} from "@sayknow-cli/coding-agent/modes/components/notifications-settings-editor";
...
} from "@sayknow-cli/coding-agent/modes/controllers/selector-controller";
import { initTheme } from "@sayknow-cli/coding-agent/modes/theme/theme";
import { getNotificationConfig, type NotificationSettingsSnapshot } from "@sayknow-cli/coding-agent/sdk/bus/config";
import {
	createTelegramActivationMarker,
	telegramActivationIdentity,
} from "@sayknow-cli/coding-agent/sdk/bus/notification-orchestration";
import type {
	NotificationHealthReport,
	NotificationRecoveryReport,
} from "@sayknow-cli/coding-agent/sdk/bus/notification-service";
import type {
	NotificationSessionReconcileResult,
	NotificationSessionStatus,
} from "@sayknow-cli/coding-agent/sdk/bus/session-control";
...
describe("notification settings controller adapter", () => {
	it("maps editor operations to notification services, session control, and atomic commits", async () => {
		...
		const settings = {
			getAgentDir: () => "/tmp/skc-settings-controller",
		...
		const dependencies: Partial<NotificationsEditorOperationDependencies> = {
			...
			recoverNotifications: async input => {
				...
				expect(input).toMatchObject({ settings, stateRoot: path.join("/workspace/current", ".skc", "state") });
				return recovery();
			},
			unregisterNotificationRoot: async () => ({ root: "/workspace/current/.skc/state", remainingRoots: 1 }),
		...
		expect(healthCalls).toContainEqual(
			expect.objectContaining({ stateRoot: path.join("/workspace/current", ".skc", "state"), probe: true, signal }),
		);
