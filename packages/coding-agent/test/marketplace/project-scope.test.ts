 ...
 *
 * Note: helpers.ts imports @sayknow-cli/natives (Rust addon via glob).
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-proj-scope-"));
	});
	...
	it("walk-up finds nearest .skc/ directory", async () => {
		// Layout: tmpDir/.skc/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .skc/ there.
		fs.mkdirSync(path.join(tmpDir, ".skc"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".skc", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .skc/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.skc/   +   tmpDir/sub/.skc/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.skc/, not climb further to tmpDir/.skc/.
		fs.mkdirSync(path.join(tmpDir, ".skc"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".skc"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".skc", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .skc/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .skc/ anywhere → second pass finds .git/ at tmpDir.
		// Returned path is relative to the .git root, not .git itself.
		fs.mkdirSync(path.join(tmpDir, ".git"), { recursive: true });
		const cwd = path.join(tmpDir, "sub");
		fs.mkdirSync(cwd, { recursive: true });

		// Bound the walk-up at os.tmpdir() so a `.skc/` polluting a shared ancestor
		// (e.g. a /tmp/.skc left by other processes on CI runners) cannot shadow the
	...
	it("returns null when neither .skc/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .skc/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.skc/.../installed_plugins.json.
		...
		try {
			// Start from a tmpDir that has no .skc/ or .git/ of its own.
			const result = await resolveActiveProjectRegistryPath(tmpDir);
			// Must not resolve to the home-dir SKC registry.
			const homeSkcPath = path.join(homeDir, ".skc", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeSkcPath);
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listAnthropic modelPluginRoots reads the user SKC registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "skc-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "skc-shadow-proj-"));

		// Create .skc/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".skc", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".skc", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".skc", "plugins", "installed_plugins.json");
	});
