...
import { substitutePluginRoot } from "@sayknow-cli/coding-agent/discovery/substitute-plugin-root";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces SKC_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${SKC_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${SKC_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
