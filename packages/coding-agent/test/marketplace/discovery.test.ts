/**
 * Discovery integration tests for SKC plugin registry reading.
 *
 * NOTE: listAnthropic modelPluginRoots() lives in discovery/helpers.ts which imports
 * @sayknow-cli/natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listAnthropic modelPluginRoots
 * depends on:
 *   1. SKC registry lives at path.join(home, ".skc", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".skc")
...
import type { InstalledPluginEntry } from "@sayknow-cli/coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@sayknow-cli/coding-agent/extensibility/plugins/marketplace";

// ── Inline validator ───────────────────────────────────────────────────────────
//
// Mirrors parseAnthropic modelPluginsRegistry() in discovery/helpers.ts exactly.
// Kept here to avoid importing helpers.ts (which pulls in @sayknow-cli/natives).
...
// Matches getConfigDirName() — single source of truth is in @sayknow-cli/utils,
// but we know the value is ".skc" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const SKC_CONFIG_DIR = ".skc";
...
let tmpHome: string;
/** ~/.skc/plugins/installed_plugins.json inside tmpHome */
let skcRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "skc-discovery-test-"));
	skcRegistryPath = path.join(tmpHome, SKC_CONFIG_DIR, "plugins", "installed_plugins.json");
	fs.mkdirSync(path.dirname(skcRegistryPath), { recursive: true });
});
...
describe("SKC registry path contract", () => {
	it("SKC registry lives at home/.skc/plugins/installed_plugins.json", () => {
		// This is the path that listAnthropic modelPluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".skc", "plugins", "installed_plugins.json");
		expect(skcRegistryPath).toBe(expected);
	});

	it("SKC config dir name is .skc", () => {
		...
		expect(SKC_CONFIG_DIR).toBe(".skc");
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("SKC registry format compatibility with Claude parser", () => {
	it("empty registry written by writeInstalledPluginsRegistry passes validator", async () => {
		await writeInstalledPluginsRegistry(skcRegistryPath, { version: 2, plugins: {} });

		const content = fs.readFileSync(skcRegistryPath, "utf8");
	...
	it("registry with installed plugin passes validator", async () => {
		...
		let reg = await readInstalledPluginsRegistry(skcRegistryPath);
		reg = addInstalledPlugin(reg, pluginId, entry);
		await writeInstalledPluginsRegistry(skcRegistryPath, reg);
...
describe("SKC registry round-trip", () => {
	it("reads back what was written — single plugin", async () => {
		...
		const readBack = await readInstalledPluginsRegistry(skcRegistryPath);
...
//
// listAnthropic modelPluginRoots must replace Anthropic model entries with SKC entries when the same
...
describe("SKC precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — SKC entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// SKC entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const skcEntry = makeEntry("/skc/cached/path");

		// SKC registry entry has installPath (required by listAnthropic modelPluginRoots)
		expect(skcEntry.installPath).toBeTruthy();
		expect(typeof skcEntry.installPath).toBe("string");
