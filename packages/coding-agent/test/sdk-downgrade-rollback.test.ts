...
const temp = () => fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-sdk-rollback-"));
...
test("v1 SDK state transforms to a rollback directory and is executable by the pinned pre-Phase-B notifications product", async () => {
	...
	expect(manifest.artifactPath).toBe("packages/coding-agent/dist/skc-rollback");
