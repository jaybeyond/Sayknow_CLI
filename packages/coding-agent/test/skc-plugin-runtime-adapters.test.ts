...
import { installSkcPluginBundle, loadAlwaysOnPluginTools } from "../src/extensibility/skc-plugins";
...
async function mkCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-rt-"));
	tempDirs.push(cwd);
	return cwd;
}
...
describe("always-on plugin tool runtime activation", () => {
	...
	test("quarantines on installed-file hash drift", async () => {
		const cwd = await mkCwd();
		await installSkcPluginBundle(sixSurface, { scope: "project", cwd });
		const installed = path.join(cwd, ".skc", "skc-plugins", "valid-six-surface-bundle", "tools", "domain-note.ts");
	...
	test("quarantines runtime_mismatch when factory name != declared name", async () => {
		const cwd = await mkCwd();
		const src = await fs.mkdtemp(path.join(os.tmpdir(), "skc-mismatch-"));
		tempDirs.push(src);
		...
		await fs.writeFile(
			path.join(src, "sayknow-plugin.json"),
			JSON.stringify({
		...
		await installSkcPluginBundle(src, { scope: "project", cwd });
		const res = await loadAlwaysOnPluginTools({ cwd, reservedToolNames: [] });
