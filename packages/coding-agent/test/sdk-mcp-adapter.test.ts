...
function fixture() {
	const repo = fs.mkdtempSync(path.join(os.tmpdir(), "skc-sdk-mcp-"));
	dirs.push(repo);
	...
	const dir = path.join(repo, ".skc", "state", "sdk");
...
test("MCP SDK schemas exclude endpoint credentials and reject G02 before any WebSocket send", async () => {
	...
	await expect(mcp.callTool("skc_session_control", { sessionId, operation: "session.get_endpoint" })).resolves.toEqual(
		{ ok: false, error: expect.objectContaining({ code: "unknown_operation" }) },
	);
	await expect(mcp.callTool("skc_session_global", { operation: "session.get_endpoint" })).resolves.toEqual({
		ok: false,
...
test("MCP lifecycle responses never expose broker endpoint credentials", async () => {
	...
	const result = await mcp.callTool("skc_session_global", {
...
test("MCP global schema exposes and requires caller lifecycle idempotency keys", async () => {
	const { repo } = fixture();
	const mcp = createSdkMcpServer({ repo });
	const global = mcp.tools.find(tool => tool.name === "skc_session_global")!;
	expect(global.inputSchema).toMatchObject({ properties: { idempotencyKey: { type: "string" } } });
	await expect(
		mcp.callTool("skc_session_global", { operation: "session.create", input: { cwd: repo } }),
...
test("MCP rejects unknown operation names before discovery or connection", async () => {
	...
	for (const [tool, args] of [
		["skc_session_control", { sessionId, operation: "not.real" }],
		["skc_session_query", { sessionId, query: "not.real" }],
		["skc_session_global", { operation: "not.real" }],
	] as const)
...
test("MCP preserves corrupt endpoint discovery errors with a relative path", async () => {
	const { repo, sessionId } = fixture();
	const endpoint = path.join(repo, ".skc", "state", "sdk", `${sessionId}.json`);
	fs.writeFileSync(endpoint, "not-json");
	const result = await createSdkMcpServer({ repo }).callTool("skc_session_query", {
		sessionId,
...
test("MCP rejects every registry-prohibited operation without sending a frame or exposing secret input", async () => {
	...
	for (const operation of blocked) {
		const tool = operation.kind === "global" ? "skc_session_global" : "skc_session_control";
		const args =
...
test("MCP rejects secret-bearing config patches before endpoint discovery", async () => {
	const { repo, sessionId, sent } = fixture();
	const result = await createSdkMcpServer({ repo }).callTool("skc_session_control", {
		sessionId,
...
test("MCP SDK control/query tools use discovered live session endpoints and unknown sessions are typed", async () => {
	...
	const mcp = createSdkMcpServer({ repo });
	await expect(
		mcp.callTool("skc_session_control", { sessionId, operation: "turn.prompt", input: { text: "hello" } }),
	).resolves.toMatchObject({ ok: true, echoed: { operation: "turn.prompt" } });
	await expect(
		mcp.callTool("skc_session_query", { sessionId, query: "session.metadata", cursor: "next" }),
	).resolves.toMatchObject({ ok: true, echoed: { query: "session.metadata", cursor: "next" } });
	await expect(
		mcp.callTool("skc_session_query", { sessionId: "missing", query: "session.metadata" }),
