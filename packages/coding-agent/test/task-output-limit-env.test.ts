...
async function readTaskOutputLimits(
	...
	delete env.SKC_TASK_MAX_OUTPUT_BYTES;
	delete env.PI_TASK_MAX_OUTPUT_BYTES;
	delete env.SKC_TASK_MAX_OUTPUT_LINES;
	delete env.PI_TASK_MAX_OUTPUT_LINES;
	Object.assign(env, overrides);

	let tempRoot: string | undefined;
	if (options.dotenv) {
		tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-task-output-env-"));
		const home = path.join(tempRoot, "home");
		const configRoot = path.join(home, ".skc");
		...
		env.HOME = home;
		env.SKC_CONFIG_DIR = ".skc";
		env.SKC_CODING_AGENT_DIR = agentDir;
...
describe("task output limit environment parsing", () => {
	...
	it("accepts complete positive decimal integers from canonical variables", async () => {
		expect(
			await readTaskOutputLimits({
				SKC_TASK_MAX_OUTPUT_BYTES: "00064000",
				SKC_TASK_MAX_OUTPUT_LINES: "250",
			}),
			...
			await readTaskOutputLimits(
				{},
				{
					dotenv: {
						location,
						values: {
							SKC_TASK_MAX_OUTPUT_BYTES: "64000",
							SKC_TASK_MAX_OUTPUT_LINES: "250",
						},
					},
				},
	...
	it.each([
		...
		expect(
			await readTaskOutputLimits({
				SKC_TASK_MAX_OUTPUT_BYTES: value,
				SKC_TASK_MAX_OUTPUT_LINES: value,
			}),
		).toEqual(defaults);
	});
});
