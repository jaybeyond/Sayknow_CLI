...
import type { RpcExtensionUIRequest, RpcUnattendedDeclaration } from "@sayknow-cli/coding-agent/modes/rpc/rpc-types";
...
async function withFakeServer(source: string, run: (client: RpcClient) => Promise<void>): Promise<void> {
	const scriptPath = path.join(os.tmpdir(), `skc-rpc-workflow-gate-redteam-${Date.now()}-${Math.random()}.js`);
