...
describe("ACP builtin slash commands", () => {
	...
	it("model: reports only default plus SKC role-agent assignment targets", async () => {
		const { output, runtime } = createRuntime();
	...
	it("model: assigns a known model to a SKC role-agent target without switching active model", async () => {
		const { output, runtime, session } = createRuntime();
...
describe("wave 4 commands", () => {
	// /mcp is intentionally not an ACP builtin in sayknow-cli. MCP-compatible
