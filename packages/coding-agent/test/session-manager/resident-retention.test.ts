...
import type { ImageContent, Message, ProviderPayload, TextContent } from "@sayknow-cli/ai";
import { MemoryBlobStore } from "@sayknow-cli/coding-agent/session/blob-store";
import { type PreparedNewSession, SessionManager } from "@sayknow-cli/coding-agent/session/session-manager";
import {
	MemorySessionStorage,
	type SessionStorage,
	type SessionStorageWriter,
} from "@sayknow-cli/coding-agent/session/session-storage";
import { getAgentDir, getBlobsDir, setAgentDir } from "@sayknow-cli/utils";
...
function isolateBlobDir(): void {
	if (!tempAgentDir) tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-session-blob-regression-"));
