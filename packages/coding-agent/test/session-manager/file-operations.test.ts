...
import { getConfigRootDir, getSessionsDir, getTerminalSessionsDir, Snowflake, setAgentDir } from "@sayknow-cli/utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-session-dir-test-"));
	...
	it("stores symlink-equivalent home cwd sessions in the v2 resolver directory", () => {
		...
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "skc-session-home-"));
		const nestedDir = path.join(realProjectDir, "nested");
		const aliasRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skc-session-home-alias-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skc-session-manager-legacy-"));
	});
