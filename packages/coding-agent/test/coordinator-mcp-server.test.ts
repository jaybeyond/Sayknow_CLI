...
async function managedFixtureRoot(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-coordinator-managed-broker-"));
...
function createBrokerTestServer(root: string, services: BrokerTestServices) {
	return createCoordinatorMcpServer({
		env: {
			...
			SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "coordinator-state"),
...
async function createSdkControlServer(
	...
	const stateRoot = path.join(root, ".skc", "coordinator-state");
	...
	const server = createCoordinatorMcpServer({
		env: {
			...
			SKC_COORDINATOR_MCP_PROFILE: "local",
			SKC_COORDINATOR_MCP_REPO: "repo",
			...(sessionCommand ? { SKC_COORDINATOR_MCP_SESSION_COMMAND: sessionCommand } : {}),
			...(serverOptions.promptAckTimeoutMs === undefined
				? {}
				: { SKC_COORDINATOR_MCP_PROMPT_ACK_TIMEOUT_MS: String(serverOptions.promptAckTimeoutMs) }),
		},
				...
				({
					...
					global: async (
						...
						if (operation === "session.create") {
							...
							await fs.mkdir(path.join(sessionCwd, ".skc", "state", "sdk"), { recursive: true });
							await Bun.write(
								path.join(sessionCwd, ".skc", "state", "sdk", `${sessionId}.json`),
	...
	await fs.mkdir(path.join(root, ".skc", "state", "sdk"), { recursive: true });
	...
	await Bun.write(
		path.join(root, ".skc", "state", "sdk", "visible-session.json"),
...
async function registerSdkSession(server: ReturnType<typeof createCoordinatorMcpServer>, root: string) {
	return await server.callTool("skc_coordinator_register_session", {
...
describe("Coordinator MCP canonical SDK controls", () => {
	it("uses agent-global SDK discovery and returns credential-free broker status", async () => {
		...
		await Bun.write(
			path.join(root, ".skc", "coordinator-state", "local", "repo", "sessions", "visible-session.json"),
			JSON.stringify({
				session_id: "visible-session",
				cwd: root,
		...
		await Bun.write(
			path.join(root, ".skc", "coordinator-state", "local", "repo", "session-states", "visible-session.json"),
			JSON.stringify({
				schema_version: 1,
				session_id: "visible-session",
		...
		const status = await server.callTool("skc_coordinator_read_status", { session_id: "visible-session" });
	...
	it("normalizes camelCase runtime acknowledgement identities into durable and public turns", async () => {
		...
		const persisted = JSON.parse(
			await fs.readFile(
				path.join(root, ".skc", "coordinator-state", "local", "repo", "turns", `${turnId}.json`),
				"utf8",
			),
	...
	it("accepts drive-letter and separator differences through the injected Windows platform seam", async () => {
		...
		expect(await server.callTool("skc_coordinator_read_status", { session_id: "visible-session" })).toMatchObject({
			ok: true,
			status: { live: true },
		});
		expect(
	...
	it("fails closed before turn persistence for malformed acknowledgement envelopes and conflicting aliases", async () => {
		...
		await expect(
			fs.readdir(path.join(root, ".skc", "coordinator-state", "local", "repo", "turns")),
	...
	it("derives aggregate liveness from scoped broker records", async () => {
		...
		const status = await server.callTool("skc_coordinator_read_status");
	...
	it("reads bounded tail output through the SDK", async () => {
		...
		await expect(
			server.callTool("skc_coordinator_read_tail", { session_id: "visible-session", lines: 1 }),
	...
	it("returns SDK query failures without a terminal fallback", async () => {
		...
		await expect(
			server.callTool("skc_coordinator_read_tail", { session_id: "visible-session" }),
	...
	it("reads active-turn status through SDK context", async () => {
		...
		await expect(server.callTool("skc_coordinator_read_turn", { turn_id: sent.turn_id })).resolves.toMatchObject({
			ok: true,
	...
	it("uses the generation-bound broker endpoint when a stale local endpoint file is absent", async () => {
		...
		await fs.rm(path.join(root, ".skc", "state", "sdk", "visible-session.json"));
	...
	it("passes a resolved mpreset into the SDK lifecycle create request and persists it with the session", async () => {
		...
		await expect(
			fs.readFile(
				path.join(root, ".skc", "coordinator-state", "local", "repo", "sessions", "created-session-1.json"),
				"utf8",
			),
	...
	it("translates the documented SKC worktree command into a typed SDK lifecycle target", async () => {
		const root = await tempRoot();
		const controls: SdkControl[] = [];
		const server = await createSdkControlServer(
			root,
	...
	it("rejects unsupported session-command flags rather than silently ignoring them", async () => {
		...
		const server = await createSdkControlServer(
			...
			undefined,
			undefined,
			"skc --worktree --model provider/model",
		);

		await expect(
			server.callTool("skc_coordinator_start_session", {
				cwd: root,
	...
	it("durably replays sequential prompt retries and rejects caller-key request conflicts", async () => {
		...
		const replay = await server.callTool("skc_coordinator_send_prompt", {
			session_id: "visible-session",
			prompt: "retry-safe prompt",
			idempotency_key: "same-prompt-key",
			allow_mutation: true,
		...
		await expect(
			server.callTool("skc_coordinator_send_prompt", {
				session_id: "visible-session",
	...
	it("serializes concurrent same-key retries into one durable turn", async () => {
		...
		const [first, replay] = await Promise.all([
			server.callTool("skc_coordinator_send_prompt", request),
			server.callTool("skc_coordinator_send_prompt", request),
		]);
		expect(replay).toEqual(first);
		expect(lifecycleControls(controls).filter(control => control.operation === "turn.prompt")).toHaveLength(1);
	...
	it("replays composite start and report mutations without allocating another turn or report", async () => {
		...
		const started = await server.callTool("skc_coordinator_start_session", startArgs);
		const replayedStart = await server.callTool("skc_coordinator_start_session", startArgs);
		...
		const delegated = await server.callTool("skc_delegate_execute", delegateArgs);
		const replayedDelegate = await server.callTool("skc_delegate_execute", delegateArgs);
		...
		const report = await server.callTool("skc_coordinator_report_status", reportArgs);
		const replayedReport = await server.callTool("skc_coordinator_report_status", reportArgs);
		expect(replayedReport).toEqual(report);
		await expect(server.callTool("skc_coordinator_read_coordination_status")).resolves.toMatchObject({
			summary: { reports: 1 },
		});
	});
	it("fails closed when a same-generation successor has a different endpoint incarnation", async () => {
		...
		await expect(
			server.callTool("skc_coordinator_stop_session", {
				session_id: "visible-session",
				allow_mutation: true,
			}),
	...
	it("does not return successor credentials after a same-generation restart between list and endpoint retrieval", async () => {
		...
		const result = await server.callTool("skc_coordinator_send_prompt", {
			session_id: "visible-session",
	...
	it("fails closed on corrupt or crash-left coordinator idempotency records", async () => {
		...
		expect(
			await fs.readdir(path.join(root, ".skc", "coordinator-state", "local", "repo", "reports")).catch(() => []),
		).toEqual([]);
	...
	it("fails closed on workspace and endpoint-generation binding changes", async () => {
		...
		await expect(
			server.callTool("skc_coordinator_register_session", {
		...
		await expect(
			server.callTool("skc_delegate_execute", {
				cwd: otherWorkspace,
				session_id: "visible-session",
	...
	it("uses an incarnation-bound close key for each reaped session incarnation", async () => {
		...
		for (const [registrationKey, endpointMtimeMs] of [
			...
			await expect(
				server.callTool("skc_coordinator_stop_session", { session_id: "visible-session", allow_mutation: true }),
	...
	it("never returns credential-contaminated reused session records", async () => {
		...
		const delegated = await server.callTool("skc_delegate_plan", {
			cwd: root,
			session_id: "visible-session",
	...
	it("routes prompts, follow-ups, abort-and-prompts, and answers through SDK controls with caller keys", async () => {
		...
		const persistedQueuedTurn = JSON.parse(
			await fs.readFile(
				path.join(root, ".skc", "coordinator-state", "local", "repo", "turns", `${queuedTurnId}.json`),
				"utf8",
			),
		) as { delivery: Record<string, unknown> };
	...
	it("materializes a legal two-page Q12 snapshot on one connection and submits its bound shared-producer answer", async () => {
		...
		const listed = await server.callTool("skc_coordinator_list_questions", { session_id: "visible-session" });
		...
		const replay = await server.callTool("skc_coordinator_submit_question_answer", {
			session_id: "visible-session",
			turn_id: sent.turn_id,
			question_id: "gate-q12",
			answer_binding: question.answer_binding,
	...
	it("rejects malformed complete Q12 snapshots without mutating question authority", async () => {
		...
		if (typeof runtimeAcknowledgement.turn_id !== "string") throw new Error("missing runtime turn id");
		runtimeTurnId = runtimeAcknowledgement.turn_id;
		const first = await server.callTool("skc_coordinator_list_questions", { session_id: "visible-session" });
		const second = await server.callTool("skc_coordinator_list_questions", { session_id: "visible-session" });
	...
	it("delivers every delegation workflow through broker lifecycle and SDK control", async () => {
		...
		for (const [tool, key] of [
			["skc_delegate_plan", "plan"],
			["skc_delegate_execute", "execute"],
			["skc_delegate_team", "team"],
		] as const) {
	...
	it("returns immediately by default and exposes bounded delegation completion when requested", async () => {
		...
		const controls: SdkControl[] = [];
		const server = await createSdkControlServer(root, controls);
		const immediate = await server.callTool("skc_delegate_plan", {
			cwd: root,
			task: "immediate",
		...
		const awaited = await server.callTool("skc_delegate_execute", {
			cwd: root,
			task: "timeout",
	...
	it("keeps coordinator metadata reports and event journals available without turning them into control authority", async () => {
		...
		const server = await createSdkControlServer(root, controls);
		await registerSdkSession(server, root);
		const report = await server.callTool("skc_coordinator_report_status", {
			session_id: "visible-session",
			status: "blocked",
	...
	it("closes an idle ephemeral coordinator session through incarnation-bound broker lifecycle authority", async () => {
		...
		expect(
			await server.callTool("skc_coordinator_stop_session", {
				session_id: "visible-session",
				allow_mutation: true,
			}),
	...
	it("idle reaping selects only stale ephemeral coordinator records and uses incarnation-bound session.close", async () => {
		...
		const sessionsDir = path.join(root, ".skc", "coordinator-state", "local", "repo", "sessions");
		...
		await fs.rm(path.join(root, ".skc", "coordinator-state", "local", "repo", "session-states", "idle-session.json"));
		await Bun.write(
	...
	describe("Coordinator MCP real broker lifecycle", () => {
		...
		it("reuses a live broker discovery without replacing its identity", async () => {
			...
			try {
				...
				const result = await createRealBrokerServer(root, agentDir).callTool("skc_coordinator_list_sessions", {});
				expect(result).toMatchObject({ ok: true, sessions: [] });
		...
		it("routes concurrent first calls through one canonical broker owner", async () => {
			...
			try {
				...
				const results = await Promise.all([
					server.callTool("skc_coordinator_list_sessions", {}),
					server.callTool("skc_coordinator_list_sessions", {}),
				]);
				...
				const discovery = await readBrokerDiscovery(agentDir);
				expect(discovery).not.toBeNull();
				await expect(server.callTool("skc_coordinator_list_sessions", {})).resolves.toMatchObject({
					ok: true,
					sessions: [],
				});
				expect(brokerOwnerForTest(agentDir)).toBe(owner);
	...
	it("maps injected broker failures by the explicit operational phase", async () => {
		...
		for (const testCase of cases) {
			...
			const result = await server.callTool("skc_coordinator_list_sessions", {});
		...
		await expect(nullServer.callTool("skc_coordinator_list_sessions", {})).resolves.toMatchObject({
			ok: false,
	...
	it("attempts close once and preserves the primary request failure", async () => {
		...
		await expect(closeFailureServer.callTool("skc_coordinator_list_sessions", {})).resolves.toMatchObject({
			ok: false,
