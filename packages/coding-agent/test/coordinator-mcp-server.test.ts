...
} from "../src/skc-runtime/session-state-sidecar";
import { createOwnerIntent, replaceOwnerGeneration } from "../src/skc-runtime/tmux-owner-isolation";

const tempDirs: string[] = [];
const ORIGINAL_RUNTIME_STATE_FILE = process.env[SKC_COORDINATOR_SESSION_STATE_FILE_ENV];
const ORIGINAL_RUNTIME_SESSION_ID = process.env[SKC_COORDINATOR_SESSION_ID_ENV];

async function tempRoot(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-coordinator-server-"));
	tempDirs.push(dir);
	return dir;
}
...
async function writeReadyMarkerFromTmuxLaunch(command: string[]): Promise<void> {
	await writeReadyMarker({
		sessionId: launchEnvValue(command, "SKC_COORDINATOR_SESSION_ID"),
		launchId: launchEnvValue(command, "SKC_COORDINATOR_SESSION_LAUNCH_ID"),
		readinessMarkerFile: launchEnvValue(command, "SKC_COORDINATOR_SESSION_READINESS_FILE"),
	});
}
...
const PRIVATE_SOCKET = "skc-test-private-socket";
...
function safeCoordinatorOwnerProbe() {
	return {
		readCallerCgroup: async () => "0::/skc-coordinator-test.scope\n",
		probeServer: async () => ({
			state: "safe" as const,
			pid: PRIVATE_OWNER_PID,
			startTime: PRIVATE_OWNER_START_TIME,
...
async function mpresetLaunchCommandRunner(commands: string[][], command: string[]) {
	...
	if (tmuxSubcommand(command) === "if-shell")
		return { exitCode: 0, stdout: "__skc_coordinator_cleanup_ok__\n", stderr: "" };
...
afterEach(async () => {
	if (ORIGINAL_RUNTIME_STATE_FILE === undefined) delete process.env[SKC_COORDINATOR_SESSION_STATE_FILE_ENV];
	else process.env[SKC_COORDINATOR_SESSION_STATE_FILE_ENV] = ORIGINAL_RUNTIME_STATE_FILE;
	if (ORIGINAL_RUNTIME_SESSION_ID === undefined) delete process.env[SKC_COORDINATOR_SESSION_ID_ENV];
	else process.env[SKC_COORDINATOR_SESSION_ID_ENV] = ORIGINAL_RUNTIME_SESSION_ID;
...
describe("Coordinator MCP server protocol", () => {
	...
	it("initializes with SKC coordinator server identity and lists SKC-named tools", async () => {
		...
		expect(initialized.result.serverInfo.name).toBe("skc-coordinator-mcp");
	...
	it("does not read ambient coordinator MCP env when explicit env is provided", async () => {
		...
		const original = process.env.SKC_COORDINATOR_MCP_MUTATIONS;
		process.env.SKC_COORDINATOR_MCP_MUTATIONS = "sessions";
		try {
			const server = createCoordinatorMcpServer({ env: { SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root } });
			const response = await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true });
			expect(response).toEqual({ ok: false, reason: "coordinator_mutation_class_disabled" });
		} finally {
			if (original === undefined) {
				delete process.env.SKC_COORDINATOR_MCP_MUTATIONS;
			} else {
				process.env.SKC_COORDINATOR_MCP_MUTATIONS = original;
			}
		}
	});
	...
	it("fails closed for mutating calls unless startup and per-call mutation are both enabled", async () => {
		...
		const enabledServer = createCoordinatorMcpServer({
			env: { SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root, SKC_COORDINATOR_MCP_MUTATIONS: "sessions" },
		});
	...
	it("rejects unsafe visible session registration before tmux inspection", async () => {
		...
		expect(
			await server.callTool("skc_coordinator_register_session", {
				session_id: "../bad",
				cwd: root,
	...
	it("refuses prompt delivery for registered sessions without persisted private owner proof", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "primary-enter-token");
		...
		const server = createCoordinatorMcpServer({
			env: {
				...
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
			},
			services: {
		...
		await server.callTool("skc_coordinator_send_prompt", {
			session_id: "visible-session",
	...
	it("rejects pane identity proof from a different tmux session", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "wrong-tmux-session");
		const deliveryCommands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const response = await server.callTool("skc_coordinator_send_prompt", {
			session_id: "visible-session",
			prompt: "-primary submit token",
			allow_mutation: true,
		});
	...
	it.each([
		...
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", `owner-proof-${proofState}`);
		...
		await persistPrivateOwnerProof(stateRoot, "visible-session");
		const prompt = await server.callTool("skc_coordinator_send_prompt", {
			session_id: "visible-session",
	...
	it("registers a visible tmux session and submits prompts with tmux Enter", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "visible-register");
		...
		const registered = await server.callTool("skc_coordinator_register_session", {
			session_id: "visible-session",
			cwd: root,
			tmux_session: "visible-session",
			tmux_target: "visible-session:0.0",
	...
	it("fails tmux-delivered turns that never receive a runtime prompt ack", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "unacknowledged-delivery");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
				...
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
				SKC_COORDINATOR_MCP_PROMPT_ACK_TIMEOUT_MS: "1",
			},
			services: {
				ownerIsolationProbe: privateOwnerProbe(),
		...
		await Bun.sleep(5);
		const read = await server.callTool("skc_coordinator_read_turn", {
			session_id: "visible-session",
		...
		const status = await server.callTool("skc_coordinator_read_coordination_status");
		...
		const delegated = await server.callTool("skc_delegate_execute", {
			cwd: root,
	...
	it("redacts private owner controls from coordinator public session responses while routing internally", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "public-owner-redaction");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const start = await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true });
		const sessionId = (start.session as { session_id: string }).session_id;
		const outputs = [
			start,
			await server.callTool("skc_coordinator_list_sessions"),
			await server.callTool("skc_coordinator_read_status", { session_id: sessionId }),
			await server.callTool("skc_coordinator_read_coordination_status"),
			await server.callTool("skc_delegate_execute", {
				cwd: root,
	...
	it("deletes tmux prompt buffers when paste delivery fails", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "paste-buffer-failure-cleanup");
		const commands: string[][] = [];
	...
	it("contains prompt bytes when buffer deletion cannot be verified", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "paste-buffer-delete-failure");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
	...
	it("submits tmux-delivered prompts with tmux Enter after literal typing", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "submit-chord-delivery");
		const deliveryCommands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
	...
	it("delivers delegated skill prompts through a tmux paste buffer preserving the slash-command separator", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "delegate-paste-buffer");
		let pastedPrompt = "";
		const deliveryCommands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
		...
		expect(
			pastedPrompt.startsWith("/skill:ultragoal\n\nDelegated by coordinator MCP tool: skc_delegate_execute"),
		).toBe(true);
	...
	it("marks tmux-delivered turns acknowledged when runtime state accepts the current turn", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "acknowledged-delivery");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
				...
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
				SKC_COORDINATOR_MCP_PROMPT_ACK_TIMEOUT_MS: "60000",
			},
			services: {
				ownerIsolationProbe: privateOwnerProbe(),
				commandRunner: async command => {
	...
	it("records a durable reason when tmux disappears after prompt acknowledgement", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "acknowledged-tmux-vanish");
		let tmuxLive = true;
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		let read = await server.callTool("skc_coordinator_read_turn", {
			session_id: "visible-session",
			turn_id: turnId,
		});
		expect(read).toMatchObject({
		...
		tmuxLive = false;
		read = await server.callTool("skc_coordinator_read_turn", {
			session_id: "visible-session",
			turn_id: turnId,
		});
	...
	it("wakes watch on runtime ack and records vanished tmux after prompt acceptance", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "watch-ack-vanish");
		let tmuxLive = true;
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const turnId = sent.turn_id as string;
		const ackWatch = server.callTool("skc_coordinator_watch_events", {
			after_seq: 0,
		...
		tmuxLive = false;
		const failed = await server.callTool("skc_coordinator_watch_events", {
	...
	it("preserves session-missing failure precedence over runtime ack timeout", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "missing-session-precedence");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
	...
	it("starts sessions through the structured SKC service adapter, not arbitrary terminal relay", async () => {
		const root = await tempRoot();
		const calls: unknown[] = [];
		const server = createCoordinatorMcpServer({
			env: {
		...
		const response = await server.handleJsonRpc({
			...
			method: "tools/call",
			params: {
				name: "skc_coordinator_start_session",
		...
		expect(payload.session.session_id).toMatch(/^skc-coordinator-/);
	...
	it.each([
		["skc_coordinator_start_session", "codex-eco"],
		["skc_coordinator_start_session", "codex-medium"],
		["skc_delegate_plan", "codex-eco"],
		["skc_delegate_plan", "codex-medium"],
		["skc_delegate_execute", "codex-eco"],
		["skc_delegate_execute", "codex-medium"],
		["skc_delegate_team", "codex-eco"],
		["skc_delegate_team", "codex-medium"],
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", `fresh-${name}-${mpreset}`),
	...
	it("canonicalizes a legacy mpreset alias into the child command like the CLI", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "mpreset-alias");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const response = await server.callTool("skc_coordinator_start_session", {
			cwd: root,
			allow_mutation: true,
	...
	it.each([
		...
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", `invalid-${name}`);
		const commands: string[][] = [];
		let starts = 0;
		const server = createCoordinatorMcpServer({
			env: {
	...
	it("surfaces a fresh session mpreset through list_sessions and read_status", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "mpreset-readback");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const listed = await server.callTool("skc_coordinator_list_sessions");
		const status = await server.callTool("skc_coordinator_read_status", { session_id: sessionId });
	...
	it.each([
		...
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", `mpreset-reuse-${sessionMpreset}-${requestedMpreset}`);
		let starts = 0;
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const sessionId = startedSessionId(started);
		const response = await server.callTool("skc_delegate_execute", {
			cwd: root,
	...
	it("keeps the delegate model hint out of the child command while mpreset remains authoritative", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "mpreset-model-separation");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
	...
	it("leaves the child command unchanged when mpreset is omitted", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "mpreset-omitted"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
		...
		expect(launchCommand).toEndWith("skc --worktree");
	...
	it("rejects missing and mismatched coordinator launch attestations", async () => {
		...
		const missing = createCoordinatorMcpServer({
			env: { ...baseEnv, SKC_COORDINATOR_MCP_REPO: "missing" },
			services: {
				startSession: async input => ({ sessionId: input.sessionId, cwd: input.cwd }),
			},
		});
		expect(await missing.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true })).toEqual({
			ok: false,
			reason: "coordinator_launch_attestation_missing",
		});

		const mismatched = createCoordinatorMcpServer({
			env: { ...baseEnv, SKC_COORDINATOR_MCP_REPO: "mismatched" },
			services: {
				startSession: async input => ({
					sessionId: input.sessionId,
		...
		expect(await mismatched.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true })).toEqual({
			ok: false,
	...
	it("times out readiness with zero sends, retains the session, and gates an explicit retry", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "readiness-timeout");
		const commands: string[][] = [];
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
				SKC_COORDINATOR_MCP_RUNTIME_READINESS_TIMEOUT_MS: "25",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
			},
		...
		const retried = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "retry",
			allow_mutation: true,
		});
	...
	it("continues waiting after invalid marker notifications and accepts the later valid marker", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "readiness-no-miss");
		const commands: string[][] = [];
		const launched = Promise.withResolvers<{ sessionId: string; launchId: string; readinessMarkerFile: string }>();
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
				SKC_COORDINATOR_MCP_RUNTIME_READINESS_TIMEOUT_MS: "1000",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
			},
		...
		const starting = server.callTool("skc_coordinator_start_session", {
			cwd: root,
			prompt: "work",
			allow_mutation: true,
		});
	...
	it("lets report terminalization beat late readiness without stale injection", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "readiness-report-race");
		const commands: string[][] = [];
		const launched = Promise.withResolvers<{ sessionId: string; launchId: string; readinessMarkerFile: string }>();
		const server = createCoordinatorMcpServer({
			env: {
		...
		const started = await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true });
		const launch = await launched.promise;
		const sessionId = startedSessionId(started);
		const sending = server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "work",
			allow_mutation: true,
		});
		...
		await server.callTool("skc_coordinator_report_status", {
			session_id: sessionId,
	...
	it("serializes force replacement against readiness and injects only the replacement turn", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "readiness-force-race");
		const commands: string[][] = [];
		const launched = Promise.withResolvers<{ sessionId: string; launchId: string; readinessMarkerFile: string }>();
		const server = createCoordinatorMcpServer({
			env: {
		...
		const launch = await launched.promise;
		const sessionId = startedSessionId(started);
		const first = server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "first",
			allow_mutation: true,
		});
		...
		const replacement = server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "replacement",
			force: true,
			allow_mutation: true,
	...
	it("prevents late readiness injection after runtime or liveness terminalization", async () => {
		for (const winner of ["runtime", "liveness"] as const) {
			const root = await tempRoot();
			const stateRoot = path.join(root, ".skc", "state", `readiness-${winner}-race`);
			const commands: string[][] = [];
			const launched = Promise.withResolvers<{ sessionId: string; launchId: string; readinessMarkerFile: string }>();
			const server = createCoordinatorMcpServer({
				env: {
					SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
					SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
					SKC_COORDINATOR_MCP_RUNTIME_READINESS_TIMEOUT_MS: "30000",
					SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
					SKC_COORDINATOR_MCP_PROFILE: "local",
					SKC_COORDINATOR_MCP_REPO: winner,
				},
				services: {
					ownerIsolationProbe: privateOwnerProbe(),
					startSession: async input => {
			...
			await waitForCondition(async () => {
				terminal = await server.callTool("skc_coordinator_read_turn", {
					session_id: sessionId,
					turn_id: active.turn_id,
				});
				return (
	...
	it("fails closed for unverified origins and trusts only strict legacy registration", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "origin-precedence");
		...
		const legacy = await server.callTool("skc_coordinator_send_prompt", {
			session_id: "legacy",
			prompt: "work",
			allow_mutation: true,
		});
	...
	it("promotes one queued turn once under concurrent terminal reports", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				...
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions,reports",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "promotion-race",
			},
			services: {
				ownerIsolationProbe: privateOwnerProbe(),
				startSession: async input =>
		...
		const sessionId = startedSessionId(
			await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true }),
		);
		const first = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "first",
			allow_mutation: true,
		});
		...
		const reports = await Promise.all([
			server.callTool("skc_coordinator_report_status", {
				session_id: sessionId,
		...
		try {
			...
			const probe = await coordinatorOwnerIsolationProbe(async command => {
				...
				if (command.includes("list-sessions"))
					return { exitCode: 0, stdout: "4242 skc-coordinator-test\n", stderr: "" };
			...
			const unavailable = await coordinatorOwnerIsolationProbe(async command =>
				command[0] === "ps"
					...
					: { exitCode: 0, stdout: "4242 skc-coordinator-test\n", stderr: "" },
			);
	...
	it("classifies a tmux >=3.7 'error connecting to' no-server diagnostic as absent, not unverifiable", async () => {
		...
		const noServer = await coordinatorOwnerIsolationProbe(async () => ({
			exitCode: 1,
			stdout: "",
			stderr: "error connecting to /private/tmp/tmux-501/skc-coordinator-abcd1234 (No such file or directory)",
		}));
		expect(await noServer.probeServer("skc-coordinator-abcd1234")).toEqual({ state: "absent" });
	});
	it("refuses atomic cleanup after a post-spawn coordinator server replacement", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "cleanup-proof"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
				...
				ownerIsolationProbe: {
					...
					probeServer: async () => {
						...
						return {
							...
							cgroup: { classification: "safe" as const },
							sessionNames: ["skc-coordinator-replacement"],
						};
					},
				},
				commandRunner: async command => {
					...
					if (tmuxSubcommand(command) === "if-shell")
						return { exitCode: 0, stdout: "__skc_coordinator_cleanup_refused__\n", stderr: "" };
					return { exitCode: 1, stdout: "", stderr: "unexpected command" };
				},
			},
		});
	...
	it("compensates durable records and appends failure history when owner publication CAS fails", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "owner-cas-rollback");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const events = await server.callTool("skc_coordinator_watch_events", { after_seq: 0 });
		expect(events.events).toEqual(
			expect.arrayContaining([
	...
	it("reports cleanup status after direct launch exits nonzero after creating a session", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "direct-nonzero"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
		...
		expect(await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true })).toMatchObject(
			{
				ok: false,
	...
	it("does not clean a foreign native session reported by a direct launch failure", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "direct-foreign-nonzero"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
			...
			services: {
				...
				commandRunner: async command => {
					...
					return tmuxSubcommand(command) === "new-session"
						? {
								exitCode: 1,
								stdout: "skc-coordinator-foreign:0.0 %2 $123\n",
								stderr: "failed after create",
							}
	...
	it("reports cleanup status after scoped launch exits nonzero after creating a session", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "scoped-nonzero"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
	...
	it("fails closed on unknown target-server diagnostics without launching tmux", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "unknown-owner"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
	...
	it("rejects coordinator tmux startup when the target server is unsafe", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "unsafe-owner"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
	...
	it("refuses atomic cleanup for a scoped same-name native-session replacement", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "scoped-replacement"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
		...
		expect(bootstrappedSessionName).toMatch(/^skc-coordinator-/);
		expect(commands).toHaveLength(2);
	...
	it("boots an absent unsafe-service owner in a scoped server and routes follow-up through its socket", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "scoped-owner");
		const commands: Array<{ command: string[]; stdinLine?: string }> = [];
		let probeCalls = 0;
		...
		expect(socketKey).toMatch(/^skc-coordinator-/);
		...
		expect(bootstrap?.command).toEqual([
			...
			"--quiet",
			"--unit",
			expect.stringMatching(/^skc-owner-[0-9a-f-]+\.scope$/),
			expect.any(String),
			expect.any(String),
	...
	it("allocates distinct private socket, generation, and routing for two coordinator owners", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "owner-uniqueness");
		const commands: string[][] = [];
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
			...
			services: {
				ownerIsolationProbe: {
					readCallerCgroup: async () => "/skc-owner-test.scope\n",
					probeServer: async () => ({
						state: "safe",
						pid: process.pid,
						startTime: "owner-uniqueness",
						cgroup: { classification: "safe", scope: "/skc-owner-test.scope" },
					}),
				},
				commandRunner: async command => {
					commands.push(command);
		...
		const first = await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true });
		const second = await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true });
	...
	it.each([
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: path.join(root, ".skc", "state", "scoped-receipt-failure"),
				SKC_COORDINATOR_MCP_SESSION_COMMAND: "skc --worktree",
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
	...
	it("delivers start-session prompts exactly once after the active turn is durable", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-start-session-prompt");
		const commands: string[][] = [];
		...
		expect(launch).toEqual([
			...
			"-c",
			root,
			expect.stringContaining(`SKC_TMUX_OWNER_GENERATION='${ownerGeneration}'`),
		]);
		const childCommand = launch?.at(-1);
		expect(childCommand).toContain(`SKC_TMUX_OWNER_STATE_DIR='${ownerStateDir}'`);
		expect(childCommand).toContain(`SKC_TMUX_OWNER_SERVER_KEY='${ownerServerKey}'`);
	...
	it.skipIf(!Bun.which("tmux"))(
		...
		async () => {
			const root = await tempRoot();
			const stateRoot = path.join(root, ".skc", "state", "real-tmux-enter-ack");
...
const stateFile = process.env.SKC_COORDINATOR_SESSION_STATE_FILE;
const sessionId = process.env.SKC_COORDINATOR_SESSION_ID;
const launchId = process.env.SKC_COORDINATOR_SESSION_LAUNCH_ID;
const readinessFile = process.env.SKC_COORDINATOR_SESSION_READINESS_FILE;
...
`,
			...
			const server = createCoordinatorMcpServer({
				env: {
					SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
					SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
					SKC_COORDINATOR_MCP_SESSION_COMMAND: `${shellQuote(process.execPath)} ${shellQuote(runtimeScript)} > ${shellQuote(runtimeOutput)} 2>&1`,
					SKC_COORDINATOR_MCP_MUTATIONS: "sessions",
					SKC_COORDINATOR_MCP_PROFILE: "local",
					SKC_COORDINATOR_MCP_REPO: "repo",
					SKC_COORDINATOR_MCP_PROMPT_ACK_TIMEOUT_MS: "2000",
				},
				services: {
					ownerIsolationProbe: {
						readCallerCgroup: async () => "/user.slice/user-1.scope",
			...
			try {
				...
				expect(tmuxSocketKey).toMatch(/^skc-coordinator-/);
	...
	it("exposes a canonical polling coordination snapshot", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-status");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const turn = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "work",
			allow_mutation: true,
		});
	...
	it("persists audited follow-up, question answers, and bounded reports", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-test");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const prompt = await server.handleJsonRpc({
			...
			method: "tools/call",
			params: {
				name: "skc_coordinator_send_prompt",
		...
		const answer = await server.handleJsonRpc({
			...
			method: "tools/call",
			params: {
				name: "skc_coordinator_submit_question_answer",
		...
		const report = await server.handleJsonRpc({
			...
			method: "tools/call",
			params: {
				name: "skc_coordinator_report_status",
	...
	it("rejects traversal-shaped session and question ids before state file access", async () => {
		...
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
				SKC_COORDINATOR_MCP_MUTATIONS: "sessions,questions",
				SKC_COORDINATOR_MCP_PROFILE: "local",
				SKC_COORDINATOR_MCP_REPO: "repo",
			},
		});
		...
		const status = await server.callTool("skc_coordinator_read_status", { session_id: traversal });
		const tail = await server.callTool("skc_coordinator_read_tail", { session_id: traversal });
		const prompt = await server.callTool("skc_coordinator_send_prompt", {
			session_id: traversal,
			prompt: "continue",
			allow_mutation: true,
		});
		const answer = await server.callTool("skc_coordinator_submit_question_answer", {
	...
	it("creates durable turns, enforces active backpressure, and reads terminal reports", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-turns");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const rejected = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "second",
			allow_mutation: true,
		});
		...
		const completed = await server.callTool("skc_coordinator_report_status", {
			session_id: sessionId,
			turn_id: first.turn_id,
			status: "completed",
			summary: "Done",
		...
		const afterTerminal = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "third",
			allow_mutation: true,
		});
	...
	it("validates turn and question ownership before path-addressed mutations", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-ids");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		expect(await server.callTool("skc_coordinator_read_turn", { turn_id: "../escape" })).toEqual({
			ok: false,
			reason: "invalid_turn_id",
		});
		expect(
			await server.callTool("skc_coordinator_read_turn", { session_id: "other-session", turn_id: turn.turn_id }),
		).toEqual({
			ok: false,
			reason: "turn_session_mismatch",
		});
		expect(
			await server.callTool("skc_coordinator_submit_question_answer", {
				session_id: sessionId,
				turn_id: turn.turn_id,
				question_id: "../escape",
				answer: "bad",
		...
		const answered = await server.callTool("skc_coordinator_submit_question_answer", {
			session_id: sessionId,
			turn_id: turn.turn_id,
			question_id: "q-safe",
			answer: "yes",
	...
	it("awaits turns with bounded timeout and preserves queued turns", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-await");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const awaited = await server.callTool("skc_coordinator_await_turn", {
			session_id: sessionId,
	...
	it("wakes await_turn from durable turn changes without waiting for the fallback interval", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-watch");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const timer = setTimeout(() => {
			void server.callTool("skc_coordinator_report_status", {
				session_id: sessionId,
				turn_id: queued.turn_id,
				status: "completed",
				summary: "Done",
	...
	it("preserves launch errors from runtime state before tmux liveness masking", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-launch-error");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
			...
			services: {
				...
				startSession: async input =>
					readyStart(input, {
						sessionId: input.sessionId,
						tmuxSession: "skc-demo",
						tmuxTarget: "skc-demo:0.0",
						tmuxSocketKey: PRIVATE_SOCKET,
						tmuxOwnerServerKey: PRIVATE_SOCKET,
						tmuxOwnerGeneration: "skc-demo-generation",
						tmuxOwnerServerPid: PRIVATE_OWNER_PID,
						tmuxOwnerServerStartTime: PRIVATE_OWNER_START_TIME,
		...
		await Bun.write(
			...
			JSON.stringify({
				...
				source: "agent_session_event",
				owner_generation: "skc-demo-generation",
				live: false,
				reason: "worktree_target_mismatch",
				final_response: {
					text: "worktree_target_mismatch:/tmp/repo.sayknow-cli-worktrees/main",
					format: "markdown",
					source: "launch_error",
					artifact_path: null,
					truncated: false,
				...
				error: {
					...
					message: "worktree_target_mismatch:/tmp/repo.sayknow-cli-worktrees/main",
					recoverable: true,
				},
			}),
		);
	...
	it("terminalizes active turns from durable runtime session state", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-runtime");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
	...
	it("accepts process-postmortem owner terminal evidence only from the registered private owner", async () => {
		...
		for (const [caseName, ownerTerminalForSession, expectedStatus] of cases) {
			const root = await tempRoot();
			const stateRoot = path.join(root, ".skc", "state", `runtime-owner-terminal-${caseName}`);
			const server = createCoordinatorMcpServer({
				env: {
					SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
					SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
	...
	it("preserves runtime completion when callback wins the turn activation race", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-runtime-race");
		...
		startedSessionId(await server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true }));
	...
	it("flags completed turns that lack reportable final responses", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-runtime-missing-final");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
	...
	it("terminalizes active turns quickly when the recorded tmux session is gone", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "hermes-stale");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
			...
			services: {
				...
				startSession: async input =>
					readyStart(input, {
						tmuxSession: "definitely-missing-skc-demo",
						tmuxTarget: "definitely-missing-skc-demo:0.0",
						cwd: input.cwd,
						createdAt: "2026-06-07T00:00:00.000Z",
					}),
				commandRunner: async command => {
		...
		const second = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
			prompt: "second",
			allow_mutation: true,
		});
	...
	it("persists monotonic coordinator events and exposes long-poll watch semantics", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "event-watch");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
			...
			services: {
				ownerIsolationProbe: privateOwnerProbe(),
				startSession: async input =>
					readyStart(input, { tmuxSession: "skc-demo", cwd: input.cwd, createdAt: "2026-06-07T00:00:00.000Z" }),
				commandRunner: privateTmuxCommandRunner,
			},
		});
		...
		const firstWatch = await server.callTool("skc_coordinator_watch_events", { after_seq: 0, limit: 2 });
		...
		const all = await server.callTool("skc_coordinator_watch_events", { after_seq: 0 });
		...
		const filtered = await server.callTool("skc_coordinator_watch_events", {
			after_seq: 0,
			session_id: sessionId,
		...
		const persisted = await persistedServer.callTool("skc_coordinator_watch_events", { after_seq: 0 });
	...
	it("serializes concurrent coordinator event appends per namespace", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "event-concurrent");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		await Promise.all(
			Array.from({ length: 8 }, () =>
				server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true }),
			),
		);
		const watched = await server.callTool("skc_coordinator_watch_events", { after_seq: 0, limit: 100 });
	...
	it("long-polls coordinator events until timeout or a journal write", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "event-long-poll");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
				SKC_COORDINATOR_MCP_STATE_ROOT: stateRoot,
		...
		const empty = await server.callTool("skc_coordinator_watch_events", { after_seq: 0, timeout_ms: 5 });
		expect(empty).toMatchObject({ ok: true, events: [], latest_seq: 0, timed_out: true });

		const watching = server.callTool("skc_coordinator_watch_events", { after_seq: 0, timeout_ms: 1000 });
		const started = Promise.withResolvers<void>();
		const timer = setTimeout(() => {
			void server.callTool("skc_coordinator_start_session", { cwd: root, allow_mutation: true }).then(
				() => started.resolve(),
	...
	it("fails closed for missing, corrupt, and unreadable durable session state without leaking owner controls", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "state-read-failures");
		...
		const missing = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
		...
		const corrupt = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
		...
		const unreadable = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
	...
	it("rejects malformed journal records without reusing event sequence", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "journal-read-failures");
		const namespace = path.join(stateRoot, "local", "repo");
		const server = createCoordinatorMcpServer({
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: root,
		...
		const malformedLine = await server.callTool("skc_coordinator_watch_events", { after_seq: 0 });
		...
		const malformedRecord = await server.callTool("skc_coordinator_watch_events", { after_seq: 0 });
	...
	it("rejects physically out-of-order or semantically invalid journal evidence without mutation", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "journal-order");
		const namespace = path.join(stateRoot, "local", "repo");
		const journalPath = path.join(namespace, "events", "event-journal.jsonl");
		...
		const result = await server.callTool("skc_coordinator_watch_events", { after_seq: 0 });
	...
	it("continues event sequence from a valid journal when the sequence marker is absent", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "journal-sequence-recovery");
		const namespace = path.join(stateRoot, "local", "repo");
	...
	it("rejects an ahead sequence marker and recovers a lagging marker from authoritative journal evidence", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "journal-marker-recovery");
		const namespace = path.join(stateRoot, "local", "repo");
	...
	it("preflights invalid journal evidence before start, register, or delegate side effects", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "mutation-preflight");
		const namespace = path.join(stateRoot, "local", "repo");
		const commands: string[][] = [];
		let starts = 0;
		await fs.mkdir(path.join(namespace, "events"), { recursive: true });
		...
		for (const [name, args] of [
			["skc_coordinator_start_session", { cwd: root, allow_mutation: true }],
			[
			...
			["skc_delegate_execute", { cwd: root, task: "do not start", allow_mutation: true }],
		] as const)
	...
	it("OWNER-TERMINAL-ROUNDTRIP accepts an actual sidecar terminal marker with its canonical socket_key", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "owner-roundtrip");
		let sessionId = "";
		let stateFile = "";
		let generation = "";
		const server = createCoordinatorMcpServer({
		...
		process.env[SKC_COORDINATOR_SESSION_ID_ENV] = sessionId;
		...
		const reconciled = await server.callTool("skc_coordinator_read_turn", {
			session_id: sessionId,
			turn_id: turn.turn_id,
		});
	...
	it("TERMINAL-PRESERVATION keeps the complete runtime marker byte-stable through reconciliation and finalization", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "terminal-preservation");
		let sessionId = "";
		let stateFile = "";
		const server = createCoordinatorMcpServer({
			env: {
		...
		await server.callTool("skc_coordinator_read_turn", { session_id: sessionId, turn_id: turn.turn_id });
	...
	it("SHARED-WRITER-RACE serializes sidecar finalization behind an active coordinator transaction", async () => {
		for (const releaseSidecarFirst of [false]) {
			const root = await tempRoot();
			const stateRoot = path.join(root, ".skc", "state", `shared-writer-${releaseSidecarFirst}`);
			let sessionId = "";
			let stateFile = "";
			...
			process.env[SKC_COORDINATOR_SESSION_STATE_FILE_ENV] = stateFile;
			process.env[SKC_COORDINATOR_SESSION_ID_ENV] = sessionId;
			const reconcile = server.callTool("skc_coordinator_read_turn", {
				session_id: sessionId,
				turn_id: turn.turn_id,
			});
			await Promise.race([
	...
	it("ENOTDIR-PREFLIGHT rejects durable-parent corruption before start, register, or delegate side effects", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "enotdir-preflight");
		const namespace = path.join(stateRoot, "local", "repo");
		...
		for (const [name, args] of [
			...
			["skc_delegate_execute", { cwd: root, task: "fixed synthetic task", allow_mutation: true }],
		] as const)
	...
	it("recovers orphaned state locks and waits for a live lock beyond the legacy retry window", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "state-lock-recovery");
		let sessionId = "";
		let stateFile = "";
		const server = createCoordinatorMcpServer({
			env: {
		...
		const write = server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
	...
	it("rejects near-valid turn file/body identity mismatches without mutations", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "turn-identity-mismatch");
		const namespace = path.join(stateRoot, "local", "repo");
		let sessionId = "";
		const server = createCoordinatorMcpServer({
			env: {
		...
		const scanned = await server.callTool("skc_coordinator_read_coordination_status");
	...
	it("rejects queued turn filename/body mismatches before promotion can redirect mutation", async () => {
		const root = await tempRoot();
		const stateRoot = path.join(root, ".skc", "state", "queued-turn-identity-mismatch");
		const namespace = path.join(stateRoot, "local", "repo");
		let sessionId = "";
		const server = createCoordinatorMcpServer({
			env: {
		...
		const active = await server.callTool("skc_coordinator_send_prompt", {
			session_id: sessionId,
		...
		const result = await server.callTool("skc_coordinator_report_status", {
			session_id: sessionId,
			turn_id: active.turn_id,
			status: "completed",
			summary: "fixed",
	...
	it("CORRUPT-TURN-NO-MUTATION fails closed for force, queue, report, and question mutations", async () => {
		for (const corruptActiveTurn of [false, true]) {
			const root = await tempRoot();
			const stateRoot = path.join(root, ".skc", "state", `corrupt-turn-${corruptActiveTurn}`);
			const namespace = path.join(stateRoot, "local", "repo");
			let sessionId = "";
			const server = createCoordinatorMcpServer({
				env: {
