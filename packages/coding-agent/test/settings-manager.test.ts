...
describe("Settings", () => {
	...
	describe("causally ordered atomic persistence", () => {
		...
		it("fails closed after an atomic read failure", async () => {
			for (const commit of [
				(settings: Settings) =>
					settings.commitAtomicBatch([{ path: "theme.dark", op: "set" as const, value: "red-octopus" }]),
				(settings: Settings) =>
					settings.commitAtomicBatchWithCurrent(() => [
						{ path: "theme.dark", op: "set" as const, value: "red-octopus" },
					]),
			]) {
				await writeSettings({ theme: { dark: "blue-octopus" } });
	...
	it("persists a retained dirty patch on the first flush after YAML repair", async () => {
		...
		try {
			...
			expect(settings.canWriteDurableConfig()).toBe(true);
			expect((await readSettings()).theme).toEqual({ dark: "blue-octopus" });
		} finally {
			settings.getStorage()?.close();
		}
	});
	it("enters syntax recovery after a debounced save encounters malformed YAML", async () => {
		...
		try {
			...
			expect(settings.getSchemaReport()).toMatchObject({ valid: false });
			expect(() => settings.getNotificationSettingsSnapshot()).toThrow("skc_notify_daemon_invalid_configuration");
			expect(() => settings.set("theme.light", "blue-octopus")).toThrow("Repair config.yml");
		} finally {
			settings.getStorage()?.close();
		}
	});
	...
	it("serializes failed-save recovery behind a later reserved save", async () => {
		...
		const warning = vi.spyOn(logger, "warn").mockImplementation(message => {
			...
			fs.writeFileSync(getConfigPath(), YAML.stringify({ theme: { dark: "red-octopus" } }, null, 2));
		...
		try {
			...
			expect((await readSettings()).theme).toEqual({ dark: "red-octopus" });
		} finally {
			warning.mockRestore();
			settings.getStorage()?.close();
		}
	...
	it("keeps malformed global recovery and notification state in cwd clones", async () => {
		...
		try {
			...
			expect(() => source.getNotificationSettingsSnapshot()).toThrow("skc_notify_daemon_invalid_configuration");
			expect(() => cloned.getNotificationSettingsSnapshot()).toThrow("skc_notify_daemon_invalid_configuration");
	...
	it("keeps the source as the sole owner of retained patches in recovered cwd clones", async () => {
		...
		try {
			source.set("theme.dark", "blue-octopus");
			await Bun.write(getConfigPath(), "theme: [");
			await Bun.sleep(300);

			const cloned = await source.cloneForCwd(clonedCwd);
			expect(cloned.canWriteDurableConfig()).toBe(false);
			expect(cloned.get("theme.dark")).toBe("blue-octopus");

			await Bun.write(getConfigPath(), "");
			await source.flushOrThrow();
			expect((await readSettings()).theme).toEqual({ dark: "blue-octopus" });

			await Bun.write(getConfigPath(), YAML.stringify({ theme: { dark: "red-octopus" } }, null, 2));
