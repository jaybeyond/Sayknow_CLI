...
describe("skill HUD bar renderer", () => {
				...
				[
					{
						...
						receipt: {
							...
							skill: "deep-interview",
							owner: "skc-state-cli",
							command: "skc state deep-interview write",
							state_path: ".skc/state/skill-active-state.json",
							storage_path: ".skc/state/deep-interview-state.json",
	...
	it("shows only the callee after a D->R handoff (caller demoted to inactive entry, HUD filters it out)", () => {
		// After `skc state deep-interview handoff --to ralplan`, the caller
	...
	it("collapses the planning pipeline to the most-recently-activated stage", () => {
		// `skc ralplan` then `skc ultragoal` activate their own rows without
