...
import { discoverAndLoadExtensions } from "@sayknow-cli/coding-agent/extensibility/extensions/loader";
import { getAgentDir, getPluginsDir, setAgentDir, TempDir } from "@sayknow-cli/utils";

const currentPiCodingAgentPath = Bun.resolveSync("@sayknow-cli/coding-agent", import.meta.dir);
const currentPiExtensionsPath = Bun.resolveSync("@sayknow-cli/coding-agent/extensibility/extensions", import.meta.dir);
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tempXdgDataHome, "skc"), { recursive: true });
	...
	it("loads installed legacy Pi plugin extensions from Windows drive-letter paths", async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				'import { isToolCallEventType as legacyRoot } from "@mariozechner/sayknow-cli";',
				'import { isToolCallEventType as legacyExtensions } from "@mariozechner/sayknow-cli/extensibility/extensions";',
