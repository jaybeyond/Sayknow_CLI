...
describe("end-to-end import", () => {
	test("claude-code imports MCP + skill", async () => {
		...
		const config = await readMCPConfigFile(path.join(cwd, ".skc", "mcp.json"));
		expect(config.mcpServers?.claudeSrv).toMatchObject({ command: "cbin" });
		expect(await fs.readFile(path.join(cwd, ".skc", "skills", "alpha", "SKILL.md"), "utf-8")).toContain(
			"description: a",
		);
	...
	test("codex imports TOML MCP + converted prompt", async () => {
		...
		expect(await fs.readFile(path.join(cwd, ".skc", "skills", "review", "SKILL.md"), "utf-8")).toContain(
			"description:",
		);
	});
	...
	test("opencode imports MCP + skill + command", async () => {
		...
		expect(await fs.readFile(path.join(cwd, ".skc", "skills", "deploy", "SKILL.md"), "utf-8")).toContain(
			"description:",
		);
	});
	...
	test("pre-existing skill destination symlink is reported as a conflict and not followed", async () => {
		...
		try {
			await fs.mkdir(path.join(cwd, ".skc", "skills"), { recursive: true });
			await fs.symlink(outside, path.join(cwd, ".skc", "skills", "alpha"), "dir");
