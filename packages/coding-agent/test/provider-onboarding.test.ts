...
import { clampThinkingLevelForModel, Effort, getSupportedEfforts } from "@sayknow-cli/ai";
import { getAgentDbPath, getAgentDir, setAgentDir } from "@sayknow-cli/utils";
...
async function tempModelsPath(): Promise<string> {
	tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-provider-onboarding-"));
...
describe("provider onboarding setup core", () => {
	...
	it("creates the models.yml parent directory on first provider add", async () => {
		tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-provider-onboarding-"));
		const modelsPath = path.join(tempRoot, "Users", "example", ".skc", "agent", "models.yml");
	...
	it("parses explicit setup command provider options", () => {
		...
		expect(parsed?.flags.apiKeyEnv).toBe("SKC_TEST_PROVIDER_KEY");
