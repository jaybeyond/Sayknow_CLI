...
} from "@sayknow-cli/coding-agent/harness-control-plane/gc-adapter";
import type { GcContext, GcPidProbe } from "@sayknow-cli/coding-agent/skc-runtime/gc-runtime";
...
function ctxFor(base: string, registryDir: string, probe: GcPidProbe = splitProbe): GcContext {
	return { probe, force: false, env: { ...process.env, SKC_HARNESS_ROOT_REGISTRY_DIR: registryDir }, cwd: base };
}
...
describe("fileLocksGcAdapter", () => {
	test("dead-pid lock removable; live + malformed locks kept; old timestamp alone never removable", async () => {
		...
		const ctx: GcContext = {
			probe: splitProbe,
			force: false,
			env: { ...process.env, SKC_RECEIPT_SPOOL_DIR: spoolDir },
			cwd: base,
		};
