...
import { installSkcPluginBundle, loadConstrainedPluginHooks } from "../src/extensibility/skc-plugins";
...
async function mkCwd(): Promise<string> {
	const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "skc-hooks-"));
	tempDirs.push(cwd);
	return cwd;
}

async function bundleWithHook(hookBody: string): Promise<string> {
	const src = await fs.mkdtemp(path.join(os.tmpdir(), "skc-hooksrc-"));
	tempDirs.push(src);
