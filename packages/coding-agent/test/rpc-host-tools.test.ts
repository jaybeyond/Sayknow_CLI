...
import type { AgentEvent } from "@sayknow-cli/agent-core";
import { RpcHostToolBridge } from "@sayknow-cli/coding-agent/modes/rpc/host-tools";
import { defineRpcClientTool, RpcClient } from "@sayknow-cli/coding-agent/modes/rpc/rpc-client";
import type {
...
describe("RpcClient custom tools", () => {
	it("delivers workflow gates and answers with the schema-validated response contract", async () => {
		const scriptPath = path.join(os.tmpdir(), `skc-rpc-workflow-gate-${Date.now()}.js`);
...
`,
	...
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `skc-rpc-host-tools-${Date.now()}.js`);
		tempPaths.push(scriptPath);
		await Bun.write(
			scriptPath,
			`
