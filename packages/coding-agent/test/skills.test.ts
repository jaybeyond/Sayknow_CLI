...
import { type Skill as CapabilitySkill, skillCapability } from "@sayknow-cli/coding-agent/capability/skill";
import { getCapability } from "@sayknow-cli/coding-agent/discovery";
import {
	...
	type Skill,
} from "@sayknow-cli/coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		it("should ignore Codex and Claude skills even when their source toggles are enabled", async () => {
			const tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-external-skills-home-"));
			const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-external-skills-project-"));
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-disabled-skill-"));
