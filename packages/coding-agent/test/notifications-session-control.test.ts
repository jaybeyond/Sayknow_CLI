...
describe("NotificationSessionController", () => {
	...
	test("starts generic endpoints for Discord, Slack, and token-only opt-in without Telegram daemon", async () => {
		for (const input of [
			...
			{ config: BASE_CONFIG, env: { SKC_NOTIFICATIONS_TOKEN: "legacy-token" } },
		]) {
			const calls: Call[] = [];
			const controller = new NotificationSessionController({
				eligible: true,
	...
	test("a dormant unconfigured or hard-disabled controller has no endpoint or daemon side effects", async () => {
		for (const input of [
			...
			{ eligible: false, config: telegramConfig(), env: { SKC_NOTIFICATIONS: "0" } },
		]) {
			const calls: Call[] = [];
			const controller = new NotificationSessionController({
	...
	test("preserves authoritative SKC_NOTIFICATIONS=0 and explicit SKC_NOTIFICATIONS=1 precedence", async () => {
		const offCalls: Call[] = [];
