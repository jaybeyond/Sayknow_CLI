...
test("production ACP routes zero-session SDK globals through the broker adapter", async () => {
	const directory = await mkdtemp(path.join(tmpdir(), "skc-sdk-acp-production-"));
	directories.push(directory);
	const agentDir = path.join(directory, ".skc", "agent");
	...
	const result = await agent.extMethod("_skc/sdk/global", { operation: "session.list" });
	...
	const lifecycle = await agent.extMethod("_skc/sdk/global", {
...
test("production ACP preserves lifecycle, turn, replay, and connection ownership contracts over SDK WebSockets", async () => {
	const directory = await mkdtemp(path.join(tmpdir(), "skc-sdk-acp-contract-"));
	directories.push(directory);
	const agentDir = path.join(directory, ".skc", "agent");
		...
		expect.arrayContaining([
			...
			expect.objectContaining({
				...
				update: expect.objectContaining({
					...
					_meta: {
						skcTranscriptImageReplay: { available: false, reason: "historical_transcript_images_unavailable" },
					},
				}),
			}),
		]),
