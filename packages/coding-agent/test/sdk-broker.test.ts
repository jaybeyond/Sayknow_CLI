...
import { getSessionsDir } from "@sayknow-cli/utils";
...
const temp = () => fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-"));
...
it("isolates source SDK children and preserves compiled self-spawn", () => {
	...
	expect(source.env.SKC_COMPILED).toBeUndefined();
...
it("SDK lifecycle model presets reach the session host parser", async () => {
	...
	const request = readSessionLifecycleLaunchRequest(
		JSON.stringify({
			...
			sessionId: "session-1",
			stateRoot: path.join(cwd, ".skc", "state"),
			cwd,
...
it("SDK lifecycle launch requests preserve validated ACP MCP transports", async () => {
	...
	try {
		...
		).toThrow("SKC_SDK_LIFECYCLE_REQUEST is invalid.");
	} finally {
		await fs.rm(agentDir, { recursive: true, force: true });
	}
});
...
it("initializes the managed target scope before lifecycle fork arguments expose it", async () => {
	...
	try {
		...
		expect(syncFs.existsSync(path.join(args.sessionDir!, ".skc-managed-session-scope.v2.json"))).toBe(true);
	} finally {
		await fs.rm(agentDir, { recursive: true, force: true });
	}
});
...
it("exposes the same prepared managed fork scope on repeated lifecycle argument creation", async () => {
	...
	try {
		...
		expect(syncFs.existsSync(path.join(first.sessionDir!, ".skc-managed-session-scope.v2.json"))).toBe(true);
	} finally {
		await fs.rm(agentDir, { recursive: true, force: true });
	}
});
...
describe("SDK broker identity and discovery", () => {
	...
	it("rejects a cross-scope live resume without returning the indexed endpoint", async () => {
		...
		const stateRoot = path.join(liveCwd, ".skc", "state");
	...
	it("rejects a non-default lifecycle state root at broker ingress", async () => {
		...
		try {
			expect(
				...
				ok: false,
				error: { code: "invalid_input", message: "stateRoot must be the default .skc/state for cwd." },
			});
		} finally {
			await fs.rm(dir, { recursive: true, force: true });
		}
	...
	it("preserves typed verified-delete partial-cleanup evidence", async () => {
		...
		try {
			...
			expect(pending).toMatchObject({
				...
				error: {
					...
					cleanup: {
						...
						metadataRoot: path.join(cwd, ".skc", "state"),
			...
			if (!pending.ok) {
				expect(pending.error.cleanup?.plannedArtifactsPath).toMatch(/\.skc-delete-[\w-]+-artifacts$/);
				expect(pending.error.cleanup?.plannedTranscriptPath).toMatch(/\.skc-delete-[\w-]+-transcript$/);
			}
						...
						error: expect.objectContaining({
							cleanup: expect.objectContaining({
								...
								lifecycleFiles: [
									expect.objectContaining({
										identity: expect.objectContaining({ sha256: expect.any(String) }),
										plannedPath: expect.stringMatching(/\.skc-delete-.*\.lifecycle\.json$/),
									}),
								],
							}),
						}),
	...
	it("advances typed retained cleanup authority to completion and exactly replays it", async () => {
		...
		try {
			...
			expect(retainedTranscript.every(entry => entry.startsWith(".skc-delete-"))).toBe(true);
			...
			expect(retainedMetadata.every(entry => entry.startsWith(".skc-delete-"))).toBe(true);
		...
		const forged = vi.spyOn(native, "exactUnlink").mockImplementation((pathname, identity) => {
			if (pathname === sessionPath) {
				return {
					ok: false,
					code: "cleanup_pending",
					detachedPath: path.join(path.dirname(sessionPath), ".skc-delete-forged-transcript"),
				};
			}
			return originalUnlink(pathname, identity);
		});
	...
	it("returns endpoint_stale without dispatching close after endpoint generation rotation", async () => {
		const dir = await temp();
		const stateRoot = path.join(dir, ".skc", "state");
