...
describe("setup CLI parsing", () => {
	...
	describe("Hermes setup", () => {
		...
		it("parses Hermes setup flags without treating models as defaults", () => {
			expect(
				...
				flags: {
					...
					profile: "bot",
					repo: "sayknow-cli",
					sessionCommand: "skc --model openai/gpt-5.5",
					worktreeName: "hermes-sayknow-cli",
		...
		it("renders Hermes setup with a model-agnostic usable SKC session command", async () => {
			tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-coordinator-setup-"));
			const stdout = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
			...
			expect(configPreview).toContain("SKC_COORDINATOR_MCP_SESSION_COMMAND: skc --worktree");
		...
		it("preserves explicit Hermes session commands exactly", async () => {
			tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-coordinator-setup-"));
			const stdout = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
			const sessionCommand = "skc --model anthropic/claude-sonnet-4";
		...
		it("installs Hermes config without overwriting unrelated servers", async () => {
			...
			expect(parsed.mcp_servers.skc_coordinator?.command).toBe("skc");
			expect(parsed.mcp_servers.skc_coordinator?.env?.SKC_COORDINATOR_MCP_MUTATIONS).toBe("sessions,questions");
			expect(parsed.mcp_servers.skc_coordinator?.env?.SKC_COORDINATOR_MCP_SESSION_COMMAND).toBe("skc --worktree");
		});
		...
		it("renders named Hermes worktree commands and allows explicit opt-out", async () => {
			...
			expect(namedOutput).toContain("SKC_COORDINATOR_MCP_SESSION_COMMAND: skc --worktree hermes-sayknow-cli");
			named.mockRestore();
			...
			expect(noWorktreeOutput).toContain("SKC_COORDINATOR_MCP_SESSION_COMMAND: skc");
			expect(noWorktreeOutput).not.toContain("SKC_COORDINATOR_MCP_SESSION_COMMAND: skc --worktree");
		});
		...
		it("smoke checks the current Hermes MCP tool contract without provider credentials", async () => {
			...
			expect(parsed.smoke.requiredTools).toContain("skc_coordinator_send_prompt");
			expect(parsed.smoke.requiredTools).toContain("skc_coordinator_submit_question_answer");
