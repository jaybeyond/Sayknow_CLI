...
- `/fork` is back as a TUI command: pick an earlier prompt and continue from it in a new session. The original transcript is kept, and the selected prompt returns as an editable draft instead of being sent. It refuses to open for an unsaved session or while a response, compaction, or foreground command is running; a second Enter while the fork is in flight is ignored; a fork that fails before the new session exists leaves you in the old one, while one that fails after still moves you to the new session and says what failed. The prompt list fits the terminal height. Adapted from upstream #5515.
