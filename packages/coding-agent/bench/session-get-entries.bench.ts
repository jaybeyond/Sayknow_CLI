...
import type { AssistantMessage, Message, TextContent, ToolCall, ToolResultMessage, Usage } from "@sayknow-cli/ai";
import { SessionManager } from "../src/session/session-manager";

const PACKAGE_NAME = "@sayknow-cli/coding-agent";
...
async function createFixture(): Promise<SessionManager> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-get-entries-"));
