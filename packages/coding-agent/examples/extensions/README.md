...
Example extensions for sayknow-cli.

## Usage

```bash
# Copy an existing extension into the user extension directory for auto-discovery
mkdir -p ~/.skc/agent/extensions
cp packages/coding-agent/examples/extensions/hello.ts ~/.skc/agent/extensions/

# Project-local extensions can live in .skc/extensions/
mkdir -p .skc/extensions
cp packages/coding-agent/examples/extensions/pirate.ts .skc/extensions/
```
