...
Example hooks for skc-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
skc --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.skc/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@sayknow-cli/coding-agent/hooks";
