...
- `resolveExtensionEntries(dir)` checks, in order:
  - `package.json` manifest (`skc` or `pi`) with `extensions[]` entries,
...
- Uses `Settings.getShellConfig()` and `Shell` from `@sayknow-cli/natives`.
...
- The `task.isolation.merge` setting controls how isolated changes are integrated back:
  ...
  - **branch**: each task commits to a temp branch (`skc/task/<id>`) via `commitToBranch(...)`, then `mergeTaskBranches(...)` cherry-picks them sequentially onto HEAD. If `git apply` fails inside `commitToBranch`, the error is non-fatal — the agent result is preserved with a `merge failed` status.
...
- Build compiled binary artifact (`dist/skc`):
