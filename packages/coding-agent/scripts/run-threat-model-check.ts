...
async function assertFixtureControlsStayTestOnly(): Promise<void> {
	...
	assert(!/SKC_.*(?:GSF1|SSH1|SELF_REAP)/.test(ensureSource), "Production environment can select fixture controls");
}
...
async function main(): Promise<void> {
	...
	const agentDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-threat-model-"));
	...
	try {
		...
		const checks: Record<string, () => Promise<void>> = {
			...
			"broker-token-rotation-on-stale-takeover": async () => {
				const takeoverDir = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-takeover-"));
			...
			"broker-per-session-token-separation": async () => {
				const sessionRoot = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "skc-broker-session-token-"));
