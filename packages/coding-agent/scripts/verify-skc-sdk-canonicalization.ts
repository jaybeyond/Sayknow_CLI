...
const repoRoot = process.env.SKC_SDK_CANONICALIZATION_SCAN_ROOT
	? path.resolve(process.env.SKC_SDK_CANONICALIZATION_SCAN_ROOT)
	: path.resolve(import.meta.dir, "..", "..", "..");
const scannerPath = "packages/coding-agent/scripts/verify-skc-sdk-canonicalization.ts";
const packageManifestPath = "packages/coding-agent/package.json";
const retiredPythonRpcPackagePath = "python/skc-rpc/";
const bridgeClientPackageManifestPath = "packages/bridge-client/package.json";
const bridgeClientPackageName = "@sayknow-cli/bridge-client";
const bridgeOrUnattendedImportPattern =
	/(?:\b(?:import|export)\s+(?:type\s+)?(?:[^"'`;]*?\s+from\s+)?|\bimport\s*\(\s*)["'][^"']*(?:(?:^|\/)unattended)(?:["'/]|$)/g;
const bridgeClientImportPattern = /(?:\bfrom\s*|\bimport\s*\(\s*)["'](@sayknow-cli\/bridge-client[^"']*)["']/g;
...
const pythonSkcRpcImportPattern = /^\s*(?:from\s+skc_rpc(?:\.|\s)|import\s+skc_rpc(?:\.|\s|,|$))/gm;
...
const retiredTmuxMachineBusPaths = new Set(["scripts/skc-session/prompt.sh", "scripts/skc-session/tail.sh"]);
...
const machineTmuxDocumentationPattern =
	/(?:scripts\/skc-session\/(?:prompt|tail)\.sh|\b(?:load-buffer|paste-buffer|send-keys|capture-pane|pipe-pane)\b|(?:\.\/)?(?:scripts\/)?skc-session\/create\.sh(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3})/g;
...
function legacyPythonRpcViolations(file: string, contents: string): string[] {
	...
	if (isPython(file)) {
		for (const match of contents.matchAll(pythonSkcRpcImportPattern)) {
			violations.push(`${file}:${lineNumber(contents, match.index ?? 0)}: imports removed skc_rpc Python client`);
		}
	...
	if (isPythonDistributionMetadata(file)) {
		for (const match of contents.matchAll(/\bskc-rpc\b/gi)) {
			violations.push(
				`${file}:${lineNumber(contents, match.index ?? 0)}: declares removed skc-rpc distribution metadata`,
			);
		}
	}
	return violations;
...
function bridgeClientOwnershipViolations(file: string, contents: string): string[] {
	...
	for (const match of contents.matchAll(relativeImportPattern)) {
		...
		if (
			/(?:^|\/)(?:coding-agent|agent-session|sdk\/(?:host|session)|session)(?:\/|$)|@sayknow-cli\/coding-agent/.test(
				specifier,
			)
		) {
			violations.push(
...
const teamRuntimeTmuxPath = "packages/coding-agent/src/skc-runtime/team-runtime.ts";
const teamWorkersTmuxPath = "packages/coding-agent/src/skc-runtime/team-workers.ts";

const coordinatorMcpRoot = "packages/coding-agent/src/coordinator-mcp/server.ts";
function isPublishedSkcSessionShellHelper(file: string): boolean {
	return file.startsWith("scripts/skc-session/") && file.endsWith(".sh");
}
...
function shellSkcBinaryReferenceViolations(contents: string): ShellRange[] {
	...
	const references = /(["']?)\$(?:SKC_BIN|SKC_SESSION_SKC_BIN|\{(?:SKC_BIN|SKC_SESSION_SKC_BIN)\})\1/g;
	...
	for (const reference of normalizedContents.matchAll(references)) {
		...
		if (/(?:^|[\s"'])SKC_SESSION_SKC_BIN\s*=\s*$/.test(before)) continue;
...
function tmuxCreateStartupViolations(file: string, contents: string): string[] {
	if (file !== "scripts/skc-session/create.sh") return [];
	const violations: string[] = [];
	...
	const canonicalCommand = /^\s*command\s*=\s*\[\s*os\.environ\[\s*["']SKC_SESSION_SKC_BIN["']\s*\]\s*\]\s*$/gm;
	...
	if (
		...
		violation(
			...
			"human-only tmux owner must launch exactly SKC_SESSION_SKC_BIN with zero startup arguments",
		);
	}
	const canonicalInteractivePopenAssignment =
		/^\s*child\s*=\s*subprocess\.Popen\(\s*command\s*(?:,\s*cwd\s*=\s*os\.environ\[\s*["']SKC_SESSION_WORKDIR["']\s*\])?\s*\)\s*$/;
	const expectedLifecycleCallsites = [
		{
			operation: "terminal observer",
			pattern:
				/\bcompleted\s*=\s*subprocess\.run\(\s*\[\s*os\.environ\[\s*["']SKC_SESSION_SKC_BIN["']\s*\]\s*,\s*["']--internal-tmux-owner-isolation["']\s*\]\s*,/g,
		},
		{
			operation: "owner-isolation plan",
			pattern:
				/\bPLAN_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$PLAN_LINE"\s*\|\s*"\$SKC_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "post-spawn proof",
			pattern:
				/\bPOST_SPAWN_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$PLAN_LINE"\s*\|\s*"\$SKC_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "generation publication",
			pattern:
				/\bGENERATION_PUBLISH_RESPONSE\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$GENERATION_PUBLISH_REQUEST"\s*\|\s*"\$SKC_BIN"\s+--internal-tmux-owner-isolation\s*\)"/g,
		},
		{
			operation: "terminal monitor",
			pattern:
				/\bif\s+verdict\s*=\s*"\$\(\s*printf\s+["'][^"']*["']\s+"\$request"\s*\|\s*timeout\s+"[^"]+"\s+"\$SKC_SESSION_SKC_BIN"\s+--internal-tmux-owner-isolation\s*\)"\s*;\s*then/g,
		},
	];
	...
	for (const match of contents.matchAll(pythonCommandAssignment)) {
		...
		violation(match.index ?? 0, "human-only tmux owner must not construct or wrap a non-lifecycle SKC argv");
	}

	const allowedSkcEnvironmentReferences: ShellRange[] = canonicalCommands.map(match => ({
		start: match.index ?? 0,
		end: (match.index ?? 0) + match[0].length,
	}));
	const allowedCommandReferences = [...allowedSkcEnvironmentReferences];
	...
	for (const match of contents.matchAll(subprocessCalls)) {
		...
		const isLifecycleCall =
			/^\s*\[\s*os\.environ\[\s*["']SKC_SESSION_SKC_BIN["']\s*\]\s*,\s*["']--internal-tmux-owner-isolation["']\s*\]\s*$/.test(
				argument.text,
			);
		...
		if (isLifecycleCall) {
			...
			allowedSkcEnvironmentReferences.push({ start: argument.start, end: argument.start + argument.text.length });
			continue;
		}
		if (/\bcommand\b|os\.environ\[\s*["']SKC_SESSION_SKC_BIN["']\s*\]/.test(argument.text)) {
			violation(
				match.index ?? 0,
				"human-only tmux owner must invoke SKC only with zero interactive argv or the exact owner-isolation lifecycle argv",
			);
		}
	}
	for (const match of contents.matchAll(/\[\s*["']skc["'](?:\s*,|\s*\])/g)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal skc executable");
	}
	for (const match of contents.matchAll(/(?:^|\n)\s*(?:export\s+)?[A-Za-z_][A-Za-z0-9_]*\s*=\s*["']skc["']\s*$/gm)) {
		violation(match.index ?? 0, "human-only tmux owner must not alias or wrap the skc executable");
	}
	for (const match of contents.matchAll(
		/(?:^|\n)\s*(?:alias\s+[A-Za-z_][A-Za-z0-9_]*\s*=\s*['"]?skc\b|(?:function\s+)?[A-Za-z_][A-Za-z0-9_]*\s*\(\s*\)\s*\{[^\n]*\bskc\b)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not alias or wrap the skc executable");
	}
	for (const match of contents.matchAll(
		/(?:^|[|;&({]\s*|\b(?:command|exec|env|sudo|nice|nohup)\s+)["']?skc["']?(?=\s|$)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal skc executable");
	}
	for (const match of contents.matchAll(
		/\benv(?:\s+(?:[A-Za-z_][A-Za-z0-9_]*=(?:"[^"]*"|'[^']*'|[^\s]+)|"[^"]*"|'[^']*'))*\s+["']?skc["']?(?=\s|$)/gm,
	)) {
		violation(match.index ?? 0, "human-only tmux owner must not launch a literal skc executable");
	}
	...
	if (interactiveLaunches !== 1) {
		violation(0, "human-only tmux owner must launch exactly one zero-argv interactive SKC process");
	}
	for (const match of contents.matchAll(/\bcommand\b/g)) {
		...
		if (!allowedCommandReferences.some(range => offset >= range.start && offset < range.end)) {
			violation(offset, "human-only tmux owner must not construct or wrap a non-lifecycle SKC argv");
		}
	}
	...
	for (const match of contents.matchAll(/os\.environ\[\s*["']SKC_SESSION_SKC_BIN["']\s*\]/g)) {
		const offset = match.index ?? 0;
		if (!allowedSkcEnvironmentReferences.some(range => offset >= range.start && offset < range.end)) {
			violation(offset, "human-only tmux owner must not construct or wrap a non-lifecycle SKC argv");
		}
	}
	...
	const shellSkcInvocations =
		/(?:^|[|;&(]\s*|\b(?:command|exec)\s+|\btimeout\s+(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s]+)\s+)(["']?)\$(?:SKC_BIN|SKC_SESSION_SKC_BIN|\{(?:SKC_BIN|SKC_SESSION_SKC_BIN)\})\1([^\r\n;|&)]*)/gm;
	for (const match of normalizedContents.matchAll(shellSkcInvocations)) {
		...
		if (
			...
			violation(invocationStart, "human-only tmux owner invokes SKC with a non-lifecycle startup argument");
		}
	}
	for (const reference of shellSkcBinaryReferenceViolations(contents)) {
		violation(reference.start, "human-only tmux owner must not construct or wrap a non-lifecycle SKC argv");
	}
	for (const match of contents.matchAll(/\bSKC_SESSION_FLAGS\b/g)) {
...
function exactTeamRuntimeSendKeysRanges(contents: string): ShellRange[] {
	...
	const continuationFunction =
		/async\s+function\s+continueStalledSkcTeamWorkers\s*\([^)]*\)\s*:\s*Promise<void>\s*\{/.exec(contents);
	const monitorFence = /await\s+withSkcTeamTaskMutation\([\s\S]{0,500}?async\s+capability\s*=>\s*\{/.exec(contents);
	...
	const continuationCall = /await\s+continueStalledSkcTeamWorkers\s*\(/.exec(monitorBody);
	const reconciliationCall = /await\s+reconcileSkcTeamStaleClaimsUnlocked\s*\(/.exec(monitorBody);
...
async function scan(): Promise<string[]> {
	...
	for (const file of files) {
		...
		if (file.startsWith(retiredPythonRpcPackagePath)) {
			violations.push(`${file}: retired Python skc-rpc package source survived`);
		}
		...
		if (isPublishedSkcSessionShellHelper(file)) {
			const contents = await Bun.file(path.join(repoRoot, file)).text();
		...
		for (const match of contents.matchAll(/\bSKC_BRIDGE_[A-Z0-9_]*\b/g)) {
			violations.push(
		...
		if (isProductionTypeScriptOrJavaScript(file)) {
			...
			if (
				...
				!/^export const SKC_TEAM_CONTINUATION_PROMPT =\n\t"Continue only your current claimed SKC team task\. Re-read current SKC team state; do not replay prior output; report status\.";$/m.test(
					contents,
				)
			)
				violations.push(
...
async function runSelfTestFixture(
	...
	const fixture = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-canonicalization-"));
	try {
		...
		const scanner = Bun.spawn([process.execPath, import.meta.path], {
			cwd: repoRoot,
			env: { ...process.env, SKC_SDK_CANONICALIZATION_SCAN_ROOT: fixture },
			stdout: "pipe",
			stderr: "pipe",
			timeout: timeoutMs,
		});
...
async function selfTest(): Promise<void> {
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/rpc-compat-mode.ts": 'Bun.spawnSync(["skc", "--mode", "rpc-compat"]);\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/bridge-compat-mode.ts":
				'Bun.spawnSync(["skc", "--mode", "bridge-compat"]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/aliased-retired-modes.ts":
				'const rpcMode = "rpc";\nlet bridgeMode = "bridge-compat";\nvar unattendedMode = "unattended";\nBun.spawnSync(["skc", "--mode", rpcMode]);\nBun.spawnSync(["skc", "--mode", bridgeMode]);\nBun.spawnSync(["skc", "--mode", unattendedMode]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	...
	await runExactRetiredModeFixture(
		...
		'const launch = () => Bun.spawnSync(["skc", "--mode", mode]);\nconst mode = "rpc";\nlaunch();\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("closure-forward-const.ts"), 1)],
	);
	await runExactRetiredModeFixture(
		"safe-block-shadow.ts",
		'const mode = "rpc"; { const mode = "acp"; Bun.spawnSync(["skc", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"parameter-shadow.ts",
		'const mode = "rpc"; function launch(mode: string) { Bun.spawnSync(["skc", "--mode", mode]); } launch("acp");\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"catch-shadow.ts",
		'const mode = "rpc"; try { throw "acp"; } catch (mode) { Bun.spawnSync(["skc", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"destructure-shadow.ts",
		'const mode = "rpc"; const { mode: shadow } = { mode: "acp" }; Bun.spawnSync(["skc", "--mode", shadow]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"sibling-shadow.ts",
		'{ const mode = "rpc"; void mode; } { const mode = "acp"; Bun.spawnSync(["skc", "--mode", mode]); }\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"direct-tdz.ts",
		'Bun.spawnSync(["skc", "--mode", mode]); const mode = "rpc";\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"pre-use-safe-to-retired.ts",
		'let mode = "acp"; mode = "rpc"; Bun.spawnSync(["skc", "--mode", mode]);\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("pre-use-safe-to-retired.ts"))],
	);
	await runExactRetiredModeFixture(
		"pre-use-retired-to-safe.ts",
		'let mode = "rpc"; mode = "acp"; Bun.spawnSync(["skc", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"later-safe-write.ts",
		'let mode = "rpc"; Bun.spawnSync(["skc", "--mode", mode]); mode = "acp";\n',
		1,
		[retiredModeDiagnostic(retiredModeFixturePath("later-safe-write.ts"))],
	);
	await runExactRetiredModeFixture(
		"later-retired-write.ts",
		'let mode = "acp"; Bun.spawnSync(["skc", "--mode", mode]); mode = "rpc";\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"branch-write.ts",
		'let mode = "acp"; if (condition) mode = "rpc"; Bun.spawnSync(["skc", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"compound-write.ts",
		'let mode = "rpc"; mode += "-compat"; Bun.spawnSync(["skc", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"update-write.ts",
		'let mode = "rpc"; mode++; Bun.spawnSync(["skc", "--mode", mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"dynamic-member-controls.ts",
		'const dynamicMode = process.env.MODE; const state = { mode: "rpc" }; Bun.spawnSync(["skc", "--mode", dynamicMode]); Bun.spawnSync(["skc", "--mode", state.mode]);\n',
		0,
		[],
	);
	await runExactRetiredModeFixture(
		"typed-acp-control.ts",
		'const mode: string = "acp"; Bun.spawnSync(["skc", "--mode", mode]);\n',
		0,
		[],
	);
	...
	await runExactRetiredModeFixture(
		"cycle.ts",
		'const a = b; const b = a; Bun.spawnSync(["skc", "--mode", a]);\n',
		0,
		[],
		{ timeoutMs: 2_000 },
	);
	await runExactRetiredModeFixture("deduplicated-direct-mode.ts", 'Bun.spawnSync(["skc", "--mode", "rpc"]);\n', 1, [
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/package.json":
				'{"exports":{"./commands/skc-runtime-bridge":"./src/bridge.ts","./unattended":"./src/unattended.ts"}}\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/package.json":
				'{"exports":{"./modes/rpc/*":null,"./commands/skc-runtime-bridge":null,"./unattended":null,"./modes/shared/agent-wire/*":null}}\n',
		},
		0,
	);
	await runSelfTestFixture(
	...
	await runSelfTestFixture(
		{
			"package.json": '{"catalog":{"@sayknow-cli/bridge-client":"0.10.1"}}\n',
			"packages/bridge-client/package.json": '{"name":"@sayknow-cli/bridge-client"}\n',
			"packages/bridge-client/src/index.ts": "export class SdkClient {}\nexport type SdkClientOptions = {};\n",
			"packages/coding-agent/package.json": '{"dependencies":{"@sayknow-cli/bridge-client":"catalog:"}}\n',
			"packages/coding-agent/src/sdk/client/client.ts":
				'export { SdkClient } from "@sayknow-cli/bridge-client";\nexport type { SdkClientOptions } from "@sayknow-cli/bridge-client";\n',
		},
		0,
	);
	await runSelfTestFixture(
		{
			"packages/bridge-client/package.json":
				'{"name":"@sayknow-cli/bridge-client","dependencies":{"unsafe":"1.0.0"}}\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/bridge-client/src/client.ts":
				'import { SdkClient } from "@sayknow-cli/coding-agent";\nvoid SdkClient;\n',
		},
		1,
		"bridge-client imports coding-agent or AgentSession authority",
	);
	await runSelfTestFixture(
		{ "packages/renamed-workspace/package.json": '{"name":"@sayknow-cli/bridge-client"}\n' },
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { handshake } from "@sayknow-cli/bridge-client";\nvoid handshake;\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { commands } from "@sayknow-cli/bridge-client";\nvoid commands;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { SdkClient } from "@sayknow-cli/bridge-client/commands";\nvoid SdkClient;\n',
		},
		1,
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts": 'import { SSE } from "@sayknow-cli/bridge-client";\nvoid SSE;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/consumer.ts":
				'import { control } from "@sayknow-cli/bridge-client";\nvoid control;\n',
		},
		1,
		"imports historical bridge-client protocol surface",
	);
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/neutral-name.ts": 'Bun.spawnSync(["skc", "--mode", "rpc"]);\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/test/fixtures/neutral-expect.ts":
				'expect(Bun.spawnSync(["skc", "--mode", "rpc"])).toBeDefined();\n',
		},
		1,
		"invokes removed --mode rpc",
	);
	...
	await runSelfTestFixture(
		{ "scripts/skc-session/prompt.sh": "#!/usr/bin/env bash\n" },
		1,
		"retired tmux machine prompt or viewing helper survived",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/unsafe.sh":
...
completed = subprocess.run([os.environ["SKC_SESSION_SKC_BIN"], "--internal-tmux-owner-isolation"], input="{}")
PLAN_RESPONSE="$(printf '%s\\n' "$PLAN_LINE" | "$SKC_BIN" --internal-tmux-owner-isolation)"
POST_SPAWN_RESPONSE="$(printf '%s\\n' "$PLAN_LINE" | "$SKC_BIN" --internal-tmux-owner-isolation)"
GENERATION_PUBLISH_RESPONSE="$(printf '%s\\n' "$GENERATION_PUBLISH_REQUEST" | "$SKC_BIN" --internal-tmux-owner-isolation)"
if verdict="$(printf '%s\\n' "$request" | timeout "1s" "$SKC_SESSION_SKC_BIN" --internal-tmux-owner-isolation)"; then
  true
fi
`;
	await runSelfTestFixture({ "scripts/skc-session/create.sh": canonicalCreateFixture }, 0);
	await runSelfTestFixture(
		{ "scripts/skc-session/create.sh": `${canonicalCreateFixture}\nsubprocess.Popen(["skc"])\n` },
		1,
		"must not launch a literal skc executable",
	);
	await runSelfTestFixture(
		{ "scripts/skc-session/create.sh": `${canonicalCreateFixture}\nlauncher="skc"\n"$launcher"\n` },
		1,
		"must not alias or wrap the skc executable",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": `${canonicalCreateFixture}\nrunner(){ skc --internal-tmux-owner-isolation; }\nrunner\n`,
		},
		1,
		"must not alias or wrap the skc executable",
	);
	await runSelfTestFixture(
		{ "scripts/skc-session/create.sh": `${canonicalCreateFixture}\n"$SKC_BIN" --internal-tmux-owner-isolation\n` },
		1,
		"invokes SKC with a non-lifecycle startup argument",
	);
	await runSelfTestFixture(
		{ "scripts/skc-session/create.sh": `${canonicalCreateFixture}\nSKC_SESSION_FLAGS=unsafe\n` },
		1,
		"exposes caller-provided startup arguments",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
				'[[ $# -eq 2 ]] || { echo "Usage: $0 <session-name> <worktree-path>" >&2; exit 2; }',
				"[[ $# -eq 2 ]] || true",
			),
		},
	...
	await runSelfTestFixture(
		...
		1,
		"must launch exactly SKC_SESSION_SKC_BIN with zero startup arguments",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		...
		1,
		"must not construct or wrap a non-lifecycle SKC argv",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		...
		1,
		"must invoke SKC only with zero interactive argv or the exact owner-isolation lifecycle argv",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
	...
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": `${canonicalCreateFixture}\nlaunch = [os.environ["SKC_SESSION_SKC_BIN"], "--file", "task.md"]\nsubprocess.Popen(launch)\n`,
		},
		1,
		"must not construct or wrap a non-lifecycle SKC argv",
	);
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
				'[os.environ["SKC_SESSION_SKC_BIN"], "--internal-tmux-owner-isolation"]',
				'[\n  os.environ["SKC_SESSION_SKC_BIN"],\n  "--internal-tmux-owner-isolation",\n  "--file",\n  "task.md",\n]',
			),
		},
		1,
		"must invoke SKC only with zero interactive argv or the exact owner-isolation lifecycle argv",
	...
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
				'"$SKC_BIN" --internal-tmux-owner-isolation',
				'command -p "$SKC_BIN" --file task.md',
			),
		},
		1,
		"must not construct or wrap a non-lifecycle SKC argv",
	...
	await runSelfTestFixture(
		{
			"scripts/skc-session/create.sh": canonicalCreateFixture.replace(
				'"$SKC_BIN" --internal-tmux-owner-isolation',
				'exec -a skc "$SKC_BIN" --file task.md',
			),
		},
		1,
		"must not construct or wrap a non-lifecycle SKC argv",
	...
	const realCreate = await Bun.file(path.join(repoRoot, "scripts/skc-session/create.sh")).text();
	await runSelfTestFixture({ "scripts/skc-session/create.sh": realCreate }, 0);
	await runSelfTestFixture(
		{ "scripts/skc-session/watch-output.sh": "#!/usr/bin/env bash\n$TMUX_BIN \\\n  pipe-pane -t session 'sink'\n" },
		1,
		"published shell helper performs tmux machine prompt injection or pane viewing",
	);
	await runSelfTestFixture(
		{ "docs/skc-session-clawhip-routing.md": "$TMUX_BIN \\\n  pipe-pane -t owner 'sink'\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{
			"docs/skc-session-clawhip-routing.md": './scripts/skc-session/create.sh bot /repo \\\n  --print "task"\n',
		},
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	...
	await runSelfTestFixture(
		{ "docs/skc-session-clawhip-routing.md": "scripts/skc-session/tail.sh visible output\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{ "docs/skc-session-clawhip-routing.md": "tmux pipe-pane -t owner 'sink'\n" },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
		{ "docs/skc-session-clawhip-routing.md": './scripts/skc-session/create.sh bot /repo --print "task"\n' },
		1,
		"published machine documentation directs tmux prompt or viewing access",
	);
	await runSelfTestFixture(
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/skc-runtime/team-runtime.ts":
...
async function continueStalledSkcTeamWorkers(): Promise<void> {
...
const SKC_TEAM_CONTINUATION_PROMPT = "Continue only your current claimed SKC team task. Re-read current SKC team state; do not replay prior output; report status.";
const SKC_TEAM_CONTINUATION_DISPATCH_TIMEOUT_MS = 5_000;
const args = Object.freeze([
	"send-keys",
	"-l",
	"-t",
	worker.pane_id,
	SKC_TEAM_CONTINUATION_PROMPT,
	";",
	"send-keys",
	"-t",
	worker.pane_id,
...
const dispatch = skcTeamRuntimeTestSeams?.continuationTmuxDispatch
	? skcTeamRuntimeTestSeams.continuationTmuxDispatch(config.tmux_command, args)
	: Bun.spawnSync([config.tmux_command, ...args], {
		stdout: "ignore",
		stderr: "ignore",
		timeout: SKC_TEAM_CONTINUATION_DISPATCH_TIMEOUT_MS,
	});
}
async function monitorSkcTeam(): Promise<void> {
	await withSkcTeamTaskMutation(taskStore(dir), async capability => {
		await continueStalledSkcTeamWorkers();
		await reconcileSkcTeamStaleClaimsUnlocked(workerOrchestrationRuntime, teamName, dir, config, env, capability);
	});
}
`;
	await runSelfTestFixture(
		{ "packages/coding-agent/src/skc-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture },
		0,
	);
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/skc-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture.replace(
				"async function continueStalledSkcTeamWorkers(): Promise<void>",
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/skc-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture.replaceAll(
				"worker.pane_id",
				'"%99"',
			),
		},
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/skc-runtime/team-runtime.ts": canonicalTeamRuntimeSendKeysFixture.replace(
				'"send-keys",\n\t"-l",\n\t"-t",\n\tworker.pane_id,\n\tSKC_TEAM_CONTINUATION_PROMPT,\n\t";",\n\t"send-keys",\n\t"-t",\n\tworker.pane_id,\n\t"Enter",',
				'"send-keys", "-t", worker.pane_id, "Enter", ";", "send-keys", "-l", "-t", worker.pane_id, SKC_TEAM_CONTINUATION_PROMPT,',
			),
		},
		1,
		"tmux send-keys content injection is outside sanctioned process lifecycle",
	...
	await runSelfTestFixture(
		{
			"packages/coding-agent/src/bridge-env.ts":
				"const endpoint = process.env.SKC_BRIDGE_ENDPOINT;\nvoid endpoint;\n",
		},
		1,
		"forbidden bridge environment reference SKC_BRIDGE_ENDPOINT",
	);
	await runSelfTestFixture(
		{
	...
	await runSelfTestFixture(
		{ "python/skc-rpc/src/skc_rpc/client.py": "class RpcClient: pass\n" },
		1,
		"retired Python skc-rpc package source survived",
	);
	await runSelfTestFixture(
		{ "python/roboskc/src/controller.py": "from skc_rpc import RpcClient\n" },
		1,
		"imports removed skc_rpc Python client",
	);
	await runSelfTestFixture(
		{ "scripts/legacy-controller.py": 'subprocess.run(["skc", "--mode", "rpc"])\n' },
		1,
		"invokes removed --mode rpc",
	);
	await runSelfTestFixture(
		{ "python/roboskc/pyproject.toml": '[project]\ndependencies = ["skc-rpc>=0.1.0"]\n' },
		1,
		"declares removed skc-rpc distribution metadata",
	);
	await runSelfTestFixture(
		{ "python/roboskc/src/controller.py": 'client.negotiate_unattended(actor="legacy")\n' },
		1,
	...
	await runSelfTestFixture({ "artifacts/closure-report.json": '{"legacy":"skc --mode rpc"}\n' }, 0);
	await runSelfTestFixture(
...
try {
	...
	if (violations.length > 0) {
		process.stderr.write(`SKC SDK canonicalization violations found:\n${violations.join("\n")}\n`);
		process.exit(1);
	}
	process.stdout.write(
		`SKC SDK canonicalization verification passed (${sanctionedServerHosts.size} sanctioned server hosts).\n`,
	);
} catch (error) {
