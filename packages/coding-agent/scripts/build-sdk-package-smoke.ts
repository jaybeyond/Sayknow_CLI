...
const packageName = "@sayknow-cli/coding-agent";
...
async function runSmoke(): Promise<Surface> {
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-sdk-package-smoke-"));
	try {
			...
			JSON.stringify(
				{
					...
					private: true,
					dependencies: {
						"@sayknow-cli/ai": `file:${aiTarballPath}`,
						"@sayknow-cli/bridge-client": `file:${bridgeClientTarballPath}`,
						[packageName]: `file:${codingAgentTarballPath}`,
						"@sayknow-cli/tui": `file:${tuiTarballPath}`,
						"@sayknow-cli/natives": `file:${nativesTarballPath}`,
						"@sayknow-cli/natives-linux-x64": `file:${linuxX64TarballPath}`,
					},
					overrides: {
						"@sayknow-cli/ai": `file:${aiTarballPath}`,
						"@sayknow-cli/bridge-client": `file:${bridgeClientTarballPath}`,
