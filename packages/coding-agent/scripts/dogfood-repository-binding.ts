 ...
 *
 * Creates two sibling git repos, runs real `skc ultragoal` / `skc ralplan`
...
import { readUltragoalPlan, startNextUltragoalGoal } from "../src/skc-runtime/ultragoal-runtime";
...
async function main(): Promise<void> {
	const dogfoodRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "skc-dogfood-2901-"));
	const left = path.join(dogfoodRoot, "sayknow-cli");
	const right = path.join(dogfoodRoot, "oh-my-openagent-senpi");
	await initRepo(left);
	await initRepo(right);

	const sessionId = `dogfood-2901-${process.pid}`;
	const env = { ...process.env, SKC_SESSION_ID: sessionId };
	...
	console.log("## 1) skc ultragoal create-goals (LEFT)");
	...
	console.log();
	console.log("## 7) skc ralplan seed (LEFT)");
	...
	const statePath = path.join(left, ".skc", `_session-${sessionId}`, "state", "ralplan-state.json");
	...
	console.log();
	console.log("## 9) skc ralplan --write planner on LEFT (match)");
