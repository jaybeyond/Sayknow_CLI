# @sayknow-cli/agent-core

Stateful agent with tool execution and event streaming. Built on `@sayknow-cli/ai`.

## Installation

```bash
npm install @sayknow-cli/agent-core
```
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@sayknow-cli/ai`).

```typescript
import { z } from "@sayknow-cli/ai";
...
```typescript
import { Agent, streamProxy } from "@sayknow-cli/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@sayknow-cli/agent-core";
