...
describe("compaction native tokenizer loading", () => {
	...
	it("does not use a package-name dynamic require for @sayknow-cli/natives", async () => {
		...
		expect(source).not.toContain('requireFromHere("@sayknow-cli/natives")');
		expect(source).not.toContain('require("@sayknow-cli/natives")');
	});
});
