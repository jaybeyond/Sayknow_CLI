#!/usr/bin/env bun
/**
 * Plans or runs live Composer V3 matrix capture via isolated SKC print-mode sessions.
...
function parseArgs(argv: string[]): CaptureArgs {
	...
	let skcBin = process.env.SKC_BIN?.trim() || "skc";
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (arg === "--skc") skcBin = argv[++i] ?? skcBin;
	...
	return { dryRun, skipCredCheck, k, out, candidateModel, baselineModel, skcBin, scenarioFilter, timeoutSec };
}
...
async function runSkcPrint(input: {
	skcBin: string;
	cwd: string;
	model: string;
	prompt: string;
...
async function executeRow(
	...
	const { exitCode, stderr } = await runSkcPrint({
		skcBin: args.skcBin,
		cwd: workdir,
		model: row.model,
...
async function main(): Promise<void> {
	...
	const outDir =
		args.out ?? path.join(REPO_ROOT, ".skc/ultragoal/artifacts", `composer-evidence-${runId}`);
	const tracePath = path.join(outDir, "traces", "real-skc-print-traces.json");
	...
	const payload = {
		...
		skc_bin_basename: path.basename(args.skcBin),
		planned,
	};
