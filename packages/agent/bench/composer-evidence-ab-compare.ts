#!/usr/bin/env bun
/**
 * Compare two scored trace corpora (e.g. skc v0.5.3 vs v0.6.4 live captures).
...
function armSummary(label: string, skcVersion: string, report: EvidenceReport) {
	return {
		label,
		skc_version: skcVersion,
...
async function main(): Promise<void> {
	...
	const payload = {
		...
		comparison_kind: "historical-frozen-trace",
		disclaimer:
			"Point estimate from paired harness failure counts on frozen trace corpora. Not a statistical hypothesis test. Live A/B requires separate captures with each skc binary on the same versioned Composer scenario prompts.",
