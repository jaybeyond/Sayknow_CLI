/**
 * OpenAI Responses HTTP wire-format ↔ skc Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes skc's {@link AssistantMessage} (and event stream) back into
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that skc's gateway doesn't honour;
