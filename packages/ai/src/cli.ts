...
async function login(provider: OAuthProvider): Promise<void> {
	...
	try {
		...
		console.log(`\nCredentials saved to ~/.skc/agent/agent.db`);
	} finally {
		store.close();
		rl.close();
	}
...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		console.log(`Usage: bunx @sayknow-cli/ai <command> [provider]
...
Examples:
  ...
  bunx @sayknow-cli/ai logout anthropic   # logout from specific provider
  bunx @sayknow-cli/ai status             # show logged-in providers
  bunx @sayknow-cli/ai list               # list providers
`);
		return;
	}
	...
	if (command === "status") {
		...
		try {
			...
			if (providers.length === 0) {
				...
				console.log(`Use 'bunx @sayknow-cli/ai login' to authenticate.`);
			} else {
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some(p => p.id === provider)) {
			...
			console.error(`Use 'bunx @sayknow-cli/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @sayknow-cli/ai --help' for usage`);
	process.exit(1);
}
