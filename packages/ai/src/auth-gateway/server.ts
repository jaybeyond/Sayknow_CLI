 ...
 * pure protocol translation: foreign wire → skc Context → pi-ai stream() →
 * skc events → foreign wire.
 *
 * Endpoints:
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (skc re-stamps every parsed Message). role +
...
async function handleFormatEndpoint(
	...
	// Parse + validate against the strict format schema, rebuild as skc's
