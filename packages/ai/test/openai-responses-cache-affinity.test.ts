...
describe("openai-responses cache affinity", () => {
	...
	it("uses SKC_CACHE_RETENTION when request and model omit cacheRetention", async () => {
		const previous = Bun.env.SKC_CACHE_RETENTION;
		...
		try {
			...
			if (previous === undefined) {
				...
				Bun.env.SKC_CACHE_RETENTION = previous;
			}
		}
	});
});
