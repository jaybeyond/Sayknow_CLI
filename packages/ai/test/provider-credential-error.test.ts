...
import { formatMissingApiKeyError, formatProviderCredentialHint, streamSimple } from "@sayknow-cli/ai/stream";
import type { Context, Model } from "@sayknow-cli/ai/types";
...
describe("formatProviderCredentialHint", () => {
	...
	it("covers opencode-zen with the same shape", () => {
		...
		expect(hint).toContain("OPENCODE_API_KEY");
		expect(hint).toContain("skc auth-broker login opencode-zen");
	});
	...
	it("names the env var for a plain env-key provider without an OpenCode note or invalid login command", () => {
		...
		expect(hint).not.toContain("skc auth-broker login");
	});
