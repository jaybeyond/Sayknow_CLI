...
describe("Google Gemini CLI user agent", () => {
	const originalSkcVersion = process.env.SKC_AI_GEMINI_CLI_VERSION;
	const originalPiVersion = process.env.PI_AI_GEMINI_CLI_VERSION;

	afterEach(() => {
		if (originalSkcVersion === undefined) {
			delete process.env.SKC_AI_GEMINI_CLI_VERSION;
		} else {
			process.env.SKC_AI_GEMINI_CLI_VERSION = originalSkcVersion;
		}
	...
	it("prefers the documented SKC Gemini CLI version override", () => {
		process.env.SKC_AI_GEMINI_CLI_VERSION = "9.8.7";
