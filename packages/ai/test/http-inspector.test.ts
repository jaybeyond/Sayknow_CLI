...
let previousSkcConfigDir: string | undefined;
...
async function useTempAgentDir(): Promise<string> {
	...
	previousSkcConfigDir = process.env.SKC_CONFIG_DIR;
	tempConfigRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skc-http-inspector-"));
...
afterEach(async () => {
	...
	if (previousSkcConfigDir === undefined) {
		delete process.env.SKC_CONFIG_DIR;
	} else {
		process.env.SKC_CONFIG_DIR = previousSkcConfigDir;
	}
	previousSkcConfigDir = undefined;
	if (previousAgentDir) {
...
describe("HTTP 400 error message safety (issue #438)", () => {
	...
	it("surfaces model/provider selection guidance for unavailable-model 400s", async () => {
		...
		expect(message).toContain("skc --list-models");
		expect(message).toContain("skc setup provider");
	...
	it("omits model/provider names from guidance when the dump is absent", () => {
		...
		expect(guidance).toContain("skc --list-models");
