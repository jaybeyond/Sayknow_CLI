 ...
 * (override via `SKC_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.skc/auth-gateway.token`.
 *
 * To run: `bun --cwd packages/ai test test/auth-gateway-openai-responses-caching.test.ts`
 * with the gateway live (`skc auth-gateway serve` or pm2).
 */
...
const GATEWAY_URL = Bun.env.SKC_E2E_GATEWAY_URL ?? "http://127.0.0.1:4000";
const TOKEN_PATH = path.join(os.homedir(), ".skc", "auth-gateway.token");
...
const MODEL = Bun.env.SKC_E2E_OPENAI_RESPONSES_MODEL ?? "gpt-5.3-codex";
...
the skc auth-gateway's OpenAI Responses prompt-caching pipeline. The same
