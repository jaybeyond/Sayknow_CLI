/**
 * Stream local SKC session JSONL files and mine compaction behavior.
 *
...
const SESSIONS_ROOT = join(homedir(), ".skc", "agent", "sessions");
...
/** User ~/.skc/agent/models.yml, verified 2026-07-16. Exact keys take precedence. */
...
async function main() {
  ...
  const output = {
    provenance: {
      sessionStore: "Local SKC session store (path redacted)",
      modelWindowMap: "User ~/.skc/agent/models.yml, verified 2026-07-16; exact provider/model keys precede documented provider/model prefixes.",
