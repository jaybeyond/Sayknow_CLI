...
const EXPECTED_PUBLIC_PACKAGE_VERSION_CATALOG_KEY = "@sayknow-cli/coding-agent";
...
const FORBIDDEN_PACKAGE_IMPORTS = [
	...
	"@sayknow-cli/coding-agent/runtime-mcp/index",
	"@sayknow-cli/coding-agent/runtime-mcp/manager",
	"@sayknow-cli/coding-agent/commands/skc-runtime-bridge",
	...
	"@sayknow-cli/coding-agent/exa",
	"@sayknow-cli/coding-agent/exa/factory",
	"@sayknow-cli/coding-agent/exa/mcp-client",
	"@sayknow-cli/coding-agent/exa/search",
	"@sayknow-cli/coding-agent/exa/types",
	"@sayknow-cli/coding-agent/internal-urls/mcp-protocol",
	"@sayknow-cli/coding-agent/modes/components/runtime-mcp-add-wizard",
	"@sayknow-cli/coding-agent/modes/controllers/runtime-mcp-command-controller",
	"@sayknow-cli/coding-agent/slash-commands/helpers/mcp",
] as const;
const FORBIDDEN_PACKAGE_SYMBOLS = [
	{
		specifier: "@sayknow-cli/coding-agent",
		symbols: ["exaTools", "callExaTool", "searchTools", "researcherTools", "websetsTools"],
	},
	{
		specifier: "@sayknow-cli/coding-agent/tools",
		symbols: ["exaTools", "callExaTool", "searchTools", "researcherTools", "websetsTools"],
	},
] as const;
...
async function verifyRebrandSurface(): Promise<GateResult> {
	...
	const hasSkcBin = typeof bin.skc === "string";
	...
	return {
		...
		passed: rootName === "sayknow-cli" && codingName.includes("sayknow") && hasSkcBin && !hasLegacyBin,
		details,
	};
}
...
async function verifyPackageVersionAndBinaryAllowlist(): Promise<GateResult> {
	...
	for (const relativePath of packagePaths) {
		...
		if (relativePath === "package.json") {
			if (packageName !== "sayknow-cli") nameFindings.push(`${relativePath}: expected sayknow-cli, found ${packageName}`);
			continue;
		}

		if (!packageName.startsWith("@sayknow-cli/") && !ALLOWED_UNSCOPED_PACKAGE_NAMES.has(packageName)) {
			nameFindings.push(`${relativePath}: package name ${packageName} is outside @sayknow-cli scope`);
		}
...
async function verifyVisibleDefinitions(): Promise<GateResult> {
	const otherDefinitionRoots = [".skc/skills", ".skc/agents", ".skc/commands", ".skc/rules"];
	const otherDefinitions: string[] = [];
	const details: string[] = [];
	const bundledSkills = readVisibleEntries("packages/coding-agent/src/defaults/skc/skills").filter(entry =>
		fs.existsSync(path.join(repoRoot, "packages/coding-agent/src/defaults/skc/skills", entry, "SKILL.md")),
	);
	...
	const ignoredDefinitions = getIgnoredDefinitionPaths([
		...expectedSkills.map(name => `packages/coding-agent/src/defaults/skc/skills/${name}/SKILL.md`),
	...
	details.push(`repo-visible .skc definitions: ${otherDefinitions.join(", ") || "<none>"}`);
...
async function verifyPublicDefinitionContent(): Promise<GateResult> {
	...
	for (const definition of EXPECTED_DEFINITIONS) {
		const relativePath = `packages/coding-agent/src/defaults/skc/skills/${definition}/SKILL.md`;
	...
	for (const agent of EXPECTED_ROLE_AGENTS) {
		...
		const text = await readText(relativePath);
		if (new RegExp(String.raw`\b` + "om" + "x" + String.raw`\b`, "iu").test(text)) findings.push(`${relativePath}: contains SKC team runtime naming`);
		for (const pattern of FORBIDDEN_SKILL_PATTERNS) {
			if (pattern.test(text)) findings.push(`${relativePath}: ${pattern.source}`);
		}
	}
...
async function verifyBroadWorkflowExposure(): Promise<GateResult> {
	...
	const publicDefinitionRoots = [".skc/skills", ".skc/agents", ".skc/commands", ".skc/rules"];
	...
	for (const [relativePath, text] of activeSkillTexts) {
		for (const token of FORBIDDEN_WORKFLOW_SURFACE_TOKENS) {
			const pattern = new RegExp(`(?:\\$|\\bskc\\s+|\\b${"om" + "x"}\\s+)${escapeRegExp(token)}\\b`, "u");
...
async function verifyMcpQuarantine(): Promise<GateResult> {
	const codingPackage = await readJson("packages/coding-agent/package.json");
	const exportsRecord = isRecord(codingPackage.exports) ? codingPackage.exports : {};
	const mcpExportKeys = Object.keys(exportsRecord).filter(key => key === "./mcp" || key.startsWith("./mcp/") || key === "./runtime-mcp" || key.startsWith("./runtime-mcp/") || key === "./commands/skc-runtime-bridge");
