...
describe("unscoped sayknow-cli package publication", () => {
	test("manifest exposes skc and depends on the scoped CLI package", async () => {
		...
		expect(aliasManifest.name).toBe("sayknow-cli");
		...
		// publisher resolves it to the current @sayknow-cli/coding-agent version.
	...
	test("release dependency normalization collapses repeated file prefixes", () => {
		...
		expect(normalizeFileDependencySpec("file:file:file:///tmp/sayknow-cli/packages/ai")).toBe(
			"file:///tmp/sayknow-cli/packages/ai",
		);
	...
	test("plans the real sayknow-cli wrapper edge before the wrapper is published", () => {
		const records = canonicalEvidenceRecords();
		const wrapper = records.find(record => record.name === "sayknow-cli")!;
		wrapper.internal_dependencies = { "@sayknow-cli/coding-agent": "1.2.3" };

		const plannedNames = planExpectedEvidencePublication(records).map(record => record.name);
		expect(plannedNames.indexOf("@sayknow-cli/coding-agent")).toBeLessThan(plannedNames.indexOf("sayknow-cli"));
	});

	test("topologically moves an early declared package behind a late dependency", () => {
		const records = canonicalEvidenceRecords();
		const utils = records.find(record => record.name === "@sayknow-cli/utils")!;
		utils.internal_dependencies = { "sayknow-cli": "1.2.3" };

		const plannedNames = planExpectedEvidencePublication(records).map(record => record.name);
		expect(plannedNames.indexOf("sayknow-cli")).toBeLessThan(plannedNames.indexOf("@sayknow-cli/utils"));
	});

	test("rejects a closed-set internal dependency cycle before registry publication begins", async () => {
		const records = canonicalEvidenceRecords();
		records.find(record => record.name === "@sayknow-cli/utils")!.internal_dependencies = { "@sayknow-cli/ai": "1.2.3" };
		records.find(record => record.name === "@sayknow-cli/ai")!.internal_dependencies = { "@sayknow-cli/utils": "1.2.3" };
	...
	test("executes dependency-safe publication from canonically name-sorted evidence", async () => {
		...
		const nativesName = "@sayknow-cli/natives";
	...
	test("rejects duplicate, missing, and extra evidence records before publication", async () => {
		...
		await expect(publishExpectedEvidencePackages([
			...records,
			{ ...records[0]!, dir: "packages/unexpected", name: "@sayknow-cli/unexpected" },
