...
describe("planTasks command shape (issue #622)", () => {
	...
	test("roboskc web tasks run `bun run <script>` in the web cwd", () => {
		const tasks = planForPaths(["python/roboskc/web/app.ts"]);
		const typecheck = tasks.find(task => task.key === "roboskc-web-typecheck");
		const build = tasks.find(task => task.key === "roboskc-web-build");
