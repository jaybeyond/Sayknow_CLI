#!/usr/bin/env bash
# Show the last N lines of a SKC tmux session pane.
...
show_durable_state() {
  ...
  if [[ -f "$state_dir/metadata.json" ]]; then
    echo "[skc-session] durable metadata: $state_dir/metadata.json" >&2
  fi
  echo "[skc-session] tmux session '$SESSION' is not readable; showing durable pane log: $log_path" >&2
  if [[ -f "$state_dir/final.json" ]]; then
    echo "[skc-session] durable final status: $state_dir/final.json" >&2
  fi
  if [[ -f "$state_dir/events.log" ]]; then
    echo "[skc-session] durable events: $state_dir/events.log" >&2
  fi
...
echo "skc-session tail failed: tmux session '$SESSION' is not readable and no durable pane log was found" >&2
exit 1
