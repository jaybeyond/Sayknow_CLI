#!/usr/bin/env bash
# Create a public-safe SKC tmux session through the owner-isolation protocol.
...
if [[ -z "$SKC_BIN" ]] || { [[ "$SKC_BIN" == */* ]] && [[ ! -x "$SKC_BIN" ]]; } || { [[ "$SKC_BIN" != */* ]] && ! command -v "$SKC_BIN" >/dev/null 2>&1; }; then
...
flock -x 9
SKC_SESSION_TRANSITION_LOCK_HELD=1
...
PY
  then
    local deadline=$((SECONDS + ${SKC_SESSION_PENDING_VERDICT_GRACE_SECONDS:-3}))
...
PY
  ...
  if ! skc_session_validate_raw_verdict "$LIFECYCLE_DIR/verdict-$prior_generation.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" "$SOCKET_KEY"; then
...
PY
  skc_session_publish_current_alias "$LIFECYCLE_DIR/verdict-$prior_generation.json" "$STATE_DIR/verdict.json" "$GENERATION_JSON" "$SESSION" "$prior_generation"
  if [[ -f "$LIFECYCLE_DIR/incident-$prior_generation.json" ]]; then
    skc_session_publish_current_alias "$LIFECYCLE_DIR/incident-$prior_generation.json" "$STATE_DIR/incident.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" owner_incident
  fi
  for marker in vanished terminal final; do
    if [[ -f "$LIFECYCLE_DIR/$marker-$prior_generation.json" ]]; then
      ...
      skc_session_publish_current_alias "$LIFECYCLE_DIR/$marker-$prior_generation.json" "$STATE_DIR/$marker.json" "$GENERATION_JSON" "$SESSION" "$prior_generation" "$marker_kind"
    fi
  done
}
...
PY
  ...
  if ! response="$("$TMUX_BIN" -L "$SOCKET_KEY" if-shell -t "$native_id" -F "$condition" "kill-session -t '$native_id' ; display-message -p __skc_creation_rollback_ok__" "display-message -p __skc_creation_rollback_refused__" 2>/dev/null)"; then
    ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_indeterminate"
    return 1
  fi
  case "$response" in
    __skc_creation_rollback_ok__) return 0 ;;
    __skc_creation_rollback_refused__) ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_refused" ;;
    *) ROLLBACK_FAILURES="${ROLLBACK_FAILURES:+$ROLLBACK_FAILURES,}${label}_rollback_indeterminate" ;;
  esac
  return 1
}
...
PY
  if generation_is_current; then
    skc_session_publish_current_alias "$LIFECYCLE_DIR/creation-failed-$OWNER_GENERATION.json" "$STATE_DIR/creation-state.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_failed
  fi
}
...
PY
  if generation_is_current; then
    skc_session_publish_current_alias "$canonical" "$STATE_DIR/creation-cleanup-failure.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_cleanup_failed
  fi
}
...
python3 - "$METADATA_CANONICAL" "$SESSION" "$WORKDIR" "$BRANCH" "$CREATED_AT" "$SKC_BIN" "$STATE_DIR" "$RUNTIME_STATE_JSON" "$WORKTREE_BASELINE_DIRTY" "$OWNER_GENERATION" "$SOCKET_KEY" <<'PY'
import json
import sys
path, session, workdir, branch, created_at, skc_bin, state_dir, runtime_state, baseline_dirty, generation, socket_key = sys.argv[1:]
with open(path, "x", encoding="utf-8") as handle:
    json.dump({
        "schema_version": 1, "session_id": session, "workdir": workdir,
        "branch": branch, "created_at": created_at, "skc_bin": skc_bin,
...
skc_session_write_public_marker "$CREATION_CANONICAL" creation_started "$SESSION" "$OWNER_GENERATION"
cat >"$STATE_DIR/runner.sh" <<'RUNNER'
#!/usr/bin/env bash
set +e
source "$SKC_SESSION_POSTMORTEM_SH"
write_terminal() {
  python3 - "$SKC_SESSION_FINAL_CANONICAL_JSON" "$SKC_SESSION_TERMINAL_CANONICAL_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" "$1" "$2" "$3" "$SKC_COORDINATOR_SESSION_STATE_FILE" "$SKC_SESSION_WORKDIR" "$SKC_SESSION_WORKTREE_BASELINE_DIRTY" "$SKC_SESSION_PROMPT_ACCEPTED_JSON" <<'PY'
import json
import datetime
import os
import subprocess
...
if runtime_path and os.path.isfile(runtime_path) and os.path.getsize(runtime_path):
    ...
    try:
        ...
        fresh_after = os.environ.get("SKC_SESSION_RUNTIME_FRESH_AFTER", "")
...
if os.path.isfile(prompt_path) and os.path.getsize(prompt_path) > 0:
    try:
        ...
        with open(os.path.join(os.environ["SKC_SESSION_STATE_DIR"], session, "owner-lifecycle", "generation.json"), encoding="utf-8") as handle:
...
PY
  skc_session_publish_current_alias "$SKC_SESSION_TERMINAL_CANONICAL_JSON" "$SKC_SESSION_TERMINAL_JSON" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" terminal
  skc_session_publish_current_alias "$SKC_SESSION_FINAL_CANONICAL_JSON" "$SKC_SESSION_FINAL_JSON" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" terminal
}
...
def write_immutable(path, record, inject=None):
    if inject and os.environ.get("SKC_SESSION_TEST_FAIL_RECEIPT_WRITE") in {inject, inject.removesuffix("_canonical")}:
...
def record_publication_failure(boundary):
    state_dir = os.environ["SKC_SESSION_STATE_DIR"]
...
def publish_current_alias(canonical_path, alias_path, kind, inject=None):
    if inject and os.environ.get("SKC_SESSION_TEST_FAIL_RECEIPT_WRITE") == inject:
        raise OSError("injected receipt write failure")
    completed = subprocess.run(
        ["bash", "-c", 'source "$1"; skc_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" "$7"', "skc-supervisor-failure-alias", os.environ["SKC_SESSION_POSTMORTEM_SH"], canonical_path, alias_path, os.path.join(os.environ["SKC_SESSION_STATE_DIR"], os.environ["SKC_SESSION_NAME"], "owner-lifecycle", "generation.json"), os.environ["SKC_SESSION_NAME"], os.environ["SKC_SESSION_OWNER_GENERATION"], kind],
...
def observe(signum):
    ...
    request = {
        "schema_version": 1,
        "op": "observe_terminal",
        "session_id": os.environ["SKC_SESSION_NAME"],
        "owner_generation": os.environ["SKC_SESSION_OWNER_GENERATION"],
        "state_dir": os.environ["SKC_SESSION_STATE_DIR"],
        "socket_key": os.environ["SKC_TMUX_OWNER_SERVER_KEY"],
    ...
    if signum == signal.SIGTERM:
        intent_path = os.path.join(
            os.environ["SKC_SESSION_STATE_DIR"],
            os.environ["SKC_SESSION_NAME"],
            "owner-lifecycle",
            f'intent-{os.environ["SKC_SESSION_OWNER_GENERATION"]}.json',
        )
        try:
            ...
            valid_intent = (
                ...
                and intent["session_id"] == os.environ["SKC_SESSION_NAME"]
                and intent["generation"] == os.environ["SKC_SESSION_OWNER_GENERATION"]
                and intent["server_key"] == os.environ["SKC_TMUX_OWNER_SERVER_KEY"]
    ...
    try:
        ...
        validated = subprocess.run(["bash", "-c", 'source "$1"; skc_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6"', "skc-supervisor-verdict", os.environ["SKC_SESSION_POSTMORTEM_SH"], canonical_path, generation_path, session, generation, os.environ["SKC_TMUX_OWNER_SERVER_KEY"]], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3, check=False)
        ...
        subprocess.run(
            [
                ...
                "skc-supervisor-alias",
                os.environ["SKC_SESSION_POSTMORTEM_SH"],
                canonical_path,
...
finalized = subprocess.run(
    ["bash", os.environ["SKC_SESSION_RUNNER_SH"], "--finalize", str(exit_code), started_at, now()],
    check=False,
)
...
PLAN_RESPONSE="$(printf '%s\n' "$PLAN_LINE" | "$SKC_BIN" --internal-tmux-owner-isolation)" || { echo "owner-isolation plan protocol failed" >&2; exit 1; }
...
try:
    ...
    if execution.get("mode") == "direct":
        ...
        expected_scope = f"skc-owner-{token}.scope" if isinstance(token, str) and token else None
...
POST_SPAWN_RESPONSE="$(printf '%s\n' "$PLAN_LINE" | "$SKC_BIN" --internal-tmux-owner-isolation)" || { echo "owner-isolation post-spawn proof failed" >&2; show_recovery_hint; exit 1; }
...
for option_value in \
  "@skc-profile=1" \
  "@skc-session-id=$SESSION" \
  "@skc-session-state-file=$RUNTIME_STATE_JSON" \
  "@skc-owner-generation=$OWNER_GENERATION" \
  "@skc-owner-server-key=$SOCKET_KEY"; do
  option="${option_value%%=*}"
  value="${option_value#*=}"
...
PY
  ...
  tag_response="$("$TMUX_BIN" -L "$SOCKET_KEY" if-shell -t "$ROLLBACK_OWNER_NATIVE_ID" -F "$OWNER_TAG_CONDITION" "$tag_command" "display-message -p __skc_owner_tag_refused__" 2>/dev/null)" || {
...
GENERATION_PUBLISH_RESPONSE="$(printf '%s\n' "$GENERATION_PUBLISH_REQUEST" | "$SKC_BIN" --internal-tmux-owner-isolation)" || { echo "generation publication protocol failed" >&2; exit 1; }
...
raise SystemExit(0 if valid else 1)
PY
skc_session_publish_current_alias "$METADATA_CANONICAL" "$STATE_DIR/metadata.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION"
skc_session_publish_current_alias "$CREATION_CANONICAL" "$STATE_DIR/creation-state.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" creation_started
STARTED_CANONICAL="$LIFECYCLE_DIR/started-$OWNER_GENERATION.json"
skc_session_write_public_marker "$STARTED_CANONICAL" started "$SESSION" "$OWNER_GENERATION"
skc_session_publish_current_alias "$STARTED_CANONICAL" "$STATE_DIR/started.json" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" started
LIFECYCLE_DIR="$STATE_DIR/$SESSION/owner-lifecycle"
GENERATION_JSON="$LIFECYCLE_DIR/generation.json"
...
while true; do
  ...
  if timeout 1s "$SKC_SESSION_TMUX_BIN" -L "$SKC_SESSION_SOCKET_KEY" has-session -t "=$SKC_SESSION_NAME" >/dev/null 2>&1; then
...
request="$(python3 - "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" "$SKC_SESSION_STATE_DIR" "$SKC_SESSION_SOCKET_KEY" "$observed_at" <<'PY'
import json, sys
...
while true; do
  ...
  if verdict="$(printf '%s\n' "$request" | timeout "${remaining_seconds}s" "$SKC_SESSION_SKC_BIN" --internal-tmux-owner-isolation)"; then break; fi
  verdict=""; sleep 1
done
now_ms="$(date +%s%3N)"
if [[ -z "$verdict" || "$now_ms" -ge "$deadline_at_ms" ]]; then
  monitor_failure_canonical="$SKC_SESSION_STATE_DIR/$SKC_SESSION_NAME/owner-lifecycle/monitor-failure-$SKC_SESSION_OWNER_GENERATION.json"
  python3 - "$monitor_failure_canonical" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" <<'PY'
import json, os, sys
...
PY
  skc_session_publish_current_alias "$monitor_failure_canonical" "$SKC_SESSION_STATE_DIR/monitor-failure.json" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" monitor_failure || exit 1
  exit 1
fi
within_recovery_deadline() { [[ "$(date +%s%3N)" -lt "$deadline_at_ms" ]]; }

within_recovery_deadline || exit 1
skc_session_validate_raw_verdict "$SKC_SESSION_VERDICT_CANONICAL_JSON" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" "$SKC_SESSION_SOCKET_KEY" || exit 1
within_recovery_deadline || exit 1
classification="$(python3 - "$SKC_SESSION_VERDICT_CANONICAL_JSON" <<'PY'
import json, sys
with open(sys.argv[1], encoding="utf-8") as handle: print(json.load(handle)["classification"])
PY
)"
within_recovery_deadline || exit 1
skc_session_publish_current_alias "$SKC_SESSION_VERDICT_CANONICAL_JSON" "$SKC_SESSION_VERDICT_JSON" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" || exit 1
within_recovery_deadline || exit 1
if [[ "$classification" == unexpected_owner_loss ]]; then
  within_recovery_deadline || exit 1
  skc_session_write_vanished_json "$SKC_SESSION_VANISHED_CANONICAL_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_WORKDIR" tmux_session_missing owner_lost failure false false "$SKC_SESSION_OWNER_GENERATION"
  within_recovery_deadline || exit 1
  skc_session_publish_current_alias "$SKC_SESSION_VANISHED_CANONICAL_JSON" "$SKC_SESSION_VANISHED_JSON" "$SKC_SESSION_GENERATION_JSON" "$SKC_SESSION_NAME" "$SKC_SESSION_OWNER_GENERATION" || exit 1
  within_recovery_deadline || exit 1
