...
function isolatedEnv(overrides: Record<string, string | undefined>): Record<string, string | undefined> {
	...
	delete env.SKC_SESSION_WORKDIR;
	delete env.SKC_SESSION_STATE_DIR;
...
describe("skc-session create", () => {
	test("fails closed when the owner exits before durable turn evidence", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-owner-exit-"));
		tempRoots.push(root);
		const session = `skc_issue_1385_exit_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
import json
import os
with open(os.path.join(os.environ["SKC_SESSION_STATE_DIR"], "env.json"), "w", encoding="utf-8") as handle:
    json.dump({
        "sessionId": os.environ.get("SKC_COORDINATOR_SESSION_ID"),
        "stateFile": os.environ.get("SKC_COORDINATOR_SESSION_STATE_FILE"),
        "branch": os.environ.get("SKC_COORDINATOR_SESSION_BRANCH"),
    }, handle)
PY
...
`,
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/create.sh", session, worktree], {
			env: isolatedEnv({
				SKC_BIN: fakeSkc,
				SKC_SESSION_MONITOR_DISABLE: "1",
				SKC_SESSION_SKIP_ROUTER: "1",
				SKC_SESSION_STATE_DIR: stateDir,
			}),
			stderr: "pipe",
			stdout: "pipe",
		});
		...
		expect(result.stderr.toString()).toContain("SKC owner exited before durable turn evidence");
	...
	test("runner treats completed runtime state as normal terminal cleanup", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-runtime-completed-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_runtime_completed_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
import json
import os
with open(os.environ["SKC_COORDINATOR_SESSION_STATE_FILE"], "w", encoding="utf-8") as handle:
    json.dump({"session_id": os.environ["SKC_COORDINATOR_SESSION_ID"], "state": "completed", "final_response": {"source": "agent_end"}}, handle)
PY
exit 0
`,
		);
	...
	test("runner treats launch_error runtime state as normal terminal cleanup", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-runtime-errored-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_runtime_errored_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
import os
with open(os.environ["SKC_COORDINATOR_SESSION_STATE_FILE"], "w", encoding="utf-8") as handle:
    json.dump({"session_id": os.environ["SKC_COORDINATOR_SESSION_ID"], "state": "errored", "final_response": {"source": "launch_error"}}, handle)
PY
exit 1
`,
		);
	...
	test("runner treats process_postmortem errored runtime state as recoverable failure", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-runtime-postmortem-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_runtime_postmortem_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
with open(os.environ["SKC_COORDINATOR_SESSION_STATE_FILE"], "w", encoding="utf-8") as handle:
    json.dump(
        {
            "schema_version": 1,
            "session_id": os.environ["SKC_COORDINATOR_SESSION_ID"],
            "state": "errored",
...
`,
	...
	test("runner rejects terminal runtime state for a mismatched session id", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-runtime-mismatch-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_runtime_mismatch_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
	...
	test("runner treats pre-acceptance pane evidence exit as failure, not normal", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-preaccept-evidence-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_preaccept_evidence_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
		await makeExecutable(fakeSkc, "#!/usr/bin/env bash\nprintf 'Sayknow forge\\nWorking before prompt acceptance\\n'\nexit 0\n");
	...
	test("runner treats non-terminal runtime state as failed even without prompt marker", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-runtime-running-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_runtime_running_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
	...
	test("external monitor records vanished tmux sessions and alerts the router", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-vanish-"));
		tempRoots.push(root);
		const session = `skc_issue_1385_vanish_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
		...
		const created = Bun.spawnSync(["bash", "scripts/skc-session/create.sh", session, worktree, "C-test"], {
			env: isolatedEnv({
				SKC_BIN: fakeSkc,
				SKC_SESSION_MONITOR_INTERVAL: "1",
				SKC_SESSION_ROUTER: fakeRouter,
				SKC_SESSION_SKIP_ROUTER: "1",
				SKC_SESSION_STATE_DIR: stateDir,
			}),
			stderr: "pipe",
	...
	test("external monitor treats process_postmortem runtime state as recoverable failure", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-monitor-postmortem-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_monitor_postmortem_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
		...
		const created = Bun.spawnSync(["bash", "scripts/skc-session/create.sh", session, worktree], {
			env: isolatedEnv({
				SKC_BIN: fakeSkc,
				SKC_SESSION_MONITOR_INTERVAL: "1",
				SKC_SESSION_SKIP_ROUTER: "1",
	...
	test("external monitor preserves terminal cleanup for agent_end and launch_error runtime states", async () => {
		for (const [source, state] of [
			...
			const root = await fs.mkdtemp(path.join(os.tmpdir(), `skc-session-monitor-${source}-`));
			tempRoots.push(root);
			const session = `skc_issue_1496_monitor_${source}_${process.pid}_${Date.now()}`;
			tmuxSessions.push(session);
			const worktree = await makeGitWorktree(root);
			const stateDir = path.join(root, "state");
			const fakeSkc = path.join(root, "bin", "skc");
	...
	test("external monitor records vanished sessions after prompt acceptance", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-post-accept-vanish-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_post_accept_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
		...
		const prompted = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "do accepted work"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
			}),
			stderr: "pipe",
			stdout: "pipe",
		});
	...
	test("prompt records vanished status and refuses to paste when tmux session is missing", async () => {
		const session = `skc_issue_1496_prompt_missing_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-missing-"));
		tempRoots.push(root);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "Sayknow forge\n> Type your message\n");
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "do not paste this prompt"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_WORKDIR: unrelatedWorkdir,
			}),
			stderr: "pipe",
			stdout: "pipe",
		});
	...
	test("prompt refuses success without durable turn evidence", async () => {
		const session = `skc_issue_1385_prompt_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "do work"], {
			env: isolatedEnv({
				SKC_SESSION_TURN_EVIDENCE_PATTERN: "__NO_TURN_EVIDENCE__",
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
			}),
			stderr: "pipe",
	...
	test("prompt ignores stale pre-existing turn evidence", async () => {
		const session = `skc_issue_1385_prompt_stale_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-stale-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "new prompt that sleeping process will not accept"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
			}),
	...
	test("prompt echo cannot satisfy durable turn evidence", async () => {
		const session = `skc_issue_1496_prompt_echo_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-echo-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, rawPrompt], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
			}),
	...
	test("prompt ignores stale evidence when capture window shifts after send", async () => {
		const session = `skc_issue_1385_prompt_window_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-window-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
		...
		const result = Bun.spawnSync(
			["bash", "scripts/skc-session/prompt.sh", session, "new prompt sleeping process should not accept"],
			{
				env: isolatedEnv({
					SKC_SESSION_STATE_DIR: stateDir,
					SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
	...
	test("prompt accepts turn evidence produced after send", async () => {
		const session = `skc_issue_1385_prompt_fresh_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-fresh-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
	...
	test("runner marks owner exit after prompt acceptance as failure final", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-owner-exit-post-accept-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_owner_exit_post_accept_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
		...
		const prompted = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "accepted then exit"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "2",
			}),
			stderr: "pipe",
			stdout: "pipe",
		});
	...
	test("runner reports new recoverable worktree changes after prompt acceptance", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-owner-exit-dirty-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_owner_exit_dirty_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
		...
		const prompted = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "accepted then dirty"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "2",
			}),
	...
	test("runner does not overclaim pre-existing dirty worktree as new work", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-owner-exit-preexisting-dirty-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_owner_exit_preexisting_dirty_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
...
`,
		...
		const prompted = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "accepted preexisting dirty"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "2",
			}),
	...
	test("monitor preserves vanished marker when failure-final hold disappears after prompt acceptance", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-failure-final-vanish-"));
		tempRoots.push(root);
		const session = `skc_issue_1496_failure_final_vanish_${process.pid}_${Date.now()}`;
		tmuxSessions.push(session);
		const worktree = await makeGitWorktree(root);
		const stateDir = path.join(root, "state");
		const fakeSkc = path.join(root, "bin", "skc");
...
`,
		...
		const prompted = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "accepted then failure final"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "2",
			}),
	...
	test("prompt accepts durable evidence when owner exits immediately after output", async () => {
		const session = `skc_issue_1496_prompt_exit_after_evidence_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-exit-after-evidence-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "exit after evidence"], {
			env: isolatedEnv({
				SKC_SESSION_STATE_DIR: stateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "2",
			}),
	...
	test("prompt echo after submit cannot satisfy durable turn evidence", async () => {
		const session = `skc_issue_1496_prompt_post_submit_echo_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-post-submit-echo-"));
		tempRoots.push(root);
		const stateDir = path.join(root, "state");
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
	...
	test("prompt uses discovered durable pane log without state dir", async () => {
		const session = `skc_issue_1496_prompt_discovery_${process.pid}_${Date.now()}`;
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-session-prompt-discovery-"));
		tempRoots.push(root);
		const stateDir = path.join(root, ".skc-session-state", session);
		await fs.mkdir(stateDir, { recursive: true });
		await Bun.write(path.join(stateDir, "pane.log"), "");
		...
		const result = Bun.spawnSync(["bash", "scripts/skc-session/prompt.sh", session, "discovery prompt"], {
			env: isolatedEnv({
				SKC_SESSION_LOG_SEARCH_ROOT: root,
				SKC_SESSION_STATE_DIR: unrelatedStateDir,
				SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS: "1",
			}),
			stderr: "pipe",
			stdout: "pipe",
