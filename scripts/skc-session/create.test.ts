...
function env(overrides: Record<string, string>) {
	return { ...process.env, SKC_SESSION_SKIP_ROUTER: "1", SKC_SESSION_MONITOR_DISABLE: "1", ...overrides };
}
...
if [[ "${"$"}{1:-}" == --internal-tmux-owner-isolation ]]; then
  request=$(cat)
  if [[ -n "${"$"}{SKC_FIXTURE_ISOLATION_LOG:-}" ]]; then printf '%s\n' "$request" >>"${"$"}SKC_FIXTURE_ISOLATION_LOG"; fi
...
if request["op"] == "publish_generation":
    ...
    if mode == "scoped" and not post_spawn:
        ...
        expected_scope = f"skc-owner-{token}.scope"
        generation_path = os.path.join(request["state_dir"], request["session_id"], "owner-lifecycle", "generation.json")
        if os.path.exists(generation_path):
    ...
    if post_spawn:
        ...
        server_start_time = os.environ.get("SKC_FIXTURE_POSTSPAWN_START_TIME", server_start_time)
...
runner_status=$?
sleep "${"$"}{SKC_SESSION_FIXTURE_HOLD_SECONDS:-1}"
exit "$runner_status"
`);
	return bin;
}
...
async function supervisorAdapter(root: string) {
	const bin = path.join(root, "bin", "skc-supervisor-adapter");
...
if len(sys.argv) > 1 and sys.argv[1] == "--internal-tmux-owner-isolation":
    ...
    session = os.environ["SKC_SESSION_NAME"]
    generation = os.environ["SKC_SESSION_OWNER_GENERATION"]
    server_key = os.environ["SKC_TMUX_OWNER_SERVER_KEY"]
    intent_path = os.path.join(os.environ["SKC_SESSION_STATE_DIR"], session, "owner-lifecycle", f"intent-{generation}.json")
    try:
    ...
    with open(os.environ["SKC_FIXTURE_SAFE_OBSERVATION_LOG"], "a", encoding="utf-8") as handle:
...
open(os.environ["SKC_FIXTURE_RAW_READY"], "w", encoding="utf-8").close()
...
describe("skc-session create public owner lifecycle", () => {
	test("rejects missing binaries, directories, git worktrees, and detached branches", async () => {
		...
		const missing = Bun.spawnSync(["bash", createScript, "x", root], { env: env({ SKC_BIN: "/definitely-not-a-skc-executable" }), stderr: "pipe" });
		expect(missing.exitCode).toBe(1); expect(missing.stderr.toString()).toContain("skc not found");
		const binary = await fixture(root);
		const nongit = Bun.spawnSync(["bash", createScript, "x", root], { env: env({ SKC_BIN: binary }), stderr: "pipe" });
		expect(nongit.exitCode).toBe(1); expect(nongit.stderr.toString()).toContain("not a git worktree");
		const absent = Bun.spawnSync(["bash", createScript, "x", path.join(root, "absent")], { env: env({ SKC_BIN: binary }), stderr: "pipe" });
		...
		const detachedResult = Bun.spawnSync(["bash", createScript, "detached", detached], { env: env({ SKC_BIN: binary }), stderr: "pipe" });
	...
	test("accepts direct canonical plans, re-proves the isolated server, and tags the owner session", async () => {
		...
		const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_FIXTURE_ISOLATION_LOG: proofLog }), stderr: "pipe" });
		if (result.exitCode !== 0) throw new Error(result.stderr.toString());
		expect(result.stdout.toString()).toContain(`created SKC session: ${name}`);
		...
		for (const [option, value] of [["@skc-profile", "1"], ["@skc-session-id", name], ["@skc-session-state-file", path.join(state, "runtime-state.json")]] as const) {
			expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", option], { stdout: "pipe" }).stdout.toString().trim()).toBe(value);
		}
		expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", "@skc-owner-generation"], { stdout: "pipe" }).stdout.toString().trim()).not.toBe("");
		expect(Bun.spawnSync(["tmux", "-L", socket, "show-options", "-t", `=${name}:`, "-v", "@skc-owner-server-key"], { stdout: "pipe" }).stdout.toString().trim()).toBe(socket);
		...
		expect(supervisor).toContain("skc_session_publish_current_alias");
	...
	test("rejects a post-spawn server start-time mismatch before tagging or generation publication", async () => {
		...
		const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_FIXTURE_POSTSPAWN_START_TIME: "1" }), stdout: "pipe", stderr: "pipe" });
	...
	test("fails closed for malformed plans before any router registration", async () => {
		...
		const dir = await worktree(root); const bad = path.join(root, "bad-skc"); const router = path.join(root, "router");
...
`);
		...
		const result = Bun.spawnSync(["bash", createScript, "bad", dir], { env: env({ SKC_BIN: bad, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0" }), stderr: "pipe" });
	...
	test("runner classifies terminal runtime completion as normal cleanup", async () => {
		...
		const bin = await fixture(root, "direct", runner); const name = `completed-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
	...
	test("runner ignores arbitrary runtime-state payload classifications", async () => {
		...
		const runner = `python3 - <<'PY'\nimport json, os\njson.dump({"session_id": os.environ["SKC_SESSION_NAME"], "cwd": os.environ["SKC_SESSION_WORKDIR"], "owner_generation": os.environ["SKC_SESSION_OWNER_GENERATION"], "state":"errored", "source":"${hostile}", "event":"${hostile}", "reason":"${hostile}", "previous_runtime_state":"${hostile}", "final_response":{"source":"${hostile}"}}, open(os.environ["SKC_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 1`;
		const bin = await fixture(root, "direct", runner); const name = `postmortem-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
...
test("runner ignores a stale prompt acceptance marker from another generation", async () => {
	...
	const bin = await fixture(root, "direct", "printf changed > README.md; exit 0"); const name = `recovery-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
...
test("uses public lifecycle markers only and excludes all private capture mechanisms", async () => {
	...
	expect(create).toContain('"op": "plan"'); expect(create).toContain('"op":"observe_terminal"'); expect(postmortem).toContain("skc_session_write_public_marker");
	...
	test("monitor disable leaves no monitor owner session and router registration keeps caller options", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-router-")); roots.push(root);
		...
		await executable(router, `#!/usr/bin/env bash\nprintf '%s\\n' "$*" > ${log}\n`);
		await executable(timeout, "#!/usr/bin/env bash\nprintf '%s\\n' \"$*\" >\"$SKC_FIXTURE_TIMEOUT_LOG\"\nexec /usr/bin/timeout \"$@\"\n");
		const name = `router-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
		expect(Bun.spawnSync(["bash", createScript, name, dir, "C1", "@team"], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0", SKC_SESSION_STALE_MINUTES: "17", SKC_FIXTURE_TIMEOUT_LOG: timeoutLog, PATH: `${path.join(root, "bin")}:${process.env.PATH}` }) }).exitCode).toBe(0);
		expect(await Bun.file(log).text()).toContain("--stale-minutes 17");
		expect(await Bun.file(timeoutLog).text()).toStartWith("10s ");
		expect(Bun.spawnSync(["tmux", "-L", `skc-${name}`, "has-session", "-t", `${name}-owner-monitor`]).exitCode).not.toBe(0);
	});

test("records router watch failures without failing creation", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-watch-failure-")); roots.push(root);
	...
	await executable(router, "#!/usr/bin/env bash\nexit 37\n");
	const name = `watch-failure-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	const created = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0" }), stderr: "pipe" });
...
test("bounds a blocking router watch through timeout and publishes its current immutable failure receipt", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-watch-timeout-")); roots.push(root);
	...
	await executable(router, "#!/usr/bin/env bash\nsleep 30\n");
	await executable(timeout, "#!/usr/bin/env bash\nprintf '%s\\n' \"$*\" >>\"$SKC_FIXTURE_TIMEOUT_LOG\"\nexec /usr/bin/timeout \"$@\"\n");
	const name = `watch-timeout-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	const startedAt = Date.now();
	const created = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0", SKC_SESSION_TEST_ROUTER_WATCH_TIMEOUT_SECONDS: "1", SKC_FIXTURE_TIMEOUT_LOG: timeoutLog, PATH: `${path.join(root, "bin")}:${process.env.PATH}` }), stderr: "pipe" });
...
test("rejects test router watch deadlines outside the production-safe bound", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-watch-timeout-bound-")); roots.push(root);
	const dir = await worktree(root); const bin = await fixture(root);
	for (const duration of ["0", "11", "one"]) {
		const result = Bun.spawnSync(["bash", createScript, `watch-bound-${duration}`, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_TEST_ROUTER_WATCH_TIMEOUT_SECONDS: duration }), stderr: "pipe" });
		expect(result.exitCode).toBe(1);
		expect(result.stderr.toString()).toContain("SKC_SESSION_TEST_ROUTER_WATCH_TIMEOUT_SECONDS must be an integer from 1 through 10");
	}
});
test("passes branch and coordinator identity to the raw owner without private lifecycle payloads", async () => {
	...
	const runner = `python3 - <<'PY'
import json, os
json.dump({key: os.environ.get(key) for key in ["SKC_COORDINATOR_SESSION_ID", "SKC_COORDINATOR_SESSION_BRANCH", "SKC_COORDINATOR_SESSION_STATE_FILE", "SKC_SESSION_STATE_DIR"]}, open(os.path.join(os.environ["SKC_SESSION_STATE_DIR"], "owner-env.json"), "w"))
PY
sleep 2`;
	const bin = await fixture(root, "direct", runner); const name = `owner-env-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	await waitFor(path.join(state, "owner-env.json"));
	expect(await Bun.file(path.join(state, "owner-env.json")).json()).toEqual({ SKC_COORDINATOR_SESSION_ID: name, SKC_COORDINATOR_SESSION_BRANCH: "session-test", SKC_COORDINATOR_SESSION_STATE_FILE: path.join(state, "runtime-state.json"), SKC_SESSION_STATE_DIR: state });
});

test("prompt requires a public started marker and records a payload-free acceptance marker in the isolated socket", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-prompt-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `prompt-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	const secret = "private prompt must not be persisted";
	const submitted = Bun.spawnSync(["bash", promptScript, name, secret], { env: { ...process.env, SKC_SESSION_STATE_DIR: state }, stdout: "pipe", stderr: "pipe" });
...
test("prompt deletes its private buffer when paste fails", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-prompt-paste-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `prompt-paste-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
	...
	const result = Bun.spawnSync(["bash", promptScript, name, secret], { env: { ...process.env, SKC_SESSION_STATE_DIR: state, SKC_SESSION_TMUX_BIN: tmux }, stdout: "pipe", stderr: "pipe" });
...
test("prompt deletes a private buffer when its load client fails after creating it", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-prompt-load-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `prompt-load-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
...
test("prompt fails when private buffer deletion fails on the reachable isolated server", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-prompt-delete-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `prompt-delete-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).toBe(0);
...
test("records one generation-bound recovery only after a prior incident replacement is started", async () => {
	...
	await executable(router, `#!/usr/bin/env bash\nprintf '%s\\n' "$*" >> ${log}\n`);
	const created = Bun.spawnSync(["bash", createScript, name, dir, "C-recovery"], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0" }), stderr: "pipe" });
...
test("reconciles an immediately replaced missing owner before publishing the next generation", async () => {
	...
	const replacement = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }), stdout: "pipe", stderr: "pipe" });
...
if __import__("os").path.exists(__import__("os").environ["SKC_FIXTURE_POSTSPAWN"]): print("{}")
else:
 open(__import__("os").environ["SKC_FIXTURE_POSTSPAWN"], "w").close()
...
`);
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_FIXTURE_POSTSPAWN: path.join(root, "postspawn") }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
	const lifecycle = path.join(state, name, "owner-lifecycle");
...
test("retries a generation-bound failed create without synthesizing owner loss or recovery", async () => {
	...
	expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_TMUX_BIN: failingTmux }) }).exitCode).toBe(1);
	const lifecycle = path.join(state, name, "owner-lifecycle");
	const failureName = (await fs.readdir(lifecycle)).find(file => file.startsWith("creation-failed-"))!;
...
test("does not recover an incident again in a third generation when a canonical recovery exists", async () => {
	...
	await executable(router, `#!/usr/bin/env bash\nprintf '%s\\n' "$*" >> ${log}\n`);
	expect(Bun.spawnSync(["bash", createScript, name, dir, "C-third"], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0" }) }).exitCode).toBe(0);
	expect(await Bun.file(path.join(state, "recovery.json")).exists()).toBe(false);
...
test("prompt fails closed without a started marker and does not expose prompt text", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-create-prompt-marker-")); roots.push(root);
	const state = path.join(root, "state"); await fs.mkdir(state, { recursive: true });
	const secret = "do not disclose this prompt";
	const result = Bun.spawnSync(["bash", promptScript, "missing", secret], { env: { ...process.env, SKC_SESSION_STATE_DIR: state }, stdout: "pipe", stderr: "pipe" });
	...
	test("does not accept a terminal runtime record from another session", async () => {
		...
		const bin = await fixture(root, "direct", runner); const name = `mismatch-${Date.now()}`; sessions.push({ name, socket: `skc-${name}` });
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "final.json"));
	...
	test("writes terminal lifecycle state even when the runner exits unsuccessfully", async () => {
		...
		Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }); await waitFor(path.join(state, "terminal.json"));
	...
	test("monitor consumes one absolute seven-second recovery budget across polling and publication", async () => {
		...
		expect(create).toContain('timeout "${remaining_seconds}s" "$SKC_SESSION_SKC_BIN"');
...
`,
		...
		const adapter = path.join(root, "skc");
		await executable(adapter, "#!/usr/bin/env bash\nprintf '{\"ok\":true}\\n'\n");
		const postmortem = path.join(root, "postmortem.sh");
		await executable(
			postmortem,
			`#!/usr/bin/env bash
skc_session_publish_current_alias() { cp "$1" "$2"; }
skc_session_validate_raw_verdict() { return 0; }
skc_session_write_vanished_json() { : >"$1"; }
`,
		);
		...
		const result = Bun.spawnSync(["bash", monitor], {
			env: {
				...
				SKC_SESSION_MONITOR_INTERVAL: "1",
				SKC_SESSION_POSTMORTEM_SH: postmortem,
				SKC_SESSION_TMUX_BIN: tmux,
				SKC_SESSION_SOCKET_KEY: "private-socket",
				SKC_SESSION_NAME: name,
				SKC_SESSION_OWNER_GENERATION: "11111111-1111-4111-8111-111111111111",
				SKC_SESSION_STATE_DIR: state,
				SKC_SESSION_SKC_BIN: adapter,
				SKC_SESSION_VERDICT_CANONICAL_JSON: path.join(lifecycle, "verdict.json"),
				SKC_SESSION_GENERATION_JSON: path.join(lifecycle, "generation.json"),
				SKC_SESSION_VERDICT_JSON: path.join(state, "verdict.json"),
				SKC_SESSION_WORKDIR: root,
				SKC_SESSION_VANISHED_CANONICAL_JSON: path.join(lifecycle, "vanished.json"),
				SKC_SESSION_VANISHED_JSON: path.join(state, "vanished.json"),
				SKC_SESSION_INCIDENT_CANONICAL_JSON: path.join(lifecycle, "incident.json"),
				SKC_SESSION_INCIDENT_JSON: path.join(state, "incident.json"),
			},
			stdout: "pipe",
			stderr: "pipe",
		});
	...
	test("monitor writes immutable verdict and incident markers after owner loss without claiming recovery", async () => {
		...
		expect(Bun.spawnSync(["bash", createScript, name, dir, "C-monitor"], { env: { ...env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router }), SKC_SESSION_MONITOR_DISABLE: "0", SKC_SESSION_MONITOR_INTERVAL: "1" } }).exitCode).toBe(0);
		...
		const recovered = Bun.spawnSync(["bash", createScript, name, dir, "C-recovered"], {
			env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }),
			stdout: "pipe",
			stderr: "pipe",
		});
...
test("serializes current aliases and refuses a stale generation overwrite", async () => {
	...
	const stale = Bun.spawnSync(
		[
			"bash",
			"-c",
			'source "$1"; skc_session_publish_current_alias "$2" "$3" "$4" "$5" "$6"',
			"skc-alias-test",
			postmortemScript,
			canonicalPath,
			aliasPath,
...
test("keeps creation-cleanup and monitor failure canonicals immutable and rejects stale aliases", async () => {
	...
	const stale = Bun.spawnSync(["bash", "-c", 'source "$1"; skc_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" creation_cleanup_failed', "skc-cleanup-alias-test", postmortemScript, canonicalPath, aliasPath, generationPath, "session", "old"], { stdout: "pipe", stderr: "pipe" });
	expect(stale.exitCode).not.toBe(0);
	...
	const staleMonitor = Bun.spawnSync(["bash", "-c", 'source "$1"; skc_session_publish_current_alias "$2" "$3" "$4" "$5" "$6" monitor_failure', "skc-monitor-alias-test", postmortemScript, monitorCanonicalPath, monitorAliasPath, generationPath, "session", "old"], { stdout: "pipe", stderr: "pipe" });
	...
	expect(create).toContain('monitor-failure-$SKC_SESSION_OWNER_GENERATION.json');
...
test("keeps canonical generation receipts immutable while aliases publish the current generation", async () => {
	...
	expect(create).toContain("SKC_SESSION_GENERATION_JSON");
	expect(postmortem).toContain("skc_session_publish_current_alias");
...
test("rejects a non-UUID prior generation before reconciliation or alias publication", async () => {
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
	...
	const validate = () =>
		Bun.spawnSync(
			[
				"bash",
				"-c",
				'source "$1"; skc_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6" "$7"',
...
test("accepts only canonical UTC calendar timestamps for raw expected and unexpected verdicts", async () => {
	...
	const validate = () => Bun.spawnSync(["bash", "-c", 'source "$1"; skc_session_validate_raw_verdict "$2" "$3" "$4" "$5" "$6" "$7"', "validate-raw-timestamp", postmortemScript, verdictPath, generationPath, session, generation, serverKey, intentPath], { stdout: "pipe", stderr: "pipe" });
...
test("forwards SIGTERM dispatch only for an exact live owner intent receipt", async () => {
	...
	for (const [, intent] of cases) {
		...
		const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: socket, SKC_SESSION_SKC_BIN: adapter, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: "/bin/true", SKC_FIXTURE_RAW_READY: ready, SKC_FIXTURE_SAFE_OBSERVATION_LOG: observationLog }, stdout: "pipe", stderr: "pipe" });
		await Promise.race([
			waitFor(ready),
...
open(os.environ["SKC_FIXTURE_RAW_READY"], "w").close()
signal.signal(signal.SIGTERM, lambda *_: raise_exit())
def raise_exit(): raise SystemExit(0)
signal.pause()
`);
	const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: adapter, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: "/bin/true", SKC_FIXTURE_RAW_READY: ready }, stdout: "pipe", stderr: "pipe" });
	await waitFor(ready);
	expect(Bun.spawnSync(["kill", "-TERM", String(supervisor.pid)]).exitCode).toBe(0);
	expect(await supervisor.exited).toBe(0);
...
test("records finalizer failure without replacing the owner exit status", async () => {
	...
	const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: child, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: finalizer }, stdout: "pipe", stderr: "pipe" });
...
test("records rollback failures and immutable collision faults", async () => {
	for (const kind of ["rollback", "probe", "canonical-collision"] as const) {
		...
		await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"set-option"* ]]; then
  if [[ "${"$"}{SKC_FIXTURE_CLEANUP_KIND}" == canonical-collision ]]; then generation="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["generation"])' "${"$"}{SKC_FIXTURE_STATE}/${"$"}{SKC_FIXTURE_SESSION}/owner-lifecycle/generation.json")"; canonical="${"$"}{SKC_FIXTURE_STATE}/${"$"}{SKC_FIXTURE_SESSION}/owner-lifecycle/creation-cleanup-failure-${"$"}{generation}.json"; alias="${"$"}{SKC_FIXTURE_STATE}/creation-cleanup-failure.json"; printf '{"schema_version":1,"kind":"creation_cleanup_failed","session_id":"%s","owner_generation":"%s","exit_code":99,"rollback_failures":["owner_session"],"failure_publication_failed":false}\n' "${"$"}{SKC_FIXTURE_SESSION}" "${"$"}{generation}" >"${"$"}{canonical}"; printf '{"schema_version":1,"kind":"creation_cleanup_failed","session_id":"%s","owner_generation":"%s","exit_code":98,"rollback_failures":["owner_session"],"failure_publication_failed":false}\n' "${"$"}{SKC_FIXTURE_SESSION}" "${"$"}{generation}" >"${"$"}{alias}"; fi
  touch "${"$"}{SKC_FIXTURE_PROBE_MARKER}"
  exit 23
fi
if [[ ( "${"$"}{SKC_FIXTURE_CLEANUP_KIND}" == rollback || "${"$"}{SKC_FIXTURE_CLEANUP_KIND}" == canonical-collision ) && "$*" == *"kill-session"* ]]; then exit 24; fi
if [[ "${"$"}{SKC_FIXTURE_CLEANUP_KIND}" == probe && -e "${"$"}{SKC_FIXTURE_PROBE_MARKER}" && "$*" == *"if-shell"* ]]; then exit 25; fi
exec tmux "$@"
`);
		const created = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_TMUX_BIN: tmux, SKC_FIXTURE_CLEANUP_KIND: kind, SKC_FIXTURE_STATE: state, SKC_FIXTURE_SESSION: name, SKC_FIXTURE_PROBE_MARKER: path.join(root, "rollback-probe") }), stdout: "pipe", stderr: "pipe" });
...
test("refuses rollback after same-name replacement of the captured native owner", async () => {
	...
	await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"set-option"* ]] && [[ ! -e "${"$"}{SKC_FIXTURE_REPLACED}" ]]; then
  touch "${"$"}{SKC_FIXTURE_REPLACED}"
  tmux -L "${"$"}{SKC_FIXTURE_SOCKET}" kill-session -t "=${"$"}{SKC_FIXTURE_SESSION}"
  tmux -L "${"$"}{SKC_FIXTURE_SOCKET}" new-session -d -s "${"$"}{SKC_FIXTURE_SESSION}" 'sleep 30'
  exit 23
fi
exec tmux "$@"
`);
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_TMUX_BIN: tmux, SKC_FIXTURE_REPLACED: path.join(root, "replaced"), SKC_FIXTURE_SOCKET: socket, SKC_FIXTURE_SESSION: name }), stdout: "pipe", stderr: "pipe" });
	expect(result.exitCode).toBe(1);
...
test("holds stale publishers behind replacement creation and preserves every current alias", async () => {
	...
	await executable(tmux, `#!/usr/bin/env bash
if [[ "$*" == *"new-session"* ]]; then touch "${"$"}{SKC_FIXTURE_TRANSITION_GATE}"; read -r <"${"$"}{SKC_FIXTURE_TRANSITION_RELEASE}"; fi
exec tmux "$@"
`);
	const replacement = Bun.spawn(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_TMUX_BIN: tmux, SKC_FIXTURE_TRANSITION_GATE: gate, SKC_FIXTURE_TRANSITION_RELEASE: release }), stdout: "pipe", stderr: "pipe" });
	await waitFor(gate);
...
test("rejects a noncanonical prior generation before owner creation", async () => {
	...
	const result = Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }), stderr: "pipe" });
	expect(result.exitCode).toBe(1);
...
test("fails closed and byte-preserves malformed, corrupt, or mismatched current generation state", async () => {
	for (const [label, body] of [
		...
		const root = await fs.mkdtemp(path.join(os.tmpdir(), `skc-generation-${label}-`)); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `generation-${label}-${Date.now()}-${Math.random()}`; sessions.push({ name, socket: `skc-${name}` });
		const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
...
test("requires every terminal runtime identity receipt field", async () => {
	for (const missing of ["session_id", "cwd", "owner_generation"] as const) {
		...
		const runner = `python3 - <<'PY'\nimport json, os\nrecord={"session_id":os.environ["SKC_SESSION_NAME"],"cwd":os.environ["SKC_SESSION_WORKDIR"],"owner_generation":os.environ["SKC_SESSION_OWNER_GENERATION"],"state":"completed"}\ndel record["${missing}"]\njson.dump(record, open(os.environ["SKC_COORDINATOR_SESSION_STATE_FILE"], "w"))\nPY\nexit 0`;
...
test("does not reconcile malformed or unknown canonical verdicts into incidents", async () => {
	for (const verdict of [
		...
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-verdict-invalid-")); roots.push(root);
		const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const name = `invalid-${Date.now()}-${Math.random()}`; sessions.push({ name, socket: `skc-${name}` });
		const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
		...
		const storedVerdict = "observed_at" in verdict
			? { schema_version: 1, generation: "prior", session_id: name, server_key: `skc-${name}`, observed_at: verdict.observed_at, signal: "UNKNOWN", exit_code: null, result: "owner_lost", observer: "replacement_reconciler", classification: "unexpected_owner_loss", reason: "tmux_session_missing", dedupe_key: `owner-loss:${name}:prior` }
		...
		expect(Bun.spawnSync(["bash", createScript, name, dir], { env: env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state }) }).exitCode).not.toBe(0);
...
test("keeps owner status while exposing injected supervisor and finalization receipt publication failures", async () => {
	for (const kind of ["supervisor_failure", "finalization_failure"] as const) {
		...
		if (kind === "supervisor_failure") {
			...
			const ready = path.join(root, "ready"); const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: owner, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: "/bin/true", SKC_FIXTURE_RAW_READY: ready, SKC_SESSION_TEST_FAIL_RECEIPT_WRITE: kind }, stdout: "pipe", stderr: "pipe" });
			...
			const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: owner, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: finalizer, SKC_SESSION_TEST_FAIL_RECEIPT_WRITE: kind }, stdout: "pipe", stderr: "pipe" });
			expect(await supervisor.exited).toBe(23);
		}
...
test("keeps owner status while exposing canonical-collision and alias publication faults", async () => {
	for (const kind of ["supervisor_failure", "finalization_failure"] as const) for (const fault of ["canonical-collision", "alias"] as const) {
		...
		if (kind === "supervisor_failure") {
			await executable(owner, `#!/usr/bin/env python3\nimport os, signal, sys\nif "--internal-tmux-owner-isolation" in sys.argv: raise SystemExit(23)\nopen(os.environ["SKC_FIXTURE_RAW_READY"], "w").close()\nsignal.signal(signal.SIGTERM, lambda *_: raise_exit())\ndef raise_exit(): raise SystemExit(0)\nsignal.pause()\n`);
			const ready = path.join(root, "ready"); const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: owner, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: "/bin/true", SKC_FIXTURE_RAW_READY: ready, ...(injected ? { SKC_SESSION_TEST_FAIL_RECEIPT_WRITE: injected } : {}) }, stdout: "pipe", stderr: "pipe" });
			await waitFor(ready); expect(Bun.spawnSync(["kill", "-TERM", String(supervisor.pid)]).exitCode).toBe(0); expect(await supervisor.exited).toBe(0);
		} else {
			await executable(owner, "#!/usr/bin/env bash\nexit 23\n"); await executable(finalizer, "#!/usr/bin/env bash\nexit 24\n");
			const supervisor = Bun.spawn(["python3", path.join(state, "supervisor.py")], { env: { ...process.env, SKC_SESSION_NAME: name, SKC_SESSION_WORKDIR: dir, SKC_SESSION_STATE_DIR: state, SKC_SESSION_OWNER_GENERATION: generation, SKC_TMUX_OWNER_SERVER_KEY: `skc-${name}`, SKC_SESSION_SKC_BIN: owner, SKC_SESSION_POSTMORTEM_SH: postmortemScript, SKC_SESSION_RUNNER_SH: finalizer, ...(injected ? { SKC_SESSION_TEST_FAIL_RECEIPT_WRITE: injected } : {}) }, stdout: "pipe", stderr: "pipe" });
			expect(await supervisor.exited).toBe(23);
		}
...
test("records nonfatal recovery and stale-owner router notification failures", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-router-notification-failure-")); roots.push(root);
	const dir = await worktree(root); const state = path.join(root, "state"); const bin = await fixture(root); const router = path.join(root, "router"); const name = `router-failure-${Date.now()}`; const socket = `skc-${name}`; sessions.push({ name, socket });
	await executable(router, "#!/usr/bin/env bash\nexit 37\n");
	const lifecycle = path.join(state, name, "owner-lifecycle"); await fs.mkdir(lifecycle, { recursive: true });
	...
	const created = Bun.spawnSync(["bash", createScript, name, dir, "C-router"], { env: { ...env({ SKC_BIN: bin, SKC_SESSION_STATE_DIR: state, SKC_SESSION_ROUTER: router, SKC_SESSION_SKIP_ROUTER: "0" }), SKC_SESSION_MONITOR_DISABLE: "0", SKC_SESSION_MONITOR_INTERVAL: "1" }, stderr: "pipe" });
	expect(created.exitCode).toBe(0);
