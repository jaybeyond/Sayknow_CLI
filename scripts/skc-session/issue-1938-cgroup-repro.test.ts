...
const withArtifacts = async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-evidence-artifacts-"));
...
const runUnsupportedSerializer = async (environment: Record<string, string>) => {
	...
	const sessionRoot = path.join(import.meta.dir, "..", "..", ".skc", `_session-${sessionId}`);
	const evidencePath = path.join(sessionRoot, "runtime", "evidence", "issue-1938", "pre-code.json");
	const proc = Bun.spawn(["bash", path.join(import.meta.dir, "issue-1938-cgroup-repro.sh"), "--phase", "pre-code", "--session-id", sessionId], { env: { ...process.env, SKC_ISSUE1938_TEST_FORCE_UNSUPPORTED: "1", ...environment }, stdout: "pipe", stderr: "pipe" });
	try {
...
const runIndeterminateCleanupProbe = async (probe: "tmux" | "systemd") => {
	...
	const sessionRoot = path.join(import.meta.dir, "..", "..", ".skc", `_session-${sessionId}`);
	const evidencePath = path.join(sessionRoot, "runtime", "evidence", "issue-1938", "pre-code.json");
	const proc = Bun.spawn(["bash", path.join(import.meta.dir, "issue-1938-cgroup-repro.sh"), "--phase", "pre-code", "--session-id", sessionId], { env: { ...process.env, SKC_ISSUE1938_TEST_CLEANUP_PROBE_ONLY: probe, ...(probe === "tmux" ? { SKC_ISSUE1938_TEST_TMUX_CLEANUP_PROBE: "error" } : { SKC_ISSUE1938_TEST_SYSTEMD_CLEANUP_PROBE: "error" }) }, stdout: "pipe", stderr: "pipe" });
	try {
...
const runHeldMonitorCleanup = async () => {
	...
	const evidencePath = path.join(sessionRoot, "runtime", "evidence", "issue-1938", "pre-code.json");
	const startedAt = Date.now();
	const proc = Bun.spawn(["bash", path.join(import.meta.dir, "issue-1938-cgroup-repro.sh"), "--phase", "pre-code", "--session-id", sessionId], { env: { ...process.env, SKC_ISSUE1938_TEST_CLEANUP_PROBE_ONLY: "monitor" }, stdout: "pipe", stderr: "pipe" });
	try {
		expect(await proc.exited).toBe(1);
		expect(Date.now() - startedAt).toBeLessThan(7_000);
		const value = JSON.parse(await Bun.file(evidencePath).text());
...
describe("issue #1938 evidence", () => {
	...
	test("uses only private disposable resources for actual restart and counterfactual", () => {
		...
		expect(script).not.toContain("SKC_SESSION_WORKDIR");
	});
	test("uses measured strict verdict deadlines", () => {
		...
		expect(script).toContain('SKC_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS"');
	...
	test("rejects a pre-trigger incident bundle even when its timestamps look fresh", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-evidence-baseline-"));
		try {
	...
	test("accepts same-millisecond publication only when the captured identity changed and rejects malformed state", async () => {
		const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skc-evidence-race-"));
		try {
	...
	test("renders semantically valid unsupported receipts through every available serializer", async () => {
		const environments: Array<Record<string, string>> = [
			{},
			{ SKC_ISSUE1938_TEST_DISABLE_PYTHON: "1" },
			{ SKC_ISSUE1938_TEST_DISABLE_PYTHON: "1", SKC_ISSUE1938_TEST_DISABLE_BUN: "1" },
		];
		for (const environment of environments) {
			...
			if (environment.SKC_ISSUE1938_TEST_DISABLE_BUN === "1") {
	...
	test("leaves no partial receipt when same-directory atomic publication is interrupted", async () => {
		for (const environment of [{}, { SKC_ISSUE1938_TEST_DISABLE_PYTHON: "1" }, { SKC_ISSUE1938_TEST_DISABLE_PYTHON: "1", SKC_ISSUE1938_TEST_DISABLE_BUN: "1" }]) {
			const sessionId = `serializer-interrupted-${randomUUID()}`;
			const sessionRoot = path.join(import.meta.dir, "..", "..", ".skc", `_session-${sessionId}`);
			const evidencePath = path.join(sessionRoot, "runtime", "evidence", "issue-1938", "pre-code.json");
			const proc = Bun.spawn(["bash", path.join(import.meta.dir, "issue-1938-cgroup-repro.sh"), "--phase", "pre-code", "--session-id", sessionId], { env: { ...process.env, SKC_ISSUE1938_TEST_FORCE_UNSUPPORTED: "1", SKC_ISSUE1938_TEST_FAIL_EVIDENCE_RENAME: "1", ...environment }, stdout: "pipe", stderr: "pipe" });
			try {
				expect(await proc.exited).toBe(1);
...
test("leaves no partial receipt when the Python serializer fails", async () => {
	...
	const sessionRoot = path.join(import.meta.dir, "..", "..", ".skc", `_session-${sessionId}`);
	const evidencePath = path.join(sessionRoot, "runtime", "evidence", "issue-1938", "pre-code.json");
	const proc = Bun.spawn(["bash", path.join(import.meta.dir, "issue-1938-cgroup-repro.sh"), "--phase", "pre-code", "--session-id", sessionId], { env: { ...process.env, SKC_ISSUE1938_TEST_FORCE_UNSUPPORTED: "1", SKC_ISSUE1938_TEST_FAIL_PYTHON_SERIALIZER: "1" }, stdout: "pipe", stderr: "pipe" });
	try {
		expect(await proc.exited).toBe(1);
		expect(await Bun.file(evidencePath).exists()).toBe(false);
		expect((await fs.readdir(path.dirname(evidencePath))).filter(name => name.includes(".pre-code.json."))).toEqual([]);
