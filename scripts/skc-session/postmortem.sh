#!/usr/bin/env bash
# Public-safe SKC session postmortem helpers. Markers contain only lifecycle
...
skc_session_write_public_marker() {
...
skc_session_validate_raw_verdict() {
...
PY
 ...
 skc_session_publish_current_alias() {
  ...
  if [[ "${SKC_SESSION_TRANSITION_LOCK_HELD:-0}" != 1 ]]; then
    exec {skc_alias_lock_fd}>"$transition_lock"
    flock -x "$skc_alias_lock_fd"
  fi
...
PY
  local rc=$?
  if [[ "${SKC_SESSION_TRANSITION_LOCK_HELD:-0}" != 1 ]]; then
    flock -u "$skc_alias_lock_fd"
    eval "exec ${skc_alias_lock_fd}>&-"
  fi
  return "$rc"
}
