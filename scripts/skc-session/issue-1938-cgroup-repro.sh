#!/usr/bin/env bash
# Disposable, public-safe evidence for issue #1938. It never targets a SKC
...
EVIDENCE_DIR="$REPO_ROOT/.skc/_session-$SESSION_ID/runtime/evidence/issue-1938"
EVIDENCE_PATH="$EVIDENCE_DIR/$PHASE.json"


now() {
  if [[ "${SKC_ISSUE1938_TEST_DISABLE_PYTHON:-}" != 1 ]] && command -v python3 >/dev/null 2>&1; then
  ...
  elif [[ "${SKC_ISSUE1938_TEST_DISABLE_BUN:-}" != 1 ]] && command -v bun >/dev/null 2>&1; then
...
if [[ "${SKC_ISSUE1938_TEST_FORCE_UNSUPPORTED:-}" == 1 ]]; then SYSTEMCTL_USER=false; DISPOSABLE_UNIT=false; fi
[[ "${SKC_ISSUE1938_TEST_DISABLE_PYTHON:-}" == 1 ]] && PYTHON3_AVAILABLE=false
[[ "${SKC_ISSUE1938_TEST_DISABLE_BUN:-}" == 1 ]] && BUN_AVAILABLE=false
...
publish_evidence_temp() {
  local temporary="$1"
  [[ "${SKC_ISSUE1938_TEST_FAIL_EVIDENCE_RENAME:-}" != 1 ]] || { rm -f "$temporary"; return 1; }
...
if os.environ.get("SKC_ISSUE1938_TEST_FAIL_PYTHON_SERIALIZER") == "1": raise SystemExit(1)
PY
...
private_tmux_session_pid() {
  ...
  [[ "${SKC_ISSUE1938_TEST_TMUX_CLEANUP_PROBE:-}" != error ]] || return 2
...
unit_is_explicitly_gone() {
  ...
  [[ "${SKC_ISSUE1938_TEST_SYSTEMD_CLEANUP_PROBE:-}" != error ]] || return 2
...
if [[ -n "${SKC_ISSUE1938_TEST_CLEANUP_PROBE_ONLY:-}" ]]; then
  case "$SKC_ISSUE1938_TEST_CLEANUP_PROBE_ONLY" in
...
if ! git -C "$REPO_ROOT" diff --quiet -- . ':!/.skc/**' || ! git -C "$REPO_ROOT" diff --cached --quiet -- . ':!/.skc/**' || [[ -n "$(git -C "$REPO_ROOT" ls-files --others --exclude-standard -- ':!/.skc/**')" ]]; then
...
SERVICE_UNIT="skc-issue1938-$RUN_PREFIX-$$-$RANDOM.service"
TMUX_SOCKET="skc-issue1938-$RUN_PREFIX-$$-$RANDOM"
...
independent_scope() { [[ "$1" =~ (^|/)skc-owner-[A-Za-z0-9-]+\.scope(/|;|$) ]]; }
owner_scope() { [[ "$1" =~ (^|/)skc-owner-([A-Za-z0-9-]+)\.scope ]] && printf 'skc-owner-%s.scope\n' "${BASH_REMATCH[2]}"; }
...
run_pre_code() {
  ...
  SCOPE_UNIT="skc-issue1938-$SESSION_ID-$$-$RANDOM.scope"; TRACKED_UNITS+=("$SCOPE_UNIT")
...
run_post_code() {
  ...
  WORKTREE="$(mktemp -d)"; WORKTREE_BRANCH="skc-issue1938-$RUN_PREFIX-$$-$RANDOM"; started="$(now)"
  ...
  mkdir -p "$WORKTREE/tmux" "$EVIDENCE_DIR/artifacts"; TMUX_SOCKET="skc-issue1938-$RUN_PREFIX-$$-$RANDOM"
  ...
  product_bin="$WORKTREE/issue-1938-skc"; tmux_wrapper="$WORKTREE/issue-1938-tmux"
  printf '#!/usr/bin/env bash\nexec %q %q "$@"\n' "$bun_bin" "$WORKTREE/packages/coding-agent/src/cli.ts" >"$product_bin"
  printf '#!/usr/bin/env bash\nexec %q -L "$SKC_ISSUE1938_TMUX_SOCKET" "$@"\n' "$tmux_bin" >"$tmux_wrapper"
  chmod 700 "$product_bin" "$tmux_wrapper"

  raw_session="issue1938-raw-$RUN_PREFIX-$$"; raw_state="$WORKTREE/.skc-session-state/$raw_session"; RAW_SOCKET="skc-${raw_session//[^A-Za-z0-9_.-]/_}"

  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" SKC_ISSUE1938_TMUX_SOCKET="$RAW_SOCKET" SKC_BIN="$product_bin" SKC_SESSION_TMUX_BIN="$tmux_wrapper" SKC_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" SKC_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/skc-session/create.sh" "$raw_session" "$WORKTREE"; then append_case raw_proof_before_exec failed "$started" "$(now)" '' "$raw_session" '' '' proven '' launch_failed "$SERVICE_UNIT"; return 1; fi
  ...
  managed_session="issue1938-managed-$RUN_PREFIX-$$"; SERVICE_UNIT="skc-issue1938-managed-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")


  if ! start_unsafe_service env -u TMUX -u TMUX_PANE -u SKC_TMUX_LAUNCHED TMUX_TMPDIR="$WORKTREE/tmux" SKC_ISSUE1938_TMUX_SOCKET="$TMUX_SOCKET" SKC_TMUX_COMMAND="$tmux_wrapper" SKC_TMUX_SESSION="$managed_session" SKC_LAUNCH_POLICY=tmux script -qefc "$product_bin --tmux" /dev/null; then append_case managed_proof_before_exec failed "$started" "$(now)" '' "$managed_session" '' '' proven '' launch_failed "$SERVICE_UNIT"; return 1; fi
  ...
  if TMUX_TMPDIR="$WORKTREE/tmux" SKC_ISSUE1938_TMUX_SOCKET="$RAW_SOCKET" SKC_TMUX_COMMAND="$tmux_wrapper" "$product_bin" session force-close "$raw_session" >/dev/null; then :; else append_case expected_close_verdict failed "$started" "$(now)" "$raw_pid" "$raw_session" "$raw_cgroup" SIGTERM expected_operator_shutdown SIGTERM unavailable; return 1; fi
  ...
  recovery_session="issue1938-recovery-$RUN_PREFIX-$$"; recovery_state="$WORKTREE/.skc-session-state/$recovery_session"; RECOVERY_SOCKET="skc-${recovery_session//[^A-Za-z0-9_.-]/_}"; SERVICE_UNIT="skc-issue1938-recovery-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")


  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" SKC_ISSUE1938_TMUX_SOCKET="$RECOVERY_SOCKET" SKC_BIN="$product_bin" SKC_SESSION_TMUX_BIN="$tmux_wrapper" SKC_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" SKC_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/skc-session/create.sh" "$recovery_session" "$WORKTREE"; then append_case unexpected_incident_recovery failed "$started" "$(now)" '' "$recovery_session" '' SIGTERM unexpected_owner_loss UNKNOWN launch_failed; return 1; fi
  ...
  SERVICE_UNIT="skc-issue1938-recovered-$RUN_PREFIX-$$-$RANDOM.service"; TRACKED_UNITS+=("$SERVICE_UNIT")
  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" SKC_ISSUE1938_TMUX_SOCKET="$RECOVERY_SOCKET" SKC_BIN="$product_bin" SKC_SESSION_TMUX_BIN="$tmux_wrapper" SKC_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" SKC_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/skc-session/create.sh" "$recovery_session" "$WORKTREE"; then append_case unexpected_incident_recovery failed "$started" "$(now)" '' "$recovery_session" '' SIGTERM unexpected_owner_loss UNKNOWN recovery_launch_failed '' '' "$recovery_key" "$recovery_latency"; return 1; fi
