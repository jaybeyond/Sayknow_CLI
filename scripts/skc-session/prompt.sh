#!/usr/bin/env bash
# Send a prompt to an existing interactive SKC tmux session.
...
PROMPT_EVIDENCE_ATTEMPTS="${SKC_SESSION_PROMPT_EVIDENCE_ATTEMPTS:-10}"
...
find_durable_pane_logs() {
  if [[ -n "${SKC_SESSION_STATE_DIR:-}" && -z "${SKC_SESSION_LOG_SEARCH_ROOT:-}" && -f "$SKC_SESSION_STATE_DIR/pane.log" ]]; then
    printf '%s\n' "$SKC_SESSION_STATE_DIR/pane.log"
  else
    find "${SKC_SESSION_LOG_SEARCH_ROOT:-$HOME/Workspace}" \( -path "*/.skc-session-state/$SESSION/pane.log" -o -path "*/$SESSION/pane.log" \) -type f 2>/dev/null | sort
  fi
}

prompt_accepted_path() {
  if [[ -n "${SKC_SESSION_STATE_DIR:-}" && -z "${SKC_SESSION_LOG_SEARCH_ROOT:-}" ]]; then
    printf '%s\n' "$SKC_SESSION_STATE_DIR/prompt-accepted.json"
    return 0
  fi
  local candidates=()
...
record_prompt_accepted() {
  ...
  local worktree_baseline_dirty="${SKC_SESSION_PROMPT_WORKTREE_BASELINE_DIRTY:-null}"
...
write_pre_prompt_vanished() {
  ...
  if [[ -z "$state_dir" && -n "${SKC_SESSION_STATE_DIR:-}" ]]; then
    state_dir="$SKC_SESSION_STATE_DIR"
...
if ! printf '%s\n' "$PANE_TEXT" | grep -qE 'Sayknow forge|Type your message|> Type your message|Working'; then
  mapfile -t candidates < <(find_durable_pane_logs)
  if [[ "${#candidates[@]}" -gt 0 ]]; then
  ...
  echo "refusing to paste prompt: SKC TUI is not ready in session $SESSION" >&2
...
if [[ -n "${SKC_SESSION_STATE_DIR:-}" && -z "${SKC_SESSION_LOG_SEARCH_ROOT:-}" && -f "$SKC_SESSION_STATE_DIR/pane.log" ]]; then
  EVIDENCE_LOG="$SKC_SESSION_STATE_DIR/pane.log"
else
  ...
  if [[ "${#durable_log_candidates[@]}" -gt 0 ]]; then
    ...
    EVIDENCE_LOG="$(mktemp "${TMPDIR:-/tmp}/skc-session-prompt-evidence.XXXXXX")"
...
SKC_SESSION_PROMPT_WORKTREE_BASELINE_DIRTY="null"
...
PYMETA
  ...
  if [[ -n "$prompt_baseline_workdir" ]]; then
    SKC_SESSION_PROMPT_WORKTREE_BASELINE_DIRTY="$(skc_session_git_dirty_boolean "$prompt_baseline_workdir")"
  fi
fi
