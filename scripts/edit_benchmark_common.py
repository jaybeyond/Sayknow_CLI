...
sys.path.insert(0, str(REPO_ROOT / "python/skc-rpc/src"))

from skc_rpc import MessageEndEvent, MessageStartEvent, MessageUpdateEvent, RpcClient, ToolExecutionStartEvent  # noqa: E402
...
def resolve_repo_skc_bin() -> str | None:
...
def resolve_skc_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_skc_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("skc")
    if not found:
        raise SystemExit("Could not find `skc` on PATH and could not resolve the repo CLI. Set --skc-bin or SKC_BIN.")
    return found
...
async def run_all(spec: BenchmarkSpec, args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    skc_bin = resolve_skc_bin(args.skc_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skc-bin",
        default=os.environ.get("SKC_BIN"),
        help="Executable to launch. Defaults to the repo checkout CLI, then falls back to `skc` on PATH.",
    )
    parser.add_argument(
