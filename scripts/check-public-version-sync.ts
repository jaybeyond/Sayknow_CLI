...
import { isEmbeddableDoc } from "./docs-index-contract";
import {
	canonicalJsonBytes,
...
const PUBLIC_HOMEPAGE = "https://sayknow-cli.com";
const PUBLIC_RELEASE_STATE = `${PUBLIC_HOMEPAGE}/release-sync.json`;
const SOURCE_REPOSITORY = "jaybeyond/Sayknow_CLI";
...
const EXPECTED_EVIDENCE_ASSET = "sayknow-release-packages-expected-v1.json";
const FINAL_EVIDENCE_ASSET = "sayknow-release-packages-v1.json";
...
const VERSIONED_MARKETING_RE = /\b(?:New in|Also new in|Sayknow-CLI|Sayknow-CLI|Feature card for the)\s+(\d+\.\d+\.\d+)\b/gi;
const MARKETING_VERSION_FILES = ["README.md", "docs/**/*.md", "packages/*/README.md"];
const LIVE_FETCH_TIMEOUT_MS = 5_000;
const PUBLIC_FETCH_HEADERS = { "User-Agent": "sayknow-cli-public-version-sync/2.0" };
...
export async function buildDocsIndexOutput(repoRoot: string): Promise<string> {
	...
	return [
		...
		`Reflect.set(globalThis, Symbol.for("skc.docs-index.generated.loaded"), true);`,
		"",
...
export async function checkPublicVersionSync(repoRoot = path.join(import.meta.dir, "..")): Promise<SyncViolation[]> {
	...
	const canonicalVersion = catalog["@sayknow-cli/coding-agent"];

	if (!canonicalVersion) {
		violations.push({ path: "package.json", message: "Missing canonical @sayknow-cli/coding-agent catalog version." });
		return violations;
	}
