...
describe("stable release policy", () => {
	test("creates a draft only after exact-SHA CI verification, records expected/final package evidence, verifies binaries, then finalizes before hinting", async () => {
		...
		expect(jobSection(ci, "release_npm_publish")).toContain("--release-serialization-key sayknow-production-release");
		expect(jobSection(ci, "release_github_final_evidence")).toContain("sayknow-release-packages-v1.json");
		expect(jobSection(ci, "release_github_verify")).toContain("sayknow-release-packages-v1.json");
	...
	test("binds retained expected evidence to the exact source SHA, upload run, and successful main-CI run", async () => {
		...
		expect(expected).toContain("sayknow-release-evidence-$RELEASE_TAG-$SOURCE_SHA");
	...
	test("confines the npm token to a read-only ephemeral-credential job and separates GitHub writes", async () => {
		...
		expect(publish).toContain("mktemp \"$RUNNER_TEMP/sayknow-release-npmrc.XXXXXX\"");
	...
	test("uses only the dedicated Trigger App for a warning-only Actions workflow hint", async () => {
		...
		expect(hint).toContain("--repo jaybeyond/Sayknow_CLI-website");
