...
/**
 * G4 verifier for bundled SKC skill documentation.
 *
 *   --report  (default)  list command references and direct `.skc` shell mutations, exit 0
 *   --fail               exit non-zero on manifest drift or direct `.skc` shell mutations
 */
...
const skills = new Set<string>(CANONICAL_SKC_WORKFLOW_SKILLS);
...
function isSkill(value: string): value is CanonicalSkcWorkflowSkill {
...
function isRoleSelectorVerb(line: string, commandEndIndex: number): boolean {
	return line.slice(0, commandEndIndex).endsWith("skc team executor") && /\bskc\s+team\s+executor\s+["'`]/u.test(line);
}
...
function collectCommandRefs(file: string, content: string): CommandRef[] {
	...
	for (let i = 0; i < lines.length; i++) {
		const line = lines[i] ?? "";
		const commandPattern = /\bskc\s+(?:state\s+)?(deep-interview|ralplan|ultragoal|team)\s+([a-z][a-z0-9-]*)\b/gu;
...
function collectDirectSkcMutations(file: string, content: string): MutationRef[] {
	const refs: MutationRef[] = [];
	const relative = path.relative(repoRoot, file);
	const lines = content.split("\n");
	const mutationPattern = /(?:^|[;&|]\s*)(?:rm\s+(?:-[A-Za-z]*\s+)*|rmdir\s+|mkdir\s+(?:-[A-Za-z]*\s+)*|touch\s+|mv\s+|cp\s+|install\s+|tee\s+(?:-[A-Za-z]*\s+)*|printf\b[^|;>]*>|echo\b[^|;>]*>|cat\b[^|;>]*>|>+\s*)\.?\.skc(?:\b|\/)/u;
	for (let i = 0; i < lines.length; i++) {
...
function main(): void {
	...
	for (const skill of CANONICAL_SKC_WORKFLOW_SKILLS) {
		const file = path.join(skillsRoot, skill, "SKILL.md");
		const content = fs.readFileSync(file, "utf8");
		commandRefs.push(...collectCommandRefs(file, content));
		mutationRefs.push(...collectDirectSkcMutations(file, content));
	}

	const drift = commandRefs.filter(ref => !ref.valid);
	console.log(`skc skill docs verifier - scanned ${path.relative(repoRoot, skillsRoot)}/*/SKILL.md`);
	console.log(`Found ${commandRefs.length} skc command reference(s).`);
	console.log(`Found ${mutationRefs.length} direct .skc shell mutation example(s).\n`);
	...
	console.log(`\nSummary: ${drift.length} command drift issue(s), ${mutationRefs.length} direct .skc shell mutation example(s).`);

	if (failMode && (drift.length > 0 || mutationRefs.length > 0)) {
		console.error(`\nG4 FAIL: skill docs must reference manifest verbs only and avoid direct .skc shell mutation examples.`);
		process.exit(1);
	}
}
