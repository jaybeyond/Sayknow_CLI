...
const canonicalSmokeArgv = ["packages/coding-agent/dist/skc", "--smoke-test"] as const;
...
async function loadNativeIdentity(): Promise<{ packageVersion: string; versionSentinelExport: string }> {
	...
	if (nativePackage.name !== "@sayknow-cli/natives" || !isString(nativePackage.version) || nativePackage.version.trim().length === 0) {
		throw new Error("darwin-receipt-invalid: @sayknow-cli/natives package version is unavailable");
	}
...
function assertReceipt(value: unknown, expectedSourceSha: string, nativeIdentity: { packageVersion: string; versionSentinelExport: string }): asserts value is DarwinReceipt {
	...
	if (value.nativePackageVersion !== nativeIdentity.packageVersion) throw new Error("darwin-receipt-invalid: native package version does not match @sayknow-cli/natives");
