...
import { listCommandRefBlocks, renderCommandRefBlock } from "../packages/coding-agent/src/skc-runtime/workflow-command-ref";
...
function usage(): never {
	console.error("Usage: bun scripts/generate-skc-skill-command-refs.ts [--check] [--strict] [--json]");
	process.exit(2);
}
...
function main(): void {
	...
	if (json) {
		...
		console.log("SKC skill command-ref proof spike (read-only)");
