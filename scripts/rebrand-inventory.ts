...
const expectedPackageScope = "@sayknow-cli/";
const expectedCliBins = ["skc", "skc-stats"] as const;
const expectedRootPackageName = "sayknow-cli";
...
const legacyAllowlist = [
...

	{
		...
		path: /^package\.json$/,
		rationale: "Specific root package scripts may reference retained legacy runtime roots while public metadata remains SKC-branded.",
	},
	{
...
function listVisibleDefinitions(): VisibleDefinition[] {
	return [
		...listSkillDirs(".skc/skills"),
		...listDefinitionFiles(".skc/agents", "agent", [".md", ".toml"]),
		...listDefinitionFiles(".skc/commands", "command", [".md"]),
		...listDefinitionFiles(".skc/rules", "rule", [".md"]),
	].sort((a, b) => a.name.localeCompare(b.name) || a.path.localeCompare(b.path));
}

function listBundledWorkflowSkills(): VisibleDefinition[] {
	return listSkillDirs("packages/coding-agent/src/defaults/skc/skills").sort((a, b) => a.name.localeCompare(b.name) || a.path.localeCompare(b.path));
}
...
function scanLegacyHits(): LegacyHit[] {
	const roots = [
		...
		"Dockerfile.roboskc.dockerignore",
	];
...
const nonSayknowPackages = packages.filter(pkg => pkg.name && !pkg.name.startsWith(expectedPackageScope) && !allowedUnscopedPackageNames.has(pkg.name));
