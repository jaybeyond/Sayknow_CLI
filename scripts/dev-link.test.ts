...
function bunMetadata(target = "@sayknow-cli\\coding-agent\\bin\\skc.js", command = "bun "): Buffer {
...
async function workspaceFixture(): Promise<string> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-dev-link-windows-"));
	tempRoots.push(root);
	...
	await Bun.write(
		...
		JSON.stringify({ bin: { skc: "bin/skc.js" }, exports: { "./cli": "./src/cli.ts" } }),
	);
	await Bun.write(
		path.join(packageRoot, "bin", "skc.js"),
		'#!/usr/bin/env bun\nimport { runCli } from "@sayknow-cli/coding-agent/cli";\n\nawait runCli(process.argv.slice(2));\n',
	);
	await Bun.write(path.join(packageRoot, "src", "cli.ts"), "export {};\n");
	await fs.mkdir(path.join(root, "node_modules", "@sayknow-cli"), { recursive: true });
	await fs.symlink(packageRoot, path.join(root, "node_modules", "@sayknow-cli", "coding-agent"), "dir");
	await fs.mkdir(path.join(root, "node_modules", ".bin"), { recursive: true });
	await Bun.write(path.join(root, "node_modules", ".bin", "skc.exe"), windowsShimExecutable());
	await Bun.write(path.join(root, "node_modules", ".bin", "skc.bunx"), bunMetadata());
...
describe("dev:link command discovery", () => {
	test("uses Windows PATH directory order and PATHEXT order with case-insensitive deduplication", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-dev-link-path-"));
		tempRoots.push(root);
		const first = path.join(root, "first");
		const second = path.join(root, "second");
		await makeExecutable(path.join(first, "skc.CMD"), "");
		await makeExecutable(path.join(first, "skc.exe"), "");
		await makeExecutable(path.join(second, "skc.exe"), "");
		expect(pathDirs(`${first};${second}`, "win32")).toEqual([first, second]);
		expect(commandExtensions("win32", ".CMD;.exe;.CMD;.EXE")).toEqual([".CMD", ".exe"]);
		expect(findSkcOnPath(`${first};${second}`, "win32", ".CMD;.exe;.CMD;.EXE").map(hit => hit.file)).toEqual([
			path.join(first, "skc.CMD"),
			path.join(first, "skc.exe"),
			path.join(second, "skc.exe"),
		]);
	});
...
describe.skipIf(process.platform === "win32")("dev:link Windows Bun workspace shim provenance", () => {
	test("accepts a direct canonical source link and the exact valid Bun workspace shim", async () => {
		...
		const shim = path.join(root, "node_modules", ".bin", "skc.exe");
	...
	test("fails closed for missing, corrupt, foreign, or substituted shim metadata", async () => {
		const root = await workspaceFixture();
		const shim = path.join(root, "node_modules", ".bin", "skc.exe");
		const metadata = path.join(root, "node_modules", ".bin", "skc.bunx");
		...
		await Bun.write(path.join(root, "node_modules", "foreign", "bin", "skc.js"), "#!/usr/bin/env bun\n");
		await Bun.write(metadata, bunMetadata("foreign\\bin\\skc.js"));
		expect(isFixtureShim(shim, root)).toBe(false);
	...
	test("rejects bin and canonical export mismatches", async () => {
		const root = await workspaceFixture();
		const shim = path.join(root, "node_modules", ".bin", "skc.exe");
		await Bun.write(path.join(root, "packages", "coding-agent", "package.json"), JSON.stringify({ bin: { skc: "bin/other.js" } }));
		expect(isFixtureShim(shim, root)).toBe(false);
		const exportRoot = await workspaceFixture();
		const exportShim = path.join(exportRoot, "node_modules", ".bin", "skc.exe");
		await Bun.write(
			path.join(exportRoot, "packages", "coding-agent", "package.json"),
			JSON.stringify({ bin: { skc: "bin/skc.js" }, exports: { "./cli": "./src/other.ts" } }),
		);
		...
		const markerShim = path.join(markerRoot, "node_modules", ".bin", "skc.exe");
		await Bun.write(
			path.join(markerRoot, "packages", "coding-agent", "bin", "skc.js"),
			'#!/usr/bin/env bun\n// from "@sayknow-cli/coding-agent/cli"\nconsole.log("smoke-test: ok");\n',
		);
	...
	test("rejects wrong-extension, compiled, and published targets even at an allowlisted path", async () => {
		const root = await workspaceFixture();
		const shim = path.join(root, "node_modules", ".bin", "skc.exe");
		expect(isFixtureSource(path.join(root, "node_modules", ".bin", "skc.cmd"), null, root, "win32")).toBe(false);
		expect(isFixtureSource(path.join(root, "dist", "skc.exe"), path.join(root, "dist", "skc.exe"), root, "win32")).toBe(false);
		expect(
			isFixtureSource(
				path.join(root, "node_modules", "sayknow-cli", "bin", "skc.js"),
				path.join(root, "node_modules", "sayknow-cli", "bin", "skc.js"),
				root,
				"win32",
			),
...
describe("dev:link", () => {
	test.skipIf(process.platform === "win32")("fails when a shadow skc earlier on PATH would make smoke-test validate the wrong command", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-dev-link-shadow-"));
		tempRoots.push(root);
		const shadowDir = path.join(root, "shadow-bin");
		const targetDir = path.join(root, "managed-bin");
		await makeExecutable(
			path.join(shadowDir, "skc"),
			`#!/usr/bin/env sh\nif [ "$1" = "--smoke-test" ]; then echo "smoke-test: ok"; exit 0; fi\necho shadow\nexit 0\n`,
		);
		const result = Bun.spawnSync([process.execPath, "scripts/dev-link.ts"], {
			env: { ...process.env, SKC_DEV_LINK_DIR: targetDir, PATH: `${shadowDir}:${targetDir}` },
			stderr: "pipe",
			stdout: "pipe",
		});
