 ...
 *
 * The locked design requires that EVERY native write to `.skc/**` flows through one sanctioned
 * writer module. This verifier reports filesystem mutation call sites whose path argument is
 * locally tied to a `.skc` path literal or a known `.skc` path helper.
 *
 *   --report  (default)  list every candidate `.skc/**` write site, exit 0
...
// The one module allowed to perform raw `.skc/**` filesystem mutations once routing is complete.
const ALLOWED_WRITER_RELATIVE = path.join("packages", "coding-agent", "src", "skc-runtime", "state-writer.ts");
...
const KNOWN_ALLOWED_SITES = new Set<string>([
	"packages/coding-agent/src/skc-runtime/team-runtime.ts:1610:fs.mkdir",
	"packages/coding-agent/src/skc-runtime/team-runtime.ts:1612:fs.mkdir",
	"packages/coding-agent/src/skc-runtime/team-runtime.ts:1623:fs.mkdir",
...
// `.skc` is referenced directly, or via a known path-helper symbol that resolves under `.skc`.
const SKC_REFERENCE_PATTERNS: readonly RegExp[] = [
	/["'`]\.skc(?:[\\/]|["'`])/u,
	/\bstateDirFor\b/u,
	/\bmodeStateFile\b/u,
	/\bworkflowStateStoragePath\b/u,
	/\bresolveSkcTeamStateRoot\b/u,
...
function locallyReferencesSkc(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 3);
	const end = Math.min(lines.length - 1, index + 3);
	for (let i = start; i <= end; i++) {
		const line = lines[i]?.trim() ?? "";
		if (line.startsWith("//") || line.startsWith("*")) continue;
		if (SKC_REFERENCE_PATTERNS.some(re => re.test(line))) return true;
	}
	return false;
}
...
function nearbyAssignmentTargetsThisLine(lines: readonly string[], index: number): boolean {
	...
	for (let i = start; i < index; i++) {
		...
		if (!SKC_REFERENCE_PATTERNS.some(re => re.test(prior))) continue;
...
function sameLineReferencesSkc(line: string): boolean {
	return SKC_REFERENCE_PATTERNS.some(re => re.test(line));
}


function isGuardedOutsideProjectSkcFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 8);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectSkc\([\s\S]*?\}\s*else\s*\{[\s\S]*$/u.test(context);
}

function isGuardedOutsideProjectSkcTernaryFallback(lines: readonly string[], index: number): boolean {
	const start = Math.max(0, index - 6);
	const context = lines.slice(start, index + 1).join("\n");
	return /isUnderProjectSkc\([\s\S]*?\?[\s\S]*?:\s*[^\n]*$/u.test(context);
}
...
function collectFindings(): Finding[] {
	...
	for (const file of listTsFiles(SCAN_ROOT)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (!allowed && !sameLineReferencesSkc(line) && !nearbyAssignmentTargetsThisLine(lines, i)) continue;
			if (!allowed && (isGuardedOutsideProjectSkcFallback(lines, i) || isGuardedOutsideProjectSkcTernaryFallback(lines, i))) continue;
...
function main(): void {
	...
	console.log(`skc .skc/** writer inventory - scanned ${path.relative(repoRoot, SCAN_ROOT)}`);
	...
	if (failMode && offending.length > 0) {
		console.error(`\nG1 FAIL: ${offending.length} direct .skc/** write site(s) must route through ${ALLOWED_WRITER_RELATIVE}.`);
		process.exit(1);
	}
}
