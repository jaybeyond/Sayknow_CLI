...
async function createRepo(files: Record<string, string>): Promise<string> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "skc-public-version-sync-"));
	tempRoots.push(root);
	...
	return JSON.stringify(
		{
			workspaces: {
				catalog: {
					"@sayknow-cli/coding-agent": version,
					"sayknow-cli": version,
				},
			},
		},
...
function packageJson(name: string, version = "1.2.3", homepage = "https://sayknow-cli.com"): string {
...
const GIT_API = "https://api.github.com/repos/jaybeyond/Sayknow_CLI/git";

const LATEST_RELEASE_API = "https://api.github.com/repos/jaybeyond/Sayknow_CLI/releases/latest";
const RELEASE_LIST_API = "https://api.github.com/repos/jaybeyond/Sayknow_CLI/releases?per_page=100";
const RELEASE_LIST_PAGE_TWO_API = "https://api.github.com/repos/jaybeyond/Sayknow_CLI/releases?per_page=100&page=2";
const TAG_API = `${GIT_API}/ref/tags/v1.2.3`;

const RELEASE_STATE_URL = "https://sayknow-cli.com/release-sync.json";
const RELEASE_URL = "https://github.com/jaybeyond/Sayknow_CLI/releases/tag/v1.2.3";
const EXPECTED_ASSET_URL = "https://assets.example/sayknow-release-packages-expected-v1.json";
const FINAL_ASSET_URL = "https://assets.example/sayknow-release-packages-v1.json";
...
function stableRelease(assets?: Array<{ name: string; browser_download_url: string }>): Record<string, unknown> {
	return {
		...
		assets: assets ?? [
			{ name: "skc-linux-x64", browser_download_url: "https://assets.example/skc-linux-x64" },
			{ name: "skc-linux-arm64", browser_download_url: "https://assets.example/skc-linux-arm64" },
			{ name: "skc-darwin-arm64", browser_download_url: "https://assets.example/skc-darwin-arm64" },
			{ name: "skc-darwin-x64", browser_download_url: "https://assets.example/skc-darwin-x64" },
			{ name: "skc-windows-x64.exe", browser_download_url: "https://assets.example/skc-windows-x64.exe" },
			{ name: "sayknow-release-packages-expected-v1.json", browser_download_url: EXPECTED_ASSET_URL },
			{ name: "sayknow-release-packages-v1.json", browser_download_url: FINAL_ASSET_URL },
		],
	};
}
	...
	return `${JSON.stringify(
		{
			...
			release: {
				...
				tag: `v${version}`,
				url: `https://github.com/jaybeyond/Sayknow_CLI/releases/tag/v${version}`,
				version,
			},
			schema_version: 1,
			source: {
				...
				repository: "jaybeyond/Sayknow_CLI",
			},
		},
		null,
		2,
...
describe("public docs/site/version sync guard", () => {
	test("passes when package versions, homepage metadata, marketing docs, and generated docs index match", async () => {
		const root = await createRepo({
			...
			"packages/coding-agent/package.json": packageJson("@sayknow-cli/coding-agent"),
			"packages/sayknow-cli/package.json": packageJson("sayknow-cli"),
			"README.md": "# Sayknow-CLI\n\n## Recent highlights\n",
			"docs/sdk.md": "# SDK\n\nCurrent docs.\n",
		});
	...
	test("fails on package, catalog, homepage, stale marketing, and generated docs drift", async () => {
		const root = await createRepo({
			...
			"packages/coding-agent/package.json": packageJson("@sayknow-cli/coding-agent", "1.2.3"),
			"packages/sayknow-cli/package.json": packageJson("sayknow-cli", "1.2.2", "https://example.invalid"),
			"README.md": "# Sayknow-CLI\n\n## New in 1.2.2\n",
			"docs/sdk.md": "# SDK\n",
		...
		expect(violations.some(violation => violation.path === "packages/sayknow-cli/package.json" && violation.message.includes("version 1.2.2"))).toBe(true);
		expect(violations.some(violation => violation.path === "packages/sayknow-cli/package.json" && violation.message.includes("homepage"))).toBe(true);
		expect(violations.some(violation => violation.path === "package.json" && violation.message.includes("catalog sayknow-cli"))).toBe(true);
	...
	test("live check rejects a stable release without final package evidence before reading deployed state", async () => {
		...
		await expect(checkLivePublicVersionSync("unused", mockFetch(responses), 50)).resolves.toEqual([
			{
				path: LATEST_RELEASE_API,
				message: "Published release v1.2.3 is incomplete: missing sayknow-release-packages-v1.json.",
			},
		]);
	});
