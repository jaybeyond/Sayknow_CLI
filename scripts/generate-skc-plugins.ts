...
- \`scripts/skc-session/create.sh\` starts interactive \`skc\` in a named tmux session, validates the worktree, preserves the pane after exit, and optionally registers a Clawhip-style router watch.
- \`scripts/skc-session/prompt.sh\` sends text or an \`@file\` prompt after the pane looks like a ready SKC TUI.
- \`scripts/skc-session/tail.sh\` captures bounded pane output for readiness and acceptance checks.
