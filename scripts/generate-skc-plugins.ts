...
/**
 * Generate the canonical sayknow-cli host plugin bundles under `plugins/`.
 *
 ...
 *
 * Usage:
 *   bun scripts/generate-skc-plugins.ts            # write files
 *   bun scripts/generate-skc-plugins.ts --check    # compare bytes, exit 1 on drift
 */
...
const DELEGATE_TOOLS = COORDINATOR_MCP_TOOL_NAMES.filter(name => name.startsWith("skc_delegate_"));

const PLUGIN_NAME = "sayknow-cli";
const NAMESPACE_LABEL = "sayknow-cli-plugin";
...
const DELEGATE_META: DelegateMeta[] = [
	{
		tool: "skc_delegate_plan",
		workflow: "plan",
		skill: "ralplan",
		summary: "Delegate consensus planning to SKC (runs /skill:ralplan to a pending-approval plan).",
	},
	{
		tool: "skc_delegate_execute",
		workflow: "execute",
		skill: "ultragoal",
		summary: "Delegate execution to SKC (runs /skill:ultragoal to completion with verification).",
	},
	{
		tool: "skc_delegate_team",
		workflow: "team",
		skill: "team",
		summary: "Delegate parallel team execution to SKC (runs /skill:team with internal tmux workers).",
	},
];
...
function coordinatorServer(projectDirToken: string): Record<string, unknown> {
	return {
		"skc-coordinator": {
			...
			env: {
				SKC_COORDINATOR_MCP_WORKDIR_ROOTS: projectDirToken,
				SKC_COORDINATOR_MCP_REPO: NAMESPACE_LABEL,
...
Call the \`${meta.tool}\` coordinator MCP tool to delegate this work to sayknow-cli.
...
SKC starts a session and runs \`/skill:${meta.skill}\` to completion, returning a
...
function sessionSkillDoc(): string {
	return `---
name: skc-session
description: Use SKC's published tmux session helpers for Clawhip-visible worktree sessions, prompt injection, tail checks, and harness owner debugging.
---

# SKC session helpers

Use this skill when a task needs an operator-visible SKC session in tmux: Clawhip/Hermes/OpenClaw can watch the pane, route stale-session alerts, and send follow-up prompts while the work stays in a dedicated git worktree.
...
- \`scripts/skc-session/create.sh\` starts interactive \`skc\` in a named tmux session, validates the worktree, preserves the pane after exit, prints and writes the session-specific durable state path, writes \`metadata.json\`, mirrors pane output to \`pane.log\`, records lifecycle events in \`events.log\`, writes normal-exit \`final.json\`, and optionally registers a Clawhip-style router watch.
- \`scripts/skc-session/prompt.sh\` sends text or an \`@file\` prompt after the pane looks like a ready SKC TUI; if the tmux session vanished, it refuses injection and prints the durable metadata/log/final/events recovery paths plus the last pane-log excerpt.
- \`scripts/skc-session/tail.sh\` captures bounded pane output for readiness and acceptance checks, with durable metadata, pane-log, event-log, and final-status fallback when tmux vanished.
- \`scripts/skc-session/harness-tmux-owner-start.sh\` starts the harness RuntimeOwner inside tmux for dogfood/debug cases that need visible owner liveness.
- \`docs/skc-session-clawhip-routing.md\` documents the full routed-session contract.

## Standard flow

1. Prepare a dedicated worktree and branch for the issue or PR. Do not use the canonical checkout for visible routed work.
2. Pick a stable, unambiguous session name that includes the repository and artifact id, such as \`sayknow-cli-issue-1055-skc-session-skill\`.
3. Start the session:
...
4. Confirm readiness with bounded tail output:
   ...
   \`\`\`sh
   ./scripts/skc-session/tail.sh <session-name> 80
   \`\`\`

   Wait for a ready SKC TUI signal such as \`Sayknow forge\`, \`Type your message\`, \`> Type your message\`, or \`Working\`.
...
\`\`\`sh
./scripts/skc-session/harness-tmux-owner-start.sh <session-name> <workspace> [issue-or-pr] [branch-label] [base]
\`\`\`
...
- Starting long-running visible repo work with \`skc -p\` instead of an interactive tmux session.
...
function readmeDoc(): string {
	return `# sayknow-cli plugin (generated)

These files are generated by \`scripts/generate-skc-plugins.ts\` from the
...
- \`.codex.mcp.json\` — Codex coordinator MCP wiring (host-neutral; \`skc setup codex\` rewrites concrete roots).
- \`commands/\`, \`skills/\` — host-facing delegate command + skill docs.

Install: \`codex plugin marketplace add ./plugins\` (Codex) or \`/plugin marketplace add ./plugins\` (Claude Code), then install the \`sayknow-cli\` plugin.
`;
}
...
export function renderPluginFiles(): Map<string, string> {
	...
	// `codex plugin marketplace add ./plugins` reads this and loads ./sayknow-cli.
	files.set(
		path.join(".agents", "plugins", "marketplace.json"),
		json({
			name: `${PLUGIN_NAME}-local`,
			interface: { displayName: "Sayknow-CLI" },
			plugins: [
				{
					name: PLUGIN_NAME,
	...
	files.set(
		...
		json({
			name: PLUGIN_NAME,
			owner: { name: "Sayknow-CLI" },
			metadata: { description: "SKC delegation plugin", version },
			plugins: [
				{
					name: PLUGIN_NAME,
					source: `./${dir}`,
					description: "Delegate SKC planning/execution/team workflows via coordinator MCP.",
					version,
					author: { name: "Sayknow-CLI" },
					keywords: ["skc", "delegation", "mcp", "planning", "agents"],
				},
			],
		}),
	);
	...
	files.set(
		...
		json({
			name: PLUGIN_NAME,
			description: "Delegate planning, execution, and team workflows to SKC through the coordinator MCP server.",
			version,
			commands: "./commands",
			skills: "./skills",
			mcpServers: "./.mcp.json",
	...
	files.set(
		...
		json({
			name: PLUGIN_NAME,
			version,
			description: "Delegate Codex tasks to SKC workflows through coordinator MCP.",
			skills: "./skills/",
	...
	// host-neutral file that `skc setup codex` rewrites with a concrete workdir root.
	...
	files.set(path.join(dir, "skills", "skc-delegation", "SKILL.md"), skillDoc());
	files.set(path.join(dir, "skills", "skc-session", "SKILL.md"), sessionSkillDoc());
