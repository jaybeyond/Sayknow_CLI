 ...
 *   bun scripts/generate-skc-plugins.ts --check    # compare complete file set + bytes, exit 1 on drift
 */
...
function runSelfTest(): void {
	...
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "skc-plugin-self-test-"));
	try {
		for (const [rel, content] of files) {
			const target = path.join(root, rel);
			fs.mkdirSync(path.dirname(target), { recursive: true });
