...
def resolve_skc_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `skc` on PATH. Set --skc-bin or SKC_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through skc RPC mode."
    )
    parser.add_argument("--skc-bin", default=os.environ.get("SKC_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"skc-fixture-runs-{timestamp}"
    )
