...
export interface SkcHit {
	dir: string;
	file: string;
	real: string | null;
}

/** All `skc` entries on PATH, in shell resolution order (first wins). */
export function findSkcOnPath(
	...
	for (const dir of pathDirs(envPath, platform)) {
		for (const extension of commandExtensions(platform, pathext)) {
			const file = path.join(dir, `skc${extension}`);
...
export function isLocalWindowsBunShim(file: string, root = repoRoot, bunExecutable = process.execPath): boolean {
	const expectedShim = path.join(root, "node_modules", ".bin", "skc.exe");
	...
	try {
		...
		if (
			...
			manifest.bin === null ||
			!("skc" in manifest.bin) ||
			manifest.bin.skc !== "bin/skc.js"
		) {
			return false;
		}
		resolvedCli = Bun.resolveSync("@sayknow-cli/coding-agent/cli", root);
...
export function smokeTest(skcPath: string): { ok: boolean; output: string } {
