...
async function checkTypeDeclarations(): Promise<void> {
	const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sayknow-cli-types-"));
	try {
		for (const pkg of packages) {
...
async function assertPinnedNpmConfiguration(): Promise<void> {
	...
	const scopedRegistry = await readNpmConfig("@sayknow-cli:registry");
	if (scopedRegistry !== undefined) validateNpmRegistryUrl(scopedRegistry, "@sayknow-cli npm registry");
...
async function packPackageTwice(pkg: PublishPackage): Promise<Buffer> {
	...
	for (let attempt = 0; attempt < 2; attempt += 1) {
		const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sayknow-cli-release-pack-"));
		try {
