...
/**
 * Static verification for the generated sayknow-cli plugin bundles.
 *
...
import { findUnexpectedPluginFiles, renderPluginFiles } from "./generate-skc-plugins";

const PLUGIN_DIR = "sayknow-cli";
...
function coordinatorMcpServer(config: Record<string, unknown>): Record<string, unknown> | undefined {
	...
	return record(servers["skc-coordinator"]);
}
...
const codexMcpOk = "mcp_servers" in codexMcpObj || "skc-coordinator" in codexMcpObj;
...
for (const rel of mcpFiles) {
	const text = read(rel);
	if (!text.includes("SKC_COORDINATOR_MCP_WORKDIR_ROOTS")) workdirRootsOk = false;
	if (/SKC_COORDINATOR_MCP_ROOTS[^_]/.test(text)) noBadRoots = false;
	if (text.includes("SKC_COORDINATOR_MCP_MUTATIONS")) noMutations = false;
}
gate("MCP config uses WORKDIR_ROOTS", mcpFiles.length > 0 && workdirRootsOk, `mcp files: ${mcpFiles.length}`);
gate("MCP config omits invalid ROOTS var", noBadRoots, "no SKC_COORDINATOR_MCP_ROOTS present");
...
const directMachineTmuxRoutePattern =
	/(?:\btmux\s+watch\b|(?:scripts\/)?skc-session\/(?:prompt|tail|watch)(?:\.sh)?\b|\bskc-session\s+(?:prompt|tail|watch)\b|(?:\.\/)?(?:scripts\/)?skc-session\/create\.sh(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3}|\bskc-session\s+create(?:[ \t]+(?:"[^"\n]*"|'[^'\n]*'|[^\s]+)){3})/g;
...
const machineTmuxRouteRegressionFixtures = [
	...
	'["capture", "pane"].join("-")',
	'["pipe", "pane"].join("-")',
	'./scripts/skc-session/create.sh bot /repo --print "task"',
	"scripts/skc-session/create.sh bot /repo positional-prompt",
	"skc-session create bot /repo --file task.md",
	"skc-session create bot /repo resume",
	"skc-session create bot /repo \\\n  positional-prompt",
