...
for pkg in "${PACKAGES[@]}"; do
    ...
    echo "Dependencies:"
    jq '.dependencies | to_entries[] | select((.value | type) == "string" and (.value | test("^(@sayknow-cli|workspace|catalog)")))' package.json 2>/dev/null || true
...
RUN verdaccio --config /root/.config/verdaccio/config.yaml &>/dev/null & \
    sleep 3 && \
    bun add sayknow-cli --registry http://localhost:4873 && \
    pkill -f verdaccio
