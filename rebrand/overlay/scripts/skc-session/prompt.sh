#!/usr/bin/env bash
# Submit a prompt to an existing isolated SKC tmux owner without reading pane data.
...
if [[ -z "$STATE_DIR" && -n "${SKC_SESSION_WORKDIR:-}" ]]; then
  STATE_DIR="$SKC_SESSION_WORKDIR/.skc-session-state/$SESSION"
fi
...
TMUX_SOCKET_OVERRIDE="${SKC_SESSION_TMUX_SOCKET:-}"
...
for option_value in "@skc-owner-generation=$OWNER_GENERATION" "@skc-owner-server-key=$TMUX_SOCKET"; do
...
worktree_baseline_dirty="${SKC_SESSION_PROMPT_WORKTREE_BASELINE_DIRTY:-null}"
...
PY
  ...
  [[ -z "$workdir" ]] || worktree_baseline_dirty="$(skc_session_git_dirty_boolean "$workdir")"
fi
...
BUFFER_NAME="skc-prompt-${OWNER_GENERATION//[^A-Za-z0-9]/}-${BASHPID}"
...
PY
skc_session_publish_current_alias "$ACCEPTED_CANONICAL" "$ACCEPTED_JSON" "$GENERATION_JSON" "$SESSION" "$OWNER_GENERATION" prompt_accepted
