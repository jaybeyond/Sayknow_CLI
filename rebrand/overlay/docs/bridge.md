...
```bash
skc --mode bridge [regular CLI options]
```
...
| --- | --- | --- | --- |
| `SKC_BRIDGE_TOKEN` | Yes | — | Bearer token for authenticated endpoints. **Secret — never commit.** |
| `SKC_BRIDGE_TLS_CERT` | Yes | — | Path to the TLS certificate (PEM). |
| `SKC_BRIDGE_TLS_KEY` | Yes | — | Path to the TLS private key (PEM). **Secret — never commit.** |
| `SKC_BRIDGE_HOST` | No | `127.0.0.1` | Bind hostname. |
| `SKC_BRIDGE_PORT` | No | `4077` | Bind port (1–65535). |
| `SKC_BRIDGE_SCOPES` | No | `prompt` | Parsed for internal compatibility, but default session endpoints are fail-closed. |
...
- **TLS is mandatory for every bind, including loopback.** Startup fails closed
  with a clear error if `SKC_BRIDGE_TLS_CERT` and `SKC_BRIDGE_TLS_KEY` are not
...
`@sayknow-cli/bridge-client` exposes `BridgeClient` with handshake, command
...
> themselves. Importing `@sayknow-cli/coding-agent` internal `rpc-types` into the
...
For Hermes/Claw-style orchestration, treat `skc` as an external runner. The orchestration agent should choose or create the repository checkout first, preferably a dedicated Git worktree for branch-local work, then launch or attach a leader session with `skc --tmux` from that directory. SKC is not embedded runtime injection into Hermes, Claw Code, or another coding tool.

Public orchestration boundaries:

1. Choose the repo/worktree and branch that will own changes, logs, and review evidence.
2. Start or attach the SKC leader with `skc --tmux` from that directory. If you want SKC to create the sibling worktree, use `skc --tmux --worktree <branch-like-name>`; the argument is a worktree/branch name, not a filesystem path.
3. Submit the workflow appropriate to the task: `/skill:deep-interview` for requirements discovery, `/skill:ralplan` for plan consensus, and `skc ultragoal ...` for durable goal tracking through execution and verification.
4. Use `skc team ...` only when coordinated parallel tmux workers help with implementation or verification; single-lane work should stay in the leader session.
...
Bridge mode remains the public remote-control protocol for an already-running SKC session, but the session-control endpoints are fail-closed by default. Keep lifecycle, worktree selection, and evidence policy above the bridge frames, and avoid documenting private deployment, routing, or credential internals. Introducing another authenticated remote-control protocol for the same purpose should require ADR-level rationale.

The same external-runner workflow is summarized in the README section [Using SKC with other coding agents](../README.md#using-skc-with-other-coding-agents).
