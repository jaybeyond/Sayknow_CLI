# Sayknow Remote — v0 Design (thin phone steering wheel)

Status: **design / pending approval** · Tracks issue #565 · Scope: **v0 only**

Sayknow Remote is a thin remote *steering wheel* for an already-running PC-side
`skc` session. It is intentionally **not** a universal phone shell, a remote
...
| --- | --- |
| Network transport, TLS, bearer auth, fail-closed posture | Bridge mode (`skc --mode bridge`), [`docs/bridge.md`](bridge.md) |
| Client SDK / framing | `@sayknow-cli/bridge-client` (`BridgeClient`, `events()`) |
| Session state storage, liveness, bounded status, submit gating | Harness control plane (`skc harness`), `packages/coding-agent/src/harness-control-plane/` |
...
The only genuinely new piece is a thin **Sayknow Remote gateway**: one PC-side
...
```
 phone (mobile web)
        ...
        ▼
 Sayknow Remote gateway  ── enumerates ─▶ harness control-plane session-state dir
...
A one-line submit travels the same path as a local `skc harness submit`, so it
...
| `harness` | `SessionState.harness` | `sayknow-cli` in v1 |
...
1. **PR 1 — this doc.** `docs/sayknow-remote.md` + README cross-link. Resolves the
...
6. **PR 6 — mobile web client.** Minimal SPA (list → open → status → submit)
   using `@sayknow-cli/bridge-client` and the `packages/stats` build pattern.
