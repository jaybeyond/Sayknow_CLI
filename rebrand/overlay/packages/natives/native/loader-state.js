...
/**
 * Native addon loader for `@sayknow-cli/natives`.
 *
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "skc"))) {
		return path.join(xdgDataHome, "skc", "natives");
	}
	return path.join(os.homedir(), ".skc", "natives");
}
 ...
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `skc`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@sayknow-cli/natives/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent skc
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/jaybeyond/Sayknow_CLI/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @sayknow-cli/natives\n" +
...
function initLoaderContext() {
	...
	const userDataDir =
		process.platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "skc")
