...
const DEFAULT_COORDINATOR_COMMAND = "skc";
...
function resolveWorkdirRoots(env: Env, presets: Map<string, GatewayPreset>): string {
	const explicit = env.SKC_COORDINATOR_MCP_WORKDIR_ROOTS?.trim();
...
function resolveSessionCommand(env: Env, presets: Map<string, GatewayPreset>): string {
	const explicit = env.SKC_COORDINATOR_MCP_SESSION_COMMAND?.trim();
...
function buildCoordinatorEnv(
	...
	for (const key of [
		"SKC_COORDINATOR_MCP_PROFILE",
		"SKC_COORDINATOR_MCP_REPO",
		"SKC_COORDINATOR_MCP_STATE_ROOT",
		"SKC_COORDINATOR_MCP_ARTIFACT_BYTE_CAP",
	]) {
		const value = env[key]?.trim();
		if (value) coordinatorEnv[key] = value;
	}
	const workdirRoots = resolveWorkdirRoots(env, presets);
	if (workdirRoots.length > 0) coordinatorEnv.SKC_COORDINATOR_MCP_WORKDIR_ROOTS = workdirRoots;
	const sessionCommand = resolveSessionCommand(env, presets);
	if (sessionCommand.length > 0) coordinatorEnv.SKC_COORDINATOR_MCP_SESSION_COMMAND = sessionCommand;
	// Force the smallest mutation set. `questions` is never enabled.
	coordinatorEnv.SKC_COORDINATOR_MCP_MUTATIONS = enableStop ? "sessions,reports" : "sessions";
...
export function loadConfigFromEnv(env: Env): ServiceConfig {
	const botToken = required(env, "SKC_TELEGRAM_REMOTE_BOT_TOKEN");
	const defaultTaskMaxLen = Number.parseInt(env.SKC_TELEGRAM_REMOTE_DEFAULT_TASK_MAX_LEN ?? "", 10);
	const taskMaxLen =
		Number.isInteger(defaultTaskMaxLen) && defaultTaskMaxLen > 0 ? defaultTaskMaxLen : DEFAULT_TASK_MAX_LEN;

	const allowedUserIds = parseIdSet(env.SKC_TELEGRAM_REMOTE_ALLOWED_USER_IDS);
	const allowedChatIds = parseIdSet(env.SKC_TELEGRAM_REMOTE_ALLOWED_CHAT_IDS);
	if (allowedUserIds.size === 0 && allowedChatIds.size === 0) {
		throw new Error("telegram_remote_no_allowlist");
	}

	const presets = parsePresets(env.SKC_TELEGRAM_REMOTE_PRESETS, taskMaxLen);
	const enableStop = isTruthy(env.SKC_TELEGRAM_REMOTE_ENABLE_STOP);

	const pollTimeoutSec = parsePositiveInt(env.SKC_TELEGRAM_REMOTE_POLL_TIMEOUT_SEC, 30);
	const backend = parseBackend(env.SKC_TELEGRAM_REMOTE_BACKEND);
	const rpcSocket = env.SKC_TELEGRAM_REMOTE_RPC_SOCKET?.trim();
	const stateDirValue = env.SKC_TELEGRAM_REMOTE_STATE_DIR?.trim();
	...
	if (backend === "rpc") {
		if (!rpcSocket) throw new Error("telegram_remote_missing_env:SKC_TELEGRAM_REMOTE_RPC_SOCKET");
		if (!stateDir) throw new Error("telegram_remote_missing_env:SKC_TELEGRAM_REMOTE_STATE_DIR");
	}

	return {
		botToken,
		apiBase: env.SKC_TELEGRAM_REMOTE_API_BASE?.trim() || undefined,
		pollTimeoutSec,
		enableEditMessageText: isTruthyDefault(env.SKC_TELEGRAM_REMOTE_ENABLE_EDIT_MESSAGE_TEXT, false),
		registerBotCommands: isTruthyDefault(env.SKC_TELEGRAM_REMOTE_REGISTER_COMMANDS, true),
		stateDir,
		backend,
		rpc:
			backend === "rpc"
				? {
						...
						stateDir: stateDir!,
						livenessMs: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_LIVENESS_MS, DEFAULT_RPC_LIVENESS_MS),
						allowAttachSocketArg: isTruthyDefault(env.SKC_TELEGRAM_REMOTE_ALLOW_ATTACH_SOCKET_ARG, false),
					}
				: undefined,
		followTtlMs: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_FOLLOW_TTL_MS, DEFAULT_FOLLOW_TTL_MS),
		enablePush: isTruthyDefault(env.SKC_TELEGRAM_REMOTE_ENABLE_PUSH, false),
		subscriptionsMax: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_SUBSCRIPTIONS_MAX, DEFAULT_SUBSCRIPTIONS_MAX),
		longPollMs: parseClampedPositiveInt(env.SKC_TELEGRAM_REMOTE_WATCH_TIMEOUT_MS, DEFAULT_LONG_POLL_MS, 30_000),
		digestThreshold: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_DIGEST_THRESHOLD, DEFAULT_DIGEST_THRESHOLD),
		policy: {
			allowedUserIds,
			allowedChatIds,
			presets,
			enableStop,
			enableRichMessages: isTruthyDefault(env.SKC_TELEGRAM_REMOTE_ENABLE_RICH, true),
			richCallbackTtlMs: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_RICH_CALLBACK_TTL_MS, 600_000),
			richCallbackMaxTokens: parsePositiveInt(env.SKC_TELEGRAM_REMOTE_RICH_CALLBACK_MAX_TOKENS, 500),
		},
		coordinator: {
			command: env.SKC_TELEGRAM_REMOTE_COORDINATOR_COMMAND?.trim() || DEFAULT_COORDINATOR_COMMAND,
			args: parseList(env.SKC_TELEGRAM_REMOTE_COORDINATOR_ARGS, DEFAULT_COORDINATOR_ARGS),
