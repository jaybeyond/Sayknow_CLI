...
describe("Coordinator MCP server protocol", () => {
	...
	it("starts sessions through the structured SKC service adapter, not arbitrary terminal relay", async () => {
		...
		expect(JSON.parse(response.result.content[0].text).session.session_id).toBe("skc-demo");
	...
	it("delivers start-session prompts exactly once after the active turn is durable", async () => {
			...
			services: {
				commandRunner: async command => {
					...
					if (command[1] === "new-session")
						return { exitCode: 0, stdout: "skc-coordinator-test:0.0 %99\n", stderr: "" };
					if (command[1] === "has-session") return { exitCode: 0, stdout: "", stderr: "" };
		...
		expect(commands.filter(command => command[1] === "send-keys")).toEqual([
			["tmux", "send-keys", "-t", "skc-coordinator-test:0.0", "hello", "C-m", "C-m"],
		]);
	});
	...
	it("exposes a canonical polling coordination snapshot", async () => {
		...
		expect(status.events).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ event_type: "session_state", session_id: "skc-demo", status: "completed" }),
				expect.objectContaining({ event_type: "turn_state", session_id: "skc-demo", status: "completed" }),
				expect.objectContaining({ event_type: "coordination_report", session_id: "skc-demo", status: "completed" }),
			]),
		);
	});
	...
	it("persists audited follow-up, question answers, and bounded reports", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({ id: "q1", session_id: "skc-demo", status: "open", schema: { max_length: 20 } }),
		);
		...
		const prompt = await server.handleJsonRpc({
			...
			params: {
				...
				arguments: { session_id: "skc-demo", prompt: "continue", allow_mutation: true },
			},
		});
	...
	it("validates turn and question ownership before path-addressed mutations", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({ id: "q-safe", session_id: "skc-demo", turn_id: turn.turn_id, status: "open" }),
		);
		await Bun.write(
	...
	it("preserves launch errors from runtime state before tmux liveness masking", async () => {
		...
		await fs.mkdir(sessionStatesDir, { recursive: true });
		await Bun.write(
			path.join(sessionStatesDir, "skc-demo.json"),
			JSON.stringify({
				schema_version: 1,
				session_id: "skc-demo",
				state: "errored",
				...
				commandRunner: async command => {
					...
					if (command[1] === "send-keys") {
						const activeTurn = JSON.parse(
							await Bun.file(path.join(stateRoot, "local", "repo", "active-turns", "skc-demo.json")).text(),
						) as {
							turn_id: string;
						};
						runtimeStatePath = path.join(stateRoot, "local", "repo", "session-states", "skc-demo.json");
