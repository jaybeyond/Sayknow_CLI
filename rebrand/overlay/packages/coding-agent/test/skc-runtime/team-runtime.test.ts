...
import { sessionReportsDir, teamStateRoot } from "../../src/skc-runtime/session-layout";
import {
	buildWorkerCommand,
	claimSkcTeamTask,
...
async function createFakeTmuxBin(
	root: string,
	options: { failDisplay?: boolean; failSplit?: boolean; skcProfile?: boolean; untaggableProfile?: boolean } = {},
): Promise<string> {
...
describe("native skc team runtime", () => {
	it("creates SKC-scoped team state, task mailboxes, and telemetry without delegating to legacy runtimes", async () => {
		...
		const snapshot = await startSkcTeam({
			...
			cwd: cleanupRoot,
			dryRun: true,
			env: { SKC_SESSION_ID: TEST_SESSION_ID, PATH: "" },
		});
	...
	it("separates managed worker lifecycle from worker-reported status", async () => {
		...
		const initialStatus = (await executeSkcTeamApiOperation(
			...
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		)) as { state: string };
		...
		let snapshot = await readSkcTeamSnapshot("worker-lifecycle-team", cleanupRoot, {
			PATH: "",
		...
		snapshot = await readSkcTeamSnapshot("worker-lifecycle-team", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("persists the active worker command so tmux workers use the same skc entrypoint", async () => {
		...
		const snapshot = await startSkcTeam({
			...
			env: {
				SKC_SESSION_ID: TEST_SESSION_ID,
				PATH: "",
				SKC_TEAM_WORKER_COMMAND: "bun ./packages/coding-agent/src/cli.ts",
			},
		});
	...
	it("builds PowerShell worker commands with an invocation operator", () => {
		const config = {
			...
			max_workers: 1,
			state_root: "C:\\state",
			worker_command: "'C:\\Program Files\\skc\\skc.exe'",
			worker_cli_plan: ["skc"],
			tmux_command: "psmux",
		...
		const worker = {
			...
			assigned_tasks: [],
		} satisfies SkcTeamWorker;

		const command = buildWorkerCommand(config, worker, "win32");

		expect(command).toContain("; & 'C:\\Program Files\\skc\\skc.exe'");
		expect(command).not.toContain("; 'C:\\Program Files\\skc\\skc.exe' ");
		expect(command).toContain("'You are worker-1 in skc team win-team.");
	});

	it("resolves Windows JavaScript entrypoints through an executable runtime", async () => {
		const command = resolveSkcWorkerCommand(
			"C:\\repo",
			{},
			"win32",
			["node", "C:\\repo\\node_modules\\@sayknow-cli\\coding-agent\\bin\\skc.js"],
	...
	it("keeps worker CLI selection limited to SKC teammate sessions", async () => {
		...
		const snapshot = await startSkcTeam({
			...
			cwd: cleanupRoot,
			dryRun: true,
			env: { SKC_SESSION_ID: TEST_SESSION_ID, PATH: "", SKC_TEAM_WORKER_CLI_MAP: "skc,auto" },
		});
		const config = await Bun.file(path.join(snapshot.state_dir, "config.json")).json();
		const manifest = await Bun.file(path.join(snapshot.state_dir, "manifest.v2.json")).json();
		const telemetry = await Bun.file(path.join(snapshot.state_dir, "telemetry.jsonl")).text();
		...
		for (const provider of ["codex", "claude", "gemini"]) {
			...
			await expect(
				startSkcTeam({
					...
					cwd: cleanupRoot,
					dryRun: true,
					env: { SKC_SESSION_ID: TEST_SESSION_ID, PATH: "", SKC_TEAM_WORKER_CLI: provider },
				}),
	...
	it("distributes explicit markdown lane sections into worker-owned initial tasks", async () => {
		...
		const task1 = await readSkcTeamTask("lane-distribution-team", "task-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		const task2 = await readSkcTeamTask("lane-distribution-team", "task-2", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("fails outside current tmux before creating team state or worktrees", async () => {
		...
		).rejects.toThrow(/skc_team_requires_tmux_leader: start a tmux session first/);
		...
		await expect(
			startSkcTeam({
				...
				env: {
					...
					PATH: process.env.PATH ?? "",
					SKC_TEAM_WORKER_COMMAND: "true",
					SKC_TEAM_TMUX_COMMAND: "skc-nonexistent-tmux-binary-xyz",
				},
			}),
		).rejects.toThrow(/skc_team_requires_tmux_leader:.*tmux_not_installed/);
	...
	it("fails with not_inside_tmux guidance when run outside any tmux session", async () => {
		...
		).rejects.toThrow(/skc_team_requires_tmux_leader:.*not_inside_tmux/);
	...
	it("rejects a tmux provider that cannot persist SKC's ownership tag (e.g. psmux)", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot, { skcProfile: false, untaggableProfile: true });
	...
	it("adopts a user-created tmux leader by tagging it even without SKC_TMUX_LAUNCHED", async () => {
		cleanupRoot = await createGitRepo();
		const fakeTmux = await createFakeTmuxBin(cleanupRoot, { skcProfile: false });
		...
		expect(snapshot.tmux_target).toBe("test-session:0");
		const tmuxLog = await Bun.file(path.join(cleanupRoot, "tmux.log")).text();
		// A real tmux session the user created (no SKC_TMUX_LAUNCHED) is adopted
		// by writing and reading back SKC's ownership tag, then split into workers.
		expect(tmuxLog).toContain("set-option -t =test-session: @skc-profile 1");
		expect(tmuxLog).toContain("split-window");
	});
	...
	it("removes clean created worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownSkcTeam("cleanup-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("preserves dirty worker worktrees on normal shutdown", async () => {
		...
		const stopped = await shutdownSkcTeam("dirty-cleanup-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("supports task claim, transition, list, status, and shutdown lifecycle operations", async () => {
		...
		await expect(
			transitionSkcTeamTask("life-team", "task-1", "completed", cleanupRoot, {
				PATH: "",
				SKC_SESSION_ID: TEST_SESSION_ID,
			}),
		).rejects.toThrow("claim_token_required:task-1");

		const claim = await claimSkcTeamTask("life-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		...
		const reclaim = await claimSkcTeamTask(
			"life-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const status = await readSkcTeamSnapshot("life-team", cleanupRoot, { PATH: "", SKC_SESSION_ID: TEST_SESSION_ID });
		expect(status.task_counts.completed).toBe(1);
		expect(await listSkcTeams(cleanupRoot, { PATH: "", SKC_SESSION_ID: TEST_SESSION_ID })).toHaveLength(1);

		const stopped = await shutdownSkcTeam("life-team", cleanupRoot, { PATH: "", SKC_SESSION_ID: TEST_SESSION_ID });
	...
	it("writes versioned structured traces linked to legacy events", async () => {
		...
		const claim = await claimSkcTeamTask("trace-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(claim.ok).toBe(true);
	...
	it("stores structured completion evidence in task listings and honors claim tokens without implicit worker defaults", async () => {
		...
		const workerTwoClaim = await claimSkcTeamTask(
			"evidence-team",
			"worker-2",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const completedTask = await readSkcTeamTask("evidence-team", "task-2", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		...
		const workerOneClaim = await claimSkcTeamTask(
			"evidence-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
	...
	it("rejects completed transitions without valid evidence and leaves task state unchanged", async () => {
		...
		const claim = await claimSkcTeamTask("invalid-evidence-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(claim.ok).toBe(true);
		const taskBefore = await readSkcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		...
		const taskAfter = await readSkcTeamTask("invalid-evidence-team", "task-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("allows non-command completion evidence and requires evidence-backed shutdown completion", async () => {
		...
		const stopped = await shutdownSkcTeam("review-evidence-team", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(stopped.phase).toBe("complete");
	...
	it("treats legacy evidence-free completed tasks as failed on shutdown", async () => {
		...
		const task = await readSkcTeamTask("legacy-completed-team", "task-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		await Bun.write(
		...
		const stopped = await shutdownSkcTeam("legacy-completed-team", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("recovers expired task claims before new claim attempts", async () => {
		...
		const claim = await claimSkcTeamTask("expired-claim-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(claim.ok).toBe(true);
		const claimedTask = await readSkcTeamTask("expired-claim-team", "task-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		...
		const recoveredClaim = await claimSkcTeamTask("expired-claim-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("recovers stale heartbeat claims during monitor and blocks stale workers from reclaiming", async () => {
		...
		const claim = await claimSkcTeamTask("stale-heartbeat-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(claim.ok).toBe(true);
	...
	it("keeps read-only status separate from mutating monitor recovery", async () => {
		...
		const claim = await claimSkcTeamTask("status-semantics-team", "worker-1", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(claim.ok).toBe(true);
	...
	it("enforces typed lane claim eligibility before leases are granted", async () => {
		...
		const wrongOwner = await claimSkcTeamTask(
			"lane-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const blockedByDependency = await claimSkcTeamTask(
			"lane-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const wrongRole = await claimSkcTeamTask(
			"lane-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const implementationClaim = await claimSkcTeamTask(
			"lane-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
		...
		const verificationClaim = await claimSkcTeamTask(
			"lane-team",
			"worker-1",
			cleanupRoot,
			{ PATH: "", SKC_SESSION_ID: TEST_SESSION_ID },
	...
	it("allows only one worker to claim a task under concurrent claim attempts", async () => {
		...
		const status = await readSkcTeamSnapshot("claim-race-team", cleanupRoot, {
			PATH: "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("checkpoint classification excludes SKC runtime paths from worker auto-commits", async () => {
		const protectedTeamPath = `.skc/_session-${TEST_SESSION_ID}/state/team/demo/worker.json`;
		const protectedReportPath = `.skc/_session-${TEST_SESSION_ID}/reports/team-commit-hygiene/demo.ledger.json`;
		expect(classifySkcTeamCheckpointFiles(["src/feature.ts", protectedTeamPath, protectedReportPath])).toEqual({
	...
	it("reports awaiting integration when all worker tasks completed after an integration request", async () => {
		...
		const status = await readSkcTeamSnapshot("awaiting-request-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(status.task_counts.completed).toBe(1);
		...
		const stopped = await shutdownSkcTeam("awaiting-request-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
	...
	it("keeps completed conflicting teams in awaiting integration instead of plain running", async () => {
		...
		const stopped = await shutdownSkcTeam("awaiting-conflict-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
		expect(stopped.task_counts.completed).toBe(1);
	...
	it("pure team reads, status, and list operations stay read-only while monitor and resume can mutate", async () => {
		...
		const listed = await listSkcTeams(cleanupRoot, { PATH: process.env.PATH ?? "", SKC_SESSION_ID: TEST_SESSION_ID });
		const read = await readSkcTeamSnapshot("pure-read-team", cleanupRoot, {
			PATH: process.env.PATH ?? "",
			SKC_SESSION_ID: TEST_SESSION_ID,
		});
