...
} from "@sayknow-cli/coding-agent/modes/shared/agent-wire/handshake";
import { UiRequestBroker } from "@sayknow-cli/coding-agent/modes/shared/agent-wire/ui-request-broker";
...
describe("live bridge unattended workflow gate wiring", () => {
	...
	it("negotiates unattended during handshake and resolves gates through bridge ui-response compatibility route", async () => {
		...
		const claim = await handle(
			authRequest("/v1/sessions/sess-b/control:claim", undefined, { "X-SKC-Bridge-Owner-Token": ownerToken }),
		);
