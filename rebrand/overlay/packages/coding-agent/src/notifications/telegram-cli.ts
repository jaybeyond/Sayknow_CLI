...
function printHelpAndExit(): never {
	process.stdout.write(
		[
			...
			"  --session-id <id>       Resolve <repo>/.skc/state/notifications/<id>.json",
...
function findLatestEndpoint(repo: string): string | undefined {
	const dir = path.join(repo, ".skc", "state", "notifications");
	let entries: string[];
	try {
...
async function main(): Promise<void> {
	...
	const endpointFile =
		args.endpointFile ??
		(args.sessionId ? path.join(args.repo, ".skc", "state", "notifications", `${args.sessionId}.json`) : undefined) ??
		findLatestEndpoint(args.repo);
	if (!endpointFile || !fs.existsSync(endpointFile)) {
		process.stderr.write(
			`error: no endpoint file found (looked under ${args.repo}/.skc/state/notifications). Start a session with SKC_NOTIFICATIONS=1 first.\n`,
		);
		process.exit(2);
	}
