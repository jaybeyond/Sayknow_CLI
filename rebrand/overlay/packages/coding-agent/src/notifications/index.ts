 ...
 * N-API) and bridges SKC session events + the `ask` tool to it so a remote client
 ...
 * root directory so linked worktrees report the repo (e.g. `sayknow-cli`)
...
function resolveToken(): string {
	return process.env.SKC_NOTIFICATIONS_TOKEN ?? crypto.randomBytes(24).toString("base64url");
}
...
export function createNotificationsExtension(api: ExtensionAPI, options: { settings?: Settings } = {}): void {
	...
	async function startSession(ctx: ExtensionContext): Promise<"started" | "already" | "disabled" | "failed"> {
		...
		try {
			...
			// structurally incompatible optional-method signatures; at runtime BrokerWorkflowGateEmitter
			// satisfies both, so the listener is registered with a widened signature.
			activeRuntime.disposeGateEmitterListener = registerWorkflowGateEmitterListener(
				id,
				attachWorkflowGate as never,
			);
			if (ctx.workflowGate) attachWorkflowGate(ctx.workflowGate as never);
			return "started";
		} catch (e) {
		...
		async handler(args: string, ctx: ExtensionCommandContext): Promise<void> {
			...
			if (command === "on") {
				...
				if (!enabledWithoutLocalOff) {
					ctx.ui.notify(
						"Notifications are not configured. Run `skc notify setup` or set SKC_NOTIFICATIONS=1.",
						"warning",
					);
					return;
				}
