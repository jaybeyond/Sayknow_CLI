...
import {
	AgentWireFrameSequencer as BridgeFrameSequencer,
	toAgentWireEventFrame as toBridgeEventFrame,
	toAgentWireWorkflowGateFrame as toBridgeWorkflowGateFrame,
...
function parseBridgeScopes(value: string | undefined): readonly BridgeCommandScope[] {
	...
	for (const raw of value.split(",")) {
		...
		if (!scope) continue;
		if (!allowed.has(scope as BridgeCommandScope)) throw new Error(`Invalid SKC_BRIDGE_SCOPES entry: ${scope}`);
		scopes.add(scope as BridgeCommandScope);
	}
	return [...scopes];
}

// Opt-in endpoint enablement via SKC_BRIDGE_ENDPOINTS (default undefined -> fail closed, backward compatible).
...
export function parseBridgeEndpoints(value: string | undefined): Partial<BridgeEndpointMatrix> | undefined {
	...
	for (const raw of value.split(",")) {
		const key = raw.trim();
		if (!key) continue;
		if (!allowed.has(key)) throw new Error(`Invalid SKC_BRIDGE_ENDPOINTS entry: ${key}`);
...
export function createBridgeFetchHandler(options: BridgeFetchHandlerOptions): (request: Request) => Promise<Response> {
	return async request => {
		...
		if (request.method === "POST" && url.pathname === `/v1/sessions/${options.sessionId}/control:claim`) {
			...
			const ownerToken = request.headers.get("X-SKC-Bridge-Owner-Token") ?? crypto.randomUUID();
		...
		if (request.method === "POST" && url.pathname === `/v1/sessions/${options.sessionId}/control:disconnect`) {
			...
			if (!hasScope(options.commandScopes, "control"))
				return jsonResponse(403, { error: "scope_denied", scope: "control" });
			const ownerToken = request.headers.get("X-SKC-Bridge-Owner-Token") ?? "";
...
export async function runBridgeMode(
	...
	const token = Bun.env.SKC_BRIDGE_TOKEN;
	if (!token) {
		throw new Error("SKC_BRIDGE_TOKEN is required for --mode bridge");
	}
	const hostname = Bun.env.SKC_BRIDGE_HOST ?? DEFAULT_BRIDGE_HOST;
	const port = Bun.env.SKC_BRIDGE_PORT ? Number.parseInt(Bun.env.SKC_BRIDGE_PORT, 10) : DEFAULT_BRIDGE_PORT;
	if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
		throw new Error(`Invalid SKC_BRIDGE_PORT: ${Bun.env.SKC_BRIDGE_PORT}`);
	}
	const commandScopes = parseBridgeScopes(Bun.env.SKC_BRIDGE_SCOPES);
	const endpointMatrix = parseBridgeEndpoints(Bun.env.SKC_BRIDGE_ENDPOINTS);

	const certPath = Bun.env.SKC_BRIDGE_TLS_CERT;
	const keyPath = Bun.env.SKC_BRIDGE_TLS_KEY;
	...
	// Upstream v0.11 split WorkflowGateEmitter into broker/unattended-session variants with
	// structurally incompatible optional-method signatures; the runtime surface is identical.
	session.setWorkflowGateEmitter(unattendedControlPlane as never);
