...
import { $pickenv, isKnownSinkPeerClosedError, logger, readLines, Snowflake } from "@sayknow-cli/utils";
import type {
	ExtensionUIContext,
...
export function shouldEmitRpcTitlesForTest(): boolean {
	const raw = $pickenv("SKC_RPC_EMIT_TITLE", "PI_RPC_EMIT_TITLE");
	if (!raw) return false;
...
export async function runRpcMode(
	...
	class RpcExtensionUIContext implements ExtensionUIContext {
		...
		setTitle(title: string): void {
			// Title updates are low-value noise for most RPC hosts; opt in via SKC_RPC_EMIT_TITLE=1.
