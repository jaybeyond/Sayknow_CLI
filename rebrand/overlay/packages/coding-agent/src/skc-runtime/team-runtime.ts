...
import {
	...
	buildSkcTmuxUntaggedSessionHint,
	resolveSkcTmuxCommand,
	SKC_TMUX_PROFILE_OPTION,
	SKC_TMUX_PROFILE_VALUE,
} from "./tmux-common";

export type SkcTeamPhase = "starting" | "running" | "awaiting_integration" | "complete" | "failed" | "cancelled";
export type SkcTeamTaskStatus = "pending" | "blocked" | "in_progress" | "completed" | "failed";
export type SkcWorkerStatusState = "idle" | "working" | "blocked" | "done" | "failed" | "draining" | "unknown";
export type SkcTeamWorkerLifecycleState =
	| "starting"
	| "ready"
	| "working"
	| "draining"
	...
	| "failed"
	| "unknown";
export type SkcTeamShutdownMode = "graceful" | "force" | "abort";

export const SKC_TEAM_DEFAULT_WORKERS = 3;
export const SKC_TEAM_MAX_WORKERS = 20;
const SKC_TEAM_WORKER_CLI_ENV = "SKC_TEAM_WORKER_CLI";
const SKC_TEAM_WORKER_CLI_MAP_ENV = "SKC_TEAM_WORKER_CLI_MAP";

export type SkcTeamWorkerCli = "skc";
type SkcTeamWorkerCliMode = "auto" | SkcTeamWorkerCli;

export interface SkcTeamLeader {
	session_id: string;
	pane_id: string;
	cwd: string;
}

export interface SkcTeamWorker {
	id: string;
	name: string;
	index: number;
	agent_type: string;
...
export interface SkcTeamTaskClaim {
	owner: string;
	token: string;
	leased_until: string;
}
export type SkcTeamTaskCompletionEvidenceKind = "command" | "inspection" | "artifact";
export type SkcTeamTaskCompletionEvidenceStatus = "passed" | "failed" | "not_run" | "verified" | "rejected";

export interface SkcTeamTaskCompletionEvidenceItem {
	kind: SkcTeamTaskCompletionEvidenceKind;
	status: SkcTeamTaskCompletionEvidenceStatus;
	summary: string;
	command?: string;
	artifact?: string;
	location?: string;
...
export interface SkcTeamTaskCompletionEvidence {
	summary: string;
	items: SkcTeamTaskCompletionEvidenceItem[];
	files?: string[];
	notes?: string;
	recorded_by: string;
	recorded_at: string;
...
export interface SkcTeamTask {
	id: string;
	subject: string;
	description: string;
	title: string;
	objective: string;
	status: SkcTeamTaskStatus;
	assignee?: string;
	owner?: string;
	result?: string;
	completion_evidence?: SkcTeamTaskCompletionEvidence;
	error?: string;
	blocked_by?: string[];
	depends_on?: string[];
	lane?: string;
...
export type SkcTeamWorktreeMode =
	| { enabled: false }
...
export interface SkcTeamConfig {
	...
	worker_cli_plan: SkcTeamWorkerCli[];
	tmux_command: string;
	tmux_session: string;
...
export type SkcTeamIntegrationStatus =
	| "idle"
	| "integrated"
	| "integration_failed"
	| "merge_conflict"
...
export interface SkcTeamWorkerIntegrationState {
...
export interface SkcTeamMonitorSnapshot {
	integration_by_worker: Record<string, SkcTeamWorkerIntegrationState>;
	updated_at: string;
}
export interface SkcTeamWorkerLifecycle {
	worker: string;
	lifecycle_state: SkcTeamWorkerLifecycleState;
	worker_status_state: SkcWorkerStatusState;
	pane_id?: string;
	pid?: number;
	started_at?: string;
	updated_at: string;
	...
	shutdown_mode?: SkcTeamShutdownMode;
}

export type SkcTeamNotificationDeliveryState =
	| "pending"
	| "sent"
	| "queued"
	| "deferred"
...
export type SkcTeamPaneAttemptResult = "sent" | "queued" | "deferred" | "failed";

export interface SkcTeamNotification {
	id: string;
	...
	delivery_state: SkcTeamNotificationDeliveryState;
	pane_attempt_result?: SkcTeamPaneAttemptResult;
...
export interface SkcTeamNotificationSummary {
	total: number;
	replay_eligible: number;
	by_state: Record<SkcTeamNotificationDeliveryState, number>;
}

export interface SkcTeamSnapshot {
	team_name: string;
	display_name: string;
	phase: SkcTeamPhase;
	state_dir: string;
	...
	tmux_target: string;
	task_total: number;
	task_counts: Record<SkcTeamTaskStatus, number>;
	workers: SkcTeamWorker[];
	integration_by_worker?: Record<string, SkcTeamWorkerIntegrationState>;
	worker_lifecycle_by_id: Record<string, SkcTeamWorkerLifecycle>;
	notification_summary: SkcTeamNotificationSummary;
	updated_at: string;
}
export interface SkcTeamSnapshotOptions {
	reconcileNotifications?: boolean;
}

export interface SkcTeamStartOptions {
	workerCount: number;
	agentType: string;
	task: string;
	teamName?: string;
	worktreeMode?: SkcTeamWorktreeMode;
	cwd?: string;
...
export type SkcTeamLivenessRecoveryReason =
	| "claim_expired"
	| "stale_heartbeat"
	| "missing_pane"
...
export interface SkcTeamRecoveredClaim {
	task_id: string;
	worker: string;
	reasons: SkcTeamLivenessRecoveryReason[];
}

export interface SkcTeamLivenessRecoveryResult {
	recovered_claims: SkcTeamRecoveredClaim[];
	stale_workers: Record<string, SkcTeamLivenessRecoveryReason[]>;
}

export interface SkcTeamMailboxMessage {
	message_id: string;
	from_worker: string;
	to_worker: string;
	body: string;
...
function taskReceiptFields(teamName: string, task: SkcTeamTask): Record<string, unknown> {
	return {
		team_name: teamName,
		task_id: task.id,
		status: task.status,
...
function mailboxMessageReceiptFields(teamName: string, message: SkcTeamMailboxMessage): Record<string, unknown> {
	return {
		team_name: teamName,
...
function notificationReceiptFields(notification: SkcTeamNotification): Record<string, unknown> {
	return {
...
function notificationSummaryReceipt(
	teamName: string,
	result: { notifications: SkcTeamNotification[]; summary: SkcTeamNotificationSummary },
...
function normalizeSkcTeamWorkerCliMode(
	raw: string | undefined,
	sourceEnv = SKC_TEAM_WORKER_CLI_ENV,
): SkcTeamWorkerCliMode {
	const normalized = String(raw ?? "auto")
		.trim()
		.toLowerCase();
	if (normalized === "" || normalized === "auto") return "auto";
	if (normalized === "skc") return "skc";
	if (normalized === "codex" || normalized === "claude" || normalized === "gemini") {
		throw new Error(`Unsupported ${sourceEnv} value "${raw}". SKC team launches SKC teammate sessions only.`);
	}
	throw new Error(`Invalid ${sourceEnv} value "${raw}". Expected: auto or skc`);
}

export function resolveSkcTeamWorkerCli(env: NodeJS.ProcessEnv = process.env): SkcTeamWorkerCli {
	const mode = normalizeSkcTeamWorkerCliMode(env[SKC_TEAM_WORKER_CLI_ENV]);
	return mode === "auto" ? "skc" : mode;
}

export function resolveSkcTeamWorkerCliPlan(
	workerCount: number,
	...
	normalizeSkcTeamWorkerCliMode(env[SKC_TEAM_WORKER_CLI_ENV]);
	const rawMap = String(env[SKC_TEAM_WORKER_CLI_MAP_ENV] ?? "").trim();
	if (rawMap === "") {
		const cli = resolveSkcTeamWorkerCli(env);
	...
	if (entries.length === 0 || entries.every(entry => entry.length === 0)) {
		throw new Error(
			`Invalid ${SKC_TEAM_WORKER_CLI_MAP_ENV} value "${env[SKC_TEAM_WORKER_CLI_MAP_ENV]}". Expected: auto or skc`,
		);
	}
	if (entries.some(entry => entry.length === 0)) {
		throw new Error(
			`Invalid ${SKC_TEAM_WORKER_CLI_MAP_ENV} value "${env[SKC_TEAM_WORKER_CLI_MAP_ENV]}". Empty entries are not allowed.`,
		);
	}
	if (entries.length !== 1 && entries.length !== workerCount) {
		throw new Error(
			`Invalid ${SKC_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; expected 1 or ${workerCount} comma-separated values.`,
		);
	}
	const expanded = entries.length === 1 ? Array.from({ length: workerCount }, () => entries[0] ?? "") : entries;
	return expanded.map(entry => {
		const mode = normalizeSkcTeamWorkerCliMode(entry, SKC_TEAM_WORKER_CLI_MAP_ENV);
		return mode === "auto" ? "skc" : mode;
	});
}

export function translateSkcWorkerLaunchArgsForCli(workerCli: SkcTeamWorkerCli, args: string[]): string[] {
	if (workerCli !== "skc") {
		throw new Error(`Unsupported team worker CLI "${workerCli}". SKC team launches SKC teammate sessions only.`);
	}
	return [...args];
}

interface SkcTmuxLeaderContext {
	sessionName: string;
	windowIndex: string;
	leaderPaneId: string;
	target: string;
}
export interface SkcTeamEvent {
	event_id: string;
	ts: string;
	type: string;
	worker?: string;
...
export interface SkcTeamTraceEvent {
	schema_version: 1;
	trace_id: string;
	span_id: string;
...
interface SkcTeamCommitHygieneEntry {
	recorded_at: string;
	operation:
		| "auto_checkpoint"
...
interface SkcWorkerIntegrationDedupeState {
	last_requested_fingerprint?: string;
	last_requested_head?: string | null;
	last_requested_status?: SkcWorkerCheckpointClassification["kind"];
...
function parseSkcTeamTaskStatus(value: unknown, allowLegacyComplete = false): SkcTeamTaskStatus {
	const raw = typeof value === "string" ? value.trim() : "";
	if (allowLegacyComplete && raw === "complete") return "completed";
	if (isSkcTeamTaskStatus(raw)) return raw;
	throw new Error(`invalid_task_status:${raw}`);
}

export const SKC_TEAM_API_OPERATIONS = [
	"send-message",
	"broadcast",
	"mailbox-list",
...
function makeTeamName(task: string, env: NodeJS.ProcessEnv): string {
	const basis = [task, env.SKC_SESSION_ID, env.CODEX_SESSION_ID, env.TMUX_PANE, env.TMUX, now()]
		.filter(Boolean)
		.join(":");
...
function assertKnownWorker(config: SkcTeamConfig, worker: string, allowLeader = false): void {
...
function findKnownWorker(config: SkcTeamConfig, worker: string): SkcTeamWorker {
...
function assertKnownParticipant(config: SkcTeamConfig, worker: string): void {
...
function collectTeamGcWorkerPids(
	...
	lifecycle: SkcTeamWorkerLifecycle | null,
): number[] {
	const pids: number[] = [];
...
function teamGcRecordDetail(heartbeat: WorkerHeartbeatFile | null, lifecycle: SkcTeamWorkerLifecycle | null): string {
	return [
...
export async function listTeamWorkerGcRecords(teamRoot: string, probe: GcPidProbe): Promise<GcRecord[]> {
	...
	for (const teamEntry of teamEntries) {
		...
		for (const workerEntry of workerEntries) {
			...
			let lifecycle: SkcTeamWorkerLifecycle | null = null;
			try {
				heartbeat = await readJsonFile<WorkerHeartbeatFile>(path.join(dir, "heartbeat.json"));
				lifecycle = await readJsonFile<SkcTeamWorkerLifecycle>(path.join(dir, "lifecycle.json"));
			} catch (error) {
				records.push({
					store: "team_workers",
...
export async function pruneTeamWorkerGcRecord(record: GcRecord, probe: GcPidProbe): Promise<boolean> {
	...
	const lifecycle = await readJsonFile<SkcTeamWorkerLifecycle>(path.join(record.path, "lifecycle.json"));
	const pids = collectTeamGcWorkerPids(heartbeat, lifecycle);
...
function evidenceRefsForEvent(event: SkcTeamEvent): string[] | undefined {
	const refs: string[] = [];
...
function traceDataForEvent(event: SkcTeamEvent): Record<string, unknown> | undefined {
...
async function appendTraceForEvent(dir: string, event: SkcTeamEvent): Promise<void> {
	const evidenceRefs = evidenceRefsForEvent(event);
	const traceData = traceDataForEvent(event);
	const trace: SkcTeamTraceEvent = {
		schema_version: 1,
...
async function appendEvent(dir: string, event: Omit<SkcTeamEvent, "ts" | "event_id">): Promise<SkcTeamEvent> {
...
async function readConfig(dir: string): Promise<SkcTeamConfig> {
	const config = await readJsonFile<SkcTeamConfig>(path.join(dir, "config.json"));
	...
	return {
		...config,
		max_workers: config.max_workers ?? SKC_TEAM_MAX_WORKERS,
		tmux_command: config.tmux_command ?? resolveSkcTmuxCommand(),
		...
		worker_cli_plan: config.worker_cli_plan ?? Array.from({ length: config.worker_count }, () => "skc"),
	};
}
async function readPhase(dir: string): Promise<SkcTeamPhase> {
	const phase = await readJsonFile<{ current_phase?: SkcTeamPhase }>(path.join(dir, "phase.json"));
	return phase?.current_phase ?? "running";
}
async function writePhase(dir: string, phase: SkcTeamPhase): Promise<void> {
	await writeJsonFile(path.join(dir, "phase.json"), { current_phase: phase, updated_at: now() });
}
function isSkcWorkerStatusState(value: string): value is SkcWorkerStatusState {
	return ["idle", "working", "blocked", "done", "failed", "draining", "unknown"].includes(value);
}

function parseSkcWorkerStatusState(value: unknown): SkcWorkerStatusState {
	return typeof value === "string" && isSkcWorkerStatusState(value) ? value : "unknown";
}
function parseRequiredSkcWorkerStatusState(value: unknown): SkcWorkerStatusState {
	const raw = typeof value === "string" ? value.trim() : "";
	if (isSkcWorkerStatusState(raw)) return raw;
	throw new Error(`invalid_worker_status:${raw}`);
}

function lifecycleStateForWorkerStatus(status: SkcWorkerStatusState): SkcTeamWorkerLifecycleState {
	switch (status) {
		case "working":
			return "working";
		case "draining":
...
function parseSkcTeamShutdownMode(value: unknown): SkcTeamShutdownMode {
...
function isSkcTeamWorkerLifecycleState(value: string): value is SkcTeamWorkerLifecycleState {
	return ["starting", "ready", "working", "draining", "stopped", "failed", "unknown"].includes(value);
}

function parseSkcTeamWorkerLifecycleState(value: unknown): SkcTeamWorkerLifecycleState {
	return typeof value === "string" && isSkcTeamWorkerLifecycleState(value) ? value : "unknown";
}
...
async function readWorkerLifecycleRecord(dir: string, worker: SkcTeamWorker): Promise<SkcTeamWorkerLifecycle> {
	...
	const rawLifecycle = await readJsonFile<Partial<SkcTeamWorkerLifecycle>>(workerLifecyclePath(dir, worker.id));
	...
	const lifecycle: SkcTeamWorkerLifecycle = {
		worker: worker.id,
		lifecycle_state: parseSkcTeamWorkerLifecycleState(rawLifecycle?.lifecycle_state),
...
function appendLivenessRecoveryReason(
	reasons: SkcTeamLivenessRecoveryReason[],
	reason: SkcTeamLivenessRecoveryReason,
): void {
...
function readClaimRecord(value: unknown): SkcTeamTaskClaim | undefined {
...
function isWorkerHeartbeatStale(
	...
	const thresholdMs = parseDurationEnv(env, "SKC_TEAM_HEARTBEAT_STALE_MS", 120_000);
...
async function detectSkcTeamWorkerLivenessReasons(
	dir: string,
	config: SkcTeamConfig,
	worker: SkcTeamWorker,
	env: NodeJS.ProcessEnv,
...
async function reconcileSkcTeamStaleClaims(
	teamName: string,
	dir: string,
	config: SkcTeamConfig,
	env: NodeJS.ProcessEnv,
	...
	for (const worker of config.workers) {
		const reasons = await detectSkcTeamWorkerLivenessReasons(dir, config, worker, env);
...
export async function recoverSkcTeamStaleClaims(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamLivenessRecoveryResult> {
	...
	return reconcileSkcTeamStaleClaims(teamName, dir, config, env);
}
...
type SkcTeamTaskMetadataInput = Partial<
	Pick<SkcTeamTask, "owner" | "lane" | "required_role" | "allowed_roles" | "depends_on" | "blocked_by">
>;

function taskMetadataFromInput(input: Record<string, unknown>, includeOwner = false): SkcTeamTaskMetadataInput {
	const metadata: SkcTeamTaskMetadataInput = {};
...
function normalizeTask(raw: SkcTeamTask): SkcTeamTask {
	const status = raw.status === ("complete" as SkcTeamTaskStatus) ? "completed" : raw.status;
	return {
		...raw,
		status,
...
const SKC_TEAM_INTEGRATION_ATTENTION_STATUSES = new Set<SkcTeamIntegrationStatus>([
	"integration_failed",
	"merge_conflict",
	"cherry_pick_conflict",
	"rebase_conflict",
]);
const SKC_TEAM_INTEGRATION_SETTLED_STATUSES = new Set<SkcTeamIntegrationStatus>(["idle", "integrated"]);

async function hasPendingSkcTeamIntegration(
	dir: string,
	config: SkcTeamConfig,
	monitor: SkcTeamMonitorSnapshot | null,
): Promise<boolean> {
	for (const worker of config.workers) {
		const integration = monitor?.integration_by_worker?.[worker.id];
		if (integration?.status && SKC_TEAM_INTEGRATION_ATTENTION_STATUSES.has(integration.status)) return true;

		const request = await readJsonFile<SkcWorkerIntegrationDedupeState>(workerIntegrationDedupePath(dir, worker.id));
		if (!request?.last_requested_at) continue;
		if (!integration?.status || !integration.updated_at) return true;
		if (SKC_TEAM_INTEGRATION_ATTENTION_STATUSES.has(integration.status)) return true;
		if (
			SKC_TEAM_INTEGRATION_SETTLED_STATUSES.has(integration.status) &&
...
async function resolveSkcTeamSnapshotPhase(
	dir: string,
	config: SkcTeamConfig,
	storedPhase: SkcTeamPhase,
	tasks: SkcTeamTask[],
	monitor: SkcTeamMonitorSnapshot | null,
): Promise<SkcTeamPhase> {
	if (storedPhase !== "running") return storedPhase;
	if (tasks.length === 0 || !tasks.every(isSkcTeamTaskCompletionVerified)) return storedPhase;
	return (await hasPendingSkcTeamIntegration(dir, config, monitor)) ? "awaiting_integration" : storedPhase;
}

function isRecord(value: unknown): value is Record<string, unknown> {
	return typeof value === "object" && value != null;
}
const SKC_TEAM_COMPLETION_EVIDENCE_SUMMARY_MAX = 4_000;
const SKC_TEAM_COMPLETION_EVIDENCE_OUTPUT_MAX = 8_000;
const SKC_TEAM_COMMAND_EVIDENCE_STATUSES = new Set<SkcTeamTaskCompletionEvidenceStatus>([
	"passed",
	"failed",
	"not_run",
]);
const SKC_TEAM_VERIFICATION_EVIDENCE_STATUSES = new Set<SkcTeamTaskCompletionEvidenceStatus>(["verified", "rejected"]);
...
function trimRequiredCompletionEvidenceString(
	...
	field: string,
	value: unknown,
	maxLength = SKC_TEAM_COMPLETION_EVIDENCE_SUMMARY_MAX,
): string {
...
function trimOptionalCompletionEvidenceString(
	...
	field: string,
	value: unknown,
	maxLength = SKC_TEAM_COMPLETION_EVIDENCE_OUTPUT_MAX,
): string | undefined {
...
function normalizeSkcTeamCompletionEvidenceStatus(
	taskId: string,
	kind: SkcTeamTaskCompletionEvidenceKind,
	value: unknown,
): SkcTeamTaskCompletionEvidenceStatus {
	const status = trimRequiredCompletionEvidenceString(taskId, "items.status", value);
	const allowed = kind === "command" ? SKC_TEAM_COMMAND_EVIDENCE_STATUSES : SKC_TEAM_VERIFICATION_EVIDENCE_STATUSES;
	if (!allowed.has(status as SkcTeamTaskCompletionEvidenceStatus))
		throw completionEvidenceError(taskId, "items.status");
	return status as SkcTeamTaskCompletionEvidenceStatus;
}

function normalizeSkcTeamCompletionEvidenceItem(taskId: string, value: unknown): SkcTeamTaskCompletionEvidenceItem {
	...
	const status = normalizeSkcTeamCompletionEvidenceStatus(taskId, kind, value.status);
	const item: SkcTeamTaskCompletionEvidenceItem = {
		kind,
		status,
...
function normalizeSkcTeamCompletionEvidenceFiles(taskId: string, value: unknown): string[] | undefined {
	if (value == null) return undefined;
...
function isSkcTeamCompletionEvidenceItemVerified(item: SkcTeamTaskCompletionEvidenceItem): boolean {
	return (
...
function normalizeSkcTeamTaskCompletionEvidence(
	taskId: string,
	owner: string,
	input: unknown,
	recordedAt = now(),
): SkcTeamTaskCompletionEvidence {
	...
	const items = itemsValue.map(item => normalizeSkcTeamCompletionEvidenceItem(taskId, item));
	if (!items.some(isSkcTeamCompletionEvidenceItemVerified))
		throw new Error(`completion_evidence_no_verified_item:${taskId}`);
	const evidence: SkcTeamTaskCompletionEvidence = {
		summary: trimRequiredCompletionEvidenceString(taskId, "summary", input.summary),
		items,
		recorded_by: owner,
		recorded_at: recordedAt,
	};
	const files = normalizeSkcTeamCompletionEvidenceFiles(taskId, input.files);
...
function getSkcTeamTaskCompletionEvidenceFailure(task: SkcTeamTask): string | null {
	...
	try {
		normalizeSkcTeamTaskCompletionEvidence(task.id, evidence.recorded_by.trim(), evidence, evidence.recorded_at);
		return null;
	} catch (error) {
...
function isSkcTeamTaskCompletionVerified(task: SkcTeamTask): boolean {
	return getSkcTeamTaskCompletionEvidenceFailure(task) == null;
}
function roleValuesForWorker(worker: SkcTeamWorker): Set<string> {
	return new Set([worker.role, worker.agent_type].map(value => value.trim()).filter(value => value.length > 0));
}

function getSkcTeamTaskClaimEligibilityReason(
	task: SkcTeamTask,
	worker: SkcTeamWorker,
	tasks: SkcTeamTask[],
): string | null {
	...
	for (const dependencyId of task.depends_on ?? []) {
		...
		if (!dependency || !isSkcTeamTaskCompletionVerified(dependency))
...
async function getActiveClaimReason(dir: string, task: SkcTeamTask): Promise<string | null> {
	const claimPath = path.join(dir, "claims", `${task.id}.json`);
	const diskClaim = readClaimRecord(await readJsonFile<unknown>(claimPath));
	const claim = task.claim ?? diskClaim;
...
function isSkcTeamTaskRecord(value: unknown): value is SkcTeamTask {
	return (
		isRecord(value) &&
		typeof value.id === "string" &&
		typeof value.status === "string" &&
		(isSkcTeamTaskStatus(value.status) || value.status === "complete") &&
		(typeof value.subject === "string" || typeof value.title === "string") &&
		(typeof value.description === "string" || typeof value.objective === "string")
	);
}
function isSkcTeamTaskFile(entry: { isFile(): boolean; name: string }): boolean {
	return entry.isFile() && entry.name.endsWith(".json") && !entry.name.endsWith(".evidence.json");
}

async function readTasks(dir: string): Promise<SkcTeamTask[]> {
	try {
		const entries = await fs.readdir(path.join(dir, "tasks"), { withFileTypes: true });
		const tasks = await Promise.all(
			entries.filter(isSkcTeamTaskFile).map(entry => readJsonFile<unknown>(path.join(dir, "tasks", entry.name))),
		);
		return tasks
			.filter(isSkcTeamTaskRecord)
			.map(normalizeTask)
...
async function writeTask(dir: string, task: SkcTeamTask): Promise<void> {
...
async function findTeamDir(
	...
	const root = resolveSkcTeamStateRoot(cwd, env);
	const exact = teamDir(root, teamName);
	if (await readJsonFile<SkcTeamConfig>(path.join(exact, "config.json"))) return exact;
	const candidates = await listSkcTeams(cwd, env);
...
function buildWorkers(count: number, agentType: string, stateRoot?: string): SkcTeamWorker[] {
...
function parseWorktreeMode(args: string[]): { mode: SkcTeamWorktreeMode; remainingArgs: string[] } {
	let mode: SkcTeamWorktreeMode = { enabled: false };
...
function resolveDefaultWorktreeMode(mode?: SkcTeamWorktreeMode): SkcTeamWorktreeMode {
...
async function ensureWorkerWorktree(
	...
	teamName: string,
	worker: SkcTeamWorker,
	mode: SkcTeamWorktreeMode,
): Promise<SkcTeamWorker> {
...
function readSkcTmuxProfileValue(tmuxCommand: string, sessionName: string): string {
	const result = Bun.spawnSync(
		[tmuxCommand, "show-options", "-qv", "-t", buildSkcTmuxExactOptionTarget(sessionName), SKC_TMUX_PROFILE_OPTION],
		{
			stdout: "pipe",
			stderr: "pipe",
		},
...
function tagTmuxSessionAsSkcLeader(tmuxCommand: string, sessionName: string): boolean {
	const result = Bun.spawnSync(
		[
			...
			"set-option",
			"-t",
			buildSkcTmuxExactOptionTarget(sessionName),
			SKC_TMUX_PROFILE_OPTION,
			SKC_TMUX_PROFILE_VALUE,
		],
		{
...
export function resolveSkcWorkerCommand(
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	...
	const explicit = env.SKC_TEAM_WORKER_COMMAND?.trim();
	...
	if (platform === "win32") {
		...
		if (pathModule.basename(entrypoint).startsWith("skc")) return powershellQuote(resolvedEntrypoint);
		return "skc";
	}
	if (entrypoint.endsWith(".ts")) return `${shellQuote(execPath)} ${shellQuote(resolvedEntrypoint)}`;
	if (path.basename(entrypoint).startsWith("skc")) return shellQuote(path.resolve(cwd, entrypoint));
	return "skc";
}
...
function parseExplicitTeamLanes(task: string): SkcTeamInitialLane[] {
	const lines = task.split(/\r?\n/);
	const lanes: SkcTeamInitialLane[] = [];
...
function buildInitialTasks(task: string, workers: SkcTeamWorker[]): SkcTeamTask[] {
...
async function startTmuxSession(
	...
	try {
		...
		const profileResult = applySkcTmuxProfile({
		...
		await appendTelemetry(dir, {
			...
			message: profileResult.skipped
				? "Skipped SKC scoped tmux profile"
				: "Applied SKC scoped tmux profile to team tmux target",
			data: {
		...
		await appendTelemetry(dir, {
			type: "tmux_started",
			message: "Started skc team worker panes in current tmux window",
...
function paneBelongsToTeamTarget(config: SkcTeamConfig, paneId: string): boolean {
...
function killWorkerPanes(config: SkcTeamConfig): void {
	for (const worker of config.workers)
...
async function rollbackCreatedWorktrees(workers: SkcTeamWorker[]): Promise<void> {
...
async function removeCleanCreatedWorktrees(workers: SkcTeamWorker[]): Promise<void> {
	for (const worker of workers.filter(worker => worker.worktree_created).reverse())
...
function integrationNowState(
	status: SkcTeamIntegrationStatus,
): Pick<SkcTeamWorkerIntegrationState, "status" | "updated_at"> {
...
async function appendCommitHygieneEntries(config: SkcTeamConfig, entries: SkcTeamCommitHygieneEntry[]): Promise<void> {
	if (entries.length === 0) return;
	const ledgerPath = commitHygieneLedgerPath(config);
	const existing = (await readJsonFile<{ version: number; entries: SkcTeamCommitHygieneEntry[] }>(ledgerPath)) ?? {
		version: 1,
		entries: [],
	};
...
export type SkcWorkerCheckpointClassification =
...
export function classifySkcTeamCheckpointFiles(files: string[]): { eligible: string[]; protected: string[] } {
...
export function classifyWorkerCheckpointStatus(cwd: string): SkcWorkerCheckpointClassification {
	...
	const classified = classifySkcTeamCheckpointFiles(files);
...
async function notifyLeader(
	...
	await sendSkcTeamMessage(config.team_name, worker.id, "leader-fixed", body, cwd, env).catch(() => undefined);
}
async function notifyWorker(
	...
	await sendSkcTeamMessage(config.team_name, "leader-fixed", worker.id, body, cwd, env).catch(() => undefined);
}
...
function autoCommitDirtyWorker(worker: SkcTeamWorker): {
	committed: boolean;
	commit: string | null;
	classification: SkcWorkerCheckpointClassification | null;
} {
	...
	const message = `skc(team): auto-checkpoint ${worker.id} [${worker.assigned_tasks[0] ?? "unknown"}]`;
...
function workerMergeRef(worker: SkcTeamWorker, workerHead: string): string {
...
async function integrateSkcWorkerCommits(
	config: SkcTeamConfig,
	dir: string,
	previous: SkcTeamMonitorSnapshot | null,
	cwd: string,
	env: NodeJS.ProcessEnv,
): Promise<Record<string, SkcTeamWorkerIntegrationState>> {
	const integrationByWorker: Record<string, SkcTeamWorkerIntegrationState> = {
		...(previous?.integration_by_worker ?? {}),
	};
	const hygieneEntries: SkcTeamCommitHygieneEntry[] = [];
	...
	for (const worker of config.workers) {
		...
		const state: SkcTeamWorkerIntegrationState = {
		...
		if (isAncestor(worker.worktree_path, leaderHead, workerHead)) {
			...
			const merge = runGitResult(leaderCwd, ["merge", "--no-ff", "-m", `skc(team): merge ${worker.id}`, mergeRef]);
			if (merge.ok) {
	...
	if (cycleLeaderHead && newLeaderHead && cycleLeaderHead !== newLeaderHead) {
		for (const worker of config.workers) {
			...
			const status = await readSkcWorkerStatus(config.team_name, worker.id, cwd, env);
...
async function initializeStateDirs(dir: string, workers: SkcTeamWorker[]): Promise<void> {
	...
	for (const worker of workers) {
		...
		await writeJsonFile(workerLifecyclePath(dir, worker.id), {
			...
			updated_at: now(),
		} satisfies SkcTeamWorkerLifecycle);
...
export async function startSkcTeam(options: SkcTeamStartOptions): Promise<SkcTeamSnapshot> {
	const cwd = options.cwd ?? process.cwd();
	const env = options.env ?? process.env;
	if (!Number.isInteger(options.workerCount) || options.workerCount < 1 || options.workerCount > SKC_TEAM_MAX_WORKERS)
		throw new Error(`invalid_team_worker_count:${options.workerCount}:expected_1_${SKC_TEAM_MAX_WORKERS}`);
	const workerCliPlan = resolveSkcTeamWorkerCliPlan(options.workerCount, env);
	const stateRoot = resolveSkcTeamStateRoot(cwd, env);
	...
	const config: SkcTeamConfig = {
		...
		max_workers: SKC_TEAM_MAX_WORKERS,
		state_root: stateRoot,
		worker_command: resolveSkcWorkerCommand(cwd, env),
		...
		leader: { session_id: env.SKC_SESSION_ID ?? env.CODEX_SESSION_ID ?? "", pane_id: tmuxContext.leaderPaneId, cwd },
		leader_cwd: cwd,
		team_state_root: stateRoot,
		workers,
	...
	await appendEvent(dir, {
		...
		message: options.dryRun
			? "Created native skc team dry-run state without starting tmux workers"
			: "Started native skc team runtime",
		data: {
			worker_count: options.workerCount,
			agent_type: options.agentType,
			workspace_mode: config.workspace_mode,
	...
	await appendTelemetry(dir, {
		type: "team_runtime",
		message: options.dryRun ? "Native skc team dry-run state initialized" : "Native skc team runtime initialized",
		data: {
			state_root: stateRoot,
			worker_command: config.worker_command,
			worker_cli_plan: workerCliPlan,
	...
	return readSkcTeamSnapshot(teamName, cwd, env);
}

export async function readSkcTeamSnapshot(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	options: SkcTeamSnapshotOptions = {},
): Promise<SkcTeamSnapshot> {
	const dir = await findTeamDir(teamName, cwd, env);
	const config = await readConfig(dir);
	const storedPhase = await readPhase(dir);
	const tasks = await readTasks(dir);
	const taskCounts: Record<SkcTeamTaskStatus, number> = {
		pending: 0,
		blocked: 0,
		in_progress: 0,
		completed: 0,
	...
	const monitor = await readJsonFile<SkcTeamMonitorSnapshot>(monitorSnapshotPath(dir));
	...
	const phase = await resolveSkcTeamSnapshotPhase(dir, config, storedPhase, tasks, monitor);
	return {
		team_name: config.team_name,
		display_name: config.display_name,
		phase,
...
export async function monitorSkcTeamSnapshot(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamSnapshot> {
	const snapshot = await monitorSkcTeam(teamName, cwd, env);
	return snapshot;
}
function workerIntegrationFingerprint(head: string | null, classification: SkcWorkerCheckpointClassification): string {
...
export async function requestSkcWorkerIntegrationAttempt(
	...
	const dedupe = (await readJsonFile<SkcWorkerIntegrationDedupeState>(dedupePath)) ?? {};
	...
	} satisfies SkcWorkerIntegrationDedupeState);
	await appendEvent(dir, {
...
export async function buildTeamHudSummary(
	snapshot: SkcTeamSnapshot,
	latestEvent?: SkcTeamEvent,
	latestMessage?: SkcTeamMailboxMessage,
...
export async function monitorSkcTeam(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamSnapshot> {
	const dir = await findTeamDir(teamName, cwd, env);
	const config = await readConfig(dir);
	const previous = await readJsonFile<SkcTeamMonitorSnapshot>(monitorSnapshotPath(dir));
	await reconcileSkcTeamStaleClaims(teamName, dir, config, env);
	const integrationByWorker = await integrateSkcWorkerCommits(config, dir, previous, cwd, env);
	await writeJsonFile(monitorSnapshotPath(dir), { integration_by_worker: integrationByWorker, updated_at: now() });
	await replaySkcTeamNotifications(teamName, cwd, env);
	await computeLifecycleNudges(config, dir, cwd, env);
	return readSkcTeamSnapshot(teamName, cwd, env);
}
export async function listSkcTeams(
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamSnapshot[]> {
	const root = resolveSkcTeamStateRoot(cwd, env);
	try {
		...
		const snapshots = await Promise.all(
			entries
				...
				.map(entry => readSkcTeamSnapshot(entry.name, cwd, env).catch(() => null)),
		);
		return snapshots.filter((snapshot): snapshot is SkcTeamSnapshot => snapshot != null);
	} catch (error) {
		if (isEnoent(error)) return [];
		throw error;
	}
...
async function writeLifecycleNudge(
	...
	const cooldownMs = parseDurationEnv(env, "SKC_TEAM_NUDGE_COOLDOWN_MS", 30_000);
...
async function computeLifecycleNudges(
	...
	const startupGraceMs = parseDurationEnv(env, "SKC_TEAM_STARTUP_GRACE_MS", 30_000);
	const heartbeatStaleMs = parseDurationEnv(env, "SKC_TEAM_HEARTBEAT_STALE_MS", 120_000);
	...
	for (const worker of config.workers) {
		...
		const heartbeat = await readSkcWorkerHeartbeat(config.team_name, worker.id, config.leader.cwd, {
			...env,
			SKC_TEAM_STATE_ROOT: config.state_root,
		});
		const heartbeatAt = Date.parse(heartbeat?.last_turn_at ?? worker.last_heartbeat);
...
export async function shutdownSkcTeam(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamSnapshot> {
	...
	const pendingIntegration = completionVerified ? await hasPendingSkcTeamIntegration(dir, config, monitor) : false;
	killWorkerPanes(config);
	...
	const shutdownPhase: SkcTeamPhase =
	...
	await appendTelemetry(dir, {
		type: "team_shutdown",
		message: `Native skc team runtime stopped with phase ${shutdownPhase}`,
		data: { shutdown_request_id: shutdownRequestId, graceful_shutdown_complete: gracefulShutdownComplete },
	});
	return readSkcTeamSnapshot(config.team_name, cwd, env);
}

export async function listSkcTeamTasks(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamTask[]> {
...
export async function readSkcTeamTask(
	teamName: string,
	taskId: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamTask> {
	const task = (await listSkcTeamTasks(teamName, cwd, env)).find(candidate => candidate.id === taskId);
	if (!task) throw new Error(`task_not_found:${taskId}`);
	return task;
}
export async function createSkcTeamTask(
	teamName: string,
	subject: string,
	description: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	taskOptions: SkcTeamTaskMetadataInput = {},
): Promise<SkcTeamTask> {
	const dir = await findTeamDir(teamName, cwd, env);
	const config = await readConfig(dir);
...
export async function updateSkcTeamTask(
	teamName: string,
	taskId: string,
	updates: Partial<
		Pick<
	...
	const dir = await findTeamDir(teamName, cwd, env);
	const task = await readSkcTeamTask(teamName, taskId, cwd, env);
...
export async function claimSkcTeamTask(
	teamName: string,
	workerId: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	taskId?: string,
): Promise<SkcTeamApiClaimResult> {
	const dir = await findTeamDir(teamName, cwd, env);
	const config = await readConfig(dir);
	const teamWorker = findKnownWorker(config, workerId);
	const livenessRecovery = await reconcileSkcTeamStaleClaims(teamName, dir, config, env);
	...
	const task = taskId
		...
		: tasks.find(candidate => getSkcTeamTaskClaimEligibilityReason(candidate, teamWorker, tasks) == null);
	if (!task) return { ok: false, reason: taskId ? `task_not_found:${taskId}` : "no_pending_task" };
	const eligibilityReason = getSkcTeamTaskClaimEligibilityReason(task, teamWorker, tasks);
	...
	const current = await readSkcTeamTask(teamName, task.id, cwd, env);
	const currentEligibilityReason = getSkcTeamTaskClaimEligibilityReason(current, teamWorker, await readTasks(dir));
	...
	if (current.status !== "pending") {
		await deleteIfOwned(claimPath, {
			...
			predicate: current => (current as SkcTeamTaskClaim).token === token,
		});
		return { ok: false, reason: `task_not_pending:${task.id}` };
	}
	const updated: SkcTeamTask = {
		...current,
		status: "in_progress",
		assignee: workerId,
		owner: workerId,
...
export async function transitionSkcTeamTaskStatus(
	teamName: string,
	taskId: string,
	status: SkcTeamTaskStatus,
	cwd = process.cwd(),
	...
	const completionEvidence =
		status === "completed"
			? normalizeSkcTeamTaskCompletionEvidence(taskId, task.claim.owner, completionEvidenceInput, transitionedAt)
			: undefined;
	const updated: SkcTeamTask = {
		...task,
		status,
	...
	if (completionEvidence) {
		eventData.completion_evidence = {
			...
			verified_item_count: completionEvidence.items.filter(isSkcTeamCompletionEvidenceItemVerified).length,
...
export async function transitionSkcTeamTask(
	teamName: string,
	taskId: string,
	status: SkcTeamTaskStatus | "complete",
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	claimToken?: string,
	completionEvidenceInput?: unknown,
): Promise<SkcTeamTask> {
	return transitionSkcTeamTaskStatus(
		teamName,
		taskId,
		parseSkcTeamTaskStatus(status, true),
		cwd,
		env,
		claimToken,
		undefined,
...
export async function releaseSkcTeamTaskClaim(
	teamName: string,
	taskId: string,
	claimToken: string,
	workerId: string,
	...
	await deleteIfOwned(claimPath, {
		...
		predicate: current => (current as SkcTeamTaskClaim).token === claimToken,
	});
	await appendEvent(dir, {
...
function emptyNotificationSummary(): SkcTeamNotificationSummary {
	return {
		total: 0,
		replay_eligible: 0,
		by_state: {
...
function isReplayEligibleNotification(state: SkcTeamNotificationDeliveryState): boolean {
	return state === "pending" || state === "queued" || state === "deferred" || state === "failed";
}
function summarizeNotifications(notifications: SkcTeamNotification[]): SkcTeamNotificationSummary {
...
async function listNotificationRecords(dir: string): Promise<SkcTeamNotification[]> {
	...
	try {
		...
		const records = await Promise.all(
			entries
				...
				.map(entry => readJsonFile<SkcTeamNotification>(path.join(notificationsDir, entry.name))),
		);
		return records
			.filter((record): record is SkcTeamNotification => record != null)
			.sort((a, b) => a.id.localeCompare(b.id));
	} catch (error) {
		if (isEnoent(error)) return [];
		throw error;
...
async function readNotificationRecord(dir: string, notificationId: string): Promise<SkcTeamNotification> {
	assertSafeId("notification_id", notificationId);
	const notification = await readJsonFile<SkcTeamNotification>(notificationPath(dir, notificationId));
	if (!notification) throw new Error(`notification_not_found:${notificationId}`);
	return notification;
}
function mergeNotificationState(
	current: SkcTeamNotificationDeliveryState,
	next: SkcTeamNotificationDeliveryState,
): SkcTeamNotificationDeliveryState {
	const rank: Record<SkcTeamNotificationDeliveryState, number> = {
		pending: 0,
		queued: 1,
		deferred: 1,
		failed: 1,
...
async function writeNotificationRecord(dir: string, notification: SkcTeamNotification): Promise<SkcTeamNotification> {
	const existing = await readJsonFile<SkcTeamNotification>(notificationPath(dir, notification.id));
	const merged: SkcTeamNotification = existing
		? {
				...existing,
				...notification,
...
async function readLegacyMailbox(dir: string, worker: string): Promise<{ messages: SkcTeamMailboxMessage[] }> {
	return (await readJsonFile<{ messages: SkcTeamMailboxMessage[] }>(mailboxPath(dir, worker))) ?? { messages: [] };
}
async function readMailbox(dir: string, worker: string): Promise<{ messages: SkcTeamMailboxMessage[] }> {
	assertSafeId("worker_id", worker);
	const byId = new Map<string, SkcTeamMailboxMessage>();
	...
	try {
		...
		const records = await Promise.all(
			entries
				.filter(entry => entry.isFile() && entry.name.endsWith(".json"))
				.map(entry => readJsonFile<SkcTeamMailboxMessage>(path.join(mailboxDirPath(dir, worker), entry.name))),
		);
...
async function writeMailboxMessage(
	...
	const existing = await readJsonFile<SkcTeamMailboxMessage>(filePath);
	if (existing) {
		if (
...
async function reconcileTeamNotifications(dir: string, config: SkcTeamConfig): Promise<SkcTeamNotificationSummary> {
...
export async function replaySkcTeamNotifications(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<{ notifications: SkcTeamNotification[]; summary: SkcTeamNotificationSummary }> {
	const dir = await findTeamDir(teamName, cwd, env);
	const config = await readConfig(dir);
	await reconcileTeamNotifications(dir, config);
	const next: SkcTeamNotification[] = [];
...
export async function sendSkcTeamMessage(
	teamName: string,
	fromWorker: string,
	toWorker: string,
	body: string,
...
export async function broadcastSkcTeamMessage(
	teamName: string,
	fromWorker: string,
	body: string,
	cwd = process.cwd(),
...
export async function listSkcTeamMailbox(
	teamName: string,
	worker: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
...
export async function markSkcTeamMailboxMessage(
	teamName: string,
	worker: string,
	messageId: string,
	...
	const existing =
		(await readJsonFile<SkcTeamNotification>(notificationPath(dir, notificationId))) ??
		(await createMessageNotification(dir, config.team_name, written));
	const nextState: SkcTeamNotificationDeliveryState = field === "delivered_at" ? "acknowledged" : "delivered";
...
export async function writeSkcWorkerInbox(
	teamName: string,
	worker: string,
	content: string,
	cwd = process.cwd(),
...
export async function writeSkcWorkerIdentity(
	teamName: string,
	worker: SkcTeamWorker,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
...
export async function readSkcTeamEvents(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamEvent[]> {
	const dir = await findTeamDir(teamName, cwd, env);
	try {
		...
		return text
			.split(/\r?\n/)
			.filter(Boolean)
			.map(line => JSON.parse(line) as SkcTeamEvent);
	} catch (error) {
		if (isEnoent(error)) return [];
		throw error;
	}
}
export async function readSkcTeamTraces(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<SkcTeamTraceEvent[]> {
	const dir = await findTeamDir(teamName, cwd, env);
	try {
		...
		return text
			.split(/\r?\n/)
			.filter(Boolean)
			.map(line => JSON.parse(line) as SkcTeamTraceEvent);
	} catch (error) {
		if (isEnoent(error)) return [];
		throw error;
	}
}
export async function appendSkcTeamEvent(
	teamName: string,
	type: string,
	worker = "leader-fixed",
	cwd = process.cwd(),
...
export async function awaitSkcTeamEvent(
	teamName: string,
	_timeoutMs = 0,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<{ status: "event" | "timeout"; event?: SkcTeamEvent }> {
	const events = await readSkcTeamEvents(teamName, cwd, env);
	const event = events.at(-1);
	return event ? { status: "event", event } : { status: "timeout" };
}
export async function writeSkcMonitorSnapshot(
	teamName: string,
	snapshot: unknown,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
...
export async function readSkcMonitorSnapshot(
	teamName: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
): Promise<unknown> {
...
export async function writeSkcTaskApproval(
	teamName: string,
	taskId: string,
...
export async function readSkcTaskApproval(
	teamName: string,
	taskId: string,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
...
export async function executeSkcTeamApiOperation(
	operation: string,
	input: Record<string, unknown>,
	cwd = process.cwd(),
	env: NodeJS.ProcessEnv = process.env,
	...
	switch (operation) {
		case "list-tasks":
			return { tasks: await listSkcTeamTasks(teamName, cwd, env) };
		case "read-task":
			return { task: await readSkcTeamTask(teamName, String(input.task_id ?? input.taskId), cwd, env) };
		case "create-task": {
			const task = await createSkcTeamTask(
				teamName,
		...
		case "update-task": {
			const task = await updateSkcTeamTask(
				teamName,
		...
		case "claim-task": {
			...
			const result = await claimSkcTeamTask(
				teamName,
				worker,
				cwd,
				env,
		...
		case "transition-task-status": {
			const task = await transitionSkcTeamTaskStatus(
				teamName,
				String(input.task_id ?? input.taskId),
				parseSkcTeamTaskStatus(input.to ?? input.status),
				cwd,
				env,
		...
		case "release-task-claim": {
			const task = await releaseSkcTeamTaskClaim(
				teamName,
		...
		case "send-message": {
			const message = await sendSkcTeamMessage(
				teamName,
		...
		case "broadcast": {
			const messages = await broadcastSkcTeamMessage(
				teamName,
				String(input.from_worker),
				String(input.body),
				cwd,
		...
		case "mailbox-list":
			return { messages: await listSkcTeamMailbox(teamName, worker, cwd, env) };
		case "mailbox-mark-delivered": {
			const message = await markSkcTeamMailboxMessage(
				teamName,
				worker,
		...
		case "notification-replay":
			return notificationSummaryReceipt(teamName, await replaySkcTeamNotifications(teamName, cwd, env));
		...
		case "worker-startup-ack":
			return writeSkcWorkerStartupAck(teamName, worker, cwd, env, input);
		case "read-config":
		...
		case "read-worker-status":
			return readSkcWorkerStatus(teamName, worker, cwd, env);
		case "update-worker-status": {
			const currentTaskIdInput = input.current_task_id ?? input.currentTaskId;
			return updateSkcWorkerStatus(
				teamName,
				worker,
				parseRequiredSkcWorkerStatusState(input.status ?? input.state),
				cwd,
				env,
		...
		case "read-worker-heartbeat":
			return readSkcWorkerHeartbeat(teamName, worker, cwd, env);
		case "recover-stale-claims":
			return recoverSkcTeamStaleClaims(teamName, cwd, env);
		case "update-worker-heartbeat":
			return updateSkcWorkerHeartbeat(
				teamName,
				worker,
				{
		...
		case "write-worker-inbox":
			return writeSkcWorkerInbox(teamName, worker, String(input.content ?? ""), cwd, env);
		case "write-worker-identity":
			return writeSkcWorkerIdentity(
				teamName,
				{
					id: worker,
					name: worker,
		...
		case "append-event":
			return appendSkcTeamEvent(teamName, String(input.type ?? "event"), worker, cwd, env);
		case "read-events":
			return { events: await readSkcTeamEvents(teamName, cwd, env) };
		case "read-traces":
			return { traces: await readSkcTeamTraces(teamName, cwd, env) };
		case "await-event":
			return awaitSkcTeamEvent(teamName, Number(input.timeout_ms ?? 0), cwd, env);
		case "write-monitor-snapshot":
			return writeSkcMonitorSnapshot(teamName, input.snapshot ?? {}, cwd, env);
		case "read-monitor-snapshot":
			return readSkcMonitorSnapshot(teamName, cwd, env);
		case "write-task-approval":
			return writeSkcTaskApproval(teamName, String(input.task_id), input, cwd, env);
		case "read-task-approval":
			return readSkcTaskApproval(teamName, String(input.task_id), cwd, env);
		case "write-shutdown-request": {
			const shutdownRequestIdInput = input.request_id ?? input.requestId;
			return writeSkcShutdownRequest(
				teamName,
				worker,
				...
				parseSkcTeamShutdownMode(input.mode),
			);
		}
		case "read-shutdown-ack":
			return readSkcShutdownAck(teamName, worker, cwd, env);
		default:
			throw new Error(`unknown_team_api_operation:${operation}`);
	}
}

export function parseTeamLaunchArgs(argv: string[]): SkcTeamStartOptions {
	...
	let workerCount = SKC_TEAM_DEFAULT_WORKERS;
	...
	if (!Number.isInteger(workerCount) || workerCount < 1 || workerCount > SKC_TEAM_MAX_WORKERS)
		throw new Error(`invalid_team_worker_count:${workerCount}:expected_1_${SKC_TEAM_MAX_WORKERS}`);
