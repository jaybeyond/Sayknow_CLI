...
import { type Api, DEFAULT_MODEL_PER_PROVIDER, type KnownProvider, type Model, modelsAreEqual } from "@sayknow-cli/ai";
...
export async function resolveModelOverrideWithAuthFallback(
	...
	if (canonicalScope && parentActiveModelPattern) {
		...
		// TODO(upstream-v0.11): ModelRegistry.seedCanonicalVariant was removed;
		// rewire canonical-variant seeding if a successor API appears.
		void parentActiveModel;
	}
	for (const pattern of modelPatterns) {
