...
Runs only when ROBSKC_INTEGRATION=1 and `skc` is available on PATH (or via
ROBSKC_SKC_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `skc --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("ROBSKC_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBSKC_INTEGRATION=1 required to run the skc-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from roboskc.sandbox import LocalGitTransport, SandboxManager
    from roboskc.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBSKC_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBSKC_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBSKC_BOT_LOGIN", "roboskc-bot")
    monkeypatch.setenv("ROBSKC_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBSKC_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBSKC_SQLITE_PATH", str(tmp_path / "roboskc.sqlite"))
    monkeypatch.setenv("ROBSKC_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBSKC_TASK_TIMEOUT_SECONDS", "300")
