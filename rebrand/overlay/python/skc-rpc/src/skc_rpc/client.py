...
class RpcClient:
    ...
    def start(self) -> RpcClient:
        ...
        self._stdout_thread = threading.Thread(target=self._read_stdout_loop, name="skc-rpc-stdout", daemon=True)
        self._stderr_thread = threading.Thread(target=self._read_stderr_loop, name="skc-rpc-stderr", daemon=True)
    ...
    def run_workflow_gate_policy(
        ...
        def handle(gate: WorkflowGate) -> None:
            threading.Thread(
                ...
                name=f"skc-rpc-workflow-gate-{gate.gate_id}",
                daemon=True,
            ).start()
    ...
    def list_sessions(sessions_dir: str | Path | None = None) -> tuple[SessionHandle, ...]:
        """Discover live skc RPC sessions from the cross-process registry (issue 10)."""
    ...
    def _handle_host_tool_call(self, payload: JsonObject) -> None:
        ...
        threading.Thread(target=run_tool, name=f"skc-rpc-host-tool:{tool_name}", daemon=True).start()
    ...
    def _handle_host_uri_request(self, payload: JsonObject) -> None:
        ...
        threading.Thread(target=run, name=f"skc-rpc-host-uri:{scheme}:{operation}", daemon=True).start()
