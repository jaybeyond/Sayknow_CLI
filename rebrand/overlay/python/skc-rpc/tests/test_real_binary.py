"""Real-binary integration lane for skc_rpc (issue 08).
...
method). This lane drives the real `skc --mode rpc` binary over the no-model
control surface so the typed client is checked against the actual protocol.

Opt-in: set ``SKC_RPC_REAL_BINARY=1`` and have ``bun`` on PATH. It is skipped
...
from skc_rpc import RpcClient, RpcCommandError, UnattendedBudget

_REPO_ROOT = Path(__file__).resolve().parents[3]
_CLI = _REPO_ROOT / "packages" / "coding-agent" / "src" / "cli.ts"
_ENABLED = (
    os.environ.get("SKC_RPC_REAL_BINARY") == "1"
    and shutil.which("bun") is not None
    and _CLI.is_file()
)


@unittest.skipUnless(_ENABLED, "set SKC_RPC_REAL_BINARY=1 with bun on PATH to run the real-binary lane")
class RealBinaryRpcTest(unittest.TestCase):
    def _client(self, tmp: str) -> RpcClient:
        ...
        env["SKC_CODING_AGENT_DIR"] = os.path.join(tmp, ".agent")
        return RpcClient(
            command=[
                "bun",
                str(_CLI),
